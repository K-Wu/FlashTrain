# Adapted from https://github.com/pytorch/PiPPy/blob/7f4c69d85b05d1b653123fd1e3571a5a66d5ed66/examples/huggingface/hf_utils.py
...
def get_sequence_length(model_cls, model_name):
    if model_name.startswith(("Blenderbot",)):
        ...
        log.info(
            f"Sequence Length not defined for {model_name}. Choosing 128"
            " arbitrarily"
        )
        seq_length = 128
    return seq_length
...
def generate_inputs_for_model(
    ...
    if model_name.startswith("Wav2Vec2"):
        ...
        return {
            ...
            "labels": rand_int_tensor(
                device, 0, vocab_size, (bs, target_length)
            ),
        }

    if model_name.endswith("MultipleChoice"):
        input = rand_int_tensor(
    ...
    if include_loss_args:
        if model_name.endswith("PreTraining"):
            ...
            input_dict["end_positions"] = rand_int_tensor(
                device, 0, seq_length, (bs,)
            )
        elif (

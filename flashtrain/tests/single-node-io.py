...
def run_cufile_multiple_files(args):
    ...
    t0 = clock()
    futures = [
        f.pwrite(data[i * chunksize : (i + 1) * chunksize])
    ...
    t0 = clock()
    futures = [
        f.pread(data[i * chunksize : (i + 1) * chunksize])
        for i, f in enumerate(files)
    ]
    res = sum(f.get() for f in futures)
    del files
...
def run_posix(args):
    ...
    assert (
        res == args.nbytes
    ), f"IO mismatch, expected {args.nbytes} got {a.nbytes}"
...
def run_zarr(args):
    ...
    if not kvikio.zarr.supported:
        raise RuntimeError(
            f"requires Zarr >={kvikio.zarr.MINIMUM_ZARR_VERSION}"
        )
...
def main(args):
    ...
    kvikio.defaults.task_size_reset(args.task_size)
    ...
    print("----------------------------------")
    print(
        f"nbytes            | {args.nbytes} bytes"
        f" ({format_bytes(args.nbytes)})"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            "Path to the directory to r/w from (default:"
    ...
    parser.add_argument(
        ...
        default=67108864,
        type=int,
        help="Task size for parallel IO operations (default: %(default)s).",
    )
    parser.add_argument(
        "-t",
        "--nthreads",

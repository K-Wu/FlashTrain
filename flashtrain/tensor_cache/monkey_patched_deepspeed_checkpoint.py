"""This is from deepspeed/runtime/activation_checkpointing/checkpointing.py (0.14.2). The only change we made is to use save_for_backward() to save tensors in order to trigger pack_/unpack_hook.
This is effectively reverting the fix https://github.com/microsoft/DeepSpeed/commit/51d42ab9ec826449c39d052669ca33a867c20cb5, which fixes partition activations issue when mp=2 and pp=2.
However, if only mp=2 or pp=2 when partition activation is enabled, there is no such issue and the monkey patch code should still work https://github.com/bigscience-workshop/Megatron-DeepSpeed/pull/184."""
import torch
...
# From: https://stackoverflow.com/a/45466846/5555077 and https://github.com/GrahamDumpleton/wrapt/blob/develop/blog/11-safely-applying-monkey-patches-in-python.md#delayed-patching-is-bad
# The monkey patch logic should be deepspeed.runtime.activation_checkpointing.checkpointing.CheckpointFunction = CheckpointFunction

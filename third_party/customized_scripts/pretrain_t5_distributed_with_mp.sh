...
T5_ARGS="--tensor-cache-log-level ${TC_LOGGING_LEVEL}"
if [ "${USE_TENSOR_CACHE}" = "true" ]; then
  T5_ARGS="${T5_ARGS} --enable-tensor-cache --cufile-malloc-hook-is-used"
elif [ "${USE_TENSOR_CACHE}" = "memory" ]; then
  T5_ARGS="${T5_ARGS} --enable-tensor-cache --cufile-malloc-hook-is-used --tensor-cache-in-memory-adapter"
fi

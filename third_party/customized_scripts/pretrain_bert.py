# Adapted from https://github.com/K-Wu/Megatron-DeepSpeed/blob/b72381ec437856b20c6150b44da2c26b9b8e7739/pretrain_bert.py (forked from github.com/microsoft/Megatron-DeepSpeed/) and added tensor cache logic
...
def data_post_process(data, data_sampler_state_dict):
    ...
    if args.data_efficiency_curriculum_learning:
        if (
            "seqlen_truncate"
            in data_sampler_state_dict["current_difficulties"]
        ):
            effective_seqlen = data_sampler_state_dict["current_difficulties"][
                "seqlen_truncate"
            ]
        else:
            effective_seqlen = torch.count_nonzero(data["padding_mask"], dim=1)
            effective_seqlen = torch.max(effective_seqlen).to(
        ...
        if effective_seqlen < args.seq_length:
            ...
            data["loss_mask"] = data["loss_mask"][
                :, :effective_seqlen
            ].contiguous()
            data["labels"] = data["labels"][:, :effective_seqlen].contiguous()
            data["padding_mask"] = data["padding_mask"][
                :, :effective_seqlen
            ].contiguous()
    return data

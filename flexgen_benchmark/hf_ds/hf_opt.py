...
from flexgen.utils import GB, project_decode_latency, write_benchmark_log
from flexgen.opt_config import (
    get_opt_config,
    disable_torch_init,
    disable_hf_opt_init,
)
    ...
    for k in keys:
        ...
        if v is not None:
            module._parameters[k] = torch.nn.Parameter(
                torch.empty(
                    *v.shape, dtype=dtype or v.dtype, device=device or v.device
                )
            )
...
def get_ds_opt_model(
    model_name, dtype, cpu_offload, disk_offload, offload_dir, dummy_weights
):
    import deepspeed
...
def run_generation(
    ...
    with torch.no_grad():
        output_ids = model.generate(
            input_ids=input_ids, **generate_kwargs_warmup
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--offload-dir", type=str, default="~/flexgen_offload_dir"
    )
    ...
    assert not (
        args.no_log and (args.output_file != "auto" or args.pkl_file != "auto")
    )

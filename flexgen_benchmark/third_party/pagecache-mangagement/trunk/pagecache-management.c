...
enum fd_state {
  ...
  FDS_LAZY =
      3, /* We were asked to close this file, but we are keeping it open */
};
...
static int lazy_last() { return lazy_i(lazy_fds - 1); }
#endif
...
static void grown_pagecache(int fd, size_t count,
                            unsigned long *pagecache_size) {
  if ((*pagecache_size) > pagecache_max_bytes) {
    off_t off = lseek(fd, 0, SEEK_CUR);
    if (off > pagesize)
      posix_fadvise(fd, 0, off - pagesize, POSIX_FADV_DONTNEED);
    (*pagecache_size) = 0;
  }
}
...
static void write_was_called(int fd, size_t count) {
  ...
  if (pagecache_max_bytes == 0) return;

#ifdef LAZY_CLOSE_FDS

  if (max_lazy_fds) lazy_purge();
#endif

  fds = get_fd_status(fd);
  if (fds->state == FDS_UNKNOWN) inspect_fd(fd, fds);
  if (fds->state == FDS_IGNORE) return;
...
static void parse_env(void) {
  ...
  if (e) {
    ...
    if (e) pagecache_chunk_size = strtoul(e, NULL, 10);
  }
#ifdef LAZY_CLOSE_FDS
...
static inline void load_symbols(void) {
  if (!symbols_loaded) __load_symbols();
}
...
static void lazy_purge() {
  ...
  if (lazy_close_timeout) {
    ...
    while ((pagecache_size_write > pagecache_max_bytes ||
            ((get_fd_status(lazy[lazy_first])->seconds - t >
              lazy_close_timeout))) &&
           lazy_fds > 0) {
      FDEBUGF(stderr, "%ld > %ld #lazy_fds: %d\n", pagecache_size_write,
              pagecache_max_bytes, lazy_fds);
      lazy_purge_first();
    }
  } else {

...
#pragma unroll
    for (int j = 0; j < ln::internal_unroll; j++) {
      ...
      mem_access::load_global<ln::granularity>(
          iteration_buffer + j * T_per_load, input_base + iteration * stride,
...
#define LAUNCH_FUSED_LN(unroll_factor)                    \
  fused_ln<T, unroll_factor><<<grid, block, 0, stream>>>( \
...
void launch_fused_ln(T* output, const T* vals, const T* gamma, const T* beta,
  ...
  int32_t unroll = 1;
  while (T_per_warp_unroll * ln::max_warps * unroll < elems_per_row) {
    unroll++;
  }

  const int sched_warps = (elems_per_row + unroll * T_per_warp_unroll - 1) /
                          (unroll * T_per_warp_unroll);
...
template void launch_fused_ln(float*, const float*, const float*, const float*,
...
#define LAUNCH_FUSED_RES_LN(unroll_factor)                                \
  fused_residual_ln<T, unroll_factor, false>                              \
...
void launch_fused_residual_ln(T* output, const T* vals, const T* residual,
  ...
  const int warps = (elems_per_row + unroll * T_per_warp_unroll - 1) /
                    (unroll * T_per_warp_unroll);
...
#define LAUNCH_FUSED_RES_LN_STORE(unroll_factor)                            \
  fused_residual_ln<T, unroll_factor, true>                                 \
...
template void launch_fused_residual_ln_store(__half*, __half*, const __half*,
                                             const __half*, const __half*,
                                             const __half*, const __half*,
                                             float, int, int, cudaStream_t);

template void launch_fused_residual_ln_store(float*, float*, const float*,

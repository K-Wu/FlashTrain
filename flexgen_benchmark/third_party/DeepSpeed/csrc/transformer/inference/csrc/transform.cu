...
__global__ void bias_add_transform_0213(__half* output,  // q
                                        ...
                                        int seq_length, unsigned seq_offset,
                                        int all_tokens, int heads,
                                        int rotary_dim, bool rotate_half,
                                        bool rotate_every_two, int head_ext,
                                        int max_out_tokens) {
...
#pragma unroll
      for (int o = 0; o < 4; o++) {
        ...
        q_h[o].x = (__half)(-1.0 * q_data[1] * sinf(inv_freq) +
                            q_data[0] * cosf(inv_freq));
        q_h[o].y =
            (__half)(q_data[0] * sinf(inv_freq) + q_data[1] * cosf(inv_freq));
      }
    }
    output_vec[d3] = q;
  } else
...
void launch_bias_add_transform_0213(T* outputs, T* vals, T* vals1,
                                    ...
                                    int max_out_tokens);
template <>
void launch_bias_add_transform_0213<__half>(
...
__global__ void pad_add_transform_0213(__half* output, const __half* vals,
                                       int hidden_dim, int seq_length,
                                       int padded_seq_len, int heads,
                                       int padded_head_size) {
  float4 ZERO;
...
void launch_pad_add_transform_0213<float>(float* output, const float* vals,
                                          ...
                                          cudaStream_t stream) {}
template <>
void launch_pad_add_transform_0213<__half>(__half* output, const __half* vals,
                                           int batch_size, int hidden_dim,
                                           int seq_length, int padded_seq_len,
                                           int heads, int padded_head_size,
                                           cudaStream_t stream) {

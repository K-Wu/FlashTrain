...
__global__ void attn_softmax_v2(__half* vals, __half* mask, __half* alibi,
                                float layer_scale, bool triangular,
                                bool recompute, bool local_attention,
                                int window_size, int total_count, int heads,
                                int sequence_length, int num_seq,
                                int head_offset, int mask_stride, int mp_size,
                                int iterations, int reduceWidth) {
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      if ((!triangular || ((data_id >> 2) <= seq_id4)) &&
        ...
        if ((sequence_length - data_id) >= 4) {
          ...
          if (alibi) {
            low_data[i].x =
                low_data[i].x + __half2float(alibi[data_id + alibi_offset]);
            low_data[i].y =
...
__global__ void attn_softmax_v2(float* vals, float* attn_mask, float* alibi,
                                float layer_scale, bool triangular,
                                bool recompute, bool local_attention,
                                int window_size, int total_count, int heads,
                                int sequence_length, int num_seq,
...
void launch_attn_softmax_v2(T* vals, T* mask, T* alibi, float layer_scale,
  ...
  if (sequence_length <= 32768)
    attn_softmax_v2<<<grid_dim, block_dim, 0, stream>>>(
        ...
        mask_stride, mp_size, iterations, reduce_width);
  else

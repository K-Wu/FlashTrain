...
class Context {
 public:
  ...
  void GenWorkSpace(const unsigned& num_layers, const unsigned& num_heads,
                    const size_t& batch_size, const size_t& prompt_len,
                    const size_t& hidden_dim, const unsigned& mp_size,
                    const bool& external_cache, const size_t& elem_size,
                    const unsigned& rank, unsigned max_out_tokens) {
    size_t total_size;
    ...
    size_t activation_size =
        16 * (num_heads * effective_head_size) * batch_size;
    ...
    if (rank == 0 && !_workspace)
      printf(
          ...
          "------------------------------------------------------\n",
          (float)_free_memory_size / GIGABYTE, (float)total_size / GIGABYTE,
          (float)workSpaceSize / GIGABYTE, _max_seq_len);
    if (!_workspace) {
  ...
  cudaEvent_t GetCompEvent(int id) {

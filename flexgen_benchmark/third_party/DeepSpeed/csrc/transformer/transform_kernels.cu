...
template <typename T>
__global__ void Transpose_Kernel(const T* inp, T* out, int row_width,
                                 int col_width) {
  ...
  int row_stride =
      rows_trans / ((rows_trans * cols_trans + THREADS - 1) / THREADS);
...
template <>
void Transpose<__half>(const __half* inp_mat, __half* out_mat, int rows,
                       int cols, cudaStream_t stream) {
  int threads = THREADS;

  Transpose_Kernel<__half>
      <<<(rows * cols + threads - 1) / threads, threads, 0, stream>>>(
          inp_mat, out_mat, cols, rows);
}

template <>
void Transpose<float>(const float* inp_mat, float* out_mat, int rows, int cols,
                      cudaStream_t stream) {
  int threads = THREADS;
...
template <typename T>
__global__ void transform_0213(T* output, const T* vals, int hidden_dim,
                               int seq_length, int heads, int head_ext);

template <>
__global__ void transform_0213<float>(float* output, const float* vals,
                                      int hidden_dim, int seq_length, int heads,
                                      int head_ext) {
  ...
  int d2 = threadIdx.y +
           (blockIdx.y % head_ext) * (heads / head_ext);  // Head (0-11)
  ...
  float4 inputs =
      vals_vec[d0 * d0_stride + d1 * d1_stride + d2 * d2_stride + d3];
  output_vec[d0 * d0_out_stride + d1 * d1_out_stride + d2 * d2_out_stride +
             d3] = inputs;
}

template <>
__global__ void transform_0213<__half>(__half* output, const __half* vals,
...
template <>
void launch_transform_0213<float>(float* output, const float* vals,
  ...
  transform_0213<float><<<grid_dim, block_dim, 0, stream>>>(
      output, vals, hidden_dim, seq_length, heads, head_ext);
}

template <>
void launch_transform_0213<__half>(__half* output, const __half* vals,
                                   int batch_size, int seq_length,
                                   int hidden_dim, int heads,
                                   cudaStream_t stream) {
  hidden_dim >>= 3;
...
__global__ void bias_add_transform_0213<float>(float* output, const float* vals,
  ...
  int d2 = threadIdx.y +
           (blockIdx.z % head_ext) * (heads / head_ext);  // Head (0-11)
  int d3 = threadIdx.x;  // Values (groups of 4)
...
__global__ void bias_add_transform_0213_v2(__half* output, const __half* vals,
...
// [B S C*H] - > C * [B A S N]
template <>
void launch_bias_add_transform_0213<float>(float* output, const float* vals,
...
template <>
void launch_bias_add_transform_0213<__half>(__half* output, const __half* vals,
...
template <>
__global__ void transform4d_0213<__half>(__half* out, const __half* in,
...
__global__ void transform4d_0213_v2(__half* out, const __half* in, int heads,
                                    int seq_length, int hidden_dim) {
#ifdef HALF_PRECISION_AVAILABLE
  __shared__ float4 in_data[3072];
...
template <>
void launch_transform4d_0213<__half>(__half* out, const __half* in,

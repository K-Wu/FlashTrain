...
at::Tensor ds_quantize(at::Tensor& vals, int groups, int bits) {
  ...
  if ((((size / groups) - 1) / 4096 + 1) <= 256) {
    launch_fake_quantize_kernel((T*)vals.data_ptr(), size, groups, bits,
...
at::Tensor ds_sr_quantize(at::Tensor& vals, int groups, int bits) {
  ...
  if (((size / groups) / 4 / 1024) <= 256) {
    launch_sr_fake_quantize_kernel((T*)vals.data_ptr(), size, groups, bits,
                                   at::cuda::getCurrentCUDAStream());
  }
  return vals;
}
...
at::Tensor ds_quantize_asym(at::Tensor& vals, int groups, int bits) {
  ...
  if ((((size / groups) - 1) / 4096 + 1) <= 256) {
    launch_fake_quantize_kernel_asym((T*)vals.data_ptr(), size, groups, bits,
                                     at::cuda::getCurrentCUDAStream());
  }
  return vals;
}
...
at::Tensor ds_sr_quantize_asym(at::Tensor& vals, int groups, int bits) {
  ...
  if (((size / groups) / 4 / 1024) <= 256) {
    launch_sr_fake_quantize_kernel_asym((T*)vals.data_ptr(), size, groups, bits,
                                        at::cuda::getCurrentCUDAStream());
  }
  return vals;
}
...
#define QUANTIZATION_CASE(TYPE, BITS)                            \
  case TYPE:                                                     \
    launch_quant<BITS, TYPE>(                                    \
        (int8_t*)output.data_ptr(), (float*)params.data_ptr(),   \
        (__half*)input_vals.data_ptr(), groups, elems_per_group, \
        at::cuda::getCurrentCUDAStream());                       \
    break;

std::vector<at::Tensor> quantize_kernel(at::Tensor& input_vals, int groups,
                                        int numBits, quantize::Type quantType) {
  auto dtype = (quantType == quantize::Type::IntegerSymmetric) ? torch::kInt32
                                                               : at::kFloat;
...
at::Tensor dequantize(at::Tensor& quantized_data, at::Tensor& params,
  ...
  launch_dequantize_kernel(
      (__half*)output.data_ptr(), (const int8_t*)quantized_data.data_ptr(),
      (const float*)params.data_ptr(), quant_type, num_bits, elems_per_group,
      total_elems, at::cuda::getCurrentCUDAStream());

  return output;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("ds_quantize_fp32", &ds_quantize<float>,
        "DeepSpeed Quantize with fp32 (CUDA)");
  m.def("ds_quantize_fp16", &ds_quantize<__half>,
        "DeepSpeed Quantize with fp16 (CUDA)");
  m.def("ds_sr_quantize_fp32", &ds_sr_quantize<float>,
        "DeepSpeed Quantize with fp32 (CUDA)");
  m.def("ds_sr_quantize_fp16", &ds_sr_quantize<__half>,
        "DeepSpeed Quantize with fp16 (CUDA)");
  m.def("ds_quantize_asym_fp32", &ds_quantize_asym<float>,
        "DeepSpeed Quantize with fp32 (CUDA)");
  m.def("ds_quantize_asym_fp16", &ds_quantize_asym<__half>,
        "DeepSpeed Quantize with fp16 (CUDA)");
  m.def("ds_sr_quantize_asym_fp32", &ds_sr_quantize_asym<float>,
        "DeepSpeed Quantize with fp32 (CUDA)");
  m.def("ds_sr_quantize_asym_fp16", &ds_sr_quantize_asym<__half>,
        "DeepSpeed Quantize with fp16 (CUDA)");

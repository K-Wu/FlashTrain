...
int32_t round_to_32(int32_t raw_value) {
  return (((raw_value - 1) >> 5) + 1) << 5;
}

#define LAUNCH_CACHED_QUANT(q_bits, quant_type, unroll_factor,               \
                            internal_unroll, threads_per_group, max_threads) \
  cached_quantization<q_bits, quant_type, unroll_factor, internal_unroll,    \
                      threads_per_group, max_threads>                        \
...
void launch_quant(int8_t* output_data, float* params, const __half* input_data,
  ...
  if (is_subblock_schedule) {
    ...
    LAUNCH_CACHED_QUANT(numBits, qType, 1, internal_unroll, max_threads,
                        max_threads);
  } else if (external_unroll == 2) {
    // 4097 - 8192 elems
    LAUNCH_CACHED_QUANT(numBits, qType, 2, internal_unroll, max_threads,
                        max_threads);
  } else if (external_unroll == 3) {
    // 8193 - 12288 elems
    LAUNCH_CACHED_QUANT(numBits, qType, 3, internal_unroll, max_threads,
                        max_threads);
  } else if (external_unroll == 4) {
    // 12289 - 16384 elems
    LAUNCH_CACHED_QUANT(numBits, qType, 4, internal_unroll, max_threads,
                        max_threads);
  }
}

template void launch_quant<8, quantize::Type::Symmetric>(int8_t*, float*,
                                                         const __half*, int,
                                                         int, cudaStream_t);

template void launch_quant<8, quantize::Type::Asymmetric>(int8_t*, float*,
                                                          const __half*, int,
                                                          int, cudaStream_t);

template void launch_quant<8, quantize::Type::IntegerSymmetric>(int8_t*, float*,
                                                                const __half*,
                                                                int, int,
                                                                cudaStream_t);

template void launch_quant<4, quantize::Type::Symmetric>(int8_t*, float*,
                                                         const __half*, int,
                                                         int, cudaStream_t);

template void launch_quant<4, quantize::Type::Asymmetric>(int8_t*, float*,
                                                          const __half*, int,
                                                          int, cudaStream_t);

template void launch_quant<4, quantize::Type::IntegerSymmetric>(int8_t*, float*,
                                                                const __half*,
                                                                int, int,
                                                                cudaStream_t);

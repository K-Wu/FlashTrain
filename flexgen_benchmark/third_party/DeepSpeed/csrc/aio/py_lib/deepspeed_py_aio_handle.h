...
struct deepspeed_aio_handle_t {
  ...
  deepspeed_aio_handle_t(const int block_size, const int queue_depth,
  ...
  int write(const torch::Tensor& buffer, const char* filename,
            const bool validate);

  int pread(const torch::Tensor& buffer, const char* filename,
            const bool validate, const bool async);

  int pwrite(const torch::Tensor& buffer, const char* filename,
             const bool validate, const bool async);
  ...
  bool _is_valid_parallel_aio_op(const bool read_op,

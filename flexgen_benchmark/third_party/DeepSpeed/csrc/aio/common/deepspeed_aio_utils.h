...
struct io_xfer_ctxt {
  ...
  io_xfer_ctxt(const int fd, const long long int file_offset,
...
struct io_prep_context {
  ...
  void prep_iocbs(const int n_iocbs, const size_t num_bytes,
                  const void* start_buffer, const long long int start_offset);
};

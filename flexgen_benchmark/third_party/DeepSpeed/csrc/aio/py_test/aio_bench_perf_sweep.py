...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Directory in which to perform I/O tests. A writeable directory on"
            " a NVMe device."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Run without sudo access. Page cache will not be flushed and"
            " reported read speeds may be higher than actual."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f" {os.path.join('.', BENCH_LOG_DIR)}"
        ),
    )

    parser.add_argument(
        "--loops", type=int, default=1, help="Count of operation repetitions"
    )
...
def get_sweep_cmd_lines(sweep_config_dict):
    ...
    flat_list = [
        flatten_options(key, value) for key, value in sweep_config_dict.items()
    ]
...
def create_read_file(sweep_config):
    ...
    read_file_name = os.path.join(
        read_folder, f"random_{sweep_config.io_size}B.pt"
    )
    ...
    dd_job = Job(
        cmd_line=[
            "dd if=/dev/urandom"
            f" of={read_file_name} bs={block_size} count={block_count}"
        ]
    )
    print(
        f"[Start] Create read file of {sweep_config.io_size} bytes by running"
        f" {dd_job.cmd()} ...."
    )
    run_job(dd_job)
    print(
        f"[Done] Create read file of {sweep_config.io_size} bytes by running"
        f" {dd_job.cmd()} ...."
    )
...
def remove_folder(folder):
    assert os.path.isdir(
        folder
    ), f"Error: cannot remove {folder} - folder not found"
...
def run_read_sweep(sweep_config, flush_cache_job, sync_job, cmd_lines):
    ...
    read_cmd_lines = [
        [f"{read_option} {sweep_config.other_options}"] + cmd
        for cmd in cmd_lines
    ]
    ...
    perf_jobs = create_perf_jobs(
        io_op_desc=READ_OP_DESC, log_dir=log_folder, cmd_lines=read_cmd_lines
    )

    launch_sweep(
        sweep_jobs=perf_jobs,
        sync_job=sync_job,
...
def run_write_sweep(sweep_config, flush_cache_job, sync_job, cmd_lines):
    ...
    write_file_name = os.path.join(
        write_folder, f"random_{sweep_config.io_size}B.pt"
    )
    write_option = (
        f"--write_size {sweep_config.io_size} --write_file {write_file_name}"
    )
    write_cmd_lines = [
        [f"{write_option} {sweep_config.other_options}"] + cmd
        for cmd in cmd_lines
    ]
    ...
    perf_jobs = create_perf_jobs(
        io_op_desc=WRITE_OP_DESC, log_dir=log_folder, cmd_lines=write_cmd_lines
    )
...
def main():
    ...
    if sweep_config.flush_cache:
        flush_cache_job = Job(
            cmd_line=["sudo", "bash -c", "'echo 1 > /proc/sys/vm/drop_caches'"]
        )
    else:

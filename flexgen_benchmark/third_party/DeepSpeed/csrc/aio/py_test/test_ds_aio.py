...
def parse_arguments():
    ...
    parser.add_argument(
        "--read_file", type=str, default=None, help="Read file."
    )

    parser.add_argument(
        "--write_file", type=str, default=None, help="Write file."
    )
    ...
    parser.add_argument(
        "--block_size", type=str, default="1M", help="I/O block size."
    )

    parser.add_argument(
        "--queue_depth", type=int, default=32, help="I/O queue depth."
    )

    parser.add_argument(
        "--threads", type=int, default=1, help="Thread parallelism count."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Submit I/O requests in singles (default is submit queue_depth"
            " amount at once.)."
        ),
    )
    ...
    parser.add_argument(
        "--handle", action="store_true", help="Use AIO handle."
    )
    ...
    parser.add_argument(
        "--io_parallel", type=int, default=None, help="Per iop parallelism"
    )
...
def main():
    ...
    multiprocess_function = (
        ...
        if args.handle
        else aio_basic_multiprocessing
    )
    if args.read_file:

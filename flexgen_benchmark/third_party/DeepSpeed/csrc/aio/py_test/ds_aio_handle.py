...
def pre_handle(args, tid, read_op):
    ...
    if args.gpu:
        ...
        buffer = torch.empty(
            num_bytes, dtype=torch.uint8, device="cpu"
        ).pin_memory()
    task_log(
        tid,
        (
            f"{io_string} file {file} of size {num_bytes} bytes from buffer on"
...
def get_schedule(args, read_op):
    ...
    if read_op:
        ...
        schedule["main"] = (
            main_parallel_read if args.io_parallel else main_handle_read
        )
    else:
        schedule["pre"] = pre_handle_write
        schedule["post"] = post_handle
        schedule["main"] = (
            main_parallel_write if args.io_parallel else main_handle_write
        )
...
def aio_handle_multiprocessing(args, read_op):
    ...
    with Pool(
        processes=args.threads, initializer=_init_tasklet, initargs=(b,)
    ) as p:

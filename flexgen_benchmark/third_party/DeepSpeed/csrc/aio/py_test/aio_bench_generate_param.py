...
def generate_aio_param(read_log_dir, write_log_dir):
    ...
    optimal_config_write = write_results.get(
        write_perf_keys[optimal_key], None
    )

    print(
        f"Best performance (GB/sec): read = {optimal_config_read:5.2f}, write"
        f" = {optimal_config_write:5.2f}"
    )

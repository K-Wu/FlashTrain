...
DS_D_INLINE void to_global(__half* global_output, const int8_t* data,
...
DS_D_INLINE void chunk(__half2* local_output, const int8_t* data,
                       Params<qType, numBits> q_params);

template <int numBits, Type qType>
DS_D_INLINE void chunk(__half* local_output, const int8_t* data,
                       Params<qType, numBits> q_params);
...
template <int numBits, Type qType, int unroll, int threads>
DS_D_INLINE void _to_global(__half* global_output, const int8_t* data,
                            const float* global_params,
                            const int elems_per_group, const int total_elems) {
  ...
  for (int i = 0; i < unroll; i++) {
    ...
    if (elem_id_iter < total_elems) {
      ...
      chunk<numBits, qType>(local_dequant_buffer + i * h_per_chunk,
                            local_load_buffer + i * load_granularity, q_params);
      mem_access::store_global<granularity>(
          global_output + elem_id_iter, local_dequant_buffer + i * h_per_chunk);
    }
  }
}

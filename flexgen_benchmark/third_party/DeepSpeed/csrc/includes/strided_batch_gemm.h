...
class StridedBatchGemm {
 public:
  struct Config {
    ...
    Config(int batch, int mm, int nn, int kk, float param_alpha,
           float param_beta, cublasOperation_t opA, cublasOperation_t opB,
  ...
  void Forward(int bsz, T* output, const T* _buffer_a, const T* _buffer_b,
    ...
    cublas_strided_batched_gemm(handle, _config.m, _config.n, _config.k,
                                &_config.alpha, &_config.beta, _buffer_a,
                                _buffer_b, output, _config.op_A, _config.op_B,
                                stride_a, stride_b, stride_c, bsz,
  ...
  void ForwardPlusSave(T* output, const T* _buffer_a, const T* _buffer_b,
                       cublasHandle_t handle) {
    int stride_a = _config.m * _config.k;
    int stride_b = _config.n * _config.k;
    int stride_c = _config.m * _config.n;
    ...
    cublas_strided_batched_gemm(
        ...
        stride_b, stride_c, _config.batch_size,
#ifdef __HIP_PLATFORM_HCC__
        rocblas_gemm_algo(_config.gemm_algos[0]));
#else
        cublasGemmAlgo_t(_config.gemm_algos[0]));
  ...
  void Backward(int bsz, const T* d_output, const T* _buffer_a,
    ...
    // Calculate d_B.
    cublas_strided_batched_gemm(handle, _config.k, _config.n, _config.m,
                                &_config.alpha, &_config.beta, _buffer_a,

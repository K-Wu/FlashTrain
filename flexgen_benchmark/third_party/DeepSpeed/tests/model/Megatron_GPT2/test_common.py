...
class BaseTestCase(unittest.TestCase):
    ...
    def run_gpt2_test(self, test_config, output):
        ds_flag = (
            "-d " + test_config["json"] if test_config["deepspeed"] else ""
        )
        ckpt_num = (
            test_config["ckpt_num_layers"]
            if "ckpt_num_layers" in test_config
            else 1
        )
        other_args = (
            "-o " + test_config["other_args"]
            if "other_args" in test_config
            else ""
        )

        cmd = (
            "./ds_gpt2_test.sh -m {0} -g {1} -n {2} -b {3} -s {4} -l {5} -h"
            " {6} -q {7} -e {8} -c {9} {10} {11}".format(
                test_config["mp"],
                test_config["gpus"],
                test_config["nodes"],
                test_config["bs"],

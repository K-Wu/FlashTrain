...
class BaseTestCase(unittest.TestCase):
    ...
    def gen_output_name(self, test_config, prefix):
        ...
        zero_args = (
            "_zero" if "zero" in test_config and test_config["zero"] else ""
        )
        ...
        if test_config["deepspeed"]:
            file_name = "_gpu{0}_{1}_ds{2}-{3}.log".format(
                test_config["gpus"], other_args, zero_args, self.timestr
            )
            save_dir = self.test_dir
        else:
            file_name = "_gpu{0}_{1}.log".format(
                test_config["gpus"], other_args
            )
    ...
    def run_BingBertSquad_test(self, test_config, output):
        ds_flag = (
            " -d --deepspeed_config " + test_config["json"]
            if test_config["deepspeed"]
            else " "
        )
        other_args = (
            " " + test_config["other_args"]
            if "other_args" in test_config
            else " "
        )

        cmd = "./run_BingBertSquad_sanity.sh -e 1 -g {0} {1} {2}".format(
            test_config["gpus"], other_args, ds_flag
        )

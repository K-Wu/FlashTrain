...
def load_tf_weights_in_bert(model, tf_checkpoint_path):
    ...
    try:
        ...
        print(
            "Loading a TensorFlow models in PyTorch, requires TensorFlow to be"
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, grads=None):
        ...
        new_context_layer_shape = context_layer1.size()[:-2] + (
            self.all_head_size,
        )
        ...
        if grads is not None:
            query_layer.register_hook(
                lambda x, self=self: grads.append([x, "Query"])
            )
            key_layer.register_hook(
                lambda x, self=self: grads.append([x, "Key"])
            )
            value_layer.register_hook(
                lambda x, self=self: grads.append([x, "Value"])
            )
...
class BertLayer(nn.Module):
    def __init__(self, i, config, weights, biases):
        ...
        self.PreAttentionLayerNorm = BertLayerNorm(
            config.hidden_size, eps=1e-12
        )
        self.PostAttentionLayerNorm = BertLayerNorm(
            config.hidden_size, eps=1e-12
        )
    ...
    def forward(
        self, hidden_states, attention_mask, grads, collect_all_grads=False
    ):
        ...
        if collect_all_grads:
            ...
            self.weight[2].register_hook(
                lambda x, self=self: grads.append([x, "V_W"])
            )
            self.biases[2].register_hook(
                lambda x, self=self: grads.append([x, "V_B"])
            )
            self.weight[3].register_hook(
                lambda x, self=self: grads.append([x, "O_W"])
            )
            self.biases[3].register_hook(
...
class BertEncoder(nn.Module):
    def __init__(self, config, weights, biases):
        ...
        self.layer = nn.ModuleList(
            [
                copy.deepcopy(BertLayer(i, config, weights, biases))
        ...
        if checkpoint_activations:
            ...
            for i, layer_module in enumerate(self.layer):
                ...
                hidden_states.register_hook(
                    lambda x, i=i, self=self: self.grads.append(
                        [x, "hidden_state"]
                    )
                )
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        if not isinstance(config, BertConfig):
            raise ValueError(
                ...
                " pretrained model use `model ="
                " {}.from_pretrained(PRETRAINED_MODEL_NAME)`".format(
        ...
        if os.path.isdir(resolved_archive_file) or from_tf:  # noqa: F821
            ...
            logger.info(
                "extracting archive file {} to temp dir {}".format(
                    resolved_archive_file, tempdir  # noqa: F821
                )
            )
            with tarfile.open(
                resolved_archive_file, "r:gz"
            ) as archive:  # noqa: F821
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def log_summary_writer(self, logs: dict, base="Train"):
        if dist.get_rank() == 0:
            module_name = (  # self._batch_module_name.get(batch_type, self._get_batch_type_error(batch_type))
                "Samples"
            )

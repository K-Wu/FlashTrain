...
class DistributedExec(ABC):
    ...
    def _launch_procs(self, num_procs):
        ...
        for local_rank in range(num_procs):
            p = Process(
                target=self._dist_init, args=(local_rank, num_procs, skip_msg)
            )
            p.start()
            processes.append(p)
        ...
        failed = [
            (rank, p) for rank, p in enumerate(processes) if p.exitcode != 0
        ]
...
class DistributedTest(DistributedExec):
    ...
    def __call__(self, request):
        ...
        self._fixture_kwargs = self._get_fixture_kwargs(
            request, self._current_test
        )

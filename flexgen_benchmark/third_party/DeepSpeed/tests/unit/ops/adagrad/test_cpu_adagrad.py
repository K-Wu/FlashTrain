...
def check_equal(first, second, atol=1e-2, verbose=False):
    ...
    np.testing.assert_allclose(
        x, y, err_msg="param-update mismatch!", atol=atol
    )
...
def test_cpu_adagrad_opt_sparse_embedding(model_size, vocabulary_size, dim):
    ...
    def gen_sparse_grad(vocabulary_size, dim, num_indices, dtype, device):
        ...
        t = torch.sparse_coo_tensor(
            i, v, (vocabulary_size, dim), device=device
        )
        t = t.coalesce()
        new_i = (
            (
                t.indices().view(-1, 1).repeat(1, dim) * dim
                + torch.tensor(range(dim))
            )
            .flatten()
            .unsqueeze(0)
        )
    ...
    param = torch.nn.Parameter(
        torch.randn((voc_size * dim,), dtype=dtype, device=device),
        requires_grad=True,
    )
    torch.set_rng_state(rng_state)

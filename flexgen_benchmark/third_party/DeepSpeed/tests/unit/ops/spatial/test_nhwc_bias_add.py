...
def allclose(x, y):
    ...
    rtol, atol = {
        torch.float32: (5e-3, 5e-4),
        torch.float16: (3e-2, 2e-3),
        torch.int8: (1, 1),
    }[x.dtype]
...
def ref_bias_add_bias_add(activations, bias, other, other_bias):
    return (activations + bias.reshape(1, -1, 1, 1)) + (
        other + other_bias.reshape(1, -1, 1, 1)
    )
...
def test_bias_add_bias_add(batch, image_size, channels):
    ...
    ref_vals = ref_bias_add_bias_add(
        activations.clone().detach(), bias, other, other_bias
    )
    ds_vals = nhwc_bias_add(
        activations, bias, other=other, other_bias=other_bias
    )

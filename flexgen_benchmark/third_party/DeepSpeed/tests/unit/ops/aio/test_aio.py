...
if not deepspeed.ops.__compatible_ops__[AsyncIOBuilder.NAME]:
    pytest.skip(
        "Skip tests since async-io is not compatible", allow_module_level=True
    )
...
class TestRead(DistributedTest):
    ...
    def test_parallel_read(self, tmpdir, single_submit, overlap_events):
        ...
        aio_buffer = torch.empty(
            IO_SIZE, dtype=torch.uint8, device="cpu"
        ).pin_memory()
        h = (
            AsyncIOBuilder()
            .load()
    ...
    def test_async_read(
        self, tmpdir, single_submit, overlap_events, cuda_device
    ):
        ref_file, _ = _do_ref_write(tmpdir)
...
class TestAsyncQueue(DistributedTest):
    ...
    def test_write(self, tmpdir, async_queue, cuda_device):
        ...
        aio_buffers = []
        for i in range(async_queue):
            f, buf = _get_test_file_and_buffer(
                tmpdir, ref_buffers[i], cuda_device, i
            )
            aio_files.append(f)
            aio_buffers.append(buf)

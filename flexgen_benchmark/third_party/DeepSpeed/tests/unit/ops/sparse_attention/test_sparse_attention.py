...
def dense_to_sparse(w, mask, block):
    ...
    Z = w.size(0)
    ret = torch.empty(
        (Z, mask.sum(), block, block), dtype=w.dtype, device=w.device
    )
    nnz = mask.nonzero()
...
def init_softmax_inputs(
    Z, H, M, N, scale, rho, block, dtype, dense_x=True, layout=None
):
    if layout is None:
        layout = make_layout(rho, (H, M // block, N // block))
    if dense_x:
        x = torch.rand(
            (Z, H, M, N), dtype=dtype, requires_grad=True, device="cuda"
        )
    else:
        x = torch.rand(
            (Z, layout.sum(), block, block),
            dtype=dtype,
            requires_grad=True,
            device="cuda",
        )
    dx = torch.rand_like(x)
    bool_attn_mask = torch.randint(
        low=0,
        high=2,
        size=(N, N),
        dtype=torch.bool,
...
def test_softmax(block, width, dtype):
    ...
    st_y, st_dx = run_softmax_sparse(
        x, scale, dx, kp_mask, fp_attn_mask, layout, block
    )
...
def init_matmul_inputs(
    Z, H, M, N, K, rho, mode, trans_a, trans_b, block, dtype, layout
):
    torch.manual_seed(1)
    ...
    x = torch.rand(
        (Z, H, AS0, AS1), dtype=dtype, requires_grad=True, device="cuda"
    )
    w = torch.rand(
        (Z, H, BS0, BS1), dtype=dtype, requires_grad=True, device="cuda"
    )

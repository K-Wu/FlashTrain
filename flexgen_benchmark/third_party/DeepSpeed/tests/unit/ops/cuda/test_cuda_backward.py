...
from unit.modeling import (
    BertConfig,
    BertLayerNorm,
    BertEncoder as BertEncoderPostln,
)
...
def check_equal(first, second, atol=1e-2, verbose=False):
    ...
    for i, (x, y) in enumerate(zip(diction_x, diction_y)):
        ...
        np.testing.assert_allclose(
            x, y, err_msg="Index: {}".format(i), atol=tolerance
        )
...
class DSEncoder(nn.Module):
    def __init__(self, config, weights, biases):
        ...
        self.layer = nn.ModuleList(
            [
                copy.deepcopy(
                    DeepSpeedTransformerLayer(config, weights, biases)
                )
        ...
        if checkpoint_activations:
            ...
            while l < num_layers:
                hidden_states = checkpoint.checkpoint(
                    custom(l, l + chunk_length),  # noqa: F821
                    hidden_states,
                    attention_mask * 1,
                )
                l += chunk_length
            ...
            for i, layer_module in enumerate(self.layer):
                hidden_states = layer_module(
                    hidden_states, attention_mask, grads=self.grads
                )
                hidden_states.register_hook(
                    lambda x, self=self: self.grads.append([x, "hidden_state"])
                )
...
def create_models(ds_config):
    ...
    for i in range(4):
        weights.append(
            nn.Parameter(
                torch.Tensor(ds_config.hidden_size, ds_config.hidden_size)
            )
        )
    ...
    weights.append(
        nn.Parameter(
            torch.Tensor(ds_config.intermediate_size, ds_config.hidden_size)
        )
    )
    weights[5].data.normal_(mean=0.0, std=ds_config.initializer_range)
    weights.append(
        nn.Parameter(
            torch.Tensor(ds_config.hidden_size, ds_config.intermediate_size)
        )
    )
...
def run_backward(ds_config, seq_len, atol=1e-2, verbose=False):
    ...
    hidden_states = torch.randn(
        ds_config.batch_size, seq_len, ds_config.hidden_size, **kwargs
    )
...
@pytest.mark.parametrize(
    (
        "batch_size, hidden_size, seq_len, heads, num_layers, is_preln,"
        " use_fp16, atol"
    ),
    [

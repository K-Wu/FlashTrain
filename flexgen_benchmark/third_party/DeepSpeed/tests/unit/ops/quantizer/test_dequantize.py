...
def run_quantize(data, num_groups, q_bits, is_symmetric_quant):
    ...
    return quantize_module.quantize(
        ...
        num_groups,
        q_bits,
        quantize_module.Symmetric
        if is_symmetric_quant
        else quantize_module.Asymmetric,
    )


def run_dequantize(
    quantized_data, params, num_groups, q_bits, is_symmetric_quant
):
    global quantize_module
    if quantize_module is None:
        quantize_module = op_builder.QuantizerBuilder().load()
...
def test_dequantize(num_elems, num_groups, is_symmetric_quant, q_bits):
    ...
    quantized_data, params = run_quantize(
        activations, num_groups, q_bits, is_symmetric_quant
    )

    ds_dequant = run_dequantize(
        quantized_data, params, num_groups, q_bits, is_symmetric_quant
    )
    ref_dequant = run_ref_dequantize(
        quantized_data, params, num_groups, q_bits, is_symmetric_quant
    )

    assert torch.allclose(
        ds_dequant.flatten(), ref_dequant.flatten(), rtol=3e-2, atol=2e-3
    )

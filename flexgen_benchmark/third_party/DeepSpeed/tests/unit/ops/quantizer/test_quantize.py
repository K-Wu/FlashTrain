...
def run_quantize_ds(activations, num_groups, q_bits, is_symmetric_quant):
    ...
    return inference_module.quantize(
        ...
        else inference_module.Asymmetric,
    )
...
def get_scale_zero_point(
    ...
    if is_symmetric_quant:
        ...
        zero_point = torch.zeros(
            scale.shape, dtype=torch.float32, device="cuda"
        )
    else:
...
def run_float_quantize(
    q_bits, is_symmetric_quant, activations_ref, num_groups
):
    # Reference implementation
    # https://pytorch.org/docs/stable/quantization-support.html

    activations_ref = activations_ref.reshape(num_groups, -1).to(
        dtype=torch.float32
    )

    max_abs_activations_ref = torch.amax(
        torch.abs(activations_ref), dim=-1
    ).view(num_groups, -1)
    max_activations_ref = torch.amax(activations_ref, dim=-1).view(
        num_groups, -1
    )
    min_activations_ref = torch.amin(activations_ref, dim=-1).view(
        num_groups, -1
    )
    ...
    data_i32 = torch.minimum(
        torch.maximum(data_i32, q_min.expand_as(data_i32)),
...
@pytest.mark.parametrize(
    "num_elems", [8, 16, 32, 64, 128, 256, 4096, 8192, 12288, 16384]
)
...
def test_float_quantize(
    ...
    if directed_case == "all_zeros":
        activations_ds = torch.zeros(
            (num_groups, num_elems), dtype=torch.float16, device="cuda"
        )
    else:
        activations_ds = torch.randn(
            (num_groups, num_elems), dtype=torch.float16, device="cuda"
        )
    activations_ref = activations_ds.clone().detach()

    ref_out_tensor, ref_params = run_float_quantize(
        q_bits, is_symmetric_quant, activations_ref, num_groups
    )

    ds_out_tensor, ds_out_params = run_quantize_ds(
        activations_ds, num_groups, q_bits, is_symmetric_quant
    )
    ...
    assert torch.all(
        torch.lt(
            torch.abs(ds_out_tensor.flatten() - ref_out_tensor.flatten()), 2
        )
    )
    if is_symmetric_quant:
        assert torch.allclose(
            ds_out_params.flatten(), ref_params[:, 0].flatten()
        )
    else:
        assert torch.allclose(
            ds_out_params[:, 0].flatten(), ref_params[:, 0].flatten()
        )
        assert torch.allclose(
            ds_out_params[:, 1].flatten(),
...
def run_integer_quantize_ds(activations, num_groups, q_bits):
    ...
    return inference_module.quantize(
        activations, num_groups, q_bits, inference_module.IntegerSymmetric
    )
...
def run_integer_quantize(q_bits, activations_ref, num_groups):
    ...
    max_abs_activations_ref = (
        (max_abs_activations_ref + 1).to(torch.int8).to(torch.float32)
    )
    print(max_abs_activations_ref)
...
def test_integer_quantize(num_elems, num_groups, q_bits):
    activations_ds = (
        torch.ones((num_groups, num_elems), dtype=torch.float16, device="cuda")
        * 0.35
    )
    activations_ref = activations_ds.clone().detach()

    ref_out_tensor, ref_params = run_integer_quantize(
        q_bits, activations_ref, num_groups
    )

    ds_out_tensor, ds_out_params = run_integer_quantize_ds(
        activations_ds, num_groups, q_bits
    )

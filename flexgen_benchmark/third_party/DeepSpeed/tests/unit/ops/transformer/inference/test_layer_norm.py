...
def allclose(x, y):
    ...
    rtol, atol = {torch.float32: (5e-4, 5e-5), torch.float16: (3e-2, 2e-3)}[
        x.dtype
    ]
...
def ref_implementation(vals, gamma, beta, espilon, channels, dtype):
    ...
    return torch.nn.functional.layer_norm(
        vals_f, (channels,), weight=gamma_f, bias=beta_f
    ).to(dtype)
...
def test_layer_norm(batch, seq_len, channels, dtype):
    ...
    gamma = torch.randn(
        (channels), dtype=dtype, device=torch.cuda.current_device()
    )
    beta = torch.rand(
        (channels), dtype=dtype, device=torch.cuda.current_device()
    )
    ...
    ref_output = ref_implementation(
        vals, gamma, beta, epsilon, channels, dtype
    )
...
def residual_ref_implementation(
    vals, bias, res, gamma, beta, espilon, channels, dtype
):
    vals_f = vals.to(torch.float32)
    ...
    beta_f = beta.to(torch.float32)
    return torch.nn.functional.layer_norm(
        vals_f + bias_f + res_f, (channels,), weight=gamma_f, bias=beta_f
    ).to(dtype)
...
def residual_ds_implementation(vals, bias, res, gamma, beta, epsilon):
    ...
    if inference_module is None:
        inference_module = InferenceBuilder().load()
    return inference_module._layer_norm_residual(
...
def test_layer_norm_residual(batch, seq_len, channels, dtype):
    ...
    new_output = residual_ds_implementation(
        vals, bias, residual, gamma, beta, epsilon
    )
    ref_output = residual_ref_implementation(
        vals, bias, residual, gamma, beta, epsilon, channels, dtype
    )

    assert allclose(new_output, ref_output)


def residual_store_ref_implementation(
    vals, bias, res, gamma, beta, espilon, channels, dtype
):
    vals_f = vals.to(torch.float32)
    bias_f = bias.to(torch.float32).reshape(1, 1, -1)
    ...
    norm_output = torch.nn.functional.layer_norm(
        res_output, (channels,), weight=gamma_f, bias=beta_f
    ).to(dtype)
...
def residual_store_ds_implementation(vals, bias, res, gamma, beta, epsilon):
    ...
    if inference_module is None:
        inference_module = InferenceBuilder().load()
    return inference_module.layer_norm_residual_store(
        vals, bias, res, gamma, beta, epsilon
    )
...
def test_layer_norm_residual_store(batch, seq_len, channels, dtype):
    ...
    ref_norm_output, norm_res_output = residual_store_ref_implementation(
        vals, bias, residual, gamma, beta, epsilon, channels, dtype
    )

    ds_norm_output, ds_res_output = residual_store_ds_implementation(
        vals, bias, residual, gamma, beta, epsilon
    )

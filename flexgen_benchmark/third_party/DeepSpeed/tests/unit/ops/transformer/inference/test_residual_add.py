...
def allclose(x, y):
    ...
    rtol, atol = {torch.float32: (5e-4, 5e-5), torch.float16: (3e-2, 2e-2)}[
        x.dtype
    ]
...
def res_add_bias_ref(
    ...
    if pre_attn_norm:
        hidden_state += (
            residual + final_bias + attn_output + attn_bias
        ) / mp_size
    else:
...
def run_residual_add_reference(
    hidden_state,
    residual,
    attn_output,
    attn_bias,
...
def test_residual_add(
    ...
    ds_out = torch.randn(
        (batch, sequence, hidden_dim), dtype=dtype, device="cuda"
    )
    ...
    ref_out = run_residual_add_reference(
        ref_out,
        residual,
        attn_output,
        attn_bias,

...
class SimpleModel(torch.nn.Module):
    def __init__(self, hidden_dim, empty_grad=False, nlayers=1):
        ...
        self.linears = torch.nn.ModuleList(
            [torch.nn.Linear(hidden_dim, hidden_dim) for i in range(nlayers)]
        )
        if empty_grad:
...
class SimpleMoEModel(torch.nn.Module):
    def __init__(
        self, hidden_dim, num_experts=4, ep_size=1, use_residual=False
    ):
...
class SimplePRMoEModel(torch.nn.Module):
    def __init__(
        self, hidden_dim, num_experts=2, ep_size=1, use_residual=False
    ):
...
class LinearStack(torch.nn.Module):
    def __init__(
        self, input_dim=128, hidden_dim=128, output_dim=128, num_layers=4
    ):
        super().__init__()
...
class LinearStackPipe(PipelineModule):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            layers=layers, loss_fn=torch.nn.CrossEntropyLoss(), **kwargs
        )
...
def random_dataset(total_samples, hidden_dim, device, dtype=torch.half):
    train_data = torch.randn(
        total_samples, hidden_dim, device=device, dtype=dtype
    )
    train_label = torch.empty(
        total_samples, dtype=torch.long, device=device
    ).random_(hidden_dim)
...
def random_dataloader(
    model, total_samples, hidden_dim, device, dtype=torch.half
):
    batch_size = model.train_micro_batch_size_per_gpu()
    train_dataset = random_dataset(
        total_samples, hidden_dim, device, dtype=dtype
    )

...
if rocm_version != (0, 0):
    pytest.skip(
        "skip inference tests on rocm for now", allow_module_level=True
    )
...
def invalid_model_task_config(model_w_task, dtype, enable_cuda_graph):
    ...
    if pkg_version.parse(torch.__version__) <= pkg_version.parse("1.2"):
        msg = (
            "DS inference injection doesn't work well on older torch versions"
        )
        ...
        if dtype != torch.half:
            ...
            msg = (
                f"Not enough GPU memory to run {model} with CUDA Graph enabled"
            )
    elif (
        "gpt-neox-20b" in model
    ):  # TODO: remove this when neox issues resolved
        msg = "Skipping gpt-neox-20b for now"
    elif ("gpt-neox-20b" in model) and (dtype != torch.half):
        msg = f"Not enough GPU memory to run {model} with dtype {dtype}"
    elif ("bloom" in model) and (dtype != torch.half):
        msg = (
            "Bloom models only support half precision, cannot use dtype"
            f" {dtype}"
        )
...
def query(model_w_task):
    ...
    if task == "fill-mask":
        ...
        return (
            "Is this review positive or negative? Review: this is the best"
            " cast iron skillet you will ever buy"
        )
    else:
...
def fill_mask_assert(x, y):
    return set(res["token_str"] for res in x) == set(
...
def text_generation_assert(x, y):
    return set(res["generated_text"] for res in x) == set(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("roberta-large", "fill-mask"), {RobertaLayer: "output.dense"}),
    ],
    ids=["t5", "roberta"],
)
...
class TestLMCorrectness(DistributedTest):
    ...
    def test(self, model_family, model_name, task):
        ...
        if "gpt-j-6B" in model_name:
            ...
            setattr(
                lm, model_family, getattr(lm, model_family).half().to(device)
            )
            lm._device = device
        else:
            lm = lm_eval.models.get_model(model_family).create_from_arg_string(
                f"pretrained={model_name}", {"device": "cuda"}
            )

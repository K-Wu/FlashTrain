...
def create_bert_model():
    ...
    for i in range(4):
        weights.append(
            torch.nn.Parameter(torch.Tensor(hidden_size, hidden_size))
        )

    weights.append(torch.nn.Parameter(torch.Tensor(hidden_size)))
    weights.append(
        torch.nn.Parameter(torch.Tensor(hidden_size * 4, hidden_size))
    )
    weights.append(
        torch.nn.Parameter(torch.Tensor(hidden_size, hidden_size * 4))
    )
    weights.append(torch.nn.Parameter(torch.Tensor(hidden_size)))
...
class TestCompression:
    ...
    def test_linear_layer_compress(self, tmpdir):
        ...
        assert isinstance(
            compressed_model.layer[0].attention.self.query,
            LinearLayer_Compress,
        )
        assert isinstance(
            compressed_model.layer[0].attention.self.key, LinearLayer_Compress
        )
        assert isinstance(
            compressed_model.layer[0].attention.self.value,
            LinearLayer_Compress,
        )
    ...
    def test_mpu_compress(self, tmpdir):
        ...
        compressed_model = init_compression(
            model, self.get_ds_config(), mpu=mpu
        )

        assert isinstance(
            compressed_model.module.language_model.transformer.layers[
                0

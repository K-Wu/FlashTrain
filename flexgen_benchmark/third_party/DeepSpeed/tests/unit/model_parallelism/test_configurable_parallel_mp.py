...
class TestConfigurableMP(ConfigurableMP):
    ...
    def test_gpt2_basic(self, tmpdir, inputs):
        ...
        assert torch.allclose(baseline, test, atol=1e-07), (
            f"Baseline output {baseline} is not equal to save-then-load output"
            f" {test}"
        )
    ...
    def test_gpt2_mp2_no_resize(self, tmpdir, inputs):
        ...
        assert torch.allclose(baseline, test, rtol=1.0, atol=1e-07), (
            f"Baseline output {baseline} is not equal to save-then-load output"
            f" {test}"
        )
...
class baseline_mp2(DistributedFixture):
    ...
    def run(self, inputs, class_tmpdir):
        ...
        with torch.no_grad():
            baseline = model(
                inputs[0].cuda(), inputs[1].cuda(), inputs[2].cuda()
            )
            if dist.get_rank() == 0:

...
def get_deepspeed_model(model):
    ...
    model, _, _, _ = deepspeed.initialize(
        model=model, model_parameters=model.parameters(), config=ds_config_dict
    )
    return model.cuda()
...
class ConfigurablePP(DistributedTest):
    ...
    def inputs(self, bs=1, seq_len=1, hidden_size=128):
        ...
        attention_mask = torch.randint(
            low=0, high=2, size=(bs, seq_len), dtype=torch.bool
        )
...
class TestConfigurablePP(ConfigurablePP):
    ...
    def test_pp_basic(self, inputs, tmpdir):
        ...
        baseline = model.eval_batch(
            data_iter=data_iter, compute_loss=False, reduce_output=None
        )
            ...
            for mb in range(len(baseline)):
                for b, t in zip(baseline[mb], test[mb]):
                    if b.is_floating_point():  # don't compare masks
                        assert torch.allclose(b, t, atol=1e-07), (
                            f"Baseline output {baseline} is not equal to"
                            f" save-then-load output {test}"
                        )
...
class _baseline(DistributedFixture):
    ...
    def run(self, inputs, class_tmpdir, mp_size, pp_size):
        assert int(os.environ["WORLD_SIZE"]) == (
            pp_size * mp_size
        ), "world size does not match provided pp_size and mp_size"
        args_defaults = {
            "num_layers": 8,
            "hidden_size": 128,
            "num_attention_heads": 8,
...
class TestConfigurableResizePP(ConfigurablePP):
    def _test(
        self, inputs, class_tmpdir, mp_size, pp_size, mp_resize, pp_resize
    ):
        args_defaults = {
            "num_layers": 8,
            "hidden_size": 128,
        ...
        with torch.no_grad():
            ...
            if test is not None:
                ...
                assert torch.allclose(baseline, test, atol=1e-03), (
                    f"Baseline output {baseline} is not equal to"
                    f" save-then-load output {test}"
                )
    ...
    @pytest.mark.parametrize(
        "mp_size, pp_size, mp_resize, pp_resize", [(1, 2, 1, 1)]
    )
    def test_world_size_2to1(
        self,
        inputs,
        class_tmpdir,
        baseline_ws2,
        ...
        self._test(
            inputs, class_tmpdir, mp_size, pp_size, mp_resize, pp_resize
        )

    @pytest.mark.world_size(1)
    @pytest.mark.parametrize(
        "mp_size, pp_size, mp_resize, pp_resize", [(2, 2, 1, 1)]
    )
    def test_world_size_4to1(
        self,
        inputs,
        class_tmpdir,
        baseline_ws4,
    ...
    @pytest.mark.parametrize(
        "mp_size, pp_size, mp_resize, pp_resize", [(2, 2, 2, 1)]
    )
    def test_world_size_4to2(
        self,
        inputs,
        class_tmpdir,
        baseline_ws4,
    ...
    @pytest.mark.parametrize(
        "mp_size, pp_size, mp_resize, pp_resize", [(1, 1, 2, 2)]
    )
    def test_world_size_1to4(
        self,
        inputs,
        class_tmpdir,
        baseline_ws1,
    ...
    @pytest.mark.world_size(4)
    @pytest.mark.parametrize(
        "mp_size, pp_size, mp_resize, pp_resize", [(1, 2, 1, 4), (2, 1, 2, 2)]
    )
    def test_world_size_2to4(
        self,
        inputs,
        class_tmpdir,
        baseline_ws2,

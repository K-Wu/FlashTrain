...
class TestBatchConfig(DistributedTest):
    ...
    def test(self, num_ranks, batch, micro_batch, gas, success):
        ...
        status = _run_batch_config(
            ds_config, train_batch=batch, micro_batch=micro_batch, gas=gas
        )
        _batch_assert(status, ds_config, batch, micro_batch, gas, success)

        # test cases when two out of three parameters are provided
        status = _run_batch_config(
            ds_config, train_batch=batch, micro_batch=micro_batch
        )
        _batch_assert(status, ds_config, batch, micro_batch, gas, success)
        ...
        if success:
            ...
            status = _run_batch_config(
                ds_config, micro_batch=micro_batch, gas=gas
            )
            _batch_assert(status, ds_config, batch, micro_batch, gas, success)
            ...
            if gas == 1:
                ...
                _batch_assert(
                    status, ds_config, batch, micro_batch, gas, success
                )
            ...
            if gas != 1:
                status = _run_batch_config(
                    ds_config, train_batch=batch, gas=gas
                )
                _batch_assert(
                    status, ds_config, batch, micro_batch, gas, success
                )
...
class TestArgs(DistributedTest):
    ...
    def test_none_args(self, base_config):
        ...
        model, _, _, _ = deepspeed.initialize(
            args=None, model=model, config=base_config
        )
        data_loader = random_dataloader(
            model=model, total_samples=5, hidden_dim=10, device=model.device
        )
        for n, batch in enumerate(data_loader):
            loss = model(batch[0], batch[1])

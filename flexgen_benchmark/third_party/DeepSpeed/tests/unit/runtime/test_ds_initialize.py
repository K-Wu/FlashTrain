...
class TestNoOptim(DistributedTest):
    ...
    def test(self, zero_stage):
        ...
        with deepspeed.zero.Init(
            enabled=zero_stage == 3, config_dict_or_path=ds_config
        ):
    ...
    def test(self, scheduler_type, optimizer_type):
        ...
        if scheduler_type is None:
            ...
            if isinstance(client_optimizer, Optimizer):
                ...
                client_scheduler = LambdaLR(
                    torch.optim.Adam(model.parameters()), _my_lambda
                )
        else:
            client_scheduler = _lr_scheduler_callable

        if isinstance(client_scheduler, _LRScheduler) and not isinstance(

    ...
    def test(self, tmpdir):
        ...
        for n, batch in enumerate(data_loader):
            assert len(batch) % 2 == 0, (
                f"multi_output_dataloader failed to return even number of data"
                f" samples (input+target)"
            )
            ...
            expected_loss = torch.tensor(
                2.302734375, dtype=torch.half, device=model.device
            )
            ...
            expected_scaled_loss = (
                summed_loss.float() / grad_accumulation_steps
            )

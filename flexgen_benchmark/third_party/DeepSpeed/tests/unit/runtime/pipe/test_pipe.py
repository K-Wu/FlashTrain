...
class TestPipeCifar10(DistributedTest):
    ...
    def test(self, topo_config):
        ...
        base_model = PipelineModule(
            layers=base_net.to_layers(),
            num_stages=1,
            loss_fn=nn.CrossEntropyLoss(),
        )

        # Train with just data parallelism
        base_losses = train_cifar(
            base_model,
            config=config_dict,
            num_steps=steps,
        ...
        if dist.get_rank() == 0:
            ...
            print(
                "first:"
                f" base={base_losses[0]} test={test_losses[0]} abs={abs_diffs[0]} rel={rel_diffs[0]}"
            )
            ...
            for lastX in [1, 10, 100]:
                ...
                print(
                    f"last-{lastX}:"
                    f" base={base_avg} test={test_avg} abs={base_avg - test_avg} rel={rel_diff(base_avg, test_avg)}"
                )
        ...
        assert (
            rel_diff(base_avg, test_avg) < 0.05
        )  # Originally 0.03, but seeing instability with AMD results

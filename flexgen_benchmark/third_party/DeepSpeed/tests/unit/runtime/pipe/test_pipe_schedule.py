...
def test_pipe_inference_schedule_firststage(micro_batches, stages=3):
    sched = schedule.InferenceSchedule(
        micro_batches=micro_batches, stages=stages, stage_id=0
    )
...
def test_pipe_inference_schedule_midstage(micro_batches, stages=3):
    sched = schedule.InferenceSchedule(
        micro_batches=micro_batches, stages=stages, stage_id=1
    )
...
def test_pipe_inference_schedule_laststage(micro_batches, stages=3):
    sched = schedule.InferenceSchedule(
        micro_batches=micro_batches, stages=stages, stage_id=2
    )
    full = list(iter(sched))
    for idx, cmds in enumerate(full):
...
def test_pipe_schedule_firststage():
    ...
    for cmds in sched:
        ...
        assert all(
            instr.__class__ != schedule.RecvActivation for instr in cmds
        )
        for instr in cmds:
...
def test_pipe_schedule_laststage():
    ...
    assert len(list(iter(sched))) == 2 * (
        sched.micro_batches + sched.stages - 1
    )
    for cmds in sched:
        assert all(
            instr.__class__ != schedule.SendActivation for instr in cmds
        )

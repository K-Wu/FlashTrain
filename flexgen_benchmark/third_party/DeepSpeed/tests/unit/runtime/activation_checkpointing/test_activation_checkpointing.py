...
def _compute(module, *inputs, do_checkpoint=False):
    ...
    sum(
        o.sum() for o in outputs if torch.is_tensor(o) and o.requires_grad
    ).backward()
...
@pytest.mark.parametrize(
    "non_tensor",
    [None, 2, True, (None, 2.5), (None, True, torch.randn(HIDDEN_DIM))],
)
...
@pytest.mark.parametrize(
    ...
    [
        None,
        (torch.randn(HIDDEN_DIM), 2.5),
        (None, torch.randn(HIDDEN_DIM), True),

...
@pytest.mark.parametrize(
    ...
    [
        (WARMUP_LR, {}),
        (WARMUP_DECAY_LR, {WARMUP_NUM_STEPS: 10, TOTAL_NUM_STEPS: 20}),
        (ONE_CYCLE, {CYCLE_MIN_LR: 0, CYCLE_MAX_LR: 0.1}),
        (LR_RANGE_TEST, {}),
    ],
)
class TestGetLrBeforeTrain(DistributedTest):
    ...
    def test(self, scheduler_type, params):
        config_dict = {
            ...
            "scheduler": {"type": scheduler_type, "params": params},
...
@pytest.mark.parametrize(
    ...
    [
        (WARMUP_LR, {}),
        (WARMUP_DECAY_LR, {WARMUP_NUM_STEPS: 5, TOTAL_NUM_STEPS: 10}),
        (
            ONE_CYCLE,
            {
                CYCLE_MIN_LR: 0,
        ...
        (
            LR_RANGE_TEST,
            {LR_RANGE_TEST_MIN_LR: 1e-4, LR_RANGE_TEST_STEP_SIZE: 1},
        ),
    ],
)
...
class TestOneCycle(DistributedTest):
    ...
    @pytest.mark.parametrize(
        "min_lr, max_lr, decay_rate, cycle_step_size, decay_step_size",
        [
    ...
    def test_lr(
        self, min_lr, max_lr, decay_rate, cycle_step_size, decay_step_size
    ):
        config_dict = {
            "train_batch_size": 2,
            "steps_per_print": 1,
        ...
        _verify_continuous_decrease(
            step_lrs[cycle_step_size : (cycle_step_size * 2)]
        )

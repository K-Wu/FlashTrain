...
class TestReduceScatterCoalesced(DistributedTest):
    ...
    def test_two_inputs(self):
        ...
        output1, output2 = reduce_scatter_coalesced(
            inputs, dist.get_world_group()
        )

        if dist.get_rank() == 0:
            assert output1.shape == (3,)
            assert torch.allclose(
                output1, torch.arange(0, 3, **tensor_kwargs) / 2
            )
            assert output2.shape == (2,)
            assert torch.allclose(
                output2, torch.arange(6, 8, **tensor_kwargs) / 2
            )
        elif dist.get_rank() == 1:
            assert output1.shape == (3,)
            assert torch.allclose(
                output1, torch.arange(3, 6, **tensor_kwargs) / 2
            )
            assert output2.shape == (1,)
            assert torch.allclose(
                output2, torch.arange(8, 9, **tensor_kwargs) / 2
            )
...
class TestReduceScatterCoalescedTensorSmallerThanWorldSize(DistributedTest):
    ...
    def test(self):
        input = torch.zeros(
            (1,), dtype=torch.half, device=torch.cuda.current_device()
        )

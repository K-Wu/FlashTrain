...
if rocm_version[0] > 4:
    pytest.skip(
        (
            "NCCL-based 1-bit compression is not yet supported w. ROCm 5 until"
            " cupy supports ROCm 5"
        ),
        allow_module_level=True,
    )
...
@pytest.mark.parametrize(
    "dtype", [torch.float32, torch.float16], ids=["fp32", "fp16"]
)
...
class TestOneBitAdamFP16Pipeline(DistributedTest):
    ...
    def test(self, topo_config):
        ...
        test_model = PipelineModule(
            layers=test_net.to_layers(),
            topology=topo,
...
class TestOneBitLambCheckpointing(DistributedTest):
    ...
    def test(self, tmpdir):
        ...
        # Test whether worker&server error is reset
        assert (
            len(optimizer_2.optimizer.worker_errors) == 0
        ), f"Incorrect worker error"
        assert (
            len(optimizer_2.optimizer.server_errors) == 0
        ), f"Incorrect server error"
        ...
        # Test whether worker&server error is reset
        assert (
            len(optimizer_3.optimizer.worker_errors) == 0
        ), f"Incorrect worker error"
        assert (
            len(optimizer_3.optimizer.server_errors) == 0
        ), f"Incorrect server error"
        # Test whether scaling_coeffs, lamb_coeff_freeze, last_factor are reset
        for v in optimizer_3.state.values():
            assert (
                v["lamb_coeff_freeze"] == 0.0
            ), f"Incorrect lamb_coeff_freeze"
    ...
    def test(self, tmpdir):
        ...
        def torch_sim(a):
            ...
            a_server_compressed = torch.cat(
                [
                    server_scale[i] * a_sign_list[i]
                    for i in range(dist.get_world_size())
                ]
            )
            rank = dist.get_rank()
            server_error = (
                a_list[rank] - server_scale[rank] * a_sign_list[rank]
            )
        ...
        a_after = backend.compressed_allreduce(
            a, worker_error, server_error, local_rank
        )
        ...
        assert (
            torch.sum(diff_server_mask) == 0 or torch.sum(check_mag_mask) == 0
        )

...
class TestAdamBF16ZeroOneCycleCompatibility(DistributedTest):
    ...
    def test(self, zero_stage=2, use_cpu_offload=False):
        if not bf16_required_version_check():
            pytest.skip(
                " DeepSpeed BFloat16 tests need torch >= 1.10, NCCL >= 2.10.3,"
                " CUDA > =11.0 and HW support for BFloat16 to run correctly"
            )
...
@pytest.mark.parametrize(
    "optimizer_constructor", [torch.optim.Adam, FusedAdam]
)
class TestZeroSupportedClientOptimizer(DistributedTest):
    ...
    def test(self, optimizer_constructor, zero_stage=2):
        ...
        model, _, _, _ = deepspeed.initialize(
            config=config_dict, model=model, optimizer=client_optimizer
        )
...
@pytest.mark.parametrize(
    "comm_type", [torch.float16, torch.bfloat16], ids=["fp16", "bfp16"]
)
class TestZeroDtypeCocktail(DistributedTest):
    ...
    def test(self, comp_type, comm_type):
        if comp_type == torch.bfloat16 or comm_type == torch.bfloat16:
            if not bf16_required_version_check():
                pytest.skip(
                    " DeepSpeed BFloat16 tests need torch >= 1.10, NCCL >="
                    " 2.10.3, CUDA > =11.0 and HW support for BFloat16 to run"
                    " correctly"
                )
        ...
        config_dict = {
            "train_batch_size": 2,
            "steps_per_print": 1,
            "fp16": {"enabled": comp_type == torch.float16},
            "bf16": {"enabled": comp_type == torch.bfloat16},
        ...
        def custom_reduce(
            tensor, dst, op=dist.ReduceOp.SUM, group=None, async_op=False
        ):

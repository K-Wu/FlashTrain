...
class TestFP16OptimizerForMoE(DistributedTest):
    ...
    def test_unfused_gradnorm(self, monkeypatch):
        ...
        def mock_unscale_and_clip_grads(total_norm, apply_scale=True):
            ...
            all_gather_results = [
                torch.zeros_like(torch_norm_tensor)
        ...
        monkeypatch.setattr(
            optimizer, "unscale_and_clip_grads", mock_unscale_and_clip_grads
        )
    ...
    def test_fused_gradnorm(self, monkeypatch):
        ...
        def mock_unscale_and_clip_grads(
            grads_groups_flat, total_norm, apply_scale=True
        ):
            torch_norm_tensor = torch.cuda.FloatTensor([total_norm])
            all_gather_results = [
                torch.zeros_like(torch_norm_tensor)
...
@pytest.mark.parametrize(
    "optimizer_constructor", [FusedAdam, torch.optim.Adam]
)
...
class TestZero3LazyScatter(DistributedTest):
    ...
    def test(self):
        config_dict = {
            ...
            "steps_per_print": 1,
            "fp16": {"enabled": True, "initial_scale_power": 10},
            "optimizer": {"type": "AdamW", "params": {"lr": 0.00015}},

...
class TestZeroToFP32(DistributedTest):
    ...
    def test_1_param_group(self, tmpdir, zero_stage):
        ...
        if zero_stage == 3:
            with deepspeed.zero.GatheredParameters(
                list(model.module.parameters(recurse=True)), modifier_rank=None
            ):
    ...
    def test_2_param_groups(self, tmpdir, zero_stage):
        ...
        optim_groups = [
            ...
            {"params": [l.bias for l in model.ll], "weight_decay": 0.0},
        ]
...
class TestPartitionNcclAlignment(DistributedTest):
    ...
    def test(self, zero_stage=2):
        ...
        parallel_partitioned_bit16_groups = (
            ...
            if zero_stage == 2
            else model.optimizer.parallel_partitioned_fp16_groups
        )
        for data_parallel_partitions in parallel_partitioned_bit16_groups:
            for partition_id, partitioned_data in enumerate(
...
class TestZero3ParamPartitioningBase(DistributedTest):
    ...
    def test(
        ...
        n = 5
        weights = [
            Parameter(torch.zeros((m, n), dtype=torch.float32))
            for _ in range(3)
        ]
        model = EltwiseMultiplicationTestNetwork(*weights)
        prefetch_bucket_size = sum(
        ...
        for train_iter in range(3):
            ...
            assert set(grad_partitions.keys()) == {0}, (
                "should have one parameter group but got"
                f" {len(grad_partitions)}"
            )
            ...
            if dist.get_rank() == 0:
                ...
                assert torch.allclose(
                    ...
                    grad_multiplier
                    * create_tensor([3 * 2 * 1] * 8, torch.float),
                )
                ...
                assert torch.allclose(
                    dloss_wrt_layer3.cuda(),
                    grad_multiplier
                    * create_tensor(([8] * 7) + [0], torch.float),
                )
                assert torch.allclose(
                    dloss_wrt_layer2.cuda(),
                    grad_multiplier
                    * create_tensor(([6 * 2] * 7) + [0], torch.float),
                )
                assert torch.allclose(
                    dloss_wrt_layer1.cuda(),
                    grad_multiplier
...
class TestZero3ParamPartitioningLargeParam(DistributedTest):
    ...
    def test(self, init_context_manager: bool, param_sz: int = 8100) -> None:
        class LargeParamModel(Module):
            def __init__(self):
                ...
                self.param = Parameter(
                    torch.zeros((param_sz,), dtype=torch.float32)
                )

                # only do weight initialization on root rank to
                # make sure we are broadcasting correctly from rank 0
                if dist.get_rank() == 0:
                    partition_sz = math.ceil(
                        self.param.numel() / dist.get_world_size()
                    )
                    offset = 0
                    for rank in range(dist.get_world_size()):
                        with torch.no_grad():
                            self.param[offset : offset + partition_sz].fill_(
                                rank
                            )
                        offset += partition_sz
        ...
        with deepspeed.zero.Init(
            mem_efficient_linear=False, enabled=init_context_manager
        ):
        ...
        for train_iter in range(
            3
        ):  # test multiple iterations to cover prefetching
            activation: Tensor = ds_engine(
                torch.ones(
                    param_sz, dtype=torch.float16, device=ds_engine.device
                )
            )

            partition_sz = math.ceil(param_sz / self.world_size)
            for rank_idx, start_idx in enumerate(
                range(0, param_sz, partition_sz)
            ):
                activation_from_partition = activation[
                    start_idx : start_idx + partition_sz
                ]
                assert torch.allclose(
...
class TestZero3ParamPartitioningManyParams(DistributedTest):
    ...
    def test(
        self, param_sz: int, n_layers: int, init_context_manager: bool
    ) -> None:
            ...
            def __init__(self) -> None:
                ...
                for layer_num, module in enumerate(self.modulelist):
                    with deepspeed.zero.GatheredParameters(
                        ...
                        partition_sz = math.ceil(
                            param.numel() / dist.get_world_size()
                        )
                        offset = 0
                        for rank in range(dist.get_world_size()):
                            with torch.no_grad():
        ...
        for _ in range(3):  # test multiple iterations to cover prefetching
            ...
            partition_sz = math.ceil(param_sz / self.world_size)
            expected_activations = torch.empty(
                param_sz, dtype=torch.float16, device=ds_engine.device
            )
...
class TestZero3InitForParentWeightInitialization(DistributedTest):
    ...
    def test(self):
        ...
        assert model.linear.weight.ds_tensor.numel() == math.ceil(
            12 / self.world_size
        )
        assert torch.allclose(
...
class TestZero3ParamPartitioningBaseBF16(DistributedTest):
    ...
    def test(
        ...
        def create_tensor(vals):
            return torch.as_tensor(
                vals, dtype=torch.bfloat16, device=ds_engine.device
            )
        ...
        for train_iter in range(3):
            _assert_partition_status(
                ds_engine, {ZeroParamStatus.NOT_AVAILABLE}
            )
            activations = ds_engine(
                x=torch.ones(
                    (m, n), dtype=torch.bfloat16, device=ds_engine.device
                ),
                y=torch.ones(
                    (m, n), dtype=torch.bfloat16, device=ds_engine.device
                ),
            ...
            if dist.get_rank() == 0:
                assert torch.allclose(
                    dloss_wrt_layer3.cuda(),
                    grad_multiplier
                    * create_tensor([2] * 8).to(expected_grad_dtype),
                )
                assert torch.allclose(
                    dloss_wrt_layer2.cuda(),
                    grad_multiplier
                    * create_tensor([3 * 1] * 8).to(expected_grad_dtype),
                )
                assert torch.allclose(
                    dloss_wrt_layer1.cuda(),
                    grad_multiplier
        ...
        for i, batch in enumerate(data_loader):
            ...
            if zero_stage == 3:
                for sub_group_id, _ in enumerate(optimizer.fp16_groups):
                    fp32_param = optimizer.fp32_partitioned_groups_flat[
                        sub_group_id
                    ]

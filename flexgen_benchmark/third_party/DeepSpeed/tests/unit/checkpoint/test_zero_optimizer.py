...
class TestZeROCheckpoint(DistributedTest):
    ...
    def test_load_optimizer_state(
        self, tmpdir, zero_stage, use_cpu_offload, adam_optimizer
    ):
        if (
            use_cpu_offload
        ...
        if zero_stage == 3:
            with deepspeed.zero.Init():
                models = [
                    SimpleModel(hidden_dim, empty_grad=False) for _ in range(2)
                ]
        else:
            models = [
                SimpleModel(hidden_dim, empty_grad=False) for _ in range(2)
        ...
        checkpoint_correctness_verification(
            config_dict, models, hidden_dim, tmpdir, load_optimizer_states=True
        )
    ...
    def test_not_load_optimizer_state(
        self, tmpdir, zero_stage, use_cpu_offload, adam_optimizer
    ):
        if (
            use_cpu_offload
    ...
    def test_hybrid_optimizer_state(self, tmpdir, zero_stage):
        ...
        optimizers = [
            HybridStateOptimizer(model.parameters()) for model in models
        ]
    ...
    def test_load_module_only(self, tmpdir, zero_stage):
        ...
        checkpoint_correctness_verification(
            config_dict, models, hidden_dim, tmpdir, load_module_only=True
        )
    ...
    def run(self, class_tmpdir, elastic_save, load_optim):
        ...
        if load_optim:
            torch.save(
                ...
                os.path.join(class_tmpdir, "opt-state-dict"),
            )
...
class TestZeROElasticCheckpoint(DistributedTest):
    ...
    def test_elastic_checkpoint_fixed_dp(
        self, tmpdir, elastic_save, elastic_load, load_optim
    ):
        ds_config = {
            "train_batch_size": 2,
            "optimizer": {"type": "Adam"},
        ...
        expected_mismatch_keys = (
            [] if required_minimum_torch_version(1, 4) else ["params"]
        )
        ...
        if load_optim:
            torch.save(
                model.optimizer.optimizer.state_dict(),
                os.path.join(tmpdir, "opt-state-dict"),
            )
        ...
        if load_optim:
            ...
            for curr_param_group, saved_param_group in zip(
                curr_sd["param_groups"], saved_sd["param_groups"]
            ):
                compare_state_dicts(
                    curr_param_group, saved_param_group, expected_mismatch_keys
                )
        ...
        if load_optim:
            with pytest.raises(
                ...
                model.load_checkpoint(
                    class_tmpdir, load_optimizer_states=load_optim
                )
        else:
            model.load_checkpoint(
                class_tmpdir, load_optimizer_states=load_optim
...
class TestZeROSaveLoadEdgeCase(DistributedTest):
    ...
    def test_immediate_save_load(self, tmpdir, zero_stage):
        ...
        ds_model = create_deepspeed_model(
            config_dict=config_dict, model=model, base_optimizer=None
        )
        ds_model.save_checkpoint(tmpdir)
        ds_model.load_checkpoint(
            tmpdir,
            load_optimizer_states=False,

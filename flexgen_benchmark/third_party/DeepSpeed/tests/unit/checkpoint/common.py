...
def compare_deepspeed_states(saved_model, loaded_model):
    ...
    assert (
        saved_model.sparse_tensor_module_names
        == loaded_model.sparse_tensor_module_names
    )
...
def compare_model_states(
    saved_model, loaded_model, compare_optimizer=True, load_module_only=False
):
    if not load_module_only:
        compare_deepspeed_states(saved_model, loaded_model)

    for p0, p1 in zip(
        saved_model.module.named_parameters(),
        loaded_model.module.named_parameters(),
    ):
        np0, p0 = p0
        np1, p1 = p1
        ...
        try:
            assert torch.allclose(p0, p1, atol=1e-07), (
                f"FP16 model state {p0} is not equal to {p1}, names:{np0},"
                f" {np1}"
            )
    ...
    if DeepSpeedZeroOptimizer_Stage3 is not None and isinstance(
        saved_model.optimizer, DeepSpeedZeroOptimizer_Stage3
    ):
        for p0, p1 in zip(
        ...
        for params0, params1 in zip(
            saved_model.optimizer.fp32_groups,
            loaded_model.optimizer.fp32_groups,
        ):
...
def compare_state_dicts(state0, state1, expected_mismatch_keys=[]):
    for (k0, s0), (k1, s1) in zip(state0.items(), state1.items()):
        ...
        if isinstance(s0, torch.Tensor) and isinstance(s1, torch.Tensor):
            ...
            assert s0 == s1, (
                f"failures with keys = {k0}, {k1}, values = {type(s0[0])} and"
                f" {type(s1[0])}"
            )
...
def compare_optimizer_states(saved_model, loaded_model, hidden_dim, fp16=True):
    saved_optimizer = (
        saved_model.optimizer.optimizer if fp16 else saved_model.optimizer
    )
    loaded_optimizer = (
        loaded_model.optimizer.optimizer if fp16 else loaded_model.optimizer
    )
...
def compare_lr_scheduler_states(saved_model, loaded_model):
    ...
    for state0, state1 in zip(saved_sd.values(), loaded_sd.values()):
        if isinstance(state0, numbers.Number) and isinstance(
            state1, numbers.Number
        ):
...
def checkpoint_correctness_verification(
    ...
    ds_model = create_deepspeed_model(
        ...
        for f in files:
            if "_expert_" in f and "_model_states" in f:
                ...
                assert needed == stored, (
                    "MoE expert checkpoint uses more storage than"
                    f" required: {f}"
                )

    loaded_model = create_deepspeed_model(
        config_dict=config_dict,
        model=models[1],
        base_optimizer=base_optimizers[1],
    )
    assert (
        list(trained_model.parameters())[0].dtype
        == list(loaded_model.parameters())[0].dtype
    )

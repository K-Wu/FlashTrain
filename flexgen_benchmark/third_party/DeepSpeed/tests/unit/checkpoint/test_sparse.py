...
class TestSparseCheckpoint(DistributedTest):
    ...
    @pytest.mark.parametrize(
        ["to_save_model_has_embedding", "to_save_model_sparse"],
        [
            [False, False],
            [True, False],
            [True, True],
    ...
    def test_non_strict_load_sparse(
        self,
        tmpdir,
        ...
        is_sparse_destination = (
            isinstance(model_destination, ModelEmbedding)
            and destination_sparse
        )
        if (
            isinstance(model_destination, ModelEmbedding)
            and model_destination.emb.sparse
        ):
            assert (
                "emb.weight" in engine_destination.sparse_tensor_module_names
            )
        engine_destination.load_checkpoint(
            save_folder,
            tag=save_tag,
        ...
        if isinstance(model_destination, ModelEmbedding) and isinstance(
            model_to_save, ModelEmbedding
        ):
            assert (
                engine_destination.sparse_tensor_module_names
                == engine_to_save.sparse_tensor_module_names
            )
        elif isinstance(model_destination, ModelEmbedding):
            assert (
                not is_sparse_destination
                or "emb.weight"
                in engine_destination.sparse_tensor_module_names
            )
        else:

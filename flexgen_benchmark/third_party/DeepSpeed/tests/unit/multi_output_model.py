...
def multi_output_dataloader(
    model, total_samples, hidden_dim, device, inputs, targets
):
    ...
    train_label = [
        torch.empty(total_samples, device=device, dtype=torch.long).fill_(y)
        for y in targets
    ]

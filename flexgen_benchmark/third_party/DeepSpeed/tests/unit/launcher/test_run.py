...
def test_parser_local():
    ...
    assert ret == {"worker-0": [1]}
    ret = dsrun.parse_resource_filter(
        hosts, include_str="worker-0:1@worker-0:0,1"
    )
...
def test_num_plus_parser():
    ...
    with pytest.raises(ValueError):
        dsrun.main(
            args="--num_nodes 1 --num_gpus 1 -i localhost foo.py".split()
        )
    with pytest.raises(ValueError):
    ...
    with pytest.raises(ValueError):
        dsrun.main(
            args="--num_nodes 1 --num_gpus 1 -e localhost foo.py".split()
        )
    with pytest.raises(ValueError):

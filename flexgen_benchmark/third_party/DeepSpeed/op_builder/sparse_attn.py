...
class SparseAttnBuilder(OpBuilder):
    ...
    def is_compatible(self, verbose=True):
        ...
        if torch.version.cuda is None:
            ...
            cuda_compatible = (int(major) == 10 and int(minor) >= 1) or (
                int(major) >= 11
            )
        ...
        if not torch_compatible:
            self.warning(
                f"{self.NAME} requires a torch version >= 1.5 but detected"
        ...
        try:
            ...
            self.warning(
                f"please install triton==1.0.0 if you want to use sparse"
                f" attention"
            )
            return False
        ...
        if triton_mismatch:
            self.warning(
                f"using untested triton version ({installed_triton}), only"
                " 1.0.0 is known to be compatible"
            )
            return False

        return (
            super().is_compatible(verbose)
            and torch_compatible
            and cuda_compatible
        )

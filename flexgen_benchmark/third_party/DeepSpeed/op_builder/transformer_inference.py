...
class InferenceBuilder(CUDAOpBuilder):
    ...
    def is_compatible(self, verbose=True):
        try:
            ...
            self.warning(
                "Please install torch if trying to pre-compile inference"
                " kernels"
            )
            return False
        ...
        if not self.is_rocm_pytorch() and torch.cuda.is_available():
            ...
            if cuda_capability < 6:
                self.warning(
                    "NVIDIA Inference is only supported on Pascal and newer"
                    " architectures"
                )
                cuda_okay = False
            if cuda_capability >= 8:
                if torch_cuda_major < 11 or sys_cuda_major < 11:
                    self.warning(
                        "On Ampere and higher architectures please use"
                        " CUDA 11+"
                    )
                    cuda_okay = False

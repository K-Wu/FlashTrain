...
try:
    ...
    print(
        f"{WARNING} unable to import torch, please install it if you want to"
        " pre-compile any deepspeed ops."
    )
else:
    ...
    if sys_cuda_version != torch_cuda_version:
        if (
            ...
            print(
                ...
                f" version torch was compiled with {torch.version.cuda} but"
                " since the APIs are compatible, accepting this combination"
            )
            return
        raise Exception(
            f"Installed CUDA version {sys_cuda_version} does not match the"
...
class OpBuilder(ABC):
    ...
    def assert_torch_info(torch_info):
        ...
        if not OpBuilder.is_rocm_pytorch():
            if (
                ...
                raise RuntimeError(
                    "PyTorch and CUDA version mismatch! DeepSpeed ops were"
                    " compiled and installed with a different version than"
                    " what is being used at runtime. Please re-install"
                    " DeepSpeed or switch torch versions. DeepSpeed install"
                    f" versions: torch={install_torch_version},"
                    f" cuda={install_cuda_version}, runtime"
                    f" versions:torch={current_torch_version},"
                    f" cuda={current_cuda_version}"
                )
        else:
            if (
                install_hip_version != current_hip_version
                or install_torch_version != current_torch_version
            ):
                raise RuntimeError(
                    "PyTorch and HIP version mismatch! DeepSpeed ops were"
                    " compiled and installed with a different version than"
                    " what is being used at runtime. Please re-install"
                    " DeepSpeed or switch torch versions. DeepSpeed install"
                    f" versions: torch={install_torch_version},"
                    f" hip={install_hip_version}, runtime"
                    f" versions:torch={current_torch_version},"
                    f" hip={current_hip_version}"
                )
    ...
    def has_function(self, funcname, libraries, verbose=False):
        ...
        try:
            ...
            prog = (
                "void %s(void); int main(int argc, char** argv) { %s(); return"
                " 0; }" % (funcname, funcname)
            )
    ...
    def cpu_arch(self):
        ...
        try:
            ...
            self.warning(
                f"{self.name} attempted to use `py-cpuinfo` but failed"
                f" (exception type: {type(e)}, {e}), falling back to `lscpu`"
    ...
    def _backup_cpuinfo(self):
        ...
        if not self.command_exists("lscpu"):
            self.warning(
                f"{self.name} attempted to query 'lscpu' after failing to use"
                " py-cpuinfo to detect the CPU architecture. 'lscpu' does not"
                " appear to exist on your system, will fall back to use"
                " -march=native and non-vectorized execution."
            )
            return None
    ...
    def command_exists(self, cmd):
        ...
        if not valid and len(cmds) > 1:
            print(
                f"{WARNING} {self.name} requires one of the following commands"
                f" '{cmds}', but it does not exist!"
            )
        elif not valid and len(cmds) == 1:
            print(
                f"{WARNING} {self.name} requires the '{cmd}' command, but it"
                " does not exist!"
            )
        return valid
    ...
    def jit_load(self, verbose=True):
        if not self.is_compatible(verbose):
            raise RuntimeError(
                f"Unable to JIT load the {self.name} op due to it not being"
                f" compatible due to hardware/software issue. {self.error_log}"
            )
        try:
            import ninja  # noqa: F401
        except ImportError:
            raise RuntimeError(
                f"Unable to JIT load the {self.name} op due to ninja not being"
                " installed."
            )
...
class CUDAOpBuilder(OpBuilder):
    def compute_capability_args(self, cross_compile_archs=None):
        ...
        if len(ccs) == 0:
            raise RuntimeError(
                f"Unable to load {self.name} op due to no compute capabilities"

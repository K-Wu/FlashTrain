...
class AsyncIOBuilder(OpBuilder):
        ...
        for pkgmgr, data in libs.items():
            ...
            if path is not None:
                ...
                if result.wait() == 0:
                    ...
                    self.warning(
                        f"{self.NAME}: please install the {lib} package with"
                        f" {tool}"
                    )
                break
        return found
    ...
    def is_compatible(self, verbose=True):
        ...
        if verbose and not aio_compatible:
            self.warning(
                f"{self.NAME} requires the dev libaio .so object and headers"
            ...
            self.warning(
                "If libaio is already installed (perhaps from source), try"
                " setting the CFLAGS and LDFLAGS environment variables to"

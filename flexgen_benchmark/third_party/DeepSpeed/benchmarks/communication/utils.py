...
def print_header(args, comm_op):
    ...
    if args.raw:
        duration_str += " (us)"
    header += (
        f"{'Size (Bytes)':20s} {'Description':25s} {duration_str:20s} {tput:20s} {busbw:20s}\n"
    )
    header += "----------------------------------------------------------------------------------------------------"
    print_rank_0(header)
...
def benchmark_parser():
    ...
    parser.add_argument(
        "--all-reduce", action="store_true", help="Run all_reduce"
    )
    parser.add_argument(
        "--all-gather", action="store_true", help="Run all_gather"
    )
    parser.add_argument(
        "--all-to-all", action="store_true", help="Run all_to_all"
    )
    parser.add_argument("--pt2pt", action="store_true", help="Run pt2pt")
    parser.add_argument(
        "--broadcast", action="store_true", help="Run broadcast"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0.4,
        help=(
            "Proportion of max available GPU memory to use for single-size"
            " evals"
        ),
    )
    parser.add_argument(

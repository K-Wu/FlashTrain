        ...
        if args.dist == "torch":
            if hasattr(torch.distributed, "_all_gather_base"):
                dist._all_gather_base(
                    output, input, group=None, async_op=args.async_op
                )
            else:
                output_tensors = list(
                ...
                dist.all_gather(
                    output_tensors, input_tensor, group=group, async_op=True
                )
        elif args.dist == "deepspeed":
            dist.allgather_fn(
                output, input, group=None, async_op=args.async_op
...
def run_all_gather(local_rank, args):
    ...
    if args.scan:
        ...
        for M in M_LIST:
            ...
            try:
                mat = torch.ones(
                    world_size, M, dtype=getattr(torch, args.dtype)
                ).cuda(local_rank)
                sync_all()
        ...
        if (
            args.dist == "torch"
            and hasattr(torch.distributed, "_all_gather_base")
        ) or (args.dist == "deepspeed" and dist.has_allgather_base):
        ...
        try:
            ...
            if "out of memory" in str(e):
                if dist.get_rank() == 0:
                    print(
                        "WARNING: Ran out of GPU memory. Try to reduce the"
                        " --mem-factor argument!"
                    )
                sync_all()
                return

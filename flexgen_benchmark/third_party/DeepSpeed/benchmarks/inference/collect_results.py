...
parser.add_argument(
    "--version", "-v", type=int, default=0, help="version to be collected"
)
parser.add_argument(
    "--gen-text-n",
    "-n",
...
parser.add_argument(
    "--output", "-o", type=str, default="./results.csv", help="output file"
)
...
def get_generated_text(file_content, gen_text_n):
    ...
    file_content = file_content.replace("\t", " ")
    matches = re.findall(
        r"RESPONSE\s(\d+):\s+[-]{30}\s+(.+?)\s+[-]{30}", file_content
    )
...
if __name__ == "__main__":
    ...
    for root, dirs, files in os.walk(args.results_dir):
        ...
        for name in files:
            ...
            if not branch:
                print(
                    f"WARNING: Could not detect branch for file {file_path},"
                    " skipping"
                )
                continue
            ...
            if not params:
                print(
                    "WARNING: Could not detect benchmark settings for file"
            ...
            if not perf_data:
                print(
                    "WARNING: Could not detect benchmark performance data for"
                    f" file {file_path}"
                )
            ...
            if not generated_text:
                print(
                    "WARNING: Could not detect generated text for file"
                    f" {file_path}"
                )
            ...
            benchmarks_data.append(
                {"branch": branch, **params, **perf_data, **generated_text}
            )

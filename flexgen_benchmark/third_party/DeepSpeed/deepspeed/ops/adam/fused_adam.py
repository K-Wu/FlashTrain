...
class FusedAdam(torch.optim.Optimizer):
    ...
    def step(
        ...
        """
        if any(
            p is not None for p in [grads, output_params, scale, grad_norms]
        ):
            raise RuntimeError(
                "FusedAdam has been updated.  Simply initialize it identically"
                " to torch.optim.Adam, and call step() with no arguments."
            )
        loss = None
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if p.grad.data.is_sparse:
                    raise RuntimeError(
                        "FusedAdam does not support sparse gradients, please"

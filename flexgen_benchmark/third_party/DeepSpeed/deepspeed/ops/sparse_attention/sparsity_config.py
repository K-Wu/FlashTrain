...
class FixedSparsityConfig(SparsityConfig):
    ...
    def __init__(
        ...
        if num_local_blocks % num_global_blocks != 0:
            raise ValueError(
                f"Number of blocks in a local window, {num_local_blocks}, must"
                " be dividable by number of global blocks,"
                f" {num_global_blocks}!"
            )
        ...
        if num_different_global_patterns > 1 and not different_layout_per_head:
            raise ValueError(
                f"Number of different layouts cannot be more than one when you"
                f" have set a single layout for all heads! Set"
                f" different_layout_per_head to True."
            )
        if num_different_global_patterns > (
            ...
            raise ValueError(
                ...
                " number of local window blocks divided by number of global"
        ...
        if global_block_end_indices is not None:
            if len(global_block_indices) != len(global_block_end_indices):
                raise ValueError(
                    ...
                    f" {len(global_block_indices)}, must be same as global"
                    " block end indices length,"
    ...
    def set_random_layout(self, h, layout):
        ...
        if num_blocks < self.num_random_blocks:
            raise ValueError(
                ...
                " smaller than overall number of blocks in a row,"
                f" {num_blocks}!"
            )
        for row in range(0, num_blocks):
            rnd_cols = random.sample(
                range(0, num_blocks), self.num_random_blocks
            )
        ...
        if self.global_block_end_indices is None:
            ...
            for _, (start_idx, end_idx) in enumerate(
                ...
                if start_idx < num_blocks:
                    ...
                    # global columns
                    first_row = (
                        0 if self.attention == "bidirectional" else start_idx
                    )
...
class BigBirdSparsityConfig(SparsityConfig):
    ...
    def set_sliding_window_layout(self, h, layout):
        ...
        if num_blocks < self.num_sliding_window_blocks:
            raise ValueError(
                "Number of sliding window blocks,"

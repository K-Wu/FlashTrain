...
class _sparse_matmul(torch.autograd.Function):
    ...
    def _sdd_matmul(
        ...
        if a_inner != b_inner:
            raise ValueError(
                f"Size of tensor A along the {a_dim} dim ({a_inner}) must"
                f" match size of tensor B along the {b_dim} dim ({b_inner})"
            )
        if a_inner % 16 != 0:
...
class MatMul:
    ...
    def _validate_inputs(self, a, b):
        if a.device != b.device:
            raise ValueError(
                f"Inputs must be on the same device; got {a.device} for tensor"
                f" A and {b.device} for tensor B"
            )
        if not a.is_cuda:
        ...
        if torch.is_autocast_enabled():
            ...
            raise ValueError(
                f"Inputs must be the same dtype; got {a.dtype} for A and"
                f" {b.dtype} for B"
            )
        ...
        if mode != "sdd":
            ...
            if dense_inner != self.dense_inner_size:
                raise ValueError(
                    f"Expected tensor {dense_name} to have size"
                    f" {self.dense_inner_size} at dim"
            ...
            if sparse.shape[-len(self.sparse_shape) :] != self.sparse_shape:
                raise ValueError(
                    ...
                    f" {self.sparse_shape} for argument {sparse_name}, got"
                    f" {sparse.shape}"
                )
        ...
        def add_extra_dims(x):
            ...
            if dims_needed > 0:
                ...
                raise ValueError(
                    "Tensors with more than 4 dimensions are not currently"
                    " supported"
                )

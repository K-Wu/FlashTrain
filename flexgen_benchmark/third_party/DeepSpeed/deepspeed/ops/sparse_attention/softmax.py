...
@triton.heuristics(
    {"num_warps": lambda *args, **meta: num_warps(args[6] * meta["BLOCK"])}
)
@triton.heuristics(
    {"TN": lambda *args, **meta: next_power_of_2(args[6] * meta["BLOCK"])}
)
@triton.jit
def _forward(
    X,
...
class Softmax:
    ...
    def make_lut(self, device):
        ...
        if key not in self.lut_cache:
            self.lut_cache[key] = _sparse_softmax.make_lut(

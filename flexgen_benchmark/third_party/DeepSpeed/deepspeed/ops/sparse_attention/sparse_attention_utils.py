...
class SparseAttentionUtils:
    ...
    def extend_position_embedding(model, max_position):
        ...
        if hasattr(model, "bert"):
            ...
            raise ValueError(
                'Please extend "extend_position_embedding" function to support'
                ' your model type. It currently only supports "bert" &'
                ' "roberta"!'
            )

        model.config.max_position_embeddings = max_position
        print(
            "Extended position embeddings to"
            f" {original_max_position * extend_multiples}"
        )
    ...
    def replace_model_self_attention_with_sparse_self_attention(
        ...
        if hasattr(model, "bert"):
            ...
            raise ValueError(
                "Please extend"
                ' "update_model_self_attention_to_sparse_self_attention"'
                " function to support                                     "
                ' your model type. It currently only supports "bert" &'
                ' "roberta"!'
            )
    ...
    def pad_to_block_size(
        ...
        if pad_len > 0:
            if inputs_embeds is not None:
                pad_input_ids = inputs_embeds.new_full(
                    (batch_size, pad_len), pad_token_id, dtype=torch.long
                )
                pad_inputs_embeds = model_embeddings(pad_input_ids)
                inputs_embeds = torch.cat(
                    [inputs_embeds, pad_inputs_embeds], dim=-2
                )
            ...
            if position_ids is not None:
                ...
                position_ids = F.pad(
                    position_ids, (0, pad_len), value=pad_token_id
                )

...
class SparseSelfAttention(nn.Module):
    ...
    def forward(
        ...
        assert query.dtype == torch.half, (
            "sparse attention only supports training in fp16 currently, please"
            " file a github issue if you need fp32 support"
        )
        ...
        if key_padding_mask is not None:
            key_padding_mask = self.transpose_mask_for_sparse(
                query.dtype, key_padding_mask, is_key_padding_mask=True
            )

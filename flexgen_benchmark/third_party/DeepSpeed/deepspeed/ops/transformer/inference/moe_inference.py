...
class DeepSpeedMoEInferenceConfig(DeepSpeedInferenceConfig):
    ...
    def __init__(
        ...
        super(DeepSpeedMoEInferenceConfig, self).__init__(
            hidden_size,
...
class DeepSpeedMoEMLP(nn.Module):
    def __init__(
        ...
        self.inter_w = nn.Parameter(
            torch.Tensor(self.config.hidden_size, interm_size)
        )
        self.inter_b = nn.Parameter(torch.Tensor(interm_size))
        self.output_w = nn.Parameter(
            torch.Tensor((interm_size), self.config.hidden_size)
        )
...
class DeepSpeedMoEInference(nn.Module):
    ...
    def __init__(
            ...
            if hasattr(op_builder, "InferenceSpecializedBuilder"):
                ...
                if builder.is_compatible():
                    ...
                    inference_cuda_module = (
                        op_builder.InferenceBuilder().load()
                    )
            else:
        ...
        if config.mlp_type == "residual":
            ...
            self.res_coef = nn.Parameter(
                torch.Tensor(self.config.hidden_size, 2)
            )
            self.coef_func = (
                inference_cuda_module.softmax_fp16
                if self.config.fp16 or self.config.q_int8
                else inference_cuda_module.softmax_fp32
            )
        ...
        print(
            "DeepSpeed MoE Transformer Inference config is ",
        ...
        self.einsum_sec_sm_ecm = (
            inference_cuda_module.einsum_sec_sm_ecm_fp16
            if self.config.fp16 or self.config.q_int8
            else inference_cuda_module.einsum_sec_sm_ecm_fp32
        )

    def res_coef_func(self, inp, async_op):
        inp = self.vector_matmul_func(inp, self.res_coef, async_op)
        return self.coef_func(
            inp, torch.empty(1), False, False, False, 256, async_op
        )
    ...
    def expert_exec(self, dispatched_input):
        ...
        for chunk, expert in zip(chunks, range(len(self.mlp))):
            expert_outputs[expert] = self.mlp[expert](
                chunk.view(
                    -1, dispatched_input.shape[-2], dispatched_input.shape[-1]
                )
            )
    ...
    def _alltoall(self, dispatched_attention):
        if dist.get_world_size(group=self.ep_group) > 1:
            ...
            dist.all_to_all_single(
                dispatched_input, dispatched_attention, group=self.ep_group
            )
            return dispatched_input
        else:
            return dispatched_attention

    def scale_expert_output(
        self, attention_output, expert_output, combined_weights
    ):
    ...
    def forward(
        ...
        if (
            self.config.fp16 or self.config.q_int8
        ...
        with torch.no_grad():
            ...
            attention_output = self.ds_layernorm(
                residual_add, self.attn_nw, self.attn_nb, self.config.epsilon
            )

            if self.config.mlp_type == "residual":
                res_mlp_out = self.res_mlp(attention_output, async_op=True)
                res_coef_out = self.res_coef_func(
                    attention_output, async_op=True
                )
            ...
            if self.expert_mp_group is not None:
                ...
                tensor_list[
                    dist.get_rank(group=self.expert_mp_group)
                ] = attention_output
                dist.all_gather(
                    tensor_list, attention_output, group=self.expert_mp_group
                )
            ...
            output = self.scale_expert_output(
                attention_output, expert_output, combined_weights
            )
            ################################################
            ...
            if self.config.mlp_type == "residual":
                inference_cuda_module.moe_res_matmul(
                    res_mlp_out, res_coef_out, output
                )

            output = self.bias_residual_func(
                output, residual_add, torch.empty(1)
            )

            if not self.config.pre_layer_norm:
                output = self.ds_layernorm(

...
class DeepSpeedSelfAttentionFunction(Function):
    ...
    def forward(
        ...
        def _transpose_for_scores(x, key=False, reshape=False):
            attention_head_size = (
                x.shape[-1] // num_attention_heads_per_partition
            )
        ...
        def backup_attention(
            mixed_x_layer, layer_past, alibi, input_mask, norm_factor
        ):
            alibi = alibi.to(torch.cuda.current_device())
            head_dim = (
                hidden_size_per_partition // num_attention_heads_per_partition
            )
            ...
            key_layer = (
                ...
                .reshape(output_size[0] * output_size[1], output_size[3], -1)
                .transpose(-1, -2)
            )
        ...
        def compute_attention(qkv_out, input_mask):
            ...
            head_size = (
                qkv_out.shape[-1] // 3 // num_attention_heads_per_partition
            )
            if no_masking:
                input_mask = torch.empty(1)
            if merge_count > 0 and config.q_int8:
                split_dim = qkv_out.dim() - 1
                qkv_split = torch.split(
                    qkv_out,
                ...
                if layer_past is None:
                    ...
                    attn_key_value = score_context_func(
                        ...
                        (
                            key_layer
                            if unfused_mode
                            else past_key.type_as(key_layer)
                        ),  # noqa: F821
                        key_layer,
                        ((1 - input_mask).half() * minus_inf)
                        if input_mask.dtype == torch.int64
                        ...
                        (
                            value_layer
                            if unfused_mode
                            else past_value.type_as(value_layer)
                        ),  # noqa: F821
                        value_layer,
                        num_attention_heads_per_partition,
                        (1 / norm_factor if config.scale_attention else 1.0),
                ...
                return (
                    ...
                    presents[0],
                    presents[1],
                )  # atten_output, key_layer, value_layer
            else:
                ...
                if config.bigscience_bloom:
                    ...
                    if alibi is not None:
                        ...
                        offset = (
                            dist.get_rank() * batch_heads
                            if dist.is_initialized()
                            else 0
                        )
                        sliced_alibi = alibi[
                            offset : batch_heads + offset, :, :
                        ]
        ...
        def selfAttention_fp():
            ...
            output = vector_matmul_func(
                context_layer, attn_ow, False, attn_ow.scale, config.q_int8
            )
...
class DeepSpeedSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        self.attn_qkvb = nn.Parameter(
            torch.empty(
                qkv_size_per_partition, dtype=data_type_fp, device=device
            ),
            requires_grad=False,
        )
        ...
        self.attn_ob = nn.Parameter(
            torch.empty(
                self.config.hidden_size, dtype=data_type_fp, device=device
            ),
            requires_grad=False,
        )

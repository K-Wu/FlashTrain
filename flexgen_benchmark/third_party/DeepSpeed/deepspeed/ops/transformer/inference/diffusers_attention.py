...
def load_triton_flash_attn():
    ...
    try:
        ...
        raise ImportError(
            "Please install triton 2.0+ or `pip install deepspeed[sd]`"
        )
    ...
    def forward(
        ...
        def selfAttention_fp(input, context, input_mask):
            ...
            if do_flash_attn and context == None:
                ...
                context_layer = triton_flash_attn_kernel(
                    qkv_out[0],
                    qkv_out[1],
                    qkv_out[2],
                    scale,
                ...
                query, key, value = inference_cuda_module.pad_transform_fp16(
                    query, key, value, config.heads, do_flash_attn
                )
                attention_scores = (
                    torch.matmul(query, key.transpose(-1, -2)) * scale
                ).softmax(dim=-1)
                context_layer = _transpose_for_context(
                    torch.matmul(attention_scores, value)
                )
...
class DeepSpeedDiffusersAttention(nn.Module):
    ...
    def __init__(
        ...
        device = (
            torch.cuda.current_device() if config.bigscience_bloom else "cpu"
        )
        ...
        if DeepSpeedDiffusersAttention.layer_id == 1:
            log_dist(
                f"DeepSpeed-Attention config: {self.config.__dict__}", [0]
            )
        ...
        self.allocate_workspace = (
            inference_cuda_module.allocate_workspace_fp32
            if not (config.fp16)
            else inference_cuda_module.allocate_workspace_fp16
        )

...
class DeepSpeedDiffusersTransformerBlock(nn.Module):
    def __init__(
        ...
        self.quantizer = module_inject.GroupQuantizer(
            q_int8=config.int8_quantization
        )
        ...
        self.ff1_w = self.quantizer.quantize(
            nn.Parameter(
                equivalent_module.ff.net[0].proj.weight.data,
                requires_grad=False,
            )
        )
        self.ff1_b = nn.Parameter(
            equivalent_module.ff.net[0].proj.bias.data, requires_grad=False
        )
        self.ff2_w = self.quantizer.quantize(
            nn.Parameter(
                equivalent_module.ff.net[2].weight.data, requires_grad=False
            )
        )
        self.ff2_b = nn.Parameter(
            equivalent_module.ff.net[2].bias.data, requires_grad=False
        )

        self.norm1_g = nn.Parameter(
            equivalent_module.norm1.weight.data, requires_grad=False
        )
        self.norm1_b = nn.Parameter(
            equivalent_module.norm1.bias.data, requires_grad=False
        )
        self.norm1_eps = equivalent_module.norm1.eps

        self.norm2_g = nn.Parameter(
            equivalent_module.norm2.weight.data, requires_grad=False
        )
        self.norm2_b = nn.Parameter(
            equivalent_module.norm2.bias.data, requires_grad=False
        )
        self.norm2_eps = equivalent_module.norm2.eps

        self.norm3_g = nn.Parameter(
            equivalent_module.norm3.weight.data, requires_grad=False
        )
        self.norm3_b = nn.Parameter(
            equivalent_module.norm3.bias.data, requires_grad=False
        )
        ...
        if isinstance(self.attn_1, DeepSpeedDiffusersAttention):
            ...
            self.attn_1_bias = nn.Paramaeter(
                torch.zeros_like(self.norm2_g), requires_grad=False
            )
        ...
        if isinstance(self.attn_2, DeepSpeedDiffusersAttention):
            ...
            self.attn_2_bias = nn.Paramaeter(
                torch.zeros_like(self.norm3_g), requires_grad=False
            )
    ...
    def forward(self, hidden_states, context=None, timestep=None):
        out_norm_1 = self.transformer_cuda_module.layer_norm(
            hidden_states, self.norm1_g, self.norm1_b, self.norm1_eps
        )
        ...
        (
            out_norm_2,
            out_attn_1,
        ) = self.transformer_cuda_module.layer_norm_residual_store(
            out_attn_1,
            self.attn_1_bias,
            hidden_states,
            self.norm2_g,
        ...
        out_geglu = self.transformer_cuda_module.bias_geglu(
            out_ff1, self.ff1_b
        )

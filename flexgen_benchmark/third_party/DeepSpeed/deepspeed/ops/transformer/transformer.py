...
class DeepSpeedTransformerConfig(TransformerConfig):
    ...
    def __init__(
        ...
        super(DeepSpeedTransformerConfig, self).__init__(
            batch_size,
            hidden_size,
        ...
        self.gelu_checkpoint = (
            gelu_checkpoint  # True: if higher batch size is required
        )
...
class DeepSpeedTransformerFunction(Function):
    ...
    def forward(
        ...
        cuda_module = (
            ...
            else transformer_cuda_module
        )
        forward_func = (
            cuda_module.forward_fp16
            if config.fp16
            else cuda_module.forward_fp32
        )
    ...
    def backward(ctx, grad_output):
        ...
        backward_func = (
            cuda_module.backward_fp16
            if ctx.config.fp16
            else cuda_module.backward_fp32
        )
        ...
        (
            ...
            (
                ...
                if (
                    ...
                    and ctx.config.normalize_invertible
                )
                else output
            ),
            (
                ...
                if (
                    ctx.config.pre_layer_norm
                    or not ctx.config.normalize_invertible
                )
                else input
            ),
            ctx.qkv_tf,
            ...
            (
                ctx.soft_inp
                if ctx.config.attn_dropout_checkpoint
                else ctx.ctx_bufB
            ),
            ctx.attn_o_inp,
...
class DeepSpeedTransformerLayer(nn.Module):
    ...
    def __init__(self, config, initial_weights=None, initial_biases=None):
        ...
        DeepSpeedTransformerLayer.layer_id = (
        ...
        if initial_weights is None and initial_biases is None:
            ...
            self.attn_ow = nn.Parameter(
                torch.Tensor(self.config.hidden_size, self.config.hidden_size)
            )
        ...
        if (
            stochastic_transformer_cuda_module is None
            and self.config.stochastic_mode
        ):
            stochastic_transformer_cuda_module = (
    ...
    def init_transformer_weights(self, adjust_init_range=False):
        ...
        if adjust_init_range and self.config.local_rank == 0:
            ...
            output_std = self.config.initializer_range / math.sqrt(
                2.0 * num_layers
            )

        self.attn_qkvw.data.normal_(

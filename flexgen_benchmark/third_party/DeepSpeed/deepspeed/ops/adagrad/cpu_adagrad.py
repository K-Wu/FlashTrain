...
class DeepSpeedCPUAdagrad(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        default_args = dict(
            lr=lr, eps=eps, weight_decay=weight_decay, amsgrad=amsgrad
        )
        ...
        self.ds_opt_adagrad.create_adagrad(
            self.opt_id, lr, eps, weight_decay, should_log_le("info")
        )
        ...
        for group_id, group in enumerate(self.param_groups):
            for param_id, p in enumerate(group["params"]):
                ...
                assert p.device == device, (
                    f"CPUAdagrad param is on {p.device} and must be 'cpu',"
                    " make sure you enabled 'offload_optimizer': 'cpu' in"
                    " your ZeRO config."
                )
                ...
                if len(state) == 0:
                    ...
                    state_dtype = (
                        torch.float if self.fp32_optimizer_states else p.dtype
                    )

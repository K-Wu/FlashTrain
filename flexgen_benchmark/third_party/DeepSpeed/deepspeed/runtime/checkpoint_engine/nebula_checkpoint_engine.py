...
class NebulaCheckpointEngine(CheckpointEngine):
    ...
    def load(self, path: str, map_location=None):
        ...
        if not self.enable_nebula_load and first_load_flag:
            self.tag_flag = tag
            logger.info(
                "[Nebula] Disable nebula load. Loading checkpoint from"
                f" {path} ..."
            )
            partition = torch.load(path, map_location=map_location)
            logger.info(
                "[Nebula] Disable nebula load. Loaded checkpoint from"
                f" {path} ."
            )
            return partition
        ...
        partititon_name = os.path.basename(path)
        logger.info(
            f"[Nebula] Loading {path} under tag {tag} from nebula path"
            f" {self.nebula_load_path}..."
        )
        ...
        if checkpoint is None or (
            checkpoint is not None and checkpoint.tag == ""
        ):
            logger.info(
                f"Unable to find valid checkpoint tag:{tag} from Nebula, try"
                " to get latest checkpoint again from nebula"
                f" {self.nebula_load_path} path!"
            )
            # nebula tier3 latest
            checkpoint = torch_nebula.get_latest_checkpoint(
                persist_path=self.nebula_load_path
            ...
            if checkpoint is None or (
                ...
                logger.info(
                    f"Unable to find latest checkpoint from Nebula tier3, try"
                    f" to get latest checkpoint again from nebula tier1 path!"
                )
                # nebula tier1 latest
                checkpoint = torch_nebula.get_latest_checkpoint()
                logger.warning(
                    "Unable to find valid checkpoint from Nebula under"
                    f" tag:{tag}."
                )
                return None
        ...
        logger.info(
            f"[Nebula] Loaded {path} under tag {tag} from"
            f" {self.nebula_load_path}."
        )
        return partition
    ...
    def commit(self, tag):
        ...
        logger.info(
            f"[Nebula] all files for {tag} are saved in tier1. It is ready to"
            " start persisting"
        )
        commit_rls = self.checkpoint.commit()
        if not commit_rls:
            logger.error(
                f"[Nebula] failed to commit the checkpoint, please check the"
                f" log."
            )
            return False
        return commit_rls

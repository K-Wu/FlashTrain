...
class SDLoaderFactory:
    ...
    def get_sd_loader_json(json_file, checkpoint_engine):
        ...
        return SDLoaderFactory.get_sd_loader(
            ckpt_list, checkpoint_engine, sd_type, version
        )
...
class SDLoaderBase(ABC):
    ...
    def get_merge_state_dicts(self, mp_world_size, mp_rank):
        ...
        ckpt_list = [
            self.ckpt_list[i]
            for i in range(
                num_to_merge * mp_rank, num_to_merge * (mp_rank + 1)
            )
        ]

        logger.info(f"mp_rank: {mp_rank}, ckpt_list: {ckpt_list}")
        sd_list = [
            self.checkpoint_engine.load(
    ...
    def get_split_state_dict(self, mp_world_size, mp_rank):
        ...
        logger.info(
            f"mp_rank: {mp_rank}, ckpt_list: {self.ckpt_list[ckpt_index]},"
            f" offset: {ckpt_offset}"
        )

        sd = self.checkpoint_engine.load(
            self.ckpt_list[ckpt_index],
    ...
    def check_ckpt_list(self):
        ...
        sd = self.checkpoint_engine.load(
            self.ckpt_list[0], map_location=lambda storage, loc: storage
        )
...
class MegatronSDLoader(SDLoaderBase):
    ...
    def merge_query_key_value(self, param_list, ckpt_ver):
        ...
        if ckpt_ver == 0:
            ...
            split_tensors = [
                torch.split(param, size_qkv, dim=0) for param in param_list
            ]
    ...
    def split_state_dict(
        ...
        for key in client_sd.keys():
            ...
            if (
                ...
                if quantize:
                    q_vals = quantizer.Quantize(
                        [value], quantize_bits, groups, key
                    )
                    value = q_vals[0]
                new_client_sd[key] = torch.split(value, split_size, dim=1)[
                    ckpt_offset
                ]
            elif "attention.query_key_value" in key:
                if quantize and "attention.query_key_value.weight" in key:
                ...
                if quantize and "mlp.dense_h_to_4h.weight" in key:
                    ...
                    value = q_vals[0]
                new_client_sd[key] = torch.split(value, split_size, dim=0)[
                    ckpt_offset
                ]
            else:
    ...
    def sanity_check(self, ckpt_file_name):
        ...
        sd = self.checkpoint_engine.load(
            ckpt_file_name, map_location=lambda storage, loc: storage
        )
    ...
    def get_checkpoint_version(self, state_dict):
        ...
        return (
            ...
            else state_dict.get("checkpoint_version", 0)
        )

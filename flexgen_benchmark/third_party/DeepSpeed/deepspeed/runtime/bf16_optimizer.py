...
class BF16_Optimizer(ZeROOptimizer):
    ...
    def _setup_for_real_optimizer(self):
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            bf16_dp_partitions = [
                self.bf16_groups_flat[i].narrow(
                    0, dp_index * partition_size, partition_size
                )
            ...
            self.fp32_groups_gradients_flat.append(
                torch.zeros_like(self.bf16_groups_flat[i], dtype=torch.float32)
            )
            ...
            self.fp32_groups_actual_gradients_flat.append(
                torch.narrow(
                    self.fp32_groups_gradients_flat[i],
                    0,
                    0,
            ...
            self.fp32_groups_has_gradients.append(
                [False] * len(self.bf16_groups[i])
            )
            ...
            if (
                ...
                padding = (
                    self.bf16_groups_flat[i].numel() - length_without_padding
                )
            else:
                padding = 0
    ...
    def step(self, closure=None):
        if closure is not None:
            raise NotImplementedError(
                f"{self.__class__} does not support closure."
            )
    ...
    def backward(
        self, loss, update_hp_grads=True, clear_lp_grads=False, **bwd_kwargs
    ):
        ...
        for i, group in enumerate(self.bf16_groups):
            for j, lp in enumerate(group):
                ...
                assert hp_grad is not None, (
                    "high precision param has no gradient, lp param_id ="
                    f" {id(lp)} group_info = [{i}][{j}]"
                )

                hp_grad.data.add_(
                    lp.grad.data.to(hp_grad.dtype).view(hp_grad.shape)
                )
                lp._hp_grad = hp_grad
        ...
        for i, group in enumerate(self.bf16_groups):
            for j, lp in enumerate(group):
                if not for_clipping:
                    ...
                    if not (
                        tensor_mp_rank == 0 or is_model_parallel_parameter(lp)
                    ):
                        continue
    ...
    def update_lp_params(self):
        for i, (bf16_partitions, fp32_partition) in enumerate(
            zip(self.bf16_partitioned_groups, self.fp32_groups_flat_partition)
        ):
            partition_id = dist.get_rank(group=self.real_dp_process_group[i])
    ...
    def _restore_from_bit16_weights(self):
        for i, group in enumerate(self.bf16_groups):
            ...
            for bf16_partitions, fp32_partition in zip(
                self.bf16_partitioned_groups, self.fp32_groups_flat_partition
            ):

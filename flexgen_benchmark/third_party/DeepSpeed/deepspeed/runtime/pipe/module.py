...
class PipelineModule(nn.Module):
        ...
        if topology:
            ...
            if topology is None:
                if self.world_size % self.num_stages != 0:
                    raise RuntimeError(
                        f"num_stages ({self.num_stages}) must divide"
                        f" distributed world size ({self.world_size})"
                    )
    ...
    def _build(self):
        ...
        for local_idx, layer in enumerate(
            specs[self._local_start : self._local_stop]
        ):
    ...
    def _find_layer_type(self, layername):
        ...
        if len(idxs) == 0:
            raise RuntimeError(
                f"Partitioning '{layername}' found no valid layers to"
                " partition."
            )
        return idxs
    ...
    def forward(self, forward_input):
        ...
        if self.activation_checkpoint_interval == 0:
            ...
            for start_idx in range(
                0, num_layers, self.activation_checkpoint_interval
            ):
                end_idx = min(
    ...
    def ckpt_prefix(self, checkpoints_path, tag):
        ...
        axes = [
            a for a in self._grid._topo.get_axis_names() if a not in omit_dims
        ]
        for dim in axes:
            rank = getattr(
                self._grid._topo.get_coord(rank=self.global_rank), dim
            )
    ...
    def _is_checkpointable(self, funcs):
        ...
        if self.__class__.__name__ in ("GPTModelPipe", "GPT2ModelPipe"):
            return all(
                "ParallelTransformerLayerPipe" in f.__class__.__name__
                for f in funcs
            )
        if self.checkpointable_layers is not None:
            return all(
                f.__class__.__name__ in self.checkpointable_layers
                for f in funcs
            )

        params = [
            f.parameters() for f in funcs if isinstance(f, torch.nn.Module)
        ]

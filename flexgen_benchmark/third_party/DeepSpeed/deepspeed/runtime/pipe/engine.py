...
class PipelineEngine(DeepSpeedEngine):
    ...
    def __init__(self, has_bool_tensors=False, *super_args, **super_kwargs):
        ...
        if self.module.tied_comms:
            ...
            for key, d in self.module.tied_comms.items():
                if self.global_rank != min(d["ranks"]):
                    tied_params += sum(
                        p.numel() for p in d["module"].parameters()
                    )
        ...
        self.dp_group_loss = torch.tensor(0.0, requires_grad=False).to(
            self.device
        )
    ...
    def train_batch(self, data_iter=None):
        ...
        if self.global_steps % self.steps_per_print() == 0:
            if self.global_rank == 0:
                elapsed = (
                    self.timers("train_batch").elapsed(reset=True) / 1000.0
                )
    ...
    def eval_batch(
        ...
        if self.is_last_stage():
            eval_output = self._reduce_outputs(
                self.fwd_outputs, reduce=reduce_output
            )
        ...
        if reduce.lower() == "avg":
            ...
            if reduce_dp and self.is_data_parallel:
                if torch.is_tensor(reduced):
                    dist.all_reduce(
                        reduced, group=self.mpu.get_data_parallel_group()
                    )
            ...
            raise NotImplementedError(
                f"reduction type {reduce} not supported."
            )
    ...
    def _aggregate_total_loss(self):
        ...
        if self.is_last_stage():
            ...
            losses = torch.Tensor([self.dp_group_loss, agg_loss]).to(
                self.device
            )
            dist.broadcast(
                tensor=losses,
    ...
    def log_for_device(self, *msg):
        if LOG_STAGE == self.stage_id or LOG_STAGE == -1:
            if (
                DATA_PARALLEL_ID == self.grid.data_parallel_id
                or DATA_PARALLEL_ID == -1
            ):
                print(
                    (
    ...
    def _exec_forward_pass(self, buffer_id):
        ...
        if isinstance(self.pipe_buffers["inputs"][buffer_id], tuple):
            inputs = tuple(
                t.clone() for t in self.pipe_buffers["inputs"][buffer_id]
            )
        else:
    ...
    def _exec_backward_pass(self, buffer_id):
        ...
        if self.is_pipe_partitioned:
            if self.is_grad_partitioned:
                ...
                self.pipe_buffers["output_tensors"][
                    buffer_id
                ].data = part_output.full()
                outputs = (
    ...
    def _send_tensor_meta(self, buffer, recv_stage):
        ...
        if isinstance(buffer, torch.Tensor):
            ...
            send_ndims = torch.LongTensor(data=[len(buffer.size())]).to(
                self.device
            )
            ...
            for idx, tensor in enumerate(buffer):
                ...
                send_dtype = torch.LongTensor(
                    data=[self.DTYPE_TO_ID[tensor.dtype]]
                ).to(self.device)
    ...
    def _recv_tensor_meta(self, send_stage):
        ...
        # List or tuple of tensors
            ...
            for idx in range(num_tensors):
                ...
                recv_shape = torch.LongTensor([1] * recv_ndims).to(self.device)
                p2p.recv(recv_shape, send_stage)
                recv_shapes_and_dtypes.append(
                    (recv_shape.tolist(), recv_dtype)
                )

            buffers = self._allocate_buffers(
                recv_shapes_and_dtypes, num_buffers=1
            )[0]
    ...
    def _exec_send_activations(self, buffer_id):
        ...
        if isinstance(outputs, torch.Tensor):
            ...
            raise NotImplementedError(
                f"Could not send output of type {type(outputs)}"
            )
    ...
    def _allocate_buffers(
        self, shapes_and_dtypes, requires_grad=False, num_buffers=-1
    ):
        buffers = []
        if num_buffers == -1:
            num_buffers = self.num_pipe_buffers
    ...
    def mem_status(self, msg, print_rank=-1, reset_max=False):
        ...
        print(
            ...
            (
                ...
                f" (delta={delta_alloced:0.4f}GB max={max_alloced:0.4f}GB)"
                f" current cache={new_cached:0.4f}GB"
                f" (delta={delta_cached:0.4f}GB max={max_cached:0.4f}GB)"
            ),
        )
    ...
    def module_state_dict(self):
        ...
        assert self._curr_ckpt_path is not None, (
            "PipelineEngine expects module_state_dict() to be called from"
            " save_checkpoint()"
        )
        ...
        for step_cmds in pipe_schedule:
            ...
            for cmd in step_cmds:
                if type(cmd) not in self._INSTRUCTION_MAP:
                    raise RuntimeError(
                        f"{self.__class__.__name__} does not understand"
                        f" instruction {repr(cmd)}"
                    )

...
def get_pld_enabled(param_dict):
    if PROGRESSIVE_LAYER_DROP in param_dict.keys():
        return get_scalar_param(
            param_dict[PROGRESSIVE_LAYER_DROP],
            PLD_ENABLED,
            PLD_ENABLED_DEFAULT,
        )
    else:
...
def get_amp_enabled(param_dict):
    if AMP in param_dict.keys():
        return get_scalar_param(
            param_dict[AMP], AMP_ENABLED, AMP_ENABLED_DEFAULT
        )
    else:
        return False
...
def get_fp16_enabled(param_dict):
    if FP16 in param_dict.keys():
        return get_scalar_param(
            param_dict[FP16], FP16_ENABLED, FP16_ENABLED_DEFAULT
        )
    else:
        return False
...
def get_fp16_auto_cast(param_dict):
    if get_fp16_enabled(param_dict):
        return get_scalar_param(
            param_dict[FP16], FP16_AUTO_CAST, FP16_AUTO_CAST_DEFAULT
        )
...
def get_sparse_gradients_enabled(param_dict):
    return get_scalar_param(
        param_dict, SPARSE_GRADIENTS, SPARSE_GRADIENTS_DEFAULT
    )
...
def get_communication_data_type(param_dict):
    ...
    raise ValueError(
        "Invalid communication_data_type. Supported data types: ['fp16',"
        f" 'bfp16', 'fp32']. Got: {val}"
    )


def get_prescale_gradients(param_dict):
    return get_scalar_param(
        param_dict, PRESCALE_GRADIENTS, PRESCALE_GRADIENTS_DEFAULT
    )
...
def get_steps_per_print(param_dict):
    return get_scalar_param(
        param_dict, STEPS_PER_PRINT, STEPS_PER_PRINT_DEFAULT
    )


def get_disable_allgather(param_dict):
    return get_scalar_param(
        param_dict, DISABLE_ALLGATHER, DISABLE_ALLGATHER_DEFAULT
    )
...
def get_gradient_clipping(param_dict):
    return get_scalar_param(
        param_dict, GRADIENT_CLIPPING, GRADIENT_CLIPPING_DEFAULT
    )
...
def get_optimizer_gradient_clipping(param_dict):
    ...
    if (
        ...
        and MAX_GRAD_NORM in optimizer_params.keys()
    ):
...
def get_optimizer_legacy_fusion(param_dict):
    if (
        OPTIMIZER in param_dict.keys()
        and LEGACY_FUSION in param_dict[OPTIMIZER].keys()
    ):
...
def get_train_batch_size(param_dict):
    return get_scalar_param(
        param_dict, TRAIN_BATCH_SIZE, TRAIN_BATCH_SIZE_DEFAULT
    )
...
def get_memory_breakdown(param_dict):
    return get_scalar_param(
        param_dict, MEMORY_BREAKDOWN, MEMORY_BREAKDOWN_DEFAULT
    )
...
def get_eigenvalue_config(param_dict):
    if get_quantize_enabled(param_dict):
        ...
        assert not get_eigenvalue_enabled(
            param_dict
        ), "Eigenvalue based MoQ is temporarily disabled"
        return (
...
def get_checkpoint_tag_validation_mode(checkpoint_params):
    ...
    if tag_validation_mode in CHECKPOINT_TAG_VALIDATION_MODES:
        ...
        raise DeepSpeedConfigError(
            "Checkpoint config contains invalid tag_validation value of"
            f" {tag_validation_mode}, expecting one of"
            f" {CHECKPOINT_TAG_VALIDATION_MODES}"
        )
...
def get_checkpoint_parallel_write_pipeline(checkpoint_params):
    ...
    if par_write_pipeline in [True, False]:
        ...
        raise DeepSpeedConfigError(
            "checkpoint::parallel_write::pipeline_stage value of"
            f" '{par_write_pipeline}' is invalid, expecting: true or false"
        )
...
class DeepSpeedConfig(object):
    def __init__(self, config: Union[str, dict], mpu=None):
        ...
        if isinstance(config, dict):
            ...
            try:
                config_decoded = base64.urlsafe_b64decode(config).decode(
                    "utf-8"
                )
                self._param_dict = json.loads(config_decoded)
            except (UnicodeDecodeError, AttributeError):
                raise ValueError(
                    "Expected a string path to an existing deepspeed config,"
        ...
        if self.elasticity_enabled:
            ...
            self.elastic_model_parallel_size = elastic_dict.get(
                MODEL_PARLLEL_SIZE, MODEL_PARLLEL_SIZE_DEFAULT
            )
            if self.elastic_model_parallel_size < 1:
                raise ElasticityConfigError(
                    "Model-Parallel size cannot be less than 1, given"
                    f" model-parallel size: {self.elastic_model_parallel_size}"
                )

            self.num_gpus_per_node = elastic_dict.get(
            ...
            if not ignore_non_elastic_batch_info:
                ...
                if any(map(lambda t: t in self._param_dict, batch_params)):
                    raise ElasticityConfigError(
                        ...
                        f" your ds_config ({TRAIN_BATCH_SIZE},"
                        f" {TRAIN_MICRO_BATCH_SIZE_PER_GPU}, and/or"
                        f" {GRADIENT_ACCUMULATION_STEPS}). These parameters"
                        " *will not be used* since elastic training is"
                        " enabled, which takes control of these parameters."
                        " If you want to suppress this error (the parameters"
                        " will be silently ignored) please set"
                        f" {IGNORE_NON_ELASTIC_BATCH_INFO}':true in your"
                        " elasticity config."
                    )
    ...
    def _initialize_params(self, param_dict):
        ...
        self.gradient_predivide_factor = get_gradient_predivide_factor(
            param_dict
        )
        self.sparse_gradients_enabled = get_sparse_gradients_enabled(
            param_dict
        )
        ...
        self.wall_clock_breakdown = (
            get_wall_clock_breakdown(param_dict)
        ...
        self.grad_accum_dtype = data_types_params.get(
            GRAD_ACCUM_DTYPE, GRAD_ACCUM_DTYPE_DEFAULT
        )

        par_write_pipe = get_checkpoint_parallel_write_pipeline(
            checkpoint_params
        )
    ...
    def _do_error_check(self):
        ...
        if self.fp16_master_weights_and_gradients:
            assert (
                ...
                "Fp16_master_weights_and_grads is only supported with ZeRO"
                " Stage 2 for now."
            )
    ...
    def _do_warning_check(self):
        ...
        vocabulary_size = self._param_dict.get(
            VOCABULARY_SIZE, VOCABULARY_SIZE_DEFAULT
        )
        if vocabulary_size and vocabulary_size % TENSOR_CORE_ALIGN_SIZE != 0:
            logger.warning(
                "DeepSpeedConfig: vocabulary size {} is not aligned to {}, may"
                " import tensor core utilization.".format(
            ...
            if fp16_enabled:
                ...
                if self.global_rank == 0:
                    logger.warning(
                        ...
                        " permit MAX_GRAD_NORM ({}) > 0, setting to zero"

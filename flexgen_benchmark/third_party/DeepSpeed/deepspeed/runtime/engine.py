...
class DeepSpeedEngine(Module):
    ...
    def __init__(
        ...
        if supported_torch_version:
            ...
            if dist_init_required is False:
                assert dist.is_initialized() is True, (
                    "Torch distributed not initialized. Please set"
                    " dist_init_required to True or initialize before calling"
        ...
        see_memory_usage(
            f"DeepSpeed Engine: After configure distributed model"
        )
        ...
        log_dist(
            (
                "DeepSpeed Flops Profiler Enabled:"
                f" {self.flops_profiler_enabled()}"
            ),
            ranks=[0],
        )
    ...
    def _get_model_parameters(self):
        if self.autotuning_profile_model_info():
            ...
            for p in self.module.parameters():
                ...
                if hasattr(
                    p, "ds_tensor"
                ):  # if the parameter is partitioned in zero 3
                    n += p.ds_numel
    ...
    def set_train_batch_size(self, train_batch_size):
        ...
        if (
            ...
            raise ValueError(
                f"Train batch size must be divisible by micro-batch data"
                f" parallelism"
            )
    ...
    def _configure_checkpointing(self, dist_init_required):
        ...
        if self._config is not None and self._config.nebula_config.enabled:
            try:
                ...
                logger.error(
                    "No torch_nebula was found! Will fall back to torch.save."
                    f" Details: {err}"
                )
                self.checkpoint_engine = TorchCheckpointEngine()
    ...
    def _configure_with_arguments(self, args, mpu):
        ...
        if "OMPI_COMM_WORLD_LOCAL_RANK" in os.environ:
            ...
            assert ompi_local_rank == local_rank, (
                f"LOCAL_RANK ({local_rank}) != OMPI_COMM_WORLD_LOCAL_RANK"
                f" ({ompi_local_rank}), not sure how to proceed as we're"
                " seeing conflicting local rank info."
            )
    ...
    def _do_args_sanity_check(self, args):
        if (
            ...
            and args.deepscale_config is not None
        ):
            logger.warning(
            ...
            if hasattr(args, "deepspeed_config"):
                assert args.deepspeed_config is None, (
                    "Not sure how to proceed, we were given both a"
                    " deepscale_config and deepspeed_config"
                )
            args.deepspeed_config = args.deepscale_config

        assert (
            "LOCAL_RANK" in os.environ
            or "OMPI_COMM_WORLD_LOCAL_RANK" in os.environ
        ), (
            "DeepSpeed requires the LOCAL_RANK environment variable, it is set"
            " by the deepspeed launcher, deepspeed.init_distributed, or the"
            " torch's launcher. If using a different launcher please ensure"
            " LOCAL_RANK is set prior to initializing deepspeed."
        )

        if hasattr(args, "local_rank") and args.local_rank != None:
            assert isinstance(args.local_rank, int), (
                f"args.local_rank of {args.local_rank} is an unknown type"
                f" {type(args.local_rank)}"
            )
            if args.local_rank >= 0:
                env_local_rank = int(os.environ.get("LOCAL_RANK"))
                assert env_local_rank == args.local_rank, (
                    "Mismatch in local rank setting,"
                    f" args.local_rank={args.local_rank} but"
                    f" env['LOCAL_RANK']={env_local_rank}."
                )
    ...
    def _do_sanity_check(self):
        ...
        assert isinstance(
            ...
            "Client Optimizer is of unexpected type"
            f" {type(self.client_optimizer)}"
        )
        ...
        if isinstance(self.client_lr_scheduler, _LRScheduler):
            assert isinstance(self.client_optimizer, Optimizer), (
                f"Client Optimizer (type = {type(self.client_optimizer)} is"
                " not instantiated but Client LR Scheduler is instantiated"
            )
    ...
    def _broadcast_model(self):
        def is_replicated(p):
            if (
                hasattr(p, "ds_status")
                and p.ds_status is not ZeroParamStatus.AVAILABLE
            ):
                return False
            return True
    ...
    def __check_params(model: Module, dtype: torch.dtype) -> None:
        ...
        if (
            ...
            raise ValueError(
                f"{dtype} is enabled but the following parameters have dtype"
                " that is "
                f"not {dtype}: "
        ...
        if self.fp16_enabled():
            if self.zero_optimization_partition_weights() and any(
                ...
                if not all(
                    ...
                    raise ValueError(
                        "fp16 is enabled but the following parameters have"
                        f" dtype that is not fp16: {', '.join(names)}"
                    )
            self.module.half()
        elif self.bfloat16_enabled():
            if self.zero_optimization_partition_weights() and any(
    ...
    def _check_for_duplicates(self, optimizer):
        for name, param in self.module.named_parameters():
            ...
            assert occurrence <= 1, (
                f"Parameter with name: {name} occurs multiple times in"
                " optimizer.param_groups. Make sure it only appears once to"
    ...
    def _do_optimizer_sanity_check(self, basic_optimizer):
        ...
        assert not (amp_enabled and zero_enabled), (
            "Amp and ZeRO are not currently compatible, please use (legacy)"
            " fp16 mode which performs similar to amp opt_mode=O2"
        )
        if zero_enabled:
            if model_dtype != grad_accum_dtype:
                raise NotImplementedError(
                    "Model data type and gradient accumulation data type must"
                    " be equal to use ZeRO"
                )
            if not is_zero_supported_optimizer(basic_optimizer):
                assert self.zero_allow_untested_optimizer(), (
                    "You are using an untested ZeRO Optimizer. Please add"
                    ' <"zero_allow_untested_optimizer": true> in the'
                    " configuration file to use it."
                )

                if self.global_rank == 0:
                    logger.warning(
                        "**** You are using ZeRO with an untested optimizer,"
            ...
            if model_dtype == torch.bfloat16 or model_dtype == torch.float16:
                raise NotImplementedError(
                    "Cannot enable both amp with (legacy) fp16 or bfloat16"
                    " mode"
                )
            try:
                logger.info(
                ...
                raise RuntimeError(
                    "Unable to import apex/amp, please make sure it is"
                    " installed"
                )
            return AMP
        # data type checks
            ...
            if model_dtype == torch.bfloat16:
                raise NotImplementedError(
                    "Bfloat16 wrapper must use a gradient accumulation type of"
                    " fp32, enable ZeRO to use Bfloat16 gradient accumulation"
                )
            if model_dtype == torch.float16:
                return FP16
            # else optimizer_wrapper = None
        elif (
            model_dtype == torch.bfloat16 and grad_accum_dtype == torch.float32
        ):
            return BFLOAT16
        else:
            raise NotImplementedError(
                "unsupported mix of model dtype and gradient accummulation"
                " type"
            )
    ...
    def _configure_optimizer(self, client_optimizer, model_parameters):
        if client_optimizer is not None:
            if isinstance(client_optimizer, tuple(self._supported_optims())):
                client_optimizer.param_groups[:] = [
                    pg
                    for pg in client_optimizer.param_groups
                    if len(pg["params"]) != 0
                ]
                log_dist(
                    (
                        "Removing param_group that has no 'params' in the"
                        " client Optimizer"
                    ),
                    ranks=[0],
                )
                ...
                log_dist(
                    "Using client Optimizer as basic optimizer", ranks=[0]
                )
            else:
                basic_optimizer = client_optimizer(model_parameters)
                log_dist(
                    "Using client callable to create basic optimizer",
                    ranks=[0],
                )
        else:
            ...
            log_dist(
                (
                    "Using DeepSpeed Optimizer param name"
                    f" {self.optimizer_name()} as basic optimizer"
                ),
                ranks=[0],
            )
        ...
        if optimizer_wrapper == ZERO_OPTIMIZATION:
            ...
            log_dist(
                f"Initializing AMP with these params: {amp_params}", ranks=[0]
            )
        ...
        log_dist(
            "DeepSpeed Final Optimizer = {}".format(self.optimizer_name()),
            ranks=[0],
        )
    ...
    def _configure_basic_optimizer(self, model_parameters):
        ...
        if "max_grad_norm" in optimizer_parameters.keys():
            raise ValueError(
                "'max_grad_norm' is not supported as an optimizer parameter,"
                " please switch to using the deepspeed parameter"
                " 'gradient_clipping' see:"
        ...
        if self.optimizer_name() in [
            ...
            adam_w_mode = optimizer_parameters.pop(
                ADAM_W_MODE, ADAM_W_MODE_DEFAULT
            )
            ...
            ), "1bit-Adam is not compatible with ZeRO"
            ...
            if not self.fp16_enabled():
                logger.warning(
                    f"Currently the convergence of 1-bit Adam is only verified"
                    f" under FP16"
                )
        elif self.optimizer_name() == ZERO_ONE_ADAM_OPTIMIZER:
            assert (
                not self.zero_optimization()
            ), "0/1 Adam is not compatible with ZeRO"
            ...
            if not self.fp16_enabled():
                logger.warning(
                    f"Currently the convergence of 0/1 Adam is only verified"
                    f" under FP16"
                )
        elif self.optimizer_name() == ONEBIT_LAMB_OPTIMIZER:
            assert (
                not self.zero_optimization()
            ), "1bit-Lamb is not compatible with ZeRO"
            from deepspeed.runtime.fp16.onebit.lamb import OnebitLamb

            optimizer = OnebitLamb(
                model_parameters, self, **optimizer_parameters
            )
            if not self.fp16_enabled():
                logger.warning(
                    f"Currently the convergence of 1-bit Lamb is only verified"
                    f" under FP16"
                )
        else:
    ...
    def _configure_quantization(self):
        ...
        if quantize_enabled and not quantize_weight_in_forward:
            assert self.fp16_enabled(), (
                "MoQ (quantize in optimization step) weight quantization is"
                " only supported for FP16"
            )
        quantizer = None
        if quantize_enabled and not quantize_weight_in_forward:
    ...
    def _configure_zero_optimizer(self, optimizer):
        ...
        if self.zero_legacy_stage1():
            raise Exception(
                "The deprecated version of ZeRO Stage 1 is not supported in"
                " deepspeed >= 0.5.9. Please downgrade to a version less than"
                " 0.5.9 if you need to use this deprecated version of ZeRO."
            )
        ...
        if zero_stage <= ZeroStageEnum.gradients:
            ...
            if isinstance(self.module, PipelineModule):
                if overlap_comm:
                    logger.warning(
                        "Pipeline parallelism does not support overlapped"
    ...
    def deepspeed_io(
        ...
        if data_sampler is None and (
            route == ROUTE_PREDICT or route == ROUTE_EVAL
        ):
    ...
    def _scale_loss_by_gas(self, prescaled_loss):
        ...
        elif isinstance(prescaled_loss, tuple) or isinstance(
            prescaled_loss, list
        ):
            scaled_loss = []
    ...
    def print_forward_breakdown(self, fwd_time):
        ...
        log_dist(
            (
                f"rank={dist.get_rank()} time (ms) | forward:"
                f" {fwd_time:.2f} (forward_moe: {moe_time:.2f}, 1st alltoall:"
                f" {falltoall:.2f}, 2nd alltoall: {salltoall:.2f}, top-k:"
                f" {gate_time:.2f})"
            ),
            ranks=[0],
        )
    ...
    def allreduce_gradients(self, bucket_size=MEMORY_OPT_ALLREDUCE_SIZE):
        assert not (self.bfloat16_enabled() and self.pipeline_parallelism), (
            f"allreduce_gradients() is not valid when"
            f" bfloat+pipeline_parallelism is enabled"
        )
        ...
        # Communicate only at gradient accumulation boundaries
            ...
            if (
                ...
                == ZeroStageEnum.optimizer_states
            ):
    ...
    def backward(
        ...
        see_memory_usage(
            "Engine before backward", force=self.memory_breakdown()
        )
        ...
        if not allreduce_gradients:
            logger.warning(
                f"Argument `allreduce_gradients` is deprecated, ignored, and"
                f" will soon be removed"
            )
        ...
        if self.zero_optimization():
            ...
            if self.eigenvalue_enabled():
                self.optimizer.backward(
                    loss, create_graph=True, retain_graph=True
                )
            else:
                self.optimizer.backward(loss, retain_graph=retain_graph)
        elif self.bfloat16_enabled():
        ...
        see_memory_usage(
            "Engine after backward", force=self.memory_breakdown()
        )
    ...
    def step(self, lr_kwargs=None):
        ...
        if self.is_gradient_accumulation_boundary():
            ...
            report_progress = (
                self.global_rank == 0 if self.global_rank else True
            )
        ...
        if self.monitor.enabled:
            if self.is_gradient_accumulation_boundary():
                if self.global_rank == 0:
                    ...
                    if self.fp16_enabled() and hasattr(
    ...
    def _autotuning_exit(self):
        if self.global_rank == 0:
            ...
            log_dist(
                (
                    f"Wrote metrics to {self.autotuning_metric_path()},"
                    f" {os.path.abspath(self.autotuning_metric_path())}"
                ),
                ranks=[0],
            )
            import atexit
            ...
            atexit.register(
                print, "Autotuning: done with running current ds config."
            )
        exit()
        ...
        if self.global_rank == 0:
            self.summary_events = [
                ...
                (
                    ...
                    self.timers(BACKWARD_INNER_GLOBAL_TIMER).elapsed(
                        reset=False
                    ),
                    self.global_samples,
                ),
                (
                    ...
                    self.timers(BACKWARD_REDUCE_GLOBAL_TIMER).elapsed(
                        reset=False
                    ),
                    self.global_samples,
                ),
    ...
    def allreduce_no_retain(
        self, bucket, dp_group, numel_per_bucket=500000000
    ):
        small_bucket = []
        numel = 0
    ...
    def _get_gradients_for_reduction(self):
        ...
        for param_name, param in self.module.named_parameters():
            ...
            if (
                param_name in self.sparse_tensor_module_names
    ...
    def _reduce_expert_gradients(self, expert_grads, elements_per_buffer):
        for ep_name, expert_grads_group in expert_grads.items():
            expert_split_buckets = split_half_float_double_sparse(
                expert_grads_group
            )
            for i, bucket_tuple in enumerate(expert_split_buckets):
                ...
                if bucket_type == SparseTensor.type():
                    ...
                    self.allreduce_no_retain(
                        bucket,
                        dp_group=groups._get_expert_data_parallel_group(
                            ep_name
                        ),
                        numel_per_bucket=elements_per_buffer,
                    )

    def buffered_allreduce_fallback(
        ...
        if grads is None:
            (
                non_expert_grads,
                expert_grads,
            ) = self._get_gradients_for_reduction()
        else:
            assert (
        ...
        self._reduce_non_expert_gradients(
            non_expert_grads, elements_per_buffer
        )
    ...
    def sparse_all_gather(self, value, dp_group):
        ...
        if value.dim() == 1:
            ...
            if fill_size > 0:
                value = torch.cat(
                    [value, value.new_empty(fill_size, value.size()[1])]
                )
            tensor_list = [
            ...
            for n_module, module in model.named_modules():
                if isinstance(
                    ...
                    for local_expert_id in range(num_local_experts):
                        ...
                        # Updating global -> local expert ids
                        moe_str_prefix = (
                            ".deepspeed_moe.experts.deepspeed_experts."
                        )
                        for key in list(expert_state_dict.keys()):
                            local_key = key.replace(
                                f"{moe_str_prefix}{global_expert_id}",
                            ...
                            expert_state_dict[
                                local_key
                            ] = expert_state_dict.pop(key)
                        state_dict.update(expert_state_dict)
                    moe_layer_id += 1

    def load_module_state_dict(
        self, state_dict, strict=True, custom_load_fn=None
    ):
        if custom_load_fn:
    ...
    def _get_ckpt_name(self, checkpoints_path, tag, mp_placeholder=None):
        if mp_placeholder is not None:
            ...
            mp_rank = (
                0 if self.mpu is None else self.mpu.get_model_parallel_rank()
            )
    ...
    def load_checkpoint(
        ...
        if tag is None:
            ...
            if os.path.isfile(latest_path):
                ...
                if self.load_universal_checkpoint():
                    raise ValueError(
                        f"Invalid for universal checkpoint: {latest_path} does"
                        " not exist"
                    )
                else:
                    logger.warning(
                        f"Unable to find latest file at {latest_path}, if"
                        " trying to load latest checkpoint please ensure this"
                        " file exists or pass an explicit checkpoint tag when"
    ...
    def _load_checkpoint(
        ...
        if load_module_only:
            ...
            if "sparse_tensor_module_names" in checkpoint:
                sparse_tensor_module_names = checkpoint[
            ...
            if sparse_tensor_module_names is not None:
                if load_module_strict:
                    ...
                    self.sparse_tensor_module_names = (
                        get_sparse_tensor_module_names(
                            self.sparse_tensor_module_names,
                            sparse_tensor_module_names,
    ...
    def _load_zero_checkpoint(self, load_dir, tag, load_optimizer_states=True):
        if self.load_universal_checkpoint():
            ...
            if (
                ...
                raise ZeRORuntimeException(
                    "The checkpoint being loaded used a DP world size of"
                    f" {self.loaded_checkpoint_dp_world_size} but the current"
                    f" world size is {self.dp_world_size}. Automatic"
                    " adjustment of ZeRO's optimizer state partitioning with"
                    " a new world size is not currently supported."
                )
        ...
        if self.load_universal_checkpoint():
            logger.info(
                "loaded universal zero checkpoints from"
                f" {checkpoint_folder} for rank {self.global_rank}"
            )
        else:
            logger.info(
                f"loading {len(zero_sd_list)} zero partition checkpoints for"
    ...
    def _get_all_zero_checkpoint_state_dicts(self, zero_ckpt_names):
        ...
        logger.info(
            f"successfully read {len(zero_optimizer_sd)} ZeRO state_dicts for"
            f" rank {self.global_rank}"
        )
        ...
        for bf16_mode in [
            ...
            if zero_ckpt_names is not None:
                ...
                if bf16_mode is not self.bfloat16_enabled():
                    ...
                    engine_bit16 = (
                        BFLOAT16 if self.bfloat16_enabled() else FP16
                    )
                    logger.warn(
                        f"Loading {checkpoint_bit16} zero checkpoints into"
                        f" {engine_bit16} training engine"
                    )
                return self._get_all_zero_checkpoint_state_dicts(
                    zero_ckpt_names
                )
    ...
    def _checkpoint_tag_validation(self, tag):
        if self.checkpoint_tag_validation_enabled():
            ...
            bhash = (
                torch.ByteTensor([s_hash.digest()]).flatten().to(self.device)
            )
            ...
            msg = (
                f"[rank={dist.get_rank()}] The checkpoint tag name '{tag}' is"
                " not consistent across all ranks. Including rank unique"
                " information in checkpoint tag could cause issues when"
    ...
    def save_checkpoint(
        ...
        rank = (
            self.local_rank
            if self.use_node_local_storage()
            ...
            if isinstance(module, MoE):  # and deepspeed.comm.get_rank() == 0:
                ...
                for (
                    ...
                    self.checkpoint_engine.save(
                        expert_state_dict, moe_save_path
                    )
                moe_layer_id += 1
    ...
    def _create_checkpoint_file(self, save_dir, tag, zero_checkpoint):
        name_function = (
            self._get_zero_ckpt_name
            if zero_checkpoint
            else self._get_ckpt_name
        )
        try:
    ...
    def _save_checkpoint(self, save_dir, tag, client_state={}):
        ...
        if self.save_non_zero_checkpoint:
            log_dist(
                message=f"Saving model checkpoint: {save_path}", ranks=[0, 1]
            )
            self.checkpoint_engine.save(state, save_path)
    ...
    def save_16bit_model(self, save_dir, save_filename="pytorch_model.bin"):
        ...
        if self.zero_optimization_partition_weights():
            if self.zero_gather_16bit_weights_on_model_save():
                ...
                logger.info(
                    f"Did not save the model {path} because"
                    " `stage3_gather_16bit_weights_on_model_save` is False"
                )
                return False
        else:

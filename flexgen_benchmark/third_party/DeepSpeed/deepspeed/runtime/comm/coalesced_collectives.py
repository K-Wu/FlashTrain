...
def _torch_reduce_scatter_fn(
    ...
    return instrument_w_nvtx(dist.reduce_scatter_fn)(
...
def reduce_scatter_coalesced(
    ...
    if len(tensors) == 1 and tensors[0].numel() % world_sz == 0:
        ...
        for rank in range(world_sz):
            for tensor_idx in range(len(tensors)):
                ...
                padding_sz = (
                    padded_partition_sz_for_each_tensor[tensor_idx]
                    - tensor_chunk.numel()
                )
                if padding_sz > 0:
    ...
    offset = 0
    for tensor_idx in range(len(tensors)):
        output_lst[tensor_idx] = tensor_partition_buffer_for_each_rank[
            this_rank
        ].narrow(
            0,
            offset,

...
class NcclBackend(object):
    ...
    def compressed_allreduce(
        ...
        dist.all_to_all_single(
            recvbuf_sign, torch.stack(sign_list_packed), group=self.world_group
        )
        # Allgather for scale
        ...
        compensated_server_m = (
            ...
            .add_(-0.5)
            .mul_(2.0)
            .mul_(torch.stack(recvbuf_scale).mul_(1 / self.size))
            .sum(0)
        )
        ...
        if self.bool_not_supported:
            cupy_server_sign_packed = (
                self.compression_backend.compress_by_chunk(
                    self.compression_backend.torch2cupy(
        ...
        dist.all_gather(
            recvbuf_sign_server, server_sign_packed[0], group=self.world_group
        )
        dist.all_gather(
            recvbuf_scale_server, server_scale, group=self.world_group
        )

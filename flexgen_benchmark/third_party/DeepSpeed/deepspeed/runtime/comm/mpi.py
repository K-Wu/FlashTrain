...
class MpiBackend(object):
    ...
    def gather_host(
        ...
        numpy_recvbuf_scale = np.zeros(
            [world_size, 1], dtype=cupy_worker_scale.dtype
        )
        ...
        for idx in range(world_size):
            numpy_sign_list_packed[idx] = cupy.asnumpy(
                cupy_sign_list_packed[idx]
            )
        ...
        for idx in range(world_size):
            cupy_sign_list_packed[idx] = cupy.asarray(
                numpy_sign_list_packed[idx]
            )
    ...
    def allgather_host(
        ...
        numpy_recvbuf_sign_server = np.zeros(
            [comm.Get_size(), cupy_server_sign_packed.size],
            dtype=cupy_server_sign_packed.dtype,
        )
        numpy_recvbuf_scale_server = np.zeros(
            [comm.Get_size(), 1], dtype=cupy_server_scale.dtype
        )
    ...
    def compressed_allreduce(
        ...
        if original_size != worker_error_size:
            empty_tensor = torch.zeros(
                worker_error_size - original_size, device=buffer_m.device
            )
        ...
        cupy_recvbuf_scale = cupy.zeros(
            [self.size, 1], dtype=cupy_worker_scale.dtype
        )
        ...
        cupy_recvbuf_scale_server = cupy.zeros(
            [self.size, 1], dtype=cupy_recvbuf_scale.dtype
        )
        ...
        buffer_m.data.copy_(
            ...
            .mul_(2.0)
            .mul_(
                self.compression_backend.cupy2torch(cupy_recvbuf_scale_server)
            )
            .flatten()
            .data
        )

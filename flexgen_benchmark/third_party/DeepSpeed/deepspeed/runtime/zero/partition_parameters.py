    ...
    if not hasattr(module, "_external_params"):
        ...
        def all_parameters(self):
            return itertools.chain(
                self.named_parameters(self, recurse=False),
                external_parameters(self),
            )

        module.ds_external_parameters = types.MethodType(
            external_parameters, module
        )
...
def unregister_external_parameter(module, parameter):
    ...
    if (
        not hasattr(module, "_external_params")
        or id(parameter) not in module._external_params
    ):
        raise RuntimeError(
            "Parameter is not a registered external parameter of module."
        )
...
def zero_wrapper_for_fp_tensor_constructor(
...
class InsertPostInitMethodToModuleSubClasses(object):
    def __init__(
        ...
        assert self.dtype in [torch.half, torch.bfloat16, torch.float], (
            f"Invalid data type {self.dtype}, allowed values are [torch.half,"
    ...
    def __enter__(self):
        ...
        def apply_with_gather(orig_module_apply_fn: Callable) -> Callable:
            ...
            def get_wrapped_fn_to_apply(fn_to_apply: Callable) -> Callable:
                ...
                def wrapped_fn_to_apply(module_to_apply_fn_to: Module) -> None:
                    ...
                    if not all(
                        is_zero_param(p)
                        for p in module_to_apply_fn_to.parameters(
                            recurse=False
                        )
                    ):
                        raise RuntimeError(
                            "not all parameters for"
                            f" {module_to_apply_fn_to.__class__.__name__}, "
                            "were zero params, is it possible that the"
                            " parameters were "
                            "overwritten after they were initialized? "
                            "params:"
                            f" {[p for p in module_to_apply_fn_to.parameters(recurse=False)]} "
                        )

                    params_to_apply_fn_to: Iterable[Parameter] = list(
                        sorted(
                            module_to_apply_fn_to.parameters(recurse=False),
                    ...
                    for param in params_to_apply_fn_to:
                        dist.broadcast(
                            param.data, 0, group=param.ds_process_group
                        )
        ...
        def partition_after(f):
            ...
            def wrapper(module, *args, **kwargs):
                ...
                print_rank_0(
                    (
                        "After initializing followed by post init for"
        ...
        if self.mem_efficient_linear:
            print_rank_0(
                (
                    "nn.functional.linear has been overridden with a more"
                    " memory efficient version. This will persist unless"
                    " manually reset."
                ),
                force=False,
            )
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if dist.get_rank() == 0:
            logger.info(
                "finished initializing model with %.2fB parameters",
                param_count / 1e9,
            )
    ...
    def _set_dtype(self, ds_config, dtype):
        if ds_config is not None and dtype is None:
            if ds_config.bfloat16_enabled and ds_config.fp16_enabled:
                raise RuntimeError(
                    "bfloat16 and fp16 cannot be enabled at once"
                )
            ...
            for rank in range(self.__world_size):
                ...
                if param_start < param.ds_numel:
                    part_to_copy = self.__partitions[rank].narrow(
                        ...
                        param_offset,
                        min(
                            param.ds_numel - param_start,
                            param.ds_tensor.ds_numel,
                        ),
                    )
                    partitions.append(part_to_copy)

            param.data = instrument_w_nvtx(torch.cat)(partitions).view(
                param.ds_shape
            )
...
class Init(InsertPostInitMethodToModuleSubClasses):
    ...
    def __init__(
        ...
        if config is not None:
            ...
            logger.warning(
                f"zero.Init: the `config` argument is deprecated. Please use"
                f" `config_dict_or_path` instead."
            )

        _ds_config = (
            deepspeed.runtime.config.DeepSpeedConfig(config_dict_or_path, mpu)
        ...
        if not dist.is_initialized():
            ...
            assert dist.is_initialized(), (
                "Parameters cannot be scattered without initializing"
                " deepspeed.comm"
            )
        ...
        self.local_device = torch.device(
            "cuda:{}".format(os.environ["LOCAL_RANK"])
        )
        torch.cuda.set_device(self.local_device)

        if (
            _ds_config is not None
            and _ds_config.zero_config.offload_param is not None
        ):
        ...
        self.remote_device = (
            self.local_device
            if remote_device in [None, OffloadDeviceEnum.none]
            else remote_device
        )
        self.pin_memory = (
            pin_memory
            if (
                self.remote_device
                in [OffloadDeviceEnum.cpu, OffloadDeviceEnum.nvme]
            )
            else False
        )
        ...
        if dist.has_allgather_base():
            ...
            logger.info(
                "_all_gather_base API is not available in torch"
                f" {torch.__version__}"
            )
            ...
            if remote_device in [None, OffloadDeviceEnum.cpu]:
                if ds_config.zero_config.offload_param is not None:
                    offload_param_device = (
                        ds_config.zero_config.offload_param.device
                    )
                    assert offload_param_device != OffloadDeviceEnum.nvme, (
                        "'device' in DeepSpeed Config cannot be"
                        f" {offload_param_device} if remote device is"
                        f" {remote_device}."
                    )

            if remote_device == OffloadDeviceEnum.nvme:
                assert ds_config.zero_config.offload_param is not None, (
                    '"offload_param" must be defined in DeepSpeed Config if'
                    f" remote device is {OffloadDeviceEnum.nvme}."
                )

                assert (
                    ds_config.zero_config.offload_param.nvme_path is not None
                ), (
                    '"nvme_path" in DeepSpeed Config cannot be None if remote'
                    f" device is {OffloadDeviceEnum.nvme}"
                )

    def _post_init_method(self, module):
        # see_memory_usage(f"Before converting parmas in {module.__class__.__name__}", force=False)
        print_rank_0(
            f"Converting Params in {module.__class__.__name__}", force=False
        )
        see_memory_usage(
            (
                "Before converting and partitioning parmas in"
                f" {module.__class__.__name__}"
            ),
            force=False,
        )
        ...
        for name, param in module.named_parameters(recurse=False):
            ...
            if not is_zero_param(param):
                ...
                print_rank_0(
                    "Partitioning param"
                    f" {debug_param2name_id_shape(param)} module={debug_module2name(module)}"
                )
                ...
                if param.is_cuda:
                    ...
                    if dist.get_rank() == 0:
                        logger.warn(
                            f"param `{name}` in {module.__class__.__name__} "
                            "not on GPU so was not broadcasted from rank 0"
                        )

                param.partition()
        see_memory_usage(
            (
                f"Param count {param_count}. After converting and partitioning"
                f" parmas in {module.__class__.__name__}"
            ),
            force=False,
        )
    ...
    def _convert_to_deepspeed_param(self, param):
        ...
        def all_gather(param_list=None, async_op=False, hierarchy=0):
            ...
            return self._all_gather(
                param_list, async_op=async_op, hierarchy=hierarchy
            )
        ...
        def all_gather_coalesced(
            ...
            if safe_mode:
                ...
                assert_ints_same_as_other_ranks(
                    [p.ds_tensor.ds_numel for p in params]
                )
            ...
            if len(params) == 1:
                ...
                param_buffer = torch.empty(
                    math.ceil(param.ds_numel / self.world_size)
                    * self.world_size,
                    dtype=param.dtype,
                ...
                instrument_w_nvtx(torch.cat)(
                    [
                        p.ds_tensor.to(torch.cuda.current_device())
                        for p in params
                    ],
        ...
        def partition(param_list=None, hierarchy=0, has_been_updated=False):
            cls = param
            print_rank_0(
                f"{'--'*hierarchy}----Partitioning param"
                f" {debug_param2name_id_shape_device(cls)}"
            )
            if param_list is None:
                param_list = [cls]
        ...
        def reduce_gradients_at_owner(param_list=None, hierarchy=0):
            ...
            if param_list is None:
                param_list = [cls]
            print_rank_0(
                f"{'--'*hierarchy}----Reducing Gradients for param with ids"
                f" {[param.ds_id for param in param_list]} to owner"
            )
        ...
        def partition_gradients(
            ...
            cls = param
            print_rank_0(
                f"{'--'*hierarchy}----Partitioning param gradient with id"
                f" {debug_param2name_id_shape_device(cls)}"
            )
            if param_list is None:
                param_list = [cls]
        ...
        def ds_summary(
            ...
            return {
                ...
                "grad_shape": tuple(slf.grad.shape)
                if slf.grad is not None
                else None,
    ...
    def _all_gather(self, param_list, async_op=False, hierarchy=None):
        ...
        for param in param_list:
            if param.ds_status == ZeroParamStatus.NOT_AVAILABLE:
                if async_op:
                    handle = self._allgather_param(
                        param, async_op=async_op, hierarchy=hierarchy
                    )
                    param.ds_status = (
        ...
        if not async_op:
            if len(param_list) == 1:
                ret_value = self._allgather_params(
                    all_gather_list, hierarchy=hierarchy
                )
            else:
                ret_value = self._allgather_params_coalesced(
                    all_gather_list, hierarchy
                )
    ...
    def _partition_param(self, param, buffer=None, has_been_updated=False):
        ...
        ), f" {param} Cannot partition a param in flight"
        ...
        if param.ds_status is ZeroParamStatus.AVAILABLE:
            print_rank_0(
                (
                    f"Partitioning param id {param.ds_id} reuse buffers"
                    f" {reuse_buffers}"
                ),
                force=False,
            )
            ...
            if param.ds_tensor is not None and not has_been_updated:
                ...
                if param.ds_tensor.final_location == OffloadDeviceEnum.nvme:
                    print_rank_0(
                        (
                            f"Param {param.ds_id} partition released since it"
                            " exists in nvme"
                        ),
                        force=False,
                    )
            ...
            if param.ds_tensor is None:
                final_location = None
                if (
                    self.remote_device == OffloadDeviceEnum.nvme
                    and self.param_swapper.swappable_tensor(
                        numel=partition_size
                    )
                ):
                    final_location = OffloadDeviceEnum.nvme
                    buffer = self.param_swapper.get_buffer(
                        param, partition_size
                    )
                    partitioned_tensor = torch.empty(
                        0, dtype=param.dtype, device=buffer.device
                    )
                    partitioned_tensor.data = buffer.data
                    print_rank_0(
                        f"ID {param.ds_id} Initializing partition for the"
                        " first time for nvme offload."
                    )
            ...
            if start < param.ds_numel and end <= param.ds_numel:
                ...
                if start < param.ds_numel:
                    ...
                    param.ds_tensor.narrow(0, 0, elements_to_copy).copy_(
                        one_dim_param.narrow(0, start, elements_to_copy)
                    )
            ...
            if param.ds_tensor.final_location == OffloadDeviceEnum.nvme:
                ...
                print_rank_0(
                    f"ID {param.ds_id} Offloaded to nvme offload and buffers"
                    " released."
                )
                see_memory_usage(
                    (
                        f"ID {param.ds_id} Offloaded to nvme offload and"
                        " buffers released."
                    ),
                    force=False,
                )
            ...
            print_rank_0(
                f"ID {param.ds_id} partitioned type {param.dtype} dev"
                f" {param.device} shape {param.shape}"
            )

    def _param_status(self, param):
        if param.ds_tensor is not None:
            print_rank_0(
                f"Param id {param.ds_id}, param status: {param.ds_status},"
                f" param numel {param.ds_numel}, partitioned numel"
                f" {param.ds_tensor.numel()}, data numel {param.data.numel()}"
            )
        else:
            print_rank_0(
                f"Param id {param.ds_id}, param status: {param.ds_status},"
                f" param numel {param.ds_numel}, partitioned ds_tensor"
                f" {param.ds_tensor}, data numel {param.data.numel()}"
            )
    ...
    def _allgather_param(self, param, async_op=False, hierarchy=0):
        ...
        assert tensor_size == aligned_param_size, (
            f"param id {param.ds_id} aligned size {aligned_param_size} does"
            f" not match tensor size {tensor_size}"
        )

        print_rank_0(
            f"{'--'* hierarchy}---- Before allocating allgather param"
            f" {debug_param2name_id_shape_status(param)} partition"
            f" size={partition_size}"
        )

        see_memory_usage(
            (
                "Before allocate allgather param"
                f" {debug_param2name_id_shape_status(param)} partition_size={partition_size} "
            ),
            force=False,
        )
        flat_tensor = torch.zeros(
            aligned_param_size, dtype=param.dtype, device=param.device
        ).view(-1)
        see_memory_usage(
            (
                "After allocate allgather param"
                f" {debug_param2name_id_shape_status(param)} {aligned_param_size} {partition_size} "
            ),
            force=False,
        )
        ...
        print_rank_0(
            f"{'--'* hierarchy}----allgather param with"
            f" {debug_param2name_id_shape_status(param)} partition"
            f" size={partition_size}"
        )
        ...
        if self.use_all_gather_base:
            ...
            for i in range(self.world_size):
                partitions.append(
                    flat_tensor.narrow(0, partition_size * i, partition_size)
                )

                if i == dist.get_rank(group=self.ds_process_group):
                    partitions[i].data.copy_(
                        param.ds_tensor.data, non_blocking=True
                    )
        ...
        replicated_tensor = flat_tensor.narrow(0, 0, param.ds_numel).view(
            param.ds_shape
        )
        param.data = replicated_tensor.data
        return handle
    ...
    def _allgather_params_coalesced(self, param_list, hierarchy=0):
        ...
        for param_idx, param in enumerate(param_list):
            ...
            if self.use_all_gather_base:
                ...
                for i in range(self.world_size):
                    ...
                    partition = allgather_params[param_idx].narrow(
                        0, i * psize, psize
                    )
                    output_list.append(partition)
                    if not partition.is_cuda:
                        logger.warning(
                            f"param {param_idx}, partition {i} is not on CUDA,"
                            f" partition shape {partition.size()}"
                        )
        ...
        for i, param in enumerate(param_list):
            ...
            param.data = (
                gathered_tensor.narrow(0, 0, param.ds_numel)
                .view(param.ds_shape)
                .data
            )
    ...
    def _allgather_params(self, param_list, hierarchy=0):
        ...
        partition_size = sum(
            [param.ds_tensor.ds_numel for param in param_list]
        )

        tensor_size = partition_size * self.world_size
        flat_tensor = torch.empty(
            tensor_size, dtype=param_list[0].dtype, device=self.local_device
        )
        ...
        for i in range(self.world_size):
            ...
            if i == self.rank:
                ...
                for param in param_list:
                    ...
                    partitions[i].narrow(0, offset, param_numel).copy_(
        ...
        for param in param_list:
            ...
            replicated_tensor = torch.empty(
                param.ds_shape, dtype=param.dtype, device=self.local_device
            )
            ...
            for i in range(self.world_size):
                ...
                if param_start < param_size:
                    numel_to_copy = min(
                        param_size - param_start, param_partition_size
                    )

                    part_to_copy = partitions[i].narrow(
                        0, param_offset, numel_to_copy
                    )

                    replicated_tensor.view(-1).narrow(
                        0, param_start, numel_to_copy
                    ).copy_(part_to_copy)
    ...
    def _reduce_scatter_gradients(self, param_list):
        ...
        for param in param_list:
            assert param.grad.numel() == param.ds_numel, (
                f"{param.grad.numel()} != {param.ds_numel} Cannot reduce"
                " scatter gradients whose size is not same as the params"
            )

            handles_and_reduced_partitions.append(
                self._reduce_scatter_gradient(param)
            )
        ...
        for param, (handle, reduced_partition) in zip(
            ...
            if start < param.ds_numel and end > param.ds_numel:
                ...
                param.grad.view(-1).narrow(0, start, elements).copy_(
                    reduced_partition.narrow(0, 0, elements)
                )
    ...
    def _reduce_scatter_gradient(self, param):
        ...
        for i in range(self.world_size):
            ...
            if start < param.ds_numel and end <= param.ds_numel:
                ...
                input = torch.zeros(
                    partition_size, dtype=param.dtype, device=param.device
                )

                if start < param.ds_numel:
                    elements = param.ds_numel - start
                    input.narrow(0, 0, elements).copy_(
                        param.grad.view(-1).narrow(0, start, elements)
                    )
    ...
    def _partition_gradients(
        ...
        for param, partition_buffer in zip(param_list, partition_buffers):
            self._partition_gradient(
                param, partition_buffer=partition_buffer, accumulate=accumulate
            )
    ...
    def _partition_gradient(
        ...
        # param.grad.test()
        print_rank_0(
            f"Partitioning param {param.ds_id} gradient of size"
            f" {param.grad.numel()} type {param.grad.dtype} part_size"
            f" {param.ds_tensor.ds_numel}"
        )
        ...
        if partition_buffer is None:
            ...
            assert partition_buffer.numel() >= partition_size, (
                f"The partition buffer size {partition_buffer.numel()} should"
                f" match the size of param.ds_tensor {partition_size}"
            )
        ...
        if start < param.ds_numel:
            ...
            # adding directly to cpu is very slow
                ...
                acc_tensor = torch.empty(
                    src_tensor.numel(), dtype=param.dtype, device=param.device
                )
...
class GatheredParameters:
    def __init__(
        self, params, modifier_rank=None, fwd_module=None, enabled=True
    ):
        ...
        if modifier_rank is not None:
            if self.params[0].ds_process_group == dist.get_world_group():
                ...
                self.src_rank = dist.get_global_rank(
    ...
    def __exit__(self, *exc):
        ...
        handles = [
            dist.broadcast(
                p, self.src_rank, group=p.ds_process_group, async_op=True
            )
            for p in self.params
        ]
        for h in handles:

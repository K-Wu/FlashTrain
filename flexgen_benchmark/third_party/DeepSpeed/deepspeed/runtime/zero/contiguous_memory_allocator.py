...
class ContiguousMemoryAllocator(object):
    ...
    def allocate_tensor(self, size):
        ...
        if self.largest_contiguous < size:
            print_rank_0(
                "Needs defragmentation to allocate. Before Defragmentation:"
            )
    ...
    def assign_to_param(self, tensor, param, numel, shape):
        ...
        assert (
            tensor.numel() >= numel
        ), "Assert tensor buffer does is not large enough"
        assert (
    ...
    def _defragment_memory(self):
        ...
        while tensor_index < len(tensor_addresses):
            ...
            assert tensor_size == tensor.numel(), (
                "Size mismatch. {tensor_size} is allocated at addr"
                " {tensor_addr} but tensor size is {tensor.numel()} "
            )

            assert empty_addr != tensor_addr, (
                f"Cannot have same empty address {empty_addr} and tensor"
                f" address {tensor_addr}"
            )

            if empty_addr < tensor_addr:
                if empty_size >= tensor_size:
                    dest_buffer = self.buffer.narrow(
                        0, empty_addr, tensor_size
                    )
                    src_buffer = self.buffer.narrow(
                        0, tensor_addr, tensor_size
                    )
                    ...
                    while src_addr < (tensor_addr + tensor_size):
                        copy_size = min(
                            empty_size, tensor_addr + tensor_size - src_addr
                        )

                        dest_buffer = self.buffer.narrow(
                            0, dest_addr, copy_size
                        )

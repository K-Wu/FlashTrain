...
def read_zero_config_deprecated(param_dict):
    ...
    if zero_config_dict["stage"] > 0:
        zero_config_dict["allgather_bucket_size"] = get_scalar_param(
            param_dict, "allgather_size", 5e8
        )
    logger.warning(
        "DeepSpeedConfig: this format of ZeRO optimization setup is"
        " deprecated. Please use the following format: {}".format(ZERO_FORMAT)
    )
...
class DeepSpeedZeroConfig(DeepSpeedConfigModel):
    ...
    cpu_offload_param: bool = Field(
        ...
        new_param_fn=(
            lambda val: DeepSpeedZeroOffloadParamConfig(
    ...
    cpu_offload: bool = Field(
        ...
        new_param_fn=(
            lambda val: DeepSpeedZeroOffloadOptimizerConfig(
                device=OffloadDeviceEnum.cpu
            )
            if val
            else None
    ...
    prefetch_bucket_size: int = Field(
        pp_int(5e7), ge=0, alias="stage3_prefetch_bucket_size"
    )
    """
    ...
    param_persistence_threshold: int = Field(
        pp_int(1e5), ge=0, alias="stage3_param_persistence_threshold"
    )
    """
    ...
    model_persistence_threshold: int = Field(
        pp_int(sys.maxsize, "sys.maxsize"),
        ge=0,
    ...
    max_live_parameters: int = Field(
        pp_int(1e9), ge=0, alias="stage3_max_live_parameters"
    )
    """
    ...
    max_reuse_distance: int = Field(
        pp_int(1e9), ge=0, alias="stage3_max_reuse_distance"
    )
    """
    ...
    stage3_gather_fp16_weights_on_model_save: bool = Field(
        False, deprecated=True, new_param="gather_16bit_weights_on_model_save"
    )
    ...
    def overlap_comm_valid(cls, field_value, values):
        if field_value is None:
            assert "stage" in values, (
                "DeepSpeedZeroConfig: 'stage' must be defined before"
                " 'overlap_comm'"
            )

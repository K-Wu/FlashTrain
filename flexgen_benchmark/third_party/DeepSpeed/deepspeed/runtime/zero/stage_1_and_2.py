...
def _get_padded_tensor(src_tensor, size):
    ...
    padded_tensor = torch.zeros(
        size, dtype=src_tensor.dtype, device=src_tensor.device
    )
...
class DeepSpeedZeroOptimizer(ZeROOptimizer):
    ...
    def __init__(
        ...
        self.device = (
            torch.cuda.current_device() if not self.cpu_offload else "cpu"
        )
        ...
        if self.fp16_master_weights_and_gradients:
            assert self.cpu_offload and type(self.optimizer) in [
                ...
                "fp16_master_and_gradients requires optimizer to support"
                " keeping fp16 master and gradients while keeping the"
                " optimizer states in fp32. Currently only supported using"
                " ZeRO-Offload with DeepSpeedCPUAdam. But current setting is"
                f" ZeRO-Offload:{self.cpu_offload} and optimizer type"
                f" {type(self.optimizer)}. Either disable"
                " fp16_master_weights_and_gradients or enable ZeRO-2 Offload"
                " with DeepSpeedCPUAdam"
            )
        ...
        if self.reduce_scatter:
            assert self.communication_data_type in (
                ...
                "ZeRO-2 supports only float16 or bfloat16"
        ...
        self.nccl_start_alignment_factor = (
            2  # 4-byte alignment/sizeof(fp16) = 2
        )
        ...
        assert allgather_bucket_size % self.nccl_start_alignment_factor == 0, (
            ...
            f" {self.nccl_start_alignment_factor} "
        )
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            see_memory_usage(
                f"After moving param group {i} to CPU", force=False
            )
            ...
            if dist.get_rank(group=self.real_dp_process_group[i]) == 0:
                see_memory_usage(
                    (
                        "After Flattening and after emptying param group"
                        f" {i} cache"
                    ),
                    force=False,
                )
            ...
            self.parallel_partitioned_bit16_groups.append(
            ...
            for partitioned_data in data_parallel_partitions:
                assert (
                    partitioned_data.data_ptr()
                    % (2 * self.nccl_start_alignment_factor)
                    == 0
                )
        ...
        for rank in range(dist.get_world_size()):
            if dist.get_rank() == rank:
                print(
                    f"Rank: {rank} partition count {self.partition_count} and"
    ...
    def _link_all_hp_params(self):
        ...
        for i, _ in enumerate(self.optimizer.param_groups):
            ...
            link_hp_params(
                ...
                partition_optimizer_state=self.optimizer.state[
                    flat_hp_partition
                ],
    ...
    def _configure_moe_settings(self):
        ...
        if self.partition_gradients:
            assert self.contiguous_gradients, (
                "Contiguous Gradients in ZeRO Stage 2 must be set to True for"
                " MoE. Other code paths are not tested with MoE"
            )
        ...
        if not self.partition_gradients and not self.contiguous_gradients:
            logger.warn(
                "ZeRO Stage 1 has not been thoroughly tested with MoE. This"
                " configuration is still experimental."
            )
        assert self.reduce_scatter, (
            "Reduce Scatter in ZeRO Stage 2 must be set to True for MoE. Other"
            " code paths are not tested with MoE"
        )

        assert any(
            [self.is_moe_group(group) for group in self.optimizer.param_groups]
        ), (
            "The model has moe layers, but None of the param groups are marked"
            " as MoE. Create a param group with 'moe' key set to True before"
            " creating optimizer"
        )
    ...
    def _update_model_bit16_weights(self, group_index):
        ...
        for p, q in zip(
            self.round_robin_bit16_groups[group_index], updated_params
        ):
            p.data = q.data
        ...
        for param_index, param in enumerate(self.bit16_groups[group_index]):
            new_index = self.round_robin_bit16_indices[group_index][
                param_index
            ]
            param.data = self.round_robin_bit16_groups[group_index][
                new_index
            ].data
    ...
    def reduce_independent_p_g_buckets_and_remove_grads(self, param, i):
        ...
        assert param.grad is not None, (
            f"rank {dist.get_rank()} - Invalid to reduce Param {param_id} with"
            " None gradient"
        )
        ...
        with torch.cuda.stream(stream):
            ...
            for i, param, param_id in self.params_in_ipg_bucket:
                ...
                assert all(
                    ...
                    "world size"
                    f" {dist.get_world_size(group=process_group)} and p_ids:"
                    f" {partition_ids}"
                )
                partition_size = self.partition_size[i]
            ...
            for i, (dst, bucket_offset, numel) in enumerate(rank_and_offsets):
                grad_slice = tensor_to_reduce.narrow(
                    0, int(bucket_offset), int(numel)
                )
                # if dist.get_rank() == 0:
    ...
    ############################# CPU Offload Methods#############################
    ##############################################################################
    def get_grad_position(
        self, group_id, tensor_list, first_offset, partition_size
    ):
        current_offset = 0
    ...
    def async_accumulate_grad_in_cpu_via_gpu(self, param):
        ...
        [i, source_offset, dest_offset, num_elements] = self.grad_position[
            param_id
        ]
        ...
        def accumulate_gradients():
            if not self.fp16_master_weights_and_gradients:
                ...
                ).add_(dest_buffer.narrow(0, source_offset, num_elements))
    ...
    def set_norm_for_param_grad(self, param):
        ...
        self.norm_for_param_grads[
            param_id
        ] = accumulated_grad.data.double().norm(2)
    ...
    def reduce_ipg_grads(self):
        if self.contiguous_gradients:
            if self.extra_large_param_to_reduce is not None:
                ...
                assert (
                    self.get_param_id(self.extra_large_param_to_reduce)
                    == param_id
        ...
        with torch.cuda.stream(stream):
            for _, param, param_id in self.params_in_ipg_bucket:
                assert self.params_already_reduced[param_id] == False, (
                    f"The parameter {param_id} has already been reduced.      "
                    "               Gradient computed twice for this"
                    " partition.                     Multiple gradient"
                ...
                if self.partition_gradients:
                    if not self.is_param_in_current_partition[param_id]:
                        if (
                            self.overlap_comm
                            and self.contiguous_gradients is False
                        ):
                            ...
                            param.grad = (
                                None  # only if self.partition_gradients
                            )
                    elif self.contiguous_gradients:
    ...
    def _model_parallel_all_reduce(self, tensor, op):
        ...
        if (
            self.model_parallel_group is None
            or self.model_parallel_world_size == 1
        ):
            pass
        else:
            dist.all_reduce(
    ...
    def get_bit16_param_group(self, group_no):
        ...
        partition_id = dist.get_rank(
            group=self.real_dp_process_group[group_no]
        )
        return [
            bit16_partitions[
                dist.get_rank(group=self.real_dp_process_group[group_no])
            ]
        ]
    ...
    def _optimizer_step(self, group_no):
        ...
        if (
            type(self.optimizer) == DeepSpeedCPUAdam
            and self.dtype == torch.half
        ):
            self.optimizer.step(
                fp16_param_groups=[self.get_bit16_param_group(group_no)]
            )
        else:
            self.optimizer.step()
    ...
    def step(self, closure=None):
        ...
        if self.overflow:
            if dist.get_rank() == 0:
                logger.info(
                    "[deepspeed] OVERFLOW! Rank {} Skipping step. Attempted"
                    " loss scale: {}, reducing to {}".format(
        ...
        for i, group in enumerate(self.bit16_groups):
            ...
            if self.cpu_offload:
                ...
                if not (
                    ...
                    bit16_partitions = self.parallel_partitioned_bit16_groups[
                        i
                    ]
        ...
        for i, norm in enumerate(norm_groups):
            if self.is_moe_param_group[i]:
                ...
                scaled_norm_tensor = torch.tensor(
                    scaled_norm, device="cuda", dtype=torch.float
                )
                dist.all_reduce(
                    scaled_norm_tensor, group=self.real_dp_process_group[i]
                )
    ...
    def get_ep_ranks(self, rank=0, group_name=None):
        ...
        expert_parallel_size_ = groups._get_expert_parallel_world_size(
            group_name
        )
    ...
    def _load_legacy_checkpoint(
        ...
        self.dynamic_loss_scale = current_rank_sd.get(
        ...
        assert (
            ckpt_version
        ), f"Empty ds_version in checkpoint, not clear how to proceed"
        ...
        if not self.partition_gradients:
            ...
            error_str = (
                f"ZeRO stage 1 changed in {required_version} and is not"
                " backwards compatible with older stage 1 checkpoints. If"
                " you'd like to load an old ZeRO-1 checkpoint please use an"
                " older version of DeepSpeed (<= 0.5.8) and set"
                " 'legacy_stage1': true in your zero config json."
            )
            assert (
...
def estimate_zero2_model_states_mem_needs(
    ...
    if cpu_offload:
        ...
        cpu_mem = (
            total_params * max(4 * total_gpus, 16) * additional_buffer_factor
        )
    else:

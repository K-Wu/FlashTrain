...
def split_tensor_along_last_dim(
    tensor, partitions, contiguous_split_chunks=False
):
...
class TiledLinear(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if (out_splits < 1) or (out_splits > out_features):
            raise RuntimeError(
                "out splits must be in range [1, out_features]."
            )
        ...
        self.out_parts = partition(
            num_items=out_features, num_parts=out_splits
        )
    ...
    def forward(self, input_):
        if self.in_splits > 1 and not self.input_is_already_split:
            ...
            split_sizes = [
                input_parts[p + 1] - input_parts[p]
                for p in range(self.in_splits)
            ]
            inputs = self._split_global_input(input_, split_sizes)
        elif self.in_splits > 1:
            inputs = input_
            assert len(inputs) == self.in_splits, (
                f"Col splits {self.in_splits} does not match input splits"
                f" {len(inputs)}"
            )
        else:
            # no splits
...
class TiledLinearReturnBias(TiledLinear):
    ...
    def _reduce_local_output(self, in_id, out_id, current_out, new_out):
        ...
        tensor = super()._reduce_local_output(
            in_id=in_id, out_id=out_id, current_out=old_tensor, new_out=tensor
        )

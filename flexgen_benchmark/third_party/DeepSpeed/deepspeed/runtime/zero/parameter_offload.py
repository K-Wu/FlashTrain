        ...
        if not is_builtin_type(outputs):
            ...
            if not warned and dist.get_rank() == 0:
                logger.warning(
                    ...
                    f" ({type(outputs)}) and the tensors embedded in it cannot"
                    " be detected. The ZeRO-3 hooks designed to trigger"
                    " before or after backward pass of the module relies on"
                    " knowing the input and output tensors and therefore may"
                    " not get triggered properly."
                )
                warned = True
        return outputs
    ...
    def __getitem__(self, key):
        ...
        if param.ds_status == ZeroParamStatus.NOT_AVAILABLE:
            if self._parent_module._parameters._in_forward:
                ...
                print_rank_0(
                    (
                        "Registering external parameter from getter"
                        f" {key} ds_id = {param.ds_id}"
                    ),
                    force=False,
                )
...
class DeepSpeedZeRoOffload(object):
    def __init__(
        ...
        print_rank_0(
            (
                f"Created module hooks: forward = {len(self.forward_hooks)},"
                f" backward = {len(self.backward_hooks)}"
            ),
            force=False,
        )
    ...
    def partition_all_parameters(self):
        ...
        ).release_and_reset_all(self.module)
    ...
    def _convert_to_zero_parameters(self, ds_config, module, mpu):
        non_zero_params = [
            p for p in module.parameters() if not is_zero_param(p)
        ]
        if non_zero_params:
            zero_params = [p for p in module.parameters() if is_zero_param(p)]
            if zero_params:
                zero_params[0].convert_to_zero_parameters(
                    param_list=non_zero_params
                )
            else:
                group = None
                if mpu:
    ...
    def _remove_module_hooks(self):
        ...
        print_rank_0(
            (
                f"Deleted module hooks: forward = {num_forward_hooks},"
                f" backward = {num_backward_hooks}"
            ),
            force=False,
        )
    ...
    def mark_persistent_parameters(self, param_threshold, model_threshold):
        ...
        print_rank_0(
            (
                "Parameter Offload: Total persistent parameters:"
        ...
        def _post_forward_module_hook(module, input, output):
            ...
            if output is None:
                ...
                if torch.is_tensor(output):
                    ...
                    outputs = []
                    output = (
                        output if isinstance(output, dict) else vars(output)
                    )
            ...
            for item in filter(lambda item: is_zero_param(item), output):
                if not any(
                    id(item) in m._external_params for m in FWD_MODULE_STACK
                ):
                    ...
                    print_rank_0(
                        (
                            "Registering dangling parameter for module"
                            f" {module_to_register.__class__.__name__}, ds_id"
                            f" = {item.ds_id}."
                        ),
                        force=False,
                    )
                    ...
                    if id(item) in module._external_params:
                        print_rank_0(
                            (
                                "  Unregistering nested dangling parameter"
                                f" from module {module.__class__.__name__},"
                                f" ds_id = {item.ds_id}"
                            ),
                            force=False,
                        )
    ...
    def pre_sub_module_forward_function(self, sub_module):
        ...
        see_memory_usage(
            (
                "Before sub module function"
                f" {sub_module.__class__.__name__} after fetch"
            ),
            force=False,
        )
    ...
    def post_sub_module_forward_function(self, sub_module):
        see_memory_usage(
            (
                "After sub module function"
                f" {sub_module.__class__.__name__} {sub_module.id} before"
                " release"
            ),
            force=False,
        )
        ...
        see_memory_usage(
            (
                f"After sub module function {sub_module.__class__.__name__} "
                f" {sub_module.id} after release"
            ),
            force=False,
        )
    ...
    def post_sub_module_backward_function(self, sub_module):
        see_memory_usage(
            (
                "After sub module backward function"
                f" {sub_module.__class__.__name__} {sub_module.id} before"
                " release"
            ),
            force=False,
        ...
        see_memory_usage(
            (
                "After sub module backward function"
                f" {sub_module.__class__.__name__} {sub_module.id} after"
                " release"
            ),
            force=False,
        )

...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def __init__(
        ...
        print_rank_0(
            f"initialized {__class__.__name__} with args: {locals()}",
            force=False,
        )
        ...
        self.persistent_parameters = (
            self.parameter_offload.persistent_parameters
        )
        ...
        self.deepspeed_adam_offload = (
            self.offload_optimizer and type(init_optimizer) == DeepSpeedCPUAdam
        )

        self.device = (
            torch.cuda.current_device()
            if not self.offload_optimizer
            else OffloadDeviceEnum.cpu
        )
        ### streams used for overlapping computation with communication
        self.__reduce_and_partition_stream = (
            Stream() if overlap_comm else torch.cuda.default_stream()
        )
        ...
        if self.reduce_scatter:
            assert self.communication_data_type in (
                ...
                "ZeRO-3 supports only float16 or bfloat16"
                " communication_data_type with reduce scatter enabled. Got:"
                f" '{self.communication_data_type}'"
            )
            assert self.gradient_predivide_factor == 1.0, (
                "gradient_predivide_factor != 1.0 is not yet supported with"
                " ZeRO-2 with reduce scatter enabled"
            )
            assert self.postscale_gradients, (
                "pre-scale gradients is not yet supported with ZeRO-2 with"
                " reduce scatter enabled"
            )
        ...
        largest_partitioned_param_numel = max(
            [
                max(
                    [
                        max(tensor.numel(), tensor.ds_numel)
        ...
        print_rank_0(
            (
                ...
                f" {largest_partitioned_param_numel}"
            ),
            force=False,
        )
        ...
        if self.dtype == torch.float or not dynamic_loss_scale:
            loss_scale_value = (
                1.0 if self.dtype == torch.float else static_loss_scale
            )
    ...
    def _setup_for_real_optimizer(self):
        ...
        for param in all_params:
            self.__param_id_to_grad_partition[
                param.ds_id
            ] = grad_partitions_flat_buffer.narrow(
    ...
    def defragment(tensors: List[Tensor]) -> Tensor:
        ...
        for tensor in tensors:
            ...
            tensor.data = torch.empty(
                0, dtype=tensor.dtype, device=tensor.device
            )
    ...
    def _configure_offloading(
        self, offload_optimizer_config, offload_param_config
    ):
        ...
        if (
            ...
            self.offload_optimizer_pin_memory = (
                offload_optimizer_config.pin_memory
            )
            self.swap_optimizer = (
                offload_optimizer_config.device == OffloadDeviceEnum.nvme
            )
            self.offload_optimizer_fast_init = (
                offload_optimizer_config.fast_init
            )
        ...
        if (
            ...
            self.params_in_nvme_and_cpu = (
                offload_param_config.device == OffloadDeviceEnum.nvme
            )
    ...
    def _configure_tensor_swapping(self, offload_optimizer_config, aio_config):
        nvme_swap_folder = os.path.join(
            offload_optimizer_config.nvme_path, "zero_stage_3"
        )
        ...
        swapper_type = (
            PipelinedOptimizerSwapper
            if offload_optimizer_config.pipeline
    ...
    def _move_to_flat_buffer(self, param_list, flat_buffer, avoid_copy=False):
        ...
        for param in param_list:
            ...
            if src.status == PartitionedParamStatus.NOT_AVAILABLE:
                print_rank_0(
                    f"Swapping in {param.ds_id} with partition size"
                    f" {param.partition_numel()} permanently to CPU"
                )
                ...
                assert (
                    src.status == PartitionedParamStatus.AVAILABLE
                ), "Partitioned Param must be available here"
                if not avoid_copy:
    ...
    def _create_param_groups_fp16_flat_cpu_memory(self):
        ...
        for j, param_group in enumerate(self.optimizer.param_groups):
            params_in_group = sum(
                [p.partition_numel() for p in param_group["params"]]
            )
            ...
            if (
                self.params_in_nvme_and_cpu
                and aggregate_params_count + params_in_group
                > self.max_params_in_cpu
            ):
            ...
            if flat_buffer_size > 0:
                ...
                print_rank_0(
                    (
                        "No flat buffer size. Param group size was "
                        f" {params_in_group}"
                    ),
                    force=False,
                )
    ...
    def _create_fp16_partitions_with_defragmentation(self):
        ...
        for param_group_idx, param_group in enumerate(param_groups):
            for sub_group in param_group:
                ...
                rank_requires_padding = (
                    dist.get_rank(self.dp_process_group)
                    == dist.get_world_size(self.dp_process_group) - 1
                )
                self.groups_padding.append(
                    [
                        p.padding_size() if rank_requires_padding else 0
                        for p in sub_group
                    ]
                )
        ...
        if (
            not self.offload_param
        ):  # partitioned params remain in GPU during training
            ...
            for sub_group in self.fp16_groups:
                sub_group_numel = sum(
                    param.partition_numel() for param in sub_group
                )
                self.fp16_partitioned_groups_flat.append(
                    device_buffer.narrow(0, offset, sub_group_numel)
                )
            ...
            for param_group_idx, param_group in enumerate(param_groups):
                ...
                for i, sub_group in enumerate(param_group):
                    total_elements = sum(
                        p.partition_numel() for p in sub_group
                    )
                    print_rank_0(
                    ...
                    if (
                        not self.params_in_nvme_and_cpu
                        or flat_offset + total_elements
                        <= self.param_groups_fp16_flat_cpu_memory[
                            param_group_idx
                        ].numel()
                    ):
                        fp16_partitioned_group_flat = (
                            self.param_groups_fp16_flat_cpu_memory[
                                param_group_idx
                            ].narrow(0, flat_offset, total_elements)
                        )
                        print_rank_0(
                            (
                                "Creating a flat buffer for subgroup"
                                f" {i} requiring {total_elements} elements,"
                                " and cumulative CPU elements"
                                f" {flat_offset + total_elements}"
                            ),
                            force=False,
                        )
                        ...
                        print_rank_0(
                            (
                                f"No flat buffer for sub group {i} of"
                        ...
                        assert False, (
                            "Either params are in nvme, or they are in CPU"
                            " memory. This code path should not be triggered."
                            " Please see you max_params_in_cpu and"
                            " params_in_nvme configs"
                        )
    ...
    def _swap_in_sub_group_to_flat_buffer(self, flat_buffer, sub_group_id):
        ...
        for param, partitioned_param in zip(
            ...
            if (
                partitioned_param.status
                == PartitionedParamStatus.NOT_AVAILABLE
            ):
                print_rank_0(
                    f"Swapping in {param.ds_id} with elements"
                    f" {param.ds_numel} and partition"
                    f" {param.partition_numel()}"
                )
        ...
        for param, partitioned_param in zip(
            ...
            if (
                ...
                sub_group_partitions.append(
                    (partitioned_param, param.partition_numel(), swap_path)
                )
            else:
                sub_group_partitions.append(
    ...
    def _create_fp32_partitions(self):
            ...
            if self._swappable_optimizer_subgroup(i):
                ...
                if self.params_in_nvme_and_cpu and tensor is None:
                    ...
                    if self.offload_optimizer_fast_init:
                        ...
                        unpinned_fp32_buffer = torch.empty(
                            num_elements, device=self.device, dtype=torch.float
                        )
        ...
        if len(nvme_fp32_dest_tensors) > 0:
            fp16_pinned_buffers = self.fp16_groups[0][
                0
            ].nvme_swapper.reserve_available_buffers()
        ...
        print_rank_0(
            (
                "Swappable FP32 Partitions:"
                f" count={num_swappable_partitions} size={nvme_gigabytes:5.2f} GB"
            ),
            force=False,
        )
        if self.params_in_nvme_and_cpu:
            print_rank_0(
                (
                    "Swap from NVMe Partitions: count ="
                    f" {num_swap_from_nvme_partitions}, size ="
                    f" {swap_from_nvme_memory_usage/GIGA_BYTES:5.2f}GB"
                ),
                force=False,
            )
            print_rank_0(
                (
                    "Swap from CPU Partitions: count ="
                    f" {num_swap_from_cpu_partitions}, size ="
                    f" {swap_from_cpu_memory_usage/GIGA_BYTES:5.2f}GB"
                ),
                force=False,
            )
        ...
        print_rank_0(
            (
                "In-Memory FP32 Partitions:"
                f" count={cpu_memory_sub_groups} size={cpu_memory_gigabytes:5.2f} GB"
            ),
            force=False,
        )
    ...
    def _create_fp16_sub_groups(self, params_group):
        params_group_numel = sum(
            [param.partition_numel() for param in params_group]
        )
    ...
    def initialize_optimizer_states(self):
        ...
        gradient_buffer = torch.zeros(
            int(largest_numel), dtype=gradient_dtype, device=self.device
        )
        ...
        for i, group in enumerate(self.fp16_groups):
            ...
            swappable_param_subgroup = (
                self.fp16_partitioned_groups_flat[i] is None
            )
            ...
            see_memory_usage(
                (
                    f"[Begin] Initialize optimizer states {i} /"
                    f" {num_subgroups} subgroups, num_elems: {num_elements},"
                    " swappable"
                    f" opt/param:{swappable_optimizer_subgroup}/{swappable_param_subgroup}"
                ),
                force=False,
            )
            ...
            if self.offload_optimizer and not swappable_optimizer_subgroup:
                subgroup_gradient_buffer = torch.zeros(
                    num_elements, dtype=gradient_dtype, device=self.device
                )
                if self.offload_optimizer_pin_memory:
                    subgroup_gradient_buffer = (
                        subgroup_gradient_buffer.pin_memory()
                    )

                self.fp32_partitioned_groups_flat[
                    i
                ].grad = subgroup_gradient_buffer
            else:
                self.fp32_partitioned_groups_flat[
                    i
                ].grad = gradient_buffer.narrow(0, 0, num_elements)
            ...
            see_memory_usage(
                (
                    f"[End] Initialize optimizer states {i} /"
                    f" {num_subgroups} subgroups, num_elems: {num_elements},"
                    " swappable"
                    f" opt/param:{swappable_optimizer_subgroup}/{swappable_param_subgroup}"
                ),
        ...
        if not self.offload_optimizer:
            for i, sub_group in enumerate(self.fp16_groups):
                self.averaged_gradients[i] = [
                    ...
                    else torch.zeros_like(param.ds_tensor)
                    for param in sub_group
                ]
    ...
    def report_ipg_memory_usage(self, tag, param_elems):
        ...
        see_memory_usage(
            (
                f"{tag}: elems in_bucket {self.elements_in_ipg_bucket} param"
                f" {param_elems} max_percent {percent_of_bucket_size}"
            ),
            force=False,
        )
    ...
    def reduce_independent_p_g_buckets_and_remove_grads(self, param, i):
        ...
        if (
            ...
            and self.elements_in_ipg_bucket + param.ds_numel
            > self.reduce_bucket_size
        ):
            self.report_ipg_memory_usage(
    ...
    def __add_grad_to_ipg_bucket(self, param: Parameter) -> None:
        ...
        if (
            ...
            and self.elements_in_ipg_bucket + param.grad.numel()
            < self.reduce_bucket_size
        ):
            ...
            with torch.cuda.stream(self.__reduce_and_partition_stream):
                ...
                new_grad_tensor = self.__ipg_bucket_flat_buffer.narrow(
                    0, self.elements_in_ipg_bucket, param.grad.numel()
                ).view_as(param.grad)
    ...
    @instrument_w_nvtx
    @torch.no_grad()
    def __reduce_and_partition_ipg_grads(
        ...
        for param in self.__params_in_ipg_bucket:
            if param.grad.numel() != param.ds_numel:
                raise RuntimeError(
                    f"{param.grad.numel()} != {param.ds_numel} Cannot reduce"
                    " scatter gradients whose size is not same as the params"
                )
        ...
        while (
            self.__param_reduce_events
            and self.__param_reduce_events[0].query()
        ):
        ...
        with torch.cuda.stream(self.__reduce_and_partition_stream):
            ...
            grad_partitions = self.__avg_scatter_grads(
                self.__params_in_ipg_bucket
            )
            self.__partition_grads(
                self.__params_in_ipg_bucket, grad_partitions
            )
    ...
    @instrument_w_nvtx
    def __avg_scatter_grads(
        self, params_to_reduce: List[Parameter]
    ) -> List[Tensor]:
        ...
        if self.postscale_gradients and self.gradient_predivide_factor != 1.0:
            full_grads_for_rank = [
                g.div(self.gradient_predivide_factor)
        ...
        if (
            ...
            and self.gradient_predivide_factor
            != dist.get_world_size(self.dp_process_group)
        ):
            grad_partitions_for_rank = [
                g.mul(self.gradient_predivide_factor)
                for g in grad_partitions_for_rank
            ]

        if self.communication_data_type == torch.float32:
            grad_partitions_for_rank = [
                g.to(dtype) for g in grad_partitions_for_rank
            ]
    ...
    def complete_grad_norm_calculation_for_cpu_offload(self, params):
        ...
        norm_type = 2.0
        for p in params:
            if is_model_parallel_parameter(p) or (
                self.model_parallel_rank == 0
            ):
        ...
        self._model_parallel_all_reduce(
            tensor=total_norm_cuda, op=dist.ReduceOp.SUM
        )
    ...
    def __partition_grads(
        ...
        for param, grad_partition in zip(params_to_release, grad_partitions):
            if (
                param.partition_numel() * dist.get_rank(self.dp_process_group)
                > param.ds_numel
            ):
            ...
            grad_buffer = self.__param_id_to_grad_partition[
                param.ds_id
            ].narrow(0, 0, grad_partition.numel())
            if self.micro_step_id == 0:  # don't accumulate
                ...
                grad_buffer = grad_buffer.to(
                    grad_partition.device, non_blocking=True
                )
                ...
                cuda_grad_buffer = grad_buffer.to(
                    grad_partition.device, non_blocking=True
                )
            ...
            if hasattr(self.__inf_or_nan_tracker, "logical_or_"):
                self.__inf_or_nan_tracker.logical_or_(
                    torch.isinf(grad_buffer).any()
                )
                self.__inf_or_nan_tracker.logical_or_(
                    torch.isnan(grad_buffer).any()
                )
            else:
            ...
            if self.offload_optimizer:
                i, dest_offset, _ = self.grad_position[
                ...
                if self.is_gradient_accumulation_boundary:
                    self.norm_for_param_grads[
                        self.get_param_id(param)
                    ] = self._constant_buffered_norm2(grad_buffer)
                    ...
                    if self._swappable_optimizer_subgroup(i):
                        ...
                        fp32_grad_tensor = self.fp32_partitioned_groups_flat[
                            i
                        ].grad.narrow(0, dest_offset, grad_buffer.numel())
    ...
    def flatten_and_print(self, message, tensors, start=0, n=5):
        ...
        def print_func():
            logger.info(
                flatten_tensor.contiguous().view(-1).narrow(0, start, n)
            )
    ...
    def allreduce_bucket(self, bucket, rank=None, log=None):
        ...
        if rank is None:
            ...
            dist.reduce(
                tensor_to_allreduce, global_rank, group=self.dp_process_group
            )
    ...
    def allreduce_no_retain(
        self, bucket, numel_per_bucket=500000000, rank=None, log=None
    ):
        small_bucket = []
        numel = 0
    ...
    def get_partition_info(self, tensor_list, partition_size, partition_id):
        ...
        for tensor in tensor_list:
            ...
            elif start_index > current_index and start_index < (
                current_index + tensor_size
            ):
                params_in_partition.append(tensor)

                assert first_offset == 0, (
                    "This can happen either zero or only once as this must be"
                    " the first tensor in the partition"
                )
    ...
    def _model_parallel_all_reduce(self, tensor, op):
        ...
        if self.model_parallel_group is None:
            ...
            dist.all_reduce(
                tensor=tensor, op=op, group=self.model_parallel_group
            )
    ...
    def get_grad_norm_direct(self, gradients, params, norm_type=2):
        ...
        if norm_type == inf:
            ...
            self._model_parallel_all_reduce(
                tensor=total_norm_cuda, op=dist.ReduceOp.MAX
            )
            ...
            for g, p in zip(gradients, params):
                if is_model_parallel_parameter(p) or (
                    ...
                    grad_norms.append(
                        g.cuda(non_blocking=True).double().norm(2)
                    )
            ...
            if self.offload_optimizer:
                ...
                norm_groups.append(
                    self.get_grad_norm_direct(
                        self.averaged_gradients[i], self.fp16_groups[i]
                    )
                )
        return norm_groups
    ...
    def _prepare_fp32_grad_for_sub_group(self, sub_group_id):
        ...
        ).to(self.fp32_partitioned_groups_flat[sub_group_id].dtype)

        assert (
            single_grad_partition.numel()
            == self.fp32_partitioned_groups_flat[sub_group_id].numel()
        ), (
            "averaged gradients have different number of elements that"
        ...
        for grad in filter(
            lambda g: g.is_cuda, self.averaged_gradients[sub_group_id]
        ):
    ...
    def _prepare_sub_group(self, sub_group_id, timer_names=set()):
        see_memory_usage(
            f"Before prepare optimizer sub group {sub_group_id}", force=False
        )
        ...
        see_memory_usage(
            f"After prepare optimizer sub group {sub_group_id}", force=False
        )

    def _optimizer_states_and_gradient_swap_in(
        self, sub_group_id, timer_names=set()
    ):
        ...
        assert self._swappable_optimizer_subgroup(sub_group_id), (
            f"Parameter {fp32_param_id} of numel={param_length} is not"
            " swappable"
        )
        ...
        self.optimizer_swapper.swap_in_optimizer_state(
            ...
            async_parameter=self.next_swappable_fp32_partitioned_groups[
                sub_group_id
            ],
        )
    ...
    def _release_sub_group(self, sub_group_id, timer_names=set()):
        see_memory_usage(
            f"Before release optimizer sub group {sub_group_id}", force=False
        )
        ...
        see_memory_usage(
            f"After release optimizer sub group {sub_group_id}", force=False
        )
    ...
    def _optimizer_states_and_gradient_swap_out(
        self, sub_group_id, timer_names=set()
    ):
        param_length = self.fp16_partitioned_groups_flat_numel[sub_group_id]
        fp32_param_id = id(self.fp32_partitioned_groups_flat[sub_group_id])
        ...
        self.optimizer_swapper.swap_out_optimizer_state(
            parameter=self.fp32_partitioned_groups_flat[sub_group_id],
            async_swap=self.next_swappable_fp32_partitioned_groups[
                sub_group_id
            ]
            is not None,
        )
    ...
    def _overflow_clean_up(self, prev_scale):
        see_memory_usage(
            "After overflow before clearing gradients", force=False
        )
        self.zero_grad()
        ...
        see_memory_usage(
            "After overflow after clearing gradients", force=False
        )

        if dist.get_rank() == 0:
            logger.info(
                "[deepspeed] OVERFLOW! Rank {} Skipping step. Attempted loss"
                " scale: {}, reducing to {}".format(
                    dist.get_rank(), prev_scale, self.loss_scale
                )
            )
    ...
    def override_loss_scale(self, loss_scale):
        if loss_scale != self.external_loss_scale:
            logger.info(
                "[deepspeed] setting loss scale from"
                f" {self.external_loss_scale} -> {loss_scale}"
            )
    ...
    def step(self, closure=None):
        ...
        if alloc_retries > self.__n_caching_allocator_flushes:
            if dist.get_rank() == 0:
                logger.warning(
                    (
                        "%d pytorch allocator cache flushes since last step."
                        " this happens when there is high memory pressure and"
                        " is detrimental to performance. if this is happening"
                        " frequently consider adjusting settings to reduce"
                        " memory consumption. If you are unable to make the"
                        " cache flushes go away consider adding"
                        " torch.cuda.empty_cache() calls in your training loop"
                        " to ensure that all ranks flush their caches at the"
                        " same time"
                    ),
    ...
    def dump_pre_step_gradients(self, debug_fp32_grads):
        ...
        for i, _ in enumerate(self.fp16_groups):
            print(
                f"Pre-Step Dump Norms for Group {i} FP16P, FP16G, FP32G,"
                " FP32GUC"
            )
            for fp16_param, fp32_grad in zip(
                self.fp16_groups[i], debug_fp32_grads[i]
            ):
    ...
    def dump_post_step_gradients(self):
        ...
        for i, group in enumerate(self.fp16_groups):
            print(
                f"Post-Step Dump Norms for Group {i} FP16P, FP16DS, FP16FLAT,"
                " FP32FLAT"
            )
            unflat_fp16 = self.unflatten(
                self.fp16_groups_flat[i], self.fp16_groups[i]
            )
            unflat_fp32 = self.unflatten(
                self.fp32_partitioned_groups_flat[i], self.fp16_groups[i]
            )
    ...
    def unscale_and_clip_grads(self, sub_group_id, total_norm):
        ...
        self.fp32_partitioned_groups_flat[sub_group_id].grad.mul_(
            1.0 / combined_scale
        )
    ...
    def has_overflow(self, partition_gradients=True):
        ...
        self._model_parallel_all_reduce(
            tensor=overflow_gpu, op=dist.ReduceOp.MAX
        )
        ...
        if self.offload_optimizer:
            for group in self.fp16_groups:
                for param_idx, param in enumerate(group):
                    group_idx, dest_offset, num_elements = self.grad_position[
                        self.get_param_id(param)
                    ]
                    fp32_grad = self.fp32_partitioned_groups_flat[
                        group_idx
    ...
    def state_dict(self):
        ...
        if self.elastic_checkpoint:
            raise NotImplementedError(
                "ZeRO-3 does not yet support elastic checkpointing, please"
                " disable for now."
            )

        if self.swap_optimizer or self.params_in_nvme_and_cpu:
            raise NotImplementedError(
                "ZeRO-3 does not yet support checkpointing with NVMe"
                " offloading, please disable for now."
            )
    ...
    def _get_flattened_partition(self, all_partition_states):
        ...
        if torch.is_tensor(local_state_partitions[0]):
            return self.flatten_dense_tensors_aligned(
                local_state_partitions, alignment
            )
    ...
    if rank == 0:
        ...
        for v_i, v in enumerate(x.data.contiguous().view(-1)):
            if not math.isfinite(float(v)):
                ...
                break
        logger.info(
            f"rank {rank} detected overflow {cpu_sum} in tensor"
            f" {i}:{t_i} shape {x.shape}"
        )
...
def estimate_zero3_model_states_mem_needs(
    ...
    if cpu_offload:
        if cpu_offload_params:
            ...
            if zero_init:
                cpu_mem = (
                    total_params * 18 * gpus_factor * additional_buffer_factor
                )
            else:
                cpu_mem = (
                    total_params
                    * max(4 * num_gpus_per_node, 18 * gpus_factor)
            ...
            if zero_init:
                cpu_mem = (
                    total_params * 16 * gpus_factor * additional_buffer_factor
                )
            else:
                cpu_mem = (
                    total_params
                    * max(4 * num_gpus_per_node, 16 * gpus_factor)
                    * additional_buffer_factor
                )
    else:
...
def estimate_zero3_model_states_mem_needs_all_cold(
    ...
    print(
        "Estimated memory needed for params, optim states and gradients for"
        f" a:\nHW: Setup with {num_nodes} {nodes_str},"
        f" {num_gpus_per_node} {gpus_str} per node.\nSW: Model with"
    ...
    for cpu_offload in [True, False]:
        for cpu_offload_params in [True, False]:
            ...
            for zero_init in [True, False]:
                ...
                print(
                    f" {cpu_mem/2**30:7.2f}GB | {gpu_mem/2**30:6.2f}GB |"
                    f" {options_str}"
                )

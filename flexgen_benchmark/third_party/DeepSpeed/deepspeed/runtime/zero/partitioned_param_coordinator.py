...
class PartitionedParameterCoordinator:
    ...
    class __InflightParamRegistry(UserDict):
        ...
        def __setitem__(
            self, param: Parameter, handle: AllGatherCoalescedHandle
        ) -> None:
            if param in self.data:
                raise RuntimeError(f"{param.ds_summary()} already in registry")
            if param.ds_status != ZeroParamStatus.INFLIGHT:
                raise RuntimeError(
                    "attempted to add non-inflight parameter to registry"
                    f" {param.ds_summary()}"
                )
    ...
    def trace_prologue(self, sub_module: Module) -> None:
        if self.is_complete_trace():
            ...
            if sub_module != self.__submodule_order[self.__step_id]:
                ...
                debug_rank0(
                    f"Invalidate trace cache @ step {self.__step_id}: expected"
                    f" module {expected_module_id}, but got module"
                    f" {sub_module.id}"
                )
    ...
    def record_parameters(self, sub_module: Module) -> None:
        ...
        for param in sorted(
            set(iter_params(sub_module)), key=lambda p: p.ds_id
        ):
            self.__param_order.append(
                __class__.__ParamInTrace(
                    param=param, step_id_last_used_at=step_id
                )
            )
    ...
    def reset_step(self) -> None:
        ...
        if not self.is_complete_trace():  # not self.trace_complete:
            ...
            assert_ints_same_as_other_ranks(
                [m.id for m in self.__submodule_order]
            )
            ...
            if self.is_record_trace():
                ...
                print_rank_0(
                    (
                        "completed record trace:"
                        f" {[m.id for m in self.__submodule_order]}"
                    ),
                    force=False,
                )
            else:
    ...
    def _dump_params(self, tag, sub_module, params, step_id=None):
        ...
        print(
            f"{tag} step = {step_id} mod ="
            f" {debug_module2name_id(sub_module)} p_names = {param_names}"
        )
    ...
    def fetch_sub_module(self, current_submodule: Module) -> None:
        ...
        debug_rank0(
            f"{self.__step_id}:"
            f" M{current_submodule.id}({type(current_submodule).__name__})"
            f" P{[p.ds_id for p in iter_params(current_submodule)]} "
            + str(
                {
                    "avail": f"{self.__n_available_params:.1e}",
                    "queue_sz": f"{len(self.__param_queue or [])}",
                    "inflight": [
                        p.ds_id for p in self.__inflight_param_registry
                    ],
                }
            )
        )
        ...
        for param in params_to_fetch:
            ...
            if param in self.__inflight_param_registry:
                with torch.cuda.stream(self.__allgather_stream):
                    while (
                        self.__ongoing_fetch_events
                        and self.__ongoing_fetch_events[0].query()
                    ):
                        self.__ongoing_fetch_events.popleft()
                    if (
                        len(self.__ongoing_fetch_events)
        ...
        if self.is_complete_trace():
            ...
            if discarded_from_prefetch_queue != params_not_already_fetched:
                raise RuntimeError(
                    ...
                    f"module id: {current_submodule.id}, training:"
                    f" {current_submodule.training}\n"
                    "expected the next"
                    f" {len(params_not_already_fetched)} parameters in the "
                    "parameter fetch queue to be"
                    f" {tuple(p.ds_summary(use_debug_name=True) for p in params_not_already_fetched)} \n"
                    "but got \n"
                    f" {tuple(p.ds_summary(use_debug_name=True) for p in discarded_from_prefetch_queue)}."
                )
            ...
            if self.__prefetch_bucket_sz > 0:
                ...
                while (
                    self.__param_queue
                    and numel_prefetching < max_params_to_prefetch
                ):
                    param_in_trace: __class__.__ParamInTrace = (
                        self.__param_queue.popleft()
                    )
                    ...
                    do_prefetch = (
                        param_in_trace.param.ds_status
    ...
    def release_sub_module(self, submodule: Module) -> None:
        ...
        params_to_release = (
            ...
            if self.is_complete_trace()
            else set(p.ds_id for p in iter_params(submodule))
        )
        for param in iter_params(submodule):
            param.ds_active_sub_modules.discard(submodule.id)
            if (
                param.ds_id in params_to_release
    ...
    def release_and_reset_all(self, module: Module) -> None:
        ...
        for param in iter_params(module, recurse=True):
            if param in self.__inflight_param_registry:
                raise RuntimeError(
                    f"param {param.ds_summary()} still in flight"
                )
        ...
        for param in iter_params(module, recurse=True):
            if param.ds_status != ZeroParamStatus.NOT_AVAILABLE:
                raise RuntimeError(
                    f"{param.ds_summary()} expected to be released"
                )
    ...
    def __all_gather_params(self, params: Set[Parameter]) -> None:
        ...
        if partitioned_params:
            with torch.cuda.stream(self.__allgather_stream):
                handle = partitioned_params[0].all_gather_coalesced(
                    partitioned_params
                )
            ...
            swap_persisted_params = [
                p
                for p in partitioned_params
                if p.ds_persist
                and p.ds_tensor.final_location == OffloadDeviceEnum.nvme
            ]
            if swap_persisted_params:
                swap_persisted_params[
                    0
                ].nvme_swapper.remove_partition_and_release_buffers(
                    swap_persisted_params
                )
    ...
    def __release_param(self, param: Parameter) -> None:
        if (
            param.ds_status == ZeroParamStatus.AVAILABLE
            and not param.ds_active_sub_modules
        ):
    ...
    def __params_to_release(
        self, submodule_to_release: Module, step_id: int
    ) -> Set[int]:
        ...
        params_to_release = set(
            p.ds_id
            for p in iter_params(submodule_to_release)
            if not p.ds_persist
        )
    ...
    def __prefetch_nvme_param_partitions(self) -> None:
        ...
        numel_in_flight = sum(
            param.ds_numel for param in self.__inflight_param_registry
        )
        ...
        for param_in_trace in self.__param_queue:
            ...
            if param.nvme_swapper is None:
                continue
            if (
                numel_considered > 2 * numel_in_flight
                or len(swap_in_params)

...
class Eigenvalue(object):
    def __init__(
        ...
        log_dist(
            (
                f"enabled eigenvalue with verbose={verbose},"
                f" max_iter={max_iter}, tol={tol}, stability={stability},"
    ...
    def normalize(self, v):
        ...
        normalized_vectors = [
            self.nan_to_num(vector) for vector in normalized_vectors
        ]
    ...
    def compute_eigenvalue(self, module, device=None, scale=1.0):
        ...
        for block in range(self.layer_num):
            ...
            if device is None:
                ...
                v = [
                    torch.randn(p.size(), device=device)
                    for p in model_block.parameters()
                    if p.grad is not None and p.grad.grad_fn is not None
                ]
            ...
            if self.verbose:
                log_dist(
                    (
                        f"block: {block}, power iteration: {i}, eigenvalue:"
                        f" {eigenvalue_current}"
                    ),
                    ranks=[0],
                )
        ...
        if self.verbose:
            log_dist(
                f"post processed block_eigenvalue: {block_eigenvalue}",
                ranks=[0],
            )

        # {param_id: (eigenvalue, layer_id)}
        ev_dict = {}
        for i, (layer_keys, value) in enumerate(
            zip(param_keys, block_eigenvalue)
        ):

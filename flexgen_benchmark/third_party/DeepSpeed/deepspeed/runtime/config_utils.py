...
class DeepSpeedConfigModel(BaseModel):
    ...
    def _process_deprecated_field(self, pydantic_config, field):
        ...
        if dep_param in fields_set:
            logger.warning(
                f"Config parameter {dep_param} is deprecated"
            ...
            if new_param and kwargs.get("set_new_param", True):
                ...
                try:
                    ...
                    logger.error(
                        f"Tried removing deprecated '{dep_param}' from config"
                    )
                    raise e
                ...
                if len(new_param_nested) > 1:
                    ...
                    pydantic_config = reduce(
                        getattr, new_param_nested[:-1], pydantic_config
                    )
                    fields_set = pydantic_config.__fields_set__
                new_param_name = new_param_nested[-1]
                assert new_param_name not in fields_set, (
                    f"Cannot provide deprecated parameter '{dep_param}' and"
                    f" replacing parameter '{new_param}' together"
                )
                ...
                try:
                    ...
                    logger.error(
                        f"Tried setting value for '{new_param}' with value"
                        f" from deprecated '{dep_param}'"
                    )
                    raise e
...
class ScientificNotationEncoder(json.JSONEncoder):
    ...
    def iterencode(self, o, _one_shot=False, level=0):
        ...
        if isinstance(o, bool):
            ...
            return "{" + ", ".join(x) + f"\n{prefix_close}" + "}"
        elif isinstance(o, collections.abc.Sequence) and not isinstance(
            o, str
        ):

...
def copy_to_device(item, device, criterion_func):
    ...
    if criterion_func(item):
        ...
        return {
            k: copy_to_device(v, device, criterion_func)
...
def move_to_device(item, device, criterion_func):
    ...
    if criterion_func(item):
        ...
        return {
            k: move_to_device(v, device, criterion_func)
            for k, v in item.items()
        }
    else:
        return item
...
class PartitionedTensor:
    ...
    def full(self, device=None):
        ...
        for part_id in range(self.num_parts):
            ...
            buf = flat_tensor.narrow(
                0, start=self.partition[part_id], length=part_size
            )
...
def memory_status(msg, print_rank=-1, reset_max=False):
    ...
    print(
        ...
        msg,
        (
            f"device={torch.cuda.current_device()} current"
            f" alloc={new_alloced:0.4f}GB (delta={delta_alloced:0.4f}GB"
            f" max={max_alloced:0.4f}GB) current cache={new_cached:0.4f}GB"
...
def see_memory_usage(message, force=False):
    ...
    logger.info(
        ...
        "         Max_MA"
        f" {round(torch.cuda.max_memory_allocated() / (1024 * 1024 * 1024),2)} GB"
        "         CA"
        f" {round(torch_memory_reserved() / (1024 * 1024 * 1024),2)} GB       "
        "  Max_CA"
        f" {round(torch_max_memory_reserved() / (1024 * 1024 * 1024))} GB "
    )

    vm_stats = psutil.virtual_memory()
    used_GB = round(((vm_stats.total - vm_stats.available) / (1024**3)), 2)
    logger.info(
        f"CPU Virtual Memory:  used = {used_GB} GB, percent ="
        f" {vm_stats.percent}%"
    )
...
def get_only_unique_item(items):
    ...
    if len(item_set) != 1:
        raise RuntimeError(
            f"expected there to be only one unique element in {items}"
        )
...
def clip_gradients(
    parameters, max_norm=1.0, global_grad_norm=None, mpu=None, eps=1e-6
):
...
def clip_tensors_by_global_norm(
    input_tensors, max_norm=1.0, global_norm=None, mpu=None, eps=1e-6
):
...
def all_gather_dp_groups(
    ...
    for group_id, partitioned_params in enumerate(partitioned_param_groups):
        ...
        num_shards = max(
            1,
            partitioned_params[partition_id].numel()
            * dp_world_size
            // allgather_bucket_size,
        )
        ...
        assert (
            shard_size * num_shards <= partitioned_params[partition_id].numel()
        )

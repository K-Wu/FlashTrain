...
class Quantizer(object):
        ...
        for index in range(self.layer_num):
            if self.q_start_bits[index] != self.q_target_bits:
                next_step = self.qsteps + (
                    TWO_D_PARAMS
                    * (self.layer_num if self.layer_num != 0 else 1)
                )
            ...
            for p in parameter_group[i]:
                if (
                    ...
                    if eigenvalue is not None:
                        ...
                        p.data = self.compute_quantization(
                            p.data, layer_id, factor
                        )
                    else:
    ...
    def quantize_tenary(self, inputs):
        ...
        alpha = ((mask * input_flat).abs().sum(dim=1) / mask.sum(dim=1)).view(
            -1, 1
        )
    ...
    def mixed_fp16_quantize(self, input, input_q, index):
        if self.q_mixed_fp16 and self.q_start_bits[index] >= (
            self.q_target_bits - 1
        ):
            input_q = (
                input * self.quantize_real_ratio
    ...
    def compute_quantization(self, input, index=0, factor=1):
        ...
        if input.start_bits != input.target_bits:
            if self.qsteps >= input.q_period:
                ...
                if self.q_verbose:
                    logger.info(
                        "Quantization settings: current bit-precision ="
                        f" {input.start_bits}, step = {self.qsteps},"
                        f" quantization period = {input.q_period}, index ="
                        f" {index}"
                    )
        assert (
            input.start_bits >= input.target_bits
        ), "Quantization bit is lower than target precision bits!"

        if self.use_quantizer_kernel:
            if input.start_bits <= 2:
                raise ValueError(
                    "Quantization bit is too low, please do it without"
                    " quantization kernel!"
                )
            ...
            if input.start_bits >= 3:
                input_flat = self.quantize_highbit(
                    input.data, input.start_bits
                )
            elif input.start_bits == 2:
                assert (
                    self.q_type == "symmetric"
                ), "Quantization type is not symmetric!"
                assert (
                    self.q_rounding == "nearest"
                ), "Quantization rounding is not nearest_neighbor!"
        ...
        if self.use_quantizer_kernel:
            ...
            if self.q_mixed_fp16 and input.start_bits >= input.target_bits - 1:
                input_flat = (
                    self.quantize_real_ratio * input.data

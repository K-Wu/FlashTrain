...
def model_parallel_cuda_manual_seed(seed):
    ...
    _CUDA_RNG_STATE_TRACKER.add(
        _MODEL_PARALLEL_RNG_TRACKER_NAME, model_parallel_seed
    )
...
def get_partition_size(item):
    global mp_rank, mp_size, mp_group
    size = item.numel()
    assert size % mp_size == 0, (
        "Doesn't handle if partition activation if item is not divisible by mp"
        " size"
    )
    partition_size = size / mp_size
    ...
    for i in range(num_args):
        ...
        if device is not None:
            flat_tensor = torch.zeros(
                [tensor_size], dtype=item.dtype, device=device
            )
        else:
            flat_tensor = torch.zeros(
                [tensor_size], dtype=item.dtype, device=item.device
            )
        partitions = []
...
def extract_tensors(all_objects):
    ...
    if type(all_objects) is tuple:
        return (
            tuple(tensor_objects),
            tuple(non_tensor_objects),
            tuple(tensor_flags),
        )
...
def partition_activations(args, cpu_checkpoint, contiguous_checkpoint):
    ...
    for arg_index, item in enumerate(args):
        ...
        partition = (
            ...
            .contiguous()
            .view(-1)
            .narrow(0, get_partition_start(item), partition_size)
            .clone()
        )

        buffer_device = (
            torch.device("cpu") if cpu_checkpoint else partition.device
        )
        ...
        if contiguous_checkpoint:
            ...
            contiguous_data_buffers[i][data_offsets[i]].data[
                range(
                    0,
                    contiguous_data_buffers[i][data_offsets[i]].data.shape[0],
                    int(
                        mmap.PAGESIZE
                        / contiguous_data_buffers[i][
                            data_offsets[i]
                        ].data.element_size()
                    ),
                )
...
def get_partitioned_activations_for_backward(
        ...
        if contiguous_checkpoint:
            ...
            if i >= len(contiguous_size_buffers):
                ...
                contiguous_size_buffers[i] = tmp.new_empty(
                    [numel * num_layers], dtype=size.dtype, device=size.device
                )
                size_offsets[i] = 0

            contiguous_size = (
                contiguous_size_buffers[i]
                .narrow(0, size_offsets[i], numel)
    ...
    def forward(ctx, run_function, all_outputs, *args):
        ...
        if cuda_device is None:
            ...
            if dist.get_rank() == 0:
                ...
                logger.info(
                    f"----Partition Activations {PARTITION_ACTIVATIONS}, CPU"
                    f" CHECKPOINTING {CPU_CHECKPOINT}"
                )
                logger.info(
                    "----contiguous Memory Checkpointing"
                    f" {CONTIGUOUS_CHECKPOINTING} with {num_layers} total"
                    " layers"
                )
        ...
        if PARTITION_ACTIVATIONS:
            inputs = partition_activations(
                args, CPU_CHECKPOINT, CONTIGUOUS_CHECKPOINTING
            )
        elif CPU_CHECKPOINT:
...
def configure(
    ...
    if CONTIGUOUS_CHECKPOINTING:
        assert PARTITION_ACTIVATIONS, (
            "Contiguous Checkpointing is only available with partitioned"
            " activations. Set partitioned activations to true in deepspeed"
            " config"
        )
    if CONTIGUOUS_CHECKPOINTING:
        assert num_layers is not None, (
            "Must specify the number of layers with contiguous memory"
            " checkpointing"
        )

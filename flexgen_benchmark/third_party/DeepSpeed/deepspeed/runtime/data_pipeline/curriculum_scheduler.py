...
class CurriculumScheduler(object):
    def __init__(self, config):
        ...
        if config["schedule_type"] == "fixed_discrete":
            ...
            """
            assert "difficulty" in config["schedule_config"], (
                "Curriculum learning with fixed_discrete schedule requires the"
                " schedule_config 'difficulty'"
            )
            assert "max_step" in config["schedule_config"], (
                "Curriculum learning with fixed_discrete schedule requires the"
                " schedule_config 'max_step'"
            )
            assert len(config["schedule_config"]["max_step"]) > 0
            assert len(config["schedule_config"]["difficulty"]) > 0
            assert (
                len(config["schedule_config"]["difficulty"])
                == len(config["schedule_config"]["max_step"]) + 1
            )
            ...
            """
            assert "total_curriculum_step" in config["schedule_config"], (
                "Curriculum learning with fixed_root schedule requires the"
                " schedule_config 'total_curriculum_step'"
            )
            assert "difficulty_step" in config["schedule_config"], (
                "Curriculum learning with fixed_root schedule requires the"
                " schedule_config 'difficulty_step'"
            )
            assert "root_degree" in config["schedule_config"], (
                "Curriculum learning with fixed_root schedule requires the"
                " schedule_config 'root_degree'"
            )
            if config["schedule_config"]["difficulty_step"] % 8 != 0:
                logger.warning(
                    f"The difficulty_step for curriculum learning has to be"
                    f" multiple of 8 (for FP16 data) or 16 (for INT8 data) to"
                    f" enable NVIDIA Tensor Core acceleration. Disregard this"
                    f" warning if this is unrelated to your hardware."
                )
            self.state["schedule"] = config["schedule_config"]
            ...
            """
            assert "total_curriculum_step" in config["schedule_config"], (
                "Curriculum learning with fixed_linear schedule requires the"
                " schedule_config 'total_curriculum_step'"
            )
            assert "difficulty_step" in config["schedule_config"], (
                "Curriculum learning with fixed_linear schedule requires the"
    ...
    def __fixed_root_get_difficulty(self, global_steps, root_degree=None):
        ...
        next_difficulty = (
            float(global_steps) / s_state["total_curriculum_step"]
        ) ** (1.0 / root_degree)
    ...
    def update_difficulty(self, global_steps):
        if self.state["current_difficulty"] < self.state["max_difficulty"]:
            self.state["current_difficulty"] = self.get_difficulty(
                global_steps
            )
        return self.state["current_difficulty"]

...
class WeightQuantization(object):
    ...
    def quantize_data(self, data, quantize_bits, groups, key=None):
        data_groups = torch.split(
            data.float().view(-1), data.numel() // groups
        )
        max_d = [max(g.max(), g.min().abs()) for g in data_groups]
        data_scale = [
            float(1 << quantize_bits) / (2 * mx + 1e-5) for mx in max_d
        ]
        data_int = [(g * s) for g, s in zip(data_groups, data_scale)]
        data_int = [
            di.round().clamp(
                -(1 << (quantize_bits - 1)), (((1 << (quantize_bits - 1)) - 1))
            )
            for di in data_int
        ]
        data_int = torch.cat(data_int).reshape(data.shape)
        data_int = data_int.to(torch.int8)
        data_scale = torch.cat(
            [s.unsqueeze(0).unsqueeze(0) for s in data_scale]
        )
        return data_int, data_scale

    def is_mlp(self, data, merge_count=1):
        return (self.mp_size * data.shape[0] * merge_count) / data.shape[
            1
        ] == 4 or (self.mp_size * data.shape[1] * merge_count) / data.shape[
            0
        ] == 4

    def is_qkv(self, data):
        return (self.mp_size * data.shape[0]) / data.shape[1] == 3 or (
            self.mp_size * data.shape[1]
        ) / data.shape[0] == 3

    def Quantize(self, value_list, quantize_bits, groups, key, merge_dim=0):
        if self.mlp_extra_grouping and self.is_mlp(
        ...
        for data in value_list:
            data_int, data_scale = self.quantize_data(
                data, quantize_bits, groups, key
            )
            q_scale.append(data_scale)
            value_list[index] = data_int
            index += 1
        q_scale = 1 / torch.cat(q_scale, dim=merge_dim).to(
    ...
    def merge_scales_split(self, split_count):
        ...
        for dense_scale, qkv_scale, m4hh_scale, mh4h_scale in zip(
            ...
            dense_scale = torch.split(
                dense_scale, dense_scale.numel() // split_count
            )
            qkv_scale = torch.split(
                qkv_scale, qkv_scale.numel() // split_count
            )
            m4hh_scale = torch.split(
                m4hh_scale, m4hh_scale.numel() // split_count
            )
            mh4h_scale = torch.split(
                mh4h_scale, mh4h_scale.numel() // split_count
            )
            for s in range(split_count):
                all_scales[s].append(
                    torch.cat(
                        [
                            torch.cat(
                                (qkv_scale[s], torch.zeros_like(qkv_scale[s])),
                                dim=1,
                            ),
                            torch.cat(
                                (
    ...
    def model_quantize(self, model, quantize_policy, quantize_bits, groups):
        ...
        def quantize_fn(layer, policy_cls):
            ...
            for key in range(len(keys)):
                ...
                elif policy_cls is HFBertLayerPolicy and self.is_qkv(
                    keys[key]
                ):
                    data_quantized, data_scale = self.quantize_data(
                ...
                layer_scales.append(
                    (
                        1
                        / data_scale.to(torch.cuda.current_device())
                        .view(-1)
                        .unsqueeze(0)
                    )
                )

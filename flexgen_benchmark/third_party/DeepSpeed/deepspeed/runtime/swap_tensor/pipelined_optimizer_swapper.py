...
class PipelinedOptimizerSwapper(OptimizerSwapper):
    def __init__(
        ...
        super(PipelinedOptimizerSwapper, self).__init__(
            swap_config,
            aio_config,
            base_folder,
            optimizer,
    ...
    def initialize_from_swapped_fp16_params(
        self,
    ...
    def swap_in_optimizer_state(self, parameter, async_parameter):
        ...
        if self.swap_ops[ASYNC_SWAP_IN]:
            ...
            self.swap_ops[SYNC_SWAP_IN] = self._swap_in_optimizer_state(
                aio_handle=self.read_aio_handle, parameter=parameter
            )
        ...
        if self.async_swap_in and async_parameter is not None:
            ...
            self.swap_ops[ASYNC_SWAP_IN] = self._swap_in_optimizer_state(
                aio_handle=self.read_aio_handle, parameter=async_parameter
            )
    ...
    def swap_out_gradients(
        self, parameter, gradient_offsets, gradient_tensors
    ):
    ...
    def _complete_swap_out(self, swap_out_type):
        ...
        self.swap_buffer_manager.free(
            self.swap_ops[swap_out_type].allocated_buffers
        )
    ...
    def _swap_out_optimizer_state(self, aio_handle, parameter, swap_in_op):
        ...
        if len(unpinned_tensors) > 0:
            ...
            for pinned_dst, unpinned_src in zip(
                new_alloc_buffers, unpinned_tensors
            ):
    ...
    def _swap_in_optimizer_state(self, aio_handle, parameter):
        ...
        required_buffer_count = len(param_info.tensors) + (
            1 if param_info.has_gradients() else 0
        )
        ...
        assert allocated_buffers is not None, (
            f"PipelinedOptimizerSwapper ran out of swap buffers, try"
            f" increasing 'buffer_count'"
        )

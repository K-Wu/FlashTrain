...
class SwapBuffer(object):
    ...
    def allocate_tensor(self, swap_path, numel, aligned_numel):
        ...
        return (
            self.swap_tensors[allocate_offset],
            self.compute_tensors[allocate_offset],
        )
    ...
    def allocate_tensor(self, numel, swap_path, aligned_numel):
        if self.has_space(aligned_numel):
            (
                swap_tensor,
                compute_tensor,
            ) = self._get_current_buffer().allocate_tensor(
    ...
    def insert_tensor(self, tensor, swap_path, aligned_numel):
        if self.has_space(aligned_numel):
            (
                swap_tensor,
                compute_tensor,
            ) = self._get_current_buffer().insert_tensor(
...
class SwapBufferManager(object):
    def __init__(self, num_elems, count, dtype):
        ...
        self.gigabytes = (
            self.all_buffers[0].element_size() * num_elems * count
        ) / (1024**3)

        if dist.get_rank() == 0:
            exclude_list = ["all_buffers"]
            print_object(
                obj=self, name="SwapBufferManager", exclude_list=exclude_list
            )
    ...
    def allocate_all(self, num_elems, dtype):
        return self.allocate(
            num_elems=num_elems, count=len(self.free_buffer_index), dtype=dtype
        )
...
def get_sized_buffer(buffer, num_elems):
    ...
    return (
        buffer.narrow(0, 0, num_elems)
        if num_elems < buffer.numel()
        else buffer
    )

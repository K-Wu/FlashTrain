...
class OptimizerStateSwapInfo(object):
    ...
    def _add_tensors(self, tensor_list):
        for t in tensor_list:
            ...
            self.swap_paths.append(
                os.path.join(self.swap_folder, f"{id(t)}.tensor.swp")
            )
    ...
    def get_swap_gradient_buffers(self, swap_buffer):
        ...
        return [
            swap_buffer.narrow(0, grad.offset, grad.length)
            for grad in self.swapped_gradients.values()
        ]
...
class OptimizerSwapper(object):
    def __init__(
        ...
        self.swap_folder = os.path.join(
            base_folder, "optimizer", f"rank{dist.get_rank()}"
        )
    ...
    def swappable_tensor(self, param=None, numel=None):
        ...
        if param is not None:
            return self.min_aio_bytes <= (
                param.numel() * self.swap_element_size
            )
    ...
    def _swap_out_gradients(
        ...
        (
            aligned_gradients,
            aligned_offsets,
        ) = self._adjust_for_misaligned_lengths(
        ...
        if len(swappable_tensors) > 0:
            ...
            gradient_swapper.swap_out_tensors(
                tensor_list=swappable_tensors, path_list=swappable_paths
            )
    ...
    def _initialize_from_swapped_fp16_params(
        ...
        fp32_swap_paths = self._get_swap_paths(
            parameters=fp32_parameters, num_elems=fp16_num_elems
        )
        ...
        assert all(
            ...
            f"numel of fp16 buffers {fp16_buffer_numel} is too small for"
            f" initializing fp32 params {self.largest_numel}"
        )
        ...
        while curr_index < len(fp32_parameters):
            ...
            if dist.get_rank() == 0 and SWAPPER_DEBUG_MODE:
                for i, tensor in enumerate(fp16_pinned_tensors):
                    ...
                    logger.info(
                        "swap_in_fp16_param: fp32_id ="
                        f" {id(fp32_parameters[true_index])} index ="
                        f" {true_index} orig_num_elem ="
                        f" {fp16_num_elems[true_index]}, swap_num_elem ="
                        f" {fp16_pinned_tensors[i].numel()}"
                    )
    ...
    def _swap_in_fp16_params(
        ...
        for i, numel in enumerate(fp16_num_elems):
            pinned_tensor, _ = fp16_swap_buffers.allocate_tensor(
                numel, None, numel
            )
    ...
    def _initialize_parameters(self, parameters, src_tensors, aio_handle):
        ...
        if dist.get_rank() == 0 and SWAPPER_DEBUG_MODE:
            for i, tensor in enumerate(src_tensors):
                logger.info(
                    f"copy_in_fp16_param: fp32_id = {id(parameters[i])} index"
                    f" = {i}, swap_num_elem = {src_tensors[i].numel()}"
                )
    ...
    def _get_swap_paths(self, parameters, num_elems):
        swap_info_list = [
            self._create_param_swap_info(parameter=p, numel=numel)
    ...
    def _swap_out_unpinned_tensors(
        ...
        for i in range(0, unpinned_tensor_count, swap_buffer_count):
            swap_tensor_count = min(
                (unpinned_tensor_count - i), swap_buffer_count
            )

            src_tensors = unpinned_tensors[i : (i + swap_tensor_count)]
            compute_lengths = [t.numel() for t in src_tensors]
            compute_buffers = get_sized_buffers(
                pinned_buffers, compute_lengths
            )
            ...
            swap_lengths = [
                self._io_aligned_numel(t.numel()) for t in src_tensors
            ]
    ...
    def _adjust_for_misaligned_lengths(self, tensors, offsets):
        ...
        for orig_tensor, orig_offset in zip(tensors, offsets):
            ...
            aligned_length = (
                orig_tensor.numel() // self.numel_alignment
            ) * self.numel_alignment
            new_tensors.append(orig_tensor.narrow(0, 0, aligned_length))
            new_offsets.append(orig_offset)

            # remainder tensor
            new_tensors.append(
                orig_tensor.narrow(0, aligned_length, remainder)
            )
    ...
    def _retrieve_unswapped_grad_partitions(self, swap_info, dest_buffer):
        ...
        if SWAPPER_DEBUG_MODE:
            logger.info(
                "optimizer_retrieve_unswapped_gradients:"
                f" param={swap_info.param_id} tensor_count={tensor_count} elem_count={num_elem_count}"
            )
    ...
    def _create_param_swap_info(self, parameter, numel):
        ...
        self.swap_params_info[param_id] = OptimizerStateSwapInfo(
            parameter=parameter, numel=numel, base_folder=self.swap_folder
        )
    ...
    def _io_aligned_numel(self, numel):
        ...
        return (
            numel
            if remainder == 0
            else (numel + self.numel_alignment - remainder)
        )

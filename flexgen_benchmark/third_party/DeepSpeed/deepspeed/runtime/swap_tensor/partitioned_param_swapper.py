...
class AsyncPartitionedParameterSwapper(object):
    ...
    def _configure_aio(self, ds_config):
        ...
        self.min_aio_bytes = max(
            MIN_AIO_BYTES, self.aio_config[AIO_BLOCK_SIZE]
        )
        self.aligned_bytes = (
            AIO_ALIGNED_BYTES * self.aio_config[AIO_THREAD_COUNT]
        )
    ...
    def _get_swap_paths(self, params, must_exist=False):
        ...
        for param in params:
            ...
            if param_id in self.id_to_path.keys():
                ...
                assert (
                    not must_exist
                ), f"Path for param id {param_id} does not exist"
    ...
    def _allocate_and_return_buffers_for_swap_in(self, params):
        ...
        for param in params:
            ...
            assert param_id not in self.param_id_to_buffer_id.keys(), (
                f"param {param_id} already assigned swap buffer id"
                f" {self.param_id_to_buffer_id[param_id]}"
            )
            assert (
            ...
            print_rank_0(
                f"param {param.ds_id} is assigned swap in buffer id"
                f" {buffer_id}  "
            )
    ...
    def remove_partition_and_release_buffers(self, params):
        for param in params:
            ...
            if param_id in self.param_id_to_buffer_id.keys():
                ...
                print_rank_0(
                    f"param {param.ds_id} releases buffer id {buffer_id}  "
                )
    ...
    def _swap_out(self, params, async_op=True):
        ...
        swap_out_tensors(
            self.aio_write_handle, swap_out_params, swap_out_paths
        )
    ...
    def swap_out_and_release(
        self, params, async_op=False, force_buffer_release=False
    ):
        if async_op:
            assert force_buffer_release, (
                "Should not release preallocated buffers without completing"
                " the swap out. Set force_buffer_release to True to do it"
                " anyways"
            )
        self._swap_out(params, async_op=async_op)

    # book keeping function for inflight swap in
    def _update_inflight_swap_in(
    ...
    def swap_in(self, params, async_op=True, swap_in_buffers=None):
        ...
        if swap_in_buffers is None:
            if len(self.available_buffer_ids) < len(swap_in_paths):
                ...
                print_rank_0(
                    (
                        "Not enough swap in buffers"
                        f" {len(self.available_buffer_ids)} for"
                        f" {len(swap_in_paths)} params, ids = {ids}"
                    ),
                    force=True,
                )
                print_rank_0(
                    (
                        f"Num inflight: params {len(self.inflight_params)},"
                        f" buffers {len(self.inflight_swap_in_buffers)}, numel"
                        f" = {self.inflight_numel}"
                    ),
                    force=True,
                )
                print_rank_0(
                    (
                        "Num available params: count ="
                        f" {len(self.available_params)}, ids ="
                        f" {self.available_params}, numel ="
                        f" {self.available_numel}"
                    ),
                    force=True,
                )

            assert len(swap_in_paths) <= len(self.available_buffer_ids), (
                f"Not enough buffers {len(self.available_buffer_ids)} for"
                f" swapping {len(swap_in_paths)}"
            )
            (
                compute_buffers,
                swap_in_buffers,
    ...
    def get_buffer(self, param, numel):
        ...
        assert self.available_swap_in_buffers() > 0, (
            f"No swap buffers to allocate for fp16 param {param_id} of numel ="
            f" {numel}"
        )
        assert (
    ...
    def reserve_partitioned_swap_space(self, partition_num_elems):
        ...
        self.partitioned_swap_buffer = torch.zeros(
            aligned_numel, device="cpu", dtype=self.dtype
        ).pin_memory()
    ...
    def swap_out_partitioned_params(self, dst_fp16_params, src_fp32_params):
        ...
        assert len(dst_fp16_params) == len(src_fp32_params), (
            f"mismatch in number of fp16 params {len(dst_fp16_params)} and"
            f" fp32 params {len(src_fp32_params)}"
        )
        ...
        for i, fp32_tensor in enumerate(src_fp32_params):
            ...
            dst_fp16_params[
                i
            ].ds_tensor.status = PartitionedParamStatus.AVAILABLE

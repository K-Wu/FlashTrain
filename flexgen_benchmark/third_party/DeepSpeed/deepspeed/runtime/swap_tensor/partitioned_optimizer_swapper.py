...
class PartitionedOptimizerSwapper(OptimizerSwapper):
    def __init__(
        ...
        super(PartitionedOptimizerSwapper, self).__init__(
            swap_config,
            aio_config,
            base_folder,
            optimizer,
    ...
    def swap_in_optimizer_state(self, parameter, async_parameter=None):
        ...
        required_buffer_count = len(swap_info.tensors) + (
            1 if swap_info.has_gradients() else 0
        )
    ...
    def swap_out_optimizer_state(self, parameter, async_swap=False):
        ...
        (
            ...
            unpinned_tensors,
            unpinned_paths,
        ) = self._separate_pinned_tensors(swap_info)
        swap_bytes = sum(
            [
    ...
    def _swap_in_pinned_gradients(
        self, aio_handle, parameter, gradient_tensor
    ):
    ...
    def _swap_in_gradients(self, aio_handle, parameter, dest_buffer):
        ...
        if swap_info.swapped_gradients:
            self._swap_in_pinned_gradients(
                aio_handle, parameter, parameter.grad
            )

        if swap_info.unswapped_gradients:
            self._retrieve_unswapped_grad_partitions(
                swap_info=swap_info, dest_buffer=parameter.grad
            )

...
def override_lr_range_test_params(args, params):
    if (
        hasattr(args, LR_RANGE_TEST_MIN_LR)
        and args.lr_range_test_min_lr is not None
    ):
...
def override_1cycle_params(args, params):
    if (
        hasattr(args, CYCLE_FIRST_STEP_SIZE)
        and args.cycle_first_step_size is not None
    ):
...
def get_torch_optimizer(optimizer):
    ...
    if hasattr(optimizer, "optimizer") and isinstance(
        optimizer.optimizer, Optimizer
    ):
        return optimizer.optimizer

    raise TypeError(
        "{} is not a subclass of torch.optim.Optimizer".format(
...
class LRRangeTest(object):
    ...
    def __init__(
        ...
        if isinstance(lr_range_test_min_lr, list) or isinstance(
            ...
            self.min_lr = [lr_range_test_min_lr] * len(
    ...
    def _staircase_interval(self):
        return math.floor(
            float(self.last_batch_iteration + 1) / self.step_size
        )
    ...
    def get_lr(self):
        ...
        return [
            lr_range_test_min_lr * lr_increase
    ...
    def get_last_lr(self):
        ...
        assert (
            getattr(self, "_last_lr", None) is not None
...
class OneCycle(object):
    ...
    def _initialize_momentum(
        ...
        if "betas" not in optimizer.defaults:
            ...
            logger.warn(
                "cycle_momentum is disabled because optimizer"
                f" {optimizer_name} does not support momentum, no betas"
    ...
    def get_lr(self):
        ...
        return self._get_decay_lr(
            self.last_batch_iteration - self.total_size + 1
        )
...
class WarmupLR(object):
    ...
    def __init__(
        ...
        self.min_lrs = self._format_param(
            self.optimizer, warmup_min_lr, "min_lr"
        )
        self.max_lrs = self._format_param(
            self.optimizer, warmup_max_lr, "max_lr"
        )
        self.delta_lrs = [
            big - small for big, small in zip(self.max_lrs, self.min_lrs)
        ]
        self.warmup_num_steps = max(2, warmup_num_steps)
        # Currently only support linear and log function
        if warmup_type not in {WARMUP_LOG_RATE, WARMUP_LINEAR_RATE}:
            logger.warning(
                f"Using unknown warmup_type: {warmup_type}. The increasing"
                " function is set to default (log)"
            )
    ...
    def get_lr(self):
        if self.last_batch_iteration < 0:
            logger.warning(
                "Attempting to get learning rate from scheduler before it has"
                " started"
            )
            return [0.0]

...
class OnebitLamb(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        if self.comm_backend_name == "nccl":
            ...
            assert TORCH_MAJOR >= 1 and TORCH_MINOR >= 8, (
                "Please use torch 1.8 or greater to enable NCCL backend in"
                " 1-bit Adam. Alternatively, please specify 'mpi' as the"
                " 'comm_backend_name' in config file to proceed with the MPI"
                " backend"
            )
            assert (
    ...
    def step(self, closure=None, grads=None):
        ...
        if self.lamb_freeze_key:
            ...
            if (
                "scaling_coeff"
                not in self.state[self.param_groups[0]["params"][0]]
            ):
                    ...
                    momentum_scales.append(
                        [
                            (
                                ...
                                / np.sqrt(
                                    torch.numel(self.state[p]["exp_avg"])
                                )
                            ).item()
                            for p in group["params"]
                        ]
        ...
        for group, grads_this_group in zip(self.param_groups, grads_group):
            ...
            for p, grad in zip(group["params"], grads_this_group):
                ...
                if len(state) == 0 or (
                    len(state) == 1 and "scaling_coeff" in state.keys()
                ):
                    state["step"] = 0
        ...
        if len(self.exp_avg_flat) == 0:
            ...
            updated_params = _unflatten_dense_tensors(
                self.exp_avg_flat[0], momentum_groups
            )
                ...
                for j, p in enumerate(group["params"]):
                    ...
                    if group["weight_decay"] > 0.0:
                        update_ratio = min(
                            1.0,
                            (
                                update_prelim.pow(2).sum().sqrt() / update_norm
                            ).item(),
                        )
                    ...
                    if factor > state["last_factor"] * (
                        1.0 + self.factor_threshold
                    ):
                        factor = state["last_factor"] * (
                            1.0 + self.factor_threshold
                        )
                    if factor < state["last_factor"] * (
                        1.0 - self.factor_threshold
    ...
    def load_state_dict(self, state_dict):
        ...
        if (
            self.state[self.param_groups[0]["params"][0]]["step"]
            < self.freeze_step
        ):
            if dist.get_rank() == 0:
                print(
                    "Checkpoint loaded and OnebitLamb warmup stage"
                    " starts/continues."
                )
            ...
            if dist.get_rank() == 0:
                print(
                    "Checkpoint loaded and OnebitLamb compression stage"
                    " starts/continues."
                )
            if self.lamb_freeze_key is False:
                self.lamb_freeze_key = True

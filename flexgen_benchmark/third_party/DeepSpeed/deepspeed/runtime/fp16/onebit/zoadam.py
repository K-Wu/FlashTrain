...
class ZeroOneAdam(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        if amsgrad:
            raise RuntimeError(
                "0/1 Adam does not support the AMSGrad variant."
            )
        ...
        if self.comm_backend_name == "nccl":
            ...
            assert TORCH_MAJOR >= 1 and TORCH_MINOR >= 8, (
                "Please use torch 1.8 or greater to enable NCCL backend in 0/1"
                " Adam. Alternatively, please specify 'mpi' as the"
    ...
    def step(self, closure=None, grads=None):
        ...
        for group, grads_this_group in zip(self.param_groups, grads_group):
            ...
            for p, grad in zip(group["params"], grads_this_group):
                ...
                if grad.is_sparse:
                    raise RuntimeError(
                        "0/1 Adam does not support sparse gradients"
                    )
                            ...
                            if self.size > 1:
                                with torch.no_grad():
                                    ...
                                    if "exp_avg_mask" in group:
                                        if (
                                            grad_onebit.device
                                            != group["exp_avg_mask"].device
                                        ):
                                            group["exp_avg_mask"] = group[
                                                "exp_avg_mask"
                                            ].to(device=grad_onebit.device)
                ...
                if self.initialize:
                    ...
                    if (
                        state["step"] % state["local_step_interval"] == 0
                        and self.freeze_key
                    ):
                        with torch.no_grad():
                            ...
                            if self.size > 1:
                                ...
                                if "exp_avg_mask" in group:
                                    ...
                                    comm_buffer.mul_(group["exp_avg_mask"])
                            exp_avg.zero_().add_(
                                comm_buffer / state["lrs"], alpha=-1
                            )
                            p.data.add_(
                                comm_buffer
                                / (exp_avg_sq.sqrt() + group["eps"])
                            )
                            comm_buffer.zero_()
    ...
    def load_state_dict(self, state_dict):
        ...
        if (
            ...
            if (
                self.state[self.param_groups[0]["params"][0]]["step"] + 1

...
class FP16_UnfusedOptimizer(DeepSpeedOptimizer):
    ...
    def step_fused_lamb(self, closure=None):
        ...
        for i, group in enumerate(self.fp16_groups):
            ...
            if len(expert_grads_for_norm) > 0:
                expert_norm_group_value = get_weight_norm(
                    _flatten_dense_tensors(expert_grads_for_norm), mpu=self.mpu
                )
        ...
        if self.overflow:
            if self.verbose:
                logger.info(
                    "[deepspeed] fp16 dynamic loss scale overflow! Skipping"
                    " step. Attempted loss scale: {}, reducing to {}".format(
                        prev_scale, self.cur_scale
                    )
                )
        ...
        combined_scale = self.unscale_and_clip_grads(
            self._global_grad_norm, apply_scale=False
        )
        self.optimizer.step(
            grads=grads_groups,
        ...
        for fp32_group, fp16_group in zip(self.fp32_groups, self.fp16_groups):
            for idx, (fp32_param, fp16_param) in enumerate(
                zip(fp32_group, fp16_group)
            ):
    ...
    def step(self, closure=None):
        ...
        for i, group in enumerate(self.fp16_groups):
            ...
            if len(grads_for_norm) > 0:
                norm_group_value = get_weight_norm(
                    grads_for_norm, mpu=self.mpu
                )
            norm_groups.append(norm_group_value)

            # copying gradients to fp32 to wor  k with fp32 parameters
            for fp32_param, fp16_param in zip(
                self.fp32_groups[i], self.fp16_groups[i]
            ):
    ...
    def _update_scale(self, skip):
        if self.dynamic_loss_scale:
            ...
            if skip:
                ...
                if self.verbose:
                    logger.info(
                        "Grad overflow on iteration: %s", self.cur_iter
                    )
                    logger.info(
                        f"Reducing dynamic loss scale from {prev_scale} to"
                        f" {self.cur_scale}"
                    )
            else:
                # Ensure self.scale_window updates since last overflow
                stable_interval = (self.cur_iter - self.last_overflow_iter) - 1
                if (stable_interval > 0) and (
                    stable_interval % self.scale_window == 0
                ):
                    self.cur_scale *= self.scale_factor
                    if self.verbose:
                        logger.info(
                            "No Grad overflow for"
                            f" {self.scale_window} iterations"
                        )
                        logger.info(
                            "Increasing dynamic loss scale from"
                            f" {prev_scale} to {self.cur_scale}"
                        )
        else:
            if skip:
    ...
    def refresh_fp32_params(self):
        for current_group, saved_group in zip(
            self.fp32_groups, self.fp16_groups
        ):

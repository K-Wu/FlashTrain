    ...
    def forward(self, x, num_bits, *args):
        ...
        *args: some extra arguments that are useless but needed for align with the interface of other quantization functions
        ...
        if self.training:
            ...
            self.x_min_max[0] = self.x_min_max[
                0
            ] * self.act_range_momentum + x_min * (1 - self.act_range_momentum)
            self.x_min_max[1] = self.x_min_max[
                1
            ] * self.act_range_momentum + x_max * (1 - self.act_range_momentum)

        x_q = self.act_function(
            x, num_bits, self.x_min_max[0], self.x_min_max[1]
        )
...
class Embedding_Compress(nn.Embedding):
    ...
    def enable_weight_quantization(
        ...
        self.weight_quantization_enabled_in_forward = (
            weight_quantization_enabled_in_forward
        )
        if self.weight_quantization_enabled_in_forward:
            logger.warning(
                "************ A lot of MoQ features are not supported in"
                " quantize_weight_in_forward mode, please consider to use"
                " DS-FP16 optimizer************"
            )
            if self.weight.target_bits >= 3:
                ...
                assert quantization_type == "symmetric", (
                    "Only symmetric quantization is supported for ternary"
                    " weight quantization"
                )
                self.weight_quantizer = TernaryQuantizer.apply
            elif self.weight.target_bits == 1:
                assert quantization_type == "symmetric", (
                    "Only symmetric quantization is supported for binary"
                    " weight quantization"
                )
    ...
    def fix_weight_quantization(self):
        self.weight.data = self.weight_quantizer(
            self.weight,
            self.weight.target_bits,
            None,
            None,
    ...
    def forward(self, input):
        if (
            self.weight_quantization_enabled_in_forward
            and self.weight_quantization_enabled
        ):
...
class LinearLayer_Compress(nn.Linear):
    ...
    def extra_repr(self):
        return (
            "in_features={}, out_features={}, bias={}, sparse pruning={}, row"
            " pruning={}, head pruning={}, activation quantization={},"
            " weight_quantization={}".format(
                self.in_features,
                self.out_features,
                self.bias is not None,
                self.sparse_pruning_method is not None,
                self.row_pruning_method is not None,
                self.head_pruning_method is not None,
    ...
    def enable_sparse_pruning(self, ratio, method):
        ...
        if method == "l1":
            ...
            mask = TopKBinarizer.apply(
                weight_norm, self.sparse_pruning_ratio, False
            )
            mask = mask.view(self.weight.size())
            mask = mask.to(self.weight.device)
        elif method == "topk":
            self.sparse_mask_scores = nn.Parameter(
                torch.Tensor(self.weight.size())
            )
    ...
    def enable_row_pruning(self, ratio, method):
        ...
        if method == "l1":
            ...
            mask = TopKBinarizer.apply(
                weight_norm, self.row_pruning_ratio, False
            )
            mask = mask.view(-1, 1)
            mask = mask.to(self.weight.device)
        elif method == "topk":
            self.row_mask_scores = nn.Parameter(
                torch.Tensor(self.weight.size(0), 1)
            )
            self.row_mask_scores.data = self.row_mask_scores.data.to(
                self.weight.device
            )
    ...
    def enable_head_pruning(self, ratio, method, num_heads):
        ...
        if method not in ["topk"]:
            ...
            self.head_pruning_ratio = ratio
            self.head_pruning_scores = nn.Parameter(
    ...
    def fix_head_pruning_helper(
        self, mask=None, num_heads=None, dim_reduction=False
    ):
        ...
        if mask is None:
            if self.head_pruning_method == "topk":
                ...
                if dim_reduction:
                    ...
                    self.weight = nn.Parameter(
                        self.weight.data.t()
                        .reshape(num_heads, -1)[mask.view(-1), :]
                        .reshape(-1, shape)
                        .t()
                    )
                    self.weight.start_bits = start_bits
                    ...
                    self.weight.data = (
                        (
                            self.weight.data.t().reshape(self.num_heads, -1)
                            * mask.view(-1, 1)
                        )
            ...
            self.weight = nn.Parameter(
                self.weight.data.reshape(num_heads, -1)[
                    mask.view(-1), :
                ].reshape(-1, shape)
            )
            self.weight.start_bits = start_bits
            ...
            if self.bias is not None:
                self.bias = nn.Parameter(
                    self.bias.data.reshape(num_heads, -1)[
                        mask.view(-1), :
                    ].reshape(-1)
                )
        self.head_pruning_enabled = False
    ...
    def get_mask(self, pruning_type="row"):
        if pruning_type == "sparse":
            if self.sparse_pruning_method == "l1":
                ...
                return TopKBinarizer.apply(
                    self.sparse_mask_scores, self.sparse_pruning_ratio, False
                )
            else:
                raise NotImplementedError
        if pruning_type == "row":
            if self.row_pruning_method == "l1":
                ...
                return TopKBinarizer.apply(
                    self.row_mask_scores, self.row_pruning_ratio, False
                )
            else:
                raise NotImplementedError
            ...
            if self.head_pruning_method == "topk":
                return TopKBinarizer.apply(
                    self.head_pruning_scores, self.head_pruning_ratio, False
                )
            else:
                raise NotImplementedError
        else:
    ...
    def enable_activation_quantization(
        self, bits, quantization_type, range_calibration
    ):
        assert bits in [
            4,
            8,
        ], "Only 4/8 bits activation quantization are supported for now"
        self.activation_quantization_bits = bits
        self.activation_quantization_method = (
            f"{quantization_type}_{range_calibration}"
        )
    ...
    def head_pruning_reshape(self, w, mask):
        shape = w.shape
        return (
            (w.t().reshape(self.num_heads, -1) * mask.view(-1, 1))
            .reshape(shape[1], shape[0])
            .t()
        )
...
class Conv2dLayer_Compress(nn.Conv2d):
    ...
    def __repr__(self):
        ...
        return (
            output
            + " sparse pruning={}, channel pruning={}, activation"
            " quantization={}, weight_quantization={}".format(
                self.sparse_pruning_method is not None,
    ...
    def enable_channel_pruning(self, ratio, method):
        ...
        if method == "l1":
            ...
            mask = TopKBinarizer.apply(
                weight_norm, self.channel_pruning_ratio, False
            )
            mask = mask.view(-1, 1, 1, 1)
            mask = mask.to(self.weight.device)
        elif method == "topk":
            self.channel_mask_scores = nn.Parameter(
                torch.Tensor(self.weight.size(0), 1, 1, 1)
            )
        ...
        if mask is None:
            if self.channel_pruning_method in ["l1", "topk"]:
                ...
                if dim_reduction:
                    ...
                    self.weight.data = self.weight.data * mask.view(
                        -1, 1, 1, 1
                    )
                    if self.bias is not None:
                        self.bias.data = self.bias.data * mask.view(-1)
    ...
    def get_mask(self, pruning_type="sparse"):
        if pruning_type == "sparse":
            ...
            if self.channel_pruning_method == "l1":
                ...
                return TopKBinarizer.apply(
                    self.channel_mask_scores, self.channel_pruning_ratio, False
                )
            else:
                raise NotImplementedError
        else:
    ...
    def enable_weight_quantization(
        ...
        if self.weight_quantization_enabled_in_forward:
            assert self.weight.target_bits >= 4, (
                "Only >=4 bits weight quantization are supported during"
...
class ColumnParallelLinear_Compress(LinearLayer_Compress):
    def __init__(
        ...
        super(ColumnParallelLinear_Compress, self).__init__(
            self.input_size, self.output_size_per_partition, bias=bias
        )
...
class RowParallelLinear_Compress(LinearLayer_Compress):
    def __init__(
        ...
        super(RowParallelLinear_Compress, self).__init__(
            self.input_size_per_partition, self.output_size, bias=bias
        )

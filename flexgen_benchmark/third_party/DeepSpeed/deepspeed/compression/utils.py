...
class SymQuantizer(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
        ctx, input, num_bits, min_value=None, max_value=None, num_groups=1
    ):
        """
        Args:
            inputs (`torch.FloatTensor`)
        ...
        if min_value is None:
            ...
            max_input = torch.amax(torch.abs(input), dim=-1).view(
                num_groups, -1
            )
        else:
        ...
        output = (input / scale).round().clamp(
            -q_range // 2, q_range // 2 - 1
        ) * scale

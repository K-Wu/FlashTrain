...
def module_replacement(
    model, module_name, compression_technique=None, mpu=None
):
    """
    ...
    if isinstance(old_module, LinearLayer_Compress) or isinstance(
        old_module, torch.nn.Linear
    ):
        if isinstance(old_module, LinearLayer_Compress):
            new_module = old_module
        else:
            new_module = LinearLayer_Compress(
                old_module.in_features, old_module.out_features, bias=need_bias
            ).to(
    ...
    elif isinstance(old_module, Conv2dLayer_Compress) or isinstance(
        old_module, torch.nn.Conv2d
    ):
        if isinstance(old_module, Conv2dLayer_Compress):
            new_module = old_module
        else:
            new_module = Conv2dLayer_Compress(
                old_module.in_channels,
                old_module.out_channels,
                old_module.kernel_size,
                old_module.stride,
                old_module.padding,
                old_module.dilation,
                old_module.groups,
                need_bias,
                old_module.padding_mode,
            ).to(
                device=old_module.weight.device, dtype=old_module.weight.dtype
            )
            new_module.weight.data = old_module.weight.data
        ...
        new_module = BNLayer_Compress(
            old_module.num_features,
            old_module.eps,
            old_module.momentum,
            old_module.affine,
            old_module.track_running_stats,
        ).to(old_module.weight.device, old_module.weight.dtype)
        new_module.weight.data = old_module.weight.data
        if need_bias:
            new_module.bias.data = old_module.bias.data
        new_module.running_mean.data = old_module.running_mean.data
        new_module.running_var.data = old_module.running_var.data
    elif isinstance(old_module, Embedding_Compress) or isinstance(
        old_module, torch.nn.Embedding
    ):
        if isinstance(old_module, Embedding_Compress):
            new_module = old_module
        else:
            new_module = Embedding_Compress(
                old_module.num_embeddings,
                old_module.embedding_dim,
                old_module.padding_idx,
                old_module.max_norm,
                old_module.norm_type,
                old_module.scale_grad_by_freq,
                old_module.sparse,
            ).to(
                device=old_module.weight.device, dtype=old_module.weight.dtype
            )
        ...
        isinstance(old_module, ColumnParallelLinear_Compress)
        ...
        isinstance(old_module, RowParallelLinear_Compress)
        ...
        for k, v in compression_technique.items():
            if k == SPARSE_PRUNING:
                if v[SPARSE_PRUNING_ENABLED]:
                    new_module.enable_sparse_pruning(
                        v[SPARSE_PRUNING_DENSE_RATIO], v[SPARSE_PRUNING_METHOD]
                    )
            elif k == ROW_PRUNING:
                if v[ROW_PRUNING_ENABLED]:
                    new_module.enable_row_pruning(
                        v[ROW_PRUNING_DENSE_RATIO], v[ROW_PRUNING_METHOD]
                    )
            elif k == HEAD_PRUNING:
                if v[HEAD_PRUNING_ENABLED]:
                    new_module.enable_head_pruning(
                        v[HEAD_PRUNING_DENSE_RATIO],
                ...
                if v[CHANNEL_PRUNING_ENABLED]:
                    new_module.enable_channel_pruning(
                        v[CHANNEL_PRUNING_DENSE_RATIO],
...
def is_module_compressible(module, mpu=None):
    ...
    if mpu is not None:
        ret = (
            ret
            or isinstance(module, mpu.RowParallelLinear)
            or isinstance(module, mpu.ColumnParallelLinear)
        )
...
def compression_preparation(model, compression_techinique_list, mpu):
    ...
    ) in compression_techinique_list:
...
def fix_compression(
    model, module_name, compression_technique, mask=None, dim_reduction=False
):
    """
    ...
    for k, v in compression_technique.items():
        if (
            ...
            and v[WEIGHT_QUANTIZE_IN_FORWARD_ENABLED]
            and v[WEIGHT_QUANTIZE_ENABLED]
        ):
            ...
            return module.fix_row_col_pruning_helper(
                mask, dim_reduction=dim_reduction
            )
        elif k == HEAD_PRUNING and (
            v[HEAD_PRUNING_ENABLED] or mask is not None
        ):
            return module.fix_head_pruning_helper(
                mask, v[HEAD_PRUNING_NUM_HEADS], dim_reduction=dim_reduction
            )
        elif k == CHANNEL_PRUNING and (
            v[CHANNEL_PRUNING_ENABLED] or mask is not None
        ):
            return module.fix_channel_pruning_helper(
                mask, dim_reduction=dim_reduction
            )

...
def check_deepspeed_config(config):
    if isinstance(config, dict):
        ...
        raise ValueError(
            "Expected a string path to an existing deepspeed config, or a"
            f" dictionary. Received: {config}"
        )


def get_module_name(
    group_name, model, key_word, exist_module_name, mpu=None, verbose=True
):
    """
        ...
        if re.search(key_word, name) is not None and module_check:
            if name in exist_module_name and verbose:
                ...
                raise ValueError(
                    ...
                    f" your config file for {group_name}."
                )
...
def get_compress_methods(model, compress_methods, mpu=None):
    ...
    for method, method_content in compress_methods.items():
        ...
        for group_name, method_parameters in method_content[
            DIFFERENT_GROUPS
        ].items():
            ...
            if method_parameters[DIFFERENT_GROUPS_RELATED_MODULE_SCOPE]:
                ...
                for key_word, related_key_words in zip(
                    method_parameters[DIFFERENT_GROUPS_MODULE_SCOPE],
                    method_parameters[DIFFERENT_GROUPS_RELATED_MODULE_SCOPE],
                ):
                    module_name, exist_module_name = get_module_name(
                        group_name, model, key_word, exist_module_name, mpu=mpu
                    )
                    ...
                    for rkw in related_key_words:
                        ...
                        module_name, _ = get_module_name(
                            group_name, model, rkw, set(), mpu=mpu
                        )
                        tmp_related_module_name_list.append(module_name)
                    related_module_name_list.append(
                        tmp_related_module_name_list
                    )
            else:
                for key_word in method_parameters[
...
def init_compression(model, deepspeed_config, teacher_model=None, mpu=None):
    ...
    """
    compress_methods = get_compression_config(
        check_deepspeed_config(deepspeed_config)
    )
    ...
    layer_added_compress_methods = get_compress_methods(
        c_model, compress_methods, mpu=mpu
    )
...
def redundancy_clean(model, deepspeed_config, mpu=None):
    ...
    layer_added_compress_methods_tmp = get_compress_methods(
        c_model, compress_methods, mpu=mpu
    )
    # sort methods
    order_list = [
...
def student_initialization(student_model, teacher_model, deepspeed_config):
    ...
    for s_name, t_name in zip(student_layer, teacher_layer):
        s_module = recursive_getattr(
            student_model, module_name_prefix + "." + str(s_name)
        )
        t_module = recursive_getattr(
            teacher_model, module_name_prefix + "." + str(t_name)
        )
        for s_param, t_param in zip(
            s_module.parameters(), t_module.parameters()
        ):

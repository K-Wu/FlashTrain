...
class compression_scheduler:
    ...
    def make_init(self):
        ...
        for method, method_content in self.compression_config.items():
            ...
            self.different_compression_methods[method][
                TECHNIQUE_ENABLED
            ] = shared_parameters[TECHNIQUE_ENABLED]
            self.different_compression_methods[method][
                SHARED_PARAMETERS
            ] = shared_parameters
    ...
    def check_weight_quantization(self):
        ...
        if not wq[TECHNIQUE_ENABLED]:
            ...
            if (
                self.training_steps
                >= shared_parameters[TECHNIQUE_SCHEDULE_OFFSET]
            ):
                for group_name, module_name_list, method_parameters in wq[
                    DIFFERENT_GROUPS
                ]:
                ...
                if not self.verbose[WEIGHT_QUANTIZATION]:
                    logger.info(
                        "Weight quantization is enabled at step"
    ...
    def check_activation_quantization(self):
        ...
        if not aq[TECHNIQUE_ENABLED]:
            ...
            if (
                ...
                for group_name, module_name_list, method_parameters in aq[
                    DIFFERENT_GROUPS
                ]:
                    for module_name in module_name_list:
                        module = recursive_getattr(self.model, module_name)
                ...
                if not self.verbose[ACTIVATION_QUANTIZATION]:
                    logger.info(
                        "Activation quantization is enabled at step"
                        f" {self.training_steps}"
                    )
    ...
    def check_sparse_pruning(self):
        ...
        if not sp[TECHNIQUE_ENABLED]:
            ...
            if (
                ...
                for group_name, module_name_list, method_parameters in sp[
                    DIFFERENT_GROUPS
                ]:
                    for module_name in module_name_list:
                        module = recursive_getattr(self.model, module_name)
                ...
                if not self.verbose[SPARSE_PRUNING]:
                    logger.info(
                        "Sparse pruning is enabled at step"
                        f" {self.training_steps}"
                    )
    ...
    def check_head_pruning(self):
        ...
        if not hp[TECHNIQUE_ENABLED]:
            ...
            if (
                ...
                for group_name, module_name_list, method_parameters in hp[
                    DIFFERENT_GROUPS
                ]:
                    for module_name in module_name_list:
                        module = recursive_getattr(self.model, module_name)
                ...
                if not self.verbose[HEAD_PRUNING]:
                    logger.info(
                        "Head pruning is enabled at step"
                        f" {self.training_steps}"
                    )
    ...
    def check_row_pruning(self):
        ...
        if not rp[TECHNIQUE_ENABLED]:
            ...
            if (
                ...
                for group_name, module_name_list, method_parameters in rp[
                    DIFFERENT_GROUPS
                ]:
                    for module_name in module_name_list:
                        module = recursive_getattr(self.model, module_name)
                ...
                if not self.verbose[ROW_PRUNING]:
                    logger.info(
                        f"Row pruning is enabled at step {self.training_steps}"
                    )
    ...
    def check_channel_pruning(self):
        ...
        if not cp[TECHNIQUE_ENABLED]:
            ...
            if (
                ...
                for group_name, module_name_list, method_parameters in cp[
                    DIFFERENT_GROUPS
                ]:
                    for module_name in module_name_list:
                        module = recursive_getattr(self.model, module_name)
                ...
                if not self.verbose[CHANNEL_PRUNING]:
                    logger.info(
                        "Channel pruning is enabled at step"
                        f" {self.training_steps}"
                    )

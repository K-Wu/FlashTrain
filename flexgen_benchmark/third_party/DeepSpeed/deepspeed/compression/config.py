...
def get_compression_config(param_dict):
    ...
    output[ACTIVATION_QUANTIZATION] = get_activation_quantization(
        sub_param_dict
    )
...
def get_layer_reduction(param_dict):
    ...
    if get_layer_reduction_enabled(param_dict):
        output[LAYER_REDUCTION_ENABLED] = get_layer_reduction_enabled(
            param_dict
        )
...
def get_weight_quantization(param_dict):
    ...
    if WEIGHT_QUANTIZATION not in param_dict.keys():
        param_dict[WEIGHT_QUANTIZATION] = {
            SHARED_PARAMETERS: {},
            DIFFERENT_GROUPS: {},
        }
    sub_param_dict = param_dict[WEIGHT_QUANTIZATION]
    # shared parameters
    output[SHARED_PARAMETERS] = get_weight_quantization_shared_parameters(
        sub_param_dict
    )
    # each sub-groups
    if output[SHARED_PARAMETERS][WEIGHT_QUANTIZE_ENABLED]:
        assert DIFFERENT_GROUPS in sub_param_dict.keys(), (
            f"Weigh Quantization is enabled, {DIFFERENT_GROUPS} must be"
            " specified"
        )
    output[DIFFERENT_GROUPS] = get_weight_quantization_different_groups(
        sub_param_dict
    )
    return output
...
def get_weight_quantization_shared_parameters(param_dict):
    ...
    if SHARED_PARAMETERS in param_dict.keys():
        ...
        output[WEIGHT_QUANTIZE_TYPE] = get_scalar_param(
            sub_param_dict, WEIGHT_QUANTIZE_TYPE, WEIGHT_QUANTIZE_TYPE_DEFAULT
        )
        ...
        assert output[WEIGHT_QUANTIZE_TYPE] in [
            ...
            "Invalid weight quantize type. Supported types:"
            f" [{WEIGHT_QUANTIZE_SYMMETRIC}, {WEIGHT_QUANTIZE_ASYMMETRIC}]"
        )
        ...
        assert output[WEIGHT_QUANTIZE_ROUNDING] in [
            ...
            "Invalid weight quantize rounding. Supported types:"
            f" [{WEIGHT_QUANTIZE_NEAREST_ROUNDING},"
            f" {WEIGHT_QUANTIZE_STOCHASTIC_ROUNDING}]"
        )
...
def get_weight_quantization_different_groups(param_dict):
    ...
    def get_params(name, group_dict):
        assert WEIGHT_QUANTIZE_START_BITS in group_dict.keys(), (
            f"{WEIGHT_QUANTIZE_START_BITS} must be specified for weight"
            f" quantization group {name}"
        )
        assert WEIGHT_QUANTIZE_TARGET_BITS in group_dict.keys(), (
            f"{WEIGHT_QUANTIZE_TARGET_BITS} must be specified for weight"
            f" quantization group {name}"
        )
    ...
    for k, v in sub_param_dict.items():
        ...
        output[k][DIFFERENT_GROUPS_PARAMETERS] = get_params(
            k, sub_param_dict[k][DIFFERENT_GROUPS_PARAMETERS]
        )
...
def get_activation_quantization(param_dict):
    ...
    if output[SHARED_PARAMETERS][ACTIVATION_QUANTIZATION_ENABLED]:
        assert DIFFERENT_GROUPS in sub_param_dict.keys(), (
            f"Activation Quantization is enabled, {DIFFERENT_GROUPS} must be"
            " specified"
        )
...
def get_activation_quantization_shared_parameters(param_dict):
    ...
    if SHARED_PARAMETERS in param_dict.keys():
        ...
        assert output[ACTIVATION_QUANTIZE_TYPE] in [
            ...
            "Invalid activation quantize type. Supported types:"
            f" [{ACTIVATION_QUANTIZE_SYMMETRIC},"
            f" {ACTIVATION_QUANTIZE_ASYMMETRIC}]"
        )
        ...
        assert output[ACTIVATION_QUANTIZE_RANGE] in [
            ...
            "Invalid activation quantize range calibration. Supported types:"
            f" [{ACTIVATION_QUANTIZE_RANGE_DYNAMIC},"
            f" {ACTIVATION_QUANTIZE_RANGE_STATIC}]"
        )
...
def get_activation_quantization_different_groups(param_dict):
    ...
    def get_params(name, group_dict):
        assert ACTIVATION_QUANTIZE_BITS in group_dict.keys(), (
            f"{ACTIVATION_QUANTIZE_BITS} must be specified for activation"
            f" quantization group {name}"
        )
        return group_dict
...
def get_sparse_pruning(param_dict):
    ...
    if SPARSE_PRUNING not in param_dict.keys():
        param_dict[SPARSE_PRUNING] = {
            SHARED_PARAMETERS: {},
            DIFFERENT_GROUPS: {},
        }
    sub_param_dict = param_dict[SPARSE_PRUNING]
    # shared parameters
    output[SHARED_PARAMETERS] = get_sparse_pruning_shared_parameters(
        sub_param_dict
    )
    # each sub-groups
    if output[SHARED_PARAMETERS][SPARSE_PRUNING_ENABLED]:
        assert (
            DIFFERENT_GROUPS in sub_param_dict.keys()
        ), f"Sparse Pruning is enabled, {DIFFERENT_GROUPS} must be specified"
    output[DIFFERENT_GROUPS] = get_sparse_pruning_different_groups(
        sub_param_dict
    )
    return output
...
def get_sparse_pruning_shared_parameters(param_dict):
    ...
    if SHARED_PARAMETERS in param_dict.keys():
        ...
        output[SPARSE_PRUNING_METHOD] = get_scalar_param(
            sub_param_dict,
        ...
        assert output[SPARSE_PRUNING_METHOD] in [
            ...
            "Invalid sparse pruning method. Supported types:"
            f" [{SPARSE_PRUNING_METHOD_L1}, {SPARSE_PRUNING_METHOD_TOPK}]"
        )
...
def get_sparse_pruning_different_groups(param_dict):
    ...
    def get_params(name, group_dict):
        assert SPARSE_PRUNING_DENSE_RATIO in group_dict.keys(), (
            f"{SPARSE_PRUNING_DENSE_RATIO} must be specified for sparse"
            f" pruning group {name}"
        )
        return group_dict
...
def get_row_pruning(param_dict):
    ...
    # shared parameters
    output[SHARED_PARAMETERS] = get_row_pruning_shared_parameters(
        sub_param_dict
    )
    # each sub-groups
...
def get_row_pruning_shared_parameters(param_dict):
    ...
    if SHARED_PARAMETERS in param_dict.keys():
        sub_param_dict = param_dict[SHARED_PARAMETERS]
        output[ROW_PRUNING_ENABLED] = get_scalar_param(
            sub_param_dict, ROW_PRUNING_ENABLED, ROW_PRUNING_ENABLED_DEFAULT
        )
        output[ROW_PRUNING_METHOD] = get_scalar_param(
            sub_param_dict, ROW_PRUNING_METHOD, ROW_PRUNING_METHOD_DEFAULT
        )
        assert output[ROW_PRUNING_METHOD] in [
            ROW_PRUNING_METHOD_L1,
            ROW_PRUNING_METHOD_TOPK,
        ], (
            "Invalid row pruning method. Supported types:"
            f" [{ROW_PRUNING_METHOD_L1}, {ROW_PRUNING_METHOD_TOPK}]"
        )
...
def get_row_pruning_different_groups(param_dict):
    ...
    def get_params(name, group_dict):
        assert ROW_PRUNING_DENSE_RATIO in group_dict.keys(), (
            f"{ROW_PRUNING_DENSE_RATIO} must be specified for row pruning"
            f" group {name}"
        )
        return group_dict
...
def get_head_pruning(param_dict):
    ...
    if HEAD_PRUNING not in param_dict.keys():
        param_dict[HEAD_PRUNING] = {
            SHARED_PARAMETERS: {},
            DIFFERENT_GROUPS: {},
        }
    sub_param_dict = param_dict[HEAD_PRUNING]
    # shared parameters
    output[SHARED_PARAMETERS] = get_head_pruning_shared_parameters(
        sub_param_dict
    )
    # each sub-groups
    ...
    output[DIFFERENT_GROUPS] = get_head_pruning_different_groups(
        sub_param_dict
    )
    return output
...
def get_head_pruning_shared_parameters(param_dict):
    ...
    if SHARED_PARAMETERS in param_dict.keys():
        sub_param_dict = param_dict[SHARED_PARAMETERS]
        output[HEAD_PRUNING_ENABLED] = get_scalar_param(
            sub_param_dict, HEAD_PRUNING_ENABLED, HEAD_PRUNING_ENABLED_DEFAULT
        )
        output[HEAD_PRUNING_METHOD] = get_scalar_param(
            sub_param_dict, HEAD_PRUNING_METHOD, HEAD_PRUNING_METHOD_DEFAULT
        )
        assert output[HEAD_PRUNING_METHOD] in [
            HEAD_PRUNING_METHOD_L1,
            HEAD_PRUNING_METHOD_TOPK,
        ], (
            "Invalid head pruning method. Supported types:"
            f" [{HEAD_PRUNING_METHOD_L1}, {HEAD_PRUNING_METHOD_TOPK}]"
        )
        ...
        if output[HEAD_PRUNING_ENABLED]:
            assert (
                HEAD_PRUNING_NUM_HEADS in sub_param_dict.keys()
            ), f"{HEAD_PRUNING_NUM_HEADS} must be specified for head pruning"
            output[HEAD_PRUNING_NUM_HEADS] = sub_param_dict[
...
def get_head_pruning_different_groups(param_dict):
    ...
    def get_params(name, group_dict):
        assert (
            HEAD_PRUNING_DENSE_RATIO in group_dict.keys()
...
def get_channel_pruning(param_dict):
    ...
    if CHANNEL_PRUNING not in param_dict.keys():
        param_dict[CHANNEL_PRUNING] = {
            SHARED_PARAMETERS: {},
            DIFFERENT_GROUPS: {},
        }
    sub_param_dict = param_dict[CHANNEL_PRUNING]
    # shared parameters
    output[SHARED_PARAMETERS] = get_channel_pruning_shared_parameters(
        sub_param_dict
    )
    # each sub-groups
    ...
    output[DIFFERENT_GROUPS] = get_channel_pruning_different_groups(
        sub_param_dict
    )
    return output
...
def get_channel_pruning_shared_parameters(param_dict):
    ...
    if SHARED_PARAMETERS in param_dict.keys():
        ...
        assert output[CHANNEL_PRUNING_METHOD] in [
            ...
            "Invalid channel pruning method. Supported types:"
            f" [{CHANNEL_PRUNING_METHOD_L1}, {CHANNEL_PRUNING_METHOD_TOPK}]"
        )
...
def get_channel_pruning_different_groups(param_dict):
    ...
    def get_params(name, group_dict):
        assert CHANNEL_PRUNING_DENSE_RATIO in group_dict.keys(), (
            f"{CHANNEL_PRUNING_DENSE_RATIO} must be specified for channel"
            f" pruning group {name}"
        )
        return group_dict

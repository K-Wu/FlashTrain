...
class FlopsProfiler(object):
        ...
        def register_module_hooks(module, ignore_list):
            ...
            if not hasattr(module, "__start_time_hook_handle"):
                module.__start_time_hook_handle__ = (
                    module.register_forward_pre_hook(start_time_hook)
                )
    ...
    def get_total_duration(self, as_string=False):
        ...
        return (
            duration_to_string(total_duration) if as_string else total_duration
        )
    ...
    def print_model_profile(
        ...
        print(
            "Notations:\ndata parallel size (dp_size), model parallel"
            " size(mp_size),\nnumber of parameters (params), number of"
            " multiply-accumulate operations(MACs),\nnumber of floating-point"
            " operations (flops), floating-point operations per second"
            " (FLOPS),\nfwd latency (forward propagation latency), bwd latency"
            " (backward propagation latency),\nstep (weights update latency),"
        ...
        if self.ds_engine and self.ds_engine.wall_clock_breakdown():
            fwd_latency = (
                self.ds_engine.timers("forward").elapsed(False) / 1000.0
            )
        print(
            "{:<60}  {:<8}".format(
        ...
        if self.ds_engine and self.ds_engine.wall_clock_breakdown():
            bwd_latency = (
                self.ds_engine.timers("backward").elapsed(False) / 1000.0
            )
            step_latency = (
                self.ds_engine.timers("step").elapsed(False) / 1000.0
            )
            print(
                "{:<60}  {:<8}".format(
            ...
            print(
                "{:<60}  {:<8}".format(
                    (
                        "bwd FLOPS per GPU = 2 * fwd flops per GPU / bwd"
                        " latency: "
                    ),
            ...
            print(
                "{:<60}  {:<8}".format(
                    (
                        "fwd+bwd FLOPS per GPU = 3 * fwd flops per GPU /"
                        " (fwd+bwd latency): "
                    ),
                    flops_to_string(
                        3 * total_flops / (fwd_latency + bwd_latency)
                    ),
                )
            )
        ...
        if detailed:
            ...
            print(
                "Each module profile is listed after its name in the following"
                " order: \nparams, percentage of total params, MACs,"
                " percentage of total MACs, fwd latency, percentage of total"
                " fwd latency, fwd FLOPS"
            )
            print(
                "\nNote: 1. A module can have torch.nn.module or"
                " torch.nn.functional to compute logits (e.g."
                " CrossEntropyLoss). They are not counted as submodules, thus"
                " not to be printed out. However they make up the difference"
                " between a parent's MACs (or latency) and the sum of its"
                " submodules'.\n2. Number of floating-point operations is a"
                " theoretical estimation, thus FLOPS computed using that could"
                " be larger than the maximum system throughput.\n3. The fwd"
                " latency listed in the top module's profile is directly"
                " captured at the module forward function in PyTorch, thus"
                " it's less than the fwd latency shown above which is captured"
                " in DeepSpeed.\n"
            )
            print(self.model)
    ...
    def print_model_aggregated_profile(self, module_depth=-1, top_modules=1):
        ...
        if not hasattr(self.model, "__flops__"):
            print(
                "no __flops__ attribute in the model, call this function after"
                " start_profile and before end_profile"
            )
            return
        ...
        print(
            f"Top {top_modules} modules in terms of params, MACs or fwd"
            " latency at different model depths:"
        )
...
def _tensor_addmm_flops_compute(
    self, mat1, mat2, *, beta=1, alpha=1, out=None
):
    """

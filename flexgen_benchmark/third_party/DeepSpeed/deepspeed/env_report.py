...
def nvcc_version():
    ...
    if cuda_home is None:
        return (
            f"{RED} [FAIL] cannot find CUDA_HOME via"
            f" torch.utils.cpp_extension.CUDA_HOME={torch.utils.cpp_extension.CUDA_HOME} {END}"
        )
    try:
...
def debug_report():
    ...
    report = [
        ("torch install path", torch.__path__),
        ("torch version", torch.__version__),
        ("torch cuda version", torch.version.cuda),
        ("torch hip version", hip_version),
        ("nvcc version", (None if hip_version else nvcc_version())),
        ("deepspeed install path", deepspeed.__path__),
        (
            "deepspeed info",
            (
                f"{deepspeed.__version__}, {deepspeed.__git_hash__},"
                f" {deepspeed.__git_branch__}"
            ),
        ),
        (
            ...
            + (
                ...
                if hip_version
                else f"cuda {torch_info['cuda_version']}"
            ),
        ),
    ]
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Suppress display of installation and compatibility statuses of"
            " DeepSpeed operators. "
        ),
    )
    parser.add_argument(

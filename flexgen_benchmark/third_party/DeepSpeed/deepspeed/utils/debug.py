...
def debug_param2name_id_shape(param):
    return (
        f"name={debug_param2name(param)} id={param.ds_id} shape={param.data.shape}"
    )


def debug_param2name_id_shape_device(param):
    return (
        f"name={debug_param2name(param)} id={param.ds_id} shape={param.data.shape} device={param.device}"
    )


def debug_param2name_id_numel(param):
    return (
        f"name={debug_param2name(param)} id={param.ds_id} numel={param.numel()}"
    )


def debug_param2name_id_shape_status(param):
    return (
        f"name={debug_param2name(param)} id={param.ds_id} shape={param.data.shape} status={param.ds_status}"
    )
        ...
        for funcs in grad_fn.next_functions:
            if funcs[0]:
                try:
                    ...
                    print(funcs[0])
                    print(
                        f"Tensor - id: {id(tensor)}, shape: {tensor.shape},"
                        f" data: {tensor}, grad: {tensor.grad}"
                    )

...
def get_full_hp_param(self, optim_state_key=None):
    ...
    if self._hp_mapping is not None:
        ...
        reduce_fragment = torch.narrow(
            reduce_buffer, 0, lp_frag_address.start, lp_frag_address.numel
        )
...
def get_hp_fragment_mapping(
    ...
    assert fragment_start < fragment_end, (
        f"fragment start {fragment_start} should be < fragment_end"
        f" {fragment_end}"
    )

    fragment_numel = fragment_end - fragment_start
    hp_frag_address = fragment_address(
        start=fragment_start - hp_start, numel=fragment_numel
    )
    hp_fragment_tensor = flat_hp_partition.narrow(
        0, hp_frag_address.start, hp_frag_address.numel
    )
    optim_fragment = {
    ...
    lp_frag_address = fragment_address(
        start=fragment_start - lp_start, numel=fragment_numel
    )
    lp_fragment_tensor = lp_param.flatten().narrow(
        0, lp_frag_address.start, lp_frag_address.numel
    )

    return tensor_fragment(
        lp_fragment=lp_fragment_tensor,

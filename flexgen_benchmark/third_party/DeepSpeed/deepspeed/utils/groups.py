...
def initialize(ep_size=1, mpu=None):
    ...
    raise DeprecatedException(
        "Please do not use the groups.initialize() API as it is deprecated."
        " Instead, pass the desired ep_size to"
        " deepspeed.moe.layer.MoE(..,ep_size,..)"
    )
...
def _create_model_parallel(model_parallel_size_):
    ...
    """
    log_dist(
        f"Creating model parallel group with size {model_parallel_size_}",
        ranks=[0],
    )
...
def _create_expert_and_data_parallel(expert_parallel_size_):
    ...
    log_dist(
        (
            "Creating expert and data parallel groups with size"
            f" {expert_parallel_size_}"
        ),
        ranks=[0],
    )
    world_size = dist.get_world_size()
    ...
    if group_name not in _EXPERT_DATA_PARALLEL_GROUP:
        for i in range(expert_parallel_size_):
            ...
            log_dist(
                (
                    "Creating expert data parallel process group named"
                    f" {group_name} with ranks: {list(ranks)}"
                ),
                [0],
            )
    ...
    if group_name not in _EXPERT_PARALLEL_GROUP:
        for i in range(world_size // expert_parallel_size_):
            ranks = range(
                i * expert_parallel_size_, (i + 1) * expert_parallel_size_
            )
            group = dist.new_group(ranks)
            log_dist(
                (
                    "creating expert parallel process group named"
                    f" {group_name} with ranks: {list(ranks)}"
                ),
                [0],
            )
...
def _create_expert_data_and_model_parallel(expert_parallel_size_, mpu):
    ...
    log_dist(
        (
            "Creating deepspeed groups with model parallel size"
            f" {model_parallel_size_}, expert parallel size"
            f" {expert_parallel_size_}, world size {world_size}, dp world size"
            f" {dp_world_size}"
        ),
        [0],
    )
    ...
    if (
        group_name not in _EXPERT_DATA_PARALLEL_GROUP
        and group_name not in _EXPERT_PARALLEL_GROUP
    ):
        (
            expert_parallel_groups,
            expert_data_parallel_groups,
...
def _get_max_expert_size():
    ...
    ), "Warning! Process group not initialized"
    keylist = []
...
def _get_expert_parallel_group(group_name):
    ...
    assert (
        group_name in _EXPERT_PARALLEL_GROUP
...
def _get_expert_data_parallel_group(group_name):
    ...
    assert (
        group_name in _EXPERT_DATA_PARALLEL_GROUP

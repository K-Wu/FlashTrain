...
def parse_optim_states(files, ds_checkpoint_dir):
    ...
    if world_size != total_files:
        raise ValueError(
            ...
            f" '{ds_checkpoint_dir}' but found {total_files} files. Possibly"
            " due to an overwrite of an old checkpoint, or a checkpoint"
            " didn't get saved by one or more processes."
        )
...
def _get_fp32_state_dict_from_zero_checkpoint(ds_checkpoint_dir):
    ...
    print(
        f"Detected checkpoint of type zero stage {zero_stage}, world_size:"
        f" {world_size}"
    )
...
def _get_fp32_state_dict_from_zero2_checkpoint(
    ...
    for shapes, full_single_fp32_vector in zip(
        ...
        if offset != avail_numel:
            raise ValueError(
                f"consumed {offset} numels out of {avail_numel} - something is"
                " wrong"
            )

    print(
        f"Reconstructed fp32 state dict with {total_params} params"
...
def _get_fp32_state_dict_from_zero3_checkpoint(
    ...
    for name, shape in param_shapes.items():
        ...
        if debug:
            print(
                f"{total_params} {name} full shape: {shape} partition0"
                f" numel={partitioned_numel} partitioned_padding_numel={partitioned_padding_numel}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "path to the pytorch fp32 state_dict output file (e.g."
            " path/checkpoint-12/pytorch_model.bin)"
        ),
    )
    parser.add_argument(

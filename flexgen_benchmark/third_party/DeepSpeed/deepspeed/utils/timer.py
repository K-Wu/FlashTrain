...
class SynchronizedWallClockTimer:
    ...
    class Timer:
        ...
        def start(self):
            ...
            assert (
                not self.started_
            ), f"{self.name_} timer has already been started"
        ...
        def stop(self, reset=False, record=False):
            ...
            end_event.record()
            self.event_timers.append(
                CudaEventTimer(self.start_event, end_event)
            )
            self.start_event = None
            self.started_ = False
    ...
    def memory_usage():
        ...
        return " | {} | {} | {} | {}".format(
            alloc, max_alloc, cache, max_cache
        )
    ...
    def log(
        ...
        for name in names:
            if name in self.timers:
                elapsed_time = (
                    self.timers[name].elapsed(reset=reset) / normalizer
                )
...
class ThroughputTimer:
    def __init__(
        ...
        if self.monitor_memory and not PSUTILS_INSTALLED:
            raise ImportError(
                "Unable to import 'psutils', please install package"
            )

...
class OnDevice(object):
    ...
    def __init__(self, dtype, device="meta", enabled=True):
        ...
        if device == "meta":
            if pkg_version.parse("1.10") > pkg_version.parse(
                torch.__version__
            ):
                raise NotImplementedError(
                    "Meta tensor support is not available, please upgrade to"
                    " torch 1.10+"
                )
    ...
    def get_new_tensor_fn_for_dtype(self, dtype: torch.dtype) -> Callable:
        def new_tensor(cls, *args) -> Tensor:
            tensor = OnDevice._orig_torch_empty(
    ...
    def __enter__(self):
        ...
        torch.empty = self.fp_tensor_constructor(
            self._orig_torch_empty, self.dtype
        )
        torch.zeros = self.fp_tensor_constructor(
            self._orig_torch_zeros, self.dtype
        )
        torch.ones = self.fp_tensor_constructor(
            self._orig_torch_ones, self.dtype
        )
        torch.full = self.fp_tensor_constructor(
            self._orig_torch_full, self.dtype
        )

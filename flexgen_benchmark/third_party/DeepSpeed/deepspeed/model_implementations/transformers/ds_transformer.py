...
from deepspeed.ops.transformer.inference.ds_attention import (
...
class DeepSpeedTransformerInference(nn.Module):
    ...
    def __init__(
        ...
        if DeepSpeedTransformerInference.layer_id == 1:
            log_dist(
                f"DeepSpeed-Inference config: {self.config.__dict__}", [0]
            )
        ...
        self.norm_w = nn.Parameter(
            torch.empty(
                self.config.hidden_size, dtype=data_type, device=device
            ),
            requires_grad=False,
        )
    ...
    def forward(
        ...
        head_mask = (
            layer_head_mask if layer_head_mask is not None else head_mask
        )
        ...
        with torch.no_grad():
            ...
            output = self.mlp(
                attention_output, input, inp_norm, self.attention.attn_ob
            )

            if not self.config.pre_layer_norm:
                output = inference_cuda_module.layer_norm(

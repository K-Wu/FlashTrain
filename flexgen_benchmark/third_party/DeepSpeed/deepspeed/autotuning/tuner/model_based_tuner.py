...
class ModelBasedTuner(BaseTuner):
    ...
    def __init__(self, exps: list, resource_manager, metric, tuning_sapce):
        ...
        logger.info(
            f"Create config dim: {self.dims}, all configs:"
            f" {self.num_all_configs}"
        )
    ...
    def update(self):
        for exp_id, (exp, err) in self.rm.finished_experiments.items():
            ...
            if err:
                logger.info(
                    f"Skipping exp_id = {exp_id}, exp_name = {exp['name']},"
                    " the experiment did not run successfully with error ="
                    f" {err}, thus a metrics.txt does not exist for it. Please"
                    f" check the stderr.log in {exp['result_dir']}"
                )
            ...
            with open(p, "r") as f:
                ...
                logger.debug(
                    f"parsing the results for {exp_id}， Result is {curr_iter}"
                )
        ...
        logger.debug(
            f"**Evaluated configs: {len(self.evaluated_configs)}, evaluated"
            f" perf: {self.evaluated_perf}"
        )

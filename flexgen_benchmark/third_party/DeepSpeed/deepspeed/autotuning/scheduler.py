...
class ResourceManager:
        ...
        for exp_path in exp_paths:
            if exp_path in self.exp_paths:
                ...
                with open(exp_path, "r") as fd:
                    ...
                    if os.path.exists(result_dir) and os.path.exists(
                        stderr_file
                    ):
                        if not was_interruptted(stderr_file):
                            ...
                            if (
                                err
                                or os.path.exists(metric_file)
                                or os.path.exists(model_info_file)
                            ):
                                logger.info(
                                    f"Skipping exp {exp['name']} whose result"
                                    " already exists"
                                )
                                continue
    ...
    def run_job(self, exp: dict, reservations):
        ...
        t = threading.Thread(
            ...
            args=(exp, reservations, user_script, user_args),
        )
        t.start()
    ...
    def experiment_check(self, pbar):
        ...
        for exp_id, exp_data in self.running_experiments.items():
            ...
            logger.debug(
                f"Checking exp_id = {exp_id}, alive = {thread.is_alive()}"
            )
            ...
            if not thread.is_alive():
                ...
                logger.debug(
                    f"Finished exp_id = {exp_id}, duration={duration:.2f} sec"
                )
    ...
    def run(self):
        ...
        while len(self.experiment_queue) > 0:
            ...
            if not reservations:
                ...
                logger.debug(
                    f'Put exp_id = {exp["exp_id"]} back into the queue'
                )
    ...
    def parse_results(self, metric):
        ...
        for exp_id, (exp, err) in self.finished_experiments.items():
            if err:
                logger.info(
                    f"The experiment exp_id = {exp_id}, exp_name ="
                    f" {exp['name']}, did not run successfully with error ="
                    f" {err}, thus a metrics.txt does not exist for it. Check"
                    f" the stderr.log in {exp['result_dir']}"
                )
                continue
...
def run_experiment(exp: dict, reservations, user_script, user_args):
    ...
    exp["ds_config_base64"] = base64.urlsafe_b64encode(ds_config_json).decode(
        "utf-8"
    )
    ...
    with open(exp["ds_config_path"], "w", buffering=BUFSIZE) as fd:
        ...
        logger.info(
            f"Scheduler wrote ds_config to {path}, {os.path.abspath(path)}"
        )
    ...
    if user_args:
        ...
        assert idx < len(user_args), (
            "there is no ds_config file specified after --deepspeed_config or"
            " --deepspeed"
        )
    ...
    logger.info(
        f"Launching exp_id = {exp['exp_id']}, exp_name = {exp['name']}, with"
        f" resource = {include_str}, and ds_config ="
        f" {os.path.abspath(ds_config_path)}"
    )
    ...
    logger.info(
        f"Done running exp_id = {exp['exp_id']}, exp_name = {exp['name']},"
        f" with resource = {include_str}"
    )
...
def clean_up(exp: dict, reservations):
    ...
    logger.debug(
        f"Cleaning up exp_id = {exp['exp_id']} on the following workers:"
        f" {nodes_str}"
    )
    ...
    logger.info(
        f"Done cleaning up exp_id = {exp['exp_id']} on the following workers:"
        f" {nodes_str}"
    )

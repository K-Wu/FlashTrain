...
class Autotuner:
    ...
    def __init__(self, args, active_resources):
        ...
        assert tabulate is not None, (
            "Missing required package `tabulate`, please install with `pip"
            " install deepspeed[autotuning]`."
        )
        ...
        ), "DeepSpeed configuration is not provided"
        ...
        if not os.path.exists(self.exps_dir):
            try:
                ...
                logger.info(
                    "Created autotuning experiments directory:"
                    f" {self.exps_dir}"
                )
            except:
                logger.error(
                    f"Failed to create {self.exps_dir}, please check"
                    " `exps_dir` in the autotuning config file is accessible"
                    " by all the nodes in the job."
                )
                exit(-1)

        self.results_dir = self.autotuning_config.results_dir
        if self.autotuning_config.overwrite and os.path.exists(
            self.results_dir
        ):
        ...
        if not os.path.exists(self.results_dir):
            try:
                ...
                logger.info(
                    f"Created autotuning resutls directory: {self.exps_dir}"
                )
            except:
                logger.error(
                    f"Failed to create {self.results_dir}, please check"
                    " `results_dir` in the autotuning config file is"
                    " accessible by all the nodes in the job."
                )
                exit(-1)
        ...
        assert self.exp_num_gpus <= self.rm.num_gpus_per_node, (
            "num_gpus in the autotuning configuration must not be less than"
            " the --num_gpus value in the train script if any"
        )
        assert self.exp_num_nodes <= len(self.rm.nodes), (
            "num_nodes in the autotuning configuration must not be less than"
            " the --num_nodes value in the train script if any"
        )
    ...
    def print_tuning_results(self):
        ...
        if best_space_records:
            ...
            print(summary)
            with open(
                os.path.join(self.results_dir, "summary.txt"),
                "w",
                buffering=BUFSIZE,
            ) as fd:
                fd.write(summary)
        ...
        if GLOBAL_TUNING_SPACE in best_space_records:
            best_exp, best_metric_val, total_num_exps = best_space_records[
                GLOBAL_TUNING_SPACE
            ]
            if best_exp:
                logger.info(
                    f"{best_exp['name']} is the optimal setup after tuning."
                    f" The exp result is at {best_exp['result_dir']}."
                )
            else:
                logger.info(
                    f"No optimal setup is found. Please check that experiments"
                    f" were run successfully."
                )
            tuning_duration = datetime.timedelta(
            ...
            with open(os.path.join(self.results_dir, "summary.txt"), "a") as f:
                f.write(
                    f"\n\nTuning completed in {tuning_duration}. Total number"
                    f" of experiments: {self.rm.experiment_count - 1}."
                )
                f.flush()
    ...
    def _get_user_config(self, user_args):
        ...
        if "--deepspeed_config" in user_args:
            ...
            assert ".json" in user_args[idx + 1], (
                "DeepSpeed --deepspeed_config requires a json file to specify"
                " the configuration"
            )
    ...
    def max_train_micro_batch_size_per_gpu(self):
        if (
            self.max_train_batch_size() and self.max_train_batch_size() > 0
            ...
            return min(
                self.autotuning_config.max_train_micro_batch_size_per_gpu,
                max_train_micro_batch_size,
            )
        else:
    ...
    def fp16_enabled(self):
        if FP16 in self.user_config.keys():
            return self.user_config[FP16].get(
    ...
    def get_instantiation_memory_required_per_gpu(self, zero_stage):
        ...
        mem_per_gpu = (
            params_mem + gradients_mem + optimizer_mem
        ) / self.mp_size()

        return mem_per_gpu

    def _generate_experiments(
        self, tuning_space, max_train_batch_size_per_gpu
    ):
        ...
        if stage == 0:
            ...
            if model_info and "hidden_size" in model_info:
                ...
                template_config[ZERO_OPTIMIZATION]["reduce_bucket_size"] = (
                    hs * hs
                )
        ...
        replace_dict(
            tuning_space,
            self.user_config,
            [ZERO_OPTIMIZATION, TRAIN_MICRO_BATCH_SIZE_PER_GPU],
        )
        ...
        for config in pruned_list:
            ...
            if config_zero:
                if (
                    OFFLOAD_OPTIMIZER not in config_zero
                    and OFFLOAD_OPTIMIZER in exp_config[ZERO_OPTIMIZATION]
                ):
                    del exp_config[ZERO_OPTIMIZATION][OFFLOAD_OPTIMIZER]
                if (
                    OFFLOAD_PARAM not in config_zero
                    and OFFLOAD_PARAM in exp_config[ZERO_OPTIMIZATION]
                ):
    ...
    def tune(self):
        ...
        logger.info(
            "The model has"
            f" {number_to_string(self.get_model_num_params())} parameters."
        )

        self.gpu_mem = self.get_gpu_memory_info()
        logger.info(
            "Memory per GPU in the system is"
            f" {memory_to_string(self.gpu_mem, postfix='B')}."
        )

        self.activation_mem = self.get_activation_memory_per_gpu()
        logger.info(
            "The model requires at least"
            f" {memory_to_string(self.activation_mem, postfix='B')} activation"
            " memory for micro batch size 1."
        )

        # TODO: FIX THIS
        stage = self.user_config.get(ZERO_OPTIMIZATION, {}).get(
            ZERO_OPTIMIZATION_STAGE, "all"
        )
        stage = "all"
        ...
        required_gpu_mem = (
            self.get_instantiation_memory_required_per_gpu(
                ZeroStageEnum.disabled
            )
            + self.activation_mem
        )
        if self.gpu_mem > required_gpu_mem:
            if (
                "all" in user_zero_stages
                or ZeroStageEnum.disabled in user_zero_stages
            ):
                logger.info(
                    "The model might be runable with ZERO 0 (which requires"
                    " at least"
                    f" {memory_to_string(required_gpu_mem, postfix='B')} memory"
                    " with mbs = 1), adding DEFAULT_TUNING_SPACE_ZERO_0 to"
                    " the global tuning space"
                )
            ...
            logger.info(
                "The model is not runable with ZERO stage"
                f" {ZeroStageEnum.disabled} (which requires at least"
                f" {memory_to_string(required_gpu_mem, postfix='B')} memory"
                " with mbs = 1)"
            )
        ...
        if self.gpu_mem > required_gpu_mem:
            if (
                "all" in user_zero_stages
                or ZeroStageEnum.optimizer_states in user_zero_stages
            ):
                logger.info(
                    "The model might be runable with ZERO 1 (which requires"
                    " at least"
                    f" {memory_to_string(required_gpu_mem, postfix='B')} memory),"
                    " adding DEFAULT_TUNING_SPACE_ZERO_1 to the global tuning"
                    " space"
                )
                next_max_mbs, next_mbs, next_metric_val = self.tune_space(
            ...
            logger.info(
                "The model is not runable with ZERO stage"
                f" {ZeroStageEnum.optimizer_states} (which requires at least"
                f" {memory_to_string(required_gpu_mem, postfix='B')} memory"
                " with mbs = 1)"
            )
        ...
        if self.gpu_mem > required_gpu_mem:
            if (
                "all" in user_zero_stages
                or ZeroStageEnum.gradients in user_zero_stages
            ):
                logger.info(
                    "The model might be runable with ZERO 2 (which requires"
                    " at least"
                    f" {memory_to_string(required_gpu_mem, postfix='B')} memory),"
                    " adding DEFAULT_TUNING_SPACE_ZERO_2 to the global tuning"
                    " space"
                )
                next_max_mbs, next_mbs, next_metric_val = self.tune_space(
            ...
            logger.info(
                "The model is not runable with ZERO stage"
                f" {ZeroStageEnum.gradients} (which requires at least"
                f" {memory_to_string(required_gpu_mem, postfix='B')} memory"
                " with mbs = 1)"
            )
        ...
        if self.gpu_mem > required_gpu_mem:
            if (
                "all" in user_zero_stages
                or ZeroStageEnum.weights in user_zero_stages
            ):
                logger.info(
                    "The model might be runable with ZERO 3 (which requires"
                    " at least"
                    f" {memory_to_string(required_gpu_mem, postfix='B')} memory),"
                    " adding DEFAULT_TUNING_SPACE_ZERO_3 to the global tuning"
                    " space"
                )
            ...
            logger.info(
                f"The model has {self.get_model_num_params()} parameters and"
                " requires at least"
                f" {memory_to_string(required_gpu_mem, postfix='B')} memory"
                " per GPU with DeepSpeed Zero stage"
                f" {ZeroStageEnum.weights} optimization. Memory per GPU in"
                f" system is {memory_to_string(self.gpu_mem)}. No tuning is"
                " performed."
            )
            return
    ...
    def tune_space(
        ...
        calculated_max_micro_batch_size = (
            int(
                self.gpu_mem
                - self.get_instantiation_memory_required_per_gpu(stage)
            )
            // self.activation_mem
        )
        logger.info(
            f"Start tuning for space {tuning_space_name},"
            " calculated_max_micro_batch_size ="
            f" {calculated_max_micro_batch_size}"
        )

        if calculated_max_micro_batch_size < prev_max_mbs:
            logger.info(
                f"No need to tune Zero stage {stage}. End tuning for space"
                f" {tuning_space_name}"
            )
            return 0, 0, 0
        ...
        if TRAIN_MICRO_BATCH_SIZE_PER_GPU in self.user_config and isinstance(
            ...
            tuning_micro_batch_sizes = [
                s
                for s in self.user_config[TRAIN_MICRO_BATCH_SIZE_PER_GPU]
                if isinstance(s, int)
            ]
            ...
            ) = self.get_min_max_micro_batch_size(
            ...
            ) = self.get_tuning_micro_batch_size_list(
                min_micro_batch_size,
                max_micro_batch_size,
        ...
        logger.info(
            f"tuning_micro_batch_sizes = {tuning_micro_batch_sizes},"
            f" max_train_batch_size_per_gpu = {max_train_batch_size_per_gpu}"
        )
        ...
        fast_best_mbs = (
            fast_best_record[0][DS_CONFIG][TRAIN_MICRO_BATCH_SIZE_PER_GPU]
            if fast_best_record
            else 0
        )
        logger.info(
            f"fast_best_mbs = {fast_best_mbs}, name ="
            f" {fast_best_record[0]['name']}"
        )
        ...
        if stage > 0:
            if (
                ...
                logger.info(
                    f"End tuning for space: {tuning_space_name}. No need to"
                    " tune other Zero configuration parameters."
                )
                return (
                    max_micro_batch_size,
                    fast_best_mbs,
                    fast_best_metric_val,
                )
        ...
        exps = self._generate_experiments(
            tuning_space, max_train_batch_size_per_gpu
        )
        ...
        if full_best_metric_val > fast_best_metric_val:
            ...
            best_mbs = (
                full_best_record[0][DS_CONFIG][TRAIN_MICRO_BATCH_SIZE_PER_GPU]
                if full_best_record
                else -1
            )
        else:
    ...
    def get_plauteu_mbs(self, tuning_space_name):
        ...
        for exp, metric_val, _ in sorted_space_records:
            ...
            prev_micro_batch_size = exp[DS_CONFIG][
    ...
    def model_info_profile_run(self):
        ...
        model_info_path = os.path.join(
            self.results_dir, "profile_model_info", "model_info.json"
        )
    ...
    def get_best_space_records(self):
        ...
        for space_name, space_records in self.records.items():
            ...
            if best_space_record:
                ...
                if (
                    not global_best_record
                    or best_space_record[1] > global_best_record[1]
                ):
    ...
    def run_tuning_micro_batch_sizes(
        self,
        tuning_micro_batch_sizes,
        max_train_batch_size_per_gpu,
        min_micro_batch_size,
        ...
        assert (
            tuning_micro_batch_sizes
        ), "the tuning micro batch size list is empty"
        ...
        for mbs in tuning_micro_batch_sizes:
            ...
            exp_name = (
                tuning_space_name + "_gas" + str(gas) + "_tmbspg" + str(mbs)
            )
            exp_config = {}
            exp_config["name"] = exp_name
            exp_config[DS_CONFIG] = ds_config
        ...
        for exp_id, (exp, err) in self.rm.finished_experiments.items():
            if exp:
                metric_file = exp[DS_CONFIG][AUTOTUNING][
                ...
                if os.path.exists(metric_file):
                    with open(metric_file, "r") as f:
                        ...
                        self.update_records(
                            tuning_space_name, exp, metric_val, 1
                        )
                        if (
                            max_micro_batch_size
                ...
                logger.info(
                    f"micro batch size = {mbs} was not run successfully"
                )
        self.rm.clear()
        ...
        min_micro_batch_size_with_same_gas = (
            (tuning_micro_batch_sizes[-2] + 1)
            if len(tuning_micro_batch_sizes) > 1
            else min_micro_batch_size
        )
        ...
        stride = (
            max_micro_batch_size - min_micro_batch_size_with_same_gas
        ) // 3
        if stride == 0:
            stride = 1
        for mbs in reversed(
    ...
    def get_min_max_micro_batch_size(
        self, stage, min_micro_batch_size, calculated_max_micro_batch_size
    ):
        ...
        if min_micro_batch_size < 1:
            if (
                TRAIN_MICRO_BATCH_SIZE_PER_GPU in self.user_config
                and isinstance(
                ...
                val = self.get_val_from_user_args(
                    TRAIN_MICRO_BATCH_SIZE_PER_GPU
                )
                if val:
                    mbs = int(val)
            ...
            assert (
                mbs > 0
            ), "The micro batch size per GPU must be greater than 0."
            ds_config[TRAIN_MICRO_BATCH_SIZE_PER_GPU] = mbs
            ds_config[GRADIENT_ACCUMULATION_STEPS] = gas
            ds_config[TRAIN_BATCH_SIZE] = (
                mbs
            ...
            if metric_val:
                ...
                self.update_records(tuning_space_name, exp, 0, 1)
                logger.info(
                    f"User-specified micro batch size per GPU {mbs} does"
                    " not run"
                )
                ...
                if not metric_val:
                    self.update_records(tuning_space_name, exp, 0, 1)
                    logger.info(
                        f"min_train_micro_batch_size_per_gpu {mbs} is not"
                        " runnable."
                    )
                    return -1, -1
                self.update_records(tuning_space_name, exp, metric_val, 1)
        ...
        space_records = (
            self.records[tuning_space_name]
            if tuning_space_name in self.records
            else []
        )
        if space_records:
            prev_idx = min(
                range(len(space_records)),
                key=lambda i: abs(
        ...
        logger.info(
            f"min_micro_batch_size = {min_micro_batch_size},"
            f" max_micro_batch_size = {max_micro_batch_size}."
        )
    ...
    def get_gas_from_user_config(self):
        ...
        if GRADIENT_ACCUMULATION_STEPS in self.user_config:
            ...
            if isinstance(gas_in_config, int):
                ...
                logger.info(
                    f"Specifying a list of {GRADIENT_ACCUMULATION_STEPS} to"
                    " tune is not supported. 1 would be used."
                )
    ...
    def get_tuning_micro_batch_size_list(
        self,
        min_micro_batch_size,
        max_micro_batch_size,
        ...
        if (
            ...
            gas = self.get_gas_from_user_config()
            max_train_batch_size_per_gpu = (
                max_micro_batch_size * gas // self.mp_size()
            )
        logger.info(
            f"max_train_batch_size_per_gpu = {max_train_batch_size_per_gpu}"
        )
        ...
        # constant stride
        stride = (
            max_micro_batch_size - min_micro_batch_size
        ) // num_tuning_micro_batch_sizes
        if stride == 0:
            stride = 1
        ls = []
    ...
    def write_optimal_config(self):
        ...
        if best_exp:
            ...
            ds_config = hjson.load(
                open(os.path.join(exp_dir, "ds_config.json"), "r")
            )
            ds_config.pop(AUTOTUNING)

            ds_config_path = os.path.join(
                self.results_dir, "ds_config_optimal.json"
            )
            ...
            logger.info(
                "Wrote the optimal DeepSpeed configuration found by"
                f" autotuning to {ds_config_path}, and the corresponding"
                f" DeepSpeed command to {cmd_path}"
            )
    ...
    def run_after_tuning(self):
        ...
        if self.optimal_cmd:
            ...
            logger.info(
                "Done running with the optimal DeepSpeed configuration using"
                f" {self.optimal_cmd}"
            )
        else:
            logger.info(
                f"No optimal DeepSpeed configuration found by autotuning."
            )

...
def find_replace_str(value, replace_dict):
    ...
    for var in matches:
        ...
        if var_key == "nvme_path":
            continue
        assert (
            var_key in replace_dict
        ), f"unknown var key: {var_key}, in {replace_dict}"
        if isinstance(replace_dict[var_key], str):
            value = value.replace(var, replace_dict[var_key])
        else:
            assert (
                len(matches) == 1
            ), "unable to replace multiple non-string matches"
...
def validate_ds_config(config: dict):
    ...
    if stage == 1:
        ...
        if (
            config_zero.get("offload_optimizer", {}).get("device")
            in offload_devices
        ):
            offload = True
        if (
            config_zero.get("offload_param", {}).get("device")
            in offload_devices
        ):
            offload = True
    else:
    ...
    def gen_combinations(d: dict):
        ...
        for v in values:
            if not isinstance(v, list):
                v = [v]
        values_choices = (
            gen_combinations(v) if isinstance(v, dict) else get_list(v)
            for v in values
        )
...
def get_first_config(config: dict):
    ...
    for key, val in cfg.items():
        if isinstance(val, dict):
            if (
                key == "optimizer"
            ):  # use user defined optimizer which might have lists of values as params
                cfg[key] = val
            else:

...
class DeepSpeedAutotuningConfig(DeepSpeedConfigObject):
    ...
    def _initialize(self, autotuning_dict):
        self.enabled = get_scalar_param(
            autotuning_dict, AUTOTUNING_ENABLED, AUTOTUNING_ENABLED_DEFAULT
        )

        self.fast = get_scalar_param(
            autotuning_dict, AUTOTUNING_FAST, AUTOTUNING_FAST_DEFAULT
        )

        self.results_dir = get_scalar_param(
            autotuning_dict,
            AUTOTUNING_RESULTS_DIR,
            AUTOTUNING_RESULTS_DIR_DEFAULT,
        )
        assert self.results_dir, "results_dir cannot be empty"
        self.exps_dir = get_scalar_param(
            autotuning_dict, AUTOTUNING_EXPS_DIR, AUTOTUNING_EXPS_DIR_DEFAULT
        )
        assert self.exps_dir, "exps_dir cannot be empty"
        self.overwrite = get_scalar_param(
            autotuning_dict, AUTOTUNING_OVERWRITE, AUTOTUNING_OVERWRITE_DEFAULT
        )
        ...
        self.end_profile_step = get_scalar_param(
            autotuning_dict,
            AUTOTUNING_END_PROFILE_STEP,
            AUTOTUNING_END_PROFILE_STEP_DEFAULT,
        )

        self.metric = get_scalar_param(
            autotuning_dict, AUTOTUNING_METRIC, AUTOTUNING_METRIC_DEFAULT
        )

        self.metric_path = get_scalar_param(
            autotuning_dict,
            AUTOTUNING_METRIC_PATH,
            AUTOTUNING_METRIC_PATH_DEFAULT,
        )

        self.tuner_type = get_scalar_param(
            autotuning_dict,
        ...
        self.tuner_num_trials = get_scalar_param(
            autotuning_dict,
            AUTOTUNING_TUNER_NUM_TRIALS,
            AUTOTUNING_TUNER_NUM_TRIALS_DEFAULT,
        )

        self.arg_mappings = get_dict_param(
            autotuning_dict,
        ...
        self.model_info_path = get_scalar_param(
            autotuning_dict,
            AUTOTUNING_MODEL_INFO_PATH,
            AUTOTUNING_MODEL_INFO_PATH_DEFAULT,
        )
        self.mp_size = get_scalar_param(
            autotuning_dict, AUTOTUNING_MP_SIZE, AUTOTUNING_MP_SIZE_DEFAULT
        )
...
def get_model_info_config(param_dict):
    if MODEL_INFO in param_dict and param_dict[MODEL_INFO] is not None:
        ...
        for key, default_value in MODEL_INFO_KEY_DEFAULT_DICT.items():
            model_info_config[key] = get_scalar_param(
                param_dict[MODEL_INFO], key, default_value
            )

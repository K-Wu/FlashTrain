...
def initialize(
    ...
    lr_scheduler: Optional[
        Union[_LRScheduler, DeepSpeedSchedulerCallable]
    ] = None,
    mpu=None,
...
def _add_core_arguments(parser):
    ...
    group.add_argument(
        ...
        action="store_true",
        help=(
            "Enable DeepSpeed (helper flag for user code, no impact on"
            " DeepSpeed backend)"
        ),
    )
    ...
    group.add_argument(
        ...
        action="store_true",
        help=(
            "Deprecated enable DeepSpeed (helper flag for user code, no impact"
    ...
    group.add_argument(
        ...
        action="store_true",
        help=(
            "Run via MPI, this will attempt to discover the necessary"
            " variables to initialize torch distributed from the MPI"
            " environment"
        ),
    )
...
def init_inference(model, config=None, **kwargs):
    ...
    if isinstance(config, str):
        ...
        raise ValueError(
            "'config' argument expected string or dictionary, got"
            f" {type(config)}"
        )
    ...
    for key in overlap_keys:
        if config_dict[key] != kwargs[key]:
            raise ValueError(
                f"Conflicting argument '{key}' in"
                f" 'config':{config_dict[key]} and kwargs:{kwargs[key]}"
            )

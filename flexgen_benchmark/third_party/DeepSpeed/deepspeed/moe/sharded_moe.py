...
def _capacity(
    ...
    capacity = torch.ceil((num_tokens / num_experts) * capacity_factor).to(
        torch.int64
    )
...
class MOELayer(Base):
    ...
    def __init__(
        ...
        if self.use_tutel:
            ...
            logger.warning(
                "To enable Tutel optimization, use top-1 instead of top-2"
                " gate. Proceeding without Tutel."
            )
    ...
    def forward(self, *input: Tensor, **kwargs: Any) -> Tensor:
        ...
        if self.use_tutel:
            (
                ...
                gates_,
                self.exp_counts,
            ) = self.gate(reshaped_input, input[1], True)
            ...
            (
                ...
                dispatch_mask,
                self.exp_counts,
            ) = self.gate(reshaped_input, input[1])

...
class MoE(torch.nn.Module):
    ...
    def __init__(
        ...
        self.enable_expert_tensor_parallelism = (
            enable_expert_tensor_parallelism
        )
        assert num_experts % ep_size == 0, (
            f"Number of experts ({num_experts}) should be divisible by expert"
            f" parallel size ({ep_size})"
        )
        ...
        experts = Experts(
            expert, self.num_local_experts, self.expert_group_name
        )
    ...
    def _create_process_groups(self):
        ...
        if (
            ...
            print(
                "No existing process group found, creating a new group named:"
                f" {self.expert_group_name}"
            )
            if (groups.mpu is None) or (
                not self.enable_expert_tensor_parallelism
            ):

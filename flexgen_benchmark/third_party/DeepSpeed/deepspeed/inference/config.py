...
class DeepSpeedInferenceConfig(DeepSpeedConfigModel):
    ...
    config: Dict = Field(
        None, alias="args"
    )  # todo: really no need for this field if we can refactor
    ...
    mp_size: int = Field(
        1, deprecated=True, new_param="tensor_parallel.tp_size"
    )
    """
    ...
    ep_group: object = Field(
        None, alias="expert_group", deprecated=True, new_param="moe.ep_group"
    )
    ep_mp_group: object = Field(
        None,
        alias="expert_mp_group",
        deprecated=True,
        new_param="moe.ep_mp_group",
    )
    moe_experts: list = Field(
        [1], deprecated=True, new_param="moe.moe_experts"
    )
    moe_type: MoETypeEnum = Field(
        MoETypeEnum.standard, deprecated=True, new_param="moe.type"
    )

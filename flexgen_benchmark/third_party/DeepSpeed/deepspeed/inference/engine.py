...
class InferenceEngine(Module):
    ...
    def __init__(self, model, config):
        ...
        self._get_model_config_generate(
            config
        )  # keep for weird backward compatibility
        ...
        if config.enable_cuda_graph:
            assert pkg_version.parse(torch.__version__) >= pkg_version.parse(
                ...
                "If you want to use cuda graph, please upgrade torch to at"
                " least v1.10"
            )
        ...
        if config.tensor_parallel.tp_size > 1:
            _rng_state = torch.cuda.get_rng_state().to(
    ...
    def profile_model_time(self, use_cuda_events=True):
        if (
            not self.model_profile_enabled
            and not self._config.enable_cuda_graph
        ):
    ...
    def _post_forward_hook(self, module, input, output):
        if self.use_cuda_events:
            ...
            elapsed_time = self.timers(INFERENCE_MODEL_TIMER).elapsed(
                reset=True
            )
        else:
            torch.cuda.synchronize()
    ...
    def _create_ep_parallel_group(self, moe_experts):
        ...
        moe_experts = (
            moe_experts if type(moe_experts) is list else [moe_experts]
        )
            ...
            if dist.get_world_size() > moe_ep_size:
                ...
                for i in range(num_expert_mp_groups):
                    ...
                    if dist.get_rank() in expert_mp_comm_ranks:
                        self.expert_mp_group.update(
                            {moe_ep_size: _expert_mp_group}
                        )
    ...
    def _validate_args(self, mpu, replace_with_kernel_inject):
        ...
        if (
            not isinstance(self._config.tensor_parallel.tp_size, int)
            or self._config.tensor_parallel.tp_size < 1
        ):
            raise ValueError(
                "mp_size must be an int >= 1, got"
                f" {self._config.tensor_parallel.tp_size}"
            )
        ...
        if self._config.checkpoint is not None and not isinstance(
            self._config.checkpoint, (str, dict)
        ):
            raise ValueError(
                "checkpoint must be None, str or dict, got"
                f" {type(self._config.checkpoint)}"
            )

        supported_dtypes = [None, torch.half, torch.int8, torch.float]
        if self._config.dtype not in supported_dtypes:
            raise ValueError(
                f"{self._config.dtype} not supported, valid dtype:"
                f" {supported_dtypes}"
            )
        ...
        if self.injection_dict is not None and not isinstance(
            ...
            raise ValueError(
                "injection_dict must be None or a dict, got:"
                f" {self.injection_dict}"
            )
    ...
    def _apply_injection_policy(self, config, client_module=None):
        ...
        checkpoint = (
            SDLoaderFactory.get_sd_loader_json(
                checkpoint_dir, self.checkpoint_engine
            )
    ...
    def _load_checkpoint(self, load_dir, load_module_strict=True, tag=None):
        ...
        if not isinstance(load_dir, dict) and os.path.isdir(load_dir):
            ...
            sd_loader = SDLoaderFactory.get_sd_loader(
                ckpt_list, self.checkpoint_engine
            )
        else:
            sd_loader = SDLoaderFactory.get_sd_loader_json(
                load_dir, self.checkpoint_engine
            )
        ...
        if type(sd_loader) is list:
            ...
            load_path, checkpoint, quantize_config = sd_loader.load(
                self._config.tensor_parallel.tp_size,
                mp_rank,
    ...
    def _choose_module_key(self, sd):
        assert not ("module" in sd and "model" in sd), (
            "checkpoint has both 'model' and 'module' keys, not sure how to"
            " proceed"
        )
        assert "module" in sd or "model" in sd, (
            "checkpoint contains neither 'model' or 'module' keys, not sure"
            " how to proceed"
        )
        if "module" in sd:
            return "module"
    ...
    def _convert_to_dtype(self, config):
        ...
        if (
            False
        ):  # config.dtype is torch.int8 and self.quantization_scales is None:
    ...
    def _create_cuda_graph(self, *inputs, **kwargs):
        ...
        with torch.cuda.graph(self._cuda_graphs):
            self.static_output = self.module(
                *self.static_inputs, **self.static_kwargs
            )
    ...
    def model_times(self):
        ...
        if self._config.enable_cuda_graph and len(self._model_times) == 0:
            raise ValueError(
                "Model times are empty and cuda graph is enabled. If this is"
                " a GPT-style model this combo is not supported. If this is a"
                " BERT-style model this is a bug, please report it. Model"
                f" type is: {type(self.module)}"
            )
        self._model_times = []
        return model_times

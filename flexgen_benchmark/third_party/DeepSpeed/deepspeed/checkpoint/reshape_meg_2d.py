...
class meg_2d_parallel_map(object):
    ...
    def get_data(self, pp_index=None, tp_index=None):
        self._validate_indices(pp_index, tp_index)
        pp_indices = (
            list(range(self.pp_degree)) if pp_index is None else [pp_index]
        )
        tp_indices = (
            list(range(self.tp_degree)) if tp_index is None else [tp_index]
        )
...
def reshape_meg_2d_parallel(
    old_pp_degree, old_tp_degree, new_pp_degree, new_tp_degree, verbose=False
):
...
def reshape(src, tgt):
    ...
    tp_ranks1, pp_ranks1, dp_ranks1 = get_mpu_ranks(
        tp_size=tp_size_src, pp_size=pp_size_src, dp_size=dp_size_src
    )
    tp_ranks2, pp_ranks2, dp_ranks2 = get_mpu_ranks(
        tp_size=tp_size_tgt, pp_size=pp_size_src, dp_size=dp_size_src
    )
    tp_ranks3, pp_ranks3, dp_ranks3 = get_mpu_ranks(
        tp_size=tp_size_tgt, pp_size=pp_size_tgt, dp_size=dp_size_src
    )

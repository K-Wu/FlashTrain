...
def merge_state_dict(dict_a, dict_b, key_list):
    ...
    for key, value in dict_b.items():
        if key in dict_a.keys():
            merged_dict[key] = merge_state(
                dict_a[key], dict_b[key], [str(key)]
            )
        else:
...
def merge_state_list(list_a, list_b, key_list):
    if len(list_a) != len(list_b):
        ...
        raise ValueError(
            f"Cannot merge lists of different lengths, a = {len(list_a)} b ="
            f" {len(list_b)}"
        )
...
def merge_state(state_a, state_b, key_list=[]):
    if type(state_a) != type(state_b):
        ...
        raise ValueError(
            f"Cannot merge two states of types {type(state_a)} and type"
            f" {type(state_b)}"
        )

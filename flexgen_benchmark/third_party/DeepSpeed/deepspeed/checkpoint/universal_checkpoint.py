...
def load_hp_checkpoint_state(self, folder, tp_rank, tp_world_size):
    ...
    checkpoint_files = {
        key: os.path.join(folder, f"{key}.pt") for key in hp_keys
    }
    ...
    for key in hp_keys:
        ...
        if vocab_divisibility_padding_tensor is not None:
            ...
            if padded_target_vocab_size > full_hp_param.shape[0]:
                ...
                full_hp_param = torch.nn.functional.pad(
                    full_hp_param, (0, 0, 0, padding_size), "constant", 0
                )
                full_hp_param[
                    :-padding_size, :
                ] = vocab_divisibility_padding_tensor
        ...
        assert full_param_numel == tp_world_size * tp_slice_numel, (
            f"Loading {ckpt_file} full param numel {full_param_numel} !="
            f" tensor slice numel {tp_slice_numel} * tp_world_size"
            f" {tp_world_size}"
        )
        dst_tensor = (
            hp_mapping.hp_fragment
            if key == FP32_WEIGHT_KEY
            else hp_mapping.get_optim_state_fragment(key)
        )
        ...
        tp_hp_fragment = tp_hp_slice.narrow(
            0, lp_frag_address.start, lp_frag_address.numel
        )
        assert dst_tensor.numel() == lp_frag_address.numel, (
            f"Load checkpoint {key} dst_tensor numel {dst_tensor.numel()} !="
            f" src numel {lp_frag_address.numel}"
        )
...
def enable_universal_checkpoint(param_list):
    for param in param_list:
        param.load_hp_checkpoint_state = types.MethodType(
            load_hp_checkpoint_state, param
        )

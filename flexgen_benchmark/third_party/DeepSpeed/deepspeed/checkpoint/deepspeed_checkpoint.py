...
class DeepSpeedCheckpoint(object):
    def __init__(self, dir, tp_degree=None, pp_degree=None, dp_degree=None):
        ...
        self.layer_files = get_files_with_prefix(
            self.file_list, LAYER_FILE_PREFIX
        )
        self.mp_rank_files = get_files_with_prefix(
            self.file_list, MODEL_FILE_PREFIX
        )
        ...
        if (
            self.is_change_pp_degree()
            or self.is_change_tp_degree()
    ...
    def show_tp_final_norm_map(self):
        self._dump_mapping(
            self.tp_to_final_norm_map, "tp_to_final_norm_layers"
        )

    def show_pp_tranformer_map(self):
        self._dump_mapping(
            self.pp_to_transformer_map, "pp_to_tranformer_layers"
        )

    def show_transformer_file_map(self):
        self._dump_mapping(
            self.transformer_file_map, "rank_to_tranformer_files"
        )
    ...
    def get_zero_files(self, pp_index, tp_index, dp_index) -> list:
        return self.zero_checkpoint.get_files_for_rank(
    ...
    def get_2d_parallel_state(self, tp_index: int, pp_index: int) -> dict:
        ...
        fname_list = self.get_2d_parallel_files(
            tp_index=tp_index, pp_index=pp_index
        )
        sd_list = [
            torch.load(fname, map_location=torch.device("cpu"))
    ...
    def _build_tp_other_layer_map(self, layer_index: int):
        ...
        layer_files = get_files_with_prefix(
            self.layer_files, self.layer_keys[layer_index]
        )
    ...
    def get_2d_parallel_files(self, tp_index: int, pp_index: int) -> list:
        assert tp_index < self.tp_degree
        assert pp_index < self.pp_degree
        file_indices = self.new_2d_map.get_data(
            pp_index=pp_index, tp_index=tp_index
        )
    ...
    def _sanity_check(self):
        ...
        assert (
            self.zero_checkpoint.num_files % (self.pp_degree * self.tp_degree)
            == 0
        )
        ...
        for key in sd_list[0].keys():
            if not key in SEQUENTIAL_LAYERS:
                ...
                merged_sd[key] = torch.cat(
                    [sd[key] for sd in sd_list], dim=cat_dim
                )
            else:
    ...
    def _validate_folder(self, dir):
        ...
        for file_prefix in [
            ...
            assert len(ckpt_files) > 0, (
                f"{dir} seems a bogus DeepSpeed checkpoint folder: Cannot find"
                f" {file_prefix}* files in there."
            )

...
class model_3d_desc(object):
    ...
    def reshape(self, target_3d_desc, verbose=False):
        ...
        flat_3d_map = flatten_dp_dimension(
            meg_2d_map=tgt_2d_map,
            src_2d_size=self.pp_degree * self.tp_degree,
            dp_degree=self.dp_degree,
        )

        return unflatten_dp_dimension(
            meg_2d_map=flat_3d_map, dp_degree=target_3d_desc.dp_degree
        )

    def get_desc(self):
        return (
            f"{PP_DIM},{TP_DIM},{DP_DIM} = ({self.pp_degree},"
    ...
    def is_valid(self, pp_index, tp_index, dp_index):
        ...
        for index, degree, dim_name in [
            ...
            if index >= degree:
                valid = False
                err_msg.append(
                    f"{dim_name} indexing error: index {index} >= degree"
                    f" {degree}"
                )
    ...
    def can_reshape(self, target_3d_desc):
        ...
        if target_3d_desc.pp_degree > self.pp_degree:
            err_msg.append(
                f"Expansion reshape not supported - {PP_DIM}:"
                f" {self.pp_degree} ---> {target_3d_desc.pp_degree}"
            )

        if target_3d_desc.tp_degree > self.tp_degree:
            err_msg.append(
                f"Expansion reshape not supported - {TP_DIM}:"
                f" {self.tp_degree} ---> {target_3d_desc.tp_degree}"
            )

        if target_3d_desc.dp_degree > self.dp_degree:
            err_msg.append(
                f"Expansion reshape not supported - {DP_DIM}:"
                f" {self.dp_degree} ---> {target_3d_desc.dp_degree}"
            )
...
def get_model_3d_descriptor(dir):
    ...
    if num_pp0_files > 0:
        ...
        pp_degree = (
            len(get_files_with_prefix(file_list, MODEL_FILE_PREFIX))
            // tp_degree
        )
...
def flatten_dp_dimension(meg_2d_map, src_2d_size, dp_degree):
    new_meg_2d_map = meg_2d_parallel_map(
        meg_2d_map.pp_degree, meg_2d_map.tp_degree
    )
    for pp_index in range(meg_2d_map.pp_degree):
        for tp_index in range(meg_2d_map.tp_degree):
            ...
            for idx in dp0_indices:
                dpX_indices = [
                    idx + (i * src_2d_size) for i in range(dp_degree)
                ]
...
def unflatten_dp_dimension(meg_2d_map, dp_degree):
    ...
    for pp_index in range(pp_degree):
        for tp_index in range(tp_degree):
            ...
            for dp_indices, _2d_map in zip(
                partitioned_dp_indices, meg_2d_map_list
            ):

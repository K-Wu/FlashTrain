...
from deepspeed.ops.transformer.inference.diffusers_attention import (
    DeepSpeedDiffusersAttention,
)
from deepspeed.ops.transformer.inference.diffusers_transformer_block import (
    DeepSpeedDiffusersTransformerBlock,
)
from deepspeed.ops.transformer.inference.diffusers_2d_transformer import (
...
class ReplaceWithTensorSlicing:
    ...
    def merge_assert(self, dim1, dim2):
        assert dim1 > dim2, (
            "Merging tensors is not allowed here! Please use deepspeed"
            " load_checkpoint            for merging your checkpoints before"
            " replacing the transformer layer with           "
            " inference-kernels"
        )
    ...
    def qkv_copy(self, dst, src):
        ...
        if len(src_shape) == 2 and len(dst_shape) == 2:
            ...
            if self.out_dim == 1:
                self.merge_assert(
                    src_shape[self.out_dim], dst_shape[self.out_dim]
                )
                qkv_size = dst_shape[self.out_dim] // 3
                qkv_split = [
                    torch.split(src_s, qkv_size, dim=self.out_dim)
                    for src_s in src_split
                ]
                weight_split = [
                    torch.cat(
                        [qkv_s[i] for qkv_s in qkv_split], axis=self.out_dim
                    )
            ...
            if self.out_dim == 1:
                ...
                qkv_split = [
                    torch.split(src_s, qkv_size, dim=0) for src_s in src_split
                ]
                bias_split = [
        ...
        if len(src_shape) == 2 and len(dst_shape) == 2:
            if src_shape[0] == dst_shape[0] and src_shape[1] == dst_shape[1]:
                ...
                if src_shape[self.in_dim] != dst_shape[self.in_dim]:
                    self.merge_assert(
                        src_shape[self.in_dim], dst_shape[self.in_dim]
                    )
                    weight_split = (
                        torch.split(
                            src, dst_shape[self.in_dim], dim=self.in_dim
                        )[self.gpu_index]
                        .to(torch.cuda.current_device())
                        .contiguous()
                    )
                    ...
                    weight_split = (
                        torch.split(
                            src.data, dst_shape[self.out_dim], dim=self.out_dim
                        )[self.gpu_index]
                        .to(torch.cuda.current_device())
                        .contiguous()
                    )
...
class GroupQuantizer:
    ...
    def quantize(self, inputs, qkv=True, count=1, parallel_dim=0):
        ...
        input_flat = (
            (input_flat / scale).round().clamp(-q_range // 2, q_range // 2 - 1)
        )
        inputs_q = input_flat.reshape(inputs.shape).to(torch.int8).contiguous()
        out = torch.nn.Parameter(inputs_q, requires_grad=False)
        # print(inputs.shape)
        inputs_split = inputs.split(
            inputs.shape[parallel_dim] // 2, dim=parallel_dim
        )
        input_flat = [
            inputs_split[i].reshape(num_groups, -1).contiguous()
            for i in range(2)
        ]
        input_min = [
            torch.min(input_flat[i], dim=1, keepdim=True)[0].float()
            for i in range(2)
        ]
        input_max = [
            torch.max(input_flat[i], dim=1, keepdim=True)[0].float()
            for i in range(2)
        ]
        scale1 = [
            (
                torch.max(input_min[i].abs(), input_max[i].abs())
                * 2.0
                / (q_range)
            )
            .squeeze()
        ...
        out.scale = (
            torch.cat(
                [scale.squeeze().unsqueeze(0), scale1[0], scale1[1]], dim=0
            )
...
def generic_injection(module, fp16=False, enable_cuda_graph=True):
    def replace_attn(child, policy):
        ...
        attn_module.attn_ob.data.copy_(
            attn_ob.data.to(torch.cuda.current_device())
        )
        return attn_module
    ...
    if isinstance(module, torch.nn.Module):
        ...
        #                          max_out_tokens=8192)
        from ..model_implementations.transformers.clip_encoder import (
            DSClipEncoder,
        )

        cg_encoder = DSClipEncoder(
            module.text_encoder, enable_cuda_graph=enable_cuda_graph
        )
                ...
                def _replace_module(module, policy):
                    for name, child in module.named_children():
                        ...
                        if child.__class__ in new_policies:
                            replaced_module = new_policies[child.__class__](
                                child, policy
                            )
                            setattr(module, name, replaced_module)

                _replace_module(sub_module, policy)
                new_module = policy.apply(
                    sub_module, enable_cuda_graph=enable_cuda_graph
                )
...
def replace_transformer_layer(
    orig_layer_impl, model, checkpoint_dict, config, model_config
):
    ...
    def replace_with_policy(
        ...
        if inference:
            ...
            assert num_attention_heads % config.tensor_parallel.tp_size == 0, (
                "To run the model parallel across the GPUs, the"
                " attention_heads require to be divisible by the world_size!"
                + "This is because the attention computation is partitioned"
                " evenly among the parallel GPUs."
            )
        ...
        if False:
            ...
            _h4h_w = (
                [moe_w1.to(torch.int8) for moe_w1 in _h4h_w]
                if moe
                else _h4h_w.to(torch.int8)
            )
            _4hh_w = (
                [moe_w1.to(torch.int8) for moe_w1 in _4hh_w]
                if moe
                else _4hh_w.to(torch.int8)
            )
        elif fp16:
            qkvw = qkvw.half()
            dense_w = dense_w.half()
            _h4h_w = (
                [moe_w1.half() for moe_w1 in _h4h_w] if moe else _h4h_w.half()
            )
            _4hh_w = (
                [moe_w1.half() for moe_w1 in _4hh_w] if moe else _4hh_w.half()
            )
        if quantize or fp16:
            qkvb = qkvb if qkvb is None else qkvb.half()
            dense_b = dense_b if dense_b is None else dense_b.half()
            _h4h_b = (
                [moe_b1.half() for moe_b1 in _h4h_b] if moe else _h4h_b.half()
            )
            _4hh_b = (
                [moe_b1.half() for moe_b1 in _4hh_b] if moe else _4hh_b.half()
            )
        ...
        if inference:
            scale_attn_by_inverse_layer_idx = (
                ...
                if hasattr(config, "scale_attn_by_inverse_layer_idx")
                else False
            )
            if moe:
                ...
                local_ep_size = (
                    1
                    if num_experts < ep_world_size
                    else num_experts // ep_world_size
                )
                ...
                rotary_dim = (
                    ...
                    if hasattr(model_config, "rotary_dim")
                ...
                transformer_config = transformer_inference.DeepSpeedInferenceConfig(
                    ...
                    if hasattr(model_config, "layer_norm_eps")
                    else (
                        model_config.layer_norm_epsilon
                        if hasattr(model_config, "layer_norm_epsilon")
                        else model_config.layernorm_epsilon
                        if hasattr(model_config, "layernorm_epsilon")
                        else 1.0e-12
                    ),
                    fp16=fp16,
                    pre_layer_norm=policy.pre_attn_norm,
                    ...
                    local_attention=(
                        (model_config.attention_layers[layer_id] == "local")
                        if hasattr(model_config, "attention_layers")
                        else False
                    ),
                    window_size=(
            ...
            if moe:
                new_module = transformer_inference.DeepSpeedMoEInference(
                    ...
                    mp_group=config.tensor_parallel.tp_group,
                    ep_group=None
                    if config.moe.ep_group is None
                    else config.moe.ep_group[num_experts],
                    expert_mp_group=None
                    if config.moe.ep_mp_group is None
                    else config.moe.ep_mp_group[num_experts],
                )
            ...
            def _transpose(x):
                ...
                (q, k, v) = torch.split(
                    x_1, (x_1.shape[-1] // 3), dim=(x_1.dim() - 1)
                )
                if len(q.shape) > 2:
                    return torch.cat(
                        (
            ...
            if megatron_v2:
                ...
                qkvw = torch.nn.parameter.Parameter(
                    _transpose(qkvw).contiguous()
                )
                qkvb = torch.nn.parameter.Parameter(
                    _transpose(qkvb).contiguous()
                )
            ...
            if qkvw.is_meta or qkvw.numel() == 0 or qkvw.is_meta:
                ...
                attn_block.attn_qkvw = quantizer.quantize(
                    mp_replace.copy(attn_block.attn_qkvw, qkvw)
                    if bigscience_bloom
                    else mp_replace.qkv_copy(attn_block.attn_qkvw, qkvw)
                )
                attn_block.attn_qkvb = (
                    mp_replace.copy(attn_block.attn_qkvb, qkvb)
                    if bigscience_bloom
                    else mp_replace.qkv_copy(attn_block.attn_qkvb, qkvb)
                )

                attn_block.attn_ow = quantizer.quantize(
                    mp_replace.copy(attn_block.attn_ow, dense_w)
                )
            ...
            if moe:
                ...
                if _4hh_w.numel() == 0 or _4hh_w.is_meta:
                    ...
                    mpl_block.inter_w = quantizer.quantize(
                        mp_replace.copy(mpl_block.inter_w, _h4h_w)
                    )
                    mpl_block.inter_b = mp_replace.copy(
                        mpl_block.inter_b, _h4h_b
                    )
                    mpl_block.output_w = quantizer.quantize(
                        mp_replace.copy(mpl_block.output_w, _4hh_w)
                    )
                    mpl_block.output_b = mp_replace.copy(
                        mpl_block.output_b, _4hh_b
                    )
            ...
            if input_nw.is_meta or input_nw.numel() == 0:
                if (
                    ...
                    with GatheredParameters(
                        [input_nw, input_nb], modifier_rank=0
                    ):
    ...
    def replace_wo_policy(module, all_reduce_linears):
        ...
        if linear_layer_setting is not None:
            ...
            if len(linear_layer_setting) == 2:
                linear_policies.update(
                    {linear_layer_setting[1]: _slice_embedding}
                )
        else:
            if orig_layer_impl is HFGPT2LayerPolicy._orig_layer_class:
                try:
                    ...
                    linear_policies = {
                        transformers.model_utils.Conv1D: _replace
                    }
                except ImportError:
    ...
    def replace_fn(child, _policy, layer_id=0):
        ...
        if training:
            ...
            new_module = replace_with_policy(
                child, _policy, config.triangular_masking
            )
    ...
    if checkpoint_dict is not None:
        ...
        ckpt_list = (
            checkpoint["tp"] if type(checkpoint) is dict else checkpoint
        )
        ...
        if ckpt_type == "pp" and type(checkpoint) is list:
            ...
            for i in range(len(checkpoint)):
                sd = [
                    torch.load(
                        os.path.join(base_dir1, checkpoint[i]),
                        map_location="cpu",
                    )
                ]
            ...
            sd_count = (
                int((rank + max(1, tp_split_size)) / tp_split_size) - sd_offset
            )
            pbar = tqdm.tqdm(
            ...
            if "non_tp" in checkpoint:
                pbar = tqdm.tqdm(
                    ...
                    desc=(
                        f"Loading {len(checkpoint['non_tp'])} checkpoint"
                        " shards"
                    ),
                )
                ...
                for i in range(len(checkpoint["non_tp"])):
                    ...
                    gc.collect()
        print(
            f"checkpoint loading time at rank {rank}:"
            ...
            ckpt_config = json.dumps(
                {
                    ...
                    "dtype": "int8"
                    if quantize
                    else ("float16" if fp16 else "float32"),
                }
            )
            with open(
                f"{config.save_mp_checkpoint_path}/ds_inference_config.json",
                "w",
            ) as cfg:
                ...
                OrderedDict(
                    {
                        ...
                        for k in keys[
                            m * partition_size : (m + 1) * partition_size
                        ]
...
def replace_module(model, orig_class, replace_fn, _replace_policy):
    ...
    assert len(policy.items()) > 0, (
        "No default policy found! Please specify your policy injection_policy"
        " (like {BertLayer:HFBEertLayerPolicy})."

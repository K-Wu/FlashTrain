    ...
    def load(module, prefix):
        ...
        if (
            ...
            if hasattr(module, "weight"):
                module.weight = mp_replace.copy(
                    module.weight.data, sd[0][prefix + "weight"]
                )
            if prefix + "bias" in sd[0].keys():
                module.bias = mp_replace.copy(
                    module.bias.data, sd[0][prefix + "bias"]
                )
        args = None
        gc.collect()
        ...
        if ckpt_type == "tp":
                ...
                for n, p in module.named_parameters():
                    if prefix + n in sd[0] and len(n.split(".")) == 1:
                        ...
                        if len(src_shape) == 2 and len(dst_shape) == 2:
                            if (
                                src_shape[inner_dim] == dst_shape[0]
                                ...
                                if src_shape[dim] > dst_shape[dim1]:
                                    weight_partition = torch.split(
                                        tmp_data, dst_shape[dim1], dim=dim
                                    )[rank].to(torch.cuda.current_device())
                                    assert tmp_data.dtype != torch.int8 or scale.numel() > weight_quantizer.num_groups * (
                                        rank + 1
                                    ...
                                    scale = (
                                        ...
                                        .reshape(
                                            weight_quantizer.num_groups, -1
                                        )
                                        .contiguous()
                                    )
                                else:
                                    assert (
                                        tmp_data.dtype != torch.int8
                                    ...
                                    all_data = [
                                        sd[j][prefix + n]
                                        if type(sd[j][prefix + n]) is list
                                        else sd[j][prefix + n].to(
                                            torch.cuda.current_device()
                                        )
                                ...
                                if tmp_data.dtype != torch.int8:
                                    weight_partition = (
                                        ...
                                        if weight_quantizer.q_int8
                                        else weight_quantizer.quantize(
                                            weight_partition
                                        )
                                    )
                                else:
                            ...
                            if src_shape[0] == dst_shape[0]:
                                ...
                                if src_shape[0] > dst_shape[0]:
                                    bias_split = (
                                        torch.split(tmp_data, dst_shape[-1])[
                                            rank
                                        ]
            ...
            def maybe_copy(
                ...
                if src_name in sd[0]:
                    ...
                    if len(dst.shape) == 1:
                        ...
                        if split_qkv:
                            dst = weight_quantizer.quantize(
                                mp_replace.qkv_copy(
                                    dst,
                                    tmp
                                    if weight_quantizer.q_int8
                                    else (transpose(tmp).contiguous()),
                                )
                            )
                        else:
                            dst = weight_quantizer.quantize(
            ...
            if len(param_names) == 14:
                ...
                maybe_copy_qkv(
                    module.attention,
                    "attn_qkvw",
                    [prefix + q_w, prefix + k_w, prefix + v_w],
                    split_qkv=split_qkv,
                )
            else:
                maybe_copy_qkv(
                ...
                maybe_copy_qkv(
                    module.attention,
                    "attn_qkvb",
                    [prefix + q_b, prefix + k_b, prefix + v_b],
                    split_qkv=split_qkv,
                )
            ...
            if child.__class__ in layer_policies:
                ...
                if not any(checking_key in item for item in sd[0].keys()):
                    if hasattr(child, "weight") and (
                        hasattr(child.weight, "ds_id")
                        and child.weight.ds_id in all_ds_ids
                    ):
                        prefix1 = all_ds_ids[child.weight.ds_id]
                        if child.__class__ is nn.Linear:
                            child = LinearLayer(
                                weight=all_ds_ids[child.weight.ds_id]
                            )
                            setattr(module, name, child)
                    continue
                child_params = list(child.parameters())
                if len(child_params) > 0 and (
                    child_params[0].numel() == 0 or child_params[0].is_meta
                ):
                    ...
                    if child.__class__ is nn.LayerNorm:
                        ...
                        child = LinearLayer(
                            weight_shape=child.weight.shape, bias=child.bias
                        )
                        setattr(module, name, child)
                        ...
                        child = EmbeddingLayer(
                            weight_shape=ds_shape, dtype=child.weight.dtype
                        )
                ...
                load_module_recursive(
                    child,
                    prefix
                    if (level == 0 and ckpt_type == "pp") and param_names[-2]
                    else prefix + name + ".",
                    level + 1,
                )

...
class UNetPolicy(DSPolicy):
    ...
    def attention(self, client_module):
        ...
        if qw.shape[1] == kw.shape[1]:
            qkvw = Parameter(
                torch.cat((qw, kw, vw), dim=0), requires_grad=False
            )
...
class HFCLIPLayerPolicy(TransformerPolicy):
    def __init__(self, client_module, inference=False):
        ...
        if HFCLIPLayerPolicy._orig_layer_class is None:
            try:
                ...
                HFCLIPLayerPolicy._orig_layer_class = (
    ...
    def layerNorm(self):
        return (
            self.client_module.layer_norm2.weight,
...
class MegatronLayerPolicy(TransformerPolicy):
    ...
    def __init__(self, client_module, inference=True):
        super().__init__(
            inference, megatron_v2=MegatronLayerPolicy.megatron_v2
        )
        self.client_module = client_module
        ...
        if MegatronLayerPolicy._orig_layer_class is None:
            if pkg_version.parse(torch.__version__) <= pkg_version.parse(
                "1.2"
            ):
                ...
                try:
                    ...
                    MegatronLayerPolicy._orig_layer_class = (
                        ParallelTransformerLayer
                    )
                except ImportError:
                    MegatronLayerPolicy._orig_layer_class = None
        ...
        if moe:
            ...
            if moe_type == "standard":
                return (
                    self.linear_layer,
                    [
                        moe_experts[i].dense_h_to_4h.weight
                        for i in range(num_experts)
                    ],
                    [
                        moe_experts[i].dense_h_to_4h.bias
                        for i in range(num_experts)
                    ],
                    [
                        moe_experts[i].dense_4h_to_h.weight
                        for i in range(num_experts)
                    ],
                    [
                        moe_experts[i].dense_4h_to_h.bias
                        for i in range(num_experts)
                    ],
                )
            else:
...
class HFGPT2LayerPolicy(TransformerPolicy):
    ...
    def get_hidden_heads(self):
        return (
            self.client_module.attn.embed_dim,
...
class GPTNEOXLayerPolicy(TransformerPolicy):
    ...
    def __init__(
        self, client_module, inference=True, megatron_v2=True, split_qkv=False
    ):
...
class HFOPTLayerPolicy(TransformerPolicy):
    ...
    def __init__(self, client_module, inference=True, use_load_prefix=True):
        ...
        try:
            ...
            if isinstance(
                TransformerPolicy.hf_model_config,
                transformers.models.opt.configuration_opt.OPTConfig,
            ):
                self.pre_attn_norm = (
                    TransformerPolicy.hf_model_config.do_layer_norm_before
                )
        except:

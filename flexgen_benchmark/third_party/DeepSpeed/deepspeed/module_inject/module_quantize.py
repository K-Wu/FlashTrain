...
def quantize_transformer_layer(
    orig_layer_impl, model, megatron=False, preln=False
):
    ...
    def bert_layer_quantize(layer):
        ...
        layer.output.dense.weight.data = quantize_weight(
    ...
    return quantize_module(
        model=model, orig_class=orig_layer_impl, quantize_fn=quantize_fn
    )

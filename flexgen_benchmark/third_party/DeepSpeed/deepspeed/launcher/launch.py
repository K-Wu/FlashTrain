...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "redirect the stdout and stderr from each rank into different log"
            " files"
        ),
    )
...
def main():
    ...
    logger.info(
        f"nnodes={args.nnodes}, num_local_procs={num_local_procs},"
    ...
    if args.save_pid:
        ...
        pid_file = os.path.join(
            PID_FILE_BASEPATH, f"{args.save_pid}.deepspeed"
        )
    ...
    if not is_torch_elastic_compatible():
        if args.enable_elastic_training:
            logger.info(
                f"Disabling elastic training support as                    "
                f" PyTorch version should be greater than 1.11.x"
            )
    ...
    if os.path.exists(DLTS_POD_ENV_PATH):
        with open(DLTS_POD_ENV_PATH) as file:
            ...
            for line in lines:
                if line.startswith(
                    "export FC_TASKROLE_NAME"
                ) or line.startswith("export FC_TASK_INDEX"):
    ...
    if not args.enable_elastic_training:
        if args.enable_each_rank_log != "None":
            ...
            if os.path.isfile(args.enable_each_rank_log):
                raise ValueError(
                    f"{args.enable_each_rank_log} should not be a file, it"
            ...
            if not os.path.exists(args.enable_each_rank_log):
                try:
                    ...
                    raise ValueError(
                        ...
                        f" {args.enable_each_rank_log} for each rank log."
                    )
        ...
        assert (
            args.max_elastic_nodes > 0 and args.min_elastic_nodes > 0
        ), "Max and Min nodes should be positive"

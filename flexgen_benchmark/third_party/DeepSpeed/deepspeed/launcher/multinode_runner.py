...
class PDSHRunner(MultiNodeRunner):
    ...
    def parse_user_args(self):
        return list(
            map(
                lambda x: x if x.startswith("-") else f"'{x}'",
                self.args.user_args,
            )
        )
    ...
    def get_cmd(self, environment, active_resources):
        ...
        kill_command = pdsh_cmd_args + [
            "pkill -f ",
            " ".join(cmd_to_search)[:-2],
        ]
        return (
            pdsh_cmd_args
            + deepspeed_launch
...
class OpenMPIRunner(MultiNodeRunner):
    ...
    def validate_args(self):
        ...
        if self.args.include != "" or self.args.exclude != "":
            raise ValueError(
                f"{self.name} backend does not support worker"
                " include/exclusion"
            )
...
class SlurmRunner(MultiNodeRunner):
    ...
    def get_cmd(self, environment, active_resources):
        assert not getattr(
            self.args, "detect_nvlink_pairs", False
...
class MVAPICHRunner(MultiNodeRunner):
    ...
    def backend_exists(self):
        ...
        if not mpiname_exists:
            warnings.warn(
                "mpiname does not exist, mvapich is not installed properly"
            )
        else:
            ...
            if "MVAPICH2-GDR" in mpiname_results:
                ...
                warnings.warn(
                    "Expected MVAPICH2-GDR as return for mpiname but received"
                    f" {mpiname_results}"
                )
        return exists

...
def parse_args(args=None):
    parser = argparse.ArgumentParser(
        description=(
            "DeepSpeed runner to help launch distributed "
    ...
    parser.add_argument(
        ...
        help=(
            "Hostfile path (in MPI style) that defines the "
    ...
    parser.add_argument(
        ...
        default=-1,
        help=(
            "Max number of GPUs to use on each node, will use "
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "(optional) IP address of node 0, will be "
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "(optional) choose launcher backend for multi-node training."
            " Options currently include PDSH, OpenMPI, MVAPICH, SLURM."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Change each process to interpret the launch "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip prepending the training script with "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Do not pass local_rank as an argument when calling "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Force multi-node launcher mode, helps in cases where user "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Save file containing launcher process id (pid) at"
            " /tmp/<main-pid>.ds, where <main-pid> is the pid of the first"
            " process that invoked `deepspeed`. Useful when launching"
            " deepspeed processes programmatically."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Run DeepSpeed autotuner to discover optimal configuration"
            " parameters before running job."
        ),
    )
    ...
    with open(hostfile_path, "r") as fd:
        ...
        for line in fd.readlines():
            ...
            try:
                ...
                logger.error(
                    "Hostfile is not formatted correctly, unable to "
                    "proceed with training."
                )
                raise err
            if hostname in resource_pool:
                logger.error(
                    "Hostfile contains duplicate hosts, unable to "
                    "proceed with training."
                )
...
def parse_num_nodes(str_num_nodes: str, elastic_training: bool):
    ...
    if len(node_list) == 1:
        ...
        raise RuntimeError(
            "MIN:MAX format is only supported in elastic training"
        )
    else:
        raise RuntimeError(
            "num_nodes {} is not in MIN:MAX format".format(str_num_nodes)
        )
...
def main(args=None):
    ...
    if args.elastic_training:
        assert (
            args.master_addr != ""
        ), "Master Addr is required when elastic training is enabled"
    ...
    if not resource_pool and len(cuda_visible_devices):
        ...
        if (
            ...
            print(
                f"{detected_str} but ignoring it because one or several of"
                " --include/--exclude/--num_gpus/--num_nodes cl args were"
                " used. If you want to use CUDA_VISIBLE_DEVICES don't pass"
                " any of these arguments to deepspeed."
            )
        else:
    ...
    if multi_node_exec and not args.no_ssh_check:
        ...
        try:
            ...
            raise RuntimeError(
                f"Using hostfile at {args.hostfile} but host={first_host} was"
                " not reachable via ssh. If you are running with a single"
                f" node please remove {args.hostfile} or setup passwordless"
                " ssh."
            )
    ...
    if not args.master_addr:
        ...
        logger.info(
            f"Using IP address of {args.master_addr} for node {first_host}"
        )
    ...
    if args.elastic_training:
        assert (
            not args.no_local_rank
        ), "--no_local_rank argument is not supported in Elastic training"
    ...
    if not multi_node_exec:
        ...
        if args.elastic_training:
            ...
            deepspeed_launch.append(
                f"--max_elastic_nodes={args.max_elastic_nodes}"
            )
            deepspeed_launch.append(
        ...
        for environ_path in DEEPSPEED_ENVIRONMENT_PATHS:
            environ_file = os.path.join(
                environ_path, DEEPSPEED_ENVIRONMENT_NAME
            )

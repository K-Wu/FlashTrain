...
def timed_op(func):
    def log_wrapper(*args, **kwargs):
        ...
        if comms_logger.enabled:
            if (
                ("prof" in kwargs and kwargs["prof"])
        ...
        try:
            ...
            if comms_logger.enabled:
                ...
                if (
                    ...
                    comms_logger.append(
                        raw_name, log_name, time_elapsed, msg_size
                    )
...
def new_group(ranks):
    ...
    assert cdb is not None and cdb.is_initialized(), (
        "DeepSpeed backend not set, please initialize it using"
        " init_process_group()"
    )
...
def set_backend(backend):
    if not use_ds_backend:
        utils.logger.error(
            "DeepSpeed communication backend is required. Please use"
            " deepspeed.comm.init_distributed(backend, use_deepspeed=True) to"
            " use this functionality"
        )
        raise RuntimeError(
            "Error: Custom DeepSpeed backend called without initializing"
...
def all_gather(
    ...
    global cdb
    return cdb.all_gather(
        tensor_list=tensor_list, tensor=tensor, group=group, async_op=async_op
    )
...
def has_reduce_scatter_base():
    ...
    assert (
        cdb.has_reduce_scatter_base is not None
    ...
    if cdb.has_reduce_scatter_base:
        ...
        if not has_warned_reduce_scatter:
            utils.logger.warning(
                "unable to find torch.distributed._reduce_scatter_base. will"
                " fall back to torch.distributed.all_gather which will result"
                " in suboptimal performance. please consider upgrading your"
...
def reduce_scatter_base(
    ...
    global cdb
    return cdb.reduce_scatter_base(
...
def has_allgather_base():
    ...
    assert (
        cdb.has_allgather_base is not None
...
def allgather_fn(
    ...
    if cdb.has_allgather_base:
        ...
        if not has_warned_all_gather and get_rank() == 0:
            utils.logger.warning(
                "unable to find torch.distributed._all_gather_base. will fall"
                " back to torch.distributed.all_gather which will result in"
                " suboptimal performance. please consider upgrading your"
                " pytorch installation."
            )
...
def monitored_barrier(
    ...
    global cdb
    return cdb.barrier(
        group=group, timeout=timeout, wait_all_ranks=wait_all_ranks
    )
...
def reduce(
    ...
    global cdb
    return cdb.reduce(
        tensor=tensor, dst=dst, op=op, group=group, async_op=async_op
    )
...
def init_distributed(
    ...
    if dist_init_required is False:
        assert cdb is not None and cdb.is_initialized() is True, (
            "Distributed backend is not initialized. Please set"
        ...
        if auto_mpi_discovery and not all(
            ...
            if verbose:
                utils.logger.info(
                    "Not using the DeepSpeed or dist launchers, attempting to"
...
def mpi_discovery(
    distributed_port=TORCH_DISTRIBUTED_DEFAULT_PORT, verbose=True
):
    """
...
def patch_aml_env_for_torch_nccl_backend(master_port=6105, verbose=True):
    ...
    if verbose:
        utils.logger.info(
            "Discovered AzureML settings of world_rank={}, local_rank={},"
            " world_size={}, master_addr={}, master_port={}".format(
                os.environ["RANK"],
                os.environ["LOCAL_RANK"],
                os.environ["WORLD_SIZE"],
...
def patch_aws_sm_env_for_torch_nccl_backend(verbose=True):
    ...
    if verbose:
        utils.logger.info(
            "Discovered AWS SageMaker settings of world_rank={},"

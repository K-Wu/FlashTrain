...
class TorchBackend(Backend):
    ...
    def init_process_group(self, backend, timeout, init_method):
        if not torch.distributed.is_initialized():
            torch.distributed.init_process_group(
                backend, timeout=timeout, init_method=init_method
            )
    ...
    def reduce_scatter(
        self, output, input_list, op=ReduceOp.SUM, group=None, async_op=False
    ):
    ...
    def all_gather_base(
        ...
        if self.has_allgather_base:
            ...
            utils.logger.warning(
                ...
                " back to torch.distributed.reduce_scatter which will result"
    ...
    def reduce_scatter_base(
        ...
        if self.has_reduce_scatter_base:
            ...
            utils.logger.warning(
                ...
                " fall back to torch.distributed.reduce_scatter which will"
                " result in suboptimal performance. please consider upgrading"
    ...
    def send(self, tensor, dst, group=None, tag=0):
        return torch.distributed.send(
            tensor=tensor, dst=dst, group=group, tag=tag
        )

    def recv(self, tensor, src=None, group=None, tag=0):
        return torch.distributed.recv(
            tensor=tensor, src=src, group=group, tag=tag
        )
    ...
    def gather(
        self, tensor, gather_list=None, dst=0, group=None, async_op=False
    ):
    ...
    def scatter(
        self, tensor, scatter_list=None, src=0, group=None, async_op=False
    ):
    ...
    def get_global_rank(self, group, group_rank):
        if hasattr(torch.distributed.distributed_c10d, "get_global_rank"):
            from torch.distributed.distributed_c10d import (
                get_global_rank as _get_global_rank,
            )
        else:

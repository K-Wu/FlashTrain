...
class ElasticityConfig:
    ...
    def __init__(self, param_dict):
        ...
        if self.enabled:
            ...
            self.micro_batches = param_dict.get(
                MICRO_BATCHES, MICRO_BATCHES_DEFAULT
            )

        if not isinstance(self.micro_batches, list):
            raise ElasticityConfigError(
                f"Elasticity expected value of {MICRO_BATCHES} to be a list of"
        ...
        if not all(map(lambda m: isinstance(m, int), self.micro_batches)):
            raise ElasticityConfigError(
                f"Elasticity expected {MICRO_BATCHES} to only contain a list"
                f" of integers, instead contains: f{self.micro_batches}"
            )

        if not all(map(lambda m: m > 0, self.micro_batches)):
            raise ElasticityConfigError(
                f"Elasticity expected {MICRO_BATCHES} to only contain positive"
                f" integers, instead contains: f{self.micro_batches}"
            )

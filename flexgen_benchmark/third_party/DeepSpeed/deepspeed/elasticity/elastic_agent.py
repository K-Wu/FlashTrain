    ...
    def _start_workers(self, worker_group: WorkerGroup) -> Dict[int, Any]:
        ...
        for worker in worker_group.workers:
            ...
            if "OMP_NUM_THREADS" in os.environ:
                worker_env_ds["OMP_NUM_THREADS"] = os.environ[
                    "OMP_NUM_THREADS"
                ]
        ...
        attempt_log_dir = os.path.join(
            self._log_dir, f"attempt_{restart_count}"
        )
    ...
    def _invoke_run(self, role: str = "default") -> RunResult:
        ...
        while True:
            ...
            if state == WorkerState.SUCCEEDED:
                log.info(
                    f"[{role}] worker group successfully finished. Waiting"
                    f" {self._exit_barrier_timeout} seconds for other agents"
                    " to finish."
                )
                self._exit_barrier()
                return run_result
            elif state in {WorkerState.UNHEALTHY, WorkerState.FAILED} or len(
                participants
            ) > len(rdzv_handler._state_holder.state.participants):
                if self._remaining_restarts > 0:
                    log.info(
                        f"[{role}] Worker group {state.name}."
                        f" {self._remaining_restarts}/{spec.max_restarts} attempts"
                        " left; will restart worker group"
                    )

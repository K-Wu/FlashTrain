...
def get_best_candidates(
    ...
    for batch_size in candidate_batch_sizes:
        current_valid_gpus = get_valid_gpus(
            batch_size, micro_batches, min_gpus, max_gpus
        )
...
def _get_compatible_gpus_v02(
    ...
    if num_gpus_per_node % model_parallel_size != 0:
        raise ElasticityError(
            "In Elasticity v0.2, number of GPUs per node:"
    ...
    final_batch_size, valid_world_size = _get_compatible_gpus_v01(
        ...
        int(
            max_gpus / num_gpus_per_node
        ),  # Passing number of max nodes as Elasticity v2 works at node level
...
def _compatible_ds_version_check(target_deepspeed_version: str):
    ...
    err_str = (
        ...
        " compatible with minimum version"
...
def ensure_immutable_elastic_config(runtime_elastic_config_dict: dict):
    ...
    if DEEPSPEED_ELASTICITY_CONFIG in os.environ:
        ...
        scheduler_elastic_config = ElasticityConfig(
            scheduler_elastic_config_dict
        )
        runtime_elastic_config = ElasticityConfig(runtime_elastic_config_dict)
        err_str = (
            "Elastic config '{}={}' seen by resource scheduler does not match"
            " config passed to runtime {}={}"
        )
        if (
...
def compute_elastic_config(
    ...
    if not isinstance(ds_config, dict):
        raise ValueError(
            "Expected ds_config to be a dictionary but received "
    ...
    if model_parallel_size > 1 and float(elastic_config.version) != 0.2:
        raise ElasticityConfigError(
            ...
            " model-parallel training. Given model-parallel size:"
    ...
    if float(elastic_config.version) == 0.1:
        ...
        if world_size != 0:
            ...
            if (
                ...
                and os.getenv("WORLD_SIZE").isnumeric()
            ):
                current_num_gpus = int(os.getenv("WORLD_SIZE"))
            else:
                WORLD_SIZE = os.getenv("WORLD_SIZE")
                raise ElasticityConfigError(
                    "Elasticity V 0.2 needs WORLD_SIZE to compute valid batch"
                    " size. Either give it as argument to function"
                    " compute_elastic_config or set it as an environment"
                    " variable. Value of WORLD_SIZE as environment variable"
                    f" is {WORLD_SIZE}"
                )
        ...
        raise NotImplementedError(
            "Unable to find elastic logic for version:"
            f" {elastic_config.version}"
        )
    ...
    if world_size > 0:
        ...
        assert micro_batch_size is not None, (
            ...
            f" world_size={world_size}, final_batch_size={final_batch_size},"
            f" and  micro_batches={elastic_config.micro_batches}."
        )

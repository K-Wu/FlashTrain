...
def get_original_command(max_width=80, full_python_path=False):
    ...
    python = (
        sys.executable if full_python_path else sys.executable.split("/")[-1]
    )
    cmd.append(python)
    ...
    while len(cmd) > 0:
        ...
        if (
            len(cmd) == 0
            or len(current_line) + len(cmd[0]) + 1 > max_width - 1
        ):
...
def process_run(
    ...
    if len(metrics) > 0:
        mean_metrics = {
            k: fmean([x[k] for x in metrics]) for k in metrics[0].keys()
        }
...
def process_results(
    ...
    sentinel_value = nan
    if base_variation is not None and len(
        df[df[variation_key] == base_variation]
    ):
    ...
    if math.isnan(sentinel_value):
        ...
        sentinel_value = df.loc[df[target_metric_key] != nan][
            target_metric_key
        ].min()
    ...
    report += [
        "*** Results:",
        df_github.to_markdown(index=False, floatfmt=".2f"),
    ]
    report += ["```"]
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Multi-dimensional variations, example: '|--fp16|--bf16' '|--tf32'"
        ),
    )
    parser.add_argument(
        "--base-variation",
        ...
        type=str,
        help=(
            "Baseline variation to compare to. if None the minimal target"
            " value will be used to compare against"
        ),
    )
    parser.add_argument(
        "--target-metric-key",
        ...
        required=True,
        help=(
            "Target metric key in output_dir/all_results.json, e.g.,"
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Report metric keys - other metric keys from"
            " output_dir/all_results.json to report, e.g., train_loss. Use a"
            " single argument e.g., 'train_loss train_samples"
        ),
    )
    parser.add_argument(
        "--repeat-times",
        ...
        type=int,
        help=(
            "How many times to re-run each variation - an average will be"
            " reported"
        ),
    )
    parser.add_argument(
        ...
        type=str,
        help=(
            "The output directory where all the benchmark reports will go to"
            " and additionally this directory will be used to override"
            " --output_dir in the script that is being benchmarked"
        ),
    )
    parser.add_argument(
        "--verbose",
        ...
        action="store_true",
        help=(
            "Whether to show the outputs of each run or just the benchmark"
            " progress"
        ),
    )
    ...
    print(
        "\nNote: each run's output is also logged under"
        f" {output_dir}/log.*.std*.txt"
    )

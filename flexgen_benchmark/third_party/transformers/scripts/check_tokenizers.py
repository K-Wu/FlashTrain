...
def check_diff(spm_diff, tok_diff, slow, fast):
    if spm_diff == list(reversed(tok_diff)):
        ...
        return True
    elif len(spm_diff) == len(tok_diff) and fast.decode(
        spm_diff
    ) == fast.decode(tok_diff):
        # Second order OK
...
def check_details(line, spm_ids, tok_ids, slow, fast):
    ...
    for i, (spm_id, tok_id) in enumerate(zip(spm_ids, tok_ids)):
        if spm_id != tok_id:
            break
    first = i
    for i, (spm_id, tok_id) in enumerate(
        zip(reversed(spm_ids), reversed(tok_ids))
    ):
        if spm_id != tok_id:
            break
    last = len(spm_ids) - i
    ...
    if last - first > 5:
        ...
        for i in range(last - first - min_width):
            if all(
                spm_ids[first + i + j] in removable_tokens
    ...
    try:
        print(
            f"Tok: {[fast.decode([tok_ids[i]]) for i in range(first, last)]}"
        )
    except Exception:
        pass

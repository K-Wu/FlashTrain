...
class DataTrainingArguments:
    ...
    target_text_column: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Column in the dataset that contains label (target text)."
                " Defaults to 'text'"
            )
        },
    )
    ...
    target_feature_extractor_sampling_rate: Optional[bool] = field(
        ...
        metadata={
            "help": (
                "Resample loaded audio to target feature extractor's sampling"
                " rate or not."
            )
        },
    )
    max_duration_in_seconds: Optional[float] = field(
        ...
        metadata={
            "help": (
                "Filters out examples longer than specified. Defaults to no"
                " filtering."
            )
        },
    )
    orthography: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Orthography used for normalization and tokenization:"
                " 'librispeech' (default), 'timit', or 'buckwalter'."
            )
        },
    )
    ...
    def training_step(
        ...
        if self.args.n_gpu > 1:
            if model.module.config.ctc_loss_reduction == "mean":
                ...
                raise ValueError(
                    f"{model.config.ctc_loss_reduction} is not valid. Choose"
                    " one of ['mean', 'sum']"
                )
...
def main():
    ...
    if data_args.max_duration_in_seconds is not None:
        ...
        if len(train_dataset) > old_train_size:
            logger.warning(
                f"Filtered out {len(train_dataset) - old_train_size} train"
                " example(s) longer than"
                f" {data_args.max_duration_in_seconds} second(s)."
            )
        if len(val_dataset) > old_val_size:
            logger.warning(
                f"Filtered out {len(val_dataset) - old_val_size} validation"
                " example(s) longer than"
                f" {data_args.max_duration_in_seconds} second(s)."
            )
    logger.info(
        f"Split sizes: {len(train_dataset)} train and"
        f" {len(val_dataset)} validation."
    )

    logger.warning(
        f"Updated {len(text_updates)} transcript(s) using"
        f" '{data_args.orthography}' orthography rules."
    )
    ...
    def prepare_dataset(batch):
        ...
        assert len(set(batch["sampling_rate"])) == 1, (
            "Make sure all inputs have the same sampling rate of"
            f" {processor.feature_extractor.sampling_rate}."
        )
        ...
        if logger.isEnabledFor(logging.DEBUG):
            for reference, predicted in zip(label_str, pred_str):
                ...
                if orthography.untransliterator is not None:
                    logger.debug(
                        "reference (untransliterated):"
                        f' "{orthography.untransliterator(reference)}"'
                    )
                    logger.debug(
                        "predicted (untransliterated):"
                        f' "{orthography.untransliterator(predicted)}"'
                    )

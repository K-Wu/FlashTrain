...
class Wav2Vec2Aligner:
    ...
    def align_single_sample(self, item):
        ...
        def get_trellis(emission, tokens, blank_id=0):
            ...
            trellis = torch.full(
                (num_frames + 1, num_tokens + 1), -float("inf")
            )
            trellis[:, 0] = 0
        ...
        def backtrack(trellis, emission, tokens, blank_id=0):
            ...
            for t in range(t_start, 0, -1):
                ...
                changed = (
                    trellis[t - 1, j - 1] + emission[t - 1, tokens[j - 1]]
                )
        ...
        def merge_repeats(path):
            ...
            while i1 < len(path):
                while (
                    i2 < len(path)
                    and path[i1].token_index == path[i2].token_index
                ):
                    i2 += 1
    ...
    def align_data(self, wav_dir, text_file, output_dir):
        ...
        for line in lines:
            ...
            items.append(
                {"sent": sentence, "wav_path": wav_path, "out_path": out_path}
            )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=16000,
        help="sampling rate of input audios",
    )
    parser.add_argument(
        "--output_dir",
        type=str,

...
class WandbArguments:
    ...
    wandb_user_name: Optional[str] = field(
        ...
        metadata={
            "help": (
                "The WandB user name for potential logging. If left None, no"
                " logging"
            )
        },
    )
...
if __name__ == "__main__":
    ...
    lm_class = (
        ...
        if model_args.performer
        else FlaxBertForMaskedLM
    )
    ...
    epochs = tqdm(
        range(nb_epochs), desc=f"Epoch ... (1/{nb_epochs})", position=0
    )
    for epoch in epochs:
        # ======================== Training ================================
        # Create sampling rng
        ...
        training_samples_idx = np.random.permutation(
            np.arange(nb_training_samples)
        )
        ...
        for batch_idx in tqdm(
            training_batch_idx, desc="Training...", position=1
        ):
            samples = [
        ...
        # Update progress bar
        epochs.desc = (
            f"Epoch... ({epoch + 1}/{nb_epochs} | Loss:"
            f" {eval_summary['loss']}, Acc: {eval_summary['accuracy']})"
        )

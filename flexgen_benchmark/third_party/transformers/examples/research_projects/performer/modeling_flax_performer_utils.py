...
def nonnegative_softmax_kernel_feature_creator(
    ...
    data_mod_shape = (
        data.shape[0 : len(batch_dims_t)] + projection_matrix.shape
    )
...
def make_fast_softmax_attention(
    ...
    logging.info(
        (
            "Fast softmax attention: %s features and orthogonal=%s,"
            " renormalize=%s"
        ),
        nb_features,
        ortho_features,
    ...
    def get_2d_array(self):
        ...
        if self.scaling == 0:
            multiplier = jnp.linalg.norm(
                random.normal(self.key, (self.nb_rows, self.nb_columns)),
                axis=1,
            )
        elif self.scaling == 1:
            multiplier = jnp.sqrt(float(self.nb_columns)) * jnp.ones(
                (self.nb_rows)
            )
        else:
            raise ValueError(
...
def _numerator(z_slice_shape, precision, unroll=1):
    ...
    def bwd(pqkv, W_ct):
        def body(carry, qkv_xct):
            ...
            p_ct += jnp.einsum(
                "...d,...m->...md", x_ct, q, precision=precision
            )
    ...
    def dot_product_attention(
        ...
        for ax in axis:
            if not (query.ndim >= 3 and 1 <= ax < query.ndim - 2):
                raise ValueError(
                    "Attention axis must be between the batch axis and the"
                    " last-two axes."
                )
        n = key.ndim

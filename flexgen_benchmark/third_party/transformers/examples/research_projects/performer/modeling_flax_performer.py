...
class FlaxPerformerEmbeddings(nn.Module):
    ...
    @nn.compact
    def __call__(
        self, input_ids, token_type_ids, position_ids, attention_mask
    ):
        # Embed
...
class FlaxPerformerAttention(nn.Module):
    ...
    def __call__(self, hidden_state, attention_mask):
        ...
        layer_norm = FlaxPerformerLayerNorm(name="layer_norm")(
            self_att + hidden_state
        )
        return layer_norm
    ...
    def convert_from_pytorch(pt_state: Dict, config: BertConfig) -> Dict:
        ...
        for key, tensor in pt_state.items():
            ...
            if "attention.output.dense" in key:
                del jax_state[key]
                key = key.replace(
                    "attention.output.dense", "attention.self.out"
                )
                jax_state[key] = tensor

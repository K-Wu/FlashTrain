...
def main(args):
    ...
    st = torch.load(
        os.path.join(serialization_dir, "pytorch_model.bin"),
        map_location="cpu",
    )

    remaining_count = (
        0  # Number of remaining (not pruned) params in the encoder
    )
    ...
    for name, param in st.items():
        ...
        if "mask_scores" in name:
            if pruning_method == "topK":
                ...
                mask_ones = (
                    ThresholdBinarizer.apply(param, threshold, True)
                    .sum()
                    .item()
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Pruning Method (l0 = L0 regularization, topK = Movement pruning,"
    ...
    parser.add_argument(
        ...
        required=False,
        help=(
            "For `topK`, it is the level of remaining weights (in %) in the"
            " fine-pruned model.For `sigmoied_threshold`, it is the threshold"
            " \tau against which the (sigmoied) scores are compared.Not needed"
            " for `l0`"
        ),
    )
    parser.add_argument(

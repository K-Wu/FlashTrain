...
def main(args):
    ...
    torch.save(
        pruned_model, os.path.join(target_model_path, "pytorch_model.bin")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        help=(
            "For `magnitude` and `topK`, it is the level of remaining weights"
            " (in %) in the fine-pruned model.For `sigmoied_threshold`, it is"
            " the threshold \tau against which the (sigmoied) scores are"
            " compared.Not needed for `l0`"
        ),
    )
    parser.add_argument(
        "--model_name_or_path",

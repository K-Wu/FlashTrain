                    ...
                    for name, param in model.named_parameters():
                        ...
                        if (
                            ...
                            if args.regularization == "l1":
                                ...
                                perc = (
                                    torch.sigmoid(
                                        param - 2 / 3 * np.log(0.1 / 1.1)
                                    )
                                ).sum().item() / param.numel()
                            tb_writer.add_scalar(
                                "retained_weights_perc/" + name,
                                perc,
                                global_step,
                            )
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if not args.data_dir and (
            ...
            try:
                ...
                raise ImportError(
                    "If not data_dir is specified, tensorflow_datasets needs"
                    " to be installed."
                )
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "The output directory where the model checkpoints and predictions"
            " will be written."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "The input training file. If a data dir is specified, will look"
            " for the file there"
        )
        + "If no data dir or train/predict files are specified, will run with"
        " tensorflow_datasets.",
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "The input evaluation file. If a data dir is specified, will look"
            " for the file there"
        )
        + "If no data dir or train/predict files are specified, will run with"
        " tensorflow_datasets.",
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Where do you want to store the pre-trained models downloaded from"
            " huggingface.co"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "The maximum number of tokens for the question. Questions longer"
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Run `initial_warmup` * `warmup_steps` steps of threshold warmup"
            " during which threshold staysat its `initial_threshold` value"
            " (sparsity schedule)."
        ),
    )
    parser.add_argument(
        ...
        type=int,
        help=(
            "Run `final_warmup` * `warmup_steps` steps of threshold cool-down"
            " during which threshold staysat its final_threshold value"
            " (sparsity schedule)."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Pruning Method (l0 = L0 regularization, magnitude = Magnitude"
            " pruning, topK = Movement pruning, sigmoied_threshold = Soft"
            " movement pruning)."
        ),
    )
    parser.add_argument(
        ...
        type=str,
        help=(
            "Initialization method for the mask scores. Choices: constant,"
            " uniform, kaiming."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Regularization intensity (used in conjunction with"
            " `regularization`."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Teacher type. Teacher tokenizer and student (model) tokenizer"
            " must output the same tokenization. Only for distillation."
        ),
    )
    parser.add_argument(
        ...
        type=str,
        help=(
            "Path to the already SQuAD fine-tuned teacher model. Only for"
            " distillation."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="O1",
        help=(
            "For fp16: Apex AMP optimization level selected in ['O0', 'O1',"
            " 'O2', and 'O3'].See details at"
    ...
    if args.doc_stride >= args.max_seq_length - args.max_query_length:
        logger.warning(
            "WARNING - You've set a doc stride which may be superior to the"
            " document length in some examples. This could result in errors"
            " when building features from the examples. Please reduce the doc"
            " stride or increase the maximum length to ensure the features are"
            " correctly built."
        )
    ...
    logger.warning(
        (
            "Process rank: %s, device: %s, n_gpu: %s, distributed training:"
            " %s, 16-bits training: %s"
        ),
        args.local_rank,
        device,
        args.n_gpu,

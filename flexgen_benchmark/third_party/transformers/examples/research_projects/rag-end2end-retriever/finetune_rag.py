...
class GenerativeQAModule(BaseTransformer):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        super().__init__(
            hparams, config=config, tokenizer=tokenizer, model=model
        )
            ...
            if (not batch_idx == 0) and (
                ...
                if (not isEmUpdateBusy) and has_free_gpus:
                    ...
                    for rank in range(num_processes):
                        logger.info(
                            "Iniitializing  embedding calculation process"
                            " rank{}".format(rank)
                        )
    ...
    def val_dataloader(self) -> DataLoader:
        return self.get_dataloader(
            "val", batch_size=self.hparams.eval_batch_size
        )

    def test_dataloader(self) -> DataLoader:
        return self.get_dataloader(
            "test", batch_size=self.hparams.eval_batch_size
        )
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument(
            ...
            default=None,
            help=(
                "Prefix added at the beginning of each text, typically used"
        ...
        parser.add_argument(
            ...
            type=str,
            help=(
                "RAG model type: sequence or token, if none specified, the"
                " type is inferred from the model_name_or_path"
            ),
        )
        parser.add_argument(
            ...
            type=str,
            help=(
                "Name of the pre-trained context encoder checkpoint from"
                " the DPR"
            ),
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            help=(
                "directory used to keep temporary shards during the re-encode"
                " process"
            ),
        )
        ...
        parser.add_argument(
            ...
            type=str,
            help=(
                "order of the GPU used during the fine-tuning.  Used to"
                " finding free GPUs during the re-encode process. I do not"
                " have many GPUs :)"
            ),
        )
    ...
    def add_retriever_specific_args(parser):
        parser.add_argument(
            ...
            default=None,
            help=(
                "Name of the index to use: 'hf' for a canonical dataset from"
                " the datasets library (default), 'custom' for a local index,"
                " or 'legacy' for the orignal one)"
            ),
        )
        parser.add_argument(
            "--passages_path",
            ...
            help=(
                "Path to the dataset of passages for custom index. More info"
                " about custom indexes in the RagRetriever documentation as"
                " well as in `examples/rag/use_own_knowledge_dataset.py`"
            ),
        )
        parser.add_argument(
            "--index_path",
            ...
            help=(
                "Path to the faiss index for custom index. More info about"
                " custom indexes in the RagRetriever documentation as well as"
                " in `examples/rag/use_own_knowledge_dataset.py`"
            ),
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            default=False,
            help=(
                "Whether to use the dummy version of the dataset index. More"
                " info about custom indexes in the RagRetriever documentation"
                " as well as in `examples/rag/use_own_knowledge_dataset.py`"
            ),
        )
        return parser
...
def main(args=None, model=None) -> GenerativeQAModule:
    ...
    if args.distributed_retriever == "ray" and args.gpus > 1:
        ...
        if (
            "LOCAL_RANK" not in os.environ or os.environ["LOCAL_RANK"] == 0
        ) and ("NODE_RANK" not in os.environ or os.environ["NODE_RANK"] == 0):
            remote_cls = ray.remote(RayRetriever)
            named_actors = [
                remote_cls.options(
                    name="retrieval_worker_{}".format(i)
                ).remote()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If True, use pytorch_lightning.profiler.AdvancedProfiler to"
            " profile the Trainer."
        ),
    )

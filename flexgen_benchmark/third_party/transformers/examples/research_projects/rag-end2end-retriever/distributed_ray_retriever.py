...
class RagRayDistributedRetriever(RagRetriever):
    ...
    def __init__(
        ...
        if (
            index is not None
            and index.is_initialized()
            and len(retrieval_workers) > 0
        ):
            raise ValueError(
                "When using Ray for distributed fine-tuning, you'll need to"
                " provide the paths instead, as the dataset and the index are"
                " loaded separately. More info in"
    ...
    def re_load(self):
        ...
        ray.get(
            [worker.clear_object.remote() for worker in self.retrieval_workers]
        )

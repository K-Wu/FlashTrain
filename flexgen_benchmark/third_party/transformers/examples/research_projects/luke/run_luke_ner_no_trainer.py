...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Finetune (m)LUKE on a token classification task (such as NER)"
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The column name of text to input in the file (a csv or JSON"
            " file)."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "The column name of label to input in the file (a csv or JSON"
            " file)."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            " will be padded if `--pad_to_max_length` is passed."
        ),
    )
    parser.add_argument(
        "--max_entity_length",
        ...
        default=32,
        help=(
            "The maximum total input entity length after tokenization (Used"
            " only for (M)Luke models). Sequences longer than this will be"
            " truncated, sequences shorter will be padded if"
            " `--pad_to_max_length` is passed."
        ),
    )
    parser.add_argument(
        "--max_mention_length",
        ...
        default=30,
        help=(
            "The maximum total input mention length after tokenization (Used"
            " only for (M)Luke models). Sequences longer than this will be"
            " truncated, sequences shorter will be padded if"
            " `--pad_to_max_length` is passed."
        ),
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Setting labels of all special tokens to -100 and thus PyTorch"
            " will ignore them."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Activate debug mode and run training only with a subset of data."
        ),
    )
    parser.add_argument(
        "--push_to_hub",
...
def main():
    ...
    if args.config_name:
        config = LukeConfig.from_pretrained(
            args.config_name, num_labels=num_labels
        )
    ...
    def compute_entity_spans_for_luke(examples):
        ...
        for labels, tokens, sentence_boundaries in zip(
            ...
            subword_lengths = [
                len(tokenizer.tokenize(token)) for token in tokens
            ]
    ...
    def get_luke_labels(outputs, ner_tags, original_entity_spans):
        ...
        for output, original_spans, tags in zip(
            ...
            true_original_spans = [
                val for val in original_spans if val != (-1, -1)
            ]
            max_indices = torch.argmax(output, axis=1)
            max_logits = torch.max(output, axis=1).values
            predictions = []

            for logit, index, span in zip(
                max_logits, max_indices, true_original_spans
            ):
                if index != 0:
            ...
            for _, span, label in sorted(
                predictions, key=lambda o: o[0], reverse=True
            ):
                if all(
                    [
                        o == label_list[0]
                        for o in predicted_sequence[span[0] : span[1]]
                    ]
                ):

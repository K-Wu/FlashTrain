...
def main(
    ...
    input_ids = tokenizer.question_encoder(question, return_tensors="pt")[
        "input_ids"
    ]
...
class RagExampleArguments:
    csv_path: str = field(
        default=str(
            Path(__file__).parent / "test_data" / "my_knowledge_dataset.csv"
        ),
        metadata={
            "help": (
                "Path to a tab-separated csv file with columns 'title' and"
                " 'text'"
            )
        },
    )
    question: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Question that is passed as input to RAG. Default is 'What"
                " does Moses' rod turn into ?'."
            )
        },
    )
    rag_model_name: str = field(
        ...
        metadata={
            "help": (
                "The RAG model to use. Either 'facebook/rag-sequence-nq' or"
    ...
    dpr_ctx_encoder_model_name: str = field(
        ...
        metadata={
            "help": (
                "The DPR context encoder model to use. Either"
                " 'facebook/dpr-ctx_encoder-single-nq-base' or"
    ...
    output_dir: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Path to a directory where the dataset passages and the index"
                " will be saved"
            )
        },
    )
...
class ProcessingArguments:
    num_proc: Optional[int] = field(
        ...
        metadata={
            "help": (
                "The number of processes to use to split the documents into"
                " passages. Default is single process."
            )
        },
    )
    batch_size: int = field(
        ...
        metadata={
            "help": (
                "The batch size to use when computing the passages embeddings"
                " using the DPR context encoder."
            )
        },
    )
...
class IndexHnswArguments:
    d: int = field(
        ...
        metadata={
            "help": (
                "The dimension of the embeddings to pass to the HNSW Faiss"
                " index."
            )
        },
    )
    m: int = field(
        ...
        metadata={
            "help": (
                "The number of bi-directional links created for every new"
                " element during the HNSW index construction."
            )
        },
    )

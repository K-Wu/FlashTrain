...
def convert_bertabs_checkpoints(path_to_checkpoints, dump_path):
    ...
    encoder_input_ids.extend(
        [tokenizer.pad_token_id] * (512 - len(encoder_input_ids))
    )
    encoder_input_ids = torch.tensor(encoder_input_ids).unsqueeze(0)
    decoder_input_ids = tokenizer.encode("This is sample 3 éàalj'-.")
    decoder_input_ids.extend(
        [tokenizer.pad_token_id] * (512 - len(decoder_input_ids))
    )
    ...
    assert (
        torch.max(
            torch.abs(
                original.generator[0].weight - new_model.generator[0].weight
            )
        )
        == 0
    )
    ...
    if are_identical:
        ...
        raise ValueError(
            "the weights are different. The new model is likely different from"
            " the original one."
        )

...
Batch = namedtuple(
    "Batch",
    ["document_names", "batch_size", "src", "segs", "mask_src", "tgt_str"],
)
...
def evaluate(args):
    ...
    for batch in tqdm(data_iterator):
        ...
        save_summaries(
            summaries, args.summaries_output_dir, batch.document_names
        )
...
def main():
    ...
    parser.add_argument(
        ...
        required=False,
        help=(
            "The folder in wich the summaries should be written. Defaults to"
            " the folder where the documents are"
        ),
    )
    parser.add_argument(
        "--compute_rouge",
        ...
        required=False,
        help=(
            "Compute the ROUGE metrics during evaluation. Only available for"
    ...
    parser.add_argument(
        ...
        type=bool,
        help=(
            "Whether to block the existence of repeating trigrams in the text"
    ...
    if not documents_dir_is_valid(args.documents_dir):
        raise FileNotFoundError(
            "We could not find the directory you specified for the documents"
            " to summarize, or it was empty. Please specify a valid path."
        )

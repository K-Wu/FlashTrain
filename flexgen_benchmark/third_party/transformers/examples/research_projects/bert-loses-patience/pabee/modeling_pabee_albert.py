...
@add_start_docstrings(
    (
        "The bare ALBERT Model transformer with PABEE outputting raw"
...
class AlbertModelWithPabee(AlbertModel):
    ...
    def log_stats(self):
        avg_inf_layers = (
            self.inference_layers_num / self.inference_instances_num
        )
        message = (
            f"*** Patience = {self.patience} Avg. Inference Layers ="
            f" {avg_inf_layers:.2f} Speed Up ="
    ...
    def forward(
        ...
        if self.training:
            ...
            res = [
                output_layers[self.config.num_hidden_layers - 1](pooled_output)
            ]
        else:
            patient_counter = 0
            patient_result = None

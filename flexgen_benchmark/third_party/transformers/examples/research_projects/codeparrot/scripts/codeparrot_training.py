...
def create_dataloaders(args):
    ...
    train_data = load_dataset(
        args.dataset_name_train, split="train", **ds_kwargs
    )
    train_data = train_data.shuffle(
        buffer_size=args.shuffle_buffer, seed=args.seed
    )
    valid_data = load_dataset(
        args.dataset_name_valid, split="train", **ds_kwargs
    )
...
def compute_tflops(elapsed_time, accelerator, args):
    ...
    flops_per_iteration = factor * (
        ...
        + (
            ...
            / (16.0 * config_model.n_layer * config_model.n_embd)
        )
    )
...
if args.resume_from_checkpoint:
    if (
        ...
        dirs = [
            f.name
            for f in os.scandir(args.save_dir)
            if f.is_dir() and "step" in str(f)
        ]

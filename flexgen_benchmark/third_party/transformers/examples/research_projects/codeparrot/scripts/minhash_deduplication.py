...
def make_duplicate_clusters(
    dataset_iterator: Type[Dataset], jaccard_threshold: float
):
    ...
    for filename, min_hash in tqdm(
        ThreadedIterator(
            minhash_iter(enumerate(dataset_iterator)), max_queue_size=100
        )
    ):
...
def find_extremes(cluster_list, dataset, jaccard_threshold):
    ...
    extremes_list = []
    f = partial(
        _find_cluster_extremes_shared, jaccard_threshold=jaccard_threshold
    )
    with mp.Pool() as pool:
...
def deduplicate_dataset(
    ...
    extreme_dict = {}
    extremes_clusters = find_extremes(
        duplicate_clusters, dataset, jaccard_threshold
    )
    ...
    for cluster in duplicate_clusters:
        for element in cluster:
            ...
            if element["is_extreme"]:
                element["copies"] = extreme_dict[element["base_index"]][
                    "copies"
                ]

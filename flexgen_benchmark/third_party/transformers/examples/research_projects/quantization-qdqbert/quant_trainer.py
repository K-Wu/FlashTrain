...
def add_arguments(parser):
    ...
    group.add_argument(
        "--aprec", type=int, default=8, help="activation precision"
    )
    group.add_argument(
        "--quant-per-tensor",
        action="store_true",
    ...
    group.add_argument(
        ...
        help=(
            ...
            " amaxes will be computed with the current quantization"
            " granularity (axis)."
        ),
    )
...
def set_default_quantizers(args):
    ...
    input_desc = QuantDescriptor(
        num_bits=args.aprec, calib_method=calib_method
    )
    ...
    if not calib:
        if args.quant_disable_embeddings:
            set_quantizer_by_name(
                model, ["embeddings"], which="weight", _disabled=True
            )
        ...
        if args.quant_disable_keyword:
            set_quantizer_by_name(
                model, args.quant_disable_keyword, _disabled=True
            )

        if args.quant_disable_layer_module:
            set_quantizer_by_name(
                model,
                ["layer.\d+." + args.quant_disable_layer_module],
                _disabled=True,
            )
        ...
        if args.quant_enable_layer_module:
            set_quantizer_by_name(
                model,
                ["layer.\d+." + args.quant_enable_layer_module],
                _disabled=False,
            )
        ...
        if name.endswith("_quantizer"):
            if module._calibrator is not None:
                if isinstance(module._calibrator, calib.MaxCalibrator):
                    ...
                    module.load_calib_amax(
                        "percentile", percentile=args.percentile
                    )
                module.enable_quant()
...
def fuse_qkv(model, args):
    ...
    def fuse3(qq, qk, qv):
        ...
        qv._amax.fill_(amax)
        logger.info(
            f"          q={q:5.2f} k={k:5.2f} v={v:5.2f} -> {amax:5.2f}"
        )
...
def clip_gelu(model, maxval):
    ...
    for name, mod in model.named_modules():
        if name.endswith(".output.dense") and not name.endswith(
            ...
            logger.info(
                f"CLIP_GELU: {name:{name_width}} amax: {amax_init:5.2f} ->"
                f" {amax:5.2f}"
            )
...
def expand_amax(model):
    ...
    for name, mod in model.named_modules():
        if (
            ...
            and mod._weight_quantizer.axis is not None
        ):
...
def recalibrate_weights(model):
    ...
    for name, mod in model.named_modules():
        if hasattr(mod, "_weight_quantizer"):
            ...
            logger.info(
                "RECALIB:"
                f" {name:{name_width}} {mod._weight_quantizer._amax.flatten()} ->"
                f" {amax.flatten()}"
            )
...
def set_quantizer_by_name(model, names, **kwargs):
    ...
    for name, mod in model.named_modules():
        if hasattr(mod, "_input_quantizer") or hasattr(
            mod, "_weight_quantizer"
        ):
            for n in names:
                if re.search(n, name):

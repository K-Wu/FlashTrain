...
parser.add_argument(
    ...
    required=True,
    help=(
        "The output directory where the model checkpoints and predictions will"
        " be written."
    ),
)
...
parser.add_argument(
    ...
    default=0.0,
    help=(
        "If null_score - best_non_null is greater than the threshold predict"
        " null."
    ),
)
...
if args.tokenizer_name:
    ...
    raise ValueError(
        ...
        " supported by this script.You can do it from another script, save it,"
...
with trt.Builder(TRT_LOGGER) as builder, builder.create_network(
    ...
    input_names = [
        _input.name for _input in network_inputs
    ]  # ex: ["actual_input1"]
    ...
    with builder.create_builder_config() as config:
        ...
        for i in range(len(input_names)):
            profile.set_shape(
                input_names[i], INPUT_SHAPE, INPUT_SHAPE, INPUT_SHAPE
            )
...
def model_infer(
    ...
    context.execute_async(
        bindings=[int(d_inp) for d_inp in d_inputs]
...
if args.max_seq_length > tokenizer.model_max_length:
    logger.warning(
        ...
        f" maximum length for themodel ({tokenizer.model_max_length}). Using"
...
with open(engine_name, "rb") as f, trt.Runtime(
    ...
    logger.info(
        "Average Inference Time = {:.3f} ms".format(total_time * 1000 / niter)
    )

...
class Distiller:
    def __init__(
        ...
        if self.mlm:
            ...
            assert (
                params.word_mask + params.word_keep + params.word_rand == 1.0
            )
        ...
        if self.multi_gpu:
            if self.fp16:
                ...
                logger.info(
                    "Using apex.parallel.DistributedDataParallel for"
                ...
                logger.info(
                    "Using nn.parallel.DistributedDataParallel for distributed"
                    " training."
                )
    ...
    def prepare_batch_mlm(self, batch):
        ...
        attn_mask = (
            torch.arange(
                token_ids.size(1), dtype=torch.long, device=lengths.device
            )
            < lengths[:, None]
        )
    ...
    def end_epoch(self):
        ...
        """
        logger.info(
            f"{self.n_sequences_epoch} sequences have been trained during this"
            " epoch."
        )

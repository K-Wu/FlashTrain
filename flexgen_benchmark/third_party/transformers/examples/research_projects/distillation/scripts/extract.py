...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Extraction some layers of the full RobertaForMaskedLM or"
            " GPT2LMHeadModel for Transfer Learned Distillation"
        )
    )
    parser.add_argument(
        "--model_type", default="roberta", choices=["roberta", "gpt2"]
    )
    ...
    if args.model_type == "roberta":
        ...
        if args.vocab_transform:
            for w in ["weight", "bias"]:
                compressed_sd[f"lm_head.dense.{w}"] = state_dict[
                    f"lm_head.dense.{w}"
                ]
        ...
        for w in ["weight", "bias"]:
            compressed_sd[f"{prefix}.ln_f.{w}"] = state_dict[
                f"{prefix}.ln_f.{w}"
            ]
    ...
    print(
        "Number of params transferred for distillation:"
        f" {len(compressed_sd.keys())}"
    )

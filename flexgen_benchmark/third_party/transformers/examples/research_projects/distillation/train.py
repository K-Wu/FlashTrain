...
def freeze_pos_embeddings(student, args):
    if args.student_type == "roberta":
        student.roberta.embeddings.position_embeddings.weight.requires_grad = (
            False
        )
...
def freeze_token_type_embeddings(student, args):
    if args.student_type == "roberta":
        student.roberta.embeddings.token_type_embeddings.weight.requires_grad = (
            False
        )
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "The binarized file (tokenized + tokens_to_ids) and grouped by"
            " sequence."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Linear weight for the MLM loss. Must be >=0. Should be used in"
            " conjunction with `mlm` flag."
        ),
    )
    parser.add_argument(
        "--alpha_clm",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "The LM step: MLM or CLM. If `mlm` is True, the MLM is used over"
            " CLM."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Smoothing parameter to emphasize more rare tokens (see XLM,"
            " similar to word2vec)."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If true, compute the distillation loss only the [MLM] prediction"
            " distribution."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Freeze positional embeddings during distillation. For"
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Freeze token type embeddings during distillation if existent. For"
            " student_type in ['roberta'] only."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_false",
        help=(
            "If true, group sequences that have similar length into the same"
    ...
    if args.is_master:
        if os.path.exists(args.dump_path):
            if not args.force:
                raise ValueError(
                    f"Serialization dir {args.dump_path} already exists, but"
                    " you have not precised wheter to overwrite itUse"
                    " `--force` if you want to overwrite it"
                )
            else:
    ...
    ) = MODEL_CLASSES[args.teacher_type]
    ...
    if args.mlm:
        logger.info(
            f"Loading token counts from {args.token_counts} (already"
            " pre-computed)"
        )
    ...
    if args.student_pretrained_weights is not None:
        logger.info(
            ...
            f" {args.student_pretrained_weights}"
        )
    ...
    assert (
        student.config.max_position_embeddings
        == teacher.config.max_position_embeddings
    )
    if args.mlm:

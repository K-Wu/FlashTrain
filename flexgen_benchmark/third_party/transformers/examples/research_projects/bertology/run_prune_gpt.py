...
def compute_heads_importance(
    ...
    for step, inputs in enumerate(
        ...
        if compute_entropy:
            for layer, attn in enumerate(all_attentions):
                ...
                attn_entropy[layer] += (
                    masked_entropy.sum(-1).sum(0).sum(0).detach()
                )
    ...
    head_ranks[
        head_importance.view(-1).sort(descending=True)[1]
    ] = torch.arange(head_importance.numel(), device=args.device)
...
def mask_heads(args, model, eval_dataloader):
    ...
    np.save(
        os.path.join(args.output_dir, "head_mask.npy"),
...
def prune_heads(args, model, eval_dataloader, head_mask):
    ...
    logger.info(
        (
            "Pruning: original num of params: %.2e, after pruning %.2e (%.1f"
            " percents)"
        ),
        original_num_params,
        pruned_num_params,
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "masking threshold in term of metrics (stop masking when metric <"
            " threshold * original metric value)."
        ),
    )
    parser.add_argument(
        "--masking_amount",

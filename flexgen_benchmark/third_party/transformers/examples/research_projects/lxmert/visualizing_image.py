...
class SingleImageViz:
    ...
    def draw_boxes(
        ...
        obj_scores = (
            obj_scores[sorted_idxs] if obj_scores is not None else None
        )
        attr_ids = attr_ids[sorted_idxs] if attr_ids is not None else None
        attr_scores = (
            attr_scores[sorted_idxs] if attr_scores is not None else None
        )

        assigned_colors = [
            self._random_color(maximum=1) for _ in range(len(boxes))
        ]
    ...
    def _create_text_labels_attr(
        self, classes, scores, attr_classes, attr_scores
    ):

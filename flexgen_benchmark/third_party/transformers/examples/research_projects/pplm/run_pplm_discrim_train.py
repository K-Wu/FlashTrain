...
def evaluate_performance(data_loader, discriminator, device="cpu"):
    ...
    print(
        "Performance on test set: Average loss: {:.4f}, Accuracy: {}/{}"
        " ({:.0f}%)".format(
            test_loss,
            correct,
            len(data_loader.dataset),
...
def train_discriminator(
    ...
    if dataset == "SST":
        ...
        with open("datasets/clickbait/clickbait_train_prefix.txt") as f:
            for i, line in enumerate(tqdm(f, ascii=True)):
                try:
                    ...
                    print(
                        "Error evaluating / tokenizing line {}, skipping it"
                        .format(i)
                    )
                    pass
        ...
        if dataset_fp is None:
            raise ValueError(
                "When generic dataset is selected, dataset_fp needs to be"
                " specified aswell."
            )
    ...
    if save_model:
        with open(
            "{}_classifier_head_meta.json".format(dataset), "w"
        ) as meta_file:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "File path of the dataset to use. Needed only in case of generic"
            " datadset"
        ),
    )
    parser.add_argument(

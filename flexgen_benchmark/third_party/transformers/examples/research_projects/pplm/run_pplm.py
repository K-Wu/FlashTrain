...
def perturb_past(
    ...
    for i in range(num_iterations):
        ...
        kl_loss = 0.0
        if kl_scale > 0.0:
            unpert_probs = nn.functional.softmax(
                unpert_logits[:, -1, :], dim=-1
            )
            unpert_probs = (
                unpert_probs
                + SMALL_CONST
...
def get_classifier(
    ...
    if "url" in params:
        ...
        raise ValueError(
            "Either url or path have to be specified in the discriminator"
            " model parameters"
        )
...
def set_generic_model_params(discrim_weights, discrim_meta):
    if discrim_weights is None:
        raise ValueError(
            "When using a generic discriminator, discrim_weights need to be"
            " specified"
        )
    if discrim_meta is None:
        raise ValueError(
            "When using a generic discriminator, discrim_meta need to be"
            " specified"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Length of past which is being optimized; 0 corresponds to"
            " infinite window length"
        ),
    )
    parser.add_argument(
        "--horizon_length",

...
def _create_traced_decoder(
    ...
    if past_values:
        return torch.jit.trace(
            decoder_for_onnx,
            (input_ids, encoder_state, attention_mask, past_values),
        )
    else:
        return torch.jit.trace(
...
class BARTGenerator(torch.nn.Module, GenerationMixin):
    ...
    def forward(
        self, input_ids, attention_mask, max_length, decoder_start_token_id
    ):
...
class BeamSearchScorerTS(torch.nn.Module):
    ...
    def init(
        ...
        self._beam_hyps_worst_scores = (
            torch.zeros(batch_size, device=device) + 1e9
        )
        self._beam_hyps = []
        self._beam_scores = []
        ...
        if (
            ...
            if hyps_count + 1 > self.num_beams:
                sorted_next_scores, sorted_indices = torch.topk(
                    torch.cat(self._beam_scores)[
                        beam_idx : beam_idx + hyps_count + 1
                    ],
                    hyps_count + 1,
                    largest=False,
                )
        ...
        for batch_idx in range(batch_size):
            if self._done[batch_idx]:
                assert self.hypo_len(batch_idx) >= self.num_beams, (
                    "Batch can only be done if at least {} beams have been"
                    " generated".format(self.num_beams)
                )
    ...
    def finalize(
        ...
        for i in range(batch_size):
            ...
            beam_scores = torch.cat(self._beam_scores)[
                batch_hypo_start:batch_hypo_end
            ]
            sorted_next_scores, sorted_indices = torch.topk(
            ...
            for j in range(self.num_beam_hyps_to_keep):
                ...
                best_hyp = self._beam_hyps[
                    batch_hypo_start + sorted_indices[j]
                ]
...
class BARTBeamSearchGenerator(BARTGenerator):
    ...
    def adjust_logits_during_generation(
        self, logits, cur_len: int, max_length: int
    ):
        if cur_len == 1 and self.config.force_bos_token_to_be_generated:
            ...
            cur_len == max_length - 1 and self.config.eos_token_id is not None
        ):
            logits = self._force_token_id_to_be_generated(

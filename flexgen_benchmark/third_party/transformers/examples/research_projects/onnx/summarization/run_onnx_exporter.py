...
def load_model_tokenizer(model_name, device="cpu"):
    huggingface_model = (
        model_dict[model_name].from_pretrained(model_name).to(device)
    )
...
def export_and_validate_model(
    model, tokenizer, onnx_file_path, num_beams, max_length
):
    model.eval()
    ...
    with torch.no_grad():
        ...
        new_onnx_file_path = remove_dup_initializers(
            os.path.abspath(onnx_file_path)
        )

        logger.info(
            "Deduplicated and optimized model written to {}".format(
                new_onnx_file_path
            )
        )
        ...
        ort_out = ort_sess.run(
            None,
            {
                ...
                "decoder_start_token_id": np.array(
                    model.config.decoder_start_token_id
                ),
            },
        )
...
def main():
    ...
    export_and_validate_model(
        model, tokenizer, output_name, num_beams, max_length
    )

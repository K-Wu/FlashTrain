...
class DataTrainingArguments:
    ...
    shuffle_buffer_size: Optional[int] = field(
        ...
        metadata={
            "help": (
                "The number of streamed examples to download before shuffling"
                " them. The large the buffer, the closer it is to real offline"
                " shuffling."
            )
        },
    )
...
class DataCollatorCTCWithPadding:
    ...
    def __call__(
        ...
        for feature in features:
            if (
                self.max_length
                and feature["input_values"].shape[-1] > self.max_length
            ):
                continue
...
def main():
    ...
    if training_args.do_train:
        ...
        if data_args.audio_column_name not in raw_column_names["train"]:
            raise ValueError(
                ...
                f" of {', '.join(raw_column_names['train'])}."
            )
    ...
    if tokenizer_name_or_path is None:
        raise ValueError(
            "Tokenizer has to be created before training in streaming mode."
            " Please specify --tokenizer_name_or_path"
        )
    ...
    eval_metrics = {
        metric: load_metric(metric) for metric in data_args.eval_metrics
    }

...
def compute_perplexity(model, test_data, context_len):
    ...
    eval_dataloader = DataLoader(
        test_data, shuffle=False, batch_size=eval_batch_size
    )
...
def collect_objective_set(
    ...
    model.train()
    model, lm_optimizer, lm_scheduler = recopy_model(
        orig_model, device, max_steps
    )

    for step in tqdm(range(max_steps)):
        context = torch.zeros(
            (1, context_len), dtype=torch.long, device=device
        )
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step % eval_freq == 0 and step > 0) or (
                ...
                for step2, batch2 in enumerate(test_dataloader):
                    ...
                    for ei, i in enumerate(
                        predicted_q2.cpu().detach().numpy()
                    ):
...
class SecondaryLearner(nn.Module):
    ...
    def __init__(self, model):
        ...
        self.conv = nn.Conv1d(
            self.embeddings.weight.size(1), 256, 3, padding=1
        )

...
class DataTrainingArguments:
    ...
    validation_split_name: Optional[str] = field(
        ...
        metadata={
            "help": (
                "The name of the validation data set split to use (via the"
    ...
    speech_file_column: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Column in the dataset that contains speech file path."
                " Defaults to 'file'"
            )
        },
    )
    ...
    pad_to_multiple_of: Optional[int] = field(
        ...
        metadata={
            "help": (
                "If set will pad the sequence to a multiple of the provided"
                " value. This is important to avoid triggering recompilations"
                " on TPU"
            )
        },
    )
...
class FlaxDataCollatorForWav2Vec2Pretraining:
    ...
    def __call__(
        ...
        if batch["attention_mask"] is not None:
            ...
            attention_mask[
                (np.arange(attention_mask.shape[0]), output_lengths - 1)
            ] = 1
...
def generate_batch_splits(
    samples_idx: np.ndarray, batch_size: int
) -> np.ndarray:
...
def compute_contrastive_loss(
    ...
    loss_logits = optax.cosine_similarity(
        transformer_features, target_features
    )
    ...
    predictions = loss_logits.transpose(2, 1, 0).reshape(
        -1, loss_logits.shape[0]
    )
    ...
    contrastive_loss = (
        optax.softmax_cross_entropy(
            predictions, onehot(targets, predictions.shape[-1])
        )
        * target_mask
    )
...
def main():
    ...
    def prepare_dataset(batch):
        ...
        batch["speech"], _ = librosa.load(
            batch[data_args.speech_file_column],
    ...
    if not config.do_stable_layer_norm or config.feat_extract_norm != "layer":
        raise ValueError(
            "PreTraining is only supported for"
            " ``config.do_stable_layer_norm=True`` and"
    ...
    num_train_steps = (
        len(vectorized_datasets["train"]) // train_batch_size * num_epochs
    )
    ...
    contrastive_logits_temperature = (
        model.config.contrastive_logits_temperature
    )
    num_codevectors = (
    ...
    for epoch in epochs:
        ...
        for step, batch_idx in enumerate(
            ...
            if cur_step % training_args.logging_steps == 0 and cur_step > 0:
                ...
                epochs.write(
                    f"Step... ({cur_step} | Loss:"
                    f" {train_metric['loss'].mean()}, Learning Rate:"
        ...
        if has_tensorboard and jax.process_index() == 0:
            cur_step = epoch * (
                len(vectorized_datasets["train"]) // train_batch_size
            )

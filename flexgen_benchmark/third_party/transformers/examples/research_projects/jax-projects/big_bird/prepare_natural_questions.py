...
def _get_single_answer(example):
    def choose_first(answer, is_long_answer=False):
        ...
        if len(answer) == 1:
            answer = answer[0]
            return (
                {k: [answer[k]] for k in answer} if is_long_answer else answer
            )
        for a in answer:
            if is_long_answer:
                a = {k: [a[k]] for k in a}
    ...
    if (
        len(answer["start_token"]) > 1
        or answer["start_token"] == answer["end_token"]
    ):
...
def get_context_and_ans(example, assertion=False):
    ...
    if answer["category"][0] in [
        "yes",
        "no",
    ]:  # category is list with one element
...
def get_strided_contexts_and_ans(
    ...
    input_ids = tokenizer(
        example["question"]["text"], out["context"]
    ).input_ids
    ...
    if answer["category"][0] in [
        ...
        doc_start_indices = range(
            q_len, len(input_ids), max_length - doc_stride
        )
...
def save_to_disk(hf_data, file_name):
    with jsonlines.open(file_name, "a") as writer:
        for example in tqdm(
            hf_data, total=len(hf_data), desc="Saving samples ... "
        ):

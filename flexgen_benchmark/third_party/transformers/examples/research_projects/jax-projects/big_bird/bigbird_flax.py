...
class FlaxBigBirdForNaturalQuestionsModule(
    FlaxBigBirdForQuestionAnsweringModule
):
    """
...
def calculate_loss_for_nq(
    ...
    def cross_entropy(logits, labels, reduction=None):
        ...
        labels = (labels[..., None] == jnp.arange(vocab_size)[None]).astype(
            "f4"
        )
...
class DataCollator:
    ...
    def collate_fn(self, features):
        ...
        batch = {
            ...
            "start_labels": jnp.array(
                features["start_token"], dtype=jnp.int32
            ),
...
class Trainer:
    ...
    def train(self, state, tr_dataset, val_dataset):
        ...
        for epoch in range(args.max_epochs):
            ...
            tr_dataloader = get_batched_dataset(
                tr_dataset, args.batch_size, seed=epoch
            )
            i = 0
            for batch in tqdm(
                ...
                if i % args.save_steps == 0:
                    self.save_checkpoint(
                        args.save_dir + f"-e{epoch}-s{i}", state=state
                    )
    ...
    def save_checkpoint(self, save_dir, state):
        ...
        joblib.dump(
            self.data_collator, os.path.join(save_dir, "data_collator.joblib")
        )

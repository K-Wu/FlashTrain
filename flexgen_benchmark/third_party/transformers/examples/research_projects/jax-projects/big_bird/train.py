...
if __name__ == "__main__":
    ...
    args = Args()
    logger = wandb.init(
        project="bigbird-natural-questions", config=args.__dict__
    )
    ...
    data_collator = DataCollator(
        pad_id=tokenizer.pad_token_id, max_length=4096
    )
    ...
    tx_args = {
        ...
        "num_train_steps": args.max_epochs
        * (len(tr_dataset) // args.batch_size),
    ...
    state = trainer.create_state(
        model,
        tx,
        num_train_steps=tx_args["num_train_steps"],
        ckpt_dir=ckpt_dir,
    )
    try:
    ...
    model.save_pretrained(
        os.path.join(args.base_dir, "final-weights"), params=params
    )

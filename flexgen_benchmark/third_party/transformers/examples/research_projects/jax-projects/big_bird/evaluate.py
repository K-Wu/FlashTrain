...
def get_best_valid_start_end_idx(
    start_scores, end_scores, top_k=1, max_size=100
):
    ...
    scores = (best_end_scores[:, None] + best_start_scores[None, :]) - (
        1e8 * mask
    )
    best_score = jnp.argmax(scores).item()

    return (
        best_start_idx[best_score % top_k],
        best_end_idx[best_score // top_k],
    )
...
def format_dataset(sample):
    ...
    for s in long_answers:
        ...
        new_answer = " ".join(
            [answer[i] for i in range(len(answer)) if not html[i]]
        )
        if new_answer not in long_targets:
            long_targets.append(new_answer)

    category = (
        "long_short" if len(short_targets + long_targets) > 0 else "null"
    )
...
def main():
    ...
    def evaluate(example):
        ...
        example["output"] = [
            tokenizer.decode(input_ids[start_score : end_score + 1])
        ]
        ...
        predictions = set(
            [s for s in predictions if s not in ["``", "''", "`", "'"]]
        )

        # if there is a common element, it's a exact match
        example["match"] = len(list(answers & predictions)) > 0
        example["has_pred"] = (
            predicted_category != "null" and len(predictions) > 0
        )

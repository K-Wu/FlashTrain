...
if datasets.__version__ <= "1.8.0":
    raise ValueError(
        "Make sure to upgrade `datasets` to a version >= 1.9.0 to use dataset"
        " streaming"
    )
...
class DataTrainingArguments:
    ...
    max_seq_length: Optional[int] = field(
        ...
        metadata={
            "help": (
                ...
                " Sequences longer than this will be truncated. Default to the"
    ...
    text_column_name: str = field(
        default="text",
        metadata={
            "help": "The name of the column to retrieve the training text."
        },
    )
...
class FlaxDataCollatorForLanguageModeling:
    ...
    def mask_tokens(
        ...
        masked_indices = np.random.binomial(1, probability_matrix).astype(
            "bool"
        )
...
def advance_iter_and_group_samples(
    train_iterator, num_samples, max_seq_length
):
    """
...
if __name__ == "__main__":
    ...
    for step in range(num_train_steps):
        ...
        if step % training_args.logging_steps == 0 and step > 0:
            steps.write(
                f"Step... ({step} | Loss: {train_metric['loss'].mean()},"
                f" Learning Rate: {train_metric['learning_rate'].mean()})"
            )
            train_time += time.time() - train_start
            if has_tensorboard and jax.process_index() == 0:
                write_train_metric(
                    summary_writer, train_metrics, train_time, step
                )
            train_metrics = []
        ...
        if step % training_args.eval_steps == 0 and step > 0:
            ...
            for i, batch_idx in enumerate(
                ...
                # process input samples
                batch_eval_samples = {
                    k: [v[idx] for idx in batch_idx]
                    for k, v in eval_samples.items()
                }
            ...
            # Update progress bar
            steps.desc = (
                f"Step... ({step + 1}/{num_train_steps} | Loss:"
                f" {eval_metrics['loss']}, Acc: {eval_metrics['accuracy']})"
            )

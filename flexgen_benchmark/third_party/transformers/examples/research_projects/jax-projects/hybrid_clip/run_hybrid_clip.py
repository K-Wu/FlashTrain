...
class ModelArguments:
    ...
    from_pt: bool = field(
        ...
        metadata={
            "help": (
                "whether to load the text and vision model using PyTorch"
                " checkpoints."
            )
        },
    )
    ...
    def __post_init__(self):
        if self.train_file is None and self.validation_file is None:
            ...
            if self.train_file is not None:
                ...
                assert (
                    extension == "json"
                ), "`train_file` should be a json file."
            if self.validation_file is not None:
                extension = self.validation_file.split(".")[-1]
                assert (
                    extension == "json"
                ), "`validation_file` should be a json file."
    ...
    def __init__(
        ...
        for example in examples:
            ...
            self.image_paths.extend(
                [example["image_path"]] * len(captions_subset)
            )
...
def main():
    ...
    def clip_loss(similarity):
        loss = (
            cross_entropy(similarity, axis=0)
            + cross_entropy(similarity, axis=1)
        ) / 2
        return loss

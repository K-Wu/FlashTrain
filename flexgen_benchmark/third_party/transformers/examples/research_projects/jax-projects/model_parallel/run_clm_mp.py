...
def main():
    ...
    if data_args.block_size is None:
        ...
        if block_size > config.max_position_embeddings:
            logger.warning(
                ...
                f" `model_max_length` ({tokenizer.model_max_length}). Picking"
                " 1024 instead. You can change that default value by passing"
                " --block_size xxx."
            )
            block_size = 1024
    else:
        if data_args.block_size > tokenizer.model_max_length:
            logger.warning(
                ...
                " than the maximum length for the"
    ...
    model.params = jax.tree_util.tree_map(
        lambda x: np.asarray(x), model.params
    )
    ...
    def train_step(params, opt_state, dropout_rng, batch, step):
        ...
        metrics = {
            "loss": loss,
            "learning_rate": linear_decay_lr_schedule_fn(step),
        }
        return (
            new_params,
            tuple(new_opt_state),
    ...
    with mesh(mesh_devices, ("dp", "mp")):
        for _ in epochs:
            ...
            for _ in tqdm(
                ...
                if (
                    cur_step % training_args.logging_steps == 0
                    and cur_step > 0
                ):
                    # Save metrics
                ...
                if cur_step % training_args.eval_steps == 0 and cur_step > 0:
                    ...
                    eval_loader = data_loader(
                        input_rng, eval_dataset, eval_batch_size
                    )
                    ...
                    logger.info(
                        f"Step... ({cur_step} | Eval loss:"
                        f" {eval_metrics['loss']} | Eval Perplexity:"

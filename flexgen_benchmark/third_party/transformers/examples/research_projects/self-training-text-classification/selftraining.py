...
class STTrainingArguments:
    ...
    do_filter_by_confidence: Optional[bool] = dataclasses.field(
        ...
        metadata={
            "help": (
                "Whether to filter the pseudo-labeled data based on the"
                " confidence score."
            )
        },
    )
    ...
    finetune_on_labeled_data: Optional[bool] = dataclasses.field(
        ...
        metadata={
            "help": (
                "Whether to fine-tune on labeled data after pseudo training."
            )
        },
    )
...
def create_pseudo_labeled_data(
    ...
    dataset = datasets.concatenate_datasets(
        [infer_input, infer_output], axis=1
    )
...
def selftrain(
    model_name_or_path, train_file, infer_file, output_dir, **kwargs
):
    ...
    for iteration in range(0, int(args.max_selftrain_iterations)):
        ...
        if os.path.exists(model_bin_file_path):
            logger.info(
                (
                    "Found existing model checkpoint at %s. Skipping"
                    " self-training: iteration: %d, stage: 1."
                ),
                model_bin_file_path,
                iteration,
            )
        ...
        if iteration > 0 and args.finetune_on_labeled_data:
            ...
            if os.path.exists(model_bin_file_path):
                logger.info(
                    (
                        "Found existing model checkpoint at %s. Skipping"
                        " self-training: iteration: %d, stage: 2."
                    ),
                    model_bin_file_path,
                    iteration,
                )
        ...
        infer_output = load_dataset(
            "csv", data_files={"data": infer_output_file}
        )["data"]
        ...
        if accelerator.is_main_process:
            ...
            shutil.copy(
                eval_results_file,
                os.path.join(
                    output_dir, f"eval_results_iter-{iteration}.json"
                ),
            )
            if os.path.exists(test_results_file):
                shutil.copy(
                    eval_results_file,
                    os.path.join(
                        output_dir, f"test_results_iter-{iteration}.json"
                    ),
                )
    ...
    if best_iteration is not None:
        ...
        logger.info(
            "Best evaluation result: %s = %f", args.eval_metric, eval_result
        )
        accelerator.wait_for_everyone()
        if accelerator.is_main_process:
            shutil.copy(

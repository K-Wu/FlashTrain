...
class FTTrainingArguments:
    ...
    weight_decay: Optional[float] = dataclasses.field(
        ...
        metadata={
            "help": (
                "The weight decay to apply (if not zero) to all layers except"
                " all bias and LayerNorm weights in [`AdamW`] optimizer."
            )
        },
    )
    ...
    max_steps: Optional[int] = dataclasses.field(
        ...
        metadata={
            "help": (
                "If set to a positive number, the total number of training"
                " steps to perform. Overrides `num_train_epochs`."
            )
        },
    )
    ...
    keep_checkpoint_max: Optional[int] = dataclasses.field(
        default=1,
        metadata={
            "help": "The maximum number of best checkpoint files to keep."
        },
    )
    early_stopping_patience: Optional[int] = dataclasses.field(
        ...
        metadata={
            "help": (
                "Number of evaluation calls with no improvement after which"
    ...
    early_stopping_threshold: Optional[float] = dataclasses.field(
        ...
        metadata={
            "help": (
                "How much the specified evaluation metric must improve to"
                " satisfy early stopping conditions."
            )
        },
    )
...
def train(
    ...
    for _ in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (
                ...
                if (
                    ...
                    and args.evaluation_strategy
                    == IntervalStrategy.STEPS.value
                    ...
                    if checkpoints is None:
                        ...
                        checkpoints = np.append(
                            checkpoints, [new_checkpoint], axis=0
                        )
                    ...
                    if len(checkpoints) > args.keep_checkpoint_max:
                        ...
                        if checkpoint_to_remove != new_checkpoint:
                            if accelerator.is_main_process:
                                shutil.rmtree(
                                    os.path.join(
                                        args.output_dir, checkpoint_to_remove
                                    ),
                                    ignore_errors=True,
                                )
                            accelerator.wait_for_everyone()
        ...
        if (
            ...
            accelerator.wait_for_everyone()
            new_checkpoint = (
                f"checkpoint-{IntervalStrategy.EPOCH.value}-{epoch}"
            )
            new_eval_result = evaluate(
                args,
                accelerator,
    ...
    if best_checkpoint is not None:
        ...
        logger.info(
            "Best evaluation result: %s = %f",
            args.eval_metric,
            best_eval_result,
        )
        best_checkpoint_output_dir = os.path.join(
        ...
        checkpoint_output_dir = os.path.join(
            args.output_dir, "best-checkpoint"
        )
        if not os.path.exists(checkpoint_output_dir):
            os.makedirs(checkpoint_output_dir)
...
def finetune(
    accelerator, model_name_or_path, train_file, output_dir, **kwargs
):
    ...
    assert args.eval_metric in datasets.list_metrics(), (
        f"{args.eval_metric} not in the list of supported metrics"
        f" {datasets.list_metrics()}."
    )
    ...
    raw_datasets = load_dataset(
        args.data_file_extension, data_files=data_files
    )

    # Labels
    is_regression = raw_datasets[Split.TRAIN.value].features[
        "label"
    ...
    config, tokenizer, model = load_from_pretrained(
    ...
    non_label_column_names = [
        name
        for name in raw_datasets[Split.TRAIN.value].column_names
        if name != "label"
    ]
    if (
    ...
    for index in random.sample(range(len(train_dataset)), 3):
        logger.info(
            "Sample %d of the training set: %s.", index, train_dataset[index]
        )
    ...
    if args.do_eval:
        ...
        if eval_dataloader is not None:
            logger.info(
                "***** Running evaluation on the eval data using the best"
                " checkpoint *****"
            )
            ...
            logger.info(
                "Evaluation job completed: avg_eval_loss = %f", avg_eval_loss
            )
            logger.info(
        ...
        if test_dataloader is not None:
            logger.info(
                "***** Running evaluation on the test data using the best"
                " checkpoint *****"
            )
            eval_results = evaluate(
                args,
            ...
            eval_metric = eval_results[args.eval_metric]
            logger.info(
                "Test job completed: avg_test_loss = %f", avg_eval_loss
            )
            logger.info(

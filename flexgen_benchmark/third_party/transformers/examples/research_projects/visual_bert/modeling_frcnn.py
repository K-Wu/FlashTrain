...
def do_nms(boxes, scores, image_shape, score_thresh, nms_thresh, mind, maxd):
    ...
    idxs = (
        torch.arange(num_objs).to(boxes.device) * num_bbox_reg_classes
        + max_classes
    )
...
class Matcher(object):
    ...
    def set_low_quality_matches_(self, match_labels, match_quality_matrix):
        ...
        if of_quality_inds.dim() == 0:
            ...
            ) = of_quality_inds.nonzero().unbind(1)
...
class RPNOutputs(object):
    ...
    def predict_proposals(self):
        ...
        for anchors_i, pred_anchor_deltas_i in zip(
            anchors, self.pred_anchor_deltas
        ):
            B = anchors_i.size(-1)
...
class LastLevelMaxPool(nn.Module):
    ...
    def forward(self, x):
        return [
            nn.functional.max_pool2d(x, kernel_size=1, stride=2, padding=0)
        ]
...
class BasicStem(nn.Module):
    def __init__(
        self, in_channels=3, out_channels=64, norm="BN", caffe_maxpool=False
    ):
        super().__init__()
        self.conv1 = Conv2d(
            in_channels,
    ...
    def make_stage(
        ...
        **kwargs,
        ...
        if first_stride is not None:
            ...
            kwargs["stride_per_block"] = [first_stride] + [1] * (
                num_blocks - 1
            )
        blocks = []
            ...
            for k, v in kwargs.items():
                if k.endswith("_per_block"):
                    assert len(v) == num_blocks, (
                        f"Argument '{k}' of make_stage should have the same"
                        f" length as num_blocks={num_blocks}."
                    )
...
class ROIPooler(nn.Module):
    ...
    def __init__(
        ...
        if len(scales) > 1:
            assert (
                min_level <= canonical_level and canonical_level <= max_level
            )
...
class ROIOutputs(object):
    ...
    def _predict_boxes(self, proposals, box_deltas, preds_per_image):
        ...
        proposals = (
            torch.cat(proposals, dim=0).unsqueeze(-2).expand(num_pred, K, B)
        )
    ...
    def inference(
        ...
        final_results = []
        zipped = zip(
            boxes_all, obj_scores_all, attr_probs_all, attrs_all, sizes
        )
        for i, (boxes, obj_scores, attr_probs, attrs, size) in enumerate(
            zipped
        ):
...
class Res5ROIHeads(nn.Module):
    ...
    def __init__(self, cfg, input_shape):
        ...
        self.out_channels = (
            cfg.RESNETS.RES2_OUT_CHANNELS * self.stage_channel_factor
        )
    ...
    def _build_res5_block(self, cfg):
        ...
        width_per_group = cfg.RESNETS.WIDTH_PER_GROUP
        bottleneck_channels = (
            num_groups * width_per_group * stage_channel_factor
        )
...
class RPN(nn.Module):
    ...
    def inference(
        self, outputs, images, image_shapes, features, gt_boxes=None
    ):
    ...
    def forward(self, images, image_shapes, features, gt_boxes=None):
        ...
        if self.training:
            ...
            return self.training(
                outputs, images, image_shapes, features, gt_boxes
            )
        else:
            return self.inference(
                outputs, images, image_shapes, features, gt_boxes
    ...
    def from_pretrained(
        ...
        if state_dict is None:
            try:
                ...
                raise OSError(
                    "Unable to load weights from pytorch checkpoint file. If"
                    " you tried to load a PyTorch model from a TF 2.0"

    ...
    OrderedDict(
        {
            ...
            "boxes": datasets.Array2D(
                (CONFIG.MAX_DETECTIONS, 4), dtype="float32"
            ),
            ...
            "sizes": datasets.Sequence(
                length=2, feature=datasets.Value("float32")
            ),
...
class Extract:
    def __init__(self, argv=sys.argv[1:]):
        ...
        opts, args = getopt.getopt(
            argv,
            "i:o:b:s",
            ["inputdir=", "outfile=", "batch_size=", "subset_list="],
        )
        for opt, arg in opts:
    ...
    def __call__(self):
        ...
        if not TEST:
            ...
            print(
                f"Success! You wrote {num_examples} entry(s) and"
                f" {num_bytes >> 20} mb"
            )

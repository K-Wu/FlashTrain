...
class FSNERTokenizerUtils(object):
    ...
    def tokenize(self, x):
        ...
        if isinstance(x, list) and all([isinstance(_x, list) for _x in x]):
            ...
            raise Exception(
                "Type of parameter x was not recognized! Only `list of"
                " strings` for query or `list of lists of strings` for"
                " supports are supported."
            )

        return d

    def extract_entity_from_scores(
        self, query, W_query, p_start, p_end, thresh=0.70
    ):
        """

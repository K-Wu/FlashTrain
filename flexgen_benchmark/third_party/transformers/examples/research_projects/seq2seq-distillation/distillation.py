...
class SummarizationDistiller(SummarizationModule):
    ...
    def __init__(self, hparams):
        ...
        assert student.config.model_type == teacher.config.model_type, (
            "teacher, student model types should be the same, got"
            f" {student.config.model_type} != {teacher.config.model_type}"
        )
        ...
        loss_ce = (
            self.ce_loss_fct(
                nn.functional.log_softmax(
                    s_logits_slct / self.temperature, dim=-1
                ),
                nn.functional.softmax(
                    t_logits_slct / self.temperature, dim=-1
                ),
            )
            * (self.temperature) ** 2
        )
    ...
    def calc_hidden_loss(
        ...
        mse = nn.functional.mse_loss(
            student_states, teacher_states, reduction="none"
        )
        masked_mse = (
            mse * mask.unsqueeze(0).unsqueeze(-1)
        ).sum() / valid_count
        return masked_mse
...
def add_distill_args(parser):
    ...
    parser.add_argument(
        "--supervise_forward", action="store_true", default=False
    )
    parser.add_argument(
        "--normalize_hidden", action="store_true", default=False
    )

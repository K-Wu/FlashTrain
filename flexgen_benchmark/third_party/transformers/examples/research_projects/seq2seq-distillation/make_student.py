...
def pick_layers_to_copy(n_student, n_teacher):
    try:
        ...
        if n_student != n_teacher:
            warnings.warn(
                f"no hardcoded layers to copy for teacher {n_teacher} ->"
                f" student {n_student}, defaulting to first {n_student}"
            )
...
def get_layers_to_supervise(n_student, n_teacher) -> List[int]:
    ...
    if n_student > n_teacher:
        raise ValueError(
            "Cannot perform intermediate supervision for student"
            f" {n_student} > teacher {n_teacher}"
        )
...
def create_student_by_copying_alternating_layers(
    ...
    **extra_config_kwargs,
    ...
    """
    _msg = (
        "encoder_layers and decoder_layers cannot be both None-- you would"
        " just have an identical teacher."
    )
    ...
    if copy_first_teacher_layers:  # Our copying is done. We just log and save
        ...
        logger.info(
            f"Copied encoder layers {e_layers_to_copy} and decoder layers"
            f" {d_layers_to_copy}. Saving them to {save_path}"
        )
    ...
    try:
        ...
        copy_layers(
            teacher.encoder.block, student.encoder.block, e_layers_to_copy
        )
        copy_layers(
            teacher.decoder.block, student.decoder.block, d_layers_to_copy
        )
    logger.info(
        f"Copied encoder layers {e_layers_to_copy} and decoder layers"
        f" {d_layers_to_copy}. Saving them to {save_path}"

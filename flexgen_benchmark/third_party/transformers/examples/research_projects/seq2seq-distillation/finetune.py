...
class SummarizationModule(BaseTransformer):
    ...
    def __init__(self, hparams, **kwargs):
        if hparams.sortish_sampler and hparams.gpus > 1:
            ...
            if hparams.sortish_sampler:
                raise ValueError(
                    "--sortish_sampler and --max_tokens_per_batch may not be"
                    " used simultaneously"
                )
    ...
    def get_dataloader(
        ...
        if (
            ...
            batch_sampler = dataset.make_dynamic_sampler(
                self.hparams.max_tokens_per_batch,
                distributed=self.hparams.gpus > 1,
            )
            return DataLoader(
                dataset,
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument(
            "--sortish_sampler", action="store_true", default=False
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--eval_beams", type=int, default=None, required=False
        )
        parser.add_argument(
            "--val_metric",
            type=str,

...
class TeacherModelArguments:
    ...
    hypothesis_template: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Template used to turn class names into mock hypotheses for"
                " teacher NLI model. Must include {{}}where class name is"
                " inserted."
            )
        },
    )
    ...
    multi_label: Optional[bool] = field(
        ...
        metadata={
            "help": (
                "Allow multiple classes to be true rather than forcing them to"
                " sum to 1 (sometimes calledmulti-class multi-label"
                " classification)."
            )
        },
    )
...
class DataTrainingArguments:
    ...
    use_fast_tokenizer: bool = field(
        ...
        metadata={
            "help": (
                "Whether to use one of the fast tokenizer (backed by the Rust"
...
class DistillTrainingArguments(TrainingArguments):
    ...
    do_eval: bool = field(
        ...
        metadata={
            "help": (
                "Whether to evaluate the agreement of the final student"
                " predictions and the teacher predictionsafter training."
            )
        },
    )
    save_total_limit: Optional[int] = field(
        ...
        metadata={
            "help": (
                ...
                " checkpoints in the output_dir. Default is 0 (no"
                " checkpoints)."
            )
        },
    )
...
class DistillationTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss = -torch.sum(
            target_p * logits.log_softmax(dim=-1), axis=-1
        ).mean()
...
def get_entailment_id(config):
    for label, ind in config.label2id.items():
        if label.lower().startswith("entail"):
            return ind
    logger.warning(
        "Could not identify entailment dimension from teacher config label2id."
        " Setting to -1."
    )
    return -1
...
def main():
    ...
    if training_args.do_eval:
        ...
        logger.info(
            "Agreement of student and teacher predictions:"
            f" {agreement * 100:0.2f}%"
        )

...
class ModelArguments:
    ...
    tokenizer_name_or_path: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Path to pretrained tokenizer or tokenizer identifier from"
                " huggingface.co/models"
            )
        },
    )
    cache_dir: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Where do you want to store the pretrained models and datasets"
    ...
    activation_dropout: float = field(
        ...
        metadata={
            "help": (
                "The dropout ratio for activations inside the fully connected"
                " layer."
            )
        },
    )
    ...
    mask_time_prob: float = field(
        ...
        metadata={
            "help": (
                "Probability of each feature vector along the time axis to be"
                " chosen as the start of the vectorspan to be masked."
                " Approximately ``mask_time_prob * sequence_length //"
                " mask_time_length`` featurevectors will be masked along the"
                " time axis."
            )
        },
    )
    ...
    mask_feature_prob: float = field(
        ...
        metadata={
            "help": (
                "Probability of each feature vector along the feature axis to"
                " be chosen as the start of the vectorspan to be masked."
                " Approximately ``mask_feature_prob * sequence_length //"
                " mask_feature_length`` feature bins will be masked along the"
                " time axis."
            )
        },
    )
    ...
    ctc_loss_reduction: Optional[str] = field(
        ...
        metadata={
            "help": (
                "The way the ctc loss should be reduced. Should be one of"
                " 'mean' or 'sum'."
            )
        },
    )
...
class DataTrainingArguments:
    ...
    task: str = field(
        ...
        metadata={
            "help": (
                "The task name of the benchmark to use (via the datasets"
                " library). Should be on of: 'fleurs-asr', 'mls', 'voxpopuli',"
                " 'covost2', 'minds14', 'fleurs-lang_id', 'babel'."
            )
        },
    )
    language: str = field(
        ...
        metadata={
            "help": (
                "The language id as defined in the datasets config name or"
    ...
    language_group: str = field(
        ...
        metadata={
            "help": (
                "The language group to select a subset of languages to train"
                " on. This option is only used the 'fleurs-asr' task. Should"
                " be one of: 'western_european_we', 'eastern_european_ee',"
                " 'central_asia_middle_north_african_cmn',"
                " 'sub_saharan_african_ssa', 'south_asian_sa',"
                " 'south_east_asian_sea', 'chinese_japanase_korean_cjk'."
            )
        },
    )
    train_split_name: str = field(
        ...
        metadata={
            "help": (
                "The name of the training dataset split to use (via the"
                " datasets library). Defaults to 'train'"
            )
        },
    )
    eval_split_name: str = field(
        ...
        metadata={
            "help": (
                "The name of the evaluation dataset split to use (via the"
                " datasets library). Defaults to 'validation'"
            )
        },
    )
    predict_split_name: str = field(
        ...
        metadata={
            "help": (
                "The name of the prediction dataset split to use (via the"
                " datasets library). Defaults to 'test'"
            )
        },
    )
    audio_column_name: str = field(
        ...
        metadata={
            "help": (
                "The name of the dataset column containing the audio data."
                " Defaults to 'audio'"
            )
        },
    )
    target_column_name: str = field(
        ...
        metadata={
            "help": (
                "The name of the dataset column containing the target data"
                " (transcription/translation/label). If None, the name will be"
                " inferred from the task. Defaults to None."
            )
        },
    )
    ...
    max_duration_in_seconds: float = field(
        ...
        metadata={
            "help": (
                "Filter audio files that are longer than"
                " `max_duration_in_seconds` seconds to"
    ...
    min_duration_in_seconds: float = field(
        ...
        metadata={
            "help": (
                "Filter audio files that are shorter than"
                " `min_duration_in_seconds` seconds"
            )
        },
    )
    preprocessing_only: bool = field(
        ...
        metadata={
            "help": (
                "Whether to only do data preprocessing and skip training. This"
                " is especially useful when data preprocessing errors out in"
                " distributed training due to timeout. In this case, one"
                " should run the preprocessing in a non-distributed setup with"
    ...
    use_auth_token: bool = field(
        ...
        metadata={
            "help": (
                "If :obj:`True`, will use the token generated when"
                " running:obj:`huggingface-cli login` as HTTP bearer"
    ...
    per_lang_metrics: bool = field(
        ...
        metadata={
            "help": (
                "If `True`, compute the test metrics separately for each"
                " language, and average the results. If `False` compute the"
                " average test metrics in a single pass for all languages at"
                " once."
            )
        },
    )
    ...
    def __call__(
        ...
        if self.pad_labels:
            ...
            if (
                ...
                and (labels[:, 0] == self.decoder_start_token_id)
                .all()
                .cpu()
                .item()
            ):
...
def create_vocabulary_from_data(
    ...
    vocab_set = (
        ...
        | (
            set(vocabs["predict"]["vocab"][0])
            if "predict" in vocabs
            else set()
        )
    )
...
def main():
    ...
    logger.warning(
        ...
        f" {training_args.device}, n_gpu: {training_args.n_gpu}distributed"
        f" training: {bool(training_args.local_rank != -1)}, 16-bits training:"
    ...
    if task_name is None:
        raise ValueError(
            "Set --task should be set to '<xtreme_s_task>' (e.g. 'fleurs-asr',"
            " 'mls', 'covost2', 'minds14') "
        )
    if lang_id is None:
        raise ValueError(
            "Set --language should be set to the language id of the sub"
            " dataset config to be used (e.g. 'pl', 'en.tr', 'fr-FR') or 'all'"
    ...
    if training_args.do_train:
        ...
        if (
            ...
            not in raw_datasets["train"].column_names
        ):
            raise ValueError(
                f"--audio_column_name '{data_args.audio_column_name}' not"
                f" found in dataset '{data_args.dataset_name}'. Make sure to"
                " set `--audio_column_name` to the correct audio column - one"
                f" of {', '.join(raw_datasets['train'].column_names)}."
            )

        if target_column_name not in raw_datasets["train"].column_names:
            raise ValueError(
                f"--target_column_name {target_column_name} not found in"
                f" dataset '{data_args.dataset_name}'. Make sure to set"
                " `--target_column_name` to the correct text column - one of"
    ...
    def remove_special_characters(batch):
        if chars_to_ignore_regex is not None:
            batch["target_text"] = (
                re.sub(
                    chars_to_ignore_regex, "", batch[target_column_name]
                ).lower()
                + " "
            )
        else:
    ...
    if target_column_name == "transcription":
        ...
        if model.config.decoder_start_token_id is None:
            raise ValueError(
                "Make sure that `config.decoder_start_token_id` is correctly"
                " defined"
            )
    else:
    ...
    if data_args.preprocessing_only:
        logger.info(
            ...
            f" {vectorized_datasets.cache_files}"
        )
        return
    ...
    kwargs = {
        ...
        "dataset_args": (
            f"Config: {config_name}, Training split:"
            f" {data_args.train_split_name}, Eval split:"
            f" {data_args.eval_split_name}, Predict split:"
            f" {data_args.predict_split_name}"
        ),

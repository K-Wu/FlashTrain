...
class FlaxDataCollatorForT5MLM:
    ...
    def __call__(self, examples: List[Dict[str, np.ndarray]]) -> BatchEncoding:
        ...
        if batch["input_ids"].shape[-1] != self.input_length:
            raise ValueError(
                "`input_ids` are incorrectly preprocessed. `input_ids` length"
                f" is {batch['input_ids'].shape[-1]}, but should be"
                f" {self.input_length}."
            )
        ...
        if batch["labels"].shape[-1] != self.target_length:
            raise ValueError(
                ...
                f" {batch['labels'].shape[-1]}, but should be"
...
def main():
    ...
    send_example_telemetry(
        "run_t5_mlm", model_args, data_args, framework="flax"
    )

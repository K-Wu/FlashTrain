...
class DataTrainingArguments:
    ...
    train_ref_file: Optional[str] = field(
        ...
        metadata={
            "help": (
                "An optional input train ref data file for whole word masking"
                " in Chinese."
            )
        },
    )
    validation_ref_file: Optional[str] = field(
        ...
        metadata={
            "help": (
                "An optional input validation ref data file for whole word"
                " masking in Chinese."
            )
        },
    )
    ...
    max_seq_length: Optional[int] = field(
        ...
        metadata={
            "help": (
                ...
                " and masking. Sequences longer than this will be truncated."
    ...
    mlm_probability: float = field(
        ...
        metadata={
            "help": (
                "Ratio of tokens to mask for span masked language modeling"
                " loss"
            )
        },
    )
    permute_sentence_ratio: float = field(
        default=1.0,
        metadata={
            "help": "Ratio of sentences to be permuted in each document"
        },
    )
    poisson_lambda: float = field(
        ...
        metadata={
            "help": (
                "Mean of Poisson distribution used to generate span-lengths to"
                " be masked"
            )
        },
    )
...
class FlaxDataCollatorForBartDenoisingLM:
    ...
    def __post_init__(self):
        if (
            self.tokenizer.mask_token is None
            or self.tokenizer.eos_token is None
        ):
            raise ValueError(
                "This tokenizer does not have a mask token or eos token token"
                " which is necessary for denoising language modeling. "
            )
    ...
    def permute_sentences(self, input_ids):
        ...
        num_sentences_map = {
            sent_idx: count
            for sent_idx, count in zip(
                example_has_multiple_sentences, num_sentences
            )
        }

        num_to_permute = np.ceil(
            num_sentences * self.permute_sentence_ratio
        ).astype(int)
        num_to_permute_map = {
            sent_idx: count
            for sent_idx, count in zip(
                example_has_multiple_sentences, num_to_permute
            )
        }
        ...
        sentence_ends_map = {
            sent_idx: count
            for sent_idx, count in zip(
                example_has_multiple_sentences, sentence_ends
            )
        }
        ...
        for i in range(input_ids.shape[0]):
            ...
            for j in ordering:
                sentence = input_ids[
                    i,
                    (
                        sentence_ends_map[i][j - 1] if j > 0 else 0
                    ) : sentence_ends_map[i][j],
                ]
    ...
    def span_mask_tokens(self, input_ids, labels, do_permute):
        ...
        special_tokens_mask_labels = np.array(
            special_tokens_mask_labels, dtype=bool
        )
        special_tokens_mask_inputs = np.array(
            special_tokens_mask_inputs, dtype=bool
        )
        ...
        cutoff_idx = (
            np.argmin(np.abs(np.cumsum(span_lengths, 0) - num_tokens_to_mask))
            + 1
        )
        ...
        new_input_ids = np.full_like(
            input_ids, fill_value=self.tokenizer.pad_token_id
        )
...
def main():
    ...
    send_example_telemetry(
        "run_bart_dlm", model_args, data_args, framework="flax"
    )
    ...
    for epoch in epochs:
        ...
        for step, batch_idx in enumerate(
            ...
            if cur_step % training_args.eval_steps == 0 and cur_step > 0:
                ...
                # Update progress bar
                epochs.desc = (
                    f"Step... ({cur_step} | Loss: {eval_metrics['loss']}, Acc:"

...
def write_metric(
    summary_writer, train_metrics, eval_metrics, train_time, step
):
...
def main():
    ...
    send_example_telemetry(
        "run_summarization", model_args, data_args, framework="flax"
    )
    ...
    model_module = __import__(
        model.__module__, fromlist=["shift_tokens_tight"]
    )
    ...
    for epoch in epochs:
        ...
        desc = (
            f"Epoch... ({epoch + 1}/{num_epochs} | Eval Loss:"
            f" {eval_metrics['loss']} | {rouge_desc})"
        )
        epochs.write(desc)
        epochs.desc = desc

...
class DataTrainingArguments:
    ...
    task_name: Optional[str] = field(
        ...
        metadata={
            "help": (
                "The name of the glue task to train on. choices"
                f" {list(task_to_keys.keys())}"
            )
        },
    )
    ...
    validation_file: Optional[str] = field(
        ...
        metadata={
            "help": (
                "An optional input evaluation data file to evaluate on (a csv"
                " or JSON file)."
            )
        },
    )
    test_file: Optional[str] = field(
        ...
        metadata={
            "help": (
                "An optional input test data file to predict on (a csv or JSON"
                " file)."
            )
        },
    )
...
def main():
    ...
    if (
        ...
        if list(sorted(label_name_to_id.keys())) == list(sorted(label_list)):
            logger.info(
                "The configuration of the model provided the following label"
    ...
    epochs = tqdm(
        range(num_epochs), desc=f"Epoch ... (0/{num_epochs})", position=0
    )
    for epoch in epochs:
        ...
        for step, batch in enumerate(
            ...
            if (
                ...
                # evaluate
                eval_loader = glue_eval_data_collator(
                    eval_dataset, eval_batch_size
                )
                for batch in tqdm(
                    eval_loader,
                ...
                logger.info(
                    f"Step... ({cur_step}/{total_steps} | Eval metrics:"
                    f" {eval_metric})"
                )

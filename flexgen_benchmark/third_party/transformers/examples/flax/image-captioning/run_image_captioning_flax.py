...
class ModelArguments:
    ...
    dtype: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Floating-point format in which the model weights should be"
                " initialized and trained. Choose one of `[float32, float16,"
                " bfloat16]`."
            )
        },
    )
    use_auth_token: bool = field(
        ...
        metadata={
            "help": (
                "Will use the token generated when running `huggingface-cli"
...
class DataTrainingArguments:
    ...
    data_dir: Optional[str] = field(
        ...
        metadata={
            "help": (
                "The data directory of the dataset to use (via the datasets"
                " library)."
            )
        },
    )
    image_column: Optional[str] = field(
        ...
        metadata={
            "help": (
                "The name of the column in the datasets containing the full"
                " image file paths."
            )
        },
    )
    caption_column: Optional[str] = field(
        ...
        metadata={
            "help": (
                "The name of the column in the datasets containing the image"
                " captions."
            )
        },
    )
    ...
    test_file: Optional[str] = field(
        ...
        metadata={
            "help": (
                "An optional input predict data file to do prediction on (a"
                " text file)."
            )
        },
    )
    ...
    val_max_target_length: Optional[int] = field(
        ...
        metadata={
            "help": (
                "The maximum total sequence length for validation target text"
                " after tokenization. Sequences longer than this will be"
                " truncated, sequences shorter will be padded. Will default to"
                " `max_target_length`.This argument is also used to override"
                " the `max_length` param of `model.generate`, which is used"
                " during evaluation."
            )
        },
    )
    ...
    predict_with_generate: bool = field(
        ...
        metadata={
            "help": (
                "Whether to use generate to calculate generative metrics"
                " (ROUGE, BLEU)."
            )
        },
    )
    num_beams: Optional[int] = field(
        ...
        metadata={
            "help": (
                ...
                " passed to `model.generate`, which is used during evaluation."
            )
        },
    )
...
def write_metric(
    summary_writer, metrics, train_time, step, metric_key_prefix="train"
):
    if train_time:
        ...
        for metric_name, value in metrics.items():
            summary_writer.scalar(
                f"{metric_key_prefix}_{metric_name}", value, step
            )
...
def main():
    ...
    dataset_columns = image_captioning_name_mapping.get(
    ...
    if data_args.image_column is None:
        ...
        if image_column not in column_names:
            raise ValueError(
                f"--image_column' value '{data_args.image_column}' needs to be"
    ...
    if data_args.caption_column is None:
        ...
        if caption_column not in column_names:
            raise ValueError(
                f"--caption_column' value '{data_args.caption_column}' needs"
                f" to be one of: {', '.join(column_names)}"
            )
    ...
    model_module = __import__(
        model.__module__, fromlist=["shift_tokens_right"]
    )
    ...
    def feature_extraction_fn(examples, check_image=True):
        ...
        if check_image:
            ...
            images = [
                Image.open(image_file) for image_file in examples[image_column]
            ]
    ...
    # Used in .map() below
    function_kwarg = (
        preprocess_fn if run_feat_ext_at_beginning else tokenization_fn
    )
    ...
    remove_columns_kwarg = [
        ...
        if x != image_column or run_feat_ext_at_beginning
    ]
    processor_names = (
    ...
    if (
        ...
        raise ValueError(
            "`training_args.block_size` needs to be a multiple of the global"
            f" train/eval batch size.Got {training_args.block_size},"
            f" {train_batch_size} and {eval_batch_size} respectively instead."
        )
    ...
    def blockwise_data_loader(
        ...
        num_splits = steps // steps_per_block + int(
            steps % steps_per_block > 0
        )
        ...
        for idx in range(num_splits):
            if not block_size:
                ...
                _ds = _ds.map(
                    ...
                    desc=(
                        f"Running feature extraction on {split} dataset"
                        .replace("  ", " ")
                    ),
                )
            ...
            loader = data_loader(
                rng, _ds, batch_size=batch_size, shuffle=False
            )
    ...
    if has_tensorboard and jax.process_index() == 0:
        ...
        logger.warning(
            "Unable to display metrics through TensorBoard because the package"
            " is not installed: Please run pip install tensorboard to enable."
        )
    ...
    if training_args.do_train:
        ...
        logger.info(
            "  Instantaneous train batch size per device ="
    ...
    if training_args.do_eval:
        ...
        logger.info(
            "  Instantaneous evaluation batch size per device ="
            f" {training_args.per_device_eval_batch_size}"
        )
        logger.info(
            "  Total evaluation batch size (w. parallel & distributed) ="
            f" {eval_batch_size}"
        )
    ...
    if training_args.do_predict:
        ...
        logger.info(
            "  Instantaneous test batch size per device ="
            f" {training_args.per_device_eval_batch_size}"
        )
        logger.info(
            "  Total test batch size (w. parallel & distributed) ="
            f" {eval_batch_size}"
        )
    ...
    def save_ckpt(ckpt_dir: str, commit_msg: str = ""):
        ...
        if jax.process_index() == 0:
            ...
            tokenizer.save_pretrained(
                os.path.join(training_args.output_dir, ckpt_dir)
            )
            if training_args.push_to_hub:
    ...
    def evaluation_loop(
        ...
        for _ in tqdm(
            ...
            if not is_prediction and _labels is None:
                raise ValueError(
                    "Evaluation requires the validation dataset to have"
                    " `labels`"
                )
        ...
        if data_args.predict_with_generate:
            if labels:
                ...
                rouge_desc = " ".join(
                    [
                        f"{'Predict' if is_prediction else 'Eval'} {key}:"
                        f" {value} |"
        ...
        if metrics:
            ...
            desc = (
                f"{'Predict' if is_prediction else 'Eval'} Loss:"
                f" {metrics['loss']} | {rouge_desc})"
            )
            if training_args.do_train and not is_prediction:
                desc = (
                    f"Epoch... ({epoch + 1}/{num_epochs} | Step: {cur_step} | "
                    + desc
                )
                epochs.write(desc)
                epochs.desc = desc
        ...
        if jax.process_index() == 0:
            ...
            if metrics:
                ...
                metrics = {
                    f"{metric_key_prefix}_{metric_name}": round(
                        value.item(), 6
                    )
                    for metric_name, value in metrics.items()
                }
    ...
    def evaluate(
        rng: jax.random.PRNGKey, dataset: Dataset, ckpt_dir: str = ""
    ):
        evaluation_loop(
            rng, dataset, metric_key_prefix="eval", ckpt_dir=ckpt_dir
        )

    def predict(rng: jax.random.PRNGKey, dataset: Dataset):
        evaluation_loop(
            rng, dataset, metric_key_prefix="test", is_prediction=True
        )
            ...
            for batch_idx, _ in enumerate(
                ...
                if (
                    ...
                    desc = (
                        ...
                        f" {cur_step} | Loss: {_train_metric['loss']} |"
                        f" Learning Rate: {_train_metric['learning_rate']} |"
                        f" Time per step: {time_per_step})"
                    )
                    epochs.desc = desc
                    epochs.write(desc)
                ...
                if (
                    ...
                    commit_msg = (
                        f"Saving weights and logs of epoch {epoch + 1} - step"
                        f" {cur_step}"
                    )

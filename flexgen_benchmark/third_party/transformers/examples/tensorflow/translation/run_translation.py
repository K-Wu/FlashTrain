    ...
    forced_bos_token: Optional[str] = field(
        ...
        metadata={
            "help": (
                ...
                " :obj:`decoder_start_token_id`.Useful for multilingual models"
                " like :doc:`mBART <../model_doc/mbart>` where the first"
                " generated token needs to be the target language"
                " token.(Usually it is the target language token)"
            )
        },
    )
...
def main():
    ...
    if isinstance(tokenizer, tuple(MULTILINGUAL_TOKENIZERS)):
        assert (
            data_args.target_lang is not None
    ...
    with training_args.strategy.scope():
        ...
        if model.config.decoder_start_token_id is None and isinstance(
            ...
            if isinstance(tokenizer, MBartTokenizer):
                model.config.decoder_start_token_id = (
                    tokenizer.lang_code_to_id[data_args.target_lang]
                )
            else:
                model.config.decoder_start_token_id = (
                    tokenizer.convert_tokens_to_ids(data_args.target_lang)
                )
        ...
        total_eval_batch_size = (
            training_args.per_device_eval_batch_size * num_replicas
        )
        ...
        languages = [
            l
            for l in [data_args.source_lang, data_args.target_lang]
            if l is not None
        ]
        if len(languages) > 0:
        ...
        if training_args.do_train:
            ...
            if training_args.xla and not data_args.pad_to_max_length:
                logger.warning(
                    "XLA training may be slow at first when"
                    " --pad_to_max_length is not set until all possible shapes"
                    " have been compiled."
                )
            ...
            eval_metrics = {
                key: val[-1] for key, val in history.history.items()
            }
        # endregion
        ...
        if (
            ...
            and not training_args.push_to_hub
        ):

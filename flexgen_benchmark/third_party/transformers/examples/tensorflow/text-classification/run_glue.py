...
class DataTrainingArguments:
    ...
    predict_file: str = field(
        metadata={
            "help": (
                "A file containing user-supplied examples to make"
                " predictions for"
            )
        },
        default=None,
...
def main():
    ...
    send_example_telemetry(
        "run_glue", model_args, data_args, framework="tensorflow"
    )
    ...
    if not (
        ...
        exit(
            "Must specify at least one of --do_train, --do_eval or"
            " --do_predict!"
        )
    # endregion
    ...
    if (
        ...
        if (
            ...
            logger.info(
                f"Checkpoint detected, resuming training at {checkpoint}. To"
    ...
    def compute_metrics(preds, label_ids):
        preds = preds["logits"]
        preds = (
            np.squeeze(preds) if is_regression else np.argmax(preds, axis=1)
        )
    ...
    with training_args.strategy.scope():
        ...
        if training_args.do_train:
            ...
            optimizer = (  # Just write anything because we won't be using it
                "adam"
            )
        if is_regression:
            metrics = []
        ...
        if training_args.do_eval:
            ...
            for raw_dataset, tf_dataset, task in zip(
                raw_datasets, tf_datasets, tasks
            ):
                eval_predictions = model.predict(tf_dataset)
                eval_metrics = compute_metrics(
                    eval_predictions, raw_dataset["label"]
                )

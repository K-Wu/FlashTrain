...
] = "1"  # Reduce the amount of console output from TF
...
class DataTrainingArguments:
    ...
    pad_to_max_length: bool = field(
        ...
        metadata={
            "help": (
                ...
                " maximum length in the batch.Data will always be padded when"
                " using TPUs."
            )
        },
    )
    ...
    def __post_init__(self):
        ...
        assert len(extensions) != 0, (
            "Need to supply at least one of --train_file, --validation_file or"
            " --test_file!"
        )
        assert len(extensions) == 1, (
            "All input files should have the same file extension, either csv"
            " or json!"
        )
        assert (
...
def main():
    ...
    if (
        ...
        if (output_dir / CONFIG_NAME).is_file() and (
            ...
            logger.info(
                ...
                f" {training_args.output_dir}. To avoid this behavior, change"
            ...
            raise ValueError(
                ...
                " and is not empty. Use --overwrite_output_dir to continue"
                " regardless."
            )
    ...
    data_files = {
        key: file for key, file in data_files.items() if file is not None
    }
    ...
    column_names = {
        col for cols in datasets.column_names.values() for col in cols
    }
        ...
        if (
            ...
            if list(sorted(label_name_to_id.keys())) == list(
                ...
                logger.warning(
                    ...
                    (
                        "model labels:"
                        f" {list(sorted(label_name_to_id.keys()))}, dataset"
                        f" labels: {list(sorted(label_list))}.\nIgnoring the"
                        " model labels as a result."
                    ),
                )
    ...
    if "validation" in datasets and config.label2id is not None:
        ...
        for val_label in validation_label_list:
            assert val_label in label_to_id, (
                f"Label {val_label} is in the validation set but not the"
                " training set!"
            )
    ...
    def preprocess_function(examples):
        ...
        if config.label2id is not None and "label" in examples:
            result["label"] = [
                (config.label2id[l] if l != -1 else -1)
    ...
    with training_args.strategy.scope():
        ...
        if training_args.do_train:
            num_train_steps = (
                len(tf_data["train"]) * training_args.num_train_epochs
            )
        ...
        if not push_to_hub_model_id:
            push_to_hub_model_id = (
                f"{model_name}-finetuned-text-classification"
            )
        ...
        if tf_data["validation"] is not None:
            ...
            if is_regression:
                ...
                logger.info(
                    f"Eval loss: {loss:.5f}, Eval accuracy:"
                    f" {accuracy * 100:.4f}%"
                )

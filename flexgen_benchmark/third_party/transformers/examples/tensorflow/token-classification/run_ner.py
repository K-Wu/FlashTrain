...
require_version(
    ...
    (
        ...
        " examples/tensorflow/token-classification/requirements.txt"
    ),
)
...
def main():
    ...
    send_example_telemetry(
        "run_ner", model_args, data_args, framework="tensorflow"
    )
    # endregion
    ...
    with training_args.strategy.scope():
        ...
        if not push_to_hub_model_id:
            if data_args.dataset_name is not None:
                ...
                push_to_hub_model_id = (
                    f"{model_name}-finetuned-token-classification"
                )
        ...
        try:
            ...
            raise ValueError(
                "Concatenating predictions failed! If your version of"
                " TensorFlow is 2.8.0 or older then you will need to use"
                " --pad_to_max_length to generate predictions, as older"
                " versions of TensorFlow cannot concatenate variable-length"
                " predictions as RaggedTensor."
            )

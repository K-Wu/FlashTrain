...
def main():
    ...
    send_example_telemetry(
        "run_qa", model_args, data_args, framework="tensorflow"
    )
    ...
    if data_args.pad_to_max_length or isinstance(
        ...
        logger.info(
            "Padding all batches to max length because argument was set or"
            " we're on TPU."
        )
        padding = "max_length"
    else:
    ...
    with training_args.strategy.scope():
        ...
        if training_args.do_train:
            ...
            num_train_steps = (
                len(training_dataset) * training_args.num_train_epochs
            )
        ...
        if not push_to_hub_model_id:
            if data_args.dataset_name is not None:
                ...
                push_to_hub_model_id = (
                    f"{model_name}-finetuned-question-answering"
                )

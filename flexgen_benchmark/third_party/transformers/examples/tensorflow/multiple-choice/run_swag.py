...
class DataTrainingArguments:
    ...
    pad_to_max_length: bool = field(
        ...
        metadata={
            "help": (
                "Whether to pad all samples to the maximum sentence length. If"
                " False, will pad the samples dynamically when batching to the"
                " maximum length in the batch. More efficient on GPU but very"
                " bad for TPU."
            )
        },
    )
...
def main():
    ...
    send_example_telemetry(
        "run_swag", model_args, data_args, framework="tensorflow"
    )
    ...
    if data_args.max_seq_length is None:
        ...
        if data_args.max_seq_length > tokenizer.model_max_length:
            logger.warning(
                ...
                " larger than the maximum length for themodel"

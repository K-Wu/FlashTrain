...
def parse_search_arg(search):
    ...
    sets = [
        list(f"--{k} {v}" for v in vs.split(":")) for k, vs in entries.items()
    ]
...
def run_search():
    ...
    parser = argparse.ArgumentParser(
        usage=(
            "\n\nImportant: this script accepts all arguments `run_eval.py`"
            " accepts and then a few extra, therefore refer to `run_eval.py"
            " -h` for the complete list."
        )
    )
    parser.add_argument(
        "--search",
        ...
        required=False,
        help=(
            'param space to search, e.g. "num_beams=5:10'
    ...
    parser.add_argument(
        ...
        const=datetime_now(),
        help=(
            "add custom notes to be printed before the results table. If no"
            " value is passed, the current datetime string will be used."
        ),
    )
    ...
    for row in results_sorted:
        print(
            " | ".join([f"{row[col]:{col_widths[col]}}" for col in col_names])
        )

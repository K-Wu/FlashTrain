...
class AbstractSeq2SeqDataset(Dataset):
    ...
    def make_dynamic_sampler(self, max_tokens_per_batch=1024, **kwargs):
        assert (
            FAIRSEQ_AVAILABLE
        ), "Dynamic batch size requires `pip install fairseq`"
        assert not self.used_char_len, (
            "You must call  python make_len_file.py before calling"
...
class LegacySeq2SeqDataset(AbstractSeq2SeqDataset):
    def __getitem__(self, index) -> Dict[str, torch.Tensor]:
        ...
        source_line = self.prefix + linecache.getline(
            str(self.src_file), index
        ).rstrip("\n")
...
class Seq2SeqDataCollator:
    def __init__(
        ...
        assert self.pad_token_id is not None, (
            ...
            f" ({self.tokenizer.__class__.__name__}), it must be defined."
        )
...
def save_json(content, path, indent=4, **json_dump_kwargs):
    with open(path, "w") as f:
        json.dump(
            content, f, indent=indent, sort_keys=True, **json_dump_kwargs
        )
...
def calculate_rouge(
    ...
    if bootstrap_aggregation:
        ...
        if return_precision_and_recall:
            ...
            return {
                k: round(v.mid.fmeasure * 100, 4) for k, v in result.items()
            }
...
def check_output_dir(args, expected_items=0):
    ...
    if (
        ...
        raise ValueError(
            f"Output directory ({args.output_dir}) already exists and has"
            f" {len(os.listdir(args.output_dir))} items in it (expected"
            f" {expected_items} items). Use --overwrite_output_dir to"
            " overcome."
        )

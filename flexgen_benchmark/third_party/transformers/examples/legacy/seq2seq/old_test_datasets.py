...
class TestAll(TestCasePlus):
    ...
    def test_seq2seq_dataset_truncation(self, tok_name):
        ...
        for batch in dataloader:
            ...
            assert (
                batch["decoder_input_ids"][0, -1].item()
                == tokenizer.eos_token_id
            )
            assert batch["input_ids"][0, -2].item() == tokenizer.eos_token_id
            assert (
                batch["input_ids"][0, -1].item()
                == tokenizer.lang_code_to_id[src_lang]
            )
    ...
    @pytest.mark.skipif(
        not FAIRSEQ_AVAILABLE, reason="This test requires fairseq"
    )
    ...
    def test_dataset_kwargs(self, tok_name):
        ...
        if tok_name == MBART_TINY:
            ...
            assert (
                len(kwargs) == 1 if tok_name == BART_TINY else len(kwargs) == 0
            )

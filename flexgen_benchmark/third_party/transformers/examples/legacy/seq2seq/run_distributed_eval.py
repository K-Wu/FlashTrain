...
def run_generate():
    parser = argparse.ArgumentParser(
        epilog=(
            "Unspecified args like --num_beams=2 --decoder_start_token_id=4"
    ...
    parser.add_argument(
        "--save_dir", type=str, help="where to save", default="tmp_gen"
    )
    ...
    parser.add_argument(
        ...
        required=False,
        help=(
            "How long should master process wait for other processes to"
            " finish."
        ),
    )
    ...
    if args.local_rank <= 0:
        ...
        if args.num_return_sequences > 1:
            ...
            print(
                f"Saving aggregated results at {save_path}, intermediate in"
                f" {json_save_dir}/"
            )
        ...
        print(metrics)
        write_txt_file(
            preds, save_dir.joinpath(f"{args.type_path}_generations.txt")
        )
        if args.debug:
            write_txt_file(
                labels, save_dir.joinpath(f"{args.type_path}.target")
            )
        else:

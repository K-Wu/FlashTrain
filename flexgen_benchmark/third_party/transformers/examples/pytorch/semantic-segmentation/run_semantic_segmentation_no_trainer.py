...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Path to a pretrained model or model identifier from"
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether or not to reduce all labels by 1 and replace background"
            " by 255."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether to use an authentication token to access the model"
            " repository."
        ),
    )
    parser.add_argument(
    ...
    if args.push_to_hub or args.with_tracking:
        if args.output_dir is None:
            raise ValueError(
                "Need an `output_dir` to create a repo when `--push_to_hub` or"
                " `with_tracking` is specified."
            )
...
def main():
    ...
    def preprocess_train(example_batch):
        ...
        for image, target in zip(
            example_batch["image"], example_batch["label"]
        ):
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(
            tqdm(
                eval_dataloader, disable=not accelerator.is_local_main_process
            )
        ):
            with torch.no_grad():
                outputs = model(**batch)
    ...
    if args.output_dir is not None:
        ...
        if accelerator.is_main_process:
            ...
            with open(
                ...
                json.dump(
                    {
                        "eval_overall_accuracy": eval_metrics[
                            "overall_accuracy"
                        ]
                    },
                    f,

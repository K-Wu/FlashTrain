...
def postprocess_qa_predictions(
    ...
    if len(predictions) != 2:
        raise ValueError(
            "`predictions` should be a tuple with two elements (start_logits,"
            " end_logits)."
        )
    ...
    if len(predictions[0]) != len(features):
        raise ValueError(
            f"Got {len(predictions[0])} predictions and"
    ...
    if output_dir is not None:
        ...
        prediction_file = os.path.join(
            ...
            "predictions.json"
            if prefix is None
            else f"{prefix}_predictions.json",
        )

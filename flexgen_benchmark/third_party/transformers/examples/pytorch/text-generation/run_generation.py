...
def prepare_ctrl_input(args, _, tokenizer, prompt_text):
    if args.temperature > 0.7:
        logger.info(
            "CTRL typically works better with lower temperatures (and lower"
            " top_k)."
        )

    encoded_prompt = tokenizer.encode(prompt_text, add_special_tokens=False)
    if not any(
        encoded_prompt[0] == x for x in tokenizer.control_codes.values()
    ):
        logger.info(
            "WARNING! You are not starting your generation from a control code"
...
def prepare_xlm_input(args, model, tokenizer, prompt_text):
    ...
    # Set the language
    use_lang_emb = (
        hasattr(model.config, "use_lang_emb") and model.config.use_lang_emb
    )
...
def main():
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "temperature of 1.0 has no effect, lower tend toward greedy"
            " sampling"
        ),
    )
    parser.add_argument(
    ...
    logger.warning(
        f"device: {args.device}, n_gpu: {args.n_gpu}, 16-bits training:"
        f" {args.fp16}"
    )
    ...
    try:
        ...
        raise KeyError(
            "the model {} you specified is not supported. You are welcome to"

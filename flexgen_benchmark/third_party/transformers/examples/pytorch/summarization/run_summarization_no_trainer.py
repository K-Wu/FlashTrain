...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Whether to ignore the tokens corresponding to padded labels in"
            " the loss computation or not."
        ),
    )
    parser.add_argument(
        "--max_source_length",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " tokenization. Sequences longer than this will be truncated,"
            " sequences shorter will be padded.during ``evaluate`` and"
            " ``predict``."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            " after tokenization.Sequences longer than this will be truncated,"
            " sequences shorter will be padded. Will default to"
            " `max_target_length`.This argument is also used to override the"
            " ``max_length`` param of ``model.generate``, which is used during"
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " passed to ``model.generate``, which is used during ``evaluate``"
            " and ``predict``."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If passed, pad all samples to `max_length`. Otherwise, dynamic"
            " padding is used."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If passed, will use a slow tokenizer (not backed by the 🤗"
            " Tokenizers library)."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Whether the various states should be saved at the end of every n"
    ...
    parser.add_argument(
        ...
        help=(
            ...
            ' platforms are `"tensorboard"`, `"wandb"` and `"comet_ml"`. Use'
            ' `"all"` (default) to report to all integrations.Only applicable'
    ...
    if args.push_to_hub:
        assert args.output_dir is not None, (
            "Need an `output_dir` to create a repo when `--push_to_hub` is"
            " passed."
        )
...
def main():
    ...
    if args.text_column is None:
        ...
        if text_column not in column_names:
            raise ValueError(
                f"--text_column' value '{args.text_column}' needs to be one"
    ...
    if args.summary_column is None:
        ...
        if summary_column not in column_names:
            raise ValueError(
                f"--summary_column' value '{args.summary_column}' needs to be"
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers(
            "summarization_no_trainer", experiment_config
        )
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            with torch.no_grad():
                ...
                decoded_preds, decoded_labels = accelerator.gather_for_metrics(
                    decoded_preds, decoded_labels
                )
                metric.add_batch(

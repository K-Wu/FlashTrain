...
class DataTrainingArguments:
    ...
    test_file: Optional[str] = field(
        ...
        metadata={
            "help": (
                "An optional input test data file to evaluate the metrics"
    ...
    pad_to_max_length: bool = field(
        ...
        metadata={
            "help": (
                ...
                " the maximum length in the batch. More efficient on GPU but"
                " very bad for TPU."
            )
        },
    )
    ...
    source_prefix: Optional[str] = field(
        ...
        metadata={
            "help": (
                "A prefix to add before every source text (useful for T5"
                " models)."
            )
        },
    )
    ...
    forced_bos_token: Optional[str] = field(
        ...
        metadata={
            "help": (
                ...
                " mBART where the first generated tokenneeds to be the target"
                " language token (Usually it is the target language token)"
            )
        },
    )
...
def main():
    ...
    if data_args.source_prefix is None and model_args.model_name_or_path in [
        ...
        logger.warning(
            ...
            " which is the expected, e.g. with `--source_prefix"
            " 'summarize: ' `"
        )
    ...
    if (
        ...
        if model_args.resize_position_embeddings is None:
            logger.warning(
                "Increasing the model's number of position embedding vectors"
                f" from {model.config.max_position_embeddings} to"
            ...
            raise ValueError(
                ...
                f" {data_args.max_source_length}, but the model only has"
                f" {model.config.max_position_embeddings} position encodings."
                " Consider either reducing `--max_source_length` to"
                f" {model.config.max_position_embeddings} or to automatically"
                " resize the model's position encodings by passing"
    ...
    if data_args.text_column is None:
        ...
        if text_column not in column_names:
            raise ValueError(
                f"--text_column' value '{data_args.text_column}' needs to be"
    ...
    if data_args.summary_column is None:
        ...
        if summary_column not in column_names:
            raise ValueError(
                f"--summary_column' value '{data_args.summary_column}' needs"

...
def random_subsample(
    wav: np.ndarray, max_length: float, sample_rate: int = 16000
):
...
class DataTrainingArguments:
    ...
    max_length_seconds: float = field(
        ...
        metadata={
            "help": (
                "Audio clips will be randomly cut to this length during"
                " training if the value is set."
            )
        },
    )
...
class ModelArguments:
    ...
    attention_mask: bool = field(
        ...
        metadata={
            "help": (
                "Whether to generate an attention mask in the feature"
                " extractor."
            )
        },
    )
    ...
    ignore_mismatched_sizes: bool = field(
        ...
        metadata={
            "help": (
                "Will enable to load a pretrained model whose head dimensions"
                " are different."
            )
        },
    )
    ...
    def __post_init__(self):
        if not self.freeze_feature_extractor and self.freeze_feature_encoder:
            warnings.warn(
                (
                    "The argument `--freeze_feature_extractor` is deprecated"
                    " and will be removed in a future version. Use"
                    " `--freeze_feature_encoder`instead. Setting"
        ...
        if self.freeze_feature_extractor and not self.freeze_feature_encoder:
            raise ValueError(
                ...
                " `--freeze_feature_encoder`.Only make use of"
...
def main():
    ...
    if (
        ...
        if (
            ...
            raise ValueError(
                ...
                " and is not empty. Use --overwrite_output_dir to train from"
                " scratch."
            )
        elif (
    ...
    if data_args.label_column_name not in raw_datasets["train"].column_names:
        raise ValueError(
            f"--label_column_name {data_args.label_column_name} not found in"
            f" dataset '{data_args.dataset_name}'. Make sure to set"
            " `--label_column_name` to the correct text column - one of"
            f" {', '.join(raw_datasets['train'].column_names)}."
        )
    ...
    def train_transforms(batch):
        ...
        output_batch["labels"] = [
            label for label in batch[data_args.label_column_name]
        ]

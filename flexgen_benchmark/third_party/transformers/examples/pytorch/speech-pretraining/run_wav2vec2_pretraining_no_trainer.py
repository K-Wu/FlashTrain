...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "The configuration names of the dataset to use (via the datasets"
            " library)."
        ),
    )
    parser.add_argument(
        ...
        required=True,
        help=(
            "The names of the training data set splits to use (via the"
            " datasets library)."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Percentage of training data that should be used for validation if"
            " no validation is present in dataset."
        ),
    )
    parser.add_argument(
        "--logging_steps",
    ...
    parser.add_argument(
        ...
        default="audio",
        help=(
            "Column in the dataset that contains speech file path. Defaults to"
            " 'audio'"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=5.0,
        help=(
            "Filter out audio files that are longer than"
    ...
    parser.add_argument(
        ...
        default=3.0,
        help=(
            "Filter out audio files that are shorter than"
...
def main():
    ...
    if not feature_extractor.do_normalize:
        raise ValueError(
            "Training is only supported for normalized inputs. Make sure"
            " ``feature_extractor.do_normalize == True``"
        )

    # set max & min audio length in number of samples
    max_length = int(
        args.max_duration_in_seconds * feature_extractor.sampling_rate
    )
    min_length = int(
        args.min_duration_in_seconds * feature_extractor.sampling_rate
    )
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.state.num_processes > 1:
                ...
                gradient_multiplier = (
                    accelerator.state.num_processes / num_losses
                )
            ...
            if (
                ...
                scale = (
                    ...
                    and accelerator.scaler is not None
                    else 1
                )
                if accelerator.state.num_processes > 1:
            ...
            if (step + 1) % (
                args.gradient_accumulation_steps * args.saving_steps
            ) == 0:
                if (
        ...
        val_logs = {
            k: v / val_logs["val_num_losses"] for k, v in val_logs.items()
        }

...
class GetFromCacheTests(unittest.TestCase):
    def test_cached_file(self):
        ...
        self.assertEqual(
            archive_file,
            os.path.join(CACHE_DIR, "snapshots", main_commit, CONFIG_NAME),
        )
        ...
        archive_file = cached_file(
            RANDOM_BERT, CONFIG_NAME, revision="9b8c223"
        )
        self.assertEqual(
            archive_file,
            os.path.join(
                CACHE_DIR, "snapshots", FULL_COMMIT_HASH, CONFIG_NAME
            ),
        )
    ...
    def test_cached_file_errors(self):
        ...
        with self.assertRaisesRegex(
            EnvironmentError, "is not a valid git identifier"
        ):
    ...
    def test_non_existence_is_cached(self):
        ...
        self.assertTrue(
            os.path.isfile(
                os.path.join(CACHE_DIR, ".no_exist", main_commit, "conf")
            )
        )
    ...
    def test_has_file(self):
        self.assertTrue(
            has_file("hf-internal-testing/tiny-bert-pt-only", WEIGHTS_NAME)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_file(
                "hf-internal-testing/tiny-bert-pt-only", FLAX_WEIGHTS_NAME
            )
        )
    ...
    def test_get_file_from_repo_distant(self):
        ...
        with self.assertRaisesRegex(
            ...
            get_file_from_repo(
                "bert-base-cased", CONFIG_NAME, revision="ahaha"
            )
    ...
    def test_get_file_from_repo_local(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            filename.touch()
            self.assertEqual(
                get_file_from_repo(tmp_dir, "a.txt"), str(filename)
            )

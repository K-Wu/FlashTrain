...
class GenericTester(unittest.TestCase):
    ...
    def test_transpose_torch(self):
        ...
        self.assertTrue(
            np.allclose(
                transpose(x, axes=(1, 2, 0)),
                transpose(t, axes=(1, 2, 0)).numpy(),
            )
        )
    ...
    def test_transpose_flax(self):
        ...
        self.assertTrue(
            np.allclose(
                transpose(x, axes=(1, 2, 0)),
                np.asarray(transpose(t, axes=(1, 2, 0))),
            )
        )
    ...
    def test_reshape_numpy(self):
        ...
        x = np.random.randn(3, 4, 5)
        self.assertTrue(
            np.allclose(reshape(x, (12, 5)), np.reshape(x, (12, 5)))
        )
    ...
    def test_reshape_torch(self):
        ...
        t = torch.tensor(x)
        self.assertTrue(
            np.allclose(reshape(x, (4, 3)), reshape(t, (4, 3)).numpy())
        )

        x = np.random.randn(3, 4, 5)
        t = torch.tensor(x)
        self.assertTrue(
            np.allclose(reshape(x, (12, 5)), reshape(t, (12, 5)).numpy())
        )
    ...
    def test_reshape_flax(self):
        ...
        t = jnp.array(x)
        self.assertTrue(
            np.allclose(reshape(x, (4, 3)), np.asarray(reshape(t, (4, 3))))
        )
    ...
    def test_squeeze_torch(self):
        ...
        t = torch.tensor(x)
        self.assertTrue(
            np.allclose(squeeze(x, axis=2), squeeze(t, axis=2).numpy())
        )
    ...
    def test_squeeze_flax(self):
        ...
        t = jnp.array(x)
        self.assertTrue(
            np.allclose(squeeze(x, axis=2), np.asarray(squeeze(t, axis=2)))
        )

    def test_expand_dims_numpy(self):
        x = np.random.randn(3, 4)
        self.assertTrue(
            np.allclose(expand_dims(x, axis=1), np.expand_dims(x, axis=1))
        )
    ...
    def test_expand_dims_flax(self):
        ...
        self.assertTrue(
            np.allclose(
                expand_dims(x, axis=1), np.asarray(expand_dims(t, axis=1))
            )
        )

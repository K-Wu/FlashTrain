...
class TestAddNewModelLike(unittest.TestCase):
    ...
    def test_re_class_func(self):
        self.assertEqual(
            _re_class_func.search("def my_function(x, y):").groups()[0],
            "my_function",
        )
        self.assertEqual(
            _re_class_func.search("class MyClass:").groups()[0], "MyClass"
        )
        self.assertEqual(
            _re_class_func.search("class MyClass(SuperClass):").groups()[0],
            "MyClass",
        )
...
"""
    ...
    "t5": "T5Config",
        ...
        self.assertEqual(
            add_content_to_text(
                test_text, line, add_before="bert", exact_match=True
            ),
            test_text,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            add_content_to_text(
                test_text, line, add_before=re.compile('^\s*"bert":')
            ),
            expected,
        )
        ...
        self.assertEqual(
            add_content_to_text(
                test_text, line, add_after="gpt", exact_match=True
            ),
            test_text,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            add_content_to_text(
                test_text, line, add_after=re.compile('^\s*"gpt":')
            ),
            expected,
        )
    ...
    "t5": "T5Config",
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.init_file(file_name, test_text)
            add_content_to_file(
                file_name, line, add_before="bert", exact_match=True
            )
            ...
            self.init_file(file_name, test_text)
            add_content_to_file(
                file_name, line, add_before=re.compile('^\s*"bert":')
            )
            self.check_result(file_name, expected)
            ...
            self.init_file(file_name, test_text)
            add_content_to_file(
                file_name, line, add_after="gpt", exact_match=True
            )
            self.check_result(file_name, test_text)
            ...
            self.init_file(file_name, test_text)
            add_content_to_file(
                file_name, line, add_after=re.compile('^\s*"gpt":')
            )
            self.check_result(file_name, expected)
    ...
    def test_replace_model_patterns(self):
        ...
        new_bert_model_patterns = ModelPatterns(
            "New Bert", "huggingface/bert-new-base"
        )
...
'''
        ...
        bert_expected = bert_expected.replace(
            '    model_type = "new-bert"\n', ""
        )
        bert_converted, replacements = replace_model_patterns(
            bert_test, bert_model_patterns, new_bert_model_patterns
        ...
        self.assertEqual(bert_converted, bert_expected)
        self.assertEqual(
            replacements, "BERT->NEW_BERT,Bert->NewBert,bert->new_bert"
        )
...
'''
        bert_expected_with_copied_from = (
            "# Copied from transformers.bert_module.TFBertPreTrainedModel with"
            " Bert->NewBert,bert->new_bert\n"
            + bert_expected
        )
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            duplicate_module(
                file_name, bert_model_patterns, new_bert_model_patterns
            )
...
'''
    ...
    def test_filter_framework_files(self):
        ...
        self.assertEqual(
            set(filter_framework_files(files, ["pt", "tf", "flax"])),
            set(files),
        )
    ...
    def test_get_model_files(self):
        ...
        model_files = {
            ...
            for f in bert_files["model_files"]
        }
        ...
        test_files = {
            str(Path(f).relative_to(REPO_PATH))
            for f in bert_files["test_files"]
        }
        bert_test_files = {
        ...
        model_files = {
            str(Path(f).relative_to(REPO_PATH))
            for f in vit_files["model_files"]
        }
        ...
        model_files = {
            str(Path(f).relative_to(REPO_PATH))
            for f in wav2vec2_files["model_files"]
        }
        ...
        test_files = {
            str(Path(f).relative_to(REPO_PATH))
            for f in wav2vec2_files["test_files"]
        }
        wav2vec2_test_files = {
    ...
    def test_find_base_model_checkpoint(self):
        self.assertEqual(
            find_base_model_checkpoint("bert"), "bert-base-uncased"
        )
        self.assertEqual(find_base_model_checkpoint("gpt2"), "gpt2")

    def test_retrieve_model_classes(self):
        gpt_classes = {
            k: set(v) for k, v in retrieve_model_classes("gpt2").items()
        }
        ...
        gpt_classes = {
            k: set(v)
            for k, v in retrieve_model_classes(
                "gpt2", frameworks=["pt", "tf"]
            ).items()
        }
        self.assertEqual(gpt_classes, expected_gpt_classes)
    ...
    def test_retrieve_info_for_model_with_bert(self):
        ...
        model_classes = {
            k: set(v) for k, v in bert_info["model_classes"].items()
        }
        ...
        model_files = {
            str(Path(f).relative_to(REPO_PATH))
            for f in all_bert_files["model_files"]
        }
        self.assertEqual(model_files, BERT_MODEL_FILES)

        test_files = {
            str(Path(f).relative_to(REPO_PATH))
            for f in all_bert_files["test_files"]
        }
        bert_test_files = {
            "tests/test_tokenization_bert.py",
            "tests/test_modeling_bert.py",
    ...
    def test_retrieve_info_for_model_with_vit(self):
        ...
        model_classes = {
            k: set(v) for k, v in vit_info["model_classes"].items()
        }
        self.assertEqual(model_classes, expected_model_classes)
        ...
        model_files = {
            str(Path(f).relative_to(REPO_PATH))
            for f in all_vit_files["model_files"]
        }
        self.assertEqual(model_files, VIT_MODEL_FILES)

        test_files = {
            str(Path(f).relative_to(REPO_PATH))
            for f in all_vit_files["test_files"]
        }
        vit_test_files = {
            "tests/test_feature_extraction_vit.py",
            "tests/test_modeling_vit.py",
        ...
        self.assertEqual(
            vit_model_patterns.checkpoint, "google/vit-base-patch16-224"
        )
    ...
    def test_retrieve_info_for_model_with_wav2vec2(self):
        ...
        self.assertEqual(
            set(wav2vec2_info["frameworks"]), {"pt", "tf", "flax"}
        )
        model_classes = {
            k: set(v) for k, v in wav2vec2_info["model_classes"].items()
        }
        self.assertEqual(model_classes, expected_model_classes)
        ...
        doc_file = str(
            Path(all_wav2vec2_files["doc_file"]).relative_to(REPO_PATH)
        )
        self.assertEqual(doc_file, "docs/source/model_doc/wav2vec2.mdx")
        ...
        self.assertEqual(
            wav2vec2_model_patterns.model_upper_cased, "WAV_2_VEC_2"
        )
        self.assertEqual(
            wav2vec2_model_patterns.config_class, "Wav2Vec2Config"
        )
        self.assertEqual(
            wav2vec2_model_patterns.feature_extractor_class,
            "Wav2Vec2FeatureExtractor",
        )
        self.assertEqual(
            wav2vec2_model_patterns.processor_class, "Wav2Vec2Processor"
        )
        self.assertEqual(
...
"""
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            duplicate_doc_file(
                doc_file, gpt2_model_patterns, new_model_patterns
            )

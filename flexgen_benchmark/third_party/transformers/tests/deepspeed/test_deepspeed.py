...
def get_launcher(distributed=False):
    ...
    return (
        f"deepspeed --num_nodes 1 --num_gpus {num_gpus} --master_port"
        f" {master_port}".split()
    )
...
def parameterized_custom_name_func(func, param_num, param):
    ...
    param_based_name = parameterized.to_safe_name(
...
class TrainerIntegrationDeepSpeed(
    ...
    def test_hf_scheduler_hf_optimizer(self):
        ...
        with mockenv_context(**self.dist_env_1_gpu):
            ...
            del ds_config_zero2_dict[
                "optimizer"
            ]  # force default HF Trainer optimizer
            del ds_config_zero2_dict[
                "scheduler"
    ...
    def test_gradient_accumulation(self, stage, dtype):
        ...
        self.assertAlmostEqual(
            no_grad_accum_loss, yes_grad_accum_loss, places=2
        )

    def check_saved_checkpoints_deepspeed(
        self, output_dir, freq, total, stage, dtype
    ):
        ...
        for step in range(freq, total, freq):
            ...
            self.assertTrue(
                ...
                f"[{stage}] {checkpoint} dir is not found",
            )
            ...
            for filename in file_list:
                ...
                self.assertTrue(
                    os.path.isfile(path), f"[{stage}] {path} is not found"
                )
    ...
    def test_save_checkpoints(self, stage, dtype):
        ...
        self.check_saved_checkpoints_deepspeed(
            output_dir, freq, total, stage, dtype
        )
    ...
    def test_config_object(self):
        ...
        with mockenv_context(**self.dist_env_1_gpu):
            trainer = get_regression_trainer(
                deepspeed=ds_config_zero3_dict, **kwargs
            )
            self.assertTrue(is_deepspeed_zero3_enabled())
            ...
            trainer = get_regression_trainer(
                deepspeed=ds_config_zero2_dict, **kwargs
            )
            self.assertFalse(is_deepspeed_zero3_enabled())
            ...
            self.assertTrue(
                bool(config), "Deepspeed config should be accessible"
            )
            ...
            self.assertFalse(is_deepspeed_zero3_enabled())
            self.assertFalse(
                bool(config), "Deepspeed config should not be accessible"
            )
    ...
    def test_load_best_model(self, stage, dtype):
        ...
        with mockenv_context(**self.dist_env_1_gpu):
            ...
            def _add_eos_to_examples(example):
                example["input_text"] = (
                    f"question: {example['question']}  context:"
                    f" {example['context']}"
                )
                example["target_text"] = (
...
class TestDeepSpeedWithLauncher(TestCasePlus):
    ...
    def do_checks(
        self, output_dir, do_train=True, do_eval=True, quality_checks=True
    ):
        if do_train:
            ...
            if quality_checks:
                self.assertGreater(
                    train_metrics["train_samples_per_second"], 0.5
                )

        if do_eval:
            eval_metrics = load_json(
    ...
    def run_trainer(
        ...
        assert (
            actions > 0
        ), "need at least do_train or do_eval for the test to run"
        ...
        ds_args = (
            f"--deepspeed {self.test_file_dir_str}/ds_config_{stage}.json"
            .split()
        )
        script = [
    ...
    def test_clm_from_config_zero3_fp16(self):
        ...
        ds_args = (
            f"--deepspeed {self.test_file_dir_str}/ds_config_zero3.json".split()
        )
        script = [
            f"{self.examples_dir_str}/pytorch/language-modeling/run_clm.py"
        ]

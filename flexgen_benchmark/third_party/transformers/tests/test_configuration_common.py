...
class ConfigTester(object):
    def __init__(
        self, parent, config_class=None, has_text_modality=True, **kwargs
    ):
        self.parent = parent
        ...
        for idx, name in enumerate(common_properties):
            try:
                ...
                self.parent.assertEqual(
                    ...
                    idx,
                    msg=(
                        f"`{name} value {idx} expected, but was"
    ...
    def create_and_test_config_to_json_file(self):
        ...
        self.parent.assertEqual(
            config_second.to_dict(), config_first.to_dict()
        )
    ...
    def check_config_arguments_init(self):
        ...
        if len(wrong_values) > 0:
            ...
            raise ValueError(
                "The following keys were not properly set in the"
                f" config:\n{errors}"
            )
...
class ConfigPushToHubTester(unittest.TestCase):
    ...
    def test_push_to_hub_in_organization(self):
        ...
        config.push_to_hub(
            "valid_org/test-config-org", use_auth_token=self._token
        )
...
class ConfigTestUtils(unittest.TestCase):
    def test_config_from_string(self):
        ...
        self.assertEqual(
            resid_pdrop, c.resid_pdrop, "mismatch for key: resid_pdrop"
        )
        self.assertEqual(
            scale_attn_weights,
        ...
        self.assertEqual(
            summary_type, c.summary_type, "mismatch for key: summary_type"
        )
    ...
    def test_config_common_kwargs_is_complete(self):
        ...
        if len(keys_with_defaults) > 0:
            raise ValueError(
                ...
                " `test_configuration_common.config_common_kwargs` pick"
                f" another value for them: {', '.join(keys_with_defaults)}."
            )
...
class ConfigurationVersioningTest(unittest.TestCase):
    ...
    def test_repo_versioning_before(self):
        ...
        old_configuration = (
            old_transformers.models.auto.AutoConfig.from_pretrained(repo)
        )

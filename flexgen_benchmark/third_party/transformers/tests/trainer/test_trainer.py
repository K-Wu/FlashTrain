...
class DynamicShapesDataset:
    def __init__(self, length=64, seed=42, batch_size=8):
        ...
        self.xs = [
            np.random.normal(size=(s,)) for s in sizes.repeat(batch_size)
        ]
        self.ys = [
            np.random.normal(size=(s,)) for s in sizes.repeat(batch_size)
        ]
...
class TrainerIntegrationCommon:
    def check_saved_checkpoints(
        self, output_dir, freq, total, is_pretrained=True
    ):
        file_list = [
            WEIGHTS_NAME,
            "training_args.bin",
        ...
        for step in range(freq, total, freq):
            ...
            for filename in file_list:
                self.assertTrue(
                    os.path.isfile(os.path.join(checkpoint, filename))
                )
    ...
    def convert_to_sharded_checkpoint(self, folder):
        ...
        shard_files = [
            WEIGHTS_NAME.replace(
                ".bin", f"-{idx+1:05d}-of-{len(keys):05d}.bin"
            )
        ...
        for param_name, shard_file in zip(keys, shard_files):
            torch.save(
                ...
                os.path.join(folder, shard_file),
            )
...
class TrainerIntegrationTest(TestCasePlus, TrainerIntegrationCommon):
    ...
    def test_number_of_steps_in_training(self):
        ...
        self.assertEqual(
            train_output.global_step, self.n_epochs * 64 / self.batch_size
        )
        ...
        train_output = trainer.train()
        self.assertEqual(
            train_output.global_step, int(1.5 * 64 / self.batch_size)
        )
    ...
    def test_number_of_steps_in_training_with_ipex(self):
        for mix_bf16 in [True, False]:
            ...
            self.assertEqual(
                train_output.global_step,
                int(1.5 * 64 / trainer.args.train_batch_size),
            )
    ...
    def test_train_and_eval_dataloaders(self):
        ...
        self.assertEqual(
            len(trainer.get_train_dataloader()), 66 // (16 * n_gpu) + 1
        )
        self.assertEqual(
            len(trainer.get_eval_dataloader()), 74 // (32 * n_gpu) + 1
        )
        ...
        self.assertEqual(
            len(trainer.get_train_dataloader()), 66 // (16 * n_gpu)
        )
        self.assertEqual(
            len(trainer.get_eval_dataloader()), 74 // (32 * n_gpu)
        )

        # Check passing a new dataset for evaluation works
        new_eval_dataset = RegressionDataset(length=128)
        self.assertEqual(
            len(trainer.get_eval_dataloader(new_eval_dataset)),
            128 // (32 * n_gpu),
        )
    ...
    def test_sampler_seed(self):
        ...
        for group_by_length in [True, False]:
            ...
            self.assertTrue(
                any(
                    not torch.equal(
                        sample42_1["input_ids"], sample["input_ids"]
                    )
    ...
    def test_predict_with_jit(self):
        ...
        # With a number of elements not a round multiple of the batch size
        trainer = get_regression_trainer(
            a=1.5, b=2.5, eval_len=66, jit_mode_eval=True
        )
        preds = trainer.predict(trainer.eval_dataset).predictions
        x = trainer.eval_dataset.x
        self.assertTrue(np.allclose(preds, 1.5 * x + 2.5))
    ...
    def test_predict_with_ipex(self):
        for mix_bf16 in [True, False]:
            ...
            self.assertTrue(np.allclose(preds[1], 1.5 * x + 2.5))
            self.assertTrue(
                np.array_equal(labels[0], trainer.eval_dataset.ys[0])
            )
            self.assertTrue(
                np.array_equal(labels[1], trainer.eval_dataset.ys[1])
            )
    ...
    def test_dynamic_shapes(self):
        ...
        for expected, seen in zip(eval_dataset.ys, preds.label_ids):
            self.assertTrue(
                np.array_equal(expected, seen[: expected.shape[0]])
            )
    ...
    def test_resume_training_with_randomness(self):
        ...
        with self.subTest("Test every step"):
            ...
            args = RegressionTrainingArguments(
                tmp_dir, save_steps=5, learning_rate=0.1
            )
            trainer = Trainer(
                model,
                args,
            ...
            trainer.train(
                resume_from_checkpoint=os.path.join(tmp_dir, "checkpoint-15")
            )
            (a1, b1) = trainer.model.a.item(), trainer.model.b.item()
        ...
        with self.subTest("Test every epoch"):
            ...
            trainer.train(
                resume_from_checkpoint=os.path.join(tmp_dir, checkpoint_dir)
            )
            (a1, b1) = trainer.model.a.item(), trainer.model.b.item()
    ...
    def test_training_iterable_dataset(self):
        ...
        args = RegressionTrainingArguments(
            output_dir="./examples", max_steps=4
        )
    ...
    def test_training_finite_iterable_dataset(self):
        ...
        with self.assertLogs("transformers.trainer", level="WARNING") as logs:
            trainer.train()
        self.assertIn(
            f"stopping training at step {available_steps}!", logs.output[0]
        )
    ...
    def test_evaluation_iterable_dataset(self):
        ...
        x, y = (
            trainer.eval_dataset.dataset.x,
            trainer.eval_dataset.dataset.ys[0],
        )
        pred = 1.5 * x + 2.5
        expected_loss = ((pred - y) ** 2).mean()
        self.assertAlmostEqual(results["eval_loss"], expected_loss)
    ...
    def test_flos_extraction(self):
        ...
        def assert_flos_extraction(trainer, wrapped_model_to_check):
            self.assertEqual(
                trainer.model, unwrap_model(wrapped_model_to_check)
            )
            self.assertGreaterEqual(
                getattr(
                    unwrap_model(wrapped_model_to_check).config,
                    "total_flos",
                    0,
                ),
                0,
    ...
    def check_checkpoint_deletion(self, trainer, output_dir, expected):
        ...
        for n in [5, 10, 15, 20, 25]:
            os.makedirs(
                os.path.join(output_dir, f"{PREFIX_CHECKPOINT_DIR}-{n}"),
                exist_ok=True,
            )
    ...
    def test_checkpoint_rotation(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            trainer = get_regression_trainer(
                output_dir=tmp_dir, save_total_limit=2
            )
            ...
            trainer.state.best_model_checkpoint = os.path.join(
                tmp_dir, "checkpoint-5"
            )
    ...
    def test_no_wd_param_group(self):
        ...
        no_wd_params = [
            p for n, p in model.named_parameters() if n not in wd_names
        ]
...
class TrainerOptimizerChoiceTest(unittest.TestCase):
    ...
    def test_bnb_adam8bit_no_bnb(self):
        args = TrainingArguments(
            optim=OptimizerNames.ADAMW_BNB, output_dir="None"
        )

...
class TrainerUtilsTest(unittest.TestCase):
    ...
    def test_distributed_length_grouped(self):
        ...
        indices_process_0 = list(
            DistributedLengthGroupedSampler(
                4, num_replicas=2, rank=0, lengths=lengths
            )
        )
        indices_process_1 = list(
            DistributedLengthGroupedSampler(
                4, num_replicas=2, rank=1, lengths=lengths
            )
        )
        # The biggest element should be first
        ...
        # The indices should be a permutation of range(100)
        self.assertEqual(
            list(sorted(indices_process_0 + indices_process_1)),
            list(range(100)),
        )
    ...
    def test_iterable_dataset_shard_with_length(self):
        ...
        self.assertListEqual(
            [list(shard) for shard in sampler_shards], expected_shards
        )
        self.assertListEqual(
    ...
    def test_shard_sampler(self):
        for n_elements in [64, 123]:
            ...
            self.check_shard_sampler(
                dataset, 4, drop_last=True, num_processes=2
            )
            self.check_shard_sampler(
                dataset, 4, drop_last=False, num_processes=2
            )

            self.check_shard_sampler(
                dataset, 4, drop_last=True, num_processes=3
            )
            self.check_shard_sampler(
    ...
    def test_executable_batch_size_no_search(self):
        ...
        @find_executable_batch_size(
            starting_batch_size=64, auto_find_batch_size=False
        )
        def mock_training_loop_function(batch_size):
            nonlocal batch_sizes
            batch_sizes.append(batch_size)

...
def merge_model_tokenizer_mappings(
    ...
    tokenizer_mapping: Dict[
        "PretrainedConfig",
        Tuple["PreTrainedTokenizer", "PreTrainedTokenizerFast"],
    ],
) -> Dict[
    ...
    for configuration in configurations:
        if (
            configuration in model_mapping
            and configuration in tokenizer_mapping
        ):
...
class TokenizerTesterMixin:
    ...
    def get_clean_sequence(
        ...
        toks = list(
            filter(
                lambda t: [t[0]]
                == tokenizer.encode(t[1], add_special_tokens=False),
                toks,
            )
        )
        ...
        if " " not in output_txt and len(toks_ids) > 1:
            output_txt = (
                ...
                + " "
                + tokenizer.decode(
                    toks_ids[1:], clean_up_tokenization_spaces=False
                )
            )
    ...
    def get_tokenizers(
        ...
        if fast and self.test_rust_tokenizer and self.test_slow_tokenizer:
            ...
            raise ValueError(
                "This tokenizer class has no tokenizer to be tested."
            )
    ...
    def tokenizer_integration_test_util(
        ...
        if sequences is None:
            sequences = [
                (
                    ...
                    " pytorch-pretrained-bert) provides general-purpose"
                    " architectures (BERT, GPT-2, RoBERTa, XLM, DistilBert,"
                    " XLNet...) for Natural Language Understanding (NLU) and"
                    " Natural Language Generation (NLG) with over 32+"
                    " pretrained models in 100+ languages and deep"
                    " interoperability between Jax, PyTorch and TensorFlow."
                ),
                (
        ...
        for tokenizer_class in tokenizer_classes:
            ...
            decoded_sequences = [
                tokenizer.decode(
                    seq, skip_special_tokens=True, **decode_kwargs
                )
    ...
    def test_model_input_names_signature(self):
        ...
        for tokenizer in tokenizers:
            ...
            self.assertTrue(
                ...
                in accepted_model_main_input_names
            )
    ...
    def test_added_tokens_do_lower_case(self):
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                if (
                    hasattr(tokenizer, "do_lower_case")
                    and tokenizer.do_lower_case
                ):
                    continue
    ...
    def test_pretrained_model_lists(self):
        ...
        self.assertGreaterEqual(
            len(
                list(self.tokenizer_class.pretrained_vocab_files_map.values())[
                    0
                ]
            ),
            1,
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                    ...
                    with self.subTest(f"Padding: {padding_state}"):
                        ...
                        self.assertTrue(
                            cm.records[0].message.startswith(
                                ...
                                " the specified maximum sequence length for"
                                " this model"
                            )
                        )
                ...
                if isinstance(tokenizer, PreTrainedTokenizerFast):
                    ...
                    self.assertEqual(
                        overflowing_tokens, sequence[-(2 + stride) :]
                    )
                else:
    ...
    def test_maximum_encoding_length_pair_input(self):
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                for padding_state in padding_strategies:
                    with self.subTest(
                        f"{tokenizer.__class__.__name__} Padding:"
                        f" {padding_state}"
                    ):
                        for truncation_state in [
                            True,
                            ...
                            with self.subTest(
                                f"{tokenizer.__class__.__name__} Truncation:"
                                f" {truncation_state}"
                            ):
                                output = tokenizer(
                                    seq_2,
                ...
                if isinstance(tokenizer, PreTrainedTokenizerFast):
                    ...
                    self.assertEqual(
                        len(truncated_sequence), len(sequence) - 2
                    )
                    self.assertEqual(
                        truncated_sequence, truncated_longest_sequence
                    )
                    ...
                    self.assertEqual(
                        overflowing_tokens, overflow_longest_sequence
                    )
                else:
                    ...
                    self.assertTrue(
                        context.exception.args[0].startswith(
                            "Not possible to return overflowing tokens for"
                            " pair of sequences with the `longest_first`."
                            " Please select another truncation strategy than"
                            " `longest_first`, for instance `only_second` or"
                            " `only_first`."
                        )
                    )
                ...
                if isinstance(tokenizer, PreTrainedTokenizerFast):
                    ...
                    self.assertEqual(len(overflowing_tokens), 2 + stride)
                    self.assertEqual(
                        overflowing_tokens, seq0_tokens[-(2 + stride) :]
                    )
                ...
                # Overflowing tokens are handled quite differently in slow and fast tokenizers
                if isinstance(tokenizer, PreTrainedTokenizerFast):
                    truncated_sequence = information_second_truncated[
                        "input_ids"
                    ][0]
                    ...
                    self.assertEqual(len(overflowing_tokens), 2 + stride)
                    self.assertEqual(
                        overflowing_tokens, seq1_tokens[-(2 + stride) :]
                    )
    ...
    def test_right_and_left_padding(self):
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                self.assertEqual(
                    [padding_idx] * padding_size + encoded_sequence,
                    padded_sequence,
                )
    ...
    def test_encode_plus_with_padding(self):
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                self.assertEqual(
                    sequence_length + padding_size,
                    right_padded_sequence_length,
                )
                self.assertEqual(
                    input_ids + [padding_idx] * padding_size,
                ...
                self.assertEqual(
                    [padding_idx] * padding_size + input_ids,
    ...
    def test_padding_warning_message_fast_tokenizer(self):
        ...
        self.assertIn(
            (
                ...
                " followed by a call to the `pad` method to get a padded"
                " encoding."
            ),
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                sequences = [
                    ...
                    (
                        "Testing batch encode plus with different sequence"
                        " lengths"
                    ),
                    (
                        "Testing batch encode plus with different sequence"
                        " lengths correctly pads"
                    ),
                ]
    ...
    def test_batch_encode_plus_overflowing_tokens(self):
        ...
        for tokenizer in tokenizers:
            string_sequences = [
                "Testing the prepare_for_model method.",
                "Test",
            ]
            ...
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                if tokenizer.pad_token_id is None:
                    ...
                    for key in encoded_sequences.keys():
                        ...
                        tensorflow_value = (
                            tensorflow_tensor[key].numpy().tolist()
                        )
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                if batch_encoded_sequence is None:
                    raise ValueError(
                        "Cannot convert list to numpy tensor on "
                        " batch_encode_plus()"
                    )
                ...
                if self.test_rust_tokenizer:
                    ...
                    batch_encoded_sequence_fast = (
                        fast_tokenizer.batch_encode_plus(
                            [sequence, sequence], return_tensors="np"
                        )
                    )
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                src_text = [
                    ...
                    (
                        " Secretary-General Ban Ki-moon says his response to"
                        " Russia's stepped up military support for Syria is"
                        " that 'there is no military solution' to the nearly"
                        " five-year conflict and more weapons will only worsen"
                        " the violence and misery for millions of people."
                    ),
                ]
                tgt_text = [
                    (
                        "Şeful ONU declară că nu există o soluţie militară în"
                        " Siria"
                    ),
                    (
                        "Secretarul General Ban Ki-moon declară că răspunsul"
                        " său la intensificarea sprijinului militar al Rusiei"
                        ' pentru Siria este că "nu există o soluţie militară"'
                        " la conflictul de aproape cinci ani şi că noi arme nu"
                        " vor face decât să înrăutăţească violenţele şi"
                        " mizeria pentru milioane de oameni."
                    ),
                ]
                try:
    ...
    def test_alignement_methods(self):
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(
                ...
                self.assertEqual(
                    max(batch_encoding.words(last_batch_index)),
                    last_word_index,
                )
                self.assertEqual(
                ...
                self.assertEqual(
                    encoding.token_to_word(0, last_token_index),
                    last_word_index,
                )
                self.assertEqual(batch_encoding.token_to_word(1, 0), 0)
                self.assertEqual(
                    batch_encoding.token_to_word(0, last_token_index),
                    last_word_index,
                )
                self.assertEqual(
                    batch_encoding.token_to_word(
                ...
                self.assertEqual(
                    encoding.word_to_tokens(last_word_index).end,
                    last_token_index + 1,
                )
                self.assertEqual(
                ...
                self.assertEqual(
                    encoding.token_to_chars(last_token_index).end,
                    last_char_index + 1,
                )
                self.assertEqual(
                ...
                self.assertEqual(
                    encoding.char_to_token(0, last_char_index),
                    last_token_index,
                )
                self.assertEqual(batch_encoding.char_to_token(1, 0), 0)
                self.assertEqual(
                    batch_encoding.char_to_token(0, last_char_index),
                    last_token_index,
                )
                self.assertEqual(
                    batch_encoding.char_to_token(
                        last_batch_index, last_char_index
                    ),
                    last_token_index,
                )
                ...
                self.assertEqual(
                    batch_encoding.char_to_word(0, last_char_index),
                    last_word_index,
                )
                self.assertEqual(
                    batch_encoding.char_to_word(
                        last_batch_index, last_char_index
                    ),
                    last_word_index,
                )
                ...
                self.assertEqual(
                    encoding.word_to_chars(last_word_index).end,
                    last_char_index + 1,
                )
                self.assertEqual(
                    encoding.word_to_chars(0, last_word_index).end,
                    last_char_index + 1,
                )
                ...
                self.assertEqual(
                    batch_encoding.token_to_sequence(
                        last_batch_index, num_tokens // 2
                    ),
                    0,
                )
    ...
    def test_tokenization_python_rust_equals(self):
        ...
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(
                ...
                for key in filter(
                    lambda x: x
                    in ["input_ids", "token_type_ids", "attention_mask"],
                    input_p.keys(),
                ):
    ...
    def test_add_tokens(self):
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(
                ...
                self.assertEqual(
                    tokenizer_r.add_tokens(["testoken1", "testtoken2"]), 2
                )
                ...
                self.assertIn(
                    "<testtoken3>",
                    tokenizer_r.special_tokens_map[
                        "additional_special_tokens"
                    ],
                )
    ...
    def test_offsets_mapping(self):
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(
                ...
                self.assertEqual(
                    len(offsets), len(tokens_with_offsets["input_ids"])
                )
    ...
    def test_batch_encode_dynamic_overflowing(self):
        ...
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            ...
            with self.subTest(
                ...
                for key in filter(
                    lambda x: "overflow_to_sample_mapping" not in x,
                    tokens.keys(),
                ):
    ...
    def test_padding(self, max_length=50):
        ...
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(
                ...
                self.assertEqual(
                    tokenizer_p.pad_token_id, tokenizer_r.pad_token_id
                )
        ...
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(
                ...
                for file_path in tokenizer_r_files + tokenizer_p_files:
                    if os.path.exists(file_path) and file_path.endswith(
                        ".json"
                    ):
                ...
                self.assertTrue(
                    any("tokenizer.json" in f for f in tokenizer_r_files)
                )
        ...
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(
                ...
                if "token_type_ids" in tokens_r:
                    self.assertEqual(
                        sum(tokens_r["token_type_ids"]),
                        sum(tokens_p["token_type_ids"]),
                    )
        ...
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(
                ...
                if self.test_slow_tokenizer:
                    ...
                    p_output = tokenizer_p.encode(
                        "Hey this is a <special> token"
                    )

                    cr_output = tokenizer_cr.encode(
                        "Hey this is a <special> token"
                    )
    ...
    def test_special_tokens_initialization_with_non_empty_additional_special_tokens(
        ...
        for tokenizer_class, tokenizer_utils in tokenizer_list:
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                tokenizer_without_change_in_init = (
    ...
    def test_training_new_tokenizer(self):
        ...
        self.assertEqual(
            ...
            new_tokenizer.max_len_single_sentence,
        )
        self.assertEqual(
            tokenizer.max_len_sentences_pair,
            new_tokenizer.max_len_sentences_pair,
        )
        ...
        for special_token in tokenizer.all_special_tokens_extended:
            if (
                ...
                self.assertTrue(
                    ...
                    (
                        f"'{special_token}' should be in"
                        f" {new_tokenizer.all_special_tokens_extended}"
                    ),
                )
                ...
                self.assertTrue(
                    ...
                    (
                        ...
                        f" '{new_tokenizer.all_special_tokens_extended}' as an"
                        " AddedToken with the same parameters as"
                        f" '{special_token}' in the list"
                        f" {tokenizer.all_special_tokens_extended}"
                    ),
                )
                ...
                with self.assertLogs("transformers", level="WARNING") as cm:
                    try:
                        ...
                        self.assertTrue(
                            ...
                            or raised_error_msg_target in error_message
                        )
                    try:
                        ...
                        self.assertTrue(
                            cm.records[0].message.startswith(
                                ...
                                " checkpoint is not the same type as the class"
        ...
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(
                ...
                with tempfile.TemporaryDirectory() as tmp_dir_1:
                    ...
                    tokenizer_slow = self.tokenizer_class.from_pretrained(
                        tmp_dir_1
                    )
...
class TokenizerUtilTester(unittest.TestCase):
    def test_cached_files_are_used_when_internet_is_down(self):
        ...
        _ = BertTokenizer.from_pretrained(
            "hf-internal-testing/tiny-random-bert"
        )

        # Under the mock environment we get a 500 error when trying to reach the tokenizer.
        with mock.patch(
            "requests.request", return_value=response_mock
        ) as mock_head:
            _ = BertTokenizer.from_pretrained(
                "hf-internal-testing/tiny-random-bert"
            )
...
class TokenizerPushToHubTester(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        try:
            delete_repo(
                token=cls._token, repo_id="valid_org/test-tokenizer-org"
            )
        except HTTPError:
            pass
    ...
    def test_push_to_hub_dynamic_tokenizer(self):
        ...
        tokenizer.push_to_hub(
            "test-dynamic-tokenizer", use_auth_token=self._token
        )

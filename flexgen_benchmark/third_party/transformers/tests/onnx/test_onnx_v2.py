...
class OnnxUtilsTestCaseV2(TestCase):
    ...
    def test_compute_effective_axis_dimension(self):
        ...
        self.assertEqual(
            compute_effective_axis_dimension(
                -1, fixed_dimension=2, num_token_to_add=0
            ),
            2,
        )
        ...
        self.assertEqual(
            compute_effective_axis_dimension(
                0, fixed_dimension=2, num_token_to_add=0
            ),
            2,
        )
        ...
        self.assertEqual(
            compute_effective_axis_dimension(
                0, fixed_dimension=8, num_token_to_add=2
            ),
            6,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            compute_effective_axis_dimension(
                0, fixed_dimension=8, num_token_to_add=3
            ),
            5,
        )
        self.assertEqual(
...
class OnnxConfigWithPastTestCaseV2(TestCase):
    ...
    def test_use_past(self):
        ...
        for (
            name,
            config,
        ) in OnnxConfigWithPastTestCaseV2.SUPPORTED_WITH_PAST_CONFIGS:
            with self.subTest(name):
                self.assertFalse(
                    OnnxConfigWithPast.from_model_config(config()).use_past,
                    (
                        "OnnxConfigWithPast.from_model_config() should not"
                        " use_past"
                    ),
                )
...
def _get_models_to_test(export_models_list):
    ...
    if is_torch_available() or is_tf_available():
        ...
        return [
            ("dummy", "dummy", "dummy", "dummy", OnnxConfig.from_model_config)
        ]
...
class OnnxExportTestCaseV2(TestCase):
    ...
    def _onnx_export(
        ...
        if is_torch_available():
            ...
            if torch_version < onnx_config.torch_onnx_minimum_version:
                pytest.skip(
                    "Skipping due to incompatible PyTorch version. Minimum"
                    f" required is {onnx_config.torch_onnx_minimum_version},"
                    f" got: {torch_version}"
                )
    ...
    def _onnx_export_encoder_decoder_models(
        ...
        encoder_onnx_config = onnx_config.get_encoder_config(
            encoder_model.config
        )
    ...
    @parameterized.expand(
        _get_models_to_test(PYTORCH_EXPORT_ENCODER_DECODER_MODELS)
    )
    @slow
    @require_torch
    @require_vision
    ...
    @parameterized.expand(
        _get_models_to_test(PYTORCH_EXPORT_SEQ2SEQ_WITH_PAST_MODELS)
    )
    @slow
    @require_torch
    ...
    @parameterized.expand(
        _get_models_to_test(TENSORFLOW_EXPORT_DEFAULT_MODELS)
    )
    @slow
    @require_tf
    @require_vision
    ...
    @parameterized.expand(
        _get_models_to_test(TENSORFLOW_EXPORT_WITH_PAST_MODELS),
        skip_on_empty=True,
    )
    @slow
    @require_tf

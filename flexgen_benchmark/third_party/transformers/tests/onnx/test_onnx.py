...
class FuncNonContiguousArgs:
    def forward(
        self, input_ids, some_other_args, token_type_ids, attention_mask
    ):
        return None
...
class OnnxExportTestCase(unittest.TestCase):
    ...
    def _test_export(
        self, model, framework, opset, tokenizer=None, **model_kwargs
    ):
        try:
            # Compute path
    ...
    def test_infer_dynamic_axis_pytorch(self):
        ...
        model = BertModel(
            BertConfig.from_pretrained("lysandre/tiny-bert-random")
        )

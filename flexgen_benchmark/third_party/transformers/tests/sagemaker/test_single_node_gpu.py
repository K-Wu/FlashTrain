...
@pytest.mark.skipif(
    ...
    reason=(
        "Skipping test because should only be run when releasing minor"
...
class SingleNodeTest(unittest.TestCase):
    def setUp(self):
        if self.framework == "pytorch":
            subprocess.run(
                "cp ./examples/pytorch/text-classification/run_glue.py"
                f" {self.env.test_path}/run_glue.py".split(),
                encoding="utf-8",
                check=True,
            )
    ...
    def test_glue(self):
        ...
        eval_accuracy = list(
            result_metrics_df[
                result_metrics_df.metric_name == "eval_accuracy"
            ]["value"]
        )
        eval_loss = list(
            result_metrics_df[result_metrics_df.metric_name == "eval_loss"][
                "value"
            ]
        )
        ...
        with open(
            f"{estimator.latest_training_job.name}.json", "w"
        ) as outfile:
            json.dump(
                {

...
class TestTrainerExt(TestCasePlus):
    ...
    def test_trainer_log_level_replica(self, experiment_id):
        ...
        with CaptureStderr() as cl:
            self.run_seq2seq_quick(
                **kwargs, extra_args_str=data["extra_args_str"]
            )
    ...
    def test_run_seq2seq_bnb(self):
        ...
        def train_and_return_metrics(optim: str) -> Tuple[int, float]:
            ...
            extra_args = (
                f"--skip_memory_metrics 0 --optim {optim} --do_eval False"
                " --do_predict False --adafactor False --log_level debug"
            )
        ...
        (
            ...
            gpu_alloc_mem_orig,
            loss_orig,
        ) = train_and_return_metrics(OptimizerNames.ADAMW_TORCH.value)
        (
            gpu_peak_mem_bnb,
            gpu_alloc_mem_bnb,
            loss_bnb,
        ) = train_and_return_metrics(OptimizerNames.ADAMW_BNB.value)
        ...
        gpu_total_mem_diff_percent = (
            gpu_total_mem_diff_bytes / gpu_total_mem_bnb
        )
        ...
        self.assertGreater(
            ...
            (
                "should use very little peak gpu memory with BNB, compared to"
                f" without itbut got gpu_peak_mem_orig={gpu_peak_mem_orig} and"
                f" gpu_peak_mem_bnb={gpu_peak_mem_bnb}"
            ),
        )
        ...
        self.assertGreater(
            ...
            (
                "Using BNB should use less total GPU memory than without"
                f" itbut got gpu_total_mem_orig={gpu_total_mem_orig} and"
                f" gpu_total_mem_bnb={gpu_total_mem_bnb}"
            ),
        )
        ...
        self.assertEqual(
            ...
            loss_bnb,
            (
                f"loss should be the same, but got loss_orig={loss_orig},"
                f" loss_bnb={loss_bnb}"
            ),
        )
        ...
        total_numel = sum(
            dict(
                (p.data_ptr(), p.numel()) for p in model.parameters()
            ).values()
        )
        ...
        self.assertGreater(
            ...
            bnb_saved_bytes
            * 0.8,  # add a safety margin, if it saved slightly less
            (
                f"BNB should have saved about {bnb_saved_bytes} bytes, but the"
                f" saved bytes were {gpu_total_mem_diff_bytes}"
            ),
        )

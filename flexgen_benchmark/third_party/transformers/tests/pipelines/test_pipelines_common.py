...
def get_checkpoint_from_architecture(architecture):
    ...
    if hasattr(module, "_CHECKPOINT_FOR_DOC"):
        ...
        logger.warning(
            f"Can't retrieve checkpoint from {architecture.__name__}"
        )
...
def get_tiny_config_from_class(configuration_class):
    ...
    try:
        ...
        logger.error(
            f"No model tester class for {configuration_class.__name__}"
        )
        return
    ...
    if hasattr(model_tester, "get_pipeline_config"):
        ...
        config = None
        logger.warning(
            f"Model tester {model_tester_class.__name__} has no"
            " `get_config()`."
        )
    ...
    def __new__(mcs, name, bases, dct):
        def gen_test(
            ...
            @skipIf(
                tiny_config is None,
                (
                    "TinyConfig does not exist, make sure that you defined a"
                    " `_CONFIG_FOR_DOC` variable in the modeling file"
                ),
            )
            @skipIf(
                checkpoint is None,
                (
                    "checkpoint does not exist, make sure that you defined a"
                    " `_CHECKPOINT_FOR_DOC` variable in the modeling file"
                ),
            )
            def test(self):
                ...
                try:
                    ...
                    self.skipTest(
                        f"Cannot run with {tiny_config} as the model requires"
                        f" a library that isn't installed: {e}"
                    )
                ...
                if tokenizer_class is not None:
                    ...
                    # provide some default tokenizer and hope for the best.
                        ...
                        self.skipTest(
                            f"Ignoring {ModelClass}, cannot create a simple"
                            " tokenizer"
                        )
                else:
                    tokenizer = None
                ...
                if tokenizer is None and feature_extractor is None:
                    self.skipTest(
                        f"Ignoring {ModelClass}, cannot create a tokenizer or"
                        " feature_extractor (PerceiverConfig with no"
                        " FastTokenizer ?)"
                    )
            ...
            if mapping:
                for configuration, model_architectures in mapping.items():
                    ...
                    for model_architecture in model_architectures:
                        ...
                        feature_extractor_class = (
                            FEATURE_EXTRACTOR_MAPPING.get(configuration, None)
                        )
...
class CommonPipelineTest(unittest.TestCase):
    ...
    def test_iterator_data(self):
        ...
        for out in pipe(data(10)):
            self.assertEqual(
                nested_simplify(out), {"label": "LABEL_0", "score": 0.504}
            )
            results.append(out)
    ...
    def test_pipeline_padding(self):
        ...
        self.assertTrue(
            torch.allclose(
                ...
                torch.LongTensor(
                    [[1, 23, 24, 2, 10, 10], [1, 23, 24, 43, 44, 2]]
                ),
            )
        )
        self.assertTrue(
            torch.allclose(
                ...
                torch.LongTensor(
                    [[10, 10, 1, 23, 24, 2], [1, 23, 24, 43, 44, 2]]
                ),
            )
        )
        self.assertTrue(
    ...
    def check_default_pipeline(
        ...
        if task == "translation":
            ...
            for translation_pair in task_dict["default"].keys():
                model_id, revision = task_dict["default"][translation_pair][
                    "model"
                ][framework]
        ...
        for model_id, revision, task in zip(model_ids, revisions, tasks):
            ...
            models_are_equal = check_models_equal_fn(
                default_pipeline.model, model
            )
            self.assertTrue(
                models_are_equal, f"{task} model doesn't match pipeline."
            )
...
class CustomPipelineTest(unittest.TestCase):
        ...
        try:
            with CaptureLogger(logger_) as cm:
                PIPELINE_REGISTRY.register_pipeline(
                    alias, PairClassificationPipeline
                )
    ...
    def test_register_pipeline(self):
        ...
        assert (
            "custom-text-classification"
            in PIPELINE_REGISTRY.get_supported_tasks()
        )
...
class DynamicPipelineTester(unittest.TestCase):
    ...
    def test_push_to_hub_dynamic_pipeline(self):
        ...
        self.assertDictEqual(
            nested_simplify(results), nested_simplify(new_results)
        )
        ...
        self.assertListEqual(
            nested_simplify(
                [{"label": results["label"], "score": results["score"]}]
            ),

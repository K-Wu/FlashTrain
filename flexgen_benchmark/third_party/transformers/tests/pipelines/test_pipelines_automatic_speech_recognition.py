...
class AutomaticSpeechRecognitionPipelineTests(
    ...
    def test_small_model_pt(self):
        ...
        self.assertEqual(
            str(v.exception),
            (
                "`chunk_length_s` is only valid for CTC models, use other"
                " chunking options for other models"
            ),
        )
        ...
        self.assertEqual(
            str(v.exception),
            "We cannot return_timestamps yet on non-ctc models !",
        )
    ...
    def test_large_model_pt_with_lm(self):
        ...
        self.assertEqual(
            ...
            {
                "text": (
                    "y en las ramas medio sumergidas revoloteaban algunos"
                    " pájaros de quimérico y legendario plumaje"
                )
            },
        )
        ...
        self.assertEqual(
            output,
            {
                "text": (
                    "y en las ramas medio sumergidas revoloteaban algunos"
                    " pájaros de quimérico y legendario plumajre"
                )
            },
        )
        ...
        self.assertEqual(
            output,
            {
                "text": (
                    "y en las ramas medio sumergidas revoloteaban algunos"
                    " pájaros de quimérico y legendario plumajcri"
                ),
                "chunks": [
    ...
    def test_simple_whisper_asr(self):
        ...
        self.assertEqual(
            ...
            {
                "text": (
                    " Mr. Quilter is the apostle of the middle classes, and we"
    ...
    def test_simple_whisper_translation(self):
        ...
        self.assertEqual(
            output_3, {"text": " Un uomo ha detto allo universo, Sir, esiste."}
        )
    ...
    def test_with_lm_fast(self):
        ...
        with self.assertRaises(TypeError) as e:
            output = speech_recognizer(
                [audio_tiled], decoder_kwargs={"num_beams": 2}
            )
            self.assertContains(
                e.msg,
                (
                    "TypeError: decode_beams() got an unexpected keyword"
                    " argument 'num_beams'"
                ),
            )
        output = speech_recognizer(
            [audio_tiled], decoder_kwargs={"beam_width": 2}
        )
    ...
    def test_chunking_and_timestamps(self):
        ...
        self.assertEqual(
            ...
            [
                {
                    "text": (
                        "A MAN SAID TO THE UNIVERSE SIR I EXIST " * n_repeats
                    ).strip()
                }
            ],
        )
        ...
        self.assertEqual(
            speech_recognizer.feature_extractor.sampling_rate, 16_000
        )
        ...
        output = speech_recognizer(
            audio, return_timestamps="word", chunk_length_s=2.0
        )
        self.assertEqual(
            output,
            {
    ...
    def test_chunk_iterator(self):
        ...
        self.assertEqual(
            [o["input_values"].shape for o in outs], [(1, 50), (1, 50)]
        )
        ...
        self.assertEqual(
            [o["input_values"].shape for o in outs], [(1, 80), (1, 20)]
        )
        self.assertEqual([o["is_last"] for o in outs], [False, True])
    ...
    def test_chunk_iterator_stride(self):
        ...
        self.assertEqual(len(outs), 2)
        self.assertEqual(
            [o["stride"] for o in outs], [(100, 0, 10), (30, 20, 0)]
        )
        self.assertEqual(
            [o["input_values"].shape for o in outs], [(1, 100), (1, 30)]
        )
        self.assertEqual([o["is_last"] for o in outs], [False, True])
        ...
        self.assertEqual(len(outs), 2)
        self.assertEqual(
            [o["stride"] for o in outs], [(80, 0, 10), (50, 20, 0)]
        )
        self.assertEqual(
            [o["input_values"].shape for o in outs], [(1, 80), (1, 50)]
        )
        self.assertEqual([o["is_last"] for o in outs], [False, True])
        ...
        self.assertEqual(len(outs), 2)
        self.assertEqual(
            [o["stride"] for o in outs], [(90, 0, 0), (30, 20, 0)]
        )
        self.assertEqual(
            [o["input_values"].shape for o in outs], [(1, 90), (1, 30)]
        )
...
class AudioUtilsTest(unittest.TestCase):
    def test_chunk_bytes_iter_too_big(self):
        iter_ = iter(
            chunk_bytes_iter(
                bytes_iter(chunk_size=3, chunks=2), 10, stride=(0, 0)
            )
        )
        self.assertEqual(
    ...
    def test_chunk_bytes_iter(self):
        iter_ = iter(
            chunk_bytes_iter(
                bytes_iter(chunk_size=3, chunks=2), 3, stride=(0, 0)
            )
        )
        self.assertEqual(
            next(iter_), {"raw": b"\x00\x01\x02", "stride": (0, 0)}
        )
        self.assertEqual(
            next(iter_), {"raw": b"\x03\x04\x05", "stride": (0, 0)}
        )
        with self.assertRaises(StopIteration):
            next(iter_)
    ...
    def test_chunk_bytes_iter_stride(self):
        iter_ = iter(
            chunk_bytes_iter(
                bytes_iter(chunk_size=3, chunks=2), 3, stride=(1, 1)
            )
        )
        self.assertEqual(
            next(iter_), {"raw": b"\x00\x01\x02", "stride": (0, 1)}
        )
        self.assertEqual(
            next(iter_), {"raw": b"\x01\x02\x03", "stride": (1, 1)}
        )
        self.assertEqual(
            next(iter_), {"raw": b"\x02\x03\x04", "stride": (1, 1)}
        )
        # This is finished, but the chunk_bytes doesn't know it yet.
        self.assertEqual(
            next(iter_), {"raw": b"\x03\x04\x05", "stride": (1, 1)}
        )
    ...
    def test_chunk_bytes_iter_stride_stream(self):
        iter_ = iter(
            chunk_bytes_iter(
                bytes_iter(chunk_size=3, chunks=2),
                5,
                stride=(1, 1),
                stream=True,
            )
        ...
        self.assertEqual(
            next(iter_),
            {"raw": b"\x00\x01\x02", "stride": (0, 0), "partial": True},
        )
        self.assertEqual(
            next(iter_),
            {
        ...
        self.assertEqual(
            next(iter_),
            {"raw": b"\x03\x04\x05", "stride": (1, 0), "partial": False},
        )
        with self.assertRaises(StopIteration):
            next(iter_)
        ...
        iter_ = iter(
            chunk_bytes_iter(
                bytes_iter(chunk_size=3, chunks=3),
                5,
                stride=(1, 1),
                stream=True,
            )
        ...
        self.assertEqual(
            next(iter_),
            {"raw": b"\x06\x07\x08", "stride": (1, 0), "partial": False},
        )
        with self.assertRaises(StopIteration):
            next(iter_)

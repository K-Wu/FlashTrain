...
class TokenClassificationPipelineTests(
        ...
        self.assertEqual(
            nested_simplify(
                token_classifier.aggregate(
                    example, AggregationStrategy.AVERAGE
                )
            ),
            [
                {
    ...
    def test_tf_only(self):
        model_name = (  # This model only has a TensorFlow version
    ...
    def test_small_model_pt(self):
        ...
        sentences = [
            "This is a test !",
            "Another test this is with longer sentence",
        ]
        ...
        self.assertEqual(
            nested_simplify(outputs_batched), nested_simplify(outputs)
        )
        self.assertEqual(
...
class TokenClassificationArgumentHandlerTestCase(unittest.TestCase):
    ...
    def test_simple(self):
        ...
        inputs, offset_mapping = self.args_parser(
            [string, string],
            offset_mapping=[[(0, 1), (1, 2)], [(0, 2), (2, 3)]],
        )
        self.assertEqual(inputs, [string, string])
    ...
    def test_errors(self):
        ...
        with self.assertRaises(ValueError):
            self.args_parser(
                [string, string], offset_mapping=[[(0, 1), (1, 2)]]
            )

...
class ZeroShotClassificationPipelineTests(
    ...
    def run_pipeline_test(self, classifier, _):
        ...
        outputs = classifier(
            ["I am happy", "I am sad"], ["positive", "negative"]
        )
        self.assertEqual(
            outputs,
            [
        ...
        with self.assertRaises(TypeError):
            classifier(
                "Who are you voting for in 2020?", candidate_labels=None
            )
        ...
        self.assertEqual(
            nested_simplify(outputs),
            {
                "sequence": (
                    "The dominant sequence transduction models are based on"
                    " complex recurrent or convolutional neural networks in an"
                    " encoder-decoder configuration. The best performing"
                    ...
                    " 2014 English-to-German translation task, improving over"
                    " the existing best results, including ensembles by over 2"
                    " BLEU. On the WMT 2014 English-to-French translation"

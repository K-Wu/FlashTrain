...
class FillMaskPipelineTests(unittest.TestCase, metaclass=PipelineTestCaseMeta):
    ...
    def get_test_pipeline(self, model, tokenizer, feature_extractor):
        if tokenizer is None or tokenizer.mask_token_id is None:
            self.skipTest(
                "The provided tokenizer has no mask token, (probably reformer"
                " or wav2vec2)"
            )
    ...
    def run_test_targets(self, model, tokenizer):
        ...
        fill_masker = FillMaskPipeline(model=model, tokenizer=tokenizer)
        outputs = fill_masker(
            f"This is a {tokenizer.mask_token}", targets=targets
        )
        self.assertEqual(
            outputs,
            [
        ...
        with self.assertRaises(ValueError):
            outputs = fill_masker(
                f"This is a {tokenizer.mask_token}", targets=[""]
            )
        with self.assertRaises(ValueError):
            outputs = fill_masker(
                f"This is a {tokenizer.mask_token}", targets=[]
    ...
    def run_test_top_k(self, model, tokenizer):
        fill_masker = FillMaskPipeline(
            model=model, tokenizer=tokenizer, top_k=2
        )
        outputs = fill_masker(f"This is a {tokenizer.mask_token}")
        self.assertEqual(
            outputs,
    ...
    def fill_mask_with_multiple_masks(self, model, tokenizer):
        ...
        outputs = fill_masker(
            (
                "This is a"
                f" {tokenizer.mask_token} {tokenizer.mask_token} {tokenizer.mask_token}"
            ),
            top_k=2,
        )
        self.assertEqual(

...
class SequenceFeatureExtractionTestMixin(FeatureExtractionSavingTestMixin):
    ...
    def _check_padding(self, numpify=False):
        ...
        self.assertEqual(
            input_9.shape[:2], (batch_size, expected_mult_pad_length)
        )
        ...
        padding_vector_sum = (
            np.ones(self.feat_extract_tester.feature_size)
            * feat_extract.padding_value
        ).sum()
        self.assertTrue(
            abs(
    ...
    def _check_truncation(self, numpify=False):
        ...
        with self.assertRaises(ValueError):
            feat_extract.pad(
                processed_features, padding="longest", truncation=True
            )[input_name]
        ...
        with self.assertRaises(ValueError):
            feat_extract.pad(
                processed_features, padding="max_length", truncation=True
            )[input_name]
    ...
    def test_attention_mask(self):
        ...
        self.assertListEqual(
            ...
            list(processed[input_name].shape[:2]),
        )
        self.assertListEqual(

...
class ModelTesterMixin:
    ...
    def test_save_load_fast_init_from_base(self):
        ...
        for model_class in self.all_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                model.save_pretrained(tmpdirname)
                torch.save(
                    state_dict, os.path.join(tmpdirname, "pytorch_model.bin")
                )
            ...
            for name, param in model.named_parameters():
                if param.requires_grad:
                    self.assertIn(
                        ...
                        [0.0, 1.0],
                        msg=(
                            f"Parameter {name} of model {model_class} seems"
    ...
    def test_torch_fx_output_loss(self):
        ...
        self._create_and_check_torch_fx_tracing(
            config, inputs_dict, output_loss=True
        )
        ...
        for model_class in self.all_model_classes:
            ...
            for i in range(num_outputs):
                self.assertTrue(
                    ...
                    (
                        f"traced {i}th output doesn't match model {i}th output"
                        f" for {model_class}"
                    ),
                )
            ...
            for i in range(num_outputs):
                self.assertTrue(
                    ...
                    (
                        f"scripted {i}th output doesn't match model {i}th"
                        f" output for {model_class}"
                    ),
                )
            ...
            with tempfile.TemporaryDirectory() as tmp_dir_name:
                ...
                try:
                    ...
                    self.fail(
                        "Couldn't serialize / deserialize the traced"
                        f" model: {e}"
                    )
                ...
                for i in range(num_outputs):
                    self.assertTrue(
                        ...
                        (
                            f"serialized model {i}th output doesn't match"
                            f" model {i}th output for {model_class}"
                        ),
                    )
    ...
    def test_model_main_input_name(self):
        for model_class in self.all_model_classes:
            model_signature = inspect.signature(
                getattr(model_class, "forward")
            )
            # The main input is the name of the argument after `self`
            observed_main_input_name = list(model_signature.parameters.keys())[
                1
            ]
            self.assertEqual(
                model_class.main_input_name, observed_main_input_name
            )
    ...
    def test_model_outputs_equivalence(self):
        ...
        def check_equivalence(
            model, tuple_inputs, dict_inputs, additional_kwargs={}
        ):
            with torch.no_grad():
                tuple_output = model(
                ...
                def recursive_check(tuple_object, dict_object):
                    if isinstance(tuple_object, (List, Tuple)):
                        for tuple_iterable_value, dict_iterable_value in zip(
                            ...
                            recursive_check(
                                tuple_iterable_value, dict_iterable_value
                            )
                        ...
                        self.assertTrue(
                            ...
                            msg=(
                                "Tuple and dict output are not equal."
                                " Difference:"
                                f" {torch.max(torch.abs(tuple_object - dict_object))}."
                                " Tuple has `nan`:"
                                f" {torch.isnan(tuple_object).any()} and"
                                f" `inf`: {torch.isinf(tuple_object)}. Dict"
                                " has `nan`:"
                                f" {torch.isnan(dict_object).any()} and `inf`:"
                                f" {torch.isinf(dict_object)}."
                            ),
                        )
    ...
    def _postprocessing_to_ignore_test_cases(
        self, tf_outputs, pt_outputs, model_class
    ):
        ...
        new_tf_outputs = type(tf_outputs)(
            **{k: tf_outputs[k] for k in tf_keys}
        )
        new_pt_outputs = type(pt_outputs)(
            **{k: pt_outputs[k] for k in pt_keys}
        )
    ...
    def check_pt_tf_outputs(
        ...
        if isinstance(tf_outputs, ModelOutput):
            self.assertTrue(
                ...
                (
                    f"{name}: `pt_outputs` should an instance of `ModelOutput`"
                    " when `tf_outputs` is"
                ),
            )
            ...
            self.assertEqual(
                tf_keys,
                pt_keys,
                f"{name}: Output keys differ between TF and PyTorch",
            )
        ...
        # Allow `list` (e.g. `TransfoXLModelOutput.mems` is a list of tensors.)
            ...
            if attributes is not None:
                ...
                self.assertEqual(
                    ...
                    len(tf_outputs),
                    (
                        f"{name}: The tuple `attributes` should have the same"
                        " length as `tf_outputs`"
                    ),
                )
            else:
                # case 2: each output has no assigned name (e.g. hidden states of each layer) -> add an index to `name`
                attributes = tuple(
                    [f"{name}_{idx}" for idx in range(len(tf_outputs))]
                )

            for tf_output, pt_output, attr in zip(
                tf_outputs, pt_outputs, attributes
            ):
                self.check_pt_tf_outputs(
            ...
            self.assertLessEqual(
                ...
                tol,
                (
                    f"{name}: Difference between PyTorch and TF is"
                    f" {max_diff} (>= {tol})."
                ),
            )
        else:
            raise ValueError(
                "`tf_outputs` should be an instance of `ModelOutput`, a"
                " `tuple`, or an instance of `tf.Tensor`. Got"
    ...
    def test_pt_tf_model_equivalence(self):
        ...
        for model_class in self.all_model_classes:
            ...
            tf_inputs_dict = self.prepare_tf_inputs_from_pt_inputs(
                pt_inputs_dict
            )
    ...
    def check_pt_flax_outputs(
        ...
        if isinstance(fx_outputs, ModelOutput):
            ...
            fx_keys = tuple(
                [k for k, v in fx_outputs.items() if v is not None]
            )
            pt_keys = tuple(
            ...
            self.assertEqual(
                fx_keys,
                pt_keys,
                f"{name}: Output keys differ between Flax and PyTorch",
            )
        ...
        # Allow `list` (e.g. `TransfoXLModelOutput.mems` is a list of tensors.)
            ...
            if attributes is not None:
                ...
                self.assertEqual(
                    ...
                    (
                        f"{name}: The tuple `attributes` should have the same"
                        " length as `fx_outputs`"
                    ),
                )
            else:
                # case 2: each output has no assigned name (e.g. hidden states of each layer) -> add an index to `name`
                attributes = tuple(
                    [f"{name}_{idx}" for idx in range(len(fx_outputs))]
                )

            for fx_output, pt_output, attr in zip(
                fx_outputs, pt_outputs, attributes
            ):
                self.check_pt_flax_outputs(
            ...
            self.assertLessEqual(
                ...
                tol,
                (
                    f"{name}: Difference between PyTorch and Flax is"
                    f" {max_diff} (>= {tol})."
                ),
            )
        else:
            raise ValueError(
                "`fx_outputs` should be an instance of `ModelOutput`, a"
                " `tuple`, or an instance of `jnp.ndarray`. Got"
    ...
    def test_equivalence_pt_to_flax(self):
        ...
        for model_class in self.all_model_classes:
            with self.subTest(model_class.__name__):
                ...
                pt_inputs = {
                    k: v for k, v in pt_inputs.items() if k in fx_input_keys
                }
                ...
                self.assertEqual(fx_keys, pt_keys)
                self.check_pt_flax_outputs(
                    fx_outputs_loaded, pt_outputs, model_class
                )
    ...
    def test_equivalence_flax_to_pt(self):
        ...
        for model_class in self.all_model_classes:
            with self.subTest(model_class.__name__):
                ...
                self.assertEqual(fx_keys, pt_keys)
                self.check_pt_flax_outputs(
                    fx_outputs, pt_outputs_loaded, model_class
                )
    ...
    def check_device_map_is_respected(self, model, device_map):
        for param_name, param in model.named_parameters():
            ...
            if param_name not in device_map:
                raise ValueError(
                    "device map is incomplete, it does not contain any device"
                    " for `param_name`."
                )
    ...
    def test_disk_offload(self):
        ...
        for model_class in self.all_model_classes:
            ...
            inputs_dict_class = self._prepare_for_class(
                inputs_dict, model_class
            )
    ...
    def test_cpu_offload(self):
        ...
        for model_class in self.all_model_classes:
            ...
            max_gpu_sizes = [
                int(p * model_size) for p in self.model_split_percents
            ]
            with tempfile.TemporaryDirectory() as tmp_dir:
                model.cpu().save_pretrained(tmp_dir)
                ...
                for max_size in max_gpu_sizes:
                    ...
                    self.assertTrue(
                        torch.allclose(base_output[0], new_output[0])
                    )
        ...
        for model_class in self.all_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                for max_size in max_gpu_sizes:
                    ...
                    # Making sure part of the model will actually end up offloaded
                    self.assertSetEqual(
                        set(new_model.hf_device_map.values()), {0, 1}
                    )
            ...
            for problem_type in problem_types:
                with self.subTest(
                    ...
                    for w in warning_list:
                        if (
                            "Using a target size that is different to the"
                            " input size"
                            in str(w.message)
                        ):
                            raise ValueError(
                                "Something is going wrong in the regression"
                                f" problem: intercepted {w.message}"
                            )
...
class ModelUtilsTest(TestCasePlus):
    ...
    def test_model_from_pretrained_with_different_pretrained_model_name(self):
        ...
        self.assertTrue(
            "You are using a model of type t5 to instantiate a model of type"
            " bert"
            in cl.out
        )
    ...
    def test_shard_checkpoint(self):
        ...
        with self.subTest("Test sharding, no weights bigger than max size"):
            shards, index = shard_checkpoint(
                state_dict, max_shard_size="300kB"
            )
        ...
        with self.subTest("Test sharding with weights bigger than max size"):
            shards, index = shard_checkpoint(
                state_dict, max_shard_size="100kB"
            )
    ...
    def test_checkpoint_sharding_local(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for max_size in [
                ...
                self.assertFalse(
                    os.path.isfile(os.path.join(tmp_dir, WEIGHTS_NAME))
                )
                ...
                shards_found = set(
                    f for f in os.listdir(tmp_dir) if f.endswith(".bin")
                )
    ...
    def test_from_pretrained_low_cpu_mem_usage_measured(self):
        ...
        one_liner_str = (
            f'{preamble}; AutoModel.from_pretrained("{mname}",'
        ...
        self.assertGreater(
            ...
            0.15,
            (
                "should use less CPU memory for low_cpu_mem_usage=True, but"
                f" got max_rss_normal={max_rss_normal} and"
                f" max_rss_low_mem={max_rss_low_mem}"
            ),
        )
    ...
    def test_safetensors_load_from_hub(self):
        ...
        # Check models are equal
        for p1, p2 in zip(
            safetensors_model.parameters(), pytorch_model.parameters()
        ):
            self.assertTrue(torch.allclose(p1, p2))
    ...
    def test_safetensors_save_and_load_sharded(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertFalse(
                os.path.isfile(os.path.join(tmp_dir, WEIGHTS_INDEX_NAME))
            )
            self.assertTrue(
...
class ModelPushToHubTester(unittest.TestCase):
    ...
    def test_push_to_hub_in_organization(self):
        ...
        model = BertModel(config)
        model.push_to_hub(
            "valid_org/test-model-org", use_auth_token=self._token
        )

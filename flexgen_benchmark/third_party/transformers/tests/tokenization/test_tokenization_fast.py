...
class PreTrainedTokenizationFastTest(TokenizerTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tokenizers_list = [
            (PreTrainedTokenizerFast, model_path, {})
...
class TokenizerVersioningTest(unittest.TestCase):
    def test_local_versioning(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            tokenizer.init_kwargs["fast_tokenizer_files"] = [
                "tokenizer.4.0.0.json"
            ]
            tokenizer.save_pretrained(tmp_dir)
            json.dump(
                json_tokenizer,
                open(os.path.join(tmp_dir, "tokenizer.4.0.0.json"), "w"),
            )
    ...
    def test_repo_versioning(self):
        ...
        old_tokenizer = (
            old_transformers.models.auto.AutoTokenizer.from_pretrained(repo)
        )
...
class ReduceMutableBorrowTests(unittest.TestCase):
    def test_async_share_tokenizer(self):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            self.assertEqual(
                return_value,
                [[1, 10, 0, 8, 0, 18, 0, 0, 0, 2] for i in range(10)],
            )

    def fetch(self, tokenizer, text):
        return tokenizer.encode(
            text, truncation="longest_first", padding="longest"
        )

...
class MixedInt8Test(BaseMixedInt8Test):
    ...
    def test_memory_footprint(self):
        ...
        self.assertAlmostEqual(
            mem_fp16 / mem_8bit, self.EXPECTED_RELATIVE_DIFFERENCE
        )
        self.assertTrue(
...
class MixedInt8ModelClassesTest(BaseMixedInt8Test):
    ...
    def test_correct_head_class(self):
        ...
        self.assertTrue(
            self.base_model.h[-1].mlp.dense_4h_to_h.weight.__class__
            == Int8Params
        )

        # Other heads should be nn.Parameter
        self.assertTrue(
            self.model_8bit.lm_head.weight.__class__ == torch.nn.Parameter
        )
        self.assertTrue(
            self.sequence_model.score.weight.__class__ == torch.nn.Parameter
        )
        self.assertTrue(
            self.seq_to_seq_model.lm_head.weight.__class__
            == torch.nn.Parameter
        )
        ...
        def get_list_devices(model):
            ...
            for _, module in model.named_children():
                if len(list(module.children())) > 0:
                    ...
                    try:
                        list_devices.append(
                            next(module.parameters()).device.index
                        )

...
class WavLMModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_base(self):
        ...
        EXPECTED_HIDDEN_STATES_SLICE = torch.tensor(
            [
                [[0.0577, 0.1161], [0.0579, 0.1165]],
                [[0.0199, 0.1237], [0.0059, 0.0605]],
            ]
        )
        # TODO: update the tolerance after the CI moves to torch 1.10
        self.assertTrue(
            torch.allclose(
                hidden_states_slice, EXPECTED_HIDDEN_STATES_SLICE, atol=5e-2
            )
        )
    ...
    def test_inference_large(self):
        ...
        EXPECTED_HIDDEN_STATES_SLICE = torch.tensor(
            [
                [[0.2122, 0.0500], [0.2118, 0.0563]],
                [[0.1353, 0.1818], [0.2453, 0.0595]],
            ]
        )

        self.assertTrue(
            torch.allclose(
                hidden_states_slice, EXPECTED_HIDDEN_STATES_SLICE, rtol=5e-2
            )
        )

...
class BigBirdModelTester:
    ...
    def create_and_check_for_pretraining(
        ...
        self.parent.assertEqual(
            result.seq_relationship_logits.shape,
...
class BigBirdModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (
        (BigBirdForCausalLM,) if is_torch_available() else ()
    )
        ...
        with torch.no_grad():
            ...
            self.assertTrue(
                torch.allclose(
                    hidden_states[0, 0, :5],
                    torch.tensor(
                        [1.4825, 0.0774, 0.8226, -0.2962, -0.9593],
                        device=torch_device,
                    ),
                    atol=1e-3,
                )
    ...
    def test_for_change_to_full_attn(self):
        ...
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_for_change_to_full_attn(
            *config_and_inputs
        )
...
class BigBirdModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_full_pretraining(self):
        ...
        self.assertEqual(
            prediction_logits.shape, torch.Size((1, 512 * 4, 50358))
        )
        self.assertEqual(seq_relationship_logits.shape, torch.Size((1, 2)))
    ...
    def test_block_sparse_attention_probs(self):
        ...
        (
            ...
            from_mask,
            to_mask,
        ) = model.create_masks_for_block_sparse_attn(
    ...
    def test_block_sparse_context_layer(self):
        ...
        self.assertTrue(
            torch.allclose(
                context_layer[0, 64:78, 300:310], targeted_cl, atol=0.0001
            )
        )
    ...
    def test_tokenizer_inference(self):
        ...
        text = [
            "Transformer-based models are unable to process long sequences due"
            " to their self-attention operation, which scales quadratically"
            " with the sequence length. To address this limitation, we"
            " introduce the Longformer with an attention mechanism that scales"
            " linearly with sequence length, making it easy to process"
            " documents of thousands of tokens or longer. Longformer’s"
            " attention mechanism is a drop-in replacement for the standard"
            " self-attention and combines a local windowed attention with a"
            " task motivated global attention. Following prior work on"
            " long-sequence transformers, we evaluate Longformer on"
            " character-level language modeling and achieve state-of-the-art"
            " results on text8 and enwik8. In contrast to most prior work, we"
            " also pretrain Longformer and finetune it on a variety of"
        ...
        for k in inputs:
            inputs[k] = torch.tensor(
                inputs[k], device=torch_device, dtype=torch.long
            )
    ...
    def test_inference_question_answering(self):
        ...
        context = (
            "The BigBird model was proposed in Big Bird: Transformers for"
            " Longer Sequences by Zaheer, Manzil and Guruganesh, Guru and"
            " Dubey, Kumar Avinava and Ainslie, Joshua and Alberti, Chris and"
            " Ontanon, Santiago and Pham, Philip and Ravula, Anirudh and Wang,"
            " Qifan and Yang, Li and others. BigBird, is a sparse-attention"
            " based transformer which extends Transformer based models, such"
            " as BERT to much longer sequences. In addition to sparse"
            " attention, BigBird also applies global attention as well as"
            " random attention to the input sequence. Theoretically, it has"
            " been shown that applying sparse, global, and random attention"
            " approximates full attention, while being computationally much"
            " more efficient for longer sequences. As a consequence of the"
            " capability to handle longer context, BigBird has shown improved"
            " performance on various long document NLP tasks, such as question"
            " answering and summarization, compared to BERT or RoBERTa."
        )
        ...
        self.assertTrue(
            torch.allclose(
                start_logits[:, 64:96], target_start_logits, atol=1e-4
            )
        )
        self.assertTrue(
    ...
    def test_auto_padding(self):
        ...
        self.assertTrue(
            torch.allclose(output[0, 64:78, 300:310], target, atol=0.0001)
        )

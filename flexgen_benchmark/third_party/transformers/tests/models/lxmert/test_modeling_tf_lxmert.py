...
class TFLxmertModelTester(object):
    ...
    def create_and_check_lxmert_model(
        ...
        result = model(
            input_ids, visual_feats, bounding_boxes, return_dict=False
        )
        result = model(
            input_ids, visual_feats, bounding_boxes, return_dict=True
        )
...
class TFLxmertModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            ...
            for attention, attention_shape in zip(
                attentions, attention_shapes
            ):
    ...
    def prepare_pt_inputs_from_tf_inputs(self, tf_inputs_dict):
        ...
        for key, value in tf_inputs_dict.items():
            if isinstance(value, dict):
                pt_inputs_dict[key] = self.prepare_pt_inputs_from_tf_inputs(
                    value
                )
                ...
                pt_inputs_dict[key] = torch.from_numpy(value.numpy()).to(
                    torch.float32
                )
...
class TFLxmertModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        ...
        _, visual_pos = np.random.seed(0), np.random.rand(
            1, num_visual_features, 4
        )
        ...
        output = model(
            input_ids, visual_feats=visual_feats, visual_pos=visual_pos
        )[0]

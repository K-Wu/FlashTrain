...
SAMPLE_PROCESSOR_CONFIG = get_tests_dir(
...
class AutoFeatureExtractorTest(unittest.TestCase):
    ...
    def test_processor_from_feat_extr_processor_class(self):
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            with open(
                os.path.join(tmpdirname, TOKENIZER_CONFIG_FILE), "r"
            ) as f:
                config_dict = json.load(f)
                config_dict.pop("processor_class")

            with open(
                os.path.join(tmpdirname, TOKENIZER_CONFIG_FILE), "w"
            ) as f:
    ...
    def test_processor_from_tokenizer_processor_class(self):
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            with open(
                os.path.join(tmpdirname, FEATURE_EXTRACTOR_NAME), "r"
            ) as f:
                config_dict = json.load(f)
                config_dict.pop("processor_class")

            with open(
                os.path.join(tmpdirname, FEATURE_EXTRACTOR_NAME), "w"
            ) as f:
                f.write(json.dumps(config_dict))
    ...
    def test_auto_processor_creates_feature_extractor(self):
        ...
        self.assertEqual(
            processor.__class__.__name__, "ConvNextFeatureExtractor"
        )
...
class ProcessorPushToHubTester(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        try:
            delete_repo(
                token=cls._token, repo_id="valid_org/test-processor-org"
            )
        except HTTPError:
            pass
    ...
    def test_push_to_hub(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            new_processor = Wav2Vec2Processor.from_pretrained(
                f"{USER}/test-processor"
            )
            for k, v in processor.feature_extractor.__dict__.items():
                self.assertEqual(
                    v, getattr(new_processor.feature_extractor, k)
                )
            self.assertDictEqual(
                new_processor.tokenizer.get_vocab(),

...
class AutoTokenizerTest(unittest.TestCase):
    ...
    def test_tokenizer_from_type(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            shutil.copy(
                ...
                os.path.join(tmp_dir, "vocab.json"),
            )
            shutil.copy(
                "./tests/fixtures/merges.txt",
                os.path.join(tmp_dir, "merges.txt"),
            )
    ...
    def test_tokenizer_identifier_non_existent(self):
        for tokenizer_class in [
            ...
            with self.assertRaisesRegex(
                EnvironmentError,
                (
                    "julien-c/herlolip-not-exists is not a local folder and is"
    ...
    def test_new_tokenizer_registration(self):
        try:
            ...
            AutoTokenizer.register(
                CustomConfig, slow_tokenizer_class=CustomTokenizer
            )
            # Trying to register something existing in the Transformers library will raise an error
            with self.assertRaises(ValueError):
                AutoTokenizer.register(
                    BertConfig, slow_tokenizer_class=BertTokenizer
                )
    ...
    def test_new_tokenizer_fast_registration(self):
        try:
            ...
            self.assertEqual(
                TOKENIZER_MAPPING[CustomConfig], (CustomTokenizer, None)
            )
            AutoTokenizer.register(
                CustomConfig, fast_tokenizer_class=CustomTokenizerFast
            )
            self.assertEqual(
                TOKENIZER_MAPPING[CustomConfig],
    ...
    def test_repo_not_found(self):
        with self.assertRaisesRegex(
            EnvironmentError,
            (
                "bert-base is not a local folder and is not a valid model"
                " identifier"
            ),
        ):
    ...
    def test_revision_not_found(self):
        with self.assertRaisesRegex(
            EnvironmentError,
            (
                r"aaaaaa is not a valid git identifier \(branch name, tag name"
                r" or commit id\)"
            ),
        ):

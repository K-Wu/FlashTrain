...
class AutoModelTest(unittest.TestCase):
    ...
    def test_model_file_not_found(self):
        with self.assertRaisesRegex(
            EnvironmentError,
            (
                "hf-internal-testing/config-no-model does not appear to have a"
                " file named pytorch_model.bin"
            ),
        ):
            _ = AutoModel.from_pretrained(
    ...
    def test_attr_not_existing(self):
        ...
        _MODEL_MAPPING = _LazyAutoMapping(
            _CONFIG_MAPPING_NAMES, _MODEL_MAPPING_NAMES
        )

        with pytest.raises(
            ValueError,
            match=r"Could not find GhostModel neither in .* nor in .*!",
        ):

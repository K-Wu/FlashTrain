...
class TFSegformerModelTester:
    ...
    def prepare_config_and_inputs_for_keras_fit(
        self, for_segmentation: bool = False
    ):
        config_and_inputs = self.prepare_config_and_inputs()
...
class TFSegformerModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        "SegFormer does not have get_input_embeddings method and"
        " get_output_embeddings methods"
    )
    def test_model_common_attributes(self):
        pass

    @unittest.skip(
        "Test was written for TF 1.x and isn't really relevant here"
    )
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            ...
            expected_reduced_seq_len = (
                ...
                // (4 * self.model_tester.sr_ratios[0])
            ) ** 2
            ...
            expected_reduced_seq_len = (
                self.model_tester.image_size
                // (32 * self.model_tester.sr_ratios[-1])
            ) ** 2
            self.assertListEqual(
    ...
    def check_keras_fit_results(
        self, val_loss1, val_loss2, atol=2e-1, rtol=2e-1
    ):
        self.assertTrue(

...
class RegNetModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_model_classes = (
        (RegNetModel, RegNetForImageClassification)
        if is_torch_available()
        else ()
    )
    ...
    @unittest.skip(
        reason="RegNet does not support input and output embeddings"
    )
    ...
    def test_hidden_states_output(self):
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            self.assertListEqual(
                ...
                [
                    self.model_tester.image_size // 2,
                    self.model_tester.image_size // 2,
                ],
            )
...
class RegNetModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_image_classification_head(self):
        ...
        expected_slice = torch.tensor([-0.4180, -1.5051, -3.4836]).to(
            torch_device
        )

...
class FlaxEncoderDecoderMixin:
    ...
    def check_encoder_decoder_model_from_encoder_decoder_pretrained(
        ...
        **kwargs
        ...
        self.assertEqual(
            decoder_config.use_cache, decoder_model.config.use_cache
        )
    ...
    def check_encoder_decoder_model_output_attentions(
        ...
        **kwargs
        ...
        seq_len = enc_dec_model._get_feat_extract_output_lengths(
            inputs.shape[1]
        )
        self.assertEqual(
    ...
    def check_encoder_decoder_model_generate(
        ...
        decoder_start_token_id = (
            enc_dec_model.config.decoder.decoder_start_token_id
        )
    ...
    def check_freeze_feature_encoder(
        ...
        **kwargs
        ...
        def compute_loss(
            ...
            loss = cross_entropy(
                logits,
                onehot(labels=decoder_input_ids, num_classes=vocab_size),
            ).sum()
            return (loss, logits)
    ...
    def check_pt_flax_equivalence(self, pt_model, fx_model, inputs_dict):
        ...
        pt_inputs = {
            k: torch.tensor(v.tolist()) for k, v in flax_inputs.items()
        }
        ...
        for fx_output_loaded, pt_output in zip(fx_outputs_loaded, pt_outputs):
            self.assert_almost_equals(
                fx_output_loaded, pt_output.numpy(), 1e-5
            )
        ...
        for fx_output, pt_output_loaded in zip(fx_outputs, pt_outputs_loaded):
            self.assert_almost_equals(
                fx_output, pt_output_loaded.numpy(), 1e-5
            )

    def check_equivalence_pt_to_flax(
    ...
    def test_real_model_save_load_from_pretrained(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dirname:
            ...
            model_1 = FlaxSpeechEncoderDecoderModel.from_pretrained(
                tmp_dirname
            )
...
class FlaxWav2Vec2GPT2ModelTest(FlaxEncoderDecoderMixin, unittest.TestCase):
    def get_pretrained_model_and_inputs(self):
        ...
        decoder_input_ids = ids_tensor(
            [batch_size, 4], model.config.decoder.vocab_size
        )
...
class FlaxWav2Vec2BartModelTest(FlaxEncoderDecoderMixin, unittest.TestCase):
    ...
    def prepare_config_and_inputs(self):
        model_tester_encoder = FlaxWav2Vec2ModelTester(self, batch_size=13)
        model_tester_decoder = FlaxBartStandaloneDecoderModelTester(
            self, batch_size=13
        )
        encoder_config_and_inputs = (
            model_tester_encoder.prepare_config_and_inputs()
...
class FlaxWav2Vec2BertModelTest(FlaxEncoderDecoderMixin, unittest.TestCase):
    def get_pretrained_model_and_inputs(self):
        ...
        input_values = floats_tensor(
            [batch_size, 512], model.config.encoder.vocab_size
        )
        attention_mask = random_attention_mask([batch_size, 512])
        decoder_input_ids = ids_tensor(
            [batch_size, 4], model.config.decoder.vocab_size

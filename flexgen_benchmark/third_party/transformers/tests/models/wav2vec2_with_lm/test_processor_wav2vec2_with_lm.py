...
class Wav2Vec2ProcessorWithLMTest(unittest.TestCase):
    ...
    def get_decoder(self, **kwargs):
        return BeamSearchDecoderCTC.load_from_hf_hub(
            self.decoder_name, **kwargs
        )
    ...
    def test_save_load_pretrained_default(self):
        ...
        # tokenizer
        self.assertEqual(
            processor.tokenizer.get_vocab(), tokenizer.get_vocab()
        )
        ...
        # decoder
        self.assertEqual(
            processor.decoder._alphabet.labels, decoder._alphabet.labels
        )
        self.assertEqual(
    ...
    def test_decoder_batch(self, pool_context):
        ...
        self.assertListEqual(
            ["<s> <s> </s>", "<s> <s> <s>"], decoded_processor.text
        )
        self.assertListEqual(
            logit_scores_decoder, decoded_processor.logit_score
        )
    ...
    def test_word_time_stamp_integration(self):
        ...
        EXPECTED_TEXT = (
            "WHY DOES A MILE SANDRA LOOK LIKE SHE WANTS TO CONSUME JOHN SNOW"
            " ON THE RIVER AT THE WALL"
        )
        ...
        end_times = torch.tensor(
            self.get_from_offsets(word_time_stamps, "end_time")
        )
        ...
        self.assertTrue(
            torch.allclose(start_times, expected_start_tensor, atol=0.01)
        )
        self.assertTrue(
            torch.allclose(end_times, expected_end_tensor, atol=0.01)
        )

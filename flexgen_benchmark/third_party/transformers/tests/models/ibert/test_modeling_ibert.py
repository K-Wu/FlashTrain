...
class IBertModelIntegrationTest(unittest.TestCase):
    def test_quant_embedding(self):
        ...
        self.assertTrue(
            torch.allclose(
                x_scaling_factor, expected_scaling_factor, atol=1e-4
            )
        )
        self.assertTrue(
            torch.allclose(
        ...
        self.assertTrue(
            torch.allclose(
                y_scaling_factor, expected_scaling_factor, atol=1e-4
            )
        )
    ...
    def test_quant_act(self):
        def _test_range():
            ...
            self.assertTrue(
                torch.allclose(act.x_min, expected_x_min, atol=1e-4)
            )
            self.assertTrue(
                torch.allclose(act.x_max, expected_x_max, atol=1e-4)
            )
    ...
    def test_force_dequant_softmax(self):
        ...
        softmax_fdqs_dict = {
            True: [
                IntSoftmax(
                    output_bit, quant_mode=True, force_dequant="nonlinear"
                ),
                IntSoftmax(
                    output_bit, quant_mode=True, force_dequant="softmax"
                ),
            ],
            False: [
                ...
                IntSoftmax(
                    output_bit, quant_mode=True, force_dequant="layernorm"
                ),
            ],
        }
    ...
    def test_int_layernorm(self):
        ...
        # some random matrix
        array = [
            [[i * j * j + j for j in range(5, 15)]] for i in range(-10, 10)
        ]
        x_int = torch.tensor(array)
        x_scaling_factor = torch.tensor(0.1)
        x = x_int * x_scaling_factor
        ...
        ln_q = IntLayerNorm(
            x.shape[1:], 1e-5, quant_mode=True, output_bit=output_bit
        )
    ...
    def test_inference_masked_lm(self):
        ...
        self.assertEqual(output.shape, expected_shape)
        self.assertTrue(
            torch.allclose(output[:, :3, :3], expected_slice, atol=0.1)
        )

...
class PerceiverModelTester:
    def __init__(
        ...
        image_chunk_size = (
            np.prod((self.num_frames, self.image_size, self.image_size))
            // self.nchunks
        )
        audio_chunk_size = (
            self.num_frames
    ...
    def prepare_config_and_inputs(self, model_class=None):
        ...
        if model_class is None or model_class.__name__ == "PerceiverModel":
            ...
            audio = torch.randn(
                (
                    self.batch_size,
                    self.num_frames * self.audio_samples_per_frame,
                    1,
                ),
                device=torch_device,
            )
    ...
    def create_and_check_for_sequence_classification(
        ...
        model.eval()
        result = model(
            inputs, attention_mask=input_mask, labels=sequence_labels
        )
        self.parent.assertEqual(
...
class PerceiverModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            ...
            self.assertEqual(
                len(self_attentions), expected_num_self_attentions
            )
            self.assertEqual(
    ...
    @unittest.skip(
        reason=(
            "Perceiver does not work with data parallel (DP) because of a bug"
            " in PyTorch: https://github.com/pytorch/pytorch/issues/36035"
        )
    )
    ...
    @unittest.skip(
        reason=(
            "Perceiver models don't have a typical head like is the case with"
            " BERT"
        )
    )
...
class PerceiverModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        ...
        self.assertTrue(
            torch.allclose(logits[0, :3, :3], expected_slice, atol=1e-4)
        )

        expected_greedy_predictions = [
            38,
            115,
            111,
            121,
        ...
        self.assertListEqual(
            expected_greedy_predictions, masked_tokens_predictions
        )
    ...
    def test_inference_image_classification(self):
        ...
        expected_slice = torch.tensor(
            [-1.1653, -0.1993, -0.7521], device=torch_device
        )
    ...
    def test_inference_optical_flow(self):
        ...
        patches = extract_image_patches(
            inputs.view(batch_size * 2, C, H, W), kernel=3
        )

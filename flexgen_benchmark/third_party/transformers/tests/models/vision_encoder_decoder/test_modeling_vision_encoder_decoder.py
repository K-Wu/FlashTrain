...
class EncoderDecoderMixin:
    ...
    def check_encoder_decoder_model_from_pretrained(
        ...
        **kwargs
        ...
        enc_dec_model = (
            VisionEncoderDecoderModel.from_encoder_decoder_pretrained(**kwargs)
        )
        enc_dec_model.to(torch_device)
        outputs_encoder_decoder = enc_dec_model(
            pixel_values=pixel_values,
        ...
        **kwargs
        ...
        with torch.no_grad():
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                enc_dec_model = VisionEncoderDecoderModel.from_pretrained(
                    tmpdirname
                )
                enc_dec_model.to(torch_device)
    ...
    def check_encoder_decoder_model_generate(
        ...
        self.assertEqual(
            ...
            (inputs.shape[0],) + (decoder_config.max_length,),
        )
    ...
    def test_real_model_save_load_from_pretrained(self):
        ...
        with torch.no_grad():
            ...
            with tempfile.TemporaryDirectory() as tmp_dirname:
                ...
                model_1 = VisionEncoderDecoderModel.from_pretrained(
                    tmp_dirname
                )
...
class DeiT2RobertaModelTest(EncoderDecoderMixin, unittest.TestCase):
    def get_pretrained_model_and_inputs(self):
        ...
        decoder_input_ids = ids_tensor(
            [batch_size, 4], model.decoder.config.vocab_size
        )
    ...
    def prepare_config_and_inputs(self):
        ...
        encoder_config_and_inputs = (
            deit_model_tester.prepare_config_and_inputs()
        )
...
class ViT2BertModelTest(EncoderDecoderMixin, unittest.TestCase):
    ...
    def prepare_config_and_inputs(self):
        ...
        bert_model_tester = BertModelTester(self)
        encoder_config_and_inputs = (
            vit_model_tester.prepare_config_and_inputs()
        )
        decoder_config_and_inputs = (
            bert_model_tester.prepare_config_and_inputs_for_decoder()
        )
...
class Swin2BartModelTest(EncoderDecoderMixin, unittest.TestCase):
    ...
    def prepare_config_and_inputs(self):
        model_tester_encoder = SwinModelTester(
            self, batch_size=13, embed_dim=32
        )
        ...
        decoder_config_and_inputs = (
            model_tester_decoder.prepare_config_and_inputs()
        )
        config, pixel_values, _ = encoder_config_and_inputs
        decoder_config, decoder_inputs_dict = decoder_config_and_inputs
        decoder_inputs_dict["labels"] = decoder_inputs_dict[
            "decoder_input_ids"
        ]
...
class TrOCRModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_handwritten(self):
        ...
        dataset = load_dataset(
            "hf-internal-testing/fixtures_ocr", split="test"
        )
    ...
    def test_inference_printed(self):
        ...
        is_pillow_less_than_9 = version.parse(PIL.__version__) < version.parse(
            "9.0.0"
        )
...
class ViT2GPT2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_coco_en(self):
        ...
        with torch.no_grad():
            logits = (
                model(pixel_values, decoder_input_ids)[0]
                .detach()
                .cpu()
                .numpy()
            )
...
class DonutModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_docvqa(self):
        ...
        dataset = load_dataset(
            "hf-internal-testing/example-documents", split="test"
        )
        ...
        expected_slice = torch.tensor([24.2731, -6.4522, 32.4130]).to(
            torch_device
        )
        self.assertTrue(
            torch.allclose(logits[0, 0, :3], expected_slice, atol=1e-4)
        )
        ...
        self.assertEqual(
            sequence,
            (
                "<s_question> When is the coffee break?</s_question><s_answer>"
                " 11-14 to 11:39 a.m.</s_answer>"
            ),
        )
        ...
        self.assertTrue(
            torch.allclose(
                ...
                torch.tensor([5.3153, -3.5276, 13.4781], device=torch_device),
                atol=1e-4,
            )
        )

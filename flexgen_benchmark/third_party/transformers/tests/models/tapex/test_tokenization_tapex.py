...
class TapexTokenizationTest(TokenizerTesterMixin, unittest.TestCase):
    ...
    def get_clean_sequence(
        ...
        if len(output_ids) < min_length:
            raise ValueError(
                "Update the code to generate the sequences so that they are"
                " larger"
            )
    ...
    def test_add_tokens_tokenizer(self):
        tokenizers: List[TapexTokenizer] = self.get_tokenizers(
            do_lower_case=False
        )
    ...
    def test_token_type_ids(self):
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                output = tokenizer(
                    empty_table, seq_0, return_token_type_ids=True
                )
    ...
    @unittest.skip(
        "TAPEX cannot handle `prepare_for_model` without passing by"
    ...
    def test_batch_encode_plus_batch_sequence_length(self):
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                encoded_sequences = [
                    tokenizer.encode_plus(table, sequence)
                    for sequence in sequences
                ]
                encoded_sequences_batch = tokenizer.batch_encode_plus(

...
@require_torch
@require_vision
class BeitFeatureExtractionTest(
    FeatureExtractionSavingTestMixin, unittest.TestCase
):
    feature_extraction_class = (
        BeitFeatureExtractor if is_vision_available() else None
    )
    ...
    def test_call_segmentation_maps(self):
        ...
        encoding = feature_extractor(
            image_inputs[0], maps[0], return_tensors="pt"
        )
        self.assertEqual(
        ...
        encoding = feature_extractor(
            images, segmentation_maps, return_tensors="pt"
        )
        self.assertEqual(
            encoding["pixel_values"].shape,
            (

...
class BeitModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason=(
            "BEiT has some layers using `add_module` which doesn't work well"
    ...
    def test_training_gradient_checkpointing(self):
        ...
        for model_class in self.all_model_classes:
            ...
            if (
                model_class
                in [*get_values(MODEL_MAPPING), BeitForMaskedImageModeling]
...
class BeitModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_image_modeling_head(self):
        ...
        bool_masked_pos = torch.ones((1, 196), dtype=torch.bool).to(
            torch_device
        )
        ...
        self.assertTrue(
            torch.allclose(
                logits[bool_masked_pos][:3, :3], expected_slice, atol=1e-2
            )
        )
    ...
    def test_inference_image_classification_head_imagenet_1k(self):
        ...
        feature_extractor = self.default_feature_extractor
        image = prepare_img()
        inputs = feature_extractor(images=image, return_tensors="pt").to(
            torch_device
        )
        ...
        expected_slice = torch.tensor([-1.2385, -1.0987, -1.0108]).to(
            torch_device
        )
    ...
    def test_inference_image_classification_head_imagenet_22k(self):
        ...
        expected_slice = torch.tensor([1.6881, -0.2787, 0.5901]).to(
            torch_device
        )

...
class MaskFormerModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        class_labels = (
            torch.rand((self.batch_size, self.num_labels), device=torch_device)
            > 0.5
        ).long()
    ...
    def check_output_hidden_state(self, output, config):
        ...
        transformer_decoder_hidden_states = (
            output.transformer_decoder_hidden_states
        )
        ...
        self.parent.assertTrue(
            len(pixel_decoder_hidden_states),
            len(config.backbone_config.depths),
        )
        self.parent.assertTrue(
            len(transformer_decoder_hidden_states),
            config.decoder_config.decoder_layers,
        )
...
class MaskFormerModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason="MaskFormer does not have a get_input_embeddings method"
    )
    ...
    @unittest.skip(
        reason=(
            "MaskFormer has some layers using `add_module` which doesn't work"
            " well with `nn.DataParallel`"
        )
    )
    ...
    def test_model_with_labels(self):
        ...
        model = MaskFormerForInstanceSegmentation(MaskFormerConfig()).to(
            torch_device
        )
    ...
    def test_retain_grad_hidden_states_attentions(self):
        ...
        transformer_decoder_hidden_states = (
            outputs.transformer_decoder_hidden_states[0]
        )
...
class MaskFormerModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        model = MaskFormerModel.from_pretrained(self.model_checkpoints).to(
            torch_device
        )
        ...
        self.assertTrue(
            (inputs_shape[-1] % 32) == 0 and (inputs_shape[-2] % 32) == 0
        )
        # check size

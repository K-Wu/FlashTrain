...
class BertTokenizationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tf_tokenizers = [
            ...
            TFBertTokenizer.from_pretrained(
                checkpoint, use_fast_bert_tokenizer=False
            )
            for checkpoint in TOKENIZER_CHECKPOINTS
        ]
        ...
        self.test_sentences = [
            ...
            (
                "Classical Irish also has some unusual characters, so in they"
                " go: Gaelaċ, ꝼ"
            ),
        ]
    ...
    def test_output_equivalence(self):
        for tokenizer, tf_tokenizer in zip(
            self.tokenizers, self.tf_tokenizers
        ):
            for test_inputs in (self.test_sentences, self.paired_sentences):
                ...
                for key in python_outputs.keys():
                    ...
                    self.assertTrue(
                        tf.reduce_all(
                            tf.cast(python_outputs[key], tf.int64)
                            == tf_outputs[key]
                        )
                    )
    ...
    def test_different_pairing_styles(self):
        for tf_tokenizer in self.tf_tokenizers:
            ...
            for key in merged_outputs.keys():
                self.assertTrue(
                    tf.reduce_all(
                        tf.cast(merged_outputs[key], tf.int64)
        ...
        for tf_tokenizer in self.tf_tokenizers:
            ...
            for test_inputs in (self.test_sentences, self.paired_sentences):
                ...
                for key in eager_outputs.keys():
                    self.assertTrue(
                        tf.reduce_all(
                            eager_outputs[key] == compiled_outputs[key]
                        )
                    )
    ...
    def test_saved_model(self):
        for tf_tokenizer in self.tf_tokenizers:
            ...
            self.assertLessEqual(
                tf.reduce_max(tf.abs(out - loaded_output)), 1e-5
            )

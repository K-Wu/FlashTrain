...
class HubertModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_intent_classification(self):
        ...
        self.assertTrue(
            torch.allclose(
                predicted_logits_action, expected_logits_action, atol=3e-1
            )
        )
        self.assertTrue(
            torch.allclose(
                predicted_logits_object, expected_logits_object, atol=3e-1
            )
        )
        self.assertTrue(
            torch.allclose(
    ...
    def test_inference_speaker_identification(self):
        ...
        with torch.no_grad():
            for example in input_data["speech"]:
                ...
                output = model(
                    input.input_values.half().to(torch_device),
                    attention_mask=None,
                )
    ...
    def test_inference_distilhubert(self):
        ...
        self.assertTrue(
            torch.allclose(
                outputs[:, :4, :4], expected_outputs_first, atol=5e-3
            )
        )
        self.assertTrue(
            torch.allclose(
                outputs[:, -4:, -4:], expected_outputs_last, atol=5e-3
            )
        )

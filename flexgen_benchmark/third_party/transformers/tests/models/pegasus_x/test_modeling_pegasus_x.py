...
class PegasusXModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(
        ...
        model = (
            PegasusXModel(config=config).get_decoder().to(torch_device).eval()
        )
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = PegasusXEncoder.from_pretrained(tmpdirname).to(
                torch_device
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = PegasusXDecoder.from_pretrained(tmpdirname).to(
                torch_device
            )
...
class PegasusXModelTest(
    ...
    @unittest.skip(
        "`PegasusXGlobalLocalAttention` returns attentions as dictionary - not"
    ...
    def test_generate_fp16(self):
        ...
        model = (
            PegasusXForConditionalGeneration(config).eval().to(torch_device)
        )
...
class PegasusXModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        model = PegasusXModel.from_pretrained("google/pegasus-x-base").to(
            torch_device
        )
    ...
    def test_seq_to_seq_generation(self):
        ...
        batch_input = [
            "While large pretrained Transformer models have proven highly"
            " capable at tackling natural language tasks, handling long"
            " sequence inputs continues to be a significant challenge. One"
            " such task is long input summarization, where inputs are longer"
            " than the maximum input context of most pretrained models."
            " Through an extensive set of experiments, we investigate what"
            " model architectural changes and pretraining paradigms can most"
            " efficiently adapt a pretrained Transformer for long input"
            " summarization. We find that a staggered, block-local Transformer"
            " with global encoder tokens strikes a good balance of performance"
            " and efficiency, and that an additional pretraining phase on long"
            " sequences meaningfully improves downstream summarization"
            " performance. Based on our findings, we introduce PEGASUS-X, an"
            " extension of the PEGASUS model with additional long input"
            " pretraining to handle inputs of up to 16K tokens. PEGASUS-X"
            " achieves strong performance on long input summarization tasks"
            " comparable with much larger models while adding few additional"
            " parameters and not requiring model parallelism to train."
        ]
        ...
        EXPECTED = [
            "we investigate the performance of a new pretrained model for long"
            " input summarization. <n> the model is a superposition of two"
            " well -"
        ]

...
class NllbTokenizationTest(TokenizerTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        "Unfortunately way too slow to build a BPE with SentencePiece."
    )
...
class NllbDistilledIntegrationTest(unittest.TestCase):
    ...
    tgt_text = [
        ...
        (
            "Secretarul General Ban Ki-moon declară că răspunsul său la"
            " intensificarea sprijinului militar al Rusiei pentru Siria este"
            ' că "nu există o soluţie militară" la conflictul de aproape cinci'
            " ani şi că noi arme nu vor face decât să înrăutăţească violenţele"
            " şi mizeria pentru milioane de oameni."
        ),
    ]
    expected_src_tokens = [
        16297,
    ...
    def test_language_codes(self):
        self.assertEqual(
            self.tokenizer.fairseq_tokens_to_ids["ace_Arab"], 256001
        )
        self.assertEqual(
            self.tokenizer.fairseq_tokens_to_ids["ace_Latn"], 256002
        )
        self.assertEqual(
    ...
    def test_special_tokens_unaffacted_by_save_load(self):
        ...
        self.assertDictEqual(
            new_tok.fairseq_tokens_to_ids, original_special_tokens
        )

...
class LayoutLMv2ModelTester:
    ...
    def create_and_check_model(
        ...
        result = model(
            input_ids, bbox=bbox, image=image, token_type_ids=token_type_ids
        )
        ...
        expected_seq_len = (
            self.seq_length
            + self.image_feature_pool_shape[0]
            * self.image_feature_pool_shape[1]
        )
...
class LayoutLMv2ModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason=(
            "LayoutLMV2 and its dependency `detectron2` have some layers using"
            " `add_module` which doesn't work well with `nn.DataParallel`"
        )
    )

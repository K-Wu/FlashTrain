...
class VisualBertModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_model_for_vqa(self):
        config_and_inputs = (
            self.model_tester.prepare_config_and_inputs_for_vqa()
        )
        self.model_tester.create_and_check_for_vqa(*config_and_inputs)

    def test_model_for_nlvr(self):
        config_and_inputs = (
            self.model_tester.prepare_config_and_inputs_for_nlvr()
        )
    ...
    def test_model_for_flickr(self):
        config_and_inputs = (
            self.model_tester.prepare_config_and_inputs_for_flickr()
        )
...
class VisualBertModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_vqa_coco_pre(self):
        ...
        input_ids = torch.tensor([1, 2, 3, 4, 5, 6], dtype=torch.long).reshape(
            1, -1
        )
        ...
        visual_embeds = (
            torch.ones(size=(1, 10, 2048), dtype=torch.float32) * 0.5
        )
        ...
        self.assertEqual(
            output.seq_relationship_logits.shape, expected_shape_2
        )
        ...
        self.assertTrue(
            torch.allclose(
                output.seq_relationship_logits, expected_slice_2, atol=1e-4
            )
        )
    ...
    def test_inference_nlvr(self):
        ...
        visual_embeds = (
            torch.ones(size=(1, 10, 1024), dtype=torch.float32) * 0.5
        )
        visual_token_type_ids = torch.ones(size=(1, 10), dtype=torch.long)
        attention_mask = torch.tensor([1] * 6).reshape(1, -1)
        visual_attention_mask = torch.tensor([1] * 10).reshape(1, -1)
        ...
        self.assertTrue(
            torch.allclose(output.logits, expected_slice, atol=1e-4)
        )
    ...
    def test_inference_vcr(self):
        ...
        visual_embeds = (
            torch.ones(size=(1, 4, 10, 512), dtype=torch.float32) * 0.5
        )

...
class FlavaImageModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        bool_masked_pos = (
            torch.rand(
                (self.batch_size, num_patches, num_patches),
        ...
        for model_class in self.all_model_classes:
            ...
            try:
                ...
                pixel_values = inputs_dict[
                    "pixel_values"
                ]  # FLAVA needs pixel_values

                if "input_ids_masked" in inputs_dict:
                    # For pretraining
                    inputs = (
                        input_ids,
                        inputs_dict["input_ids_masked"],
                        pixel_values,
                    )
                else:
            ...
            loaded_model_state_dict.pop(
                "text_model.embeddings.token_type_ids", None
            )
    ...
    def test_load_image_text_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            ...
            self.assertDictEqual(
                config.image_config.to_dict(), image_config.to_dict()
            )
...
class FlavaForPreTrainingTester(FlavaModelTester):
    ...
    def _test_model(self, config, inputs, test_image=False, test_text=False):
        ...
        with torch.no_grad():
            result = model(
                input_ids=inputs["input_ids"] if test_text else None,
                input_ids_masked=inputs["input_ids_masked"]
                if test_text
                else None,
                attention_mask=inputs["attention_mask"] if test_text else None,
                token_type_ids=inputs["token_type_ids"] if test_text else None,
...
class FlavaForPreTrainingIntegrationTest(unittest.TestCase):
    ...
    def test_inference(self):
        model_name = "facebook/flava-full"
        model = FlavaForPreTraining.from_pretrained(model_name).to(
            torch_device
        )
        processor = FlavaProcessor.from_pretrained(model_name)
        torch.manual_seed(1)
        ...
        self.assertAlmostEqual(
            outputs.loss_info.mmm_text.item(), 1.75533199, places=4
        )
        self.assertAlmostEqual(
            outputs.loss_info.mmm_image.item(), 7.0290069, places=4
        )

...
if is_vision_available():
    ...
    FLAVA_IMAGE_MEAN = (
        FLAVA_IMAGE_STD
    ) = FLAVA_CODEBOOK_MEAN = FLAVA_CODEBOOK_STD = None
...
class FlavaFeatureExtractionTester(unittest.TestCase):
    def __init__(
        ...
        self.size = size
        self.resample = (
            resample if resample is not None else PILImageResampling.BICUBIC
        )
...
@require_torch
@require_vision
class FlavaFeatureExtractionTest(
    FeatureExtractionSavingTestMixin, unittest.TestCase
):
    feature_extraction_class = (
        FlavaFeatureExtractor if is_vision_available() else None
    )
    maxDiff = None
    ...
    def test_call_pytorch(self):
        self._test_call_framework(
            torch.Tensor, prepare_kwargs={"torchify": True}
        )

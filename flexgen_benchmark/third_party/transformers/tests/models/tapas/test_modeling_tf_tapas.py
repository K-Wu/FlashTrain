...
class TFTapasModelTester:
    def __init__(
        ...
        self.init_cell_selection_weights_to_zero = (
    ...
    def prepare_config_and_inputs(self):
        ...
        if self.use_labels:
            ...
            float_answer = ids_tensor(
                [self.batch_size], vocab_size=2, dtype=tf.float32
            )
    ...
    def create_and_check_for_question_answering(
        ...
        result = model(inputs)
        self.parent.assertEqual(
            result.logits.shape, (self.batch_size, self.seq_length)
        )
...
class TFTapasModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason=(
            "The default test gets NaN losses with the test-generated inputs"
        )
    )
...
def prepare_tapas_batch_inputs_for_inference():
    ...
    queries = [
        "Which footballer is 33 years old?",
...
class TFTapasModelIntegrationTest(unittest.TestCase):
    ...
    def test_training_question_answering_head_weak_supervision(self):
        ...
        tf.debugging.assert_near(
            logits_aggregation[1, -4:], expected_tensor, atol=1e-4
        )
...
class TFTapasUtilsTest(unittest.TestCase):
    ...
    def test_flatten(self):
        ...
        batched_index = IndexMap(
            indices=tf.zeros(shape, dtype=tf.int32),
            num_segments=1,
            batch_dims=3,
        )
    ...
    def test_reduce_max(self):
        ...
        index = IndexMap(
            indices=tf.convert_to_tensor([0, 1, 0, 1]), num_segments=2
        )
    ...
    def test_reduce_sum_vectorized(self):
        ...
        index = IndexMap(
            indices=tf.convert_to_tensor([0, 0, 1]),
            num_segments=2,
            batch_dims=0,
        )
    ...
    def test_gather_vectorized(self):
        ...
        index = IndexMap(
            indices=tf.convert_to_tensor([[0, 1], [1, 0]]),
            num_segments=2,
            batch_dims=1,
        )

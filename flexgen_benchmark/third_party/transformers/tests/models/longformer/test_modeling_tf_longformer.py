...
class TFLongformerModelTester:
    ...
    def create_and_check_model(
        ...
        self.parent.assertListEqual(
            shape_list(result.pooler_output),
    ...
    def prepare_config_and_inputs_for_question_answering(self):
        ...
        input_ids = tf.concat(
            [
                input_ids[:, :-3],
                tf.ones_like(input_ids)[:, -3:] * config.sep_token_id,
            ],
            axis=-1,
        )
...
class TFLongformerModelIntegrationTest(unittest.TestCase):
    ...
    def test_pad_and_transpose_last_two_dims(self):
        ...
        paddings = tf.constant(
            [[0, 0], [0, 0], [0, 1], [0, 0]], dtype=tf.int64
        )
    ...
    def test_mask_invalid_locations(self):
        ...
        self.assertTrue(
            tf.math.reduce_sum(tf.cast(tf.math.is_inf(hid_states_1), tf.int64))
            == 8
        )
        self.assertTrue(
            tf.math.reduce_sum(tf.cast(tf.math.is_inf(hid_states_2), tf.int64))
            == 24
        )
        self.assertTrue(
            tf.math.reduce_sum(tf.cast(tf.math.is_inf(hid_states_3), tf.int64))
            == 24
        )
        self.assertTrue(
            tf.math.reduce_sum(tf.cast(tf.math.is_inf(hid_states_4), tf.int64))
            == 12
        )
    ...
    def test_layer_global_attn(self):
        ...
        hidden_states = tf.concat(
            [self._get_hidden_states(), self._get_hidden_states() - 0.5],
            axis=0,
        )
        batch_size, seq_length, hidden_size = hidden_states.shape
    ...
    def test_layer_attn_probs(self):
        ...
        self.assertTrue(
            (
                tf.math.abs(
                    tf.math.reduce_sum(global_attentions[0, :, :2, :], axis=-1)
                    - 1
                )
                < 1e-6
            )
        ...
        self.assertTrue(
            (
                tf.math.abs(
                    tf.math.reduce_sum(global_attentions[1, :, :1, :], axis=-1)
                    - 1
                )
                < 1e-6
            )
        ...
        tf.debugging.assert_near(
            ...
            tf.convert_to_tensor(
                [0.2500, 0.2500, 0.2500, 0.2500], dtype=tf.float32
            ),
            rtol=1e-3,
            atol=1e-4,
        )
        tf.debugging.assert_near(
            ...
            tf.convert_to_tensor(
                [0.2497, 0.2500, 0.2499, 0.2504], dtype=tf.float32
            ),
            rtol=1e-3,
            atol=1e-4,
        )

...
class M2M100ModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(
        ...
        model = (
            M2M100Model(config=config).get_decoder().to(torch_device).eval()
        )
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = M2M100Encoder.from_pretrained(tmpdirname).to(
                torch_device
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = M2M100Decoder.from_pretrained(tmpdirname).to(
                torch_device
            )
...
class M2M100ModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        model = M2M100Model.from_pretrained("facebook/m2m100_418M").to(
            torch_device
        )
    ...
    def test_seq_to_seq_generation(self):
        ...
        src_fr = [
            (
                "L'affaire NSA souligne l'absence totale de débat sur le"
                " renseignement"
            ),
            (
                "Selon moi, il y a deux niveaux de réponse de la part du"
                " gouvernement français."
            ),
            (
                "Lorsque François Hollande téléphone à Barack Obama ou quand"
                " le ministre des affaires étrangères Laurent Fabius convoque"
                " l'ambassadeur des Etats-Unis, ils réagissent à une vraie"
                " découverte, qui est celle de l'ampleur de la surveillance"
                " américaine sur l'ensemble des communications en France."
            ),
        ]
        ...
        expected_en = [
            ...
            (
                "I think there are two levels of response from the French"
                " government."
            ),
            (
                "When François Hollande calls Barack Obama or when Foreign"
                " Minister Laurent Fabius calls the U.S. Ambassador, they"
                " respond to a real discovery, which is that of the scale of"
                " U.S. surveillance on all communications in France."
            ),
        ]

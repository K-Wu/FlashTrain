...
class TFGroupViTVisionModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            ...
            # GroupViT returns attention grouping of each stage
            self.assertEqual(
                len(self_attentions), expected_num_attention_outputs
            )
    ...
    @unittest.skip(
        "TFGroupViTVisionModel does not convert `hidden_states` and"
        " `attentions` to tensors as they are all of different dimensions, and"
        " we get `Got a non-Tensor value` error when saving the model."
    )
    @slow
    def test_saved_model_creation_extended(self):
        ...
        for model_class in self.all_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                self.assertEqual(
                    len(output_hidden_states), expected_num_layers
                )
                self.assertEqual(
    ...
    def test_saved_model_creation_extended(self):
        ...
        for model_class in self.all_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                key_length = getattr(
                    self.model_tester, "key_length", seq_length
                )
...
class TFGroupViTModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason=(
            "`saved_model` doesn't work with nested outputs so no preparation"
            " happens."
        )
    )
    @slow

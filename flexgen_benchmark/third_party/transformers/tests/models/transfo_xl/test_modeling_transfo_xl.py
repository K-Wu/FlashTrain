...
class TransfoXLModelTester:
    ...
    def create_transfo_xl_lm_head_trainer_compatible_tuple(
        ...
        outputs2 = model(
            input_ids_2, labels=lm_labels, mems=mems_1, return_dict=False
        )
...
class TransfoXLModelTest(
    ...
    def check_cutoffs_and_n_token(
        ...
        for i in range(len(copied_cutoffs)):
            if i < layer:
                ...
                if model_class == TransfoXLLMHeadModel:
                    self.assertEqual(
                        model.crit.cutoffs[i],
                        copied_cutoffs[i] + resized_value,
                    )
                if i < len(model.config.cutoffs):
                    self.assertEqual(
                        model.config.cutoffs[i],
                        copied_cutoffs[i] + resized_value,
                    )
    ...
    def test_transfo_xl_lm_head(self):
        ...
        output_result = self.model_tester.create_transfo_xl_lm_head_trainer_compatible_tuple(
            *config_and_inputs
        )
        self.model_tester.check_transfo_xl_lm_head_output(output_result)

        output_result = self.model_tester.create_transfo_xl_lm_head_trainer_incompatible_tuple(
            *config_and_inputs
        )
        self.model_tester.check_transfo_xl_lm_head_output(output_result)
    ...
    def test_resize_tokens_embeddings(self):
        ...
        for model_class in self.all_model_classes:
            ...
            for layer in test_layers:
                ...
                self.assertEqual(
                    model.config.vocab_size, model_vocab_size + 10
                )
    ...
    def _mock_init_weights(self, module):
        ...
        if (
            hasattr(module, "cluster_weight")
            and module.cluster_weight is not None
        ):

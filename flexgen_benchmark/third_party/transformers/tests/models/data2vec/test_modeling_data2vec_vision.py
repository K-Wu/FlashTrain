...
class Data2VecVisionModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason=(
            "Data2VecVision has some layers using `add_module` which doesn't"
            " work well with `nn.DataParallel`"
        )
    )
    ...
    def test_training_gradient_checkpointing(self):
        ...
        for model_class in self.all_model_classes:
            ...
            # this can then be incorporated into _prepare_for_class in test_modeling_common.py
                ...
                model_class.__name__ == "Data2VecVisionForSemanticSegmentation"
            ):
...
class Data2VecVisionModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_image_classification_head_imagenet_1k(self):
        ...
        expected_slice = torch.tensor([0.3277, -0.1395, 0.0911]).to(
            torch_device
        )

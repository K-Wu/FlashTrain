...
class DeformableDetrModelTester:
        ...
        if self.use_labels:
            ...
            for i in range(self.batch_size):
                ...
                target["boxes"] = torch.rand(
                    self.n_targets, 4, device=torch_device
                )
    ...
    def create_and_check_deformable_detr_object_detection_head_model(
        ...
        self.parent.assertEqual(
            result.logits.shape,
            (self.batch_size, self.num_queries, self.num_labels),
        )
        self.parent.assertEqual(
...
class DeformableDetrModelTest(
    ...
    def test_deformable_detr_model(self):
        ...
        self.model_tester.create_and_check_deformable_detr_model(
            *config_and_inputs
        )
    ...
    @unittest.skip(
        reason="Deformable DETR does not have a get_input_embeddings method"
    )
...
class DeformableDetrModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_object_detection_head(self):
        ...
        image = prepare_img()
        encoding = feature_extractor(images=image, return_tensors="pt").to(
            torch_device
        )
        ...
        self.assertTrue(
            torch.allclose(
                outputs.logits[0, :3, :3], expected_logits, atol=1e-4
            )
        )
        ...
        self.assertTrue(
            torch.allclose(
                outputs.pred_boxes[0, :3, :3], expected_boxes, atol=1e-4
            )
        )
        ...
        expected_scores = torch.tensor(
            [0.7999, 0.7894, 0.6331, 0.4720, 0.4382]
        )
        expected_labels = [17, 17, 75, 75, 63]
        expected_slice_boxes = torch.tensor(
            [16.5028, 52.8390, 318.2544, 470.7841]
        )

        self.assertEqual(len(results["scores"]), 5)
        self.assertTrue(
            torch.allclose(results["scores"], expected_scores, atol=1e-4)
        )
        self.assertSequenceEqual(results["labels"].tolist(), expected_labels)
        self.assertTrue(
            torch.allclose(results["boxes"][0, :], expected_slice_boxes)
        )
    ...
    def test_inference_object_detection_head_equivalence_cpu_gpu(self):
        ...
        for key in cpu_outputs.keys():
            assert torch.allclose(
                cpu_outputs[key], gpu_outputs[key].cpu(), atol=1e-4
            )
        ...
        assert torch.allclose(
            cpu_outputs.logits[0, :3, :3], expected_logits, atol=1e-4
        )

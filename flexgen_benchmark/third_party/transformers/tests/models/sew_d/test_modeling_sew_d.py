...
class SEWDModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=SEWDConfig, hidden_size=37
        )
...
class SEWDModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_pretrained_batched(self):
        model = SEWDModel.from_pretrained("asapp/sew-d-tiny-100k").to(
            torch_device
        )
        ...
        self.assertTrue(
            torch.allclose(
                outputs[:, :4, :4], expected_outputs_first, atol=1e-3
            )
        )
        self.assertTrue(
            torch.allclose(
                outputs[:, -4:, -4:], expected_outputs_last, atol=1e-3
            )
        )
    ...
    def test_inference_ctc_batched(self):
        ...
        EXPECTED_TRANSCRIPTIONS = [
            ...
            (
                "swet covered breon's body trickling into the titlowing closs"
                " that was the only garmened he war"
            ),
        ]

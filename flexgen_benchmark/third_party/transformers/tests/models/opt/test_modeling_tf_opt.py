...
def prepare_opt_inputs_dict(
    config, input_ids, attention_mask=None, head_mask=None
):
...
class TFOPTModelTest(TFModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (TFOPTModel, TFOPTForCausalLM) if is_tf_available() else ()
    )
    all_generative_model_classes = (
        (TFOPTForCausalLM,) if is_tf_available() else ()
    )
    is_encoder_decoder = False
    test_pruning = False
    test_onnx = False
...
class OPTModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        self.assertTrue(
            np.allclose(output[:, :3, :3], expected_slice, atol=4e-3)
        )

        xla_generate = tf.function(model, jit_compile=True)
        output = xla_generate(input_ids, attention_mask)[0]
        self.assertTrue(
            np.allclose(output[:, :3, :3], expected_slice, atol=4e-2)
        )
...
class TFOPTEmbeddingsTest(unittest.TestCase):
    ...
    def test_logits(self):
        ...
        logits = tf.math.reduce_mean(
            model(inputs.input_ids, attention_mask=inputs.attention_mask)[0],
            axis=-1,
        )
...
class TFOPTGenerationTest(unittest.TestCase):
    ...
    def test_batch_generation(self):
        ...
        self.assertListEqual(
            batch_out_sentence, [non_padded_sentence, padded_sentence]
        )

...
class BloomModelTester:
    ...
    def create_and_check_bloom_model(
    ...
    def create_and_check_bloom_model_past(
        self, config, input_ids, input_mask, *args
    ):
        model = BloomModel(config=config)
...
class BloomModelTest(
    ...
    all_generative_model_classes = (
        (BloomForCausalLM,) if is_torch_available() else ()
    )
    fx_compatible = True
    test_missing_keys = False
    test_pruning = False
    test_torchscript = (
        True  # torch.autograd functions seems to be not supported
    )

    def setUp(self):
        self.model_tester = BloomModelTester(self)
        self.config_tester = ConfigTester(
            self, config_class=BloomConfig, n_embd=37
        )
    ...
    def test_simple_generation(self):
        ...
        EXPECTED_OUTPUT = (
            "I enjoy walking with my cute dog, and I love to watch the kids"
            " play with the kids. I am a very active person, and I enjoy"
            " working out, and I am a very active person. I am a very active"
            " person, and I"
        )
    ...
    def test_batch_generation_padd(self):
        ...
        # test token values
        self.assertEqual(
            greedy_output[-1, 3:].tolist(),
            greedy_output_without_pad[0, :-3].tolist(),
        )
...
class BloomEmbeddingTest(unittest.TestCase):
    ...
    @unittest.skipIf(
        ...
        (
            "Test failed with torch < 1.10 (`LayerNormKernelImpl` not"
    ...
    def test_embeddings(self):
        ...
        for i, idx in enumerate(EXAMPLE_IDS):
            output_dict["min"][idx] = (
                embeddings.min(dim=-1).values[0][i].item()
            )
            output_dict["max"][idx] = (
                embeddings.max(dim=-1).values[0][i].item()
            )
        ...
        for i, idx in enumerate(EXAMPLE_IDS):
            output_dict_norm["min"][idx] = (
                embeddings_ln.min(dim=-1).values[0][i].item()
            )
            output_dict_norm["max"][idx] = (
                embeddings_ln.max(dim=-1).values[0][i].item()
            )
            output_dict_norm["mean"][idx] = embeddings_ln.mean(dim=-1)[0][
                i
            ].item()

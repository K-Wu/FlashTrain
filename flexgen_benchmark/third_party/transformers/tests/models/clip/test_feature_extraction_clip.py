        ...
        if equal_resolution:
            ...
            for i in range(self.batch_size):
                ...
                image_inputs.append(
                    np.random.randint(
                        255,
                        size=(self.num_channels, width, height),
                        dtype=np.uint8,
                    )
                )
...
@require_torch
@require_vision
class CLIPFeatureExtractionTest(
    FeatureExtractionSavingTestMixin, unittest.TestCase
):
    feature_extraction_class = (
        CLIPFeatureExtractor if is_vision_available() else None
    )
...
class CLIPFeatureExtractionTestFourChannels(
    ...
    def setUp(self):
        self.feature_extract_tester = CLIPFeatureExtractionTester(
            self, num_channels=4
        )

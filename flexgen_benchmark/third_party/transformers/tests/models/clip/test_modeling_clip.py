...
class CLIPVisionModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason=(
            "CLIPVisionModel has no base class and is not available in"
            " MODEL_MAPPING"
        )
    )
...
class CLIPTextModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason=(
            "CLIPTextModel has no base class and is not available in"
            " MODEL_MAPPING"
        )
    )
    def test_save_load_fast_init_from_base(self):
    ...
    def test_equivalence_pt_to_flax(self):
        ...
        for model_class in self.all_model_classes:
            with self.subTest(model_class.__name__):
                ...
                for fx_output, pt_output in zip(
                    fx_outputs[:4], pt_outputs[:4]
                ):
                    self.assert_almost_equals(
                        fx_output, pt_output.numpy(), 4e-2
                    )

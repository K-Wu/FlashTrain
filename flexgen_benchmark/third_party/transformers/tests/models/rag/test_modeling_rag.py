    ...
    def check_model_with_retriever(
        ...
        **kwargs
        ...
        for model_class in self.all_model_classes:
            model = model_class(
                config, retriever=self.get_retriever(config)
            ).to(torch_device)
            model.eval()
    ...
    def check_model_custom_n_docs(
        ...
        **kwargs
        ...
        for model_class in self.all_model_classes:
            ...
            # doc scores
            self.assertEqual(
                outputs.doc_scores.shape, (input_ids.shape[0], n_docs)
            )
...
class RagDPRBartTest(RagTestMixin, unittest.TestCase):
    ...
    def config_and_inputs(self):
        ...
        dpr_config_and_inputs = (
            question_encoder_tester.prepare_config_and_inputs()
        )
        generator_tester = BartModelTester(self)
        bart_config_and_inputs = (
            generator_tester.prepare_config_and_inputs_for_common()
        )
...
class RagModelIntegrationTests(unittest.TestCase):
    ...
    def test_rag_sequence_inference(self):
        ...
        rag_question_encoder_tokenizer = (
        ...
        _assert_tensors_equal(
            expected_doc_scores, output.doc_scores, atol=TOLERANCE
        )
...
class RagModelSaveLoadTests(unittest.TestCase):
    ...
    def test_rag_sequence_from_pretrained(self):
        ...
        self.assertAlmostEqual(
            loss_pretrained.item(), loss_init.item(), places=4
        )
    ...
    def test_rag_token_from_pretrained(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dirname:
            ...
            self.assertTrue(
                rag_token.question_encoder.config.max_length == 200
            )

...
class RagRetrieverTest(TestCase):
    ...
    def get_dummy_legacy_index_retriever(self):
        ...
        pickle.dump(
            dataset["id"], open(index_file_name + ".index_meta.dpr", "wb")
        )
    ...
    def test_canonical_hf_index_retriever_retrieve(self):
        ...
        self.assertEqual(
            sorted(doc_dicts[0]), ["embeddings", "id", "text", "title"]
        )
    ...
    def test_custom_hf_index_retriever_save_and_from_pretrained_from_disk(
        self,
    ):
        retriever = self.get_dummy_custom_hf_index_retriever(from_disk=True)
        with tempfile.TemporaryDirectory() as tmp_dirname:
    ...
    def test_custom_hf_index_end2end_retriever_call(self):
        ...
        self.assertEqual(
            all(
                k in out
                for k in ("tokenized_doc_ids", "tokenized_doc_attention_mask")
            ),
            True,

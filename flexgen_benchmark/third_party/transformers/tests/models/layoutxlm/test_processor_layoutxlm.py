...
class LayoutXLMProcessorTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tokenizer_pretrained_name = (
...
class LayoutXLMProcessorIntegrationTests(unittest.TestCase):
    ...
    def get_tokenizers(self):
        slow_tokenizer = LayoutXLMTokenizer.from_pretrained(
    ...
    def test_processor_case_4(self):
        ...
        for tokenizer in tokenizers:
            ...
            # this was obtained with Tesseract 4.1.1
            expected_decoding = (
                "<s> what's the time</s></s> 7 ITC Limited REPORT AND ACCOUNTS"
                " 2013</s>"
            )
            decoding = processor.decode(input_processor.input_ids[1].tolist())
            self.assertSequenceEqual(decoding, expected_decoding)
    ...
    def test_processor_case_5(self):
        ...
        for tokenizer in tokenizers:
            ...
            # verify input_ids
            expected_decoding = (
                "<s> How old is he?</s></s> hello world</s><pad><pad>"
            )
            decoding = processor.decode(input_processor.input_ids[0].tolist())
            self.assertSequenceEqual(decoding, expected_decoding)

            expected_decoding = (
                "<s> what's the time</s></s> my name is niels</s>"
            )
            decoding = processor.decode(input_processor.input_ids[1].tolist())
            self.assertSequenceEqual(decoding, expected_decoding)

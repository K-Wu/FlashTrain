...
class Swinv2ModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=Swinv2Config, embed_dim=37
        )
    ...
    def check_hidden_states_output(
        self, inputs_dict, config, model_class, image_size
    ):
        model = model_class(config)
        model.to(torch_device)
        model.eval()
    ...
    def test_hidden_states_output(self):
        ...
        image_size = (
            ...
            if isinstance(
                self.model_tester.image_size, collections.abc.Iterable
            )
    ...
    def test_hidden_states_output_with_padding(self):
        ...
        padded_height = (
            image_size[0] + patch_size[0] - (image_size[0] % patch_size[0])
        )
        padded_width = (
            image_size[1] + patch_size[1] - (image_size[1] % patch_size[1])
        )
...
class Swinv2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_image_classification_head(self):
        ...
        expected_slice = torch.tensor([-0.3947, -0.4306, 0.0026]).to(
            torch_device
        )
        self.assertTrue(

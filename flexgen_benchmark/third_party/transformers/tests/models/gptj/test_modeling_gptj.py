...
class GPTJModelTester:
    ...
    def create_and_check_gptj_model(
        ...
        result = model(
            input_ids, token_type_ids=token_type_ids, head_mask=head_mask
        )
...
class GPTJModelTest(
    ...
    all_generative_model_classes = (
        (GPTJForCausalLM,) if is_torch_available() else ()
    )
    fx_compatible = True
    test_pruning = False
...
class GPTJModelLanguageGenerationTest(unittest.TestCase):
    ...
    def test_gptj_sample(self):
        ...
        if torch_device == "cuda":
            EXPECTED_OUTPUT_STR = (
                "Today is a nice day and I've already been enjoying it. I"
                " walked to work with my wife"
            )
        else:
            EXPECTED_OUTPUT_STR = (
                "Today is a nice day and one of those days that feels a bit"
                " more alive. I am ready"
            )
    ...
    def test_gptj_sample_max_time(self):
        ...
        model.generate(
            input_ids, do_sample=True, max_time=MAX_TIME, max_length=256
        )
        ...
        start = datetime.datetime.now()
        model.generate(
            input_ids, do_sample=False, max_time=MAX_TIME, max_length=256
        )
        duration = datetime.datetime.now() - start
        self.assertGreater(duration, datetime.timedelta(seconds=MAX_TIME))
        self.assertLess(duration, datetime.timedelta(seconds=1.5 * MAX_TIME))
        ...
        start = datetime.datetime.now()
        model.generate(
            input_ids, do_sample=False, max_time=None, max_length=256
        )
        duration = datetime.datetime.now() - start
        self.assertGreater(

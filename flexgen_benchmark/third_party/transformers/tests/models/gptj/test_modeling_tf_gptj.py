...
class TFGPTJModelTest(
    ...
    all_generative_model_classes = (
        (TFGPTJForCausalLM,) if is_tf_available() else ()
    )
    test_onnx = False
    test_pruning = False
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=GPTJConfig, n_embd=37
        )
    ...
    def test_gptj_lm_head_model(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_gptj_lm_head_model(
            *config_and_inputs
        )
    ...
    def test_model_from_pretrained(self):
        model = TFGPTJModel.from_pretrained(
            "EleutherAI/gpt-j-6B", from_pt=True
        )
        self.assertIsNotNone(model)

    @unittest.skip(
        reason=(
            "Currently, model embeddings are going to undergo a major"
            " refactor."
        )
    )
...
class TFGPTJModelLanguageGenerationTest(unittest.TestCase):
    ...
    def test_gptj_sample(self):
        ...
        with tf.device(":/CPU:0"):
            output_ids = model.generate(
                **tokenized, do_sample=True, seed=[42, 0]
            )
    ...
    def _get_beam_search_test_objects(self):
        ...
        expected_output_sentences = [
            (
                "Hello, my dog is a little over a year old and has been"
    ...
    def test_xla_beam_search(self):
        ...
        xla_sentence = tokenizer.batch_decode(
            outputs_xla, skip_special_tokens=True
        )

...
class LukeModelTester:
    def __init__(
        ...
        self.num_entity_classification_labels = (
        ...
        self.num_hidden_states_types = (
            2  # hidden_states and entity_hidden_states
        )
...
class LukeModelTest(ModelTesterMixin, unittest.TestCase):
        ...
        if model_class == LukeForEntitySpanClassification:
            inputs_dict["entity_start_positions"] = torch.zeros(
                (
                    ...
                    self.model_tester.entity_length,
                ),
                dtype=torch.long,
                device=torch_device,
            )
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=LukeConfig, hidden_size=37
        )
    ...
    def test_for_entity_classification(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_for_entity_classification(
            *config_and_inputs
        )
...
class LukeModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_base_model(self):
        ...
        expected_slice = torch.tensor([[0.1457, 0.1044, 0.0174]]).to(
            torch_device
        )
        self.assertTrue(
            torch.allclose(
    ...
    def test_inference_large_model(self):
        ...
        expected_slice = torch.tensor([[0.0466, -0.0106, -0.0179]]).to(
            torch_device
        )
        self.assertTrue(
            torch.allclose(

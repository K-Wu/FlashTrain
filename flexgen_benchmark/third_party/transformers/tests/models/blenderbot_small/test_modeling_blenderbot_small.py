...
class BlenderbotSmallModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(
        ...
        model = (
            BlenderbotSmallModel(config=config)
            .get_decoder()
            .to(torch_device)
            .eval()
        )
...
class BlenderbotSmallModelTest(
    ...
    def test_generate_fp16(self):
        ...
        model = (
            BlenderbotSmallForConditionalGeneration(config)
            .eval()
            .to(torch_device)
        )
...
class Blenderbot90MIntegrationTests(unittest.TestCase):
    ...
    def test_90_generation_from_long_input(self):
        src_text = [
            ...
            " end up sweating and blushing and feel like       i'm going to"
            " throw up.\nand why is that?"
        ]

        model_inputs = self.tokenizer(src_text, return_tensors="pt").to(
            torch_device
        )
        ...
        assert reply in (
            (
                "i don't know. i just feel like i'm going to throw up. it's"
                " not fun."
            ),
            (
                "i'm not sure. i just feel like i've been feeling like i have"
    ...
    def test_90_generation_from_short_input(self):
        model_inputs = self.tokenizer(["sam"], return_tensors="pt").to(
            torch_device
        )
        ...
        assert clean_txt in (
            (
                "have you ever been to a sam club? it's a great club in the"
                " south."
            ),
            (
                "have you ever heard of sam harris? he's an american singer,"

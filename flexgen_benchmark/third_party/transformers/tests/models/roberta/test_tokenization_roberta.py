...
class RobertaTokenizationTest(TokenizerTesterMixin, unittest.TestCase):
    ...
    def test_change_add_prefix_space_and_trim_offsets_args(self):
        for trim_offsets, add_prefix_space in itertools.product(
            ...
            self.assertEqual(
                pre_tokenizer_state["add_prefix_space"], add_prefix_space
            )

            self.assertEqual(
                post_processor_state["add_prefix_space"], add_prefix_space
            )
            self.assertEqual(
                post_processor_state["trim_offsets"], trim_offsets
            )
    ...
    def test_offsets_mapping_with_different_add_prefix_space_and_trim_space_arguments(
        ...
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(
                ...
                text_of_1_token = (  # `hello` is a token in the vocabulary of `pretrained_name`
                    "hello"
                )
                ...
                self.assertEqual(
                    encoding.offset_mapping[0], (0, len(text_of_1_token))
                )
                self.assertEqual(

...
class BertJapaneseTokenizationTest(TokenizerTesterMixin, unittest.TestCase):
    ...
    def test_pickle_sudachi_tokenizer(self):
        tokenizer = self.tokenizer_class(
            self.vocab_file, word_tokenizer_type="sudachi"
        )
        self.assertIsNotNone(tokenizer)
    ...
    def test_sudachi_tokenizer_split_mode_A(self):
        tokenizer = SudachiTokenizer(
            sudachi_dict_type="core", sudachi_split_mode="A"
        )
    ...
    def test_sudachi_tokenizer_split_mode_B(self):
        tokenizer = SudachiTokenizer(
            sudachi_dict_type="core", sudachi_split_mode="B"
        )
    ...
    def test_sudachi_tokenizer_split_mode_C(self):
        tokenizer = SudachiTokenizer(
            sudachi_dict_type="core", sudachi_split_mode="C"
        )
    ...
    def test_sudachi_tokenizer_lower(self):
        tokenizer = SudachiTokenizer(
            do_lower_case=True, sudachi_dict_type="core"
        )
    ...
    def test_sudachi_tokenizer_no_normalize(self):
        tokenizer = SudachiTokenizer(
            normalize_text=False, sudachi_dict_type="core"
        )
    ...
    def test_sudachi_tokenizer_trim_whitespace(self):
        tokenizer = SudachiTokenizer(
            trim_whitespace=True, sudachi_dict_type="core"
        )
    ...
    def test_pickle_jumanpp_tokenizer(self):
        tokenizer = self.tokenizer_class(
            self.vocab_file, word_tokenizer_type="jumanpp"
        )
        self.assertIsNotNone(tokenizer)
    ...
    def test_wordpiece_tokenizer(self):
        ...
        self.assertListEqual(
            tokenizer.tokenize("こんばんは こんばんにちは こんにちは"),
            ["こん", "##ばんは", "[UNK]", "こんにちは"],
        )
...
class BertJapaneseCharacterTokenizationTest(
    TokenizerTesterMixin, unittest.TestCase
):
    tokenizer_class = BertJapaneseTokenizer
    test_rust_tokenizer = False
        ...
        with self.assertLogs("transformers", level="WARNING") as cm:
            ...
            self.assertTrue(
                cm.records[0].message.startswith(
                    ...
                    " the same type as the class this function is called from."
                )
            )

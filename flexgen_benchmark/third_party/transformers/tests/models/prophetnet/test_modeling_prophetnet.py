...
class ProphetNetModelTester:
    def __init__(
        ...
        self.num_hidden_states_types = (
            3  # encoder, decoder_main, decoder_ngram
        )
    ...
    def create_and_check_with_lm_head(
        ...
        model = (
            ProphetNetForConditionalGeneration(config=config)
            .to(torch_device)
            .eval()
        )
        outputs = model(
    ...
    def check_fast_integration(
        ...
        *args,
        ...
        self.parent.assertTrue(
            torch.allclose(
                result.loss,
                torch.tensor(4.5819, device=torch_device),
                atol=1e-3,
            )
        )
    ...
    def check_model_with_attn_mask(
        self, config, input_ids, decoder_input_ids, *args
    ):
        model = ProphetNetModel(config=config)
        model.to(torch_device)
        model.eval()
        ...
        outputs_no_mask = model(
            ...
            decoder_input_ids=decoder_input_ids[:, :5],
        )
    ...
    def check_causal_lm_from_pretrained(
        ...
        *args
        ...
        model = (
            ProphetNetForConditionalGeneration(config).to(torch_device).eval()
        )
...
class ProphetNetStandaloneDecoderModelTest(
    ...
    all_model_classes = (
        (ProphetNetDecoder, ProphetNetForCausalLM)
        if is_torch_available()
        else ()
    )
    all_generative_model_classes = (
...
@require_torch
class ProphetNetStandaloneEncoderModelTest(
...
class ProphetNetModelIntegrationTest(unittest.TestCase):
    ...
    def test_cnndm_inference(self):
        ...
        ARTICLE_TO_SUMMARIZE = (
            "USTC was founded in Beijing by the Chinese Academy of Sciences"
            " (CAS) in September 1958. The Director of CAS, Mr. Guo Moruo was"
            " appointed the first president of USTC. USTC's founding mission"
            " was to develop a high-level science and technology workforce, as"
            " deemed critical for development of China's economy, defense, and"
            " science and technology education. The establishment was hailed"
            ' as "A Major Event in the History of Chinese Education and'
            ' Science." CAS has supported USTC by combining most of its'
            " institutes with the departments of the university. USTC is"
            " listed in the top 16 national key universities, becoming the"
            " youngest national key university.".lower()
        )
        input_ids = tokenizer(
        ...
        EXPECTED_SUMMARIZE_512 = (
            "us ##tc was founded by the chinese academy of sciences ( cas ) in"
            " 1958 . [X_SEP] us ##tc is listed in the top 16 national key"
            " universities ."
        )
        generated_titles = [
            " ".join(
        ...
        EXPECTED_SUMMARIZE_100 = (
            r"us ##tc was founded in beijing by the chinese academy of"
            r" sciences ( cas ) in 1958 . [X_SEP] us ##tc "
            "'"
            " s founding mission was to develop a high - level science and"
            " technology workforce . [X_SEP]"
            ' establishment hailed as " a major event in the history of'
            ' chinese education and science "'
        )
        generated_titles = [
            " ".join(
                tokenizer.convert_ids_to_tokens(g, skip_special_tokens=True)
    ...
    def test_question_gen_inference(self):
        ...
        INPUTS = [
            (
                "Bill Gates [SEP] Microsoft was founded by Bill Gates and Paul"
                " Allen on April 4, 1975."
            ),
            (
                "1975 [SEP] Microsoft was founded by Bill Gates and Paul Allen"
                " on April 4, 1975."
            ),
            (
                "April 4, 1975 [SEP] Microsoft was founded by Bill Gates and"

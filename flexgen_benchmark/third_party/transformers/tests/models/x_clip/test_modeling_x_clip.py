...
class XCLIPVisionModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason=(
            "XCLIPVisionModel has no base class and is not available in"
            " MODEL_MAPPING"
        )
    )
...
class XCLIPTextModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason=(
            "XCLIPTextModel has no base class and is not available in"
            " MODEL_MAPPING"
        )
    )
    def test_save_load_fast_init_from_base(self):
...
class XCLIPModelTester:
    def __init__(
        self, parent, projection_dim=64, mit_hidden_size=64, is_training=True
    ):
        self.parent = parent
...
class XCLIPModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference(self):
        ...
        self.assertTrue(
            torch.allclose(
                outputs.logits_per_video, expected_logits, atol=1e-3
            )
        )

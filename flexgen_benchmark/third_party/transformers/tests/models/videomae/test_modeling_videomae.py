    ...
    def _prepare_for_class(
        ...
        if model_class == VideoMAEForPreTraining:
            ...
            mask = torch.cat(
                [
                    mask,
                    torch.zeros(self.model_tester.seq_length - mask.size(0)),
                ]
            )
            bool_masked_pos = mask.expand(
...
class VideoMAEModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_for_video_classification(self):
        ...
        expected_slice = torch.tensor([0.3669, -0.0688, -0.2421]).to(
            torch_device
        )
    ...
    def test_inference_for_pretraining(self):
        ...
        expected_loss = torch.tensor(
            torch.tensor([0.6469]), device=torch_device
        )
        self.assertTrue(torch.allclose(outputs.loss, expected_loss, atol=1e-4))

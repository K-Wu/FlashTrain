...
class FSMTModelTest(
    ...
    all_model_classes = (
        (FSMTModel, FSMTForConditionalGeneration)
        if is_torch_available()
        else ()
    )
    ...
    def test_model_common_attributes(self):
        ...
        for model_class in self.all_model_classes:
            ...
            self.assertTrue(
                x is None or isinstance(x, nn.modules.sparse.Embedding)
            )
    ...
    def test_advanced_inputs(self):
        ...
        decoder_features = model(
            decoder_attention_mask=useless_mask, **inputs_dict
        )[0]
        self.assertTrue(
...
class FSMTModelIntegrationTests(unittest.TestCase):
    ...
    def get_model(self, mname):
        if mname not in self.models_cache:
            self.models_cache[
                mname
            ] = FSMTForConditionalGeneration.from_pretrained(mname).to(
                torch_device
            )
            if torch_device == "cuda":
    ...
    def test_translation_direct(self, pair):
        ...
        input_ids = tokenizer.encode(src_text, return_tensors="pt").to(
            torch_device
        )

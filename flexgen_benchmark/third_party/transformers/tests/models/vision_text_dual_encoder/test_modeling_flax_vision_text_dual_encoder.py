...
class VisionTextDualEncoderMixin:
    ...
    def check_vision_text_output_attention(
        ...
        **kwargs
        ...
        self.assertEqual(
            len(vision_attentions), vision_config.num_hidden_layers
        )
    ...
    def check_pt_flax_equivalence(self, pt_model, fx_model, inputs_dict):
        ...
        for fx_output_loaded, pt_output in zip(
            ...
            self.assert_almost_equals(
                fx_output_loaded, pt_output.numpy(), 4e-2
            )
        ...
        for fx_output, pt_output_loaded in zip(
            fx_outputs[:4], pt_outputs_loaded[:4]
        ):
            self.assert_almost_equals(
                fx_output, pt_output_loaded.numpy(), 4e-2
            )

    def check_equivalence_pt_to_flax(
        self, vision_config, text_config, inputs_dict
    ):
        config = VisionTextDualEncoderConfig.from_vision_text_configs(
            vision_config, text_config
        )
    ...
    def test_pt_flax_equivalence(self):
        ...
        self.check_equivalence_pt_to_flax(
            vision_config, text_config, inputs_dict
        )
        self.check_equivalence_flax_to_pt(
            vision_config, text_config, inputs_dict
        )
    ...
    def test_real_model_save_load_from_pretrained(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dirname:
            ...
            model_1 = FlaxVisionTextDualEncoderModel.from_pretrained(
                tmp_dirname
            )
...
class FlaxViTBertModelTest(VisionTextDualEncoderMixin, unittest.TestCase):
    def get_pretrained_model_and_inputs(self):
        ...
        input_ids = ids_tensor(
            [batch_size, 4], model.config.text_config.vocab_size
        )
...
@require_torch
class FlaxCLIPVisionBertModelTest(
    VisionTextDualEncoderMixin, unittest.TestCase
):
    def get_pretrained_model_and_inputs(self):
        model = FlaxVisionTextDualEncoderModel.from_vision_text_pretrained(
    ...
    def prepare_config_and_inputs(self):
        ...
        bert_model_tester = FlaxBertModelTester(self)
        vision_config_and_inputs = (
            clip_model_tester.prepare_config_and_inputs()
        )
        text_config_and_inputs = bert_model_tester.prepare_config_and_inputs()

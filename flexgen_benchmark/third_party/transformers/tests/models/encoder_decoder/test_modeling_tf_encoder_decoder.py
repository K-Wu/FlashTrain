...
class TFEncoderDecoderMixin:
    ...
    def check_encoder_decoder_model_output_attentions_from_config(
        ...
        **kwargs
        ...
        enc_dec_model.config.output_attentions = (
            True  # model config -> won't work
        )
        outputs_encoder_decoder = enc_dec_model(
            input_ids=input_ids,
            decoder_input_ids=decoder_input_ids,
    ...
    def check_pt_tf_equivalence(self, tf_model, pt_model, tf_inputs_dict):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            tf_model = TFEncoderDecoderModel.from_pretrained(
                tmpdirname, from_pt=True
            )

        self.check_pt_tf_models(tf_model, pt_model, tf_inputs_dict)

    def check_pt_to_tf_equivalence(
        self, config, decoder_config, tf_inputs_dict
    ):
    ...
    def check_tf_to_pt_equivalence(
        self, config, decoder_config, tf_inputs_dict
    ):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            pt_model = EncoderDecoderModel.from_pretrained(
    ...
    def test_encoder_decoder_model_output_attentions_from_config(self):
        input_ids_dict = self.prepare_config_and_inputs()
        self.check_encoder_decoder_model_output_attentions_from_config(
            **input_ids_dict
        )
    ...
    def test_model_save_load_from_pretrained(self):
        ...
        decoder_input_ids = ids_tensor(
            [13, 1], model_2.config.decoder.vocab_size
        )
...
@require_tf
class TFRoBertaEncoderDecoderModelTest(
    TFEncoderDecoderMixin, unittest.TestCase
):
    def setUp(self):
...
@require_tf
class TFRembertEncoderDecoderModelTest(
    TFEncoderDecoderMixin, unittest.TestCase
):
    def setUp(self):
...
class TFEncoderDecoderModelSaveLoadTests(unittest.TestCase):
    ...
    def test_encoder_decoder_save_load_from_encoder_decoder(self):
        ...
        encoder_decoder_orig = TFEncoderDecoderModel(
    ...
    def test_encoder_decoder_from_pretrained(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dirname:
            ...
            encoder = TFAutoModel.from_pretrained(
                "bert-base-uncased", name="encoder"
            )
            ...
            pretrained_encoder_dir = os.path.join(
                tmp_dirname, "pretrained_encoder"
            )
            pretrained_decoder_dir = os.path.join(
                tmp_dirname, "pretrained_decoder"
            )

...
class FlaxEncoderDecoderMixin:
    ...
    def check_encoder_decoder_model_from_pretrained(
        ...
        **kwargs
        ...
        enc_dec_model = (
            FlaxEncoderDecoderModel.from_encoder_decoder_pretrained(**kwargs)
        )
        outputs_encoder_decoder = enc_dec_model(
            input_ids=input_ids,
            decoder_input_ids=decoder_input_ids,
        ...
        **kwargs
        ...
        with tempfile.TemporaryDirectory() as enc_tmpdir:
            with tempfile.TemporaryDirectory() as dec_tmpdir:
                ...
                enc_dec_model = (
                    FlaxEncoderDecoderModel.from_encoder_decoder_pretrained(
        ...
        self.assertNotEqual(
            config.use_cache, enc_dec_model.config.encoder.use_cache
        )
        self.assertNotEqual(
...
class FlaxGPT2EncoderDecoderModelTest(
    FlaxEncoderDecoderMixin, unittest.TestCase
):
    def get_encoder_decoder_model(self, config, decoder_config):
...
@require_flax
class FlaxBertEncoderDecoderModelTest(
    FlaxEncoderDecoderMixin, unittest.TestCase
):
    def get_encoder_decoder_model(self, config, decoder_config):
        encoder_model = FlaxBertModel(config)

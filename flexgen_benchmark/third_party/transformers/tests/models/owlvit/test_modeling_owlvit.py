...
class OwlViTVisionModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason=(
            "OwlViTVisionModel has no base class and is not available in"
            " MODEL_MAPPING"
        )
    )
...
class OwlViTTextModelTester:
    ...
    def create_and_check_model(self, config, input_ids, input_mask):
        ...
        self.parent.assertEqual(
            result.last_hidden_state.shape,
            (
                self.batch_size * self.num_queries,
                self.seq_length,
                self.hidden_size,
            ),
        )
...
class OwlViTTextModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason=(
            "OwlViTTextModel has no base class and is not available in"
            " MODEL_MAPPING"
        )
    )
    def test_save_load_fast_init_from_base(self):
...
class OwlViTForObjectDetectionTester:
    ...
    def create_and_check_model(
        self, config, pixel_values, input_ids, attention_mask
    ):
        ...
        pred_boxes_size = (
            ...
            (
                ...
                // self.vision_model_tester.patch_size
            )
            ** 2,
            4,
        )
...
class OwlViTForObjectDetectionTest(ModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason=(
            "Test_save_load_fast_init_from_base is tested in individual model"
            " tests"
        )
    )
    def test_save_load_fast_init_from_base(self):
...
class OwlViTModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference(self):
        ...
        self.assertTrue(
            torch.allclose(
                outputs.logits_per_image, expected_logits, atol=1e-3
            )
        )
    ...
    def test_inference_object_detection(self):
        ...
        self.assertEqual(
            outputs.pred_boxes.shape, torch.Size((1, num_queries, 4))
        )

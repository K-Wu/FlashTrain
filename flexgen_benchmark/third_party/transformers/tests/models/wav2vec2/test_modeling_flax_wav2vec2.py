...
def _test_wav2vec2_with_lm_invalid_pool(in_queue, out_queue, timeout):
    ...
    try:
        ...
        resampled_audio = librosa.resample(
            sample["audio"]["array"], 48_000, 16_000
        )
        ...
        input_values = processor(
            resampled_audio, return_tensors="np"
        ).input_values
...
class FlaxWav2Vec2UtilsTest(unittest.TestCase):
    ...
    def test_compute_perplexity(self):
        ...
        ppl = FlaxWav2Vec2GumbelVectorQuantizer._compute_perplexity(
            probs, mask
        )
    ...
    def test_sample_negatives(self):
        ...
        features = (
            np.arange(sequence_length * hidden_size) // hidden_size
        ).reshape(
        ...
        for negative in negatives:
            self.assertTrue(
                ((negative - features.reshape(negative.shape)) == 0).sum()
                == 0.0
            )
...
class FlaxWav2Vec2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_pretrained(self):
        ...
        inputs_dict = feature_extractor(
            input_speech, return_tensors="np", padding=True
        )

    ...
    def create_and_check_batch_inference(self, config, input_values, *args):
        ...
        for i in range(input_values.shape[0]):
            ...
            self.parent.assertTrue(
                np.allclose(output, batch_output, atol=1e-3)
            )
...
class TFWav2Vec2ModelTest(TFModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (TFWav2Vec2Model, TFWav2Vec2ForCTC) if is_tf_available() else ()
    )
    ...
    def test_hidden_states_output(self):
        ...
        def check_hidden_states_output(config, inputs_dict, model_class):
            ...
            self.assertListEqual(
                ...
                [
                    self.model_tester.output_seq_length,
    ...
    @unittest.skip(
        reason=(
            "Dataset conversion goes OOM and crashes with the default options!"
        )
    )
    ...
    @unittest.skip(
        reason="Training goes OOM and crashes with the default options!"
    )

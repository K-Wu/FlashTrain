...
def _test_wav2vec2_with_lm_invalid_pool(in_queue, out_queue, timeout):
    ...
    try:
        ...
        input_values = processor(
            resampled_audio, return_tensors="pt"
        ).input_values
        ...
        with CaptureLogger(
            ...
            transcription = processor.batch_decode(
                logits.cpu().numpy(), pool
            ).text

        unittest.TestCase().assertIn(
            "Falling back to sequential decoding.", cl.out
        )
...
class Wav2Vec2ModelTester:
    ...
    def prepare_config_and_inputs(self):
        input_values = floats_tensor(
            [self.batch_size, self.seq_length], scale=1.0
        )
    ...
    def check_seq_classifier_loss(self, config, input_values, *args):
        ...
        input_lengths = [input_values.shape[-1] // i for i in [4, 2, 1]]
        labels = ids_tensor(
            (input_values.shape[0], 1), len(model.config.id2label)
        )
...
class Wav2Vec2ModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_model_with_adapter(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_model_with_adapter(
            *config_and_inputs
        )
    ...
    def test_retain_grad_hidden_states_attentions(self):
        ...
        labels = ids_tensor(
            (input_values.shape[0], output_lengths[0] - 2),
                    ...
                    if any([x in name for x in uniform_init_parms]):
                        self.assertTrue(
                            ...
                            msg=(
                                ...
                                f" {model_class} seems not properly"
                                " initialized"
                            ),
                        )
                    else:
...
class Wav2Vec2RobustModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_model_for_pretraining(self):
        ...
        mask_time_indices = torch.from_numpy(mask_time_indices).to(
            torch_device
        )
...
class Wav2Vec2UtilsTest(unittest.TestCase):
    ...
    def test_sample_negatives(self):
        ...
        self.assertTrue(
            negatives.shape
            == (num_negatives, batch_size, sequence_length, hidden_size)
        )
...
class Wav2Vec2ModelIntegrationTest(unittest.TestCase):
    def _load_datasamples(self, num_samples):
        ...
        speech_samples = ds.sort("id").filter(
            lambda x: x["id"]
            in [f"1272-141231-000{i}" for i in range(num_samples)]
        )[:num_samples]["audio"]
    ...
    def test_inference_ctc_normal_batched(self):
        ...
        EXPECTED_TRANSCRIPTIONS = [
            ...
            (
                "sweat covered brion's body trickling into the tight lowing"
                " cloth that was the only garment he wore"
            ),
        ]
        self.assertListEqual(predicted_trans, EXPECTED_TRANSCRIPTIONS)
    ...
    def test_inference_ctc_robust_batched(self):
        ...
        EXPECTED_TRANSCRIPTIONS = [
            "a man said to the universe sir i exist",
            (
                "sweat covered brion's body trickling into the tight loin"
                " cloth that was the only garment he wore"
            ),
            (
                "the cut on his chest still dripping blood the ache of his"
                " overstrained eyes even the soaring arena around him with the"
                " thousands of spectators were trivialities not worth thinking"
                " about"
            ),
            (
                "his instant panic was followed by a small sharp blow high on"
                " his chest"
            ),
        ]
        self.assertListEqual(predicted_trans, EXPECTED_TRANSCRIPTIONS)
    ...
    def test_inference_integration(self):
        ...
        inputs_dict = feature_extractor(
            input_speech, return_tensors="pt", padding=True
        )
        ...
        self.assertTrue(
            torch.allclose(
                cosine_sim_masked, expected_cosine_sim_masked, atol=1e-3
            )
        )
    ...
    def test_inference_pretrained(self):
        ...
        self.assertTrue(
            cosine_sim_masked.mean().item()
            - 5 * cosine_sim_masked_rand.mean().item()
            > 0
        )
    ...
    def test_loss_pretraining(self):
        ...
        num_codevectors = (
            model.config.num_codevectors_per_group
            * model.config.num_codevector_groups
        )
        diversity_loss = (
    ...
    def test_inference_keyword_spotting(self):
        ...
        inputs = processor(
            input_data["speech"], return_tensors="pt", padding=True
        )
    ...
    def test_inference_intent_classification(self):
        ...
        self.assertListEqual(
            predicted_ids_action.tolist(), expected_labels_action
        )
        self.assertListEqual(
            predicted_ids_object.tolist(), expected_labels_object
        )
        self.assertListEqual(
            predicted_ids_location.tolist(), expected_labels_location
        )

        self.assertTrue(
            torch.allclose(
                predicted_logits_action, expected_logits_action, atol=1e-2
            )
        )
        self.assertTrue(
            torch.allclose(
                predicted_logits_object, expected_logits_object, atol=1e-2
            )
        )
        self.assertTrue(
            torch.allclose(
    ...
    def test_inference_speaker_identification(self):
        ...
        with torch.no_grad():
            for example in input_data["speech"]:
                ...
                output = model(
                    input.input_values.to(torch_device), attention_mask=None
                )
    ...
    def test_phoneme_recognition(self):
        ...
        EXPECTED_TRANSCRIPTIONS = [
            ...
            (
                "s w ɛ t k ʌ v ɚ d b ɹ iː ɔ n z b ɑː d i t ɹ ɪ k l ɪ ŋ ɪ n t ə"
                " ð ə t aɪ t l oɪ n k l ɑː θ ð æ w ʌ z ð ɪ oʊ n l i ɡ ɑːɹ m ə"
                " n t h iː w ɔːɹ"
            ),
            (
                "ð ə k aɪ t ɔ n h ɪ z tʃ ɛ s t s t ɪ l d ɹ ɪ p ɪ ŋ b l ʌ d ð ɪ"
                " eɪ k ʌ v h ɪ z oʊ v ɚ s t ɹ eɪ n d aɪ z iː v ə n ð ə s ɔːɹ ɹ"
                " ɪ ŋ ɐ ɹ iː n ɐ ɚ ɹ aʊ n d h ɪ m w ɪ ð ə θ aʊ z ə n d z ʌ v s"
                " p ɛ k t eɪ ɾ ɚ z w ɜː t ɹ ɪ v ɪ æ l ᵻ ɾ i z n ɑː t w ɜː θ θ"
                " ɪ ŋ k ɪ ŋ ɐ b aʊ t"
            ),
            (
                "h ɪ z ɪ n s t ə n t v p æ n ɪ k w ʌ z f ɑː l oʊ d b aɪ ɐ s m"
                " ɔː l ʃ ɑːɹ p b l oʊ h aɪ ɔ n h ɪ z tʃ ɛ s t"
            ),
        ]

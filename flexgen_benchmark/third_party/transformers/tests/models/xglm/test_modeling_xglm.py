...
class XGLMModelTester:
    ...
    def create_and_check_xglm_model(
        ...
        self.parent.assertEqual(
            len(result.past_key_values), config.num_hidden_layers
        )
...
class XGLMModelTest(
    ...
    all_model_classes = (
        (XGLMModel, XGLMForCausalLM) if is_torch_available() else ()
    )
    all_generative_model_classes = (
        (XGLMForCausalLM,) if is_torch_available() else ()
    )
    fx_compatible = True
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=XGLMConfig, n_embd=37
        )
    ...
    def test_batch_generation(self):
        ...
        expected_output_sentence = [
            (
                "Hello, my dog is a little bit of a shy one, but he is very"
                " friendly"
            ),
...
class XGLMModelLanguageGenerationTest(unittest.TestCase):
    ...
    def test_xglm_sample(self):
        ...
        EXPECTED_OUTPUT_STR = (
            "Today is a nice day and the sun is shining. A nice day with warm"
            " rainy"
        )
    ...
    def test_xglm_sample_max_time(self):
        ...
        duration = datetime.datetime.now() - start
        self.assertGreater(
            duration, datetime.timedelta(seconds=1.25 * MAX_TIME)
        )

...
class FlaxXGLMModelTester:
    ...
    def check_use_cache_forward(
        ...
        past_key_values = model.init_cache(
            input_ids.shape[0], max_decoder_length
        )
        attention_mask = jnp.ones(
            (input_ids.shape[0], max_decoder_length), dtype="i4"
        )
...
class FlaxXGLMModelTest(
    ...
    all_generative_model_classes = (
        (FlaxXGLMForCausalLM,) if is_flax_available() else ()
    )
    ...
    def test_batch_generation(self):
        ...
        expected_string = [
            (
                "Hello this is a long string of questions, but I'm not sure"
                " if I'm"
            ),
        ...
        for model_class in self.all_model_classes:
            with self.subTest(model_class.__name__):
                ...
                for batch_idx, start_index in enumerate(rnd_start_indices):
                    ...
                    prepared_inputs_dict["attention_mask"][
                ...
                fx_outputs_loaded = fx_model_loaded(

...
class TFXGLMModelTest(TFModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (TFXGLMModel, TFXGLMForCausalLM) if is_tf_available() else ()
    )
    all_generative_model_classes = (
        (TFXGLMForCausalLM,) if is_tf_available() else ()
    )
    test_onnx = False
...
class TFXGLMModelLanguageGenerationTest(unittest.TestCase):
    ...
    def test_xglm_sample(self):
        ...
        EXPECTED_OUTPUT_STR = (
            "Today is a nice day and warm evening here over Southern Alberta!!"
            " Today when they closed schools due"
        )
    ...
    def test_lm_generate_xglm_left_padding(self):
        ...
        expected_output_string = (
            "Today is a beautiful day and I am so glad that we have the"
        ...
        output_ids = model.generate(
            **input_ids, **generation_kwargs, max_length=28
        )
        output_strings = tokenizer.batch_decode(
            output_ids, skip_special_tokens=True
        )

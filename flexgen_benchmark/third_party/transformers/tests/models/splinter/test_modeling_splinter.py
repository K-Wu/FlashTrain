    ...
    def prepare_config_and_inputs(self):
        ...
        if self.use_labels:
            start_positions = ids_tensor(
                [self.batch_size, self.num_questions],
...
class SplinterModelIntegrationTest(unittest.TestCase):
    ...
    def test_splinter_pretraining_loss_with_padding(self):
        ...
        end_positions = torch.tensor([7, 12], dtype=torch.long)
        question_positions = torch.tensor([[1, 5]], dtype=torch.long)
        start_positions_with_padding = torch.tensor(
            [[7, 10, 0]], dtype=torch.long
        )
        end_positions_with_padding = torch.tensor([7, 12, 0], dtype=torch.long)
        question_positions_with_padding = torch.tensor(
            [[1, 5, 0]], dtype=torch.long
        )
        output = model(
            input_ids,
            start_positions=start_positions,
        ...
        self.assertAlmostEqual(
            output.loss.item(), output_with_padding.loss.item(), 4
        )
    ...
    def test_splinter_pretraining_prepare_question_positions(self):
        ...
        output_with_positions = model(
            input_ids, question_positions=question_positions
        )
        self.assertTrue(
            (
        ...
        self.assertTrue(
            (
                output_without_positions.end_logits
                == output_with_positions.end_logits
            ).all()
        )

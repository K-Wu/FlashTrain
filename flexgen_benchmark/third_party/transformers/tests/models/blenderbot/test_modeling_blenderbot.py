...
class BlenderbotModelTester:
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = BlenderbotEncoder.from_pretrained(tmpdirname).to(
                torch_device
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = BlenderbotDecoder.from_pretrained(tmpdirname).to(
                torch_device
            )
...
class BlenderbotModelTest(
    ...
    def test_generate_fp16(self):
        ...
        model = (
            BlenderbotForConditionalGeneration(config).eval().to(torch_device)
        )
...
class Blenderbot3BIntegrationTests(unittest.TestCase):
    ...
    def test_generation_from_short_input_same_as_parlai_3B(self):
        ...
        src_text = (
            ...
            " end up sweating and blushing and feel like i'm going to throw"
            " up.\nand why is that?"
        )
        ...
        assert (
            "I think it's because we are so worried about what people think"
            " of us."
            == reply.strip()
        )
        del model
...
class BlenderbotStandaloneDecoderModelTest(
    ...
    all_model_classes = (
        (BlenderbotDecoder, BlenderbotForCausalLM)
        if is_torch_available()
        else ()
    )
    all_generative_model_classes = (

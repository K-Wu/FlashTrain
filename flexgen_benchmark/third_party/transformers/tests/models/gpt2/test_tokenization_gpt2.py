...
class GPT2TokenizationTest(TokenizerTesterMixin, unittest.TestCase):
    ...
    def test_rust_and_python_full_tokenizers(self):
        ...
        input_bpe_tokens = [14, 15, 10, 9, 3, 2, 15, 19]
        self.assertListEqual(
            rust_tokenizer.convert_tokens_to_ids(input_tokens),
            input_bpe_tokens,
        )
    ...
    def test_padding_if_pad_token_set_slow(self):
        ...
        out_s = tokenizer(
            s, padding="max_length", max_length=30, return_tensors="np"
        )
        out_s2 = tokenizer(
            s2, padding=True, truncate=True, return_tensors="np"
        )
        out_p = tokenizer(
            *p, padding="max_length", max_length=60, return_tensors="np"
        )
        out_p2 = tokenizer(
            p2, padding=True, truncate=True, return_tensors="np"
        )
    ...
    def test_special_tokens_mask_input_pairs_and_bos_token(self):
        ...
        tokenizers = [
            self.get_tokenizer(do_lower_case=False, add_bos_token=True)
        ]
...
class OPTTokenizationTest(unittest.TestCase):
    def test_serialize_deserialize_fast_opt(self):
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            "facebook/opt-350m", from_slow=True
        )
    ...
    def test_fast_slow_equivalence(self):
        tokenizer = AutoTokenizer.from_pretrained(
            "facebook/opt-350m", use_slow=True
        )
        text = "A photo of a cat"

...
class LevitModelTester:
    ...
    def create_and_check_model(self, config, pixel_values, labels):
        ...
        for _ in range(4):
            height = floor(
                ((height + 2 * self.padding - self.kernel_size) / self.stride)
                + 1
            )
            width = floor(
                ((width + 2 * self.padding - self.kernel_size) / self.stride)
                + 1
            )
...
class LevitModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_image_classification_head(self):
        ...
        expected_slice = torch.tensor([1.0448, -0.3745, -1.8317]).to(
            torch_device
        )

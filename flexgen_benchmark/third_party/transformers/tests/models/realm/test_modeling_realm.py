...
class RealmModelTester:
    ...
    def create_and_check_embedder(
        ...
        self.parent.assertEqual(
            result.projected_score.shape,
            (self.batch_size, self.retriever_proj_size),
        )
    ...
    def create_and_check_scorer(
        ...
        self.parent.assertEqual(
            result.relevance_score.shape,
            (self.batch_size, self.num_candidates),
        )
        self.parent.assertEqual(
            result.query_score.shape,
            (self.batch_size, self.retriever_proj_size),
        )
        self.parent.assertEqual(
...
class RealmModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_training(self):
        ...
        input_ids, token_type_ids, input_mask, scorer_encoder_inputs = inputs[
            0:4
        ]
        ...
        inputs_dict = {
            ...
            "relevance_score": floats_tensor(
                [
                    ...
                    self.model_tester.num_candidates,
                ]
            ),
        }
...
class RealmModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_embedder(self):
        ...
        self.assertTrue(
            torch.allclose(output[:, :3], expected_slice, atol=1e-4)
        )
    ...
    def test_inference_encoder(self):
        ...
        self.assertTrue(
            torch.allclose(output[1, :2, :2], expected_slice, atol=1e-4)
        )

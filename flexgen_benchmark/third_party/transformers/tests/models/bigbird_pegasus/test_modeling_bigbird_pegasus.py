...
class BigBirdPegasusModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(
        ...
        model = (
            BigBirdPegasusModel(config=config)
            .get_decoder()
            .to(torch_device)
            .eval()
        )
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = BigBirdPegasusEncoder.from_pretrained(tmpdirname).to(
                torch_device
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = BigBirdPegasusDecoder.from_pretrained(tmpdirname).to(
                torch_device
            )
    ...
    def create_and_check_model(self, config, inputs_dict):
        ...
        result = model(
            input_ids, decoder_input_ids=decoder_input_ids, use_cache=True
        )
...
class BigBirdPegasusModelTest(
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=BigBirdPegasusConfig
        )
    ...
    def test_generate_fp16(self):
        ...
        model = (
            BigBirdPegasusForConditionalGeneration(config)
            .eval()
            .to(torch_device)
        )
    ...
    def _check_batched_forward(self, attn_type, tolerance=1e-3):
        ...
        attention_mask = torch.tensor(
            [
                [1] * 3 * chunk_length + [0] * chunk_length,
                [1] * 4 * chunk_length,
            ],
            device=torch_device,
            dtype=torch.long,
...
class BigBirdPegasusModelIntegrationTests(unittest.TestCase):
    ...
    def test_seq_to_seq_generation(self):
        ...
        EXPECTED_LEP = (
            "motivated by some recent studies on the light cp - odd higgs"
            " boson @xmath0 in non - minimal supersymmetric models, we"
            " investigate the rare @xmath1-decays @xmath2 ( @xmath3 ) in the"
            " two higgs doublet model ( 2hdm ), the nearly minimal"
            " supersymmetric standard model ( nmssm ), the next - to - minimal"
            " supersymmetric standard model ( nmssm ) and the minimal"
            " supersymmetric standard model ( mssm ).<n> we find that the"
            " branching ratios of @xmath4 can reach @xmath5 in 2hdm, @xmath6"
            " in nmssm and @xmath7 in mssm, which are at the level of @xmath8"
            " in 2hdm, @xmath9 in nmssm and @xmath10 in mssm, respectively.<n>"
            " these rates can be significantly enhanced in new physics models"
            " which lie within the expected sensitivity of the gigaz option of"
            " the international linear collider ( ilc ). <n> = # 1,nucl. <n>"
            " phys. <n> b * # 1"
        )

        EXPECTED_MAGNET = (
            "a positive, nonsaturating and dominantly linear magnetoresistance"
            " can appear within quite wide magnetic - field range in the"
            " surface state of a topological insulator having a positive and"
            " finite effective g - factor. this linear magnetoresistance shows"
            " up in the system of high carrier concentration and low mobility"
            " when electrons are in extended states and spread over many"
            " smeared landau levels, and persists up to room temperature,"
            " providing a possible mechanism for the recently observed linear"
            " magnetoresistance in topological insulator bi@xmath0se@xmath1"
            " nanoribbons."
        )

...
class TrajectoryTransformerModelTester:
    ...
    def prepare_config_and_inputs(self):
        trajectories = torch.LongTensor(
            [
                np.random.permutation(self.seq_length)
...
class TrajectoryTransformerModelTest(
    ...
    all_model_classes = (
        (TrajectoryTransformerModel,) if is_torch_available() else ()
    )
    ...
    def test_model_main_input_name(self):
        ...
        self.assertEqual(
            TrajectoryTransformerModel.main_input_name,
    ...
    def test_model_from_pretrained(self):
        for model_name in TRAJECTORY_TRANSFORMER_PRETRAINED_MODEL_ARCHIVE_LIST[
            :1
        ]:
...
class TrajectoryTransformerModelIntegrationTest(unittest.TestCase):
    ...
    def test_prediction(self):
        ...
        self.assertTrue(
            torch.allclose(output_slice, expected_slice, atol=1e-4)
        )

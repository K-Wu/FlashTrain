...
class MarianModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(
        ...
        model = (
            MarianModel(config=config).get_decoder().to(torch_device).eval()
        )
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = MarianEncoder.from_pretrained(tmpdirname).to(
                torch_device
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = MarianDecoder.from_pretrained(tmpdirname).to(
                torch_device
            )
...
class MarianModelTest(
    ...
    def test_share_encoder_decoder_embeddings(self):
        ...
        for model_class in self.all_model_classes:
            model = model_class(config)
            self.assertIs(
                model.get_encoder().embed_tokens,
...
class MarianIntegrationTest(unittest.TestCase):
    ...
    expected_text = [
        ...
        (
            "Jetzt kann ich die 100 Wörter des Deutschen vergessen, die ich"
            " kenne."
        ),
...
class TestMarian_EN_DE_More(MarianIntegrationTest):
    ...
    def test_forward(self):
        ...
        model_inputs = self.tokenizer(
            src, text_target=tgt, return_tensors="pt"
        ).to(torch_device)
...
class TestMarian_MT_EN(MarianIntegrationTest):
    ...
    tgt = "en"
    src_text = [
        "Billi messu b'mod ġentili, Ġesù fejjaq raġel li kien milqut bil -"
        " marda kerha tal - ġdiem."
    ]
    expected_text = [
        "Touching gently, Jesus healed a man who was affected by the sad"
        " disease of leprosy."
    ]

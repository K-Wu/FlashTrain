...
class FlaxPegasusModelTester:
    ...
    def prepare_config_and_inputs_for_common(self):
        ...
        eos_tensor = np.expand_dims(
            np.array([self.eos_token_id] * self.batch_size), 1
        )
...
def prepare_pegasus_inputs_dict(
    ...
    if attention_mask is None:
        attention_mask = np.not_equal(input_ids, config.pad_token_id).astype(
            np.int8
        )
...
class FlaxPegasusModelTest(FlaxModelTesterMixin, unittest.TestCase):
    ...
    def test_pegasus_xsum_summary(self):
        ...
        tgt_text = [
            (
                "California's largest electricity provider has turned off"
                " power to hundreds of thousands of customers."
            ),
            (
                "Pop group N-Dubz have revealed they were surprised to get"
                " four nominations for this year's Mobo Awards."
            ),
        ]

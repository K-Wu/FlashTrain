...
class TFLayoutLMv3ModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        bbox = ids_tensor(
            [self.batch_size, self.text_seq_length, 4], self.range_bbox
        )
        bbox = bbox.numpy()
        ...
        if self.use_input_mask:
            input_mask = random_attention_mask(
                [self.batch_size, self.text_seq_length]
            )
    ...
    def create_and_check_model(
        ...
        result = model(
            input_ids, bbox=bbox, pixel_values=pixel_values, training=False
        )
            ...
            if getattr(model, "hf_compute_loss", None):
                ...
                if "labels" in prepared_for_class:
                    ...
                    if len(labels.shape) > 1 and labels.shape[1] != 1:
                        labels[0] = -100
                        prepared_for_class["labels"] = tf.convert_to_tensor(
                            labels
                        )
                        loss = model(input_ids, **prepared_for_class)[0]
                        self.assertTrue(
...
class TFLayoutLMv3ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        pixel_values = feature_extractor(
            images=image, return_tensors="tf"
        ).pixel_values

        input_ids = tf.constant([[1, 2]])
        bbox = tf.expand_dims(
            tf.constant([[1, 2, 3, 4], [5, 6, 7, 8]]), axis=0
        )

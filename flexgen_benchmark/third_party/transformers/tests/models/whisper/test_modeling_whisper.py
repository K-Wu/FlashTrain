...
class WhisperModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        decoder_input_ids = torch.tensor(
            self.batch_size * [[self.decoder_start_token_id]],
            device=torch_device,
        )
    ...
    def create_and_check_model_forward(
        self, config, inputs_dict, freeze_encoder=False
    ):
    ...
    def create_and_check_decoder_model_past_large_inputs(
        ...
        model = (
            WhisperModel(config=config).get_decoder().to(torch_device).eval()
        )
        ...
        next_tokens = ids_tensor(
            (self.batch_size, 3), config.vocab_size
        ).clamp(2)
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = WhisperEncoder.from_pretrained(tmpdirname).to(
                torch_device
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = WhisperDecoder.from_pretrained(tmpdirname).to(
                torch_device
            )
...
class WhisperModelTest(
        ...
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            if config.is_encoder_decoder:
                ...
                decoder_seq_length = getattr(
                    self.model_tester, "decoder_seq_length", 1
                )
    ...
    def test_attention_outputs(self):
        ...
        decoder_key_length = getattr(
            self.model_tester, "decoder_key_length", 1
        )
        ...
        for model_class in self.all_model_classes:
            ...
            subsampled_encoder_seq_length = (
                model._get_feat_extract_output_lengths(encoder_seq_length)
            )
            subsampled_encoder_key_length = (
                model._get_feat_extract_output_lengths(encoder_key_length)
            )
    ...
    def _check_outputs(
        ...
        subsampled_seq_length = (
            self.model_tester.get_subsampled_output_lengths(seq_length)
        )
...
class WhisperModelIntegrationTests(unittest.TestCase):
    ...
    def _load_datasamples(self, num_samples):
        ...
        speech_samples = ds.sort("id").select(range(num_samples))[
            :num_samples
        ]["audio"]
    ...
    def test_tiny_logits_librispeech(self):
        ...
        self.assertTrue(
            torch.allclose(
                logits[0][0, 0, :30].cpu(), EXPECTED_LOGITS, atol=1e-4
            )
        )
        ...
        self.assertTrue(
            torch.allclose(
                head_logits[0, 0, :30].cpu(), EXPECTED_GENERATION, atol=1e-4
            )
        )
    ...
    def test_tiny_en_generation(self):
        ...
        EXPECTED_TRANSCRIPT = (
            "<|startoftranscript|><|notimestamps|> Mr. Quilter is the apostle"
            " of the middle classes, and we are glad to"
        )
    ...
    def test_tiny_generation(self):
        ...
        EXPECTED_TRANSCRIPT = (
            "<|startoftranscript|><|en|><|transcribe|><|notimestamps|> Mr."
            " Quilter is the apostle of the middle classes and we are glad"
        )
        self.assertEqual(transcript, EXPECTED_TRANSCRIPT)

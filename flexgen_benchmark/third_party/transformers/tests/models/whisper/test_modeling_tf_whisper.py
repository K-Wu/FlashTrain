...
class TFWhisperModelTest(TFModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (TFWhisperModel, TFWhisperForConditionalGeneration)
        if is_tf_available()
        else ()
    )
    ...
    def test_lm_head_model_random_no_beam_search_generate(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            with self.assertRaises(ValueError):
                ...
                model.generate(
                    input_features, do_sample=False, num_return_sequences=2
                )

            # num_return_sequences > 1, sample
            self._check_generated_ids(
                model.generate(
                    input_features, do_sample=True, num_return_sequences=2
                )
            )
...
def _load_datasamples(num_samples):
    ...
    speech_samples = ds.sort("id").select(range(num_samples))[:num_samples][
        "audio"
    ]
...
def _test_large_batched_generation(in_queue, out_queue, timeout):
    ...
    try:
        ...
        unittest.TestCase().assertTrue(
            np.allclose(generated_ids, EXPECTED_LOGITS)
        )
...
class TFWhisperModelIntegrationTests(unittest.TestCase):
    ...
    def test_tiny_logits_librispeech(self):
        ...
        # fmt: on
        self.assertTrue(
            np.allclose(logits[0][0, 0, :30], EXPECTED_LOGITS, atol=1e-4)
        )
    ...
    def test_tiny_xla_generation(self):
        ...
        generated_ids_xla = xla_generate(
            input_features, num_beams=5, max_length=20
        )

...
class BeamSearchTester:
    ...
    def prepare_inputs(self):
        input_ids = ids_tensor(
            (self.batch_size * self.num_beams, self.sequence_length),
            self.vocab_size,
        )
        ...
        next_scores, _ = (
            -floats_tensor((self.batch_size, 2 * self.num_beams)).to(
                torch_device
            )
    ...
    def check_beam_scorer_update(
        ...
        expected_output_indices = (
            ...
            + (
                torch.arange(
                    self.num_beams * self.batch_size, device=torch_device
                )
                // self.num_beams
            )
            * self.num_beams
        ...
        for batch_idx in range(self.batch_size):
            correct_idx = (
                batch_idx * self.num_beams + next_indices[batch_idx, 1]
            )
            self.parent.assertListEqual(
            ...
            self.parent.assertListEqual(
                ...
                torch.tensor(
                    beam_scorer._beam_hyps[batch_idx].beams[0][2]
                ).tolist(),
            )
...
class ConstrainedBeamSearchTester:
    ...
    def prepare_inputs(self):
        ...
        scores_for_all_vocab, _ = (
            -floats_tensor(
                (self.batch_size * self.num_beams, self.vocab_size)
            ).to(torch_device)
        ).sort(descending=True)
        return (
            input_ids,
    ...
    def check_constrained_beam_scorer_update(
        ...
        for constraint in self.constraints:
            ...
            token_ids = (
                token_ids[0] if isinstance(token_ids[0], list) else token_ids
            )
        ...
        for batch_idx in range(self.batch_size):
            ...
            self.parent.assertListEqual(
                input_ids[correct_idx].tolist(),
                constrained_beam_scorer._beam_hyps[batch_idx]
                .beams[0][1]
                .tolist(),
            )
    ...
    def check_constrained_beam_scorer_finalize(
        ...
        for output, constraint in [
            (s, c) for s in sequences for c in constraints
        ]:

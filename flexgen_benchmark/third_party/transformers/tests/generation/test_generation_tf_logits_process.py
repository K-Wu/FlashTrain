...
class TFLogitsProcessorTest(unittest.TestCase):
    ...
    def test_min_length_dist_processor(self, use_xla):
        ...
        if use_xla:
            min_dist_processor = tf.function(
                min_dist_processor, jit_compile=True
            )
        ...
        self.assertFalse(
            tf.math.reduce_any(
                tf.math.is_inf(scores_before_min_length)
            ).numpy()
        )
    ...
    def test_temperature_dist_warper(self, use_xla):
        ...
        warped_prob_sharp = tf.nn.softmax(
            temp_dist_warper_sharper(input_ids, tf.identity(scores), cur_len),
            axis=-1,
        )
        warped_prob_smooth = tf.nn.softmax(
            temp_dist_warper_smoother(input_ids, tf.identity(scores), cur_len),
            axis=-1,
        )
        ...
        self.assertLess(
            ...
            tf.math.reduce_max(warped_prob_sharp[1, :]),
        )
        self.assertGreater(
            tf.math.reduce_min(probs[1, :]),
            tf.math.reduce_min(warped_prob_sharp[1, :]),
        )
    ...
    def test_repetition_penalty_dist_process(self, use_xla):
        ...
        mask = tf.cast(
            tf.constant([10 * [0], 5 * [0] + [1] + 4 * [0]]), tf.bool
        )
    ...
    def test_top_k_dist_warper(self, use_xla):
        ...
        self.assertListEqual(
            tf.math.is_inf(scores[0]).numpy().tolist(),
            7 * [True] + 3 * [False],
        )
        self.assertListEqual(
        ...
        self.assertListEqual(
            tf.math.reduce_sum(tf.where(scores == 0.0, 1, 0), axis=-1)
            .numpy()
            .tolist(),
            [0, 0],
        )
    ...
    def test_top_p_dist_warper(self, use_xla):
        ...
        ramp_logits = np.broadcast_to(
            np.arange(vocab_size, dtype=np.float32)[None, :],
    ...
    def test_forced_bos_token_logits_processor(self, use_xla):
        ...
        logits_processor = TFForcedBOSTokenLogitsProcessor(
    ...
    def test_suppress_tokens_at_begin_logits_processor(self, use_xla):
        ...
        self.assertTrue(
            tf.math.reduce_all(
                tf.math.is_inf(
                    tf.gather(scores, begin_suppress_tokens, axis=1)
                )
            )
        )
    ...
    def test_suppress_tokens_logits_processor(self, use_xla):
        ...
        keep_tokens = [
            i for i in range(vocab_size) if i not in suppress_tokens
        ]
        ...
        self.assertFalse(
            tf.math.reduce_any(
                tf.math.is_inf(tf.gather(scores, keep_tokens, axis=1))
            )
        )
    ...
    def test_force_tokens_logits_processor(self, use_xla):
        ...
        logits_processor = TFForceTokensLogitsProcessor(
            force_token_map=force_token_map
        )
        if use_xla:
            logits_processor = tf.function(logits_processor, jit_compile=True)

...
class GenerationTesterMixin:
    ...
    def test_greedy_generate(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            self.assertListEqual(
                output_greedy.tolist(), output_generate.tolist()
            )
    ...
    def test_greedy_generate_dict_outputs_use_cache(self):
        for model_class in self.all_generative_model_classes:
            ...
            for output in (output_greedy, output_generate):
                self._check_outputs(
                    output, input_ids, model.config, use_cache=True
                )
    ...
    def test_sample_generate(self):
        for model_class in self.all_generative_model_classes:
            ...
            self.assertListEqual(
                output_sample.tolist(), output_generate.tolist()
            )
    ...
    def test_sample_generate_dict_output(self):
        for model_class in self.all_generative_model_classes:
            ...
            self.assertListEqual(
                output_generate.sequences.tolist(),
                output_sample.sequences.tolist(),
            )
    ...
    def test_beam_search_generate(self):
        for model_class in self.all_generative_model_classes:
            ...
            self.assertListEqual(
                output_generate.tolist(), output_beam_search.tolist()
            )
    ...
    def test_beam_search_generate_dict_output(self):
        for model_class in self.all_generative_model_classes:
            ...
            if model.config.is_encoder_decoder:
                ...
                self.assertIsInstance(
                    output_beam_search, BeamSearchDecoderOnlyOutput
                )
                self.assertIsInstance(
    ...
    def test_beam_sample_generate(self):
        for model_class in self.all_generative_model_classes:
            ...
            self.assertListEqual(
                output_generate.tolist(), output_beam_sample.tolist()
            )
    ...
    def test_beam_sample_generate_dict_output(self):
        for model_class in self.all_generative_model_classes:
            ...
            if model.config.is_encoder_decoder:
                ...
                self.assertIsInstance(
                    output_beam_sample, BeamSampleDecoderOnlyOutput
                )
                self.assertIsInstance(
    ...
    def test_group_beam_search_generate(self):
        for model_class in self.all_generative_model_classes:
            ...
            (
                beam_kwargs,
                beam_scorer,
            ) = self._get_diverse_beam_scorer_and_kwargs(
                input_ids.shape[0], max_length
            )
            (
                output_generate,
    ...
    def test_constrained_beam_search_generate(self):
        for model_class in self.all_generative_model_classes:
            ...
            (
                beam_kwargs,
                beam_scorer,
            ) = self._get_constrained_beam_scorer_and_kwargs(
                input_ids.shape[0],
                max_length,
                constraints,
                num_return_sequences=1,
            ...
            for generation_output in output_generate:
                self._check_sequence_inside_sequence(
                    force_tokens, generation_output
                )
    ...
    def test_generate_with_head_masking(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            if not set(head_masking.keys()) < set(
            ...
            for attn_name, (name, mask) in zip(
                ...
                self.assertEqual(
                    sum([w.sum().item() for w in attn_weights]), 0.0
                )
    ...
    def _check_attentions_for_generate(
        ...
        self.assertEqual(
            len(attentions), (max_length - min_length) * num_beam_groups
        )
...
class UtilsFunctionsTest(unittest.TestCase):
    ...
    def test_top_k_top_p_filtering(self):
        ...
        non_inf_output = output[output != -float("inf")].to(
            device=torch_device
        )
        non_inf_idx = (
    ...
    def test_top_k_top_p_filtering_with_filter_value(self):
        ...
        output = top_k_top_p_filtering(
            logits, top_k=4, top_p=0.5, filter_value=0.0
        )
...
class GenerationIntegrationTests(unittest.TestCase):
    ...
    def test_diverse_beam_search(self):
        ...
        self.assertListEqual(
            ...
            [
                (
                    "The couple announced the birth of their son, Silas"
                    " Randall Timberlake, in a statement. Silas was the middle"
                    " name of Timberlake's maternal grandfather Bill Bomar."
                    " Randall is the musician's own middle name, as well as"
                    " his father's first. It is the first baby for both of"
                    " them."
                ),
                (
                    "Justin Timberlake and Jessica Biel have a son. The baby"
                    " is named Silas Randall Timberlake. It is the first child"
                    " for both. The couple announced the pregnancy in January."
                    " The name Silas is the middle name of Timberlake's"
                    " maternal grandfather. It's also his own middle name."
                ),
            ],
        )
    ...
    def test_contrastive_search_bart(self):
        ...
        self.assertListEqual(
            generated_text,
            [
                "Liana Barrientos, 39, pleaded not guilty to charges related"
                " to false marriage statements. Prosecutors say she married at"
                " least 10 times, sometimes within two weeks of each other."
                " She is accused of being part of an immigration scam to get"
                " permanent residency. If convicted, she faces up to four"
                " years in"
            ],
        )
    ...
    def test_contrastive_search_t5(self):
        ...
        generated_text = t5_tokenizer.batch_decode(
            outputs, skip_special_tokens=True
        )
        ...
        self.assertListEqual(
            generated_text,
            [
                "Liana Barrientos has been married 10 times, nine of them in"
                " the Bronx. Her husbands filed for permanent residence after"
                " the marriages, prosecutors say."
            ],
        )
    ...
    def test_contrastive_search_opt(self):
        article = (
            "A chat between a curious human and the Statue of"
            " Liberty.\n\nHuman: What is your name?\nStatue: I am the Statue"
            " of Liberty.\nHuman: Where do you live?\nStatue: New York"
            " City.\nHuman: How long have you lived there?"
        )

        opt_tokenizer = GPT2Tokenizer.from_pretrained("facebook/opt-1.3b")
        opt_model = OPTForCausalLM.from_pretrained("facebook/opt-1.3b").to(
            torch_device
        )
        ...
        generated_text = opt_tokenizer.batch_decode(
            outputs, skip_special_tokens=True
        )
    ...
    def test_contrastive_search_gptj(self):
        article = (
            "DeepMind Technologies is a British artificial intelligence"
            " subsidiary of Alphabet Inc. and research laboratory founded in"
            " 2010. DeepMind was acquired by Google in 2014. The company is"
            " based"
        )
        ...
        input_ids = tokenizer(article, return_tensors="pt").input_ids.to(
            torch_device
        )
        ...
        self.assertListEqual(
            ...
            [
                "DeepMind Technologies is a British artificial intelligence"
                " subsidiary of Alphabet Inc. and research laboratory founded"
                " in 2010. DeepMind was acquired by Google in 2014. The"
                " company is based in London, United Kingdom with offices in"
                " Mountain View, San Francisco, New York City, Paris, Tokyo,"
                " Seoul, Beijing, Singapore, Tel Aviv, Dublin, Sydney, and"
                " Melbourne.[1]\n\nContents\n\nIn 2010, Google's parent"
                " company, Alphabet, announced a $500 million investment in"
                " DeepMind, with the aim of creating a company that would"
                " apply deep learning to problems in healthcare, energy,"
                " transportation, and other areas.[2]\n\nOn April 23, 2014,"
                " Google announced that it had acquired DeepMind for $400"
                " million in cash and stock.[3] The acquisition was seen as a"
                " way for Google to enter the fast-growing field of artificial"
                " intelligence (AI), which it had so far avoided due to"
                " concerns about ethical and social implications.[4] Google"
                ' co-founder Sergey Brin said that he was "thrilled" to have'
                ' acquired DeepMind, and that it would "help us push the'
                " boundaries of AI even further.\"[5]\n\nDeepMind's founders,"
                " Demis Hassabis and Mustafa Suleyman, were joined by a number"
                " of Google employees"
            ],
        )
    ...
    def test_contrastive_search_gpt2(self):
        ...
        gpt2_model = GPT2LMHeadModel.from_pretrained("gpt2-large").to(
            torch_device
        )
        ...
        generated_text = gpt2_tokenizer.batch_decode(
            outputs, skip_special_tokens=True
        )
        ...
        self.assertListEqual(
            ...
            [
                ...
                " lot of data on its users and uses it to improve its"
                " products, such as Google Now, which helps users find the"
                " information they're looking for on the web. But the company"
                " is not the only one to collect data on its users. Facebook,"
                " for example, has its own facial recognition technology, as"
                " well as a database of millions of photos that it uses to"
                " personalize its News Feed.\n\nFacebook's use of data is a"
                " hot topic in the tech industry, with privacy advocates"
                " concerned about the company's ability to keep users'"
                " information private. In a blog post last year, Facebook CEO"
                ' Mark Zuckerberg said his company would "do our best to be'
                ' transparent about our data use and how we use it."\n\n"We'
                " have made it clear that we do not sell or share your data"
                ' with third parties," Zuckerberg wrote. "If you have'
                " questions or concerns, please reach out to us at"
                ' privacy@facebook.com."\n\nGoogle declined to comment on the'
                " privacy implications of its use of data, but said in a"
    ...
    def test_max_length_backward_compat_greedy(self):
        ...
        model_kwargs = (
            bart_model._prepare_encoder_decoder_kwargs_for_generation(
                input_ids, {}
            )
        )
    ...
    def test_stop_sequence_stopping_criteria(self):
        ...
        self.assertEqual(
            ...
            [
                {
                    "generated_text": (
                        "Hello I believe in in in number number number number"
    ...
    def test_custom_logits_processor(self):
        ...
        logits_processor.append(
            MinLengthLogitsProcessor(min_length=10, eos_token_id=0)
        )
    ...
    def test_max_new_tokens_decoder_only(self):
        ...
        with self.assertRaises(ValueError):
            gpt2_model.generate(
                input_ids=input_ids, max_new_tokens=10, max_length=20
            )
    ...
    def test_generate_encoder_outputs_attention_mask(self):
        ...
        output_sequences_no_mask = model.generate(
        ...
        self.assertListEqual(
            output_sequences_no_mask.tolist(),
            output_sequences_with_mask.tolist(),
        )
    ...
    def test_transition_scores_beam_search_encoder_decoder(self):
        articles = [
            "Justin Timberlake and Jessica Biel, welcome to parenthood.",
            (
                "Michael Phelps is arguably the most decorated Olympian of all"
                " time."
            ),
        ]
        tokenizer = BartTokenizer.from_pretrained(
        ...
        self.assertTrue(
            torch.allclose(
                transition_scores_sum, outputs.sequences_scores, atol=1e-3
            )
        )
    ...
    def test_transition_scores_early_stopping(self):
        ...
        model = AutoModelForSeq2SeqLM.from_pretrained("t5-small").to(
            torch_device
        )
        ...
        self.assertListEqual(
            sum_transition_scores.cpu().tolist(),
            result.sequences_scores.cpu().tolist(),
        )
    ...
    def test_log_scores_sample_decoder_only(self):
        ...
        inputs = tokenizer(articles, return_tensors="pt", padding=True).to(
            torch_device
        )
        ...
        self.assertTrue(
            torch.allclose(gen_probs.cpu(), expected_probs, atol=1e-3)
        )
    ...
    def test_beam_search_example_integration(self):
        ...
        encoder_input_ids = tokenizer(
            encoder_input_str, return_tensors="pt"
        ).input_ids
    ...
    def test_constrained_beam_search(self):
        ...
        self.assertListEqual(
            generated_text,
            [
                "The soldiers were not prepared and didn't know what to do."
                " They had no idea how they would react if the enemy attacked"
                " them, big weapons scared"
            ],
        )
    ...
    def test_constrained_beam_search_mixed(self):
        ...
        self.assertListEqual(
            ...
            [
                (
                    "The soldiers, who had been stationed at the base for more"
                    " than a year before being evacuated screaming scared"
                ),
                (
    ...
    def test_constrained_beam_search_mixin_type_checks(self):
        ...
        with self.assertRaises(ValueError):
            force_words = ["sind"]
            force_words_ids = [
                tokenizer(force_words, return_tensors="pt").input_ids
            ]
            model.generate(
                input_ids,
                force_words_ids=force_words_ids,

...
class LogitsProcessorTest(unittest.TestCase):
    ...
    def test_temperature_dist_warper(self):
        ...
        temp_dist_warper_smoother = FlaxTemperatureLogitsWarper(
            temperature=1.3
        )

        warped_prob_sharp = jax.nn.softmax(
            temp_dist_warper_sharper(input_ids, scores.copy(), cur_len=None),
            axis=-1,
        )
        warped_prob_smooth = jax.nn.softmax(
            temp_dist_warper_smoother(input_ids, scores.copy(), cur_len=None),
            axis=-1,
        )

        # uniform distribution stays uniform
        self.assertTrue(
            jnp.allclose(probs[0, :], warped_prob_sharp[0, :], atol=1e-3)
        )
        self.assertTrue(
            jnp.allclose(probs[0, :], warped_prob_smooth[0, :], atol=1e-3)
        )
    ...
    def test_top_k_dist_warper(self):
        ...
        self.assertListEqual(
            jnp.isinf(scores[0]).tolist(), 7 * [True] + 3 * [False]
        )
        self.assertListEqual(
            jnp.isinf(scores[1]).tolist(),
    ...
    def test_top_p_dist_warper(self):
        ...
        # exp (-inf) => 0
        EXPECTED_FILTERED_DIST = np.array(
            [[0.3, 0.0, 0.0, 0.5], [0.0, 0.3, 0.3, 0.25]]
        )
        self.assertTrue(
            np.allclose(filtered_dist, EXPECTED_FILTERED_DIST, atol=1e-3)
        )
        ...
        top_p_warp = FlaxTopPLogitsWarper(
    ...
    def test_forced_bos_token_logits_processor(self):
        ...
        logits_processor = FlaxForcedBOSTokenLogitsProcessor(
    ...
    def test_processor_list(self):
        ...
        bos_dist_proc = FlaxForcedBOSTokenLogitsProcessor(
            bos_token_id=bos_token_id
        )
    ...
    def test_processor_list_jitted(self):
        ...
        scores_comp = jitted_run_processor_list(
            input_ids, scores_comp, cur_len
        )

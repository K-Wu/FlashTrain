...
class LogitsProcessorTest(unittest.TestCase):
    ...
    def test_temperature_dist_warper(self):
        ...
        self.assertTrue(
            torch.allclose(probs[0, :], warped_prob_sharp[0, :], atol=1e-3)
        )
        self.assertTrue(
    ...
    def test_top_k_dist_warper(self):
        ...
        self.assertListEqual(
            torch.isinf(scores[0]).tolist(), 7 * [True] + 3 * [False]
        )
        self.assertListEqual(
            torch.isinf(scores[1]).tolist(),
    ...
    def test_typical_dist_warper(self):
        ...
        # make sure at least 2 tokens are kept
        typical_warp = TypicalLogitsWarper(
            0.7, min_tokens_to_keep=2, filter_value=0.0
        )
    ...
    def test_normalization(self):
        ...
        ones = torch.ones(
            scores.shape[0], device=torch_device, dtype=torch.float
        )

    ...
    def test_generate_tf_function_export(self):
        ...
        class DummyModel(tf.Module):
            ...
            @tf.function(
                input_signature=(
                    tf.TensorSpec(
                        (None, max_length), tf.int32, name="input_ids"
                    ),
                    tf.TensorSpec(
                        (None, max_length), tf.int32, name="attention_mask"
                    ),
                ),
                jit_compile=True,
            )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            serving_func = tf.saved_model.load(tmp_dir).signatures[
                "serving_default"
            ]
            for batch_size in range(1, len(dummy_input_ids) + 1):
                inputs = {
                    ...
                    "attention_mask": tf.constant(
                        dummy_attention_masks[:batch_size]
                    ),
                }

...
class TFModelTesterMixin:
        ...
        if return_labels:
            if model_class in get_values(TF_MODEL_FOR_MULTIPLE_CHOICE_MAPPING):
                ...
                *get_values(TF_MODEL_FOR_SPEECH_SEQ_2_SEQ_MAPPING),
                ...
                inspect.signature(model_class.call).parameters
            ):
                inputs_dict["labels"] = tf.zeros(
                    (
        ...
        for model_class in self.all_model_classes:
            ...
            for k, v in serving_outputs.items():
                ...
                if isinstance(v, tuple):
                    self.assertTrue(
                        all(isinstance(elem, tf.Tensor) for elem in v)
                    )
                elif v is not None:
        ...
        # Allow `list` (e.g. `TransfoXLModelOutput.mems` is a list of tensors.)
            ...
            if attributes is not None:
                ...
                self.assertEqual(
                    ...
                    len(tf_outputs),
                    (
                        f"{name}: The tuple `names` should have the same"
            ...
            self.assertLessEqual(
                ...
                tol,
                (
                    f"{name}: Difference between torch and tf is"
                    f" {max_diff} (>= {tol})."
                ),
            )
        else:
            raise ValueError(
                "`tf_outputs` should be an instance of `tf.Tensor`, a"
    ...
    def test_pt_tf_model_equivalence(self):
        ...
        for model_class in self.all_model_classes:
            ...
            tf_inputs_dict_with_labels = self._prepare_for_class(
                ...
                return_labels=True
                if "labels"
                in inspect.signature(model_class.call).parameters.keys()
                else False,
            )
            ...
            if tf_inputs_dict_with_labels:
                self.check_pt_tf_models(
                    tf_model, pt_model, tf_inputs_dict_with_labels
                )
    ...
    def test_compile_tf_model(self):
        ...
        for model_class in self.all_model_classes:
            ...
            outputs = tf.keras.layers.Dense(
                2, activation="softmax", name="outputs"
            )(hidden_states)
            ...
            def check_attentions_validity(attentions):
                ...
                for t in attentions:
                    self.assertLess(
                        (
                            tf.math.reduce_sum(
                                tf.cast(tf.math.is_nan(t), tf.float32)
                            )
                        ).numpy(),
                        (tf.size(t) / 4).numpy(),
                ...
                self.assertAlmostEqual(
                    tf.math.reduce_sum(attentions[0][..., 0, :, :]).numpy(),
                    0.0,
                )
                self.assertNotEqual(
    ...
    def test_hidden_states_output(self):
        ...
        def check_hidden_states_output(config, inputs_dict, model_class):
            ...
            if model.config.is_encoder_decoder:
                ...
                self.assertEqual(
                    len(decoder_hidden_states), expected_num_layers
                )
                self.assertListEqual(
    ...
    def test_model_common_attributes(self):
        ...
        speech_in_text_out_models = get_values(
            TF_MODEL_FOR_SPEECH_SEQ_2_SEQ_MAPPING
        )
    ...
    def test_numpy_arrays_inputs(self):
        ...
        for model_class in self.all_model_classes:
            ...
            self.assert_outputs_same(
                output_for_dict_input, output_for_kw_input
            )
    ...
    def test_lm_head_model_random_no_beam_search_generate(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            with self.assertRaises(ValueError):
                ...
                model.generate(
                    input_ids, do_sample=False, num_return_sequences=2
                )
        ...
        for model_class in self.all_generative_model_classes:
            ...
            if model.config.is_encoder_decoder:
                self.assertIsInstance(
                    output_greedy, TFGreedySearchEncoderDecoderOutput
                )
                self.assertIsInstance(
                    output_sample, TFSampleEncoderDecoderOutput
                )
            else:
                self.assertIsInstance(
                    output_greedy, TFGreedySearchDecoderOnlyOutput
                )
    ...
    def test_lm_head_model_beam_search_generate_dict_outputs(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            if model.config.is_encoder_decoder:
                ...
                self.assertIsInstance(
                    output_beam_search, TFBeamSearchDecoderOnlyOutput
                )
                self.assertIsInstance(
                    output_beam_sample, TFBeamSampleDecoderOnlyOutput
                )
    ...
    def check_keras_fit_results(
        self, val_loss1, val_loss2, atol=1e-2, rtol=1e-3
    ):
        self.assertTrue(
            np.allclose(val_loss1, val_loss2, atol=atol, rtol=rtol)
        )
    ...
    def test_keras_fit(self):
        ...
        for model_class in self.all_model_classes:
            ...
            label_names = possible_label_cols.intersection(
                set(prepared_for_class)
            )
    ...
    def test_dataset_conversion(self):
        ...
        for model_class in self.all_model_classes:
            ...
            tf_inputs_dict["extra_unwanted_column"] = list(
                tf_inputs_dict.values()
            )[
                0
            ...
            if isinstance(test_batch, tf.Tensor):
                ...
                self.assertEqual(
                    len(test_batch), len(input_dataset.features) - 1
                )
            ...
            if (
                ...
                self.assertEqual(
                    ...
                    len(input_dataset.features) - 1,
                )
                if isinstance(test_batch, dict):
                    self.assertNotIn("extra_unwanted_column", test_batch)
                if isinstance(test_batch_labels, dict):
                    self.assertNotIn(
                        "extra_unwanted_column", test_batch_labels
                    )
        ...
        def _generate_and_check_results(model, config, inputs_dict):
            if "input_ids" in inputs_dict:
                ...
                if config.pad_token_id is not None:
                    ...
                    new_pad_token = None
                inputs = tf.where(
                    inputs != config.pad_token_id, inputs, new_pad_token
                )
        ...
        for bad_word_ids in bad_words_ids:
            ...
            for generated_ids_slice in generated_ids:
                ...
                for i in range(len(bad_word_ids), len(generated_ids_slice)):
                    # if tokens match
                    if (
                        generated_ids_slice[i - len(bad_word_ids) : i]
                        == bad_word_ids
                    ):
                        return True
        return False
...
def random_attention_mask(shape, rng=None, name=None, dtype=None):
    attn_mask = ids_tensor(
        shape, vocab_size=2, rng=None, name=None, dtype=dtype
    )
    # make sure that at least one token is attended to for each batch
    attn_mask = tf.concat(
        [attn_mask[:, :-1], tf.ones_like(attn_mask[:, -1:], dtype=dtype)],
        axis=-1,
    )
    return attn_mask
...
class UtilsFunctionsTest(unittest.TestCase):
    ...
    def test_xla_stable_softmax(self):
        ...
        boolean_mask = tf.convert_to_tensor(
            [[1] * (n_tokens - masked_tokens) + [0] * masked_tokens],
            dtype=tf.int32,
        )
    ...
    def test_shard_checkpoint(self):
        ...
        with self.subTest("Test sharding, no weights bigger than max size"):
            ...
            shard1 = [
                weights_dict["dense/kernel:0"],
                weights_dict["dense_1/kernel:0"],
            ]
            shard2 = [
    ...
    def test_special_layer_name_sharding(self):
        ...
        model = TFRagModel.from_pretrained(
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            for max_size in ["150kB", "150kiB", "200kB", "200kiB"]:
                ...
                ref_model = TFRagModel.from_pretrained(
                    tmp_dir, retriever=retriever
                )
                for p1, p2 in zip(model.weights, ref_model.weights):
                    assert np.allclose(p1.numpy(), p2.numpy())
    ...
    def test_checkpoint_sharding_local(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for max_size in ["150kB", "150kiB", "200kB", "200kiB"]:
                ...
                shards_found = set(
                    f for f in os.listdir(tmp_dir) if f.endswith(".h5")
                )
    ...
    def test_save_pretrained_signatures(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            model_loaded = tf.keras.models.load_model(
                f"{tmp_dir}/saved_model/1"
            )
            self.assertTrue(
                "serving_default" in list(model_loaded.signatures.keys())
            )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            model.save_pretrained(
                tmp_dir,
                saved_model=True,
                signatures={"custom_signature": serving_fn},
            )
            model_loaded = tf.keras.models.load_model(
                f"{tmp_dir}/saved_model/1"
            )
            self.assertTrue(
                "custom_signature" in list(model_loaded.signatures.keys())
            )
...
class TFModelPushToHubTester(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        try:
            delete_repo(
                token=cls._token, repo_id="valid_org/test-model-tf-org"
            )
        except HTTPError:
            pass
    ...
    def test_push_to_hub(self):
        ...
        self.assertIn(
            "Uploading README.md to __DUMMY_TRANSFORMERS_USER__/test-model-tf",
            cl.out,
        )
    ...
    def test_push_to_hub_in_organization(self):
        ...
        model.push_to_hub(
            "valid_org/test-model-tf-org", use_auth_token=self._token
        )

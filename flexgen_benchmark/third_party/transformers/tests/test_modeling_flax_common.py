...
class FlaxModelTesterMixin:
            ...
            def recursive_check(tuple_object, dict_object):
                if isinstance(tuple_object, (List, Tuple)):
                    ...
                    self.assert_almost_equals(
                        jnp.nan_to_num(tuple_object),
                        jnp.nan_to_num(dict_object),
                        1e-5,
                    )
        ...
        for model_class in self.all_model_classes:
            ...
            for name, type_ in types.items():
                self.assertEquals(
                    type_,
                    jnp.float32,
                    msg=f"param {name} is not initialized in fp32.",
                )
    ...
    def test_to_bf16(self):
        ...
        for model_class in self.all_model_classes:
            ...
            types = flatten_dict(
                jax.tree_util.tree_map(lambda x: x.dtype, params)
            )
            ...
            )  # choose a random param
            mask = {
                path: path != key for path in flat_params
            }  # don't cast the key
            ...
            for name, type_ in types.items():
                if name == key:
                    self.assertEqual(
                        type_,
                        jnp.float32,
                        msg=f"param {name} should be in fp32.",
                    )
                else:
                    self.assertEqual(
                        type_,
                        jnp.bfloat16,
                        msg=f"param {name} is not in bf16.",
                    )
            ...
            for name, type_ in types.items():
                if name == key:
                    self.assertEqual(
                        type_,
                        jnp.float16,
                        msg=f"param {name} should be in fp16.",
                    )
                else:
                    self.assertEqual(
                        type_, jnp.float32, msg=f"param {name} is not in fp32."
    ...
    def test_headmasking(self):
        ...
        for model_class in self.all_model_classes:
            ...
            def _check_attentions_validity(attentions):
                ...
                attentions = [
                    np.where(np.isnan(t), 0.0, t) for t in attentions
                ]
                ...
                self.assertAlmostEqual(
                    attentions[-1][..., -2, :, :].sum(), 0.0
                )
                self.assertNotEqual(attentions[-1][..., -1, :, :].sum(), 0.0)

            if model.config.is_encoder_decoder:
                raise NotImplementedError(
                    "The test has not been implemented for encoder-decoder"
                    " models yet."
                )
            else:
    ...
    def test_no_automatic_init(self):
        ...
        for model_class in self.all_model_classes:
            ...
            for k, v in flatten_dict(
                unfreeze(model.params_shape_tree)
            ).items():
                self.assertEqual(
                    v.shape,
                    flat_params[k].shape,
    ...
    def test_from_pretrained_with_no_automatic_init(self):
        ...
        for model_class in self.all_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                model.save_pretrained(tmpdirname)
                model, params = model_class.from_pretrained(
                    tmpdirname, _do_init=False
                )
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                params = model.init_weights(
                    model.key, model.input_shape, params=params
                )
                # Check if all required parmas are loaded
                _assert_all_params_initialised(model, params)
    ...
    def test_checkpoint_sharding_from_hub(self):
        ...
        for p1, p2 in zip(
            ...
            flatten_dict(ref_model.params).values(),
        ):
                ...
                for shard_file, size in shard_to_size.items():
                    ...
                    if size >= max_size_int + 50000:
                        with open(shard_file, "rb") as state_f:
                            state_file = from_bytes(
                                FlaxBertModel, state_f.read()
                            )
...
class FlaxModelPushToHubTester(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        try:
            delete_repo(
                token=cls._token, repo_id="valid_org/test-model-flax-org"
            )
        except HTTPError:
            pass
    ...
    def test_push_to_hub_in_organization(self):
        ...
        model = FlaxBertModel(config)
        model.push_to_hub(
            "valid_org/test-model-flax-org", use_auth_token=self._token
        )
...
class FlaxModelUtilsTest(unittest.TestCase):
    def test_model_from_pretrained_subfolder(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            model_loaded = FlaxBertModel.from_pretrained(

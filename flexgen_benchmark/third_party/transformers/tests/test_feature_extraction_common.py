...
def prepare_image_inputs(
    ...
    for i in range(feature_extract_tester.batch_size):
        if equal_resolution:
            ...
            width, height = np.random.choice(
                np.arange(
                    min_resolution, feature_extract_tester.max_resolution
                ),
                2,
            )
        image_inputs.append(
    ...
    if not numpify and not torchify:
        ...
        image_inputs = [
            Image.fromarray(np.moveaxis(image, 0, -1))
...
class FeatureExtractionSavingTestMixin:
    ...
    def test_feat_extract_to_json_file(self):
        ...
        self.assertEqual(
            feat_extract_second.to_dict(), feat_extract_first.to_dict()
        )
...
class FeatureExtractorPushToHubTester(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        try:
            delete_repo(
                token=cls._token, repo_id="test-dynamic-feature-extractor"
            )
        except HTTPError:
            pass
    ...
    def test_push_to_hub_in_organization(self):
        ...
        # Reset repo
        delete_repo(
            token=self._token, repo_id="valid_org/test-feature-extractor"
        )

...
class TFLogitsProcessor:
    ...
    def __call__(
        ...
        raise NotImplementedError(
            f"{self.__class__} is an abstract class. Only classes inheriting"
            ...
            if len(function_args) > 3:
                if not all(
                    ...
                    raise ValueError(
                        ...
                        f" {list(function_args.keys())} for"
                        f" {processor.__class__} are passed to the logits"
                        " processor."
                    )
...
class TFTopKLogitsWarper(TFLogitsWarper):
    ...
    def __init__(
        ...
        if not isinstance(top_k, int) or top_k <= 0:
            raise ValueError(
                "`top_k` has to be a strictly positive integer, but is"
                f" {top_k}"
            )
    ...
    def __call__(
        ...
        indices_to_remove = (
            scores < tf.math.top_k(scores, k=top_k)[0][..., -1:]
        )
...
class TFTopPLogitsWarper(TFLogitsWarper):
    ...
    def __call__(
        ...
        cumulative_probs = tf.math.cumsum(
            stable_softmax(topk_scores, axis=-1), axis=-1
        )
        ...
        score_mask = tf.concat(
            (
                tf.ones([score_mask.shape[0], 1], dtype=tf.bool),
                score_mask[:, :-1],
            ),
            axis=-1,
        )
        ...
        score_mask = tf.concat(
            (
                tf.ones(
                    [score_mask.shape[0], self.min_tokens_to_keep],
                    dtype=tf.bool,
                ),
...
class TFMinLengthLogitsProcessor(TFLogitsProcessor):
    ...
    def __init__(self, min_length: int, eos_token_id: int):
        if not isinstance(min_length, int) or min_length < 0:
            raise ValueError(
                "`min_length` has to be a positive integer, but is"
                f" {min_length}"
            )

        if not isinstance(eos_token_id, int) or eos_token_id < 0:
            raise ValueError(
                "`eos_token_id` has to be a positive integer, but is"
                f" {eos_token_id}"
            )
...
class TFRepetitionPenaltyLogitsProcessor(TFLogitsProcessor):
    ...
    def __init__(self, penalty: float):
        if not isinstance(penalty, float) or not (penalty > 0):
            raise ValueError(
                "`penalty` has to be a strictly positive float, but is"
                f" {penalty}"
            )
    ...
    def _create_score_penalties(
        ...
        logit_penalties = tf.where(
            logit_penalties < 0, self.penalty, logit_penalties
        )
        ...
        indexable_prev_input_ids = tf.concat(
            (
                tf.expand_dims(
                    tf.repeat(tf.range(batch_size), seq_len), axis=-1
                ),
    ...
    def __call__(
        ...
        score_penalties = self._create_score_penalties(
...
class TFNoBadWordsLogitsProcessor(TFLogitsProcessor):
    ...
    def __init__(self, bad_words_ids: List[List[int]], eos_token_id: int):
        ...
        if any(
            not isinstance(bad_word_ids, list)
        ...
        if any(
            ...
            raise ValueError(
                "Each list in `bad_words_ids` has to be a list of positive"
                f" integers, but is {bad_words_ids}."
            )
        ...
        self.bad_word_seqs_len = tf.convert_to_tensor(
            bad_word_seqs_len, dtype=tf.int32
        )
    ...
    def _calc_row_banned_bad_tokens(
        ...
        def _tokens_match(bad_word_seq_number):
            def _len_one():
                ...
                return tf.cond(
                    tf.math.equal(
                        self.bad_word_seqs_len[bad_word_seq_number], 1
                    ),
                ...
                return tf.cond(
                    tf.math.reduce_all(
                        tf.math.equal(
                            ...
                            self.bad_word_seqs_ids[
                                bad_word_seq_number, :compare_len
                            ],
                        )
                    ),
                    lambda: tf.ones((), dtype=tf.bool),
    ...
    def __call__(
        ...
        def _get_row_updated_score(row_inputs: Tuple[tf.Tensor]) -> tf.Tensor:
            ...
            banned_tokens = self._calc_row_banned_bad_tokens(
                row_input_ids[:cur_len]
            )
...
class TFNoRepeatNGramLogitsProcessor(TFLogitsProcessor):
    ...
    def calc_banned_ngram_tokens(self, input_ids, num_hypos, cur_len):
        ...
        for idx in range(num_hypos):
            ...
            for ngram in zip(
                *[gen_tokens[i:] for i in range(self.ngram_size)]
            ):
    ...
    def __call__(
        ...
        if not tf.executing_eagerly():
            raise NotImplementedError(
                "TFNoRepeatNGramLogitsProcessor is only implemented for eager"
                " execution."
            )

        batch_size, vocab_size = scores.shape
        banned_tokens = self.calc_banned_ngram_tokens(
...
class TFForcedBOSTokenLogitsProcessor(TFLogitsProcessor):
    ...
    def __init__(self, bos_token_id: int):
        if bos_token_id < 0:
            raise ValueError(
                "The forced bos token id  must be a non-negative integer, got"
                f" {bos_token_id}"
            )
        ...
        if cur_len == 1:
            ...
            if self.bos_token_id > 0:
                scores = tf.concat(
                    (
                        tf.broadcast_to(
                            -float("inf"), (batch_size, self.bos_token_id)
                        ),
                        scores,
                    ),
                    axis=-1,
...
class TFForcedEOSTokenLogitsProcessor(TFLogitsProcessor):
    ...
    def __init__(self, max_length: int, eos_token_id: int):
        ...
        if eos_token_id < 0:
            raise ValueError(
                "The forced eos token id must be a non-negative integer, got"
                f" {eos_token_id}"
            )
        self.eos_token_id = eos_token_id
        ...
        if cur_len == self.max_length - 1:
            ...
            if self.eos_token_id > 0:
                scores = tf.concat(
                    (
                        tf.broadcast_to(
                            -float("inf"), (batch_size, self.eos_token_id)
                        ),
                        scores,
                    ),
                    axis=-1,
...
class TFForceTokensLogitsProcessor(TFLogitsProcessor):
    ...
    def __init__(self, force_token_map: List[List[int]]):
        ...
        self.force_token_array = tf.convert_to_tensor(
            force_token_array, dtype=tf.int32
        )
    ...
    def __call__(
        ...
        def _force_token(generation_idx):
            ...
            new_scores = tf.ones_like(scores, dtype=scores.dtype) * -float(
                "inf"
            )
            indices = tf.stack(
                (tf.range(batch_size), tf.tile([current_token], [batch_size])),
                axis=1,
            )
            updates = tf.zeros((batch_size,), dtype=scores.dtype)
            new_scores = tf.tensor_scatter_nd_update(
                new_scores, indices, updates
            )
            return new_scores

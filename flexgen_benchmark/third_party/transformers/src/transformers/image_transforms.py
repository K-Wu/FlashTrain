...
def to_pil_image(
    ...
    do_rescale = (
        isinstance(image.flat[0], float) if do_rescale is None else do_rescale
    )
    if do_rescale:
...
def normalize(
    ...
    if isinstance(image, PIL.Image.Image):
        warnings.warn(
            (
                "PIL.Image.Image inputs are deprecated and will be removed in"
                " v4.26.0. Please use numpy arrays instead."
            ),
            FutureWarning,
        )
    ...
    if isinstance(mean, Iterable):
        if len(mean) != num_channels:
            raise ValueError(
                f"mean must have {num_channels} elements if it is an iterable,"
                f" got {len(mean)}"
            )
    else:
    ...
    if isinstance(std, Iterable):
        if len(std) != num_channels:
            raise ValueError(
                f"std must have {num_channels} elements if it is an iterable,"
                f" got {len(std)}"
            )
    else:
...
def center_crop(
    ...
    if not isinstance(size, Iterable) or len(size) != 2:
        raise ValueError(
            "size must have 2 elements representing the height and width of"
            " the output image"
        )

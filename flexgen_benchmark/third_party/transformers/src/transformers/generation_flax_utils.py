...
class FlaxGenerationMixin:
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        if decoder_start_token_id is None and self.config.is_encoder_decoder:
            raise ValueError(
                "`decoder_start_token_id` has to be defined for"
    ...
    def _get_logits_warper(
        ...
        if top_k is not None and top_k != 0:
            warpers.append(
                FlaxTopKLogitsWarper(top_k=top_k, min_tokens_to_keep=1)
            )
        if top_p is not None and top_p < 1.0:
            warpers.append(
                FlaxTopPLogitsWarper(top_p=top_p, min_tokens_to_keep=1)
            )
    ...
    def _get_logits_processor(
        ...
        if (
            ...
            processors.append(
                FlaxMinLengthLogitsProcessor(min_length, eos_token_id)
            )
        if forced_bos_token_id is not None:
            processors.append(
                FlaxForcedBOSTokenLogitsProcessor(forced_bos_token_id)
            )
        if forced_eos_token_id is not None:
            processors.append(
                FlaxForcedEOSTokenLogitsProcessor(
    ...
    def _greedy_search(
        ...
        sequences = jnp.full(
            (batch_size, max_length), pad_token_id, dtype=jnp.int32
        )
    ...
    def _beam_search(
        ...
        running_sequences = lax.dynamic_update_slice(
            sequences, input_ids, (0, 0, 0)
        )
        ...
        def beam_search_cond_fn(state):
            ...
            still_open_beam = ~(
                jnp.all(state.is_sent_finished) & early_stopping
            )
        ...
        def beam_search_body_fn(state, input_ids_length=1):
            ...
            model_outputs = model(
                input_token, params=params, **state.model_kwargs
            )
            ...
            log_probs = log_probs + jnp.expand_dims(
                state.running_scores, axis=2
            )
            ...
            did_topk_just_finished = (
                topk_sequences[:, :, state.cur_len] == eos_token_id
            )
            running_topk_log_probs = (
                topk_log_probs + did_topk_just_finished * np.array(-1.0e7)
            )
            ...
            merged_scores = jnp.concatenate(
                [state.scores, topk_log_probs], axis=1
            )
        ...
        scores = jnp.where(
            none_finished[:, None], state.scores, state.running_scores
        )

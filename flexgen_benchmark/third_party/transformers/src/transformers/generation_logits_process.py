...
class NoBadWordsLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, bad_words_ids: List[List[int]], eos_token_id: int):
        ...
        for banned_token_seq in self.bad_words_id_length_greater_than_1:
            if len(banned_token_seq) == 0:
                raise ValueError(
                    f"Banned words token sequences {bad_words_ids} cannot have"
                    " an empty list"
                )
    ...
    def __call__(
        ...
        if (
            ...
            self.static_bad_words_mask = self._calc_static_bad_word_mask(
                scores
            )
        ...
        for idx, batch_banned_tokens in enumerate(banned_tokens):
            for token in batch_banned_tokens:
                ...
                if token <= scores.shape[1]:
                    ...
                    logger.error(
                        f"An invalid bad word ID is defined: {token}. This ID"
                        " is not contained in the vocabulary, and is"
                        " therefore ignored."
                    )

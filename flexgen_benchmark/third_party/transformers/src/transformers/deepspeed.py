...
class HfTrainerDeepSpeedConfig(HfDeepSpeedConfig):
    ...
    def trainer_config_finalize(self, args, model, num_training_steps):
        ...
        if len(self.mismatches) > 0:
            ...
            raise ValueError(
                "Please correct the following DeepSpeed config values that"
                f" mismatch TrainingArguments values:\n{mismatches}\nThe"
                " easiest method is to set these DeepSpeed config values to"
                " 'auto'."
            )
...
def deepspeed_optim_sched(
    ...
    if "optimizer" in config:
        if args.adafactor:
            raise ValueError(
                "--adafactor was passed, but also found `optimizer` configured"
                " in the DeepSpeed config. Only one optimizer can be"
                " configured."
            )
    else:
        if hf_deepspeed_config.is_offload():
            logger.info(
                "Detected ZeRO Offload and non-DeepSpeed optimizers: This"
                " combination should work as long as the custom optimizer has"
                " both CPU and GPU implementation (except LAMB)"
            )
...
def deepspeed_init(
    ...
    if inference:
        ...
        if not hf_deepspeed_config.is_zero3():
            raise ValueError(
                "ZeRO inference only makes sense with ZeRO Stage 3 - please"
                " adjust your config"
            )
    ...
    if resume_from_checkpoint is not None:
        ...
        if len(deepspeed_checkpoint_dirs) > 0:
            ...
            if load_path is None:
                raise ValueError(
                    "[deepspeed] failed to resume from checkpoint"
                    f" {resume_from_checkpoint}"
                )
        else:
            logger.info(
                f"{resume_from_checkpoint} doesn't have deepspeed checkpoints,"
                " doing nothing"
            )

...
class GenerationMixin:
    ...
    def _prepare_input_ids_for_generation(
        ...
        if bos_token_id is None:
            raise ValueError(
                "`bos_token_id` has to be defined when no `input_ids` are"
                " provided."
            )
        return (
    ...
    def _reorder_cache(self, past, beam_idx):
        raise NotImplementedError(
            "Make sure that a `_reorder_cache` function is correctly"
            f" implemented in {self.__class__.__module__} to enable beam"
            f" search for {self.__class__}"
        )
    ...
    def _get_logits_processor(
        ...
        if (
            ...
            if self.config.is_encoder_decoder:
                ...
                raise ValueError(
                    "It's impossible to use `encoder_no_repeat_ngram_size`"
        ...
        for default in default_list:
            for custom in custom_list:
                if type(custom) is type(default):
                    ...
                    raise ValueError(
                        f"A custom {object_type} of type {type(custom)} with"
                        f" values {custom} has been passed to `generate`, but"
                        " it has already been created with the values"
                        f" {default}. {default} has been created by passing"
                        " the corresponding arguments to generate or by the"
                        " model's config default values. If you just want to"
                        f" change the default values of {object_type} consider"
                        " passing them as arguments to `generate` instead of"
                        f" using a custom {object_type}."
                    )
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        if is_group_beam_gen_mode and do_sample is True:
            raise ValueError(
                "Diverse beam search cannot be used in sampling mode. Make"
                " sure that `do_sample` is set to `False`."
            )

        if self.device.type != input_ids.device.type:
            warnings.warn(
                (
                    "You are calling .generate() with the `input_ids` being"
                    " on a device type different than your model's device."
                    f" `input_ids` is on {input_ids.device.type}, whereas the"
                    f" model is on {self.device.type}. You may experience"
                    " unexpected behaviors or slower generation. Please make"
                    " sure that you have put `input_ids` to the correct"
                    " device by calling for example input_ids ="
                    f" input_ids.to('{self.device.type}') before running"
                    " `.generate()`."
                ),
                UserWarning,
            )
        ...
        if is_greedy_gen_mode:
            ...
            if num_return_sequences > 1:
                raise ValueError(
                    "num_return_sequences has to be 1, but is"
                    f" {num_return_sequences} when doing contrastive search."
                )
            ...
            if num_beams % num_beam_groups != 0:
                raise ValueError(
                    "`num_beams` should be divisible by `num_beam_groups` for"
                    " group beam search."
                )
            ...
            if typical_p is not None:
                raise ValueError(
                    "Decoder argument `typical_p` is not supported with beam"
                    " groups."
                )
            ...
            if num_beam_groups is not None and num_beam_groups > 1:
                raise ValueError(
                    "`num_beam_groups` not supported yet for constrained"
                    " generation."
                )
            ...
            if force_words_ids is not None:
                ...
                def typeerror():
                    raise ValueError(
                        "`force_words_ids` has to either be a"
                        " `List[List[List[int]]]` or `List[List[int]]`of"
                        f" positive integers, but is {force_words_ids}."
                    )
    ...
    def contrastive_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if model_kwargs.get("past") is None:
                ...
                if "past_key_values" not in outputs:
                    raise ValueError(
                        ...
                        " `past_key_values` and can therefore **not** be used"
            ...
            if (
                self.config.is_encoder_decoder
                and "encoder_outputs" in model_inputs
            ):
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        if len(stopping_criteria) == 0:
            warnings.warn(
                (
                    "You don't have defined any stopping_criteria, this will"
                    " likely loop forever"
                ),
                UserWarning,
            )
        ...
        if num_beams * batch_size != batch_beam_size:
            raise ValueError(
                ...
                f" {num_beams * batch_size}, but is {batch_beam_size}."
            )

    ...
    def build_shape_dict(name: str, tensor, is_input: bool, seq_len: int):
        if isinstance(tensor, (tuple, list)):
            ...
            axes = {
                [
                    axis
                    for axis, numel in enumerate(tensor.shape)
                    if numel == 1
                ][0]: "batch"
            }
            if is_input:
        ...
        print(
            f"Found {'input' if is_input else 'output'} {name} with shape:"
            f" {axes}"
        )
        return axes
...
def load_graph_from_args(
    ...
    **models_kwargs,
    ...
    if framework == "pt" and not is_torch_available():
        raise Exception(
            "Cannot convert because PyTorch is not installed. Please install"
            " torch first."
        )
    if framework == "tf" and not is_tf_available():
        raise Exception(
            "Cannot convert because TF is not installed. Please install"
            " tensorflow first."
        )
...
def convert_tensorflow(nlp: Pipeline, opset: int, output: Path):
    ...
    try:
        ...
        print(
            f"Using framework TensorFlow: {tf.version.VERSION}, tf2onnx:"
            f" {t2ov}"
        )
        ...
        raise Exception(
            f"Cannot import {e.name} required to convert TF model to ONNX."
            f" Please install {e.name} first. {e}"
        )
...
def convert(
    ...
    **model_kwargs,
    ...
    warnings.warn(
        (
            "The `transformers.convert_graph_to_onnx` package is deprecated"
            " and will be removed in version 5 of Transformers"
        ),
        FutureWarning,
    )
    ...
    if not output.parent.exists():
        ...
        raise Exception(
            f"Folder {output.parent.as_posix()} is not empty, aborting"
            " conversion"
        )
...
def optimize(onnx_model_path: Path) -> Path:
    ...
    print(
        f"Optimized model has been written at {opt_model_path}:"
        f" \N{heavy check mark}"
    )
    print(
        "/!\\ Optimized model contains hardware specific operators which might"
        " not be portable. /!\\"
    )
...
def quantize(onnx_model_path: Path) -> Path:
    ...
    if parse(onnx.__version__) < parse("1.5.0"):
        print(
            "Models larger than 2GB will fail to quantize due to protobuf"
    ...
    # Save model
    print(
        f"Quantized model has been written at {quantized_model_path}:"
        f" \N{heavy check mark}"
    )
    ...
    try:
        ...
        if args.quantize:
            ...
            if args.framework == "tf":
                print(
                    "\t Using TensorFlow might not provide the same"
                    " optimization level compared to PyTorch.\n"
                    "\t For TensorFlow users you can try optimizing the model"
                    " directly through onnxruntime_tools.\n"
                    "\t For more information, please refer to the onnxruntime"
                    " documentation:\n"

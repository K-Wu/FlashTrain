...
class KerasMetricCallback(Callback):
    ...
    def __init__(
        ...
        metric_fn: Callable,
        eval_dataset: Union[
            tf.data.Dataset, np.ndarray, tf.Tensor, tuple, dict
        ],
        ...
        if not isinstance(eval_dataset, tf.data.Dataset):
            if batch_size is None:
                raise ValueError(
                    "When passing data to KerasMetricCallback that is not a"
                    " pre-batched tf.data.Dataset the batch_size argument must"
                    " be set."
                )
        ...
        if label_cols is not None:
            for label in label_cols:
                if label not in input_spec:
                    raise ValueError(
                        f"Label {label} is in label_cols but could not be"
                        " found in the dataset inputs!"
                    )
            ...
            self.use_keras_label = False
            logging.warning(
                "No label_cols specified for KerasMetricCallback, assuming you"
                " want the 'labels' key."
            )
            ...
            logging.warning(
                "No label_cols specified for KerasMetricCallback, assuming you"
                " want the start_positions and end_positions keys."
            )
        else:
            raise ValueError(
                "Could not autodetect label_cols for KerasMetricCallback,"
                " please specify them!"
            )
        if parse(tf.__version__) < parse("2.7"):
            logging.warning(
                "TF versions less than 2.7 may encounter issues with"
                " KerasMetricCallback!"
            )

        self.use_xla_generation = use_xla_generation
        self.generate_kwargs = (
            {} if generate_kwargs is None else generate_kwargs
        )
    ...
    def _postprocess_predictions_or_labels(self, inputs):
        if isinstance(inputs[0], dict):
            ...
            outputs = self._concatenate_batches(
                [tensor.numpy() for tensor in inputs]
            )
        else:
            raise TypeError(
                f"Couldn't handle batch of type {type(inputs[0])}!"
            )
        return outputs
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if not isinstance(metric_output, dict):
            raise TypeError(
                "metric_fn should return a dict mapping metric names to"
                f" values but instead returned {metric_output}"
            )
    ...
    def __init__(
        ...
        **model_card_args,
        ...
        if self.save_strategy == IntervalStrategy.STEPS and (
            ...
            raise ValueError(
                "Please supply a positive integer argument for save_steps when"
                " save_strategy == 'steps'!"
            )

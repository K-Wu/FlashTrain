...
class BatchFeature(UserDict):
    ...
    def __getitem__(self, item: str) -> Union[Any]:
        ...
        if isinstance(item, str):
            ...
            raise KeyError(
                "Indexing with integers is not available when using Python"
        ...
        for key, value in self.items():
            try:
                ...
                if key == "overflowing_values":
                    raise ValueError(
                        "Unable to create tensor returning overflowing values"
                        " of different lengths. "
                    )
                raise ValueError(
                    "Unable to create tensor, you should probably activate"
                    " padding with 'padding=True' to have batched tensors with"
                    " the same length."
                )
    ...
    def to(self, device: Union[str, "torch.device"]) -> "BatchFeature":
        ...
        if (
            ...
            logger.warning(
                f"Attempting to cast a BatchFeature to type {str(device)}."
    ...
    def get_feature_extractor_dict(
        ...
        try:
            ...
            raise EnvironmentError(
                ...
                f" '{resolved_feature_extractor_file}' is not a valid JSON"
                " file."
            )
        ...
        if is_local:
            ...
            logger.info(
                f"loading configuration file {feature_extractor_file} from"
                f" cache at {resolved_feature_extractor_file}"
            )

...
class BatchEncoding(UserDict):
    ...
    def __getitem__(self, item: Union[int, str]) -> Union[Any, EncodingFast]:
        ...
        if isinstance(item, str):
            ...
            raise KeyError(
                "Indexing with integers (to access backend Encoding for a"
                " given batch index) is not available when using Python based"
                " tokenizers"
            )
    ...
    def tokens(self, batch_index: int = 0) -> List[str]:
        ...
        if not self._encodings:
            raise ValueError(
                "tokens() is not available when using non-fast tokenizers"
                " (e.g. instance of a `XxxTokenizerFast` class)."
            )
    ...
    def sequence_ids(self, batch_index: int = 0) -> List[Optional[int]]:
        ...
        if not self._encodings:
            raise ValueError(
                "sequence_ids() is not available when using non-fast"
                " tokenizers (e.g. instance of a `XxxTokenizerFast` class)."
            )
    ...
    def words(self, batch_index: int = 0) -> List[Optional[int]]:
        ...
        if not self._encodings:
            raise ValueError(
                "words() is not available when using non-fast tokenizers (e.g."
                " instance of a `XxxTokenizerFast` class)."
            )
        warnings.warn(
            (
                "`BatchEncoding.words()` property is deprecated and should be"
                " replaced with the identical, but more self-explanatory"
                " `BatchEncoding.word_ids()` property."
            ),
            FutureWarning,
        )
    ...
    def word_ids(self, batch_index: int = 0) -> List[Optional[int]]:
        ...
        if not self._encodings:
            raise ValueError(
                "word_ids() is not available when using non-fast tokenizers"
                " (e.g. instance of a `XxxTokenizerFast` class)."
            )
    ...
    def token_to_sequence(
        ...
        if not self._encodings:
            raise ValueError(
                "token_to_sequence() is not available when using Python based"
                " tokenizers"
            )
    ...
    def token_to_word(
        ...
        if not self._encodings:
            raise ValueError(
                "token_to_word() is not available when using Python based"
                " tokenizers"
            )
        if token_index is not None:
            batch_index = batch_or_token_index
    ...
    def word_to_tokens(
        ...
        if not self._encodings:
            raise ValueError(
                "word_to_tokens() is not available when using Python based"
                " tokenizers"
            )
    ...
    def token_to_chars(
        ...
        if not self._encodings:
            raise ValueError(
                "token_to_chars() is not available when using Python based"
                " tokenizers"
            )
        if token_index is not None:
            batch_index = batch_or_token_index
    ...
    def word_to_chars(
        ...
        if not self._encodings:
            raise ValueError(
                "word_to_chars() is not available when using Python based"
                " tokenizers"
            )
        if word_index is not None:
            batch_index = batch_or_word_index
    ...
    def char_to_word(
        ...
        if not self._encodings:
            raise ValueError(
                "char_to_word() is not available when using Python based"
                " tokenizers"
            )
        if char_index is not None:
            batch_index = batch_or_char_index
    ...
    def convert_to_tensors(
        ...
        if tensor_type == TensorType.TENSORFLOW:
            ...
            if not is_torch_available():
                raise ImportError(
                    "Unable to convert output to PyTorch tensors format,"
            ...
            if not is_flax_available():
                raise ImportError(
                    "Unable to convert output to JAX tensors format, JAX is"
                    " not installed."
                )
        ...
        for key, value in self.items():
            try:
                ...
                if key == "overflowing_tokens":
                    raise ValueError(
                        "Unable to create tensor returning overflowing tokens"
                        " of different lengths. Please see if a fast version"
                        " of this tokenizer is available to have this feature"
                        " available."
                    )
                raise ValueError(
                    "Unable to create tensor, you should probably activate"
                    " truncation and/or padding with 'padding=True'"
                    " 'truncation=True' to have batched tensors with the same"
                    f" length. Perhaps your features (`{key}` in this case)"
                    " have excessive nesting (inputs type `list` where type"
                    " `int` is expected)."
                )
    ...
    def to(self, device: Union[str, "torch.device"]) -> "BatchEncoding":
        ...
        if (
            ...
            logger.warning(
                f"Attempting to cast a BatchEncoding to type {str(device)}."
        ...
        for key, value in kwargs.items():
            ...
            if key in self.SPECIAL_TOKENS_ATTRIBUTES:
                if key == "additional_special_tokens":
                    ...
                    raise TypeError(
                        f"special token {key} has to be either str or"
                        f" AddedToken but got: {type(value)}"
                    )
        ...
        for key, value in special_tokens_dict.items():
            ...
            if key == "additional_special_tokens":
                assert isinstance(value, (list, tuple)) and all(
                    ...
                    f"Tokens {value} for key {key} should all be str or"
                    " AddedToken instances"
                )
                added_tokens += self.add_tokens(value, special_tokens=True)
            else:
                assert isinstance(value, (str, AddedToken)), (
                    f"Token {value} for key {key} should be a str or an"
                    " AddedToken instance"
                )
                added_tokens += self.add_tokens([value], special_tokens=True)
...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
    ...
    def __init__(self, **kwargs):
        ...
        if self.padding_side not in ["right", "left"]:
            raise ValueError(
                "Padding side should be selected between 'right' and 'left',"
                f" current value: {self.padding_side}"
            )
        ...
        if self.truncation_side not in ["right", "left"]:
            raise ValueError(
                "Padding side should be selected between 'right' and 'left',"
                f" current value: {self.truncation_side}"
            )
    ...
    def max_len_single_sentence(self, value) -> int:
        ...
        if (
            ...
            if not self.deprecation_warnings.get(
                ...
                logger.warning(
                    "Setting 'max_len_single_sentence' is now deprecated. This"
                    " value is automatically set up."
                )
    ...
    def max_len_sentences_pair(self, value) -> int:
        ...
        if (
            ...
            if not self.deprecation_warnings.get(
                ...
                logger.warning(
                    "Setting 'max_len_sentences_pair' is now deprecated. This"
                    " value is automatically set up."
                )
        ...
        **kwargs
        ...
        if os.path.isfile(pretrained_model_name_or_path) or is_remote_url(
            ...
            if len(cls.vocab_files_names) > 1:
                raise ValueError(
                    f"Calling {cls.__name__}.from_pretrained() with the path"
                    " to a single file or url is not supported for this"
                    " tokenizer. Use a model identifier or the path to a"
                    " directory instead."
                )
            warnings.warn(
                (
                    f"Calling {cls.__name__}.from_pretrained() with the path"
                    " to a single file or url is deprecated and won't be"
                    " possible anymore in v5. Use a model identifier or the"
            ...
            if "tokenizer_file" in vocab_files:
                ...
                if resolved_config_file is not None:
                    with open(
                        resolved_config_file, encoding="utf-8"
                    ) as reader:
        ...
        if len(unresolved_files) > 0:
            logger.info(
                "Can't load following files from cache:"
                f" {unresolved_files} and cannot check if these files are"
                " necessary for the tokenizer to operate."
            )
        ...
        if all(
            ...
            raise EnvironmentError(
                ...
                " 'https://huggingface.co/models', make sure you don't have a"
                " local directory with the same name. Otherwise, make sure"
                f" '{pretrained_model_name_or_path}' is the correct path to a"
                " directory containing all relevant files for a"
        ...
        for file_id, file_path in vocab_files.items():
            ...
            if is_local:
                ...
                logger.info(
                    f"loading file {file_path} from cache at"
                    f" {resolved_vocab_files[file_id]}"
                )
        ...
        if config_tokenizer_class is not None:
            if cls.__name__.replace(
                ...
                logger.warning(
                    ...
                    " the same type as the class this function is called"
                    " from. It may result in unexpected tokenization. \nThe"
                    " tokenizer class you load from this checkpoint is"
                    f" '{config_tokenizer_class}'. \nThe class this function"
                    f" is called from is '{cls.__name__}'."
                )
        ...
        if added_tokens_file is not None:
            ...
            for token, index in added_tok_encoder_sorted:
                ...
                if (
                    ...
                    raise ValueError(
                        f"Wrong index found for {token}: should be"
                        f" {tokenizer.convert_tokens_to_ids(token)} but found"
                        f" {index}."
                    )
                    ...
                    raise ValueError(
                        f"Non-consecutive added token '{token}' found. Should"
                        f" have index {current_index} but has index {index} in"
                        " saved vocabulary."
                    )
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        if os.path.isfile(save_directory):
            logger.error(
                f"Provided path ({save_directory}) should be a directory, not"
                " a file"
            )
            return
    ...
    def _save_pretrained(
        ...
        if legacy_format is False:
            raise ValueError(
                "Only fast tokenizers (instances of PreTrainedTokenizerFast)"
                " can be saved in non legacy format."
            )
        ...
        **kwargs
            ...
            if verbose:
                if not self.deprecation_warnings.get(
                    ...
                    logger.warning(
                        ...
                        " `max_length` is provided a specific value, please"
                        " use `truncation=True` to explicitly truncate"
                        " examples to max length. Defaulting to"
                        " 'longest_first' truncation strategy. If you encode"
                        " pairs of sequences (GLUE-style) with the tokenizer"
                        " you can select this strategy more precisely by"
        ...
        if padding is False and old_pad_to_max_length:
            if verbose:
                warnings.warn(
                    (
                        "The `pad_to_max_length` argument is deprecated and"
                        " will be removed in a future version, use"
                        " `padding=True` or `padding='longest'` to pad to the"
                        " longest sequence in the batch, or use"
                        " `padding='max_length'` to pad to a max length. In"
                        " this case, you can give a specific length with"
                        " `max_length` (e.g. `max_length=45`) or leave"
                        " max_length to None to pad to the maximal input size"
            ...
            if padding is True:
                if verbose:
                    if max_length is not None and (
                        ...
                        warnings.warn(
                            "`max_length` is ignored when `padding`=`True` and"
                            " there is no truncation strategy. To pad to max"
                            " length, use `padding='max_length'`."
                        )
                    if old_pad_to_max_length is not False:
                        warnings.warn(
                            "Though `pad_to_max_length` = `True`, it is"
                            " ignored because `padding`=`True`."
                        )
                padding_strategy = (
        ...
        if truncation is None and old_truncation_strategy != "do_not_truncate":
            if verbose:
                warnings.warn(
                    (
                        "The `truncation_strategy` argument is deprecated and"
                        " will be removed in a future version, use"
                        " `truncation=True` to truncate examples to a max"
                        " length. You can give a specific length with"
                        " `max_length` (e.g. `max_length=45`) or leave"
                        " max_length to None to truncate to the maximal input"
                        " size of the model (e.g. 512 for Bert).  If you have"
                        " pairs of inputs, you can give a specific truncation"
                        " strategy selected among `truncation='only_first'`"
                        " (will only truncate the first sentence in the pairs)"
                        " `truncation='only_second'` (will only truncate the"
                        " second sentence in the pairs) or"
                        " `truncation='longest_first'` (will iteratively"
                        " remove tokens from the longest sentence in the"
                        " pairs)."
                    ),
                    FutureWarning,
                )
                    ...
                    if verbose:
                        if not self.deprecation_warnings.get(
                            ...
                            logger.warning(
                                ...
                                " length is provided and the model has no"
                                " predefined maximum length. Default to no"
                                " padding."
                            )
                        self.deprecation_warnings[
                ...
                if self.model_max_length > LARGE_INTEGER:
                    if verbose:
                        if not self.deprecation_warnings.get(
                            ...
                            logger.warning(
                                "Asking to truncate to max_length but no"
                                " maximum length is provided and the model has"
                                " no predefined maximum length. Default to no"
                                " truncation."
                            )
                        self.deprecation_warnings[
        ...
        if padding_strategy != PaddingStrategy.DO_NOT_PAD and (
            ...
            raise ValueError(
                ...
                " token. Please select a token to use as `pad_token`"
        ...
        if (
            ...
            raise ValueError(
                "Truncation and padding are both activated but truncation"
                f" length ({max_length}) is not a multiple of"
        ...
        **kwargs
        ...
        if is_batched:
            if isinstance(text_pair, str):
                raise TypeError(
                    "when tokenizing batches of text, `text_pair` must be a"
                    " list or tuple with the same length as `text`."
                )
            if text_pair is not None and len(text) != len(text_pair):
                raise ValueError(
                    f"batch length of `text`: {len(text)} does not match batch"
                    f" length of `text_pair`: {len(text_pair)}."
                )
    ...
    def pad(
        ...
        if self.__class__.__name__.endswith("Fast"):
            if not self.deprecation_warnings.get(
                ...
                logger.warning_advice(
                    f"You're using a {self.__class__.__name__} tokenizer."
        ...
        if self.model_input_names[0] not in encoded_inputs:
            raise ValueError(
                "You should supply an encoding or a list of encodings to this"
                f" method that includes {self.model_input_names[0]}, but you"
                f" provided {list(encoded_inputs.keys())}"
            )
        ...
        if not isinstance(first_element, (int, list, tuple)):
            if is_tf_tensor(first_element):
                ...
                raise ValueError(
                    ...
                    " Should be one of a python, numpy, pytorch or tensorflow"
                    " object."
                )
        ...
        assert all(len(v) == batch_size for v in encoded_inputs.values()), (
            "Some items in the output dictionary have a different batch size"
            " than others."
        )
    ...
    def prepare_for_model(
        ...
        **kwargs
        ...
        if return_token_type_ids and not add_special_tokens:
            raise ValueError(
                "Asking to return token_type_ids while setting"
                " add_special_tokens to False results in an undefined"
                " behavior. Please set add_special_tokens to True or set"
                " return_token_type_ids to None."
            )
        ...
        if (
            ...
            raise ValueError(
                "Not possible to return overflowing tokens for pair of"
                " sequences with the `longest_first`. Please select another"
                " truncation strategy than `longest_first`, for instance"
                " `only_second` or `only_first`."
            )
    ...
    def truncate_sequences(
        ...
        if truncation_strategy == TruncationStrategy.ONLY_FIRST or (
            ...
            if len(ids) > num_tokens_to_remove:
                ...
                error_msg = (
                    f"We need to remove {num_tokens_to_remove} to truncate the"
                    f" input but the first sequence has a length {len(ids)}. "
                )
                if truncation_strategy == TruncationStrategy.ONLY_FIRST:
                    error_msg = (
                        error_msg
                        + "Please select another truncation strategy than"
                        f" {truncation_strategy}, for instance"
                        " 'longest_first' or 'only_second'."
                    )
                logger.error(error_msg)
        elif truncation_strategy == TruncationStrategy.LONGEST_FIRST:
            logger.warning(
                "Be aware, overflowing tokens are not returned for the"
                " setting you have chosen, i.e. sequence pairs with the"
                f" '{TruncationStrategy.LONGEST_FIRST.value}' truncation"
                " strategy. So the returned list will always be empty even if"
            ...
            if len(pair_ids) > num_tokens_to_remove:
                ...
                logger.error(
                    f"We need to remove {num_tokens_to_remove} to truncate the"
                    " input but the second sequence has a length"
                    f" {len(pair_ids)}. Please select another truncation"
                    f" strategy than {truncation_strategy}, for instance"
                    " 'longest_first' or 'only_first'."
                )
    ...
    def get_special_tokens_mask(
        ...
        assert already_has_special_tokens and token_ids_1 is None, (
            "You cannot use ``already_has_special_tokens=False`` with this"
            " tokenizer. Please use a slow (full python) tokenizer to activate"
            " this argument. Or set `return_special_tokens_mask=True` when"
            " calling the encoding method to get the special tokens mask in"
            " any tokenizer. "
        )
        ...
        if max_length is None and len(ids) > self.model_max_length and verbose:
            if not self.deprecation_warnings.get(
                ...
                logger.warning(
                    ...
                    f" ({len(ids)} > {self.model_max_length}). Running this"
                    " sequence through the model will result in indexing"
                    " errors"
                )
            self.deprecation_warnings[
    ...
    def as_target_tokenizer(self):
        ...
        warnings.warn(
            ...
            " `text_target` of the regular `__call__` method (either in the"
            " same call as your input texts if you use the same keyword"
            " arguments, or in a separate call."
        )
        self._switch_to_target_mode()

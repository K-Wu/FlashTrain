...
if _has_comet:
    try:
        ...
        if hasattr(comet_ml, "config") and comet_ml.config.get_config(
            "comet.api_key"
        ):
            _has_comet = True
        else:
...
def is_wandb_available():
    ...
    if os.getenv("WANDB_DISABLED", "").upper() in ENV_VARS_TRUE_VALUES:
        logger.warning(
            ...
            " will be removed in v5. Use the --report_to flag to control the"
            " integrations used for logging result (for instance --report_to"
            " none)."
        )
        return False
...
def run_hp_search_optuna(
    ...
    if trainer.args.process_index == 0:
        ...
        def _objective(trial, checkpoint_dir=None):
            ...
            if trainer.args.world_size > 1:
                if trainer.args.parallel_mode != ParallelMode.DISTRIBUTED:
                    raise RuntimeError(
                        "only support DDP optuna HPO for"
                        " ParallelMode.DISTRIBUTED currently."
                    )
        ...
        study.optimize(
            _objective, n_trials=n_trials, timeout=timeout, n_jobs=n_jobs
        )
        best_trial = study.best_trial
        return BestRun(
            str(best_trial.number), best_trial.value, best_trial.params
        )
    else:
        for i in range(n_trials):
            ...
            if trainer.args.parallel_mode != ParallelMode.DISTRIBUTED:
                raise RuntimeError(
                    "only support DDP optuna HPO for ParallelMode.DISTRIBUTED"
                    " currently."
                )
...
def run_hp_search_ray(
    ...
    def _objective(trial, local_trainer, checkpoint_dir=None):
        ...
        if getattr(local_trainer, "objective", None) is None:
            ...
            ray.tune.report(
                objective=local_trainer.objective, **metrics, done=True
            )
    ...
    if "keep_checkpoints_num" in kwargs and kwargs["keep_checkpoints_num"] > 0:
        ...
        if kwargs["keep_checkpoints_num"] > 1:
            logger.warning(
                "Currently keeping"
                f" {kwargs['keep_checkpoints_num']} checkpoints for each"
                " trial. Checkpoints are usually huge, consider setting"
    ...
    if "scheduler" in kwargs:
        ...
        if isinstance(kwargs["scheduler"], PopulationBasedTraining):
            if not trainer.use_tune_checkpoints:
                logger.warning(
                    "You are using PopulationBasedTraining but you haven't"
                    " enabled checkpointing. This means your trials will train"
                    " from scratch everytime they are exploiting new"
                    " configurations. Consider enabling checkpointing by"
                    " passing `keep_checkpoints_num=1` as an additional"
                    " argument to `Trainer.hyperparameter_search`."
                )
        ...
        if isinstance(
            ...
            raise RuntimeError(
                "You are using {cls} as a scheduler but you haven't enabled"
                " evaluation during training. This means your trials will not"
                " report intermediate results to Ray Tune, and can thus not be"
                " stopped early or used to exploit other trials parameters. If"
                " this is what you want, do not use {cls}. If you would like"
                " to use {cls}, make sure you pass `do_eval=True` and"
                " `evaluation_strategy='steps'` in the Trainer `args`.".format(
...
def run_hp_search_sigopt(
    ...
    if trainer.args.process_index == 0:
        if importlib_metadata.version("sigopt") >= "8.0.0":
            ...
            logger.info(
                "created experiment:"
                f" https://app.sigopt.com/experiment/{experiment.id}"
            )
            ...
            for run in experiment.loop():
                with run:
                    ...
                    if trainer.args.world_size > 1:
                        if (
                            trainer.args.parallel_mode
                            != ParallelMode.DISTRIBUTED
                        ):
                            raise RuntimeError(
                                "only support DDP Sigopt HPO for"
                                " ParallelMode.DISTRIBUTED currently."
                            )
                        ...
                        trainer.train(
                            resume_from_checkpoint=None, trial=run.run
                        )
                    # If there hasn't been any evaluation during the training loop.
                    if getattr(trainer, "objective", None) is None:
                        metrics = trainer.evaluate()
            ...
            while (
                ...
                if trainer.args.world_size > 1:
                    ...
                    trainer.train(
                        resume_from_checkpoint=None, trial=suggestion
                    )
                # If there hasn't been any evaluation during the training loop.
                if getattr(trainer, "objective", None) is None:
                    metrics = trainer.evaluate()
                ...
                logger.info(
                    f"[suggestion_id, observation_id]: [{suggestion.id},"
                    f" {obs.id}]"
                )
        ...
        for i in range(n_trials):
            ...
            if trainer.args.parallel_mode != ParallelMode.DISTRIBUTED:
                raise RuntimeError(
                    "only support DDP Sigopt HPO for ParallelMode.DISTRIBUTED"
                    " currently."
                )
            torch.distributed.broadcast_object_list(args_main_rank, src=0)
            args = pickle.loads(bytes(args_main_rank))
...
def run_hp_search_wandb(
    ...
    def _objective():
        ...
        trainer.train(
            resume_from_checkpoint=None, trial=vars(config)["_items"]
        )
        # If there hasn't been any evaluation during the training loop.
        if getattr(trainer, "objective", None) is None:
            metrics = trainer.evaluate()
            ...
            if metric not in format_metrics:
                logger.warning(
                    f"Provided metric {metric} not found. This might result in"
                    " unexpected sweeps charts. The available metrics are"
                    f" {format_metrics.keys()}"
                )
        best_score = False
            ...
            for k, v in logs.items():
                if isinstance(v, (int, float)):
                    ...
                    logger.warning(
                        ...
                        f' type {type(v)} for key "{k}" as a scalar. This'
                        " invocation of Tensorboard's writer.add_scalar() is"
                        " incorrect so we dropped this attribute."
                    )
...
class WandbCallback(TrainerCallback):
    ...
    def __init__(self):
        ...
        if not has_wandb:
            raise RuntimeError(
                "WandbCallback requires wandb to be installed. Run `pip"
                " install wandb`."
            )
        if has_wandb:
            import wandb
    ...
    def setup(self, args, state, model, **kwargs):
        ...
        if state.is_world_process_zero:
            ...
            if (
                ...
                and os.getenv("WANDB_WATCH") != "false"
            ):
                self._wandb.watch(
                    model,
...
class CometCallback(TrainerCallback):
    ...
    def __init__(self):
        if not _has_comet:
            raise RuntimeError(
                "CometCallback requires comet-ml to be installed. Run `pip"
                " install comet-ml`."
            )
        self._initialized = False
    ...
    def setup(self, args, state, model):
        ...
        if state.is_world_process_zero:
            ...
            if comet_mode == "ONLINE":
                ...
                experiment.log_other("Created from", "transformers")
                logger.info(
                    "Automatic Comet.ml offline logging enabled; use `comet"
...
class AzureMLCallback(TrainerCallback):
    ...
    def __init__(self, azureml_run=None):
        if not is_azureml_available():
            raise RuntimeError(
                "AzureMLCallback requires azureml to be installed. Run `pip"
                " install azureml-sdk`."
            )
...
class MLflowCallback(TrainerCallback):
    ...
    def __init__(self):
        if not is_mlflow_available():
            raise RuntimeError(
                "MLflowCallback requires mlflow to be installed. Run `pip"
                " install mlflow`."
            )
        import mlflow
    ...
    def setup(self, args, state, model):
        ...
        logger.debug(
            f"MLflow experiment_name={self._experiment_name},"
            f" run_name={args.run_name}, nested={self._nested_run},"
        ...
        if state.is_world_process_zero:
            if (
                ...
                logger.debug(
                    "MLflow run started with"
                    f" run_id={self._ml_flow.active_run().info.run_id}"
                )
            ...
            for name, value in list(combined_dict.items()):
                ...
                if len(str(value)) > self._MAX_PARAM_VAL_LENGTH:
                    logger.warning(
                        f'Trainer is attempting to log a value of "{value}"'
                        f' for key "{name}" as a parameter. MLflow\'s'
                        " log_param() only accepts values no longer than 250"
                        " characters so we dropped this attribute. You can"
                        " use `MLFLOW_FLATTEN_PARAMS` environment variable to"
                        " flatten the parameters and avoid this message."
                    )
        ...
        if state.is_world_process_zero:
            ...
            for k, v in logs.items():
                if isinstance(v, (int, float)):
                    ...
                    logger.warning(
                        f'Trainer is attempting to log a value of "{v}" of'
                        f' type {type(v)} for key "{k}" as a metric. MLflow\'s'
                        " log_metric() only accepts float and int types so we"
    ...
    def on_save(self, args, state, control, **kwargs):
        if (
            ...
            logger.info(
                f"Logging checkpoint artifacts in {ckpt_dir}. This may take"
                " time."
            )
...
class NeptuneCallback(TrainerCallback):
    ...
    def __init__(
        ...
        **neptune_run_kwargs,
        ...
        if not is_neptune_available():
            raise ValueError(
                "NeptuneCallback requires the Neptune client library to be"
                " installed. To install the library, run `pip install"
                " neptune-client`."
            )
        ...
        if self._log_checkpoints in {"last", "best"}:
            self._target_checkpoints_namespace = (
                f"checkpoints/{self._log_checkpoints}"
            )
    ...
    def _initialize_run(self, **additional_neptune_kwargs):
        ...
        try:
            self._run = init_run(
                **self._init_run_kwargs, **additional_neptune_kwargs
            )
    ...
    def _ensure_run_with_monitoring(self):
        if self._initial_run is not None:
            ...
            if (
                not self._force_reset_monitoring_run
                and self._is_monitoring_run
            ):
                return
    ...
    def _log_hyper_param_search_parameters(self, state):
        if state and hasattr(state, "trial_name"):
            self._metadata_namespace[
                NeptuneCallback.trial_name_key
            ] = state.trial_name

        if (
            state
            and hasattr(state, "trial_params")
            and state.trial_params is not None
        ):
            self._metadata_namespace[
    ...
    def _log_model_checkpoint(self, source_directory: str, checkpoint: str):
        target_path = relative_path = os.path.join(
            source_directory, checkpoint
        )
        ...
        if self._volatile_checkpoints_dir is not None:
            ...
            try:
                ...
                logger.warning(
                    "NeptuneCallback was unable to made a copy of checkpoint"
                    " due to I/O exception: '{}'.Could fail trying to upload."
                    .format(e)
                )
        ...
        if (
            ...
            self._metadata_namespace[
                self._target_checkpoints_namespace
            ].delete_files(self._recent_checkpoint_path)
    ...
    def on_init_end(self, args, state, control, **kwargs):
        ...
        if self._log_checkpoints == "best" and not args.load_best_model_at_end:
            raise ValueError(
                "To save the best model checkpoint, the load_best_model_at_end"
                " argument must be enabled."
            )
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        if self._log_checkpoints == "best":
            ...
            self._should_upload_checkpoint = (
    ...
    def get_run(cls, trainer):
        ...
        raise Exception(
            "The trainer doesn't have a NeptuneCallback configured."
        )
...
class CodeCarbonCallback(TrainerCallback):
    ...
    def __init__(self):
        if not is_codecarbon_available():
            raise RuntimeError(
                "CodeCarbonCallback requires `codecarbon` to be installed. Run"
...
def get_reporting_integration_callbacks(report_to):
    for integration in report_to:
        if integration not in INTEGRATION_TO_CALLBACK:
            raise ValueError(
                f"{integration} is not supported, only"
                f" {', '.join(INTEGRATION_TO_CALLBACK.keys())} are supported."
            )

...
def load_sharded_checkpoint(model, folder, strict=True):
    ...
    if not os.path.isfile(index_file):
        raise ValueError(
            f"Can't find a checkpoint index ({WEIGHTS_INDEX_NAME}) in"
            f" {folder}."
        )
    ...
    if checkpoint_file.endswith(".safetensors") and is_safetensors_available():
        ...
        if metadata.get("format") not in ["pt", "tf", "flax"]:
            raise OSError(
                ...
                " contain the valid metadata. Make sure you save your model"
        ...
        try:
            with open(checkpoint_file) as f:
                if f.read().startswith("version"):
                    ...
                    raise ValueError(
                        f"Unable to locate the file {checkpoint_file} which is"
                        " necessary to load this pretrained model. Make sure"
                        " you have saved the model properly."
                    ) from e
        except (UnicodeDecodeError, ValueError):
            raise OSError(
                "Unable to load weights from pytorch checkpoint file for"
                f" '{checkpoint_file}' at '{checkpoint_file}'. If you tried to"
                " load a PyTorch model from a TF 2.0 checkpoint, please set"
                " from_tf=True."
            )
...
class ModuleUtilsMixin:
    ...
    def _hook_rss_memory_pre_forward(module, *args, **kwargs):
        try:
            ...
            raise ImportError(
                "You need to install psutil (pip install psutil) to use memory"
                " tracing."
            )
    ...
    def create_extended_attention_mask_for_decoder(
        ...
        if device is not None:
            warnings.warn(
                (
                    "The `device` argument is deprecated and will be removed"
                    " in v5 of Transformers."
                ),
                FutureWarning,
            )
        else:
    ...
    def get_extended_attention_mask(
        ...
        if not (attention_mask.dim() == 2 and self.config.is_decoder):
            ...
            if device is not None:
                warnings.warn(
                    (
                        "The `device` argument is deprecated and will be"
    ...
    def estimate_tokens(
        ...
        if self.main_input_name in input_dict:
            ...
            logger.warning(
                ...
                " floating-point operations will not be computed"
            )
...
class PreTrainedModel(
    ...
    def _from_config(cls, config, **kwargs):
        ...
        if is_deepspeed_zero3_enabled():
            ...
            logger.info(
                "Detected DeepSpeed ZeRO-3: activating zero.init() for this"
                " model"
            )
    ...
    def _set_default_torch_dtype(cls, dtype: torch.dtype) -> torch.dtype:
        ...
        if not dtype.is_floating_point:
            raise ValueError(
                f"Can't instantiate {cls.__name__} model under"
                f" dtype={dtype} since it is not a floating point dtype"
            )
    ...
    def _tie_encoder_decoder_weights(
        ...
        if decoder.__class__ != encoder.__class__:
            logger.info(
                ...
                " In this case make sure that all encoder weights are"
        ...
        if len(uninitialized_encoder_weights) > 0:
            logger.warning(
                "The following encoder weights were not tied to the decoder"
    ...
    def _get_resized_embeddings(
        ...
        if not isinstance(old_embeddings, nn.Embedding):
            raise TypeError(
                f"Old embeddings are of type {type(old_embeddings)}, which is"
                f" not an instance of {nn.Embedding}. You should either use a"
                " different resize function or make sure that"
                f" `old_embeddings` are an instance of {nn.Embedding}."
            )
    ...
    def _get_resized_lm_head(
        ...
        if not isinstance(old_lm_head, nn.Linear):
            raise TypeError(
                f"Old language model head is of type {type(old_lm_head)},"
                f" which is not an instance of {nn.Linear}. You should either"
                " use a different resize function or make sure that"
                f" `old_lm_head` are an instance of {nn.Linear}."
            )
    ...
    def resize_position_embeddings(self, new_num_position_embeddings: int):
        raise NotImplementedError(
            ...
            f" {self.__class__}`. To implement it, you should overwrite this"
            f" method in the class {self.__class__} in"
            f" `modeling_{self.__class__.__module__}.py`"
        )
    ...
    def get_position_embeddings(
        ...
        raise NotImplementedError(
            "`get_position_embeddings` is not implemented for"
            f" {self.__class__}`. To implement it, you should overwrite this"
            f" method in the class {self.__class__} in"
            f" `modeling_{self.__class__.__module__}.py`"
        )
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        if "save_config" in kwargs:
            warnings.warn(
                "`save_config` is deprecated and will be removed in v5 of"
                " Transformers. Use `is_main_process` instead."
            )
            is_main_process = kwargs.pop("save_config")
        if safe_serialization and not is_safetensors_available():
            raise ImportError(
                "`safe_serialization` requires the `safetensors library: `pip"
                " install safetensors`."
            )
        ...
        if index is None:
            logger.info(
                ...
                f" {os.path.join(save_directory, WEIGHTS_NAME)}"
            )
        else:
            save_index_file = (
                SAFE_WEIGHTS_INDEX_NAME
        ...
        * `low_cpu_mem_usage` algorithm:
        ...
        if device_map is not None:
            if low_cpu_mem_usage is None:
                ...
                raise ValueError(
                    "Passing along a `device_map` requires"
                    " `low_cpu_mem_usage=True`"
                )
        ...
        if low_cpu_mem_usage:
            ...
            if is_deepspeed_zero3_enabled():
                raise ValueError(
                    ...
                    " `low_cpu_mem_usage=True` or with passing a `device_map`."
                )
            elif not is_accelerate_available():
                raise ImportError(
                    "Using `low_cpu_mem_usage=True` or a `device_map` requires"
        ...
        if load_in_8bit:
            if not (is_accelerate_available() and is_bitsandbytes_available()):
                raise ImportError(
                    "Using `load_in_8bit=True` requires Accelerate: `pip"
                    " install accelerate` and the latest version of"
                    " bitsandbytes `pip install -i"
                    " https://test.pypi.org/simple/ bitsandbytes` or pip"
            ...
            if torch_dtype == "auto" or torch_dtype != torch.float16:
                ...
                logger.info(
                    "Loading the model in mixed int8 - forcing the weights to"
                    " be casted in float16"
                )
            if device_map is None:
                raise ValueError(
                    "A device map needs to be passed to run convert models"
                    " into mixed-int8 format. Please run`.from_pretrained`"
                    " with `device_map='auto'`"
                )
            if from_tf or from_flax:
                raise ValueError(
                    "Converting into mixed 8-bit weights from tf/flax weights"
                    " is currently not supported, please make sure the weights"
                    " are in PyTorch format."
                )
            ...
            if is_local:
                ...
                # At this stage we don't have a weight file so we will raise an error.
                    ...
                    raise EnvironmentError(
                        ...
                        " is a file for TensorFlow weights. Use"
                        " `from_tf=True` to load this model from those"
                        " weights."
                    )
                elif os.path.isfile(
                    os.path.join(
                    ...
                    raise EnvironmentError(
                        f"Error no file named {WEIGHTS_NAME} found in"
                        f" directory {pretrained_model_name_or_path} but there"
                        " is a file for Flax weights. Use `from_flax=True` to"
                        " load this model from those weights."
                    )
                else:
                    raise EnvironmentError(
                        f"Error no file named {WEIGHTS_NAME},"
                        f" {TF2_WEIGHTS_NAME}, {TF_WEIGHTS_NAME + '.index'} or"
                        f" {FLAX_WEIGHTS_NAME} found in directory"
                ...
                if not from_tf:
                    raise ValueError(
                        ...
                        f" {pretrained_model_name_or_path + '.index'}, please"
                        " set from_tf to True to load from this checkpoint."
                    )
                archive_file = os.path.join(
                    subfolder, pretrained_model_name_or_path + ".index"
                )
                ...
                try:
                        ...
                        if has_file(
                            ...
                            **has_file_kwargs,
                            ...
                            raise EnvironmentError(
                                ...
                                f" {WEIGHTS_NAME} but there is a file for"
                                " TensorFlow weights. Use `from_tf=True` to"
                                " load this model from those weights."
                            )
                        elif has_file(
                            pretrained_model_name_or_path,
                            ...
                            **has_file_kwargs,
                            ...
                            raise EnvironmentError(
                                f"{pretrained_model_name_or_path} does not"
                                " appear to have a file named"
                                f" {WEIGHTS_NAME} but there is a file for Flax"
                                " weights. Use `from_flax=True` to load this"
                                " model from those weights."
                            )
                        else:
                            raise EnvironmentError(
                                f"{pretrained_model_name_or_path} does not"
                                f" appear to have a file named {WEIGHTS_NAME},"
                    ...
                    raise EnvironmentError(
                        ...
                        f" {WEIGHTS_NAME}, {TF2_WEIGHTS_NAME},"
            ...
            if torch_dtype is not None:
                if isinstance(torch_dtype, str):
                    if torch_dtype == "auto":
                        ...
                        raise ValueError(
                            "`torch_dtype` can be either a `torch.dtype` or"
                            f" `auto`, but received {torch_dtype}"
                        )
                dtype_orig = cls._set_default_torch_dtype(torch_dtype)

            if is_sharded:
                loaded_state_dict_keys = sharded_metadata[
                    "all_checkpoint_keys"
                ]
            else:
        ...
        if load_in_8bit:
            from .utils.bitsandbytes import (
                get_keys_to_not_convert,
                replace_8bit_linear,
            )

            logger.info(
                "Detected 8-bit loading: activating 8-bit loading for this"
                " model"
            )
        ...
        if isinstance(device_map, str):
            if model._no_split_modules is None:
                raise ValueError(
                    f"{model.__class__.__name__} does not support"
            ...
            if device_map not in [
                ...
                raise ValueError(
                    "If passing a string for `device_map`, please choose"
                    " 'auto', 'balanced', 'balanced_low_0' or 'sequential'."
                )
            elif (
                ...
                raise ValueError(
                    f"`device_map={device_map}` requires a source install of"
                    " Accelerate."
                )
        ...
        if from_tf:
            if resolved_archive_file.endswith(".index"):
                ...
                try:
                    ...
                    logger.error(
                        "Loading a TensorFlow model in PyTorch, requires both"
                        " PyTorch and TensorFlow to be installed. Please see"
            ...
            try:
                ...
                logger.error(
                    "Loading a Flax model in PyTorch, requires both PyTorch"
                    " and Flax to be installed. Please see"
                    " https://pytorch.org/ and"
    ...
    def _load_pretrained_model(
        ...
        if device_map is not None and "disk" in device_map.values():
            if offload_folder is None:
                raise ValueError(
                    "The current `device_map` had weights offloaded to the"
                    " disk. Please provide an `offload_folder` for them."
                )
        ...
        if (
            ...
            if any(key in expected_keys_not_prefixed for key in loaded_keys):
                raise ValueError(
                    "The state dictionary of the model you are trying to load"
                    " is corrupted. Are you sure it was properly saved?"
                )
            if device_map is not None:
                device_map = {
        ...
        if len(error_msgs) > 0:
            ...
            if "size mismatch" in error_msg:
                error_msg += (
                    ...
                    " `ignore_mismatched_sizes=True` in the model"

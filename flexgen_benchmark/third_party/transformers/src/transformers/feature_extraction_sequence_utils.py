...
class SequenceFeatureExtractor(FeatureExtractionMixin):
    ...
    def pad(
        ...
        if self.model_input_names[0] not in processed_features:
            raise ValueError(
                ...
                " or list of `transformers.BatchFeature` to this method that"
                f" includes {self.model_input_names[0]}, but you provided"
    ...
    def _truncate(
        ...
        if not truncation:
            ...
            raise ValueError(
                "When setting ``truncation=True``, make sure that"
    ...
    def _get_padding_strategies(self, padding=False, max_length=None):
        ...
        if max_length is None:
            if padding_strategy == PaddingStrategy.MAX_LENGTH:
                raise ValueError(
                    f"When setting ``padding={PaddingStrategy.MAX_LENGTH}``,"
                    " make sure that max_length is defined"
                )
        ...
        if padding_strategy != PaddingStrategy.DO_NOT_PAD and (
            ...
            raise ValueError(
                "Asking to pad but the feature_extractor does not have a"
                " padding value. Please select a value to use as"
                " `padding_value`. For example:"

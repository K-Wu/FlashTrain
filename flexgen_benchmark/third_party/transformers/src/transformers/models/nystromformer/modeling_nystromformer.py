    ...
    def forward(
        ...
        if self.num_landmarks == self.seq_len:
            ...
            kernel_1 = torch.nn.functional.softmax(
                torch.matmul(query_layer, k_landmarks.transpose(-1, -2)),
                dim=-1,
            )
            ...
            attention_scores = torch.matmul(
                q_landmarks, key_layer.transpose(-1, -2)
            )
            ...
            attention_probs = torch.matmul(
                kernel_1, self.iterative_inv(kernel_2)
            )
...
@add_start_docstrings(
    (
        "The bare Nyströmformer Model transformer outputting raw hidden-states"

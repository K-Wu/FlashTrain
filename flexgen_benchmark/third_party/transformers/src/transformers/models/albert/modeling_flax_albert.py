...
class FlaxAlbertSelfAttention(nn.Module):
    ...
    def __call__(
        ...
        layernormed_attn_output = self.LayerNorm(
            projected_attn_output + hidden_states
        )
        outputs = (
...
class FlaxAlbertLayerCollections(nn.Module):
    ...
    def setup(self):
        self.albert_layers = FlaxAlbertLayerCollection(
...
class FlaxAlbertEncoder(nn.Module):
    ...
    def setup(self):
        ...
        self.albert_layer_groups = FlaxAlbertLayerGroups(
            self.config, dtype=self.dtype
        )
...
class FlaxAlbertForPreTrainingModule(nn.Module):
    ...
    def setup(self):
        ...
        self.predictions = FlaxAlbertOnlyMLMHead(
            config=self.config, dtype=self.dtype
        )
        self.sop_classifier = FlaxAlbertSOPHead(
            config=self.config, dtype=self.dtype
        )

    ...
    def hf_compute_loss(
        ...
        if self.config.tf_legacy_loss:
            ...
            sentence_order_active_loss = tf.not_equal(
                tf.reshape(tensor=labels["sentence_order_label"], shape=(-1,)),
                -100,
            )
            ...
            sentence_order_label = tf.boolean_mask(
                tensor=tf.reshape(
                    tensor=labels["sentence_order_label"], shape=(-1,)
                ),
                mask=sentence_order_active_loss,
            )
            ...
            sentence_order_loss = loss_fn(
                y_true=sentence_order_label,
                y_pred=sentence_order_reduced_logits,
            )
            masked_lm_loss = tf.reshape(
                tensor=masked_lm_loss,
                shape=(-1, shape_list(sentence_order_loss)[0]),
            )
            masked_lm_loss = tf.reduce_mean(
                input_tensor=masked_lm_loss, axis=0
            )
        ...
        unmasked_sop_loss = loss_fn(
            y_true=tf.nn.relu(labels["sentence_order_label"]),
            y_pred=sop_logits,
        )
        sop_loss_mask = tf.cast(
            labels["sentence_order_label"] != -100,
            dtype=unmasked_sop_loss.dtype,
        )

        masked_sop_loss = unmasked_sop_loss * sop_loss_mask
        reduced_masked_sop_loss = tf.reduce_sum(
            masked_sop_loss
        ) / tf.reduce_sum(sop_loss_mask)

        return tf.reshape(
            reduced_masked_lm_loss + reduced_masked_sop_loss, (1,)
        )
...
class TFAlbertMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        self, config: AlbertConfig, add_pooling_layer: bool = True, **kwargs
    ):
        super().__init__(**kwargs)
...
class TFAlbertForPreTraining(TFAlbertPreTrainedModel, TFAlbertPreTrainingLoss):
    ...
    def call(
        ...
        sop_scores = self.sop_classifier(
            pooled_output=pooled_output, training=training
        )
        total_loss = None
...
class TFAlbertForMaskedLM(
    TFAlbertPreTrainedModel, TFMaskedLanguageModelingLoss
):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
...
class TFAlbertForTokenClassification(
    ...
    @add_code_sample_docstrings(
        ...
        config_class=_CONFIG_FOR_DOC,
        expected_output=(
            "['LABEL_1', 'LABEL_1', 'LABEL_1', 'LABEL_0', 'LABEL_1',"
            " 'LABEL_0', 'LABEL_1', 'LABEL_1', 'LABEL_0', 'LABEL_1',"
...
class TFAlbertForQuestionAnswering(
    TFAlbertPreTrainedModel, TFQuestionAnsweringLoss
):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    _keys_to_ignore_on_load_unexpected = [r"pooler", r"predictions"]

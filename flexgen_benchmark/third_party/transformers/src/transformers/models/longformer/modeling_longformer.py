...
def _get_question_end_index(input_ids, sep_token_id):
    ...
    assert (
        sep_token_indices.shape[1] == 2
    ), "`input_ids` should have two dimensions"
    assert sep_token_indices.shape[0] == 3 * batch_size, (
        f"There should be exactly three separator tokens: {sep_token_id} in"
        " every sample for questions answering. You might also consider to"
        " set `global_attention_mask` manually in the forward function to"
        " avoid this error."
    )
...
def _compute_global_attention_mask(
    input_ids, sep_token_id, before_sep_token=True
):
    """
    ...
    if before_sep_token is True:
        attention_mask = (
            attention_mask.expand_as(input_ids) < question_end_index
        ).to(torch.uint8)
    else:
...
class LongformerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        *attention_window* happens in [`LongformerModel.forward`] to avoid redoing the padding on each layer.
        ...
        assert embed_dim == self.embed_dim, (
            f"hidden_states should have embed_dim = {self.embed_dim}, but has"
            f" {embed_dim}"
        )
        ...
        assert list(attn_scores.size()) == [
            ...
            f" {self.num_heads}, {self.one_sided_attn_window_size * 2 + 1}),"
            f" but is of size {attn_scores.size()}"
        )
    ...
    def _chunk(hidden_states, window_overlap, onnx_export=False):
        ...
        if not onnx_export:
            ...
            return hidden_states.as_strided(
                size=chunk_size, stride=chunk_stride
            )
    ...
    def _mask_invalid_locations(
        ...
        input_tensor[
            :, :affected_seq_len, :, : affected_seq_len + 1
        ] = torch.full_like(beginning_input, -float("inf")).where(
            beginning_mask.bool(), beginning_input
        )
    ...
    def _get_global_attn_indices(is_index_global_attn):
        ...
        is_local_index_global_attn_nonzero = (
            is_local_index_global_attn.nonzero(as_tuple=True)
        )
    ...
    def _compute_global_attn_output_from_hidden(
        ...
        assert list(global_attn_output.size()) == [
            ...
            f" but is {global_attn_output.size()}."
        )
    ...
    def forward(
        ...
        if head_mask is not None:
            assert head_mask.size()[0] == (len(self.layer)), (
                ...
                f" {len(self.layer)} layers, but it is for"
                f" {head_mask.size()[0]}."
            )
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        return module(
                            *inputs, is_global_attn, output_attentions
                        )
...
class LongformerModel(LongformerPreTrainedModel):
    ...
    @replace_return_docstrings(
        output_type=LongformerBaseModelOutputWithPooling,
...
class LongformerForQuestionAnswering(LongformerPreTrainedModel):
    ...
    @replace_return_docstrings(
        output_type=LongformerQuestionAnsweringModelOutput,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
        self,
        ...
        if global_attention_mask is None:
            if input_ids is None:
                logger.warning(
                    "It is not possible to automatically generate the"
                    " `global_attention_mask` because input_ids is None."
                    " Please make sure that it is correctly set."
                )
            else:

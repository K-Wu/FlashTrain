...
def _compute_global_attention_mask(
    ...
    question_end_index = tf.reshape(
        sep_token_indices, (input_ids_shape[0], 3, 2)
    )[:, 0, 1][:, None]
...
class TFLongformerEmbeddings(tf.keras.layers.Layer):
    ...
    def create_position_ids_from_input_ids(
        ...
        """
        mask = tf.cast(
            tf.math.not_equal(input_ids, self.padding_idx),
    ...
    def call(
        ...
        if input_ids is not None:
            ...
            tf.debugging.assert_less(
                ...
                message=(
                    "input_ids must be smaller than the embedding layer's"
                    f" input dimension (got {tf.math.reduce_max(input_ids)} >="
                    f" {self.vocab_size})"
                ),
            )
...
class TFLongformerSelfAttention(tf.keras.layers.Layer):
    def __init__(self, config, layer_id, **kwargs):
        ...
        assert attention_window % 2 == 0, (
            f"`attention_window` for layer {self.layer_id} has to be an even"
            f" value. Given {attention_window}"
        )
        assert attention_window > 0, (
            f"`attention_window` for layer {self.layer_id} has to be positive."
    ...
    def call(
        ...
        *attention_window* happens in LongformerModel.forward to avoid redoing the padding on each layer.
        ...
        tf.debugging.assert_equal(
            ...
            self.embed_dim,
            message=(
                f"hidden_states should have embed_dim = {self.embed_dim}, but"
                f" has {embed_dim}"
            ),
        )
        ...
        query_vectors /= tf.math.sqrt(
            tf.cast(self.head_dim, dtype=query_vectors.dtype)
        )
        ...
        float_mask = (
            tf.cast(
                remove_from_windowed_attention_mask, dtype=query_vectors.dtype
            )
            * LARGE_NEGATIVE
        )
        ...
        tf.debugging.assert_equal(
            ...
            message=(
                f"attn_probs should be of size ({batch_size}, {seq_len},"
                f" {self.num_heads},"
                f" {self.one_sided_attn_window_size * 2 + 1}), but is of size"
                f" {shape_list(attn_scores)}"
            ),
        )
        ...
        if layer_head_mask is not None:
            ...
            attn_probs = (
                tf.reshape(layer_head_mask, (1, 1, -1, 1)) * attn_probs
            )
        ...
        attn_probs = tf.where(
            ...
            tf.zeros(
                shape_list(masked_global_attn_index), dtype=attn_probs.dtype
            ),
            attn_probs,
        )
    ...
    def _sliding_chunks_query_key_matmul(self, query, key, window_overlap):
        ...
        tf.debugging.assert_equal(
            ...
            0,
            message=(
                f"Sequence length should be multiple of {window_overlap * 2}."
                f" Given {seq_len}"
            ),
        )
        tf.debugging.assert_equal(
            ...
            message=(
                ...
                f" {shape_list(query)} and key: {shape_list(key)}"
            ),
        )
        ...
        first_chunk_mask = (
            tf.tile(
                tf.range(chunks_count + 1, dtype=tf.int64)[
                    None, :, None, None
                ],
    ...
    def _chunk(hidden_states, window_overlap):
        ...
        tf.debugging.assert_equal(
            ...
            message=(
                ...
                f" {shape_list(chunked_hidden_states)}."
            ),
        )
    ...
    def _concat_with_global_key_attn_probs(
        ...
        mask_shape = (
            shape_list(is_local_index_no_global_attn_nonzero)[0],
        ) + tuple(shape_list(attn_probs_from_global_key_trans)[-2:])
    ...
    def _compute_attn_output_with_global_indices(
        ...
        attn_output_without_global = (
            self._sliding_chunks_matmul_attn_probs_value(
    ...
    def _compute_global_attn_output_from_hidden(
        ...
        global_query_vectors_only_global /= tf.math.sqrt(
            tf.cast(
                self.head_dim, dtype=global_query_vectors_only_global.dtype
            )
        )
        ...
        tf.debugging.assert_equal(
            ...
            message=(
                ...
                f" {(batch_size * self.num_heads, max_num_global_attn_indices, seq_len)},"
                f" but is {shape_list(global_attn_scores)}."
            ),
        )
        ...
        tf.debugging.assert_equal(
            ...
            message=(
                ...
                f" {(batch_size * self.num_heads, max_num_global_attn_indices, self.head_dim)},"
                f" but is {shape_list(global_attn_output)}."
            ),
        )
    ...
    def reshape_and_transpose(self, vector, batch_size):
        return tf.reshape(
            tf.transpose(
                tf.reshape(
                    vector, (batch_size, -1, self.num_heads, self.head_dim)
                ),
                (0, 2, 1, 3),
            ),
...
class TFLongformerMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, add_pooling_layer=True, **kwargs):
        ...
        if isinstance(config.attention_window, int):
            ...
            assert len(config.attention_window) == config.num_hidden_layers, (
                "`len(config.attention_window)` should equal"
                " `config.num_hidden_layers`. Expected"
                f" {config.num_hidden_layers}, given"
                f" {len(config.attention_window)}"
            )
        ...
        self.pooler = (
            TFLongformerPooler(config, name="pooler")
    ...
    def _pad_to_window_size(
        ...
        if padding_len > 0:
            logger.info(
                f"Input ids are automatically padded from {seq_len} to"
                f" {seq_len + padding_len} to be a multiple of"
        ...
        if input_ids is not None:
            input_ids = tf.pad(
                input_ids, paddings, constant_values=pad_token_id
            )
        ...
        if inputs_embeds is not None:
            ...
            def pad_embeddings():
                input_ids_padding = tf.cast(
                    tf.fill((batch_size, padding_len), self.pad_token_id),
                    tf.int64,
                )
...
class TFLongformerForMaskedLM(
    ...
    def call(
        ...
        loss = (
            None
            if labels is None
            else self.hf_compute_loss(labels, prediction_scores)
        )
    ...
    def call(
        ...
        if global_attention_mask is None and input_ids is not None:
            if (
                ...
                logger.warning(
                    "There should be exactly three separator tokens:"
                    f" {self.config.sep_token_id} in every sample for"
                    " questions answering. You might also consider to set"
                    " `global_attention_mask` manually in the forward"
                    " function to avoid this. This is most likely an error."
                    " The global attention is disabled for this forward pass."
                )
...
class TFLongformerForMultipleChoice(
    TFLongformerPreTrainedModel, TFMultipleChoiceLoss
):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    ...
    def dummy_inputs(self):
        input_ids = tf.convert_to_tensor(
            MULTIPLE_CHOICE_DUMMY_INPUTS, dtype=tf.int64
        )
        # make sure global layers are initialized
        global_attention_mask = tf.convert_to_tensor(
    ...
    def call(
        ...
        flat_input_ids = (
            tf.reshape(input_ids, (-1, seq_length))
            if input_ids is not None
            else None
        )
        flat_attention_mask = (
        ...
        flat_global_attention_mask = (
            tf.reshape(
                global_attention_mask,
                (-1, shape_list(global_attention_mask)[-1]),
            )
            if global_attention_mask is not None
            else None
        )
        flat_inputs_embeds = (
            tf.reshape(
                inputs_embeds, (-1, seq_length, shape_list(inputs_embeds)[3])
            )
            if inputs_embeds is not None
            else None
        )
        ...
        loss = (
            None
            if labels is None
            else self.hf_compute_loss(labels, reshaped_logits)
        )

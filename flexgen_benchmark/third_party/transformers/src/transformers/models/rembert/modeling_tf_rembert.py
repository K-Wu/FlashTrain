...
class TFRemBertLayer(tf.keras.layers.Layer):
    def __init__(self, config: RemBertConfig, **kwargs):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = TFRemBertAttention(
...
class TFRemBertLMPredictionHead(tf.keras.layers.Layer):
    ...
    def call(self, hidden_states: tf.Tensor) -> tf.Tensor:
        ...
        hidden_states = tf.matmul(
            a=hidden_states, b=self.decoder, transpose_b=True
        )
        hidden_states = tf.reshape(
...
class TFRemBertMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        self, config: RemBertConfig, add_pooling_layer: bool = True, **kwargs
    ):
        super().__init__(**kwargs)
        ...
        self.pooler = (
            TFRemBertPooler(config, name="pooler")
...
@add_start_docstrings(
    (
        "The bare RemBERT Model transformer outputing raw hidden-states"
...
class TFRemBertForMaskedLM(
    TFRemBertPreTrainedModel, TFMaskedLanguageModelingLoss
):
    def __init__(self, config: RemBertConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `TFRemBertForMaskedLM` make sure"
...
class TFRemBertForCausalLM(
    TFRemBertPreTrainedModel, TFCausalLanguageModelingLoss
):
    def __init__(self, config: RemBertConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        ...
        if not config.is_decoder:
            logger.warning(
                "If you want to use `TFRemBertForCausalLM` as a standalone,"
                " add `is_decoder=True.`"
            )
...
class TFRemBertForMultipleChoice(
    TFRemBertPreTrainedModel, TFMultipleChoiceLoss
):
    def __init__(self, config: RemBertConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
    ...
    @unpack_inputs
    @add_start_docstrings_to_model_forward(
        REMBERT_INPUTS_DOCSTRING.format(
...
class TFRemBertForQuestionAnswering(
    TFRemBertPreTrainedModel, TFQuestionAnsweringLoss
):
    def __init__(self, config: RemBertConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)

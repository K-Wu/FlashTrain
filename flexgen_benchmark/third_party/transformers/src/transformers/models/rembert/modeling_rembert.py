...
def load_tf_weights_in_rembert(model, config, tf_checkpoint_path):
    ...
    for name, shape in init_vars:
        ...
        if any(
            deny in name
            for deny in ("adam_v", "adam_m", "output_embedding", "cls")
        ):
...
class RemBertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(
            config.hidden_size, config.output_embedding_size
        )
...
class RemBertForMaskedLM(RemBertPreTrainedModel):
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `RemBertForMaskedLM` make sure"
...
class RemBertForCausalLM(RemBertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning(
                "If you want to use `RemBertForCausalLM` as a standalone, add"
                " `is_decoder=True.`"
            )

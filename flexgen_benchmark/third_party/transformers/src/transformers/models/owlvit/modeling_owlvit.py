...
class OwlViTModel(OwlViTPreTrainedModel):
    ...
    def __init__(self, config: OwlViTConfig):
        ...
        self.logit_scale = nn.Parameter(
            torch.ones([]) * config.logit_scale_init_value
        )
    ...
    @replace_return_docstrings(
        output_type=OwlViTOutput, config_class=OwlViTConfig
    )
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        ...
        return_base_image_embeds = (
            return_base_image_embeds
            if return_base_image_embeds is not None
            else False
        )
...
class OwlViTForObjectDetection(OwlViTPreTrainedModel):
    ...
    def normalize_grid_corner_coordinates(
        self, feature_map: torch.FloatTensor
    ):
        # Computes normalized xy corner coordinates from feature_map.
        if not feature_map.ndim == 4:
            raise ValueError(
                "Expected input shape is [batch_size, num_patches,"
        ...
        box_coordinates = np.stack(
            np.meshgrid(
                np.arange(1, num_patches + 1), np.arange(1, num_patches + 1)
            ),
            axis=-1,
        ).astype(np.float32)
    ...
    def compute_box_bias(
        ...
        box_size_bias = torch.log(box_size + 1e-4) - torch.log1p(
            -box_size + 1e-4
        )

...
def convert_clip_backbone(flax_params, torch_config):
    ...
    for flax_key, v in flax_clip_params.items():
        ...
        torch_key = torch_key.replace(
            "text_projection.kernel", "text_projection"
        )
        ...
        if "conv" in torch_key or "downsample.0.weight" in torch_key:
            ...
            "weight" in torch_key
            and v.ndim == 2
            and "embedding" not in torch_key
        ):
...
def convert_owlvit_checkpoint(
    ...
    repo = Repository(
        pytorch_dump_folder_path,
        clone_from=f"google/{pytorch_dump_folder_path}",
    )
    repo.git_pull()
...
if __name__ == "__main__":
    ...
    variables = checkpoints.restore_checkpoint(
        args.owlvit_checkpoint, target=None
    )["optimizer"]["target"]

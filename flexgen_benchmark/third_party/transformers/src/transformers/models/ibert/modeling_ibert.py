...
class IBertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if self.position_embedding_type != "absolute":
            raise ValueError(
                "I-BERT only supports 'absolute' for"
...
class IBertLayer(nn.Module):
    ...
    def forward(
        ...
        (
            ...
            self_attention_outputs_scaling_factor,
        ) = self.attention(
            hidden_states,
            hidden_states_scaling_factor,
            attention_mask,
        ...
        attention_output_scaling_factor = (
            self_attention_outputs_scaling_factor[0]
        )
...
class IBertEncoder(nn.Module):
    ...
    def forward(
        ...
        all_cross_attentions = (
            None  # `config.add_cross_attention` is not supported
        )
...
@add_start_docstrings(
    (
        "The bare I-BERT Model transformer outputting raw hidden-states"

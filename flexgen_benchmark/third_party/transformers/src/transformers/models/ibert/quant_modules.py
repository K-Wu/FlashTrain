...
class QuantAct(nn.Module):
    ...
    def __repr__(self):
        return (
            ...
            f" quant_mode: {self.quant_mode}, Act_min:"
            f" {self.x_min.item():.2f}, Act_max: {self.x_max.item():.2f})"
        )
...
class QuantLinear(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "fc_scaling_factor", torch.zeros(self.out_features)
        )
        if bias:
    ...
    def forward(self, x, prev_act_scaling_factor=None):
        if not self.quant_mode:
            return (
                nn.functional.linear(x, weight=self.weight, bias=self.bias),
                None,
            )
        ...
        assert (
            ...
            "Input activation to the QuantLinear layer should be globally"
            " (non-channel-wise) quantized. Please add a QuantAct layer with"
            " `per_channel = True` before this QuantAct layer"
        )
...
class IntSoftmax(nn.Module):
    ...
    def int_exp(self, x_int, scaling_factor):
        ...
        exp_int = torch.clamp(
            floor_ste.apply(exp_int * 2 ** (self.const - q)), min=0
        )
...
class IntLayerNorm(nn.Module):
    ...
    def set_shift(self, y_int):
        with torch.no_grad():
            ...
            shift = (
                torch.log2(torch.sqrt(var_int / 2**self.max_bit)).ceil()
            ).max()
            shift_old = self.shift
            self.shift = torch.max(self.shift, shift)
            logger.info(
                f"Dynamic shift adjustment: {int(shift_old)} ->"
                f" {int(self.shift)}"
            )
        ...
        if self.training:
            ...
            if var_int.max() >= 2**self.max_bit:
                ...
                assert var_int.max() < 2**self.max_bit + 0.1, (
                    "Error detected in overflow handling: `var_int` exceeds"
                    " `self.max_bit` (the maximum possible bit width)"
                )
...
def symmetric_linear_quantization_params(
    ...
    with torch.no_grad():
        ...
        if per_channel:
            scale, _ = torch.max(
                torch.stack(
                    [saturation_min.abs(), saturation_max.abs()], dim=1
                ),
                dim=1,
            )

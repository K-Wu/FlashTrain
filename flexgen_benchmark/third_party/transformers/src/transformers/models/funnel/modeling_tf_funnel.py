...
class TFFunnelAttentionStructure:
    ...
    def token_type_ids_to_mat(self, token_type_ids):
        ...
        token_type_mat = tf.equal(
            tf.expand_dims(token_type_ids, -1),
...
class TFFunnelEncoder(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.blocks = [
            [
                TFFunnelLayer(
                    config, block_index, name=f"blocks_._{block_index}_._{i}"
                )
            ...
            for layer_index, layer in enumerate(block):
                for repeat_index in range(self.block_repeats[block_index]):
                    ...
                    if do_pooling:
                        query = pooled_hidden
                        key = value = (
                            hidden if self.pool_q_only else pooled_hidden
                        )
                    else:
...
@add_start_docstrings(
    (
        "The bare Funnel Transformer Model transformer outputting raw"
...
class TFFunnelForMaskedLM(
    TFFunnelPreTrainedModel, TFMaskedLanguageModelingLoss
):
    def __init__(self, config: FunnelConfig, *inputs, **kwargs) -> None:
        super().__init__(config, *inputs, **kwargs)
...
class TFFunnelForQuestionAnswering(
    TFFunnelPreTrainedModel, TFQuestionAnsweringLoss
):
    def __init__(self, config: FunnelConfig, *inputs, **kwargs) -> None:
        super().__init__(config, *inputs, **kwargs)
        self.num_labels = config.num_labels

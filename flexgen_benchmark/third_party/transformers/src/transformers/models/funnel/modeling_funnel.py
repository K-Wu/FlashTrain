...
def load_tf_weights_in_funnel(model, config, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        ...
        for m_name in name[1:]:
            ...
            elif m_name == "r" and isinstance(
                pointer, FunnelRelMultiheadAttention
            ):
...
class FunnelAttentionStructure(nn.Module):
    ...
    def init_attention_inputs(
        ...
        cls_mask = (
            nn.functional.pad(
                inputs_embeds.new_ones([seq_len - 1, seq_len - 1]),
                (1, 0, 1, 0),
            )
    ...
    def stride_pool_pos(self, pos_id: torch.Tensor, block_index: int):
        ...
        if self.config.separate_cls:
            ...
            pooled_pos_id = (
                pos_id[1:-1] if self.config.truncate_seq else pos_id[1:]
            )
    ...
    def __init__(self, config: FunnelConfig) -> None:
        ...
        self.blocks = nn.ModuleList(
            [
                nn.ModuleList(
                    [
                        FunnelLayer(config, block_index)
        ...
        for block_index, block in enumerate(self.blocks):
            ...
            for layer_index, layer in enumerate(block):
                for repeat_index in range(
                    self.config.block_repeats[block_index]
                ):
                    do_pooling = (
                        (repeat_index == 0)
                        and (layer_index == 0)
...
class FunnelDiscriminatorPredictions(nn.Module):
    ...
    def forward(
        self, discriminator_hidden_states: torch.Tensor
    ) -> torch.Tensor:

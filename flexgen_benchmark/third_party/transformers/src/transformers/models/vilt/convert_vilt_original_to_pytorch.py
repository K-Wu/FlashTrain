...
def create_rename_keys(
    config, vqa_model=False, nlvr_model=False, irtr_model=False
):
    rename_keys = []
...
def read_in_q_k_v(state_dict, config):
    for i in range(config.num_hidden_layers):
        ...
        in_proj_weight = state_dict.pop(
            f"transformer.blocks.{i}.attn.qkv.weight"
        )
...
def convert_vilt_checkpoint(checkpoint_url, pytorch_dump_folder_path):
    ...
    config = ViltConfig(
        image_size=384, patch_size=32, tie_word_embeddings=False
    )
    mlm_model = False
    vqa_model = False
    nlvr_model = False
    ...
    if mlm_model:
        ...
        assert torch.allclose(
            outputs.logits[0, 0, :3], expected_slice, atol=1e-4
        )

...
if not is_torch_greater_or_equal_than_1_10:
    logger.warning(
        f"You are using torch=={torch.__version__}, but torch>=1.10.0 is"
...
class ViltEmbeddings(nn.Module):
    ...
    def visual_embed(self, pixel_values, pixel_mask, max_image_length=200):
        ...
        x_mask = nn.functional.interpolate(
            x_mask, size=(x.shape[2], x.shape[3])
        ).long()
        ...
        select = list()
        for i, (v, nv, p) in enumerate(
            zip(valid_nums, non_valid_nums, pad_nums)
        ):
            if p <= 0:
        ...
        patch_index = patch_index[select[:, 0], select[:, 1]].view(
            batch_size, -1, 2
        )
    ...
    def forward(
        ...
        if image_embeds is None:
            image_embeds, image_masks, patch_index = self.visual_embed(
                pixel_values,
                pixel_mask,
                max_image_length=self.config.max_image_length,
            )
        else:
...
class ViltLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = attention_output + hidden_states.to(
...
@add_start_docstrings(
    (
        "The bare ViLT Model transformer outputting raw hidden-states without"
...
class ViltModel(ViltPreTrainedModel):
    ...
    def forward(
        ...
        if pixel_values is not None and image_embeds is not None:
            raise ValueError(
                "You cannot specify both pixel_values and image_embeds at the"
                " same time"
            )
        ...
        if image_batch_size != text_batch_size:
            raise ValueError(
                "The text inputs and image inputs need to have the same batch"
                " size"
            )
...
class ViltForImagesAndTextClassification(ViltPreTrainedModel):
    ...
    def forward(
        ...
        num_images = (
            pixel_values.shape[1] if pixel_values is not None else None
        )
        if num_images is None:
            num_images = (
                image_embeds.shape[1] if image_embeds is not None else None
            )
        if num_images != self.config.num_images:
            raise ValueError(
                "Make sure to match the number of images in the model with the"
                " number of images in the input."
            )
        pooler_outputs = []
        ...
        for i in range(num_images):
            ...
            pooler_output = (
                outputs.pooler_output if return_dict else outputs[1]
            )

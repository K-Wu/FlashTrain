...
class TFDebertaContextPooler(tf.keras.layers.Layer):
    def __init__(self, config: DebertaConfig, **kwargs):
        ...
        self.dense = tf.keras.layers.Dense(
            config.pooler_hidden_size, name="dense"
        )
        self.dropout = TFDebertaStableDropout(
            config.pooler_dropout, name="dropout"
        )
        self.config = config
    ...
    def call(self, hidden_states, training: bool = False):
        ...
        pooled_output = get_tf_activation(self.config.pooler_hidden_act)(
            pooled_output
        )
    ...
    def xdropout(self, inputs):
        ...
        mask = tf.cast(
            ...
            - tf.compat.v1.distributions.Bernoulli(
                probs=1.0 - self.drop_prob
            ).sample(sample_shape=shape_list(inputs)),
            tf.bool,
        )
        scale = tf.convert_to_tensor(
            1.0 / (1 - self.drop_prob), dtype=tf.float32
        )
        if self.drop_prob > 0:
...
class TFDebertaEncoder(tf.keras.layers.Layer):
    ...
    def get_rel_embedding(self):
        rel_embeddings = (
            self.rel_embeddings if self.relative_attention else None
        )
...
def build_relative_position(query_size, key_size):
    ...
    rel_pos_ids = q_ids[:, None] - tf.tile(
        tf.reshape(k_ids, [1, -1]), [query_size, 1]
    )
...
class TFDebertaDisentangledSelfAttention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        attention_scores = tf.matmul(
            query_layer, tf.transpose(key_layer, [0, 1, 3, 2])
        )
        if self.relative_attention:
            rel_embeddings = self.pos_dropout(
                rel_embeddings, training=training
            )
        ...
        if self.talking_head:
            attention_scores = tf.transpose(
                self.head_logits_proj(
                    tf.transpose(attention_scores, [0, 2, 3, 1])
                ),
                [0, 3, 1, 2],
            )
        ...
        if self.talking_head:
            attention_probs = tf.transpose(
                self.head_weights_proj(
                    tf.transpose(attention_probs, [0, 2, 3, 1])
                ),
                [0, 3, 1, 2],
            )
    ...
    def disentangled_att_bias(
        ...
        # bxhxqxk
            ...
            raise ValueError(
                ...
                f" {len(shape_list_pos)}"
            )

        att_span = tf.cast(
            tf.minimum(
                tf.maximum(
                    shape_list(query_layer)[-2], shape_list(key_layer)[-2]
                ),
        ...
        if "c2p" in self.pos_att_type:
            ...
            c2p_att = tf.matmul(
                query_layer, tf.transpose(pos_key_layer, [0, 1, 3, 2])
            )
        ...
        if "p2c" in self.pos_att_type:
            ...
            p2c_att = tf.matmul(
                key_layer, tf.transpose(pos_query_layer, [0, 1, 3, 2])
            )
            p2c_att = tf.transpose(
                torch_gather(
                    p2c_att,
                    p2c_dynamic_expand(p2c_pos, query_layer, key_layer),
                    -1,
                ),
                [0, 1, 3, 2],
            )
...
class TFDebertaEmbeddings(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        ...
        if self.embedding_size != config.hidden_size:
            self.embed_proj = tf.keras.layers.Dense(
                config.hidden_size, use_bias=False
            )
        self.LayerNorm = tf.keras.layers.LayerNormalization(
            epsilon=config.layer_norm_eps, name="LayerNorm"
        )
    ...
    def call(
        ...
        if input_ids is None and inputs_embeds is None:
            raise ValueError(
                "Need to provide either `input_ids` or `input_embeds`."
            )
...
class TFDebertaLMPredictionHead(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.transform = TFDebertaPredictionHeadTransform(
...
@add_start_docstrings(
    (
        "The bare DeBERTa Model transformer outputting raw hidden-states"
...
class TFDebertaForMaskedLM(
    TFDebertaPreTrainedModel, TFMaskedLanguageModelingLoss
):
    def __init__(self, config: DebertaConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `TFDebertaForMaskedLM` make sure"
    ...
    def call(
        ...
        prediction_scores = self.mlm(
            sequence_output=sequence_output, training=training
        )
        loss = (
            None
            if labels is None
...
class TFDebertaForQuestionAnswering(
    TFDebertaPreTrainedModel, TFQuestionAnsweringLoss
):
    def __init__(self, config: DebertaConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)

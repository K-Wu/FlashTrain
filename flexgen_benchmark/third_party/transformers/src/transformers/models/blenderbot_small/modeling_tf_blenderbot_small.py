...
class TFBlenderbotSmallMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config: BlenderbotSmallConfig, **kwargs):
        ...
        self.encoder = TFBlenderbotSmallEncoder(
            config, self.shared, name="encoder"
        )
        self.decoder = TFBlenderbotSmallDecoder(
            config, self.shared, name="decoder"
        )
...
@add_start_docstrings(
    (
        "The bare BLENDERBOT_SMALL Model outputting raw hidden-states without"
...
@add_start_docstrings(
    (
        "The BLENDERBOT_SMALL Model with a language modeling head. Can be used"
        " for summarization."
    ),
    BLENDERBOT_SMALL_START_DOCSTRING,
)

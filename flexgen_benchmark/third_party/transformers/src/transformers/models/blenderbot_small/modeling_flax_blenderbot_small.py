...
class FlaxBlenderbotSmallEncoderLayer(nn.Module):
    ...
    def setup(self) -> None:
        ...
        self.self_attn_layer_norm = nn.LayerNorm(
            dtype=self.dtype, epsilon=1e-05
        )
...
class FlaxBlenderbotSmallEncoderLayerCollection(nn.Module):
    ...
    def setup(self):
        self.layers = [
            FlaxBlenderbotSmallEncoderLayer(
...
class FlaxBlenderbotSmallEncoder(nn.Module):
    ...
    def setup(self):
        ...
        self.embed_scale = (
            math.sqrt(embed_dim) if self.config.scale_embedding else 1.0
        )
        ...
        self.layers = FlaxBlenderbotSmallEncoderLayerCollection(
...
class FlaxBlenderbotSmallDecoder(nn.Module):
    ...
    def setup(self):
        ...
        self.layers = FlaxBlenderbotSmallDecoderLayerCollection(
            self.config, self.dtype
        )
        self.layernorm_embedding = nn.LayerNorm(
            dtype=self.dtype, epsilon=1e-05
...
@add_start_docstrings(
    (
        "The bare BlenderbotSmall Model transformer outputting raw"
...
class FlaxBlenderbotSmallForConditionalGenerationModule(nn.Module):
    ...
    def setup(self):
        self.model = FlaxBlenderbotSmallModule(
    ...
    def __call__(
        ...
        lm_logits += jax.lax.stop_gradient(
            self.final_logits_bias.astype(self.dtype)
        )

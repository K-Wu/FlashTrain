...
def convert_t5x_checkpoint_to_flax(
    ...
    if (
        ...
        and config.encoder_attention_type == "local"
    ):
        ...
        raise ValueError(
            "Given config is expected to have `model_type='t5'`, or"
            " `model_type='longt5` with `encoder_attention_type` attribute"
            " with a value from ['local', 'transient-global]."
        )
    ...
    for layer_index in range(config.num_layers):
        ...
        flax_model_encoder_layer_block["1"]["layer_norm"][
            "weight"
        ] = t5x_mlp_layer_norm
    ...
    flax_model.params["encoder"]["block"]["0"]["layer"]["0"][
        encoder_attn_name
    ...
    for layer_index in range(config.num_layers):
        ...
        t5x_enc_dec_attention_module = t5x_model["target"]["decoder"][
            layer_name
        ]["encoder_decoder_attention"]
        t5x_enc_dec_attention_key = t5x_enc_dec_attention_module["key"][
            "kernel"
        ]
        t5x_enc_dec_attention_out = t5x_enc_dec_attention_module["out"][
            "kernel"
        ]
        t5x_enc_dec_attention_query = t5x_enc_dec_attention_module["query"][
            "kernel"
        ]
        t5x_enc_dec_attention_value = t5x_enc_dec_attention_module["value"][
            "kernel"
        ]
        ...
        flax_model_decoder_layer_block["2"]["layer_norm"][
            "weight"
        ] = tx5_mlp_layer_norm

...
def _split_into_blocks(
    x: jnp.ndarray, block_len: int, axis: int
) -> jnp.ndarray:
    ...
    output_shape = (
        x.shape[:axis] + (num_blocks, block_len) + x.shape[(axis + 1) :]
    )
...
def _make_global_fixed_block_ids(
    ...
    def handle_orphan_tokens(block_ids: np.ndarray) -> jnp.ndarray:
        block_ends = (
            jnp.arange(seq_len) % global_block_size
        ) == global_block_size - 1
    ...
    global_segment_ids = (
        jnp.cumsum(jnp.ones((batch_size, num_globals)), axis=-1) - 1
    )
...
class FlaxLongT5LayerNorm(nn.Module):
    ...
    def setup(self):
        self.weight = self.param(
            "weight", self.weight_init, (self.hidden_size,)
        )
    ...
    def __call__(self, hidden_states):
        ...
        variance = jnp.power(hidden_states.astype("f4"), 2).mean(
...
class FlaxLongT5DenseActDense(nn.Module):
    ...
    def setup(self):
        wi_init_std = self.config.initializer_factor * (
            self.config.d_model**-0.5
        )
        wo_init_std = self.config.initializer_factor * (
...
class FlaxLongT5LayerFF(nn.Module):
    ...
    def setup(self):
        if self.config.is_gated_act:
            ...
            self.DenseReluDense = FlaxLongT5DenseActDense(
                self.config, dtype=self.dtype
            )
...
class FlaxLongT5Attention(nn.Module):
    ...
    def __call__(
        ...
        if self.causal:
            causal_attention_mask = make_causal_mask(
...
class FlaxLongT5LocalAttention(nn.Module):
    ...
    def _merge_heads(self, hidden_states):
        return hidden_states.reshape(
            hidden_states.shape[0], -1, self.inner_dim
        )
    ...
    def _create_position_bias(
        ...
        if self.has_relative_attention_bias:
            ...
            position_bias = jnp.zeros(
                (1, 1, self.n_heads, block_len, 3 * block_len),
                dtype=self.dtype,
            )
    ...
    def __call__(
        ...
        key_states = _concatenate_3_blocks(
            key_states, block_axis=1, sequence_axis=2
        )
        ...
        if attention_mask is not None:
            attention_mask = _get_local_attention_mask(
...
class FlaxLongT5TransientGlobalAttention(nn.Module):
    ...
    def __call__(
        ...
        value_states = jnp.concatenate(
            (value_states, side_value_states), axis=2
        )
        ...
        if position_bias is None:
            ...
            if local_attention_mask is not None:
                position_bias = position_bias + local_attention_mask.swapaxes(
                    1, 2
                )
    ...
    def setup(self):
        ...
        if self.causal:
            ...
            raise ValueError(
                ...
                f" {self.config.encoder_attention_type}."
            )
        self.layer = (
            attention_layer(
                self.config,
...
class FlaxLongT5BlockCollection(nn.Module):
    ...
    def __call__(
        ...
        all_cross_attentions = (
            () if (output_attentions and self.causal) else None
        )
        position_bias = None
        encoder_decoder_position_bias = None
...
class FlaxLongT5PreTrainedModel(FlaxPreTrainedModel):
    ...
    def __call__(
        ...
        if decoder_input_ids is None:
            raise ValueError(
                "Make sure to provide both `input_ids` and"
                " `decoder_input_ids`. `decoder_input_ids` is not passed here."
            )
...
@add_start_docstrings(
    (
        "The bare LONGT5 Model transformer outputting raw hidden-stateswithout"

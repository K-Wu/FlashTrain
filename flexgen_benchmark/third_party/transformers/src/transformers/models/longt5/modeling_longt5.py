...
def _split_into_blocks(
    x: torch.Tensor, block_len: int, dim: int
) -> torch.Tensor:
    ...
    output_shape = (
        x.shape[:dim] + (num_blocks, block_len) + x.shape[(dim + 1) :]
    )
...
def _make_3block_relative_position_ids(block_len: int) -> torch.Tensor:
    ...
    relative_position_ids = position_ids.unsqueeze(
        0
    ) - center_position_ids.unsqueeze(1)
...
def _get_local_attention_mask(
    ...
    local_attention_mask = _mask_local_attention_mask(
        local_attention_mask, block_len
    )
...
def _make_global_fixed_block_ids(
    ...
    def handle_orphan_tokens(block_ids: torch.Tensor) -> torch.Tensor:
        ...
        full_blocks = (
            true_block_ends.sum(-1).unsqueeze(-1).type(block_ids.dtype) - 1
        )
        block_ids = torch.where(
            block_ids < full_blocks, block_ids, full_blocks
        )
        return block_ids
    ...
    fixed_block_mask = (
        torch.cumsum(fixed_block_mask, axis=1) - fixed_block_mask
    )
    mask = torch.where(attention_mask != 0.0, 1.0, -1000.0).type(
    ...
    global_block_ids = (global_block_ids * attention_mask) + (
        attention_mask - 1
    )
    # [batch_size, seq_len]
    ...
    if num_globals > 0:
        ...
        _sequence_block_ids_max = torch.zeros(
            batch_size,
            0,
            dtype=global_block_ids.dtype,
            device=global_block_ids.device,
        )
    global_segment_ids = (
        torch.cumsum(torch.ones(batch_size, num_globals), dim=-1) - 1
    )
...
def _create_global_aggregates(
    ...
    block_ids = block_ids.where(
        block_ids >= 0,
        torch.tensor(
            global_seq_len, dtype=block_ids.dtype, device=block_ids.device
        ),
    )
    ...
    return torch.einsum(
        "...nd,...ng->...gd",
        hidden_states,
        one_hot_block_ids.type(hidden_states.dtype),
    )
...
try:
    ...
    logger.info(
        "Discovered apex.normalization.FusedRMSNorm - will use it instead of"
        " LongT5LayerNorm"
    )
except ImportError:
    ...
    logger.warning(
        "discovered apex but it failed to load, falling back to"
        " LongT5LayerNorm"
    )
    pass
...
class LongT5LayerFF(nn.Module):
    def __init__(self, config: LongT5Config):
        ...
        self.layer_norm = LongT5LayerNorm(
...
class LongT5Attention(nn.Module):
    def __init__(
        self, config: LongT5Config, has_relative_attention_bias=False
    ):
        super().__init__()
    ...
    def forward(
        ...
        outputs = (
            (attn_output,) + (present_key_value_state,) + (position_bias,)
        )
...
class LongT5LocalAttention(nn.Module):
    ...
    def forward(
        ...
        key_states = _concatenate_3_blocks(
            key_states, block_dim=1, sequence_dim=2
        )
        value_states = _concatenate_3_blocks(
            value_states, block_dim=1, sequence_dim=2
        )
...
class LongT5TransientGlobalAttention(nn.Module):
    ...
    def compute_side_bias(
        ...
        side_attention_mask = torch.eq(
            mask[..., None], global_segment_ids[:, None, :]
        )[:, None, ...]
        ...
        side_bias = self.global_relative_attention_bias(
    ...
    def forward(
        ...
        if mask is not None:
            ...
            local_attention_mask = torch.where(
                local_attention_mask > 0, 0.0, -1e10
            )
        else:
        ...
        if position_bias is None:
            ...
            if local_attention_mask is not None:
                ...
                position_bias = position_bias + local_attention_mask.transpose(
                    1, 2
                )
            ...
            position_bias = torch.cat(
                [position_bias, side_position_bias], dim=-1
            )
...
class LongT5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        if config.is_decoder:
            ...
            raise ValueError(
                "For encoder attention mechanism, either `local` or"
                " `transient-global` attention type is expected, but got"
                f" {config.encoder_attention_type}."
            )
    ...
    def forward(
        ...
        if past_key_value is not None:
            if not self.is_decoder:
                logger.warning(
                    "`past_key_values` is passed to the encoder. Please make"
            ...
            if len(past_key_value) != expected_num_past_key_values:
                raise ValueError(
                    f"There should be {expected_num_past_key_values} past"
                    " states."
                    f" {'2 (past / key) for cross attention. ' if expected_num_past_key_values == 4 else ''}Got"
                    f" {len(past_key_value)} past key / value states"
                )
...
class LongT5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, LongT5LayerNorm):
            ...
            module,
            (LongT5Model, LongT5ForConditionalGeneration, LongT5EncoderModel),
        ):
    ...
    def _shift_right(self, input_ids):
        ...
        assert decoder_start_token_id is not None, (
            ...
            " LongT5 it is usually set to the pad_token_id. See LongT5 docs"
    ...
    def forward(
        ...
        if use_cache is True:
            assert self.is_decoder, (
                f"`use_cache` can only be set to `True` if {self} is used as a"
                " decoder"
            )
...
@add_start_docstrings(
    (
        "The bare LONGT5 Model transformer outputting raw hidden-states"
...
@add_start_docstrings(
    (
        "The bare LONGT5 Model transformer outputting encoder's raw"

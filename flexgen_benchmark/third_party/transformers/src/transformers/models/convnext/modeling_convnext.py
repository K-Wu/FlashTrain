...
class ConvNextLayer(nn.Module):
    ...
    def __init__(self, config, dim, drop_path=0):
        ...
        self.layer_scale_parameter = (
            nn.Parameter(
                config.layer_scale_init_value * torch.ones((dim)),
                requires_grad=True,
            )
...
class ConvNextStage(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.Sequential(
            *[
                ConvNextLayer(
                    config, dim=out_channels, drop_path=drop_path_rates[j]
                )
...
@add_start_docstrings(
    (
        "The bare ConvNext model outputting raw features without any specific"
        " head on top."
    ),

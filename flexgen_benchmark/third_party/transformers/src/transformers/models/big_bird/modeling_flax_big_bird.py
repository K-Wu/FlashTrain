...
class FlaxBigBirdSelfAttention(nn.Module):
    ...
    def setup(self):
        ...
        if self.config.hidden_size % self.config.num_attention_heads != 0:
            raise ValueError(
                "`config.hidden_size`: {self.config.hidden_size} has to be a"
                " multiple of `config.num_attention_heads`                   "
...
class FlaxBigBirdBlockSparseAttention(nn.Module):
    ...
    def bigbird_block_sparse_attention(
        ...
        second_last_context_layer = jnp.expand_dims(
            second_last_context_layer, 2
        )
        ...
        context_layer = jnp.transpose(
            context_layer, axes=(0, 2, 1, 3)
        ).reshape(bsz, from_seq_len, -1)
...
class FlaxBigBirdSelfOutput(nn.Module):
    ...
    def __call__(
        self, hidden_states, input_tensor, deterministic: bool = True
    ):
...
class FlaxBigBirdAttention(nn.Module):
    ...
    def setup(self):
        if self.config.attention_type == "original_full":
            ...
            raise ValueError(
                "Your `config.attention_type` is"
                f" {self.config.attention_type} but it can either be"
                " `original_full` or `block_sparse`"
            )
...
class FlaxBigBirdOutput(nn.Module):
    ...
    def __call__(
        self, hidden_states, attention_output, deterministic: bool = True
    ):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(
            hidden_states, deterministic=deterministic
...
class FlaxBigBirdLayer(nn.Module):
    ...
    def setup(self):
        ...
        self.intermediate = FlaxBigBirdIntermediate(
...
class FlaxBigBirdLMPredictionHead(nn.Module):
    ...
    def setup(self):
        ...
        self.bias = self.param(
            "bias", self.bias_init, (self.config.vocab_size,)
        )
...
class FlaxBigBirdOnlyMLMHead(nn.Module):
    ...
    def setup(self):
        self.predictions = FlaxBigBirdLMPredictionHead(
            self.config, dtype=self.dtype
        )
    ...
    def __call__(
        ...
        if self.config.add_cross_attention:
            ...
            if past_key_values is not None and return_dict:
                ...
                outputs = (
                    outputs[:1]
                    + (unfreeze(past_key_values["cache"]),)
                    + outputs[1:]
                )
...
class FlaxBigBirdForPreTrainingModule(nn.Module):
    ...
    def setup(self):
        ...
        self.cls = FlaxBigBirdPreTrainingHeads(
...
class FlaxBigBirdForSequenceClassificationModule(nn.Module):
    ...
    def setup(self):
        ...
        self.classifier = FlaxBigBirdClassificationHead(
            self.config, dtype=self.dtype
        )
...
class FlaxBigBirdForQuestionAnsweringHead(nn.Module):
    ...
    def __call__(self, encoder_output, deterministic=True):
        hidden_states = self.dropout(
            encoder_output, deterministic=deterministic
        )
...
class FlaxBigBirdForQuestionAnswering(FlaxBigBirdPreTrainedModel):
    ...
    def __call__(
        ...
        if question_lengths is None and input_ids is not None:
            ...
            question_lengths = (
                jnp.argmax(
                    (input_ids == self.config.sep_token_id).astype("i4"),
                    axis=-1,
                )
                + 1
            )

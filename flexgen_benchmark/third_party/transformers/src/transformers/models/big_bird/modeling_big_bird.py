...
def load_tf_weights_in_big_bird(model, tf_checkpoint_path, is_trivia_qa=False):
    ...
    def load_tf_weights_bert(init_vars, tf_path):
        ...
        for name, shape in init_vars:
            ...
            name = name.replace(
                "bert/encoder/LayerNorm", "bert/embeddings/LayerNorm"
            )
    ...
    for txt_name in names:
        ...
        try:
            ...
            if pointer.shape != array.shape:
                raise ValueError(
                    ...
                    f" {array.shape} mismatched of {txt_name}."
                )
        ...
        logger.info(
            f"Initialize PyTorch weight {pt_weight_name} from {txt_name}."
        )
    ...
    logger.info(
        f"Weights not initialized in PyTorch model: {', '.join(pt_names)}."
    )
    return model
...
class BigBirdBlockSparseAttention(nn.Module):
    ...
    def forward(
        ...
        if to_seq_length % to_block_size != 0:
            raise ValueError(
                "Key/Value sided sequence length must be multiple of block"
                " size"
            )
    ...
    def torch_bmm_nd(inp_1, inp_2, ndim=None):
        ...
        return torch.bmm(
            ...
            inp_1.shape[: ndim - 2]
            + (inp_1.shape[ndim - 2], inp_2.shape[ndim - 1])
        )
    ...
    def bigbird_block_sparse_attention(
        ...
        first_context_layer = self.torch_bmm_nd(
            first_attn_weights, value_layer, ndim=4
        )
        ...
        # [bsz, n_heads, from_block_size, to_seq_len] x [bsz, n_heads, to_seq_len, -1] ==> [bsz, n_heads, from_block_size, -1]
        last_context_layer = self.torch_bmm_nd(
            last_attn_weights, value_layer, ndim=4
        )
        ...
        context_layer = (
            context_layer.view((bsz, n_heads, from_seq_len, -1)) * from_mask
        )
        ...
        if output_attentions:
            ...
            attention_probs[
                :,
                :,
                2 * from_block_size : -2 * from_block_size,
                :to_block_size,
            ...
            attention_probs[
                :,
                :,
                -2 * from_block_size : -from_block_size,
                -3 * to_block_size :,
            ] = second_last_attn_weights[
            ...
            # random keys
            for p1, i1, w1 in zip(
                range(bsz), rand_attn, second_last_attn_weights
            ):
                # p1, i1, w1 corresponds to batch_dim i.e. following operation is done for each sequence in batch
                for p2, i2, w2 in zip(range(n_heads), i1, w1):
                    # p2, i2, w2 corresponds to head_dim i.e. following operation is done for each heads
    ...
    def torch_gather_b2(params, indices):
        ...
        if params.shape[:2] != indices.shape[:2]:
            raise ValueError(
                "Make sure that the first two dimensions of params and"
                " indices are identical,                 but they are params:"
                f" {params.shape[:2]} vs. indices: {indices.shape[:2]}"
            )
        ...
        flattened_params = params.reshape(
            -1, params.shape[-2], params.shape[-1]
        )
    ...
    def _get_rand_attn_plan(from_seq_length, from_block_size, num_rand_blocks):
        ...
        if (2 * num_rand_blocks + 5) < (from_seq_length // from_block_size):
            ...
            plan_from_length.append(
                int((num_rand_blocks + 5) * from_block_size)
            )
            plan_num_rand_blocks.append(num_rand_blocks // 2)
            plan_from_length.append(from_seq_length)
            plan_num_rand_blocks.append(
                num_rand_blocks - (num_rand_blocks // 2)
            )
        else:
            plan_from_length.append(from_seq_length)
            plan_num_rand_blocks.append(num_rand_blocks)
    ...
    def _bigbird_block_rand_mask(
        ...
        for i in range(1, from_seq_length // from_block_size - 1):
            ...
            if i == 1:
                ...
                rand_attn[i - 1, :] = np.random.permutation(middle_seq[:last])[
                    :r
                ]
        ...
        for plan_idx in range(max_plan_idx + 1):
            ...
            if plan_idx > 0:
                ...
                if plan_num_rand_blocks[plan_idx] > 0:
                    ...
                    curr_r_cnt = int(
                        np.sum(plan_num_rand_blocks[: plan_idx + 1])
                    )
                    for blk_rw_idx in range(
                        ...
                        for h in range(num_heads):
                            rand_attn[h][
                                ...
                                block_id=blk_rw_idx,
                                to_start_block_id=plan_block_length[
                                    plan_idx - 1
                                ],
                ...
                for pl_id in range(plan_idx):
                    ...
                    for blk_rw_idx in range(
                        ...
                        if pl_id > 0:
                            rnd_r_cnt = int(
                                np.sum(plan_num_rand_blocks[:pl_id])
                            )
                            to_start_block_id = plan_block_length[pl_id - 1]
                        curr_r_cnt = int(
                            np.sum(plan_num_rand_blocks[: pl_id + 1])
                        )
                        for h in range(num_heads):
                            rand_attn[h][
                                blk_rw_idx, rnd_r_cnt:curr_r_cnt
    ...
    def _get_single_block_row_attention(
        ...
        illegal_blocks = list(
            range(
                block_id - window_block_left, block_id + window_block_right + 1
            )
        )
...
class BigBirdAttention(nn.Module):
    def __init__(self, config, seed=None):
        ...
        if self.config.attention_type == "original_full":
            ...
            raise ValueError(
                "attention_type can either be original_full or block_sparse,"
                f" but is {self.config.attention_type}"
            )
    ...
    def set_attention_type(self, value: str):
        if value not in ["original_full", "block_sparse"]:
            raise ValueError(
                "attention_type can only be set to either 'original_full' or"
                f" 'block_sparse', but is {value}"
            )
    ...
    def forward(
        ...
        if self.attention_type == "original_full":
            ...
            if encoder_hidden_states is not None:
                raise ValueError(
                    "BigBird cannot be used as a decoder when"
    ...
    def __init__(self, config, seed=None):
        ...
        if self.add_cross_attention:
            if not self.is_decoder:
                raise TypeError(
                    f"{self} should be used as a decoder model if cross"
                    " attention is added"
                )
...
@add_start_docstrings(
    (
        "The bare BigBird Model transformer outputting raw hidden-states"
...
class BigBirdModel(BigBirdPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        if (
            ...
            and config.add_cross_attention
        ):
            logger.warning(
                "When using `BigBirdForCausalLM` as decoder, then"
                " `attention_type` must be `original_full`. Setting"
    ...
    def forward(
        ...
        if (
            ...
            and seq_length <= max_tokens_to_attend
        ):
            ...
            logger.warning(
                "Attention type 'block_sparse' is not possible if"
                f" sequence_length: {sequence_length} <= num global tokens: 2"
                " * config.block_size + min. num sliding tokens: 3 *"
                " config.block_size + config.num_random_blocks *"
                " config.block_size + additional buffer:"
                ...
                f" {self.config.num_random_blocks}. Changing attention type to"
                " 'original_full'..."
            )
            self.set_attention_type("original_full")
    ...
    def create_masks_for_block_sparse_attn(
        ...
        if seq_length % block_size != 0:
            raise ValueError(
                "Sequence length must be multiple of block size, but sequence"
                f" length is {seq_length}, while block size is {block_size}."
            )
...
class BigBirdForMaskedLM(BigBirdPreTrainedModel):
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `BigBirdForMaskedLM` make sure"
...
class BigBirdForCausalLM(BigBirdPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning(
                "If you want to use `BigBirdForCausalLM` as a standalone, add"
                " `is_decoder=True.`"
            )
...
class BigBirdForMultipleChoice(BigBirdPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        BIG_BIRD_INPUTS_DOCSTRING.format(

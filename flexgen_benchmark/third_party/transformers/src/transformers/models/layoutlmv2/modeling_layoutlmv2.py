...
class LayoutLMv2Encoder(nn.Module):
    ...
    def _calculate_2d_position_embeddings(self, hidden_states, bbox):
        ...
        rel_pos_x_2d_mat = position_coord_x.unsqueeze(
            -2
        ) - position_coord_x.unsqueeze(-1)
        rel_pos_y_2d_mat = position_coord_y.unsqueeze(
            -2
        ) - position_coord_y.unsqueeze(-1)
...
def my_convert_sync_batchnorm(module, process_group=None):
    ...
    if isinstance(module, torch.nn.modules.batchnorm._BatchNorm):
        return nn.modules.SyncBatchNorm.convert_sync_batchnorm(
    ...
    for name, child in module.named_children():
        module_output.add_module(
            name, my_convert_sync_batchnorm(child, process_group)
        )
    del module
...
class LayoutLMv2VisualBackbone(nn.Module):
    def __init__(self, config):
        ...
        if torch.are_deterministic_algorithms_enabled():
            ...
            input_shape = (224, 224)
            backbone_stride = self.backbone.output_shape()[
                self.out_feature_key
            ].stride
    ...
    def forward(self, images):
        images_input = (
            (images if torch.is_tensor(images) else images.tensor)
            - self.pixel_mean
        ) / self.pixel_std
    ...
    def synchronize_batch_norm(self):
        if not (
            ...
            raise RuntimeError(
                "Make sure torch.distributed is set up properly."
            )
        ...
        if not (world_size & node_size == 0):
            raise RuntimeError(
                "Make sure the number of processes can be divided by the"
                " number of nodes"
            )
...
@add_start_docstrings(
    (
        "The bare LayoutLMv2 Model transformer outputting raw hidden-states"
...
class LayoutLMv2Model(LayoutLMv2PreTrainedModel):
    ...
    def _calc_text_embeddings(
        ...
        spatial_position_embeddings = (
            self.embeddings._calc_spatial_position_embeddings(bbox)
        )
    ...
    def _calc_visual_bbox(
        self, image_feature_pool_shape, bbox, device, final_shape
    ):

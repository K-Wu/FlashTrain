        ...
        **kwargs,
        ...
        if not valid_images:
            raise ValueError(
                ...
                f" examples), but is of type {type(images)}."
            )
        ...
        if self.do_resize and self.size is not None:
            images = [
                self.resize(
                    image=image, size=self.size, resample=self.resample
                )
                for image in images
            ]

        images = [
            self.to_numpy_array(image, rescale=False) for image in images
        ]

...
class LayoutLMv2Processor(ProcessorMixin):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.feature_extractor.apply_ocr and (boxes is not None):
            raise ValueError(
                "You cannot provide bounding boxes if you initialized the"
                " feature extractor with apply_ocr set to True."
            )

        if self.feature_extractor.apply_ocr and (word_labels is not None):
            raise ValueError(
                "You cannot provide word labels if you initialized the feature"
                " extractor with apply_ocr set to True."
            )

        if (
            return_overflowing_tokens is True
            and return_offsets_mapping is False
        ):
            raise ValueError(
                "You cannot return overflowing tokens without returning the"
                " offsets mapping."
            )
        ...
        if (
            text is not None
            and self.feature_extractor.apply_ocr
    ...
    def get_overflowing_images(self, images, overflow_to_sample_mapping):
        ...
        if len(images_with_overflow) != len(overflow_to_sample_mapping):
            raise ValueError(
                "Expected length of images to be the same as the length of"
                " `overflow_to_sample_mapping`, but got"
                f" {len(images_with_overflow)} and"
                f" {len(overflow_to_sample_mapping)}"
            )

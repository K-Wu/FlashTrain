...
class FlaxCLIPVisionEmbeddings(nn.Module):
    ...
    def __call__(self, pixel_values):
        ...
        patch_embeds = jnp.reshape(
            patch_embeds, (batch_size, height * width, channels)
        )
...
class FlaxCLIPTextEmbeddings(nn.Module):
    ...
    def setup(self):
        ...
        self.position_ids = jnp.expand_dims(
            jnp.arange(0, self.config.max_position_embeddings, dtype="i4"),
            axis=(0, 1),
        )
...
class FlaxCLIPVisionTransformer(nn.Module):
    ...
    def setup(self):
        self.embeddings = FlaxCLIPVisionEmbeddings(
...
class FlaxCLIPTextModule(nn.Module):
    ...
    def setup(self):
        self.text_model = FlaxCLIPTextTransformer(
            self.config, dtype=self.dtype
        )
...
overwrite_call_docstring(
    FlaxCLIPTextModel,
    CLIP_TEXT_INPUTS_DOCSTRING + FLAX_CLIP_TEXT_MODEL_DOCSTRING,
)
append_replace_return_docstrings(
    FlaxCLIPTextModel,
...
overwrite_call_docstring(
    FlaxCLIPVisionModel,
    CLIP_VISION_INPUTS_DOCSTRING + FLAX_CLIP_VISION_MODEL_DOCSTRING,
)
append_replace_return_docstrings(
    FlaxCLIPVisionModel,
    output_type=FlaxBaseModelOutputWithPooling,

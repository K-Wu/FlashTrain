        ...
        with tf.name_scope("position_embedding"):
            self.position_embedding = self.add_weight(
                ...
                initializer=get_initializer(
                    self.config.initializer_range * factor
                ),
                trainable=True,
                name="embeddings",
            )
...
class TFCLIPVisionMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config: CLIPVisionConfig, **kwargs):
        super().__init__(**kwargs)
        self.config = config
        self.vision_model = TFCLIPVisionTransformer(
...
class TFCLIPMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config: CLIPConfig, **kwargs):
        ...
        if not isinstance(config.text_config, CLIPTextConfig):
            raise ValueError(
                "config.text_config is expected to be of type CLIPTextConfig"
        ...
        if not isinstance(config.vision_config, CLIPVisionConfig):
            raise ValueError(
                ...
                " CLIPVisionConfig but is of type"
...
class TFCLIPModel(TFCLIPPreTrainedModel):
    ...
    @replace_return_docstrings(
        output_type=TFCLIPOutput, config_class=CLIPConfig
    )
    def call(
        self,
        input_ids: Optional[TFModelInputType] = None,

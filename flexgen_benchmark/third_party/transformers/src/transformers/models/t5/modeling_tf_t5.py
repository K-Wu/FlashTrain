...
class TFT5LayerNorm(tf.keras.layers.Layer):
    ...
    def call(self, hidden_states):
        ...
        hidden_states = hidden_states * tf.math.rsqrt(
...
class TFT5DenseActDense(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.wi = tf.keras.layers.Dense(
            ...
            use_bias=False,
            name="wi",
            kernel_initializer=wi_initializer,
        )  # Update init weights as in flax
        self.wo = tf.keras.layers.Dense(
            config.d_model,
            use_bias=False,
            name="wo",
            kernel_initializer=wo_initializer,
        )  # Update init weights as in flax
...
class TFT5LayerFF(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        if config.is_gated_act:
            ...
            self.DenseReluDense = TFT5DenseActDense(
                config, name="DenseReluDense"
            )
    ...
    def call(self, hidden_states, training=False):
        ...
        hidden_states = hidden_states + self.dropout(
            dense_output, training=training
        )
        return hidden_states
...
class TFT5Attention(tf.keras.layers.Layer):
    ...
    def __init__(self, config, has_relative_attention_bias=False, **kwargs):
        ...
        self.relative_attention_bias_initializer = (
            tf.keras.initializers.RandomNormal(
                mean=0,
                stddev=config.initializer_factor * (self.inner_dim**-0.5),
            )
        )
        ...
        self.q = tf.keras.layers.Dense(
            ...
            use_bias=False,
            name="q",
            kernel_initializer=q_initializer,
        )  # Update init weights as in flax
        ...
        self.v = tf.keras.layers.Dense(
            ...
            use_bias=False,
            name="v",
            kernel_initializer=v_initializer,
        )  # Update init weights as in flax
        self.o = tf.keras.layers.Dense(
            self.d_model,
            use_bias=False,
            name="o",
            kernel_initializer=o_initializer,
        )  # Update init weights as in flax
        self.dropout = tf.keras.layers.Dropout(config.dropout_rate)
        ...
        if layer_head_mask is not None:
            tf.debugging.assert_equal(
                ...
                message=(
                    ...
                    f" {(self.n_heads)}, but is {shape_list(layer_head_mask)}"
                ),
            )
...
class TFT5Block(tf.keras.layers.Layer):
    def __init__(self, config, has_relative_attention_bias=False, **kwargs):
        ...
        self.layer.append(
            TFT5LayerFF(config, name=f"layer_._{len(self.layer)}")
        )
...
class TFT5MainLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        present_key_value_states = (
            () if use_cache and self.is_decoder else None
        )
...
class TFT5PreTrainedModel(TFPreTrainedModel):
    ...
    def _shift_right(self, input_ids):
        ...
        assert decoder_start_token_id is not None, (
            "self.model.config.decoder_start_token_id has to be defined. In TF"
            " T5 it is usually set to the pad_token_id. See T5 docs for more"
            " information"
        )
...
class TFT5Model(TFT5PreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.encoder = TFT5MainLayer(
            encoder_config, self.shared, name="encoder"
        )
        ...
        self.decoder = TFT5MainLayer(
            decoder_config, self.shared, name="decoder"
        )
    ...
    def call(
        ...
        if not return_dict:
            if past is not None:
                decoder_outputs = (
                    decoder_outputs[:1] + (past,) + decoder_outputs[2:]
                )
...
class TFT5ForConditionalGeneration(
    TFT5PreTrainedModel, TFCausalLanguageModelingLoss
):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
    ...
    def call(
        ...
        if self.config.tie_word_embeddings:
            ...
            logits = tf.matmul(
                sequence_output, self.shared.weights, transpose_b=True
            )
        else:
...
@add_start_docstrings(
    (
        "The bare T5 Model transformer outputting encoder's raw"

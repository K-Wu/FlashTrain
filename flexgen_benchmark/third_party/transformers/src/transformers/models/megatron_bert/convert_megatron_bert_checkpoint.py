...
def convert_megatron_checkpoint(args, input_state_dict, config):
    ...
    # The transformer.
    transformer = (
        lm["transformer"] if "transformer" in lm.keys() else lm["encoder"]
    )
    ...
    for key, val in transformer.items():
        ...
        # Transpose the bias.
            ...
            output_state_dict[
                f"{layer_name}.attention.self.query.bias"
            ] = q_bias
            ...
            output_state_dict[
                f"{layer_name}.attention.self.value.bias"
            ] = v_bias
    ...
    # The final layernorm.
    output_state_dict["bert.encoder.ln.weight"] = transformer[
        "final_layernorm.weight"
    ]
    output_state_dict["bert.encoder.ln.bias"] = transformer[
...
def main():
    ...
    if args.path_to_checkpoint.endswith(".zip"):
        ...
        input_state_dict = torch.load(
            args.path_to_checkpoint, map_location="cpu"
        )
    ...
    if args.config_file == "":
        ...
        config.vocab_size = input_state_dict["model"]["lm_head"][
            "bias"
        ].numel()
    else:

...
class DeformableDetrConvModel(nn.Module):
    ...
    def forward(self, pixel_values, pixel_mask):
        ...
        for feature_map, mask in out:
            # position encoding
            pos.append(
                self.position_embedding(feature_map, mask).to(
                    feature_map.dtype
                )
            )
...
def _expand_mask(
    ...
    expanded_mask = (
        mask[:, None, None, :]
        .expand(batch_size, 1, target_len, source_len)
        .to(dtype)
    )
...
class DeformableDetrMultiscaleDeformableAttention(nn.Module):
    ...
    def __init__(
        ...
        if embed_dim % num_heads != 0:
            raise ValueError(
                "embed_dim (d_model) must be divisible by num_heads, but got"
                f" {embed_dim} and {num_heads}"
            )
        dim_per_head = embed_dim // num_heads
        # check if dim_per_head is power of 2
        if not (
            (dim_per_head & (dim_per_head - 1) == 0) and dim_per_head != 0
        ):
            warnings.warn(
                "You'd better set embed_dim (d_model) in"
                " DeformableDetrMultiscaleDeformableAttention to make the"
                " dimension of each attention head a power of 2 which is more"
                " efficient in the authors' CUDA implementation."
            )
    ...
    def with_pos_embed(
        ...
        return (
            ...
            else tensor + position_embeddings
        )
    ...
    def forward(
        ...
        if position_embeddings is not None:
            hidden_states = self.with_pos_embed(
        ...
        if (
            ...
            raise ValueError(
                "Make sure to align the spatial shapes with the sequence"
                " length of the encoder hidden states"
            )
    ...
    def _init_weights(self, module):
        ...
        if hasattr(module, "reference_points") and not self.config.two_stage:
            nn.init.xavier_uniform_(
                module.reference_points.weight.data, gain=1.0
            )
...
class DeformableDetrEncoder(DeformableDetrPreTrainedModel):
    ...
    def __init__(self, config: DeformableDetrConfig):
        ...
        self.layers = nn.ModuleList(
            [
                DeformableDetrEncoderLayer(config)
    ...
    def get_reference_points(spatial_shapes, valid_ratios, device):
        ...
        for level, (height, width) in enumerate(spatial_shapes):
            ...
            ref_y = ref_y.reshape(-1)[None] / (
                valid_ratios[:, None, level, 1] * height
            )
            ref_x = ref_x.reshape(-1)[None] / (
                valid_ratios[:, None, level, 0] * width
            )
...
class DeformableDetrDecoder(DeformableDetrPreTrainedModel):
    ...
    def __init__(self, config: DeformableDetrConfig):
        ...
        self.layers = nn.ModuleList(
            [
                DeformableDetrDecoderLayer(config)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                if reference_points.shape[-1] == 4:
                    ...
                    if reference_points.shape[-1] != 2:
                        raise ValueError(
                            "Reference points' last dimension must be of size"
                            f" 2, but is {reference_points.shape[-1]}"
                        )
...
class DeformableDetrModel(DeformableDetrPreTrainedModel):
    ...
    def gen_encoder_output_proposals(
        ...
        for level, (height, width) in enumerate(spatial_shapes):
            ...
            grid = (
                grid.unsqueeze(0).expand(batch_size, -1, -1, -1) + 0.5
            ) / scale
            width_heigth = torch.ones_like(grid) * 0.05 * (2.0**level)
            proposal = torch.cat((grid, width_heigth), -1).view(
                batch_size, -1, 4
            )
        ...
        object_query = object_query.masked_fill(
            padding_mask.unsqueeze(-1), float(0)
        )
        object_query = object_query.masked_fill(
    ...
    def forward(
        ...
        features, position_embeddings_list = self.backbone(
        ...
        if self.config.num_feature_levels > len(sources):
            ...
            for level in range(_len_sources, self.config.num_feature_levels):
                ...
                pos_l = self.backbone.position_embedding(source, mask).to(
                    source.dtype
                )
        ...
        if self.config.two_stage:
            ...
            query_embed, target = torch.split(
                pos_trans_out, num_channels, dim=2
            )
        else:
            query_embed, target = torch.split(
                query_embeds, num_channels, dim=1
            )
...
class DeformableDetrForObjectDetection(DeformableDetrPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(DEFORMABLE_DETR_INPUTS_DOCSTRING)
    @replace_return_docstrings(
        output_type=DeformableDetrObjectDetectionOutput,
    ...
    def forward(
        ...
        init_reference = (
            outputs.init_reference_points if return_dict else outputs[0]
        )
        inter_references = (
        ...
        for level in range(hidden_states.shape[1]):
            ...
            if reference.shape[-1] == 4:
                ...
                raise ValueError(
                    "reference.shape[-1] should be 4 or 2, but got"
                    f" {reference.shape[-1]}"
                )
        ...
        if not return_dict:
            ...
            tuple_outputs = (
                ((loss, loss_dict) + output) if loss is not None else output
            )
...
class DeformableDetrLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        if "auxiliary_outputs" in outputs:
            for i, auxiliary_outputs in enumerate(
...
class DeformableDetrHungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        class_cost = (
            pos_cost_class[:, target_ids] - neg_cost_class[:, target_ids]
        )
...
def generalized_box_iou(boxes1, boxes2):
    ...
    if not (boxes1[:, 2:] >= boxes1[:, :2]).all():
        raise ValueError(
            "boxes1 must be in [x0, y0, x1, y1] (corner) format, but got"
            f" {boxes1}"
        )
    if not (boxes2[:, 2:] >= boxes2[:, :2]).all():
        raise ValueError(
            "boxes2 must be in [x0, y0, x1, y1] (corner) format, but got"
            f" {boxes2}"
        )

...
def read_in_q_k_v(state_dict):
    ...
    for i in range(6):
        ...
        in_proj_weight = state_dict.pop(
            f"decoder.layers.{i}.self_attn.in_proj_weight"
        )
        in_proj_bias = state_dict.pop(
            f"decoder.layers.{i}.self_attn.in_proj_bias"
        )
...
def convert_deformable_detr_checkpoint(
    ...
    prefix = "model."
    for key in state_dict.copy().keys():
        if not key.startswith("class_embed") and not key.startswith(
            "bbox_embed"
        ):
            val = state_dict.pop(key)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dilation",
        action="store_true",
        help="Whether to set config.dilation=True.",
    )
    parser.add_argument(
        "--with_box_refine",
        action="store_true",
    ...
    parser.add_argument(
        "--two_stage",
        action="store_true",
        help="Whether to set config.two_stage=True.",
    )
    parser.add_argument(
        "--pytorch_dump_folder_path",
        default=None,

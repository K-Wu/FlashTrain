    ...
    def __call__(
        ...
        **kwargs,
        ...
        questions = (
            ...
            else [questions] * n_passages
        )
        assert len(titles) == len(texts), (
            "There should be as many titles than texts but got"
            f" {len(titles)} titles and {len(texts)} texts."
        )
        ...
        if return_attention_mask is not False:
            ...
            for input_ids in encoded_inputs["input_ids"]:
                attention_mask.append(
                    [
                        int(input_id != self.pad_token_id)

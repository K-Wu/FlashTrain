...
_CTC_EXPECTED_OUTPUT = (
    "'MISTER QUILTER IS THE APPOSTILE OF THE MIDDLE CLASSES AND WE ARE GLAD TO"
    " WELCOME HIS GOSPOLLE'"
)
...
class SEWEncoder(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attention_ids = (
                torch.arange(
                    0, max_encoder_length, device=output_lengths.device
                )
                .view(1, -1)
                .expand(output_lengths.shape[0], -1)
            )
            attention_mask = (
                attention_ids < output_lengths.view(-1, 1)
            ).long()
        ...
        min_length = min(
            position_embeddings.size(-1), pooled_hidden_states.size(-1)
        )
        hidden_states = (
        ...
        if hidden_states.shape[1] < n_input_timesteps:
            hidden_states = nn.functional.pad(
                hidden_states,
                (0, 0, 0, n_input_timesteps - hidden_states.shape[1]),
            )
...
@add_start_docstrings(
    (
        "The bare SEW Model transformer outputting raw hidden-states without"
    ...
    def __init__(self, config):
        ...
        if config.vocab_size is None:
            raise ValueError(
                ...
                " follows: `SEWForCTC.from_pretrained(...,"
...
class SEWForSequenceClassification(SEWPreTrainedModel):
    def __init__(self, config):
        ...
        if hasattr(config, "add_adapter") and config.add_adapter:
            raise ValueError(
                "Sequence classification does not support the use of SEW"

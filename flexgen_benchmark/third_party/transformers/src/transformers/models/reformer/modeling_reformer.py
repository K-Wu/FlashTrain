...
def _get_least_common_mult_chunk_len(config):
    ...
    if len(attn_types_set) == 1 and attn_types[0] == "lsh":
        ...
        return np.lcm(
            config.lsh_attn_chunk_length, config.local_attn_chunk_length
        )
    else:
        raise NotImplementedError(
            "Only attn layer types 'lsh' and 'local' exist, but"
            f" `config.attn_layers`: {config.attn_layers}. Select attn layer"
            " types from ['lsh', 'local'] only."
        )
...
class AxialPositionEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if sum(self.axial_pos_embds_dim) != config.hidden_size:
            raise ValueError(
                "Make sure that config.axial_pos_embds factors:"
                f" {self.axial_pos_embds_dim} sum to config.hidden_size:"
        ...
        for axis, axial_pos_embd_dim in enumerate(self.axial_pos_embds_dim):
            ...
            self.weights.append(
                nn.Parameter(torch.ones(ax_shape, dtype=torch.float32))
            )
    ...
    def forward(self, position_ids):
        ...
        broadcasted_weights = [
            weight.expand(
                (batch_size,) + self.axial_pos_shape + weight.shape[-1:]
            )
        ...
        if self.training is True:
            if reduce(mul, self.axial_pos_shape) != sequence_length:
                raise ValueError(
                    "If training, make sure that config.axial_pos_shape"
                    f" factors: {self.axial_pos_shape} multiply to sequence"
                    f" length. Got prod({self.axial_pos_shape}) !="
                    f" sequence_length: {sequence_length}. You might want to"
                    " consider padding your sequence length to"
                    f" {reduce(mul, self.axial_pos_shape)} or changing"
            ...
            if self.dropout > 0:
                ...
                position_encodings = torch.cat(
                    [
                        torch.reshape(
                            weight, (batch_size, sequence_length, -1)
                        )
            ...
            if reduce(mul, self.axial_pos_shape) < sequence_length:
                raise ValueError(
                    "Make sure that config.axial_pos_shape factors:"
                    f" {self.axial_pos_shape} multiply at least to"
                    " max(sequence_length, least_common_mult_chunk_length):"
                    f" max({sequence_length},"
            ...
            position_encodings = torch.reshape(
                position_encodings,
                (batch_size, -1, position_encodings.shape[-1]),
            )
...
class ReformerEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if position_ids.shape[-1] > self.max_position_embeddings:
            raise ValueError(
                f"Sequence Length: {position_ids.shape[-1]} has to be less or"
                " equal than config.max_position_embeddings"
...
class EfficientAttentionMixin:
    ...
    def _merge_hidden_size_dims(self, x, num_attn_heads, attn_head_size):
        ...
        return torch.reshape(
            x, (x.size()[0], -1, num_attn_heads * attn_head_size)
        )
...
class LSHSelfAttention(nn.Module, EfficientAttentionMixin):
        ...
        **kwargs,
        ...
        if do_cached_attention:
            assert sequence_length == 1, (
                "At the moment, auto-regressive language generation is only"
                " possible one word at a time. Make sure that input sequence"
                f" length {sequence_length} equals 1, when"
                " `past_buckets_states` is passed."
            )
        ...
        assert query_key_vectors.shape[-1] == self.attention_head_size, (
            "last dim of query_key_vectors is"
            f" {query_key_vectors.shape[-1]} but should be"
            f" {self.attention_head_size}."
        )
        assert value_vectors.shape[-1] == self.attention_head_size, (
            f"last dim of value_vectors is {value_vectors.shape[-1]} but"
            f" should be {self.attention_head_size}."
        )

        do_standard_self_attention = (
            sequence_length <= self.chunk_length
        ) or (use_cache and past_buckets_states[1] is not None)
        ...
        if not do_standard_self_attention:
            ...
            assert int(buckets.shape[-1]) == num_hashes * sequence_length, (
                f"last dim of buckets is {buckets.shape[-1]}, but should be"
                f" {num_hashes * sequence_length}"
            )
            ...
            if self.chunk_length is None:
                assert (
                    ...
                    "If `config.chunk_length` is `None`, make sure"
                    " `config.num_chunks_after` and `config.num_chunks_before`"
                    " are set to 0."
                )
        ...
        assert out_vectors.shape == (
            ...
            "out_vectors have be of shape `[batch_size,"
            " config.num_attention_heads, sequence_length,"
    ...
    def _hash_vectors(
        ...
        if isinstance(self.num_buckets, int):
            assert self.num_buckets % 2 == 0, (
                "There should be an even number of buckets, but"
                f" `self.num_buckets`: {self.num_buckets}"
            )
            ...
            for bucket_factor in self.num_buckets:
                assert bucket_factor % 2 == 0, (
                    "The number of buckets should be even, but `num_bucket`:"
                    f" {bucket_factor}"
                )
        ...
        if isinstance(self.num_buckets, int) or len(self.num_buckets) == 1:
            ...
            for bucket_factor in self.num_buckets:
                ...
                if buckets is None:
                    ...
                    buckets = buckets + (
                        cur_product
                        * torch.argmax(rotated_vectors_factor, dim=-1)
                    )
        ...
        if attention_mask is not None and (
            ...
            buckets_mask = attention_mask.to(torch.uint8)[
                :, None, None, :
            ].expand(buckets.shape)
            buckets = torch.where(
                buckets_mask,
                buckets,
                torch.tensor(
        ...
        if not do_standard_self_attention:
            ...
            query_bucket_idx = (
                key_value_bucket_idx.new_ones(
                    key_value_bucket_idx.shape[:-1] + (1,)
                )
            ...
            ) * torch.ones_like(query_key_dots)[:, :, :, -1]
    ...
    def _get_relevant_hid_states_and_buckets(
        ...
        assert bucket_idx.shape == (
            ...
            "bucket_idx should have shape"
            f" {(batch_size, self.num_attention_heads, num_hashes, sequence_length)},"
            f" but has shape {bucket_idx.shape}."
        )

        # find indices of new bucket indices
        relevant_bucket_idx = (
            bucket_idx == (bucket_idx.shape[-1] - 1)
        ).nonzero()
        ...
        assert (
            ...
            * self.chunk_length
            ...
            "There should be"
            f" {(self.num_chunks_before + self.num_chunks_after + 1) * self.chunk_length} `hidden_states`,"
            f" there are {relevant_bucket_idx_chunk.shape[-1]} `bucket_idx`."
        )
    ...
    def _expand_to_indices_in_relevant_chunk(self, indices, sequence_length):
        ...
        chunk_sequence_indices = (
            chunk_sequence_indices.flatten() % sequence_length
        )
    ...
    def _gather_by_expansion(self, vectors, idxs, num_hashes):
        ...
        """
        expanded_idxs = idxs.unsqueeze(-1).expand(
            -1, -1, -1, self.attention_head_size
        )
...
class LocalSelfAttention(nn.Module, EfficientAttentionMixin):
        ...
        **kwargs,
        ...
        if use_cache and past_buckets_states[1] is not None:
            assert past_buckets_states[0] is None, (
                "LocalSelfAttention should not make use of `buckets`. There"
                " seems to be an error when caching hidden_states_and_buckets."
            )
            key_value_hidden_states = self._retrieve_relevant_hidden_states(
                past_buckets_states[1],
                self.chunk_length,
        ...
        assert query_vectors.shape[-1] == self.attention_head_size, (
            f"last dim of query_key_vectors is {query_vectors.shape[-1]} but"
            f" should be {self.attention_head_size}."
        )
        assert key_vectors.shape[-1] == self.attention_head_size, (
            f"last dim of query_key_vectors is {key_vectors.shape[-1]} but"
            f" should be {self.attention_head_size}."
        )
        assert value_vectors.shape[-1] == self.attention_head_size, (
            f"last dim of query_key_vectors is {value_vectors.shape[-1]} but"
            f" should be {self.attention_head_size}."
        )
        ...
        if self.chunk_length is None:
            assert (
                ...
                "If `config.chunk_length` is `None`, make sure"
                " `config.num_chunks_after` and `config.num_chunks_before` are"
                " set to 0."
            )
        ...
        indices = torch.arange(
            sequence_length, device=query_vectors.device
        ).repeat(batch_size, self.num_attention_heads, 1)
    ...
    def _retrieve_relevant_hidden_states(
        ...
        start_position = (
            (previous_hidden_states.shape[1] // chunk_length)
            - num_chunks_before
        ) * chunk_length
...
class ReformerAttention(nn.Module):
    def __init__(self, config, layer_id=0):
        ...
        if len(set(self.attn_layers)) == 1 and self.attn_layers[0] == "lsh":
            ...
            len(set(self.attn_layers)) == 1 and self.attn_layers[0] == "local"
        ):
            ...
            raise NotImplementedError(
                "Only attn layer types 'lsh' and 'local' exist, but got"
                f" `config.attn_layers`: {self.attn_layers}. Select attn layer"
                " types from ['lsh', 'local'] only."
            )
...
class ReformerLayer(nn.Module):
    ...
    def _init_attention_seed(self):
        ...
        if (
            ...
            self.attention_seed = torch.cuda.default_generators[
                device_idx
            ].seed()
        else:
            # CPU
    ...
    def _init_feed_forward_seed(self):
        ...
        if (
            ...
            # GPU
            device_idx = torch.cuda.current_device()
            self.feed_forward_seed = torch.cuda.default_generators[
                device_idx
            ].seed()
        else:
            # CPU
    ...
    def backward_pass(
        ...
        assert self.training, (
            "If you want to train `ReformerModel` and its variations, make"
            " sure to use `model.train()` to put the model into training mode."
        )
...
class _ReversibleFunction(Function):
    ...
    def forward(
        ...
        for layer_id, (layer, layer_head_mask) in enumerate(
            zip(layers, head_mask)
        ):
...
class ReformerEncoder(nn.Module):
    ...
    def forward(
        ...
        if past_buckets_states is None:
            past_buckets_states = [
                ((None), (None)) for i in range(len(self.layers))
            ]
...
@add_start_docstrings(
    (
        "The bare Reformer Model transformer outputting raw"
        " hidden-stateswithout any specific head on top."
    ),
...
class ReformerModel(ReformerPreTrainedModel):
    def __init__(self, config):
        ...
        assert self.config.num_hidden_layers > 0, (
            "`config.attn_layers` is empty. Select at least one attn layer"
            " form ['lsh', 'local']"
        )
    ...
    def forward(
        ...
        assert len(input_shape) == 2, (
            "`input_ids` have be of shape `[batch_size, sequence_length]`,"
            f" but got shape: {input_shape}"
        )

        if past_buckets_states is not None:
            assert not self.training, (
                "`past_buckets_states` can only be used for inference, not for"
                " training`."
            )
        ...
        if must_pad_to_match_chunk_length:
            ...
            if self.training is True:
                raise ValueError(
                    f"If training, sequence length {input_shape[-1]} has to be"
                    " a multiple of least common multiple chunk_length"
                    f" {least_common_mult_chunk_length}. Please consider"
                    " padding the input to a length of"
                    f" {input_shape[-1] + padding_length}."
                )
        ...
        past_buckets_states = (
            encoder_outputs.past_buckets_states if use_cache else None
        )
        hidden_states = (
            encoder_outputs.all_hidden_states if output_hidden_states else None
        )
        attentions = (
            encoder_outputs.all_attentions if output_attentions else None
        )
    ...
    def _pad_to_mult_of_chunk_length(
        ...
        logger.info(
            f"Input ids are automatically padded from {input_shape[-1]} to"
            f" {input_shape[-1] + padding_length} to be a multiple of"
            f" `config.chunk_length`: {padded_seq_length}"
        )
...
class ReformerModelWithLMHead(ReformerPreTrainedModel):
    def __init__(self, config):
        ...
        assert config.is_decoder, (
            "If you want to use `ReformerModelWithLMHead` make sure that"
            " `is_decoder=True`."
        )
        assert (
            "local" not in self.config.attn_layers
            or config.local_num_chunks_after == 0
        ), (
            "If causal mask is enabled, make sure that"
            " `config.local_num_chunks_after` is set to 0 and not"
            f" {config.local_num_chunks_after}."
        )
        assert (
            "lsh" not in self.config.attn_layers
            or config.lsh_num_chunks_after == 0
        ), (
            "If causal mask is enabled, make sure that"
            " `config.lsh_num_chunks_after` is set to 1 and not"
...
class ReformerForMaskedLM(ReformerPreTrainedModel):
    def __init__(self, config):
        ...
        assert not config.is_decoder, (
            "If you want to use `ReformerForMaskedLM` make sure"
...
class ReformerForSequenceClassification(ReformerPreTrainedModel):
    def __init__(self, config):
        ...
        if config.is_decoder is True:
            logger.warning(
                "You might want to disable causal masking for sequence"
                " classification"
            )

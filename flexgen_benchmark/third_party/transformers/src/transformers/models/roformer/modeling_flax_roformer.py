...
class FlaxRoFormerLayer(nn.Module):
    ...
    def setup(self):
        ...
        self.intermediate = FlaxRoFormerIntermediate(
...
class FlaxRoFormerOnlyMLMHead(nn.Module):
    ...
    def setup(self):
        self.predictions = FlaxRoFormerLMPredictionHead(
            self.config, dtype=self.dtype
        )
...
class FlaxRoFormerForMaskedLMModule(nn.Module):
    ...
    def setup(self):
        self.roformer = FlaxRoFormerModule(
            config=self.config, dtype=self.dtype
        )
        self.cls = FlaxRoFormerOnlyMLMHead(
            config=self.config, dtype=self.dtype
        )

    ...
    def __init__(
        ...
        **kwargs,
        ...
        if len(act_info) > 1 and act_info[0] != "gated" or len(act_info) > 2:
            raise ValueError(
                f"`feed_forward_proj`: {feed_forward_proj} is not a valid"
                " activation function of the dense layer.Please make sure"
                " `feed_forward_proj` is of the format `gated-{ACT_FN}` or"
                " `{ACT_FN}`, e.g. 'gated-gelu' or 'relu'"
            )
    ...
    def inputs(self) -> Mapping[str, Mapping[int, str]]:
        ...
        if self.use_past:
            common_inputs["attention_mask"][
                1
            ] = "past_encoder_sequence + sequence"

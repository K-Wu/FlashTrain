...
class FlaxElectraLayer(nn.Module):
    ...
    def setup(self):
        ...
        self.intermediate = FlaxElectraIntermediate(
...
class FlaxElectraSequenceSummary(nn.Module):
    ...
    def setup(self):
        ...
        if (
            hasattr(self.config, "summary_use_proj")
            and self.config.summary_use_proj
        ):
            if (
    ...
    def __call__(
        self, hidden_states, cls_index=None, deterministic: bool = True
    ):
        """
        Compute a single vector summary of a sequence hidden states.

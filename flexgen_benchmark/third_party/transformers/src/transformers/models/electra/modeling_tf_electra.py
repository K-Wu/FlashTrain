...
class TFElectraLayer(tf.keras.layers.Layer):
    def __init__(self, config: ElectraConfig, **kwargs):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = TFElectraAttention(
...
class TFElectraDiscriminatorPredictions(tf.keras.layers.Layer):
    ...
    def call(self, discriminator_hidden_states, training=False):
        ...
        hidden_states = get_tf_activation(self.config.hidden_act)(
            hidden_states
        )
...
@add_start_docstrings(
    (
        ...
        " without any specific head on top. Identical to the BERT model except"
        " that it uses an additional linear layer between the embedding layer"
        " and the encoder if the hidden size and embedding size are different."
        " Both the generator and discriminator checkpoints may be loaded into"
        " this model."
    ),
...
class TFElectraModel(TFElectraPreTrainedModel):
    ...
    def serving_output(self, output):
        ...
        if not (
            self.config.output_attentions and self.config.add_cross_attention
        ):
            cross_attns = None
...
class TFElectraForMaskedLM(
    TFElectraPreTrainedModel, TFMaskedLanguageModelingLoss
):
    def __init__(self, config, **kwargs):
        super().__init__(config, **kwargs)
    ...
    def call(
        ...
        prediction_scores = self.generator_lm_head(
            prediction_scores, training=training
        )
        loss = (
            None
            if labels is None
...
class TFElectraForMultipleChoice(
    TFElectraPreTrainedModel, TFMultipleChoiceLoss
):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
...
class TFElectraForQuestionAnswering(
    TFElectraPreTrainedModel, TFQuestionAnsweringLoss
):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)

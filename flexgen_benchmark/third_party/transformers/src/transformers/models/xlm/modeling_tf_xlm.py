...
class TFXLMMultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        def shape(x):
            """projection"""
            return tf.transpose(
                tf.reshape(x, (bs, -1, self.n_heads, dim_per_head)),
                perm=(0, 2, 1, 3),
            )
        ...
        if lengths is None:
            if input_ids is not None:
                lengths = tf.reduce_sum(
                    tf.cast(
                        tf.not_equal(input_ids, self.pad_index),
        ...
        if inputs_embeds is None:
            ...
            tf.debugging.assert_less(
                ...
                message=(
                    ...
                    f" {self.embeddings.vocab_size})"
                ),
            )
            inputs_embeds = self.embeddings(input_ids)

        tensor = inputs_embeds + tf.gather(
...
@add_start_docstrings(
    (
        "The bare XLM Model transformer outputting raw hidden-states without"
...
class TFXLMForMultipleChoice(TFXLMPreTrainedModel, TFMultipleChoiceLoss):
    ...
    def call(
        ...
        flat_langs = (
            tf.reshape(langs, (-1, seq_length)) if langs is not None else None
        )
        flat_inputs_embeds = (
            tf.reshape(
        ...
        if lengths is not None:
            logger.warning(
                (
                    "The `lengths` parameter cannot be used with the XLM"
                    " multiple choice models. Please use the attention mask"
                    " instead."
                ),
            )
            lengths = None
...
class TFXLMForTokenClassification(
    TFXLMPreTrainedModel, TFTokenClassificationLoss
):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.num_labels = config.num_labels
...
class TFXLMForQuestionAnsweringSimple(
    TFXLMPreTrainedModel, TFQuestionAnsweringLoss
):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.transformer = TFXLMMainLayer(config, name="transformer")

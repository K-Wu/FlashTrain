...
def load_tf_weights_in_trajectory_transformer(
...
class EinLinear(nn.Module):
    def __init__(self, n_models, in_features, out_features, bias):
        ...
        self.weight = nn.Parameter(
            torch.Tensor(n_models, out_features, in_features)
        )
        if bias:
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if config.n_embd % config.n_head != 0:
            raise ValueError(
                f"n_head ({config.n_head}) should be a divisor of n_embd"
                f" ({config.n_embd})"
            )
...
@add_start_docstrings(
    (
        "The bare TrajectoryTransformer Model transformer outputting raw"
...
class TrajectoryTransformerModel(TrajectoryTransformerPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        TRAJECTORY_TRANSFORMER_INPUTS_DOCSTRING.format(
    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(
            ...
            if self.gradient_checkpointing and self.training:
                ...
                outputs = block(
                    hidden_states, layer_past, use_cache, output_attentions
                )
        ...
        if targets is not None:
            ...
            if (
                ...
                weights = torch.cat(
                    [
                        torch.ones(
                            self.observation_dim, device=trajectories.device
                        ),

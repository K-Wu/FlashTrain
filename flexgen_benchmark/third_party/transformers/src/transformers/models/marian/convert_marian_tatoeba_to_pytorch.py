...
class TatoebaConverter:
    ...
    def convert_models(self, tatoeba_ids, dry_run=False):
        ...
        for model in tqdm(
            ...
            if "SentencePiece" not in model["pre-processing"]:
                print(
                    f"Skipping {model['release']} because it doesn't appear to"
                    " use SentencePiece"
                )
                continue
            ...
            convert(
                save_dir / model["_name"], dest_dir / f"opus-mt-{pair_name}"
            )
            self.write_model_card(model, dry_run=dry_run)

    def expand_group_to_two_letter_codes(self, grp_name):
        return [
            self.alpha3_to_alpha2.get(x, x) for x in GROUP_MEMBERS[grp_name][1]
        ]
    ...
    def write_model_card(self, model_dict, dry_run=False) -> str:
        ...
        assert len(long_pair) == 2, (
            f"got a translation pair {model_dict['_name']} that doesn't appear"
            " to be a pair"
        )
        ...
        resolved_src_tags, resolved_tgt_tags = self.resolve_lang_code(
            a3_src, a3_tgt
        )
        ...
        download = (
            ...
            f" [{model_base_filename}]({model_dir_url}/{model_dict['release']})\n"
        )

        langtoken = ""
        if tgt_multilingual:
            langtoken = (
                "* a sentence-initial language token is required in the form"
                " of >>id<<(id = valid, usually three-letter target language"
                " ID)\n"
            )
        ...
        testset = (
            "* test set translations file:"
            f" [test.txt]({model_dir_url}/{testsetfilename})\n"
        )
        testscores = (
            "* test set scores file:"
            f" [eval.txt]({model_dir_url}/{testscoresfilename})\n"
        )
...
"""
        ...
        content = (
            FRONT_MATTER_TEMPLATE.format(lang_tags) + extra_markdown + content
        )
        ...
        with open(
            sub_dir / "metadata.json", "w", encoding="utf-8"
        ) as writeobj:
    ...
    def parse_metadata(
        self, model_name, repo_path=DEFAULT_MODEL_DIR, method="best"
    ):
        ...
        if method == "best":
            ...
            ymls = [
                ...
                if f.endswith(".yml") and f[:-4] in results
            ]
            ...
            metadata.update(
                self.model_type_info_from_model_name(ymls[-1][:-4])
            )
        else:

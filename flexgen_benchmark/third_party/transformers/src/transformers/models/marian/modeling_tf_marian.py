...
class TFMarianDecoder(tf.keras.layers.Layer):
    ...
    def call(
        ...
        hidden_states = self.dropout(
            hidden_states + positions, training=training
        )
        ...
        for attn_name, attn_mask in [
            ...
            if attn_mask is not None:
                tf.debugging.assert_equal(
                    ...
                    len(self.layers),
                    message=(
                        f"The {attn_name} should be specified for"
                        f" {len(self.layers)} layers, but it is for"
    ...
    def adjust_logits_during_generation(
        ...
        **kwargs,
        ...
        if cur_len == 1 and forced_bos_token_id is not None:
            vocab_range = tf.constant(range(self.config.vocab_size))
            return tf.where(
                vocab_range != forced_bos_token_id, LARGE_NEGATIVE, logits
            )
        elif cur_len == max_length - 1 and forced_eos_token_id is not None:
            vocab_range = tf.constant(range(self.config.vocab_size))
            return tf.where(
                vocab_range != forced_eos_token_id, LARGE_NEGATIVE, logits
            )
        else:
            return logits

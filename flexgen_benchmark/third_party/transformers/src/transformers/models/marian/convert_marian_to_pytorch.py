...
def convert_encoder_layer(opus_dict, layer_prefix: str, converter: dict):
    ...
    for k in opus_dict:
        ...
        v = opus_dict[
            k
        ].T  # besides embeddings, everything must be transposed.
...
def load_layers_(
    ...
    for i, layer in enumerate(layer_lst):
        layer_tag = (
            f"decoder_l{i + 1}_" if is_decoder else f"encoder_l{i + 1}_"
        )
...
def find_pretrained_model(src_lang: str, tgt_lang: str) -> List[str]:
    ...
    src_and_targ = [
        remove_prefix(m, prefix).lower().split("-")
        for m in model_ids
        if "+" not in m
    ]  # + cant be loaded.
    matching = [
...
OPUS_GITHUB_URL = (
    "https://github.com/Helsinki-NLP/OPUS-MT-train/blob/master/models/"
)
...
def write_model_card(
    ...
    if repo_root not in ("OPUS-MT-train", "Tatoeba-Challenge"):
        raise ValueError(
            f"Repos root is {repo_root}. Expected either OPUS-MT-train or"
            " Tatoeba-Challenge"
        )
    ...
    extra_markdown = (
        ...
        f" target group: {metadata['tgt_name']} \n*  OPUS readme:"
        f" [{opus_name}]({readme_url})\n"
    )
...
def make_registry(repo_path="Opus-MT-train/models"):
    if not (Path(repo_path) / "fr-en" / "README.md").exists():
        raise ValueError(
            f"repo_path:{repo_path} does not exist: You must run: git clone"
            " git@github.com:Helsinki-NLP/Opus-MT-train.git before calling."
        )
    results = {}
...
def fetch_test_set(test_set_url):
    ...
    if not (len(gold) == len(mar_model) == len(src)):
        raise ValueError(
            f"Gold, marian and source lengths {len(gold)}, {len(mar_model)},"
            f" {len(src)} mismatched"
        )
    os.remove(fname)
...
class OpusState:
    def __init__(self, source_dir, eos_token_id=0):
        ...
        eos_token_id = (
            self.tokenizer.eos_token_id if tokenizer_has_eos_token_id else 0
        )
        ...
        if cfg["vocab_size"] != self.tokenizer.vocab_size:
            raise ValueError(
                f"Original vocab size {cfg['vocab_size']} and new vocab size"
                f" {len(self.tokenizer.encoder)} mismatched."
            )
        ...
        if hidden_size != cfg["dim-emb"]:
            raise ValueError(
                f"Hidden size {hidden_size} and configured size"
                f" {cfg['dim_emb']} mismatched"
            )
    ...
    def load_marian_model(self) -> MarianMTModel:
        ...
        if self.cfg["tied-embeddings-src"]:
            ...
            decoder_wemb_tensor = nn.Parameter(
                torch.FloatTensor(self.dec_wemb)
            )
            bias_tensor = nn.Parameter(torch.FloatTensor(self.final_bias))
        ...
        if model.get_input_embeddings().padding_idx != self.pad_token_id:
            raise ValueError(
                "Padding tokens"
                f" {model.get_input_embeddings().padding_idx} and"
                f" {self.pad_token_id} mismatched"
            )
        return model
...
if __name__ == "__main__":
    ...
    dest_dir = (
        f"converted-{source_dir.name}" if args.dest is None else args.dest
    )
    convert(source_dir, dest_dir)

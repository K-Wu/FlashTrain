...
class MarianSinusoidalPositionalEmbedding(nn.Embedding):
    ...
    def _init_weight(out: nn.Parameter) -> nn.Parameter:
        ...
        out.requires_grad = (
            False  # set early to avoid an error in pytorch-1.8+
        )
...
class MarianPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(
        self,
        module: Union[
            nn.Linear, nn.Embedding, MarianSinusoidalPositionalEmbedding
        ],
    ):
...
@add_start_docstrings(
    (
        "The bare Marian Model outputting raw hidden-states without any"
...
class MarianModel(MarianPreTrainedModel):
    ...
    def get_decoder_input_embeddings(self):
        if self.config.share_encoder_decoder_embeddings:
            raise ValueError(
                "`get_decoder_input_embeddings` should not be called if"
                " `config.share_encoder_decoder_embeddings` is `True`. Please"
                " use `get_input_embeddings` instead."
            )
    ...
    def set_decoder_input_embeddings(self, value):
        if self.config.share_encoder_decoder_embeddings:
            raise ValueError(
                "`config.share_encoder_decoder_embeddings` is set to `True`"
                " meaning the decoder input embeddings are shared with the"
                " encoder. In order to set the decoder input embeddings, you"
                " should simply set the encoder input embeddings by calling"
                " `set_input_embeddings` with the appropriate embeddings."
            )
    ...
    def resize_decoder_token_embeddings(
        ...
        if self.config.share_encoder_decoder_embeddings:
            raise ValueError(
                "`resize_decoder_token_embeddings` should not be called if"
                " `config.share_encoder_decoder_embeddings` is `True`. Please"
                " use `resize_token_embeddings` instead."
            )
...
class MarianMTModel(MarianPreTrainedModel):
    ...
    def __init__(self, config: MarianConfig):
        ...
        self.register_buffer(
            "final_logits_bias", torch.zeros((1, target_vocab_size))
        )
    ...
    def forward(
        ...
        if labels is not None:
            ...
            masked_lm_loss = loss_fct(
                lm_logits.view(-1, self.config.decoder_vocab_size),
                labels.view(-1),
            )
    ...
    def adjust_logits_during_generation(self, logits, cur_len):
        logits[:, self.config.pad_token_id] = float(
            "-inf"

        ...
        if attn_prompt is not None:
            ...
            if attention_mask is not None:
                prompt_mask = torch.zeros(
                    bsz, 1, tgt_len, attn_prompt[0].size(1)
...
class MvpPrompt(nn.Module):
    ...
    def __init__(self, config, num_layers, num_heads):
        ...
        self.prompt_embedding = nn.Embedding(
            config.prompt_length, config.d_model
        )
...
@add_start_docstrings(
    (
        "The bare MVP Model outputting raw hidden-states without any specific"
        " head on top."
    ),
    MVP_START_DOCSTRING,
)
class MvpModel(MvpPreTrainedModel):
    ...
    def set_lightweight_tuning(self):
        assert self.use_prompt, (
            "If you want to use lightweight tuning, make sure that"
            " `use_prompt=True`."
        )
...
@add_start_docstrings(
    (
        "The MVP Model with a language modeling head. Can be used for various"

...
def convert_fsmt_checkpoint_to_pytorch(
    ...
    print(
        f"Generating {src_vocab_file} of {src_vocab_size} of"
        f" {src_lang} records"
    )
    ...
    print(
        f"Generating {tgt_vocab_file} of {tgt_vocab_size} of"
        f" {tgt_lang} records"
    )
    ...
    for fn in [
        "bpecodes",
        "code",
    ]:  # older fairseq called the merges file "code"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Path to the official PyTorch checkpoint file which is expected to"
            " reside in the dump dir with dicts, bpecodes, etc."
        ),
    )
    parser.add_argument(

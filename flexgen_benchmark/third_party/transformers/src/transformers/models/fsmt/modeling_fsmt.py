...
def _prepare_fsmt_decoder_inputs(
    ...
    causal_mask = triu_onnx(
        fill_with_neg_inf(
            torch.zeros(tgt_len, tgt_len, dtype=causal_mask_dtype)
        ),
        1,
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.cache_key = (
            "encoder_decoder" if self.encoder_decoder_attention else "self"
        )
...
class SinusoidalPositionalEmbedding(nn.Embedding):
    ...
    def make_weight(self, num_positions, embedding_dim, padding_idx):
        ...
        if not hasattr(self, "weight"):
            # in ___init__
            super().__init__(
                num_positions, embedding_dim, padding_idx, _weight=weight
            )
        else:

...
class PegasusTokenizerFast(PreTrainedTokenizerFast):
        ...
        **kwargs,
        ...
        if additional_special_tokens is not None:
            if not isinstance(additional_special_tokens, list):
                raise TypeError(
                    "additional_special_tokens should be of type"
                    f" {type(list)}, but is {type(additional_special_tokens)}"
                )
            ...
            if len(set(additional_special_tokens_extended)) != len(
                ...
                raise ValueError(
                    "Please make sure that the provided"
                    " additional_special_tokens do not contain an incorrectly"
                    " shifted list of <unk_x> tokens. Found"
                    f" {additional_special_tokens_extended}."
                )
    ...
    def _special_token_mask(self, seq):
        ...
        if all_special_ids != set(
            range(len(self.additional_special_tokens) + 3)
        ):
            raise ValueError(
                "There should be 3 special tokens: mask_token, pad_token, and"
                " eos_token +"
                f" {len(self.additional_special_tokens)} additional_special_tokens,"
                f" but got {all_special_ids}"
            )

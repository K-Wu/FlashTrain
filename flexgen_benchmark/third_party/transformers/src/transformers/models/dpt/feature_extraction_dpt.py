...
class DPTFeatureExtractor(FeatureExtractionMixin, ImageFeatureExtractionMixin):
    ...
    def constrain_to_multiple_of(self, size, min_val=0, max_val=None):
        y = (
            np.round(size / self.ensure_multiple_of) * self.ensure_multiple_of
        ).astype(int)

        if max_val is not None and y > max_val:
            y = (
                np.floor(size / self.ensure_multiple_of)
                * self.ensure_multiple_of
            ).astype(int)

        if y < min_val:
            y = (
                np.ceil(size / self.ensure_multiple_of)
                * self.ensure_multiple_of
            ).astype(int)
        ...
        **kwargs
        ...
        if self.do_resize and self.size is not None:
            for idx, image in enumerate(images):
                ...
                images[idx] = self.resize(
                    image, size=size, resample=self.resample
                )

    ...
    def forward(self, hidden_states: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        for i, hidden_state in enumerate(hidden_states):
            ...
            if self.config.readout_type == "project":
                ...
                hidden_state = hidden_state.flatten(2) + cls_token.unsqueeze(
                    -1
                )
...
@add_start_docstrings(
    (
        "The bare DPT Model transformer outputting raw hidden-states without"
    ...
    * DPTFeatureFusionStage.
    ...
    def forward(self, hidden_states: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        if len(hidden_states) != len(self.config.neck_hidden_sizes):
            raise ValueError(
                "The number of hidden states should be equal to the number of"
                " neck hidden sizes."
            )
        ...
        features = [
            self.convs[i](feature) for i, feature in enumerate(features)
        ]

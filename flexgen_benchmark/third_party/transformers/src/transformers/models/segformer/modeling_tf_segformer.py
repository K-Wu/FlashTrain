...
class TFSegformerEfficientSelfAttention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        if self.sr_ratio > 1:
            ...
            hidden_states = tf.reshape(
                hidden_states, (batch_size, -1, num_channels)
            )
...
class TFSegformerDWConv(tf.keras.layers.Layer):
    ...
    def call(
        self, hidden_states: tf.Tensor, height: int, width: int
    ) -> tf.Tensor:
        batch_size = shape_list(hidden_states)[0]
...
class TFSegformerMixFFN(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.depthwise_convolution = TFSegformerDWConv(
            hidden_features, name="dwconv"
        )
    ...
    def call(
        ...
        hidden_states = self.depthwise_convolution(
...
class TFSegformerEncoder(tf.keras.layers.Layer):
    def __init__(self, config: SegformerConfig, **kwargs):
        ...
        drop_path_decays = [
            ...
            for x in tf.linspace(
                0.0, config.drop_path_rate, sum(config.depths)
            )
        ]
        ...
        self.layer_norms = [
            tf.keras.layers.LayerNormalization(
                epsilon=1e-05, name=f"layer_norm.{i}"
            )
            for i in range(config.num_encoder_blocks)
        ]
    ...
    def call(
        ...
        for idx, x in enumerate(
            zip(self.embeddings, self.block, self.layer_norms)
        ):
...
@add_start_docstrings(
    (
        "The bare SegFormer encoder (Mix-Transformer) outputting raw"
...
class TFSegformerMLP(tf.keras.layers.Layer):
    ...
    def __init__(self, config: SegformerConfig, **kwargs):
        ...
        self.proj = tf.keras.layers.Dense(
            config.decoder_hidden_size, name="proj"
        )
    ...
    def call(self, hidden_states: tf.Tensor) -> tf.Tensor:
        ...
        hidden_states = tf.reshape(
            hidden_states, (-1, height * width, hidden_dim)
        )
...
class TFSegformerDecodeHead(TFSegformerPreTrainedModel):
    ...
    def call(self, encoder_hidden_states, training: bool = False):
        ...
        for encoder_hidden_state, mlp in zip(encoder_hidden_states, self.mlps):
            ...
            encoder_hidden_state = tf.transpose(
                encoder_hidden_state, perm=[0, 2, 3, 1]
            )

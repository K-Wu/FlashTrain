...
def load_tf_weights_in_realm(model, config, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        if isinstance(model, RealmReader) and "reader" not in name:
            logger.info(
                f"Skipping {name} as it is not {model.__class__.__name__}'s"
                " parameter"
            )
            continue
        ...
        if name.startswith("reader"):
            ...
            name = name.replace(
                "reader/module/bert/", f"{reader_prefix}realm/"
            )
            name = name.replace("reader/module/cls/", f"{reader_prefix}cls/")
            name = name.replace(
                "reader/dense/",
                f"{reader_prefix}qa_outputs/dense_intermediate/",
            )
            name = name.replace(
        ...
        if name.startswith("module/module/module/"):  # finetuned
            embedder_prefix = (
                "" if isinstance(model, RealmEmbedder) else "embedder/"
            )
            name = name.replace(
                "module/module/module/module/bert/", f"{embedder_prefix}realm/"
            )
            name = name.replace(
                "module/module/module/LayerNorm/",
                f"{embedder_prefix}cls/LayerNorm/",
            )
            name = name.replace(
            ...
            name = name.replace(
                "module/module/dense/", f"{embedder_prefix}cls/dense/"
            )
...
class RealmReaderProjection(nn.Module):
    ...
    def forward(self, hidden_states, block_mask):
        ...
        candidate_hidden = (
            candidate_start_projections + candidate_end_projections
        )
        ...
        # [reader_beam_size, num_candidates]
        reader_logits += mask_to_score(
            candidate_mask, dtype=reader_logits.dtype
        )
...
@add_start_docstrings(
    (
        "The embedder of REALM outputting projected score that will be used to"
...
@add_start_docstrings(
    (
        "The scorer of REALM outputting relevance scores representing the"
        " score of document candidates (before softmax)."
    ),
    REALM_START_DOCSTRING,
)
class RealmScorer(RealmPreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is None and inputs_embeds is None:
            raise ValueError(
                "You have to specify either input_ids or input_embeds."
            )

        if candidate_input_ids is None and candidate_inputs_embeds is None:
            raise ValueError(
                "You have to specify either candidate_input_ids or"
        ...
        relevance_score = torch.einsum(
            "BD,BND->BN", query_score, candidate_score
        )
...
@add_start_docstrings(
    (
        "The knowledge-augmented encoder of REALM outputting masked language"
        " model logits and marginal log-likelihood loss."
    ),
    REALM_START_DOCSTRING,
)
    ...
    def forward(
        ...
        if labels is not None:
            if candidate_score is None:
                raise ValueError(
                    "You have to specify `relevance_score` when `labels` is"
                    " specified in order to compute loss."
                )
...
class RealmReader(RealmPreTrainedModel):
    ...
    def forward(
        ...
        if relevance_score is None:
            raise ValueError(
                "You have to specify `relevance_score` to calculate logits and"
                " loss."
            )
        if block_mask is None:
            raise ValueError(
                "You have to specify `block_mask` to separate question block"
                " and evidence block."
            )
        if token_type_ids.size(1) < self.config.max_span_width:
            raise ValueError(
                "The input sequence length must be greater than or equal to"
        ...
        # []
        predicted_block_index = torch.argmax(
            torch.max(reader_logits, dim=1).values
        )
        # []
        predicted_candidate = torch.argmax(
            torch.max(reader_logits, dim=0).values
        )
        # [1]
        ...
        if (
            ...
            def compute_correct_candidates(
                ...
                # [reader_beam_size, num_candidates]
                return torch.any(
                    torch.logical_and(is_gold_start, is_gold_end), 1
                )
            ...
            def marginal_log_loss(logits, is_correct):
                ...
                log_numerator = torch.logsumexp(
                    logits + mask_to_score(is_correct, dtype=logits.dtype),
                    dim=-1,
                )
            ...
            retriever_loss = marginal_log_loss(
                relevance_score, retriever_correct
            )
...
class RealmForOpenQA(RealmPreTrainedModel):
    ...
    def forward(
        ...
        batch_scores = torch.einsum(
            "BD,QD->QB",
            self.block_emb,
            question_projection.to(self.block_emb.device),
        )
        ...
        if has_answers is not None:
            ...
            end_pos = torch.tensor(
                end_pos, dtype=torch.long, device=self.reader.device
            )

...
def _compute_mask_indices(
    ...
    if mask_length > sequence_length:
        raise ValueError(
            ...
            f" `mask_length`: {mask_length} and `sequence_length`:"
            f" {sequence_length}`"
        )
    ...
    def __init__(self, config):
        ...
        if is_deepspeed_zero3_enabled():
            ...
            deepspeed.zero.register_external_parameter(
                self, self.conv.weight_v
            )
            deepspeed.zero.register_external_parameter(
                self, self.conv.weight_g
            )
        else:
            self.conv = nn.utils.weight_norm(self.conv, name="weight", dim=2)

        self.padding = Wav2Vec2ConformerSamePadLayer(
...
class Wav2Vec2ConformerFeatureEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.feat_extract_norm == "group":
            conv_layers = [
                Wav2Vec2ConformerGroupNormConvLayer(config, layer_id=0)
            ] + [
...
class Wav2Vec2ConformerConvolutionModule(nn.Module):
    ...
    def __init__(self, config):
        ...
        if (config.conv_depthwise_kernel_size - 1) % 2 == 1:
            raise ValueError(
                "`config.conv_depthwise_kernel_size` should be a odd number"
                " for 'SAME' padding"
            )
...
class Wav2Vec2ConformerSelfAttention(nn.Module):
    ...
    def _apply_rotary_embedding(
        self, hidden_states, relative_position_embeddings
    ):
        batch_size, sequence_length, hidden_size = hidden_states.size()
    ...
    def _apply_relative_embeddings(
        ...
        proj_relative_position_embeddings = (
            proj_relative_position_embeddings.view(
        ...
        proj_relative_position_embeddings = (
            proj_relative_position_embeddings.transpose(1, 2)
        )
        proj_relative_position_embeddings = (
            proj_relative_position_embeddings.transpose(2, 3)
        )
        ...
        scores_bd = torch.matmul(
            q_with_bias_v, proj_relative_position_embeddings
        )

        # 5. shift matrix b and matrix d
        zero_pad = torch.zeros(
            (*scores_bd.size()[:3], 1),
            device=scores_bd.device,
            dtype=scores_bd.dtype,
        )
...
class Wav2Vec2ConformerEncoder(nn.Module):
    def __init__(self, config):
        ...
        if config.position_embeddings_type == "relative":
            self.embed_positions = Wav2Vec2ConformerRelPositionalEmbedding(
                config
            )
        elif config.position_embeddings_type == "rotary":
            self.embed_positions = Wav2Vec2ConformerRotaryPositionalEmbedding(
                config
            )
        else:
...
class Wav2Vec2ConformerGumbelVectorQuantizer(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.codevector_dim % self.num_groups != 0:
            raise ValueError(
                f"`config.codevector_dim {config.codevector_dim} must be"
                " divisible by `config.num_codevector_groups`"
    ...
    def _compute_perplexity(probs, mask=None):
        ...
        perplexity = torch.exp(
            -torch.sum(
                marginal_probs * torch.log(marginal_probs + 1e-7), dim=-1
            )
        ).sum()
        return perplexity
    ...
    def forward(self, hidden_states, mask_time_indices=None):
        ...
        if self.training:
            ...
            codevector_probs = hidden_states.new_zeros(
                hidden_states.shape
            ).scatter_(-1, codevector_idx.view(-1, 1), 1.0)
...
class Wav2Vec2ConformerPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, Wav2Vec2ConformerGumbelVectorQuantizer):
            ...
            nn.init.normal_(
                ...
                std=2
                * math.sqrt(
                    1 / (module.conv.kernel_size[0] * module.conv.in_channels)
                ),
            )
    ...
    def _get_feat_extract_output_lengths(
        ...
        add_adapter = (
            self.config.add_adapter if add_adapter is None else add_adapter
        )
...
@add_start_docstrings(
    (
        "The bare Wav2Vec2Conformer Model transformer outputting raw"
...
class Wav2Vec2ConformerModel(Wav2Vec2ConformerPreTrainedModel):
    def __init__(self, config: Wav2Vec2ConformerConfig):
        ...
        self.adapter = (
            Wav2Vec2ConformerAdapter(config) if config.add_adapter else None
        )
...
class Wav2Vec2ConformerForPreTraining(Wav2Vec2ConformerPreTrainedModel):
    ...
    def compute_contrastive_logits(
        ...
        target_features = torch.cat(
            [target_features, negative_features], dim=0
        )
    ...
    @add_start_docstrings_to_model_forward(WAV2VEC2_CONFORMER_INPUTS_DOCSTRING)
    @replace_return_docstrings(
        output_type=Wav2Vec2ConformerForPreTrainingOutput,
        config_class=_CONFIG_FOR_DOC,
    )
...
class Wav2Vec2ConformerForCTC(Wav2Vec2ConformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.vocab_size is None:
            raise ValueError(
                f"You are trying to instantiate {self.__class__} with a"
                " configuration that does not define the vocabulary size of"
                " the language model head. Please instantiate the model as"
                " follows: `Wav2Vec2ConformerForCTC.from_pretrained(...,"
                " vocab_size=vocab_size)`. or define `vocab_size` of your"
...
class Wav2Vec2ConformerForSequenceClassification(
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "add_adapter") and config.add_adapter:
            raise ValueError(
                "Sequence classification does not support the use of"
                " Wav2Vec2Conformer adapters (config.add_adapter=True)"
            )
        self.wav2vec2_conformer = Wav2Vec2ConformerModel(config)
        num_layers = (
    ...
    def forward(
        ...
        if self.config.use_weighted_layer_sum:
            ...
            hidden_states = (hidden_states * norm_weights.view(-1, 1, 1)).sum(
                dim=1
            )
        else:
            hidden_states = outputs[0]
...
class Wav2Vec2ConformerForAudioFrameClassification(
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "add_adapter") and config.add_adapter:
            raise ValueError(
                "Audio frame classification does not support the use of"
                " Wav2Vec2Conformer adapters (config.add_adapter=True)"
            )
        self.wav2vec2_conformer = Wav2Vec2ConformerModel(config)
        num_layers = (
...
class TDNNLayer(nn.Module):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.in_conv_dim = (
            config.tdnn_dim[layer_id - 1]
            if layer_id > 0
            else config.tdnn_dim[layer_id]
        )
        ...
        self.kernel = nn.Linear(
            self.in_conv_dim * self.kernel_size, self.out_conv_dim
        )
...
class Wav2Vec2ConformerForXVector(Wav2Vec2ConformerPreTrainedModel):
    def __init__(self, config):
        ...
        self.objective = AMSoftmaxLoss(
            config.xvector_output_dim, config.num_labels
        )

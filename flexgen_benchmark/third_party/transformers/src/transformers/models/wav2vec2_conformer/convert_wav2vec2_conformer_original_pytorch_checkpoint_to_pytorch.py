...
MAPPING = {
    ...
    "conv_module.pointwise_conv1": (
        "encoder.layers.*.conv_module.pointwise_conv1"
    ),
    "conv_module.pointwise_conv2": (
        "encoder.layers.*.conv_module.pointwise_conv2"
    ),
    "conv_module.depthwise_conv": (
        "encoder.layers.*.conv_module.depthwise_conv"
    ),
    ...
    if type_id == 0:
        ...
        if "bias" in name:
            if (
                ...
                raise ValueError(
                    f"{full_name} has size {value.shape}, but"
                    f" {feature_extractor.conv_layers[layer_id].layer_norm.bias.data.shape} was"
                    " found."
                )
            feature_extractor.conv_layers[
                layer_id
            ...
            if (
                ...
                raise ValueError(
                    f"{full_name} has size {value.shape}, but"
                    f" {feature_extractor.conv_layers[layer_id].layer_norm.weight.data.shape} was"
                    " found."
                )
            feature_extractor.conv_layers[
                layer_id

...
class TFSwinIntermediate(tf.keras.layers.Layer):
    def __init__(self, config: SwinConfig, dim: int, **kwargs) -> None:
        ...
        self.dense = tf.keras.layers.Dense(
            int(config.mlp_ratio * dim), name="dense"
        )
...
class TFSwinLayer(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.attention = TFSwinAttention(
            config, dim, num_heads, name="attention"
        )
        self.drop_path = (
        ...
        self.intermediate = TFSwinIntermediate(
            config, dim, name="intermediate"
        )
        ...
        if shift_size > 0:
            ...
            for height_slice in height_slices:
                for width_slice in width_slices:
                    ...
                    indices = tf.reshape(
                        tf.stack(
                            tf.meshgrid(height_inds, width_inds), axis=-1
                        ),
                        (-1, 2),
                    )
                    if len(indices) >= 1:
                        updates = (
                            tf.ones((len(indices),), dtype=img_mask.dtype)
                            * count
                        )
    ...
    def call(
        ...
        window_size = (
            min_res if min_res <= self.window_size else self.window_size
        )
        ...
        hidden_states_windows = window_partition(
            shifted_hidden_states, window_size
        )
        ...
        hidden_states = shortcut + self.drop_path(
            attention_windows, training=training
        )

        layer_output = self.layernorm_after(hidden_states, training=training)
        layer_output = self.intermediate(layer_output)
        layer_output = hidden_states + self.swin_output(
...
class TFSwinStage(tf.keras.layers.Layer):
    ...
    def call(
        ...
        if self.downsample is not None:
            height_downsampled, width_downsampled = (height + 1) // 2, (
                width + 1
            ) // 2
            output_dimensions = (
                height,
...
class TFSwinEncoder(tf.keras.layers.Layer):
    def __init__(
        ...
        dpr = list(
            (
                tf.linspace(0, 1, sum(config.depths)) * config.drop_path_rate
            ).numpy()
        )
        self.layers = [
            TFSwinStage(
...
class TFSwinMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.encoder = TFSwinEncoder(
            config, self.embeddings.patch_grid, name="encoder"
        )
        ...
        self.pooler = (
            AdaptiveAveragePooling1D(output_size=(1,))
    ...
    def call(
        ...
        if self.pooler is not None:
            ...
            pooled_output = tf.reshape(
                pooled_output, (batch_size, num_features)
            )
...
@add_start_docstrings(
    (
        "The bare Swin Model transformer outputting raw hidden-states without"
...
class TFSwinPixelShuffle(tf.keras.layers.Layer):
    ...
    def __init__(self, upscale_factor: int, **kwargs) -> None:
        ...
        if not isinstance(upscale_factor, int) or upscale_factor < 2:
            raise ValueError(
                "upscale_factor must be an integer value >= 2 got"
                f" {upscale_factor}"
            )
...
class TFSwinDecoder(tf.keras.layers.Layer):
    def __init__(self, config: SwinConfig, **kwargs):
        ...
        self.pixel_shuffle = TFSwinPixelShuffle(
            config.encoder_stride, name="1"
        )
...
@add_start_docstrings(
    (
        "Swin Model with a decoder on top for masked image modeling, as"
        " proposed in [SimMIM](https://arxiv.org/abs/2111.09886)."
    ),
    SWIN_START_DOCSTRING,
)
...
class TFSwinForImageClassification(
    TFSwinPreTrainedModel, TFSequenceClassificationLoss
):
    def __init__(self, config: SwinConfig):
        super().__init__(config)

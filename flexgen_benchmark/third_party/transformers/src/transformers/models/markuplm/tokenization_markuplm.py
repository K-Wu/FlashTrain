...
class MarkupLMTokenizer(PreTrainedTokenizer):
    ...
    def get_xpath_seq(self, xpath):
        ...
        for unit in xpath_units:
            ...
            xpath_tags_list.append(
                self.tags_dict.get(tag_name, self.unk_tag_id)
            )
        ...
        xpath_tags_list += [self.pad_tag_id] * (
            self.max_depth - len(xpath_tags_list)
        )
        xpath_subs_list += [self.pad_width] * (
            self.max_depth - len(xpath_subs_list)
        )
    ...
    def convert_tokens_to_string(self, tokens):
        ...
        logger.warning(
            "MarkupLM now does not support generative tasks, decoding is"
        ...
        with open(merge_file, "w", encoding="utf-8") as writer:
            ...
            for bpe_tokens, token_index in sorted(
                ...
                if index != token_index:
                    logger.warning(
                        f"Saving vocabulary to {merge_file}: BPE merge indices"
                        " are not consecutive. Please check that the"
                        " tokenizer is not corrupted!"
                    )
                    index = token_index
    ...
    def __call__(
        ...
        text_pair: Optional[
            Union[PreTokenizedInput, List[PreTokenizedInput]]
        ] = None,
        ...
        **kwargs,
        ...
        if text_pair is not None:
            ...
            if not _is_valid_text_input(text):
                raise ValueError(
                    "text input must of type `str` (single example) or"
                    " `List[str]` (batch of examples). "
                )
            if not isinstance(text_pair, (list, tuple)):
                raise ValueError(
                    "Nodes must be of type `List[str]` (single pretokenized"
                    " example), or `List[List[str]]` (batch of pretokenized"
                    " examples)."
                )
        else:
        ...
        if is_batched:
            assert len(nodes) == len(xpaths), (
                "You must provide nodes and xpaths for an equal amount of"
                " examples"
            )
    ...
    def _batch_prepare_for_model(
        ...
        for idx, example in enumerate(zip(batch_text_or_text_pairs, xpaths)):
            ...
            outputs = self.prepare_for_model(
                batch_text_or_text_pair[0]
                if is_pair
                else batch_text_or_text_pair,
    ...
    def prepare_for_model(
        ...
        *truncation_strategy = longest_first* or `True`, it is not possible to return overflowing tokens. Such a
        ...
        if text_pair is None:
            if node_labels is None:
                ...
                for word, xpath, label in zip(text, xpaths, node_labels):
                    ...
                    if self.only_label_first_subword:
                        ...
                        labels.extend(
                            [label]
                            + [self.pad_token_label] * (len(word_tokens) - 1)
                        )
                    else:
            ...
            xpath_tags_seq = [
                self.pad_xpath_tags_seq for _ in range(len(tokens))
            ]
            xpath_subs_seq = [
                self.pad_xpath_subs_seq for _ in range(len(tokens))
            ]
            ...
            for word, xpath in zip(text_pair, xpaths):
                ...
                pair_xpath_tags_seq.extend(
                    [xpath_tags_list] * len(word_tokens)
                )
                pair_xpath_subs_seq.extend(
                    [xpath_subs_list] * len(word_tokens)
                )

        # Create ids + pair_ids
        ids = self.convert_tokens_to_ids(tokens)
        pair_ids = (
            self.convert_tokens_to_ids(pair_tokens) if pair_tokens else None
        )
        ...
        if add_special_tokens:
            ...
            if labels:
                labels = (
                    [self.pad_token_label] + labels + [self.pad_token_label]
                )
        else:
            ...
            if self.padding_side == "right":
                ...
                if "labels" in encoded_inputs:
                    encoded_inputs["labels"] = (
                        encoded_inputs["labels"]
                        + [self.pad_token_label] * difference
                    )

...
class XPathEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.xpath_unitseq2_inner = nn.Linear(
            config.xpath_unit_hidden_size * self.max_depth,
        ...
        self.xpath_tag_sub_embeddings = nn.ModuleList(
            [
                nn.Embedding(
                    config.max_xpath_tag_unit_embeddings,
                    config.xpath_unit_hidden_size,
                )
        ...
        self.xpath_subs_sub_embeddings = nn.ModuleList(
            [
                nn.Embedding(
                    config.max_xpath_subs_unit_embeddings,
                    config.xpath_unit_hidden_size,
                )
                for _ in range(self.max_depth)
            ]
    ...
    def forward(self, xpath_tags_seq=None, xpath_subs_seq=None):
        ...
        xpath_embeddings = self.inner2emb(
            self.dropout(
                self.activation(self.xpath_unitseq2_inner(xpath_embeddings))
            )
        )
...
class MarkupLMEmbeddings(nn.Module):
    ...
    def forward(
        ...
        xpath_embeddings = self.xpath_embeddings(
...
@add_start_docstrings(
    (
        "The bare MarkupLM Model transformer outputting raw hidden-states"

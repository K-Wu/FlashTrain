...
class MarkupLMFeatureExtractor(FeatureExtractionMixin):
    ...
    def get_three_from_single(self, html_string):
        ...
        if len(all_doc_strings) != len(string2xtag_seq):
            raise ValueError(
                "Number of doc strings and xtags does not correspond"
            )
        if len(all_doc_strings) != len(string2xsubs_seq):
            raise ValueError(
                "Number of doc strings and xsubs does not correspond"
            )
    ...
    def __call__(self, html_strings) -> BatchFeature:
        ...
        if not valid_strings:
            raise ValueError(
                "HTML strings must of type `str`, `List[str]` (batch of"
                f" examples), but is of type {type(html_strings)}."
            )

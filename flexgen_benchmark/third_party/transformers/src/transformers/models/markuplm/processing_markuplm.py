        ...
        **kwargs
        ...
        if self.parse_html:
            if html_strings is None:
                raise ValueError(
                    "Make sure to pass HTML strings in case `parse_html` is"
                    " set to `True`"
                )

            if (
                nodes is not None
                or xpaths is not None
                or node_labels is not None
            ):
                raise ValueError(
                    "Please don't pass nodes, xpaths nor node labels in case"
                    " `parse_html` is set to `True`"
                )
            ...
            if html_strings is not None:
                raise ValueError(
                    "You have passed HTML strings but `parse_html` is set to"
                    " `False`."
                )
            if nodes is None or xpaths is None:
                raise ValueError(
                    "Make sure to pass nodes and xpaths in case `parse_html`"
                    " is set to `False`"
                )

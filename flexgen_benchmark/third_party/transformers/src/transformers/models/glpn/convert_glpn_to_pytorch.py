...
def rename_keys(state_dict):
    ...
    for key, value in state_dict.items():
        ...
        if "patch_embed" in key:
            ...
            key = key.replace(
                f"patch_embed{idx}", f"patch_embeddings.{int(idx)-1}"
            )
        if "norm" in key:
        ...
        if "glpn.encoder.layer_norm" in key:
            ...
            idx = key[
                key.find("glpn.encoder.layer_norm")
                + len("glpn.encoder.layer_norm")
            ]
...
def read_in_k_v(state_dict, config):
    ...
    for i in range(config.num_encoder_blocks):
        for j in range(config.depths[i]):
            ...
            state_dict[
                f"glpn.encoder.block.{i}.{j}.attention.self.key.bias"
...
def convert_glpn_checkpoint(
    ...
    if model_name is not None:
        ...
        assert torch.allclose(
            predicted_depth[0, :3, :3], expected_slice, atol=1e-4
        )
        print("Looks ok!")

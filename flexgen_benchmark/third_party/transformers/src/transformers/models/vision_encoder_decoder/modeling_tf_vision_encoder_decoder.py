...
DEPRECATION_WARNING = (
    "Version v4.17.0 introduces a better way to train encoder-decoder models"
    " by computing the loss inside the encoder-decoder framework rather than"
    " in the decoder itself. You may observe training discrepancies if"
    " fine-tuning a model trained with versions anterior to 4.17.0. The"
    " decoder_input_ids are now created based on the labels, no need to pass"
...
def shift_tokens_right(
    ...
    if pad_token_id is None:
        raise ValueError(
            "Make sure to set the pad_token_id attribute of the model's"
            " configuration."
        )
    ...
    if decoder_start_token_id is None:
        raise ValueError(
            "Make sure to set the decoder_start_token_id attribute of the"
            " model's configuration."
        )
    decoder_start_token_id = tf.cast(decoder_start_token_id, input_ids.dtype)

    start_tokens = tf.fill(
        (shape_list(input_ids)[0], 1), decoder_start_token_id
    )
...
class TFVisionEncoderDecoderModel(
    TFPreTrainedModel, TFCausalLanguageModelingLoss
):
    r"""
    ...
    def __init__(
        ...
        if config is None and (encoder is None or decoder is None):
            raise ValueError(
                "Either a configuration or an encoder and a decoder has to be"
                " provided."
            )
        if config is None:
        ...
        if config.decoder.cross_attention_hidden_size is not None:
            if (
                ...
                raise ValueError(
                    "If `cross_attention_hidden_size` is specified in the"
                    " decoder's configuration, it has to be equal to the"
                    " encoder's `hidden_size`. Got"
                    f" {config.decoder.cross_attention_hidden_size} for"
                    " `config.decoder.cross_attention_hidden_size` and"
                    f" {config.encoder.hidden_size} for"
                    " `config.encoder.hidden_size`."
                )
        ...
        if decoder is None:
            decoder = TFAutoModelForCausalLM.from_config(
                config.decoder, name="decoder"
            )
        ...
        if self.encoder.config.to_dict() != self.config.encoder.to_dict():
            logger.warning(
                f"Config of the encoder: {self.encoder.__class__} is"
                f" overwritten by shared encoder config: {self.config.encoder}"
            )
        if self.decoder.config.to_dict() != self.config.decoder.to_dict():
            logger.warning(
                f"Config of the decoder: {self.decoder.__class__} is"
                f" overwritten by shared decoder config: {self.config.decoder}"
            )
        ...
        if self.encoder.get_output_embeddings() is not None:
            raise ValueError(
                f"The encoder {self.encoder} should not have a LM Head. Please"
                " use a model without LM Head"
            )
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                if hasattr(_model, "enc_to_dec_proj"):
                    enc_to_dec_proj_kernel = tf.transpose(
                        tf.constant(
                            _model.enc_to_dec_proj.weight.detach()
                            .to("cpu")
                            .numpy()
                        ),
                        perm=(1, 0),
        ...
        **kwargs
        ...
        if encoder is None:
            if encoder_pretrained_model_name_or_path is None:
                raise ValueError(
                    "If `encoder_model` is not defined as an argument, a"
                    " `encoder_pretrained_model_name_or_path` has to be"
                    " defined."
                )
            ...
            if "config" not in kwargs_encoder:
                ...
                if (
                    ...
                    logger.info(
                        "Initializing"
                        f" {encoder_pretrained_model_name_or_path} as a"
                        " encoder model from a decoder model. Cross-attention"
                        " and casual mask are disabled."
                    )
        ...
        if decoder is None:
            if decoder_pretrained_model_name_or_path is None:
                raise ValueError(
                    "If `decoder_model` is not defined as an argument, a"
                    " `decoder_pretrained_model_name_or_path` has to be"
                    " defined."
                )
            ...
            if "config" not in kwargs_decoder:
                ...
                if (
                    ...
                    logger.info(
                        "Initializing"
                        f" {decoder_pretrained_model_name_or_path} as a"
                        " decoder model. Cross attention layers are added to"
                        f" {decoder_pretrained_model_name_or_path} and"
                        " randomly initialized if"
                        f" {decoder_pretrained_model_name_or_path}'s"
                        " architecture allows for cross attention layers."
                    )
            ...
            if (
                ...
                logger.warning(
                    f"Decoder model {decoder_pretrained_model_name_or_path} is"
                    " not initialized as a decoder. In order to initialize"
                    f" {decoder_pretrained_model_name_or_path} as a decoder,"
                    " make sure that the attributes `is_decoder` and"
                    " `add_cross_attention` of `decoder_config` passed to"
                    " `.from_encoder_decoder_pretrained(...)` are set to"
                    " `True` or do not pass a `decoder_config` to"
        ...
        if encoder.name != "encoder":
            raise ValueError(
                "encoder model must be created with the name `encoder`."
            )
        if decoder.name != "decoder":
            raise ValueError(
                "decoder model must be created with the name `decoder`."
            )
    ...
    @unpack_inputs
    @add_start_docstrings_to_model_forward(
        VISION_ENCODER_DECODER_INPUTS_DOCSTRING.format(
        ...
        **kwargs,
        ...
        if encoder_outputs is not None:
            if return_dict and not isinstance(encoder_outputs, ModelOutput):
                raise ValueError(
                    "If `return_dict=True` and `encoder_outputs` is provided,"
                    " it should be an instance of `ModelOutput`. Got an"
                    f" instance {type(encoder_outputs)} for `encoder_outputs`."
                )
        ...
        if encoder_outputs is None:
            ...
            if "input_ids" in encoder_inputs:
                encoder_inputs["pixel_values"] = encoder_inputs.pop(
                    "input_ids"
                )
            ...
            if "decoder_attention_mask" in encoder_inputs:
                decoder_attention_mask = encoder_inputs.pop(
        ...
        if not return_dict:
            ...
            start_index = sum(
                [
                    ...
                    for x in (loss, logits, past_key_values)
                ]
            )
    ...
    def resize_token_embeddings(self, *args, **kwargs):
        raise NotImplementedError(
            "Resizing the embedding layers via the TFVisionEncoderDecoderModel"
            " directly is not supported.Please use the respective methods of"
            " the wrapped objects (model.decoder.resize_token_embeddings(...))"
        )

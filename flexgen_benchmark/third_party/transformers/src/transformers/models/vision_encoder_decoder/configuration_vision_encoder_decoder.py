...
class VisionEncoderDecoderConfig(PretrainedConfig):
    ...
    def __init__(self, **kwargs):
        ...
        if "encoder" not in kwargs or "decoder" not in kwargs:
            raise ValueError(
                ...
                " instantiated because not both `encoder` and `decoder`"
    ...
    def from_encoder_decoder_configs(
        ...
        **kwargs
        ...
        """
        logger.info(
            "Setting `config.is_decoder=True` and"
            " `config.add_cross_attention=True` for decoder_config"
        )
...
class VisionEncoderDecoderDecoderOnnxConfig(OnnxConfig):
    ...
    def inputs(self) -> Mapping[str, Mapping[int, str]]:
        ...
        common_inputs["encoder_hidden_states"] = {
            0: "batch",
            1: "encoder_sequence",
        }
...
class VisionEncoderDecoderOnnxConfig(OnnxConfig):
    ...
    def get_encoder_config(
        self, encoder_config: PretrainedConfig
    ) -> OnnxConfig:
        r"""

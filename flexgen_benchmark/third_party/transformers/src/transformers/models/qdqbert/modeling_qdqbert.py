...
if is_pytorch_quantization_available():
    try:
        ...
        logger.error(
            "QDQBERT model are not usable since `pytorch_quantization` can't"
            " be loaded. Please try to reinstall it"
...
class QDQBertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense = quant_nn.QuantLinear(
...
@add_start_docstrings(
    (
        "The bare QDQBERT Model transformer outputting raw hidden-states"
...
class QDQBertLMHeadModel(QDQBertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning(
                "If you want to use `QDQBertLMHeadModel` as a standalone, add"
                " `is_decoder=True.`"
            )
...
class QDQBertForMaskedLM(QDQBertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `QDQBertForMaskedLM` make sure"
    ...
    def forward(
        ...
        **kwargs,
        ...
        if "next_sentence_label" in kwargs:
            warnings.warn(
                (
                    "The `next_sentence_label` argument is deprecated and will"
                    " be removed in a future version, use `labels` instead."
                ),
                FutureWarning,
            )
...
class QDQBertForMultipleChoice(QDQBertPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        QDQBERT_INPUTS_DOCSTRING.format(

...
def get_masks(slen, lengths, causal, padding_mask=None):
    ...
    if causal:
        attn_mask = (
            alen[None, None, :].repeat(bs, slen, 1) <= alen[None, :, None]
        )
    else:
        attn_mask = mask
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        mask_reshape = (
            (bs, 1, qlen, klen) if mask.dim() == 3 else (bs, 1, 1, klen)
        )
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Embedding):
            if (
                ...
                and self.config.embed_init_std is not None
            ):
                nn.init.normal_(
                    module.weight, mean=0, std=self.config.embed_init_std
                )
        ...
        if isinstance(module, nn.Linear):
            if self.config is not None and self.config.init_std is not None:
                nn.init.normal_(
                    module.weight, mean=0, std=self.config.init_std
                )
...
class FlaubertModel(FlaubertPreTrainedModel):
    ...
    def __init__(self, config):  # , dico, is_encoder, with_output):
        ...
        for _ in range(self.n_layers):
            ...
            self.layer_norm1.append(
                nn.LayerNorm(self.dim, eps=config.layer_norm_eps)
            )
            # if self.is_decoder:
    ...
    def forward(
        ...
        if position_ids is None:
            if hasattr(self, "position_ids"):
                ...
                position_ids = torch.arange(
                    slen, dtype=torch.long, device=device
                )
        ...
        tensor = inputs_embeds + self.position_embeddings(
            position_ids
...
class FlaubertForQuestionAnswering(FlaubertPreTrainedModel):
    ...
    @replace_return_docstrings(
        output_type=FlaubertForQuestionAnsweringOutput,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
        self,

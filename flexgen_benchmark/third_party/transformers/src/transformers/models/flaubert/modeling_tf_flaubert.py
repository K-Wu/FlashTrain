...
@add_start_docstrings(
    (
        "The bare Flaubert Model transformer outputting raw hidden-states"
...
class TFFlaubertForMultipleChoice(
    TFFlaubertPreTrainedModel, TFMultipleChoiceLoss
):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
    ...
    @unpack_inputs
    @add_start_docstrings_to_model_forward(
        FLAUBERT_INPUTS_DOCSTRING.format(
    ...
    def call(
        ...
        if lengths is not None:
            logger.warning(
                (
                    "The `lengths` parameter cannot be used with the Flaubert"

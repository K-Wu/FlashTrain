...
class TFViTMAEEmbeddings(tf.keras.layers.Layer):
    ...
    def __init__(self, config: ViTMAEConfig, **kwargs):
        ...
        self.patch_embeddings = TFViTMAEPatchEmbeddings(
    ...
    def random_masking(
        self, sequence: tf.Tensor, noise: Optional[tf.Tensor] = None
    ):
        """
    ...
    def call(
        self, pixel_values: tf.Tensor, noise: tf.Tensor = None
    ) -> tf.Tensor:
...
@add_start_docstrings(
    (
        "The bare ViTMAE Model transformer outputting raw hidden-states"
...
class TFViTMAEModel(TFViTMAEPreTrainedModel):
    ...
    def serving_output(
        self, output: TFViTMAEModelOutput
    ) -> TFViTMAEModelOutput:
        hidden_states = (
...
class TFViTMAEDecoder(tf.keras.layers.Layer):
    ...
    def call(
        ...
        mask_tokens = tf.tile(
            ...
            (
                shape_list(x)[0],
                shape_list(ids_restore)[1] + 1 - shape_list(x)[1],
                1,
            ),
        )
        x_ = tf.concat([x[:, 1:, :], mask_tokens], axis=1)  # no cls token
        x_ = tf.gather(
            x_, axis=1, batch_dims=1, indices=ids_restore
        )  # unshuffle
...
@add_start_docstrings(
    (
        "The ViTMAE Model transformer with the decoder on top for"
...
class TFViTMAEForPreTraining(TFViTMAEPreTrainedModel):
    ...
    def patchify(self, pixel_values):
        ...
        tf.debugging.assert_equal(
            ...
            0,
            message=(
                "Make sure the pixel values have a size that is divisible by"
                " the patch size"
            ),
        )
        tf.debugging.assert_equal(
            ...
            num_channels,
            message=(
                "Make sure the number of channels of the pixel values is equal"
                " to the one set in the configuration"
            ),
        )
        ...
        patchified_pixel_values = tf.einsum(
            "nhpwqc->nhwpqc", patchified_pixel_values
        )
        patchified_pixel_values = tf.reshape(
    ...
    def unpatchify(self, patchified_pixel_values):
        ...
        num_patches_one_direction = int(
            shape_list(patchified_pixel_values)[1] ** 0.5
        )
        # sanity check
        tf.debugging.assert_equal(
            num_patches_one_direction * num_patches_one_direction,
        ...
        patchified_pixel_values = tf.einsum(
            "nhwpqc->nhpwqc", patchified_pixel_values
        )

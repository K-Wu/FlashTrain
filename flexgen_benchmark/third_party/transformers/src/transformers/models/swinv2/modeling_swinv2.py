...
class Swinv2SelfAttention(nn.Module):
    def __init__(
        ...
        self.key = nn.Linear(
            self.all_head_size, self.all_head_size, bias=False
        )
...
class Swinv2Attention(nn.Module):
    def __init__(
        self, config, dim, num_heads, window_size, pretrained_window_size=0
    ):
        super().__init__()
...
class Swinv2Encoder(nn.Module):
    def __init__(
        self, config, grid_size, pretrained_window_sizes=(0, 0, 0, 0)
    ):
        super().__init__()
...
@add_start_docstrings(
    (
        "The bare Swinv2 Model transformer outputting raw hidden-states"
...
class Swinv2Model(Swinv2PreTrainedModel):
    def __init__(self, config, add_pooling_layer=True, use_mask_token=False):
        ...
        self.embeddings = Swinv2Embeddings(
...
class Swinv2ForMaskedImageModeling(Swinv2PreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(SWINV2_INPUTS_DOCSTRING)
    @replace_return_docstrings(
        output_type=Swinv2MaskedImageModelingOutput,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
        self,

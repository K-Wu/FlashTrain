...
def add_checkpointing_args(parser):
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If True, convert a Megatron checkpoint to a Transformers"
            " checkpoint. If False, convert a Transformers checkpoint to a"
            " Megatron checkpoint."
        ),
    )
    parser.add_argument(
...
def add_megatron_checkpoint_args(parser):
    parser.add_argument(
        ...
        default=1,
        help=(
            "The tensor model parallel size of the converted checkpoint. Only"
            " used when converting a Transformers checkpoint to a Megatron"
            " checkpoint."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            " Only used when converting a Transformers checkpoint to a"
            " Megatron checkpoint."
        ),
    )
    parser.add_argument(
        ...
        default=1,
        help=(
            "The data parallel size of the converted checkpoint. Only used"
            " when converting a Transformers checkpoint to a Megatron"
            " checkpoint."
        ),
    )
    parser.add_argument(
        ...
        default="fp32",
        help=(
            "The dtype of the converted checkpoint. Only used when converting"
            " a Transformers checkpoint to a Megatron checkpoint."
        ),
    )
    parser.add_argument(
        ...
        default=128,
        help=(
            "Pad the vocab size to be divisible by this value. This is added"
            " for computational efficieny reasons. Only used when converting a"
            " Transformers checkpoint to a Megatron checkpoint."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If True, use the distributed optimizer. Only used when converting"
            " a Transformers checkpoint to a Megatron checkpoint."
        ),
    )
    return parser
...
def add_transformers_checkpoint_args(parser):
    parser.add_argument(
        ...
        default=None,
        help=(
            "The name of the pre-trained tokenizer to save. If not None, the"
            " tokenizer will be saved. Only used when converting a Megatron"
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " size. If expressed as a string, needs to be digits followed by a"
            " unit (like `5MB`). Only used when converting a Megatron"
            " checkpoint to a Transformers checkpoint."
        ),
    )
...
transformers_to_megatron = {
    v[1:-1]: k for k, v in megatron_to_transformers.items()
}
...
def get_megatron_sharded_states(args, tp_size, pp_size, pp_rank):
    ...
    for i in range(tp_size):
        ...
        checkpoint_name = os.listdir(
            os.path.join(args.load_path, sub_dir_name)
        )[0]
        checkpoint_path = os.path.join(
...
def convert_checkpoint_from_megatron_to_transformers(args):
    ...
    for sub_dir in possible_sub_dirs:
        if sub_dir in sub_dirs:
            rank0_checkpoint_name = os.listdir(
                os.path.join(args.load_path, sub_dir)
            )[0]
            ...
            break
    print(
        "Loading Megatron-LM checkpoint arguments from:"
    ...
    if megatron_args is None:
        raise ValueError(
            ...
            " megatron arguments. This is because it loads all config related"
            " to model architecture, the tensor and pipeline model parallel"
            " size from the checkpoint insead of user having to manually"
            " specify all the details. Please save Megatron-LM checkpoint"
            " along with all the megatron arguments to use this utility."
        )
    ...
    for pp_rank in range(pp_size):
        ...
        # Extract the layers.
        for key, val in get_element_from_dict_by_path(
            ...
            # Transpose the weights.
                ...
                output_state_dict[
                    layer_name + out_name + "weight"
                ] = params.transpose(0, 1)
...
def convert_checkpoint_from_transformers_to_megatron(args):
    ...
    if len(sub_dirs) == 1:
        ...
        state_dict = merge_transformers_sharded_states(
    ...
    if config.num_hidden_layers % args.target_tensor_model_parallel_size != 0:
        raise ValueError(
            f"Number of layers ({config.num_hidden_layers}) must be divisible"
            " by number of tensor parallelism"
            f" ({args.target_tensor_model_parallel_size})"
        )
    num_layers = (
        config.num_hidden_layers // args.target_pipeline_model_parallel_size
    )
    ...
    for pp_rank in range(args.target_pipeline_model_parallel_size):
        ...
        for layer in range(num_layers):
            ...
            for layer_name in layers_to_copy:
                ...
                # handle attention K, V, Q weights
                    ...
                    op_name.startswith("attn.c_attn")
                    and weight_or_bias == "weight"
                ):
        ...
        if pp_rank == args.target_pipeline_model_parallel_size - 1:
            ...
            for weight_or_bias in ["weight", "bias"]:
                params = state_dict[f"transformer.ln_f.{weight_or_bias}"].to(
                    dtype
                )
        ...
        for tp_rank in range(args.target_tensor_model_parallel_size):
            ...
            if args.print_checkpoint_structure:
                print(
                    "Checkpoint structure of model state dict shard belonging"
                    f" to TP rank {tp_rank} and PP rank {pp_rank}:"
                )

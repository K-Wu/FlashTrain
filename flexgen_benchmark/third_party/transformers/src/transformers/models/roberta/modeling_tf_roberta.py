...
class TFRobertaLayer(tf.keras.layers.Layer):
    def __init__(self, config: RobertaConfig, **kwargs):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = TFRobertaAttention(
...
class TFRobertaMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, add_pooling_layer=True, **kwargs):
        ...
        self.pooler = (
            TFRobertaPooler(config, name="pooler")
...
@add_start_docstrings(
    (
        "The bare RoBERTa Model transformer outputting raw hidden-states"
...
class TFRobertaForMaskedLM(
    TFRobertaPreTrainedModel, TFMaskedLanguageModelingLoss
):
...
class TFRobertaForCausalLM(
    TFRobertaPreTrainedModel, TFCausalLanguageModelingLoss
):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    _keys_to_ignore_on_load_unexpected = [r"pooler", r"lm_head.decoder.weight"]
    ...
    def __init__(self, config: RobertaConfig, *inputs, **kwargs):
        ...
        if not config.is_decoder:
            logger.warning(
                "If you want to use `TFRobertaLMHeadModel` as a standalone,"
                " add `is_decoder=True.`"
            )
...
class TFRobertaForMultipleChoice(
    TFRobertaPreTrainedModel, TFMultipleChoiceLoss
):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
...
class TFRobertaForQuestionAnswering(
    TFRobertaPreTrainedModel, TFQuestionAnsweringLoss
):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    _keys_to_ignore_on_load_unexpected = [r"pooler", r"lm_head"]

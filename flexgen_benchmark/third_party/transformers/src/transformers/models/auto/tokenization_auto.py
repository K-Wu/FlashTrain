    ...
    TOKENIZER_MAPPING_NAMES = OrderedDict(
        [
            ...
            (
                "mluke",
                (
                    "MLukeTokenizer" if is_sentencepiece_available() else None,
                    None,
                ),
            ),
            (
...
class AutoTokenizer:
        ...
        if tokenizer_type is not None:
            ...
            if tokenizer_class_tuple is None:
                raise ValueError(
                    ...
                    " `tokenizer_type` should be one of"
            ...
            if tokenizer_class is None:
                raise ValueError(
                    f"Tokenizer class {tokenizer_class_name} is not currently"
                    " imported."
                )
        ...
        if config_tokenizer_class is not None:
            ...
            if tokenizer_auto_map is not None:
                if not trust_remote_code:
                    raise ValueError(
                        f"Loading {pretrained_model_name_or_path} requires you"
                        " to execute the tokenizer file in that repo on your"
                        " local machine. Make sure you have read the code"
                        " there to avoid malicious use, then set the option"
                        " `trust_remote_code=True` to remove this error."
                    )
                if kwargs.get("revision", None) is None:
                    logger.warning(
                        "Explicitly passing a `revision` is encouraged when"
                        " loading a model with custom code to ensure no"
                        " malicious code has been contributed in a newer"
                        " revision."
                    )
            ...
            if tokenizer_class is None:
                raise ValueError(
                    f"Tokenizer class {tokenizer_class_candidate} does not"
        ...
        if isinstance(config, EncoderDecoderConfig):
            if type(config.decoder) is not type(config.encoder):  # noqa: E721
                logger.warning(
                    ...
                    f" {config.encoder.__class__} is different from the"
                    f" decoder model config class: {config.decoder.__class__}."
                    " It is not recommended to use the"
                    " `AutoTokenizer.from_pretrained()` method in this case."
                    " Please use the encoder and decoder specific tokenizer"
                    " classes."
                )
        ...
        if model_type is not None:
            ...
            if tokenizer_class_fast and (
                ...
                if tokenizer_class_py is not None:
                    ...
                    raise ValueError(
                        "This tokenizer cannot be instantiated. Please make"
                        " sure you have `sentencepiece` installed in order to"
                        " use this tokenizer."
                    )

        raise ValueError(
            f"Unrecognized configuration class {config.__class__} to build an"
            " AutoTokenizer.\nModel type should be one of"
            f" {', '.join(c.__name__ for c in TOKENIZER_MAPPING.keys())}."
        )
    ...
    def register(
        ...
        if slow_tokenizer_class is None and fast_tokenizer_class is None:
            raise ValueError(
                "You need to pass either a `slow_tokenizer_class` or a"
        ...
        if (
            ...
            raise ValueError(
                "The fast tokenizer class you are passing has a"
                " `slow_tokenizer_class` attribute that is not consistent"
                " with the slow tokenizer class you passed (fast tokenizer"
                f" has {fast_tokenizer_class.slow_tokenizer_class} and you"
                f" passed {slow_tokenizer_class}. Fix one of those so they"
                " match!"
            )

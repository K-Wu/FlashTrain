...
def get_feature_extractor_config(
    ...
    **kwargs,
    ...
    if resolved_config_file is None:
        logger.info(
            "Could not locate the feature extractor configuration file, will"
...
class AutoFeatureExtractor:
    ...
    def __init__(self):
        raise EnvironmentError(
            "AutoFeatureExtractor is designed to be instantiated using the"
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
            ...
            if feature_extractor_auto_map is not None:
                if not trust_remote_code:
                    raise ValueError(
                        ...
                        " to execute the feature extractor file in that repo"
                        " on your local machine. Make sure you have read the"
                        " code there to avoid malicious use, then set the"
                        " option `trust_remote_code=True` to remove this"
                        " error."
                    )
                if kwargs.get("revision", None) is None:
                    logger.warning(
                        "Explicitly passing a `revision` is encouraged when"
                        " loading a feature extractor with custom code to"
                        " ensure no malicious code has been contributed in a"
                        " newer revision."
                    )
        ...
        raise ValueError(
            ...
            f" {pretrained_model_name_or_path}. Should have a"
            f" `feature_extractor_type` key in its {FEATURE_EXTRACTOR_NAME} of"
            f" {CONFIG_NAME}, or one of the following `model_type` keys in its"
            f" {CONFIG_NAME}:"

...
class _LazyConfigMapping(OrderedDict):
    ...
    def register(self, key, value):
        ...
        if key in self._mapping.keys():
            raise ValueError(
                f"'{key}' is already used by a Transformers config, pick"
                " another name."
            )
...
class _LazyLoadAllMappings(OrderedDict):
    ...
    def _initialize(self):
        ...
        warnings.warn(
            (
                "ALL_PRETRAINED_CONFIG_ARCHIVE_MAP is deprecated and will be"
                " removed in v5 of Transformers. It does not contain all"
                " available model checkpoints, far from it. Checkout"
                " hf.co/models for that."
            ),
            FutureWarning,
        )
...
def _list_model_options(indent, config_to_class=None, use_model_types=True):
    if config_to_class is None and not use_model_types:
        raise ValueError(
            "Using `use_model_types=False` requires a `config_to_class`"
            " dictionary."
        )
    if use_model_types:
        ...
        lines = [
            f"{indent}- **{model_type}** --"
            f" {model_type_to_name[model_type]} ({MODEL_NAMES_MAPPING[model_type]} model)"
...
def replace_list_option_in_docstrings(
    ...
    def docstring_decorator(fn):
        ...
        if i < len(lines):
            ...
            raise ValueError(
                f"The function {fn} should have an empty 'List options' in its"
                " docstring as placeholder, current docstring"
                f" is:\n{docstrings}"
            )
...
class AutoConfig:
    ...
    def for_model(cls, model_type: str, *args, **kwargs):
        ...
        raise ValueError(
            f"Unrecognized model identifier: {model_type}. Should contain one"
            f" of {', '.join(CONFIG_MAPPING.keys())}"
        )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        if (
            ...
            if not trust_remote_code:
                raise ValueError(
                    f"Loading {pretrained_model_name_or_path} requires you to"
                    " execute the configuration file in that repo on your"
                    " local machine. Make sure you have read the code there"
                    " to avoid malicious use, then set the option"
                    " `trust_remote_code=True` to remove this error."
                )
            if kwargs.get("revision", None) is None:
                logger.warning(
                    "Explicitly passing a `revision` is encouraged when"
                    " loading a configuration with custom code to ensure no"
        ...
        raise ValueError(
            f"Unrecognized model in {pretrained_model_name_or_path}. Should"
            f" have a `model_type` key in its {CONFIG_NAME}, or contain one of"
            " the following strings in its name:"
            f" {', '.join(CONFIG_MAPPING.keys())}"
        )
    ...
    def register(model_type, config):
        ...
        if (
            ...
            raise ValueError(
                "The config you are passing has a `model_type` attribute that"
                " is not consistent with the model type you passed (config"
                f" has {config.model_type} and you passed {model_type}. Fix"

...
class _BaseAutoModelClass:
    ...
    def __init__(self, *args, **kwargs):
        raise EnvironmentError(
            ...
            " the"
            f" `{self.__class__.__name__}.from_pretrained(pretrained_model_name_or_path)`"
            f" or `{self.__class__.__name__}.from_config(config)` methods."
        )
    ...
    def from_config(cls, config, **kwargs):
        ...
        if hasattr(config, "auto_map") and cls.__name__ in config.auto_map:
            if not trust_remote_code:
                raise ValueError(
                    "Loading this model requires you to execute the modeling"
                    " file in that repo on your local machine. Make sure you"
                    " have read the code there to avoid malicious use, then"
                    " set the option `trust_remote_code=True` to remove this"
                    " error."
                )
            if kwargs.get("revision", None) is None:
                logger.warning(
                    "Explicitly passing a `revision` is encouraged when"
                    " loading a model with custom code to ensure no malicious"
        ...
        raise ValueError(
            f"Unrecognized configuration class {config.__class__} for this"
            f" kind of AutoModel: {cls.__name__}.\nModel type should be one of"
            f" {', '.join(c.__name__ for c in cls._model_mapping.keys())}."
        )
        ...
        if hasattr(config, "auto_map") and cls.__name__ in config.auto_map:
            if not trust_remote_code:
                raise ValueError(
                    ...
                    " execute the modeling file in that repo on your local"
                    " machine. Make sure you have read the code there to"
                    " avoid malicious use, then set the option"
    ...
    def register(cls, config_class, model_class):
        ...
        if (
            ...
            raise ValueError(
                ...
                " attribute that is not consistent with the config class you"
                f" passed (model has {model_class.config_class} and you passed"
                f" {config_class}. Fix one of those so they match!"
            )
...
def insert_head_doc(docstring, head_doc=""):
    if len(head_doc) > 0:
        return docstring.replace(
            ...
            (
                "one of the model classes of the library (with a"
                f" {head_doc} head) "
            ),
        )
    return docstring.replace(
...
def getattribute_from_module(module, attr):
    ...
    if module != transformers_module:
        try:
            ...
            raise ValueError(
                f"Could not find {attr} neither in {module} nor in"
                f" {transformers_module}!"
            )
    else:

...
if is_tensorflow_probability_available():
    try:
        ...
        logger.error(
            "GroupViT models are not usable since `tensorflow_probability`"
            " can't be loaded.It seems you have `tensorflow_probability`"
            " installed with the wrong tensorflow version.Please try to"
            " reinstall it following the instructions here:"
...
def resize_attention_map(
    ...
    attentions = tf.reshape(
        attentions, (batch_size, groups, feat_height, feat_width)
    )
...
class TFGroupViTAssignAttention(tf.keras.layers.Layer):
    ...
    def call(self, query: tf.Tensor, key: tf.Tensor, training: bool = False):
        ...
        soft_attn = self.get_attn(
            raw_attn, training=training, gumbel=False, hard=False
        )
...
class TFGroupViTTokenAssign(tf.keras.layers.Layer):
    ...
    def call(
        ...
        new_image_tokens, attention = self.assign(
            projected_group_tokens, image_tokens
        )
...
class TFGroupViTPatchEmbeddings(tf.keras.layers.Layer):
    ...
    def call(
        ...
        embeddings = tf.reshape(
            tensor=projection,
            shape=(batch_size, num_patches, self.hidden_size),
        )
...
class TFGroupViTVisionEmbeddings(tf.keras.layers.Layer):
    ...
    def __init__(self, config: GroupViTVisionConfig, **kwargs):
        ...
        self.dropout = tf.keras.layers.Dropout(
            rate=config.dropout, name="dropout"
        )
    ...
    def interpolate_pos_encoding(self, embeddings, height, width) -> tf.Tensor:
        ...
        patch_pos_embed = tf.reshape(
            tensor=patch_pos_embed, shape=(1, -1, dim)
        )
...
class TFGroupViTVisionEncoder(tf.keras.layers.Layer):
    ...
    def call(
        ...
        for stage in self.stages:
            ...
            layer_outputs = stage(
                hidden_states, group_tokens, output_attentions
            )
...
class TFGroupViTTextTransformer(tf.keras.layers.Layer):
    ...
    def _build_causal_attention_mask(
        self, batch_size, seq_length, dtype=tf.float32
    ):
...
class TFGroupViTMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config: GroupViTConfig, **kwargs):
        ...
        if not isinstance(config.text_config, GroupViTTextConfig):
            raise ValueError(
                "config.text_config is expected to be of type"
                " GroupViTTextConfig but is of type"
                f" {type(config.text_config)}."
            )

        if not isinstance(config.vision_config, GroupViTVisionConfig):
            raise ValueError(
                "config.vision_config is expected to be of type"
                " GroupViTVisionConfig but is of type"
        ...
        self.text_model = TFGroupViTTextTransformer(
            text_config, name="text_model"
        )
        self.vision_model = TFGroupViTVisionTransformer(
        ...
        self.visual_projection = [
            ...
            tf.keras.layers.Dense(
                self.projection_dim, name="visual_projection.3"
            ),
        ]
        self.text_projection = [
            ...
            tf.keras.layers.Dense(
                self.projection_dim, name="text_projection.3"
            ),
        ]
    ...
    def call(
        ...
        logits_per_text = (
            tf.matmul(text_embeds, image_embeds, transpose_b=True)
            * logit_scale
        )
        ...
        if output_segmentation:
            ...
            image_group_embeds = tf.reshape(
                image_group_embeds,
                shape=(-1, shape_list(image_group_embeds)[-1]),
            )
            for layer in self.visual_projection:
            ...
            grouping = get_grouping_from_attentions(
                attentions, pixel_values.shape[2:]
            )

            # normalized features
            image_group_embeds = image_group_embeds / tf.norm(
                tensor=image_group_embeds,
                ord="euclidean",
                axis=-1,
                keepdims=True,
            )
            ...
            seg_logits = (
                tf.matmul(logits_per_image_group, flatten_grouping)
                * logit_scale
            )

...
def resize_attention_map(attentions, height, width, align_corners=False):
    ...
    attentions = attentions.reshape(
        batch_size, groups, feat_height, feat_width
    )
...
class GroupViTStage(nn.Module):
    ...
    def forward(
        ...
        """
        if self.with_group_token:
            group_token = self.group_token.expand(
                hidden_states.size(0), -1, -1
            )
            if self.group_projector is not None:
                group_token = group_token + self.group_projector(
                    prev_group_token
                )
        else:
            group_token = None
        ...
        for layer in self.layers:
            layer_out = layer(
                cat_x, attention_mask=None, causal_attention_mask=None
            )
            cat_x = layer_out[0]
    ...
    def forward(
        ...
        if is_cross_attention:
            key_states = self._shape(
                self.k_proj(encoder_hidden_states), -1, bsz
            )
...
class GroupViTModel(GroupViTPreTrainedModel):
    ...
    def __init__(self, config: GroupViTConfig):
        ...
        self.visual_projection = nn.Sequential(
            nn.Linear(
                ...
                self.projection_intermediate_dim,
                bias=True,
            ),
    ...
    def forward(
        ...
        if output_segmentation:
            ...
            seg_logits = (
                torch.matmul(logits_per_image_group, flatten_grouping)
                * logit_scale
            )

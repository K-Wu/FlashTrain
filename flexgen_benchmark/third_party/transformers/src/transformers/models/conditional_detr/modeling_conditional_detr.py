...
class ConditionalDetrSinePositionEmbedding(nn.Module):
    ...
    def forward(self, pixel_values, pixel_mask):
        ...
        dim_t = self.temperature ** (
            2 * torch_int_div(dim_t, 2) / self.embedding_dim
        )
    ...
    def forward(
        ...
        if is_cross_attention:
            ...
            key_states = self._shape(
                self.k_proj(key_value_states), -1, batch_size
            )
...
class ConditionalDetrAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.v_head_dim * num_heads != self.out_dim:
            raise ValueError(
                "out_dim must be divisible by num_heads (got `out_dim`:"
                f" {self.out_dim} and `num_heads`: {num_heads})."
            )
        self.scaling = self.head_dim**-0.5
    ...
    def forward(
        ...
        query_states = self._qk_shape(
            query_states, target_len, batch_size
        ).view(*proj_shape)
        key_states = key_states.view(*proj_shape)
        ...
        if attn_output.size() != (
            ...
            raise ValueError(
                "`attn_output` should be of size"
                f" {(batch_size, self.num_heads, target_len, self.v_head_dim)},"
                f" but is {attn_output.size()}"
            )
...
class ConditionalDetrDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        k_pos = k_pos.view(
            batch_size, source_len, self.nhead, n_model // self.nhead
        )
        k = torch.cat([k, k_pos], dim=3).view(
...
class ConditionalDetrEncoder(ConditionalDetrPreTrainedModel):
    ...
    def __init__(self, config: ConditionalDetrConfig):
        ...
        self.layers = nn.ModuleList(
            [
                ConditionalDetrEncoderLayer(config)
...
class ConditionalDetrDecoder(ConditionalDetrPreTrainedModel):
    ...
    def __init__(self, config: ConditionalDetrConfig):
        ...
        self.layers = nn.ModuleList(
            [
                ConditionalDetrDecoderLayer(config)
...
class ConditionalDetrModel(ConditionalDetrPreTrainedModel):
    ...
    def forward(
        ...
        position_embeddings = (
            position_embeddings_list[-1].flatten(2).permute(0, 2, 1)
        )
...
class ConditionalDetrForObjectDetection(ConditionalDetrPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(CONDITIONAL_DETR_INPUTS_DOCSTRING)
    @replace_return_docstrings(
        output_type=ConditionalDetrObjectDetectionOutput,
...
class ConditionalDetrForSegmentation(ConditionalDetrPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(CONDITIONAL_DETR_INPUTS_DOCSTRING)
    @replace_return_docstrings(
        output_type=ConditionalDetrSegmentationOutput,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
        self,
        ...
        ) = self.conditional_detr.model.backbone(
...
class ConditionalDetrMaskHeadSmallConv(nn.Module):
    ...
    def __init__(self, dim, fpn_dims, context_dim):
        ...
        if dim % 8 != 0:
            raise ValueError(
                "The hidden_size + number of attention heads must be divisible"
                " by 8 as the number of groups in GroupNorm is set to 8"
            )
    ...
    def forward(self, x: Tensor, bbox_mask: Tensor, fpns: List[Tensor]):
        ...
        x = torch.cat(
            [_expand(x, bbox_mask.shape[1]), bbox_mask.flatten(0, 1)], 1
        )
...
class ConditionalDetrMHAttentionMap(nn.Module):
    ...
    def forward(self, q, k, mask: Optional[Tensor] = None):
        ...
        weights = nn.functional.softmax(weights.flatten(2), dim=-1).view(
            weights.size()
        )

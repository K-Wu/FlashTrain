...
def convert_xlm_roberta_xl_checkpoint_to_pytorch(
    ...
    model.roberta.embeddings.token_type_embeddings.weight.data = (
        torch.zeros_like(model.roberta.embeddings.token_type_embeddings.weight)
    )  # just zero them out b/c RoBERTa doesn't use them.

    model.roberta.encoder.LayerNorm.weight = (
        roberta_sent_encoder.layer_norm.weight
    )
    ...
    for i in range(config.num_hidden_layers):
        ...
        roberta_layer: TransformerSentenceEncoderLayer = (
            roberta_sent_encoder.layers[i]
        )
        ...
        attention.self_attn_layer_norm.bias = (
    ...
    if classification_head:
        ...
        model.lm_head.layer_norm.bias = (
            roberta.model.encoder.lm_head.layer_norm.bias
        )

...
class MaskFormerModelOutput(ModelOutput):
    ...
    transformer_decoder_hidden_states: Optional[
...
def upsample_like(
    pixel_values: Tensor, like: Tensor, mode: str = "bilinear"
) -> Tensor:
    """
...
class MaskFormerSwinSelfAttention(nn.Module):
    ...
    def forward(
        ...
        attention_scores = attention_scores + relative_position_bias.unsqueeze(
            0
        )
...
class MaskFormerSwinModel(nn.Module, ModuleUtilsMixin):
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = MaskFormerSwinEncoder(
            config, self.embeddings.patch_grid
        )
...
class MaskFormerHungarianMatcher(nn.Module):
    ...
    def forward(
        ...
        for pred_probs, pred_mask, target_mask, labels in zip(
            ...
            cost_mask = pair_wise_sigmoid_focal_loss(
                pred_mask_flat, target_mask_flat
            )
    ...
    def loss_masks(
        ...
        losses = {
            "loss_mask": sigmoid_focal_loss(
                pred_masks, target_masks, num_masks
            ),
    ...
    def forward(
        ...
        if auxiliary_predictions is not None:
            for idx, aux_outputs in enumerate(auxiliary_predictions):
                ...
                loss_dict = {
                    f"{key}_{idx}": value for key, value in loss_dict.items()
                }
...
class MaskFormerFPNLayer(nn.Module):
    ...
    def forward(self, down: Tensor, left: Tensor) -> Tensor:
        ...
        down = nn.functional.interpolate(
            down, size=left.shape[-2:], mode="nearest"
        )
        down += left
...
class MaskFormerPixelDecoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        """
        super().__init__()
        self.fpn = MaskFormerFPNModel(
            *args, feature_size=feature_size, **kwargs
        )
...
class PredictionBlock(nn.Module):
    def __init__(
        self, in_dim: int, out_dim: int, activation: nn.Module
    ) -> None:
        super().__init__()
...
class MaskFormerPixelLevelModule(nn.Module):
    def __init__(self, config: MaskFormerConfig):
        ...
        """
        super().__init__()
        self.encoder = MaskFormerSwinTransformerBackbone(
...
class MaskFormerTransformerModule(nn.Module):
    ...
    def forward(
        ...
        inputs_embeds = torch.zeros_like(
            queries_embeddings, requires_grad=True
        )
...
class MaskFormerPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: nn.Module):
        ...
        if isinstance(module, MaskFormerTransformerModule):
            if module.input_projection is not None:
                nn.init.xavier_uniform_(
                    module.input_projection.weight, gain=xavier_std
                )
        ...
        # FPN
            ...
            nn.init.xavier_uniform_(
                module.get_submodule("0").weight, gain=xavier_std
            )
        # The MLP head
...
@add_start_docstrings(
    (
        "The bare MaskFormer Model outputting raw hidden-states without any"
...
class MaskFormerForInstanceSegmentation(MaskFormerPreTrainedModel):
    ...
    def get_logits(
        ...
        if self.config.use_auxiliary_loss:
            ...
            # get the masks
            mask_embeddings = self.mask_embedder(
            ...
            for aux_binary_masks, aux_classes in zip(
                binaries_masks[:-1], classes[:-1]
            ):

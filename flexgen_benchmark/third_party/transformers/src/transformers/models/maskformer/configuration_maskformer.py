        ...
        **kwargs,
        ...
        if backbone_config is None:
            ...
            if backbone_model_type not in self.backbones_supported:
                raise ValueError(
                    f"Backbone {backbone_model_type} not supported, please use"
                    f" one of {','.join(self.backbones_supported)}"
                )
        ...
        if decoder_config is None:
            ...
            if decoder_type not in self.decoders_supported:
                raise ValueError(
                    f"Transformer Decoder {decoder_type} not supported, please"
                    f" use one of {','.join(self.decoders_supported)}"
                )
            decoder_config = AutoConfig.for_model(
                decoder_type, **decoder_config
            )

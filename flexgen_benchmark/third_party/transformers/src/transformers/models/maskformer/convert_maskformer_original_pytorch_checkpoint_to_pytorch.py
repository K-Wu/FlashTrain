...
class OriginalMaskFormerConfigToOursConverter:
    def __call__(self, original_config: object) -> MaskFormerConfig:
        ...
        id2label = {
            idx: label
            for idx, label in enumerate(dataset_catalog.stuff_classes)
        }
...
class OriginalMaskFormerCheckpointToOursConverter:
    ...
    def replace_pixel_module(
        ...
        renamed_keys.extend(
            rename_keys_for_conv(
                f"{src_prefix}.layer_4", f"{dst_prefix}.fpn.stem"
            )
        )
    ...
    def replace_detr_decoder(
        ...
        dst_prefix: str = "transformer_module.decoder"
        src_prefix: str = "sem_seg_head.predictor.transformer.decoder"
        renamed_keys = self.rename_keys_in_detr_decoder(
        ...
        renamed_keys.extend(
            [
                (
                    f"{src_prefix}.norm.weight",
                    f"{dst_prefix}.layernorm.weight",
                ),
    ...
    def replace_transformer_module(
        ...
        renamed_keys = [
            ...
            (
                f"{src_prefix}.input_proj.bias",
                f"{dst_prefix}.input_projection.bias",
            ),
        ]
    ...
    def replace_instance_segmentation_module(
        ...
        renamed_keys = [
            (
                f"{src_prefix}.class_embed.weight",
                f"{dst_prefix}class_predictor.weight",
            ),
            (
                f"{src_prefix}.class_embed.bias",
                f"{dst_prefix}class_predictor.bias",
            ),
        ]
    ...
    def convert_instance_segmentation(
        ...
        self.replace_instance_segmentation_module(
            dst_state_dict, src_state_dict
        )
...
def get_name(checkpoint_file: Path):
    ...
    if "coco" in parent_name:
        ...
        raise ValueError(
            f"{parent_name} must be wrong since we didn't find 'coco' or 'ade'"
            " in it "
        )
    ...
    backbone_type = list(
        filter(lambda x: x in model_name_raw, backbone_types)
    )[0]
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description=(
            "Command line to convert the original maskformers (with swin"
            " backbone) to our implementations."
        )
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "A directory containing the model's checkpoints. The directory has"
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "A directory containing the model's configs, see detectron2 doc."
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "A path to MaskFormer's original implementation directory. You can"
            " download from here:"
    ...
    for (
        ...
        feature_extractor = (
            OriginalMaskFormerConfigToFeatureExtractorConverter()(
        ...
        converter = OriginalMaskFormerCheckpointToOursConverter(
        ...
        mask_former_for_instance_segmentation = (
            MaskFormerForInstanceSegmentation(config=config).eval()
        )

        mask_former_for_instance_segmentation.model = mask_former
        mask_former_for_instance_segmentation = (
            converter.convert_instance_segmentation(
                mask_former_for_instance_segmentation
            )
        )

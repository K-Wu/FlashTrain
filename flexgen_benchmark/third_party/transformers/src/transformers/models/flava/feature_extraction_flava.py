...
class FlavaMaskingGenerator:
    ...
    def __repr__(self):
        repr_str = (
            "MaskingGenerator(%d, %d -> [%d ~ %d], max = %d, %.3f ~ %.3f)"
            % (
                self.height,
                self.width,
    ...
    def _mask(self, mask, max_mask_patches):
        ...
        for _attempt in range(10):
            target_area = random.uniform(
                self.mask_group_min_patches, max_mask_patches
            )
    ...
    def __call__(self):
        ...
        while mask_count < self.total_mask_patches:
            ...
            max_mask_patches = min(
                max_mask_patches, self.mask_group_max_patches
            )
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        if return_codebook_pixels:
            ...
            if (
                self.codebook_do_center_crop
                and self.codebook_crop_size is not None
            ):
                images = [
                    self.center_crop(image, self.codebook_crop_size)
                    for image in images
                ]

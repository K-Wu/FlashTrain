...
FlavaPossibleConfigs = Union[
    FlavaTextConfig, FlavaImageConfig, FlavaMultimodalConfig
]
...
class FlavaForPreTrainingOutput(ModelOutput):
    ...
    def to_tuple(self) -> Tuple[Any]:
        ...
        return tuple(
            self[k]
            if k not in transformer_outputs
            else getattr(self, k).to_tuple()
            for k in self.keys()
        )
...
class FlavaImageEmbeddings(nn.Module):
    ...
    def __init__(
        self, config: FlavaImageConfig, use_mask_token: bool = False
    ) -> None:
        super().__init__()
    ...
    def interpolate_pos_encoding(
        ...
        if (
            ...
            raise ValueError(
                "Number of patches for images"
                f" ({int(num_h_patches), int(num_w_patches)}) don't match the"
                " shape of position embedding"
                f" ({patch_pos_embed.shape[-2], patch_pos_embed.shape[-1]})"
            )
...
@add_start_docstrings(
    (
        "The bare FLAVA Image Model transformer outputting raw hidden-states"
...
class FlavaImageModel(FlavaPreTrainedModel):
    ...
    def __init__(
        self, config: FlavaImageConfig, add_pooling_layer: bool = True
    ):
...
@add_start_docstrings(
    (
        "The bare FLAVA Text Model transformer outputting raw hidden-states"
        " without any specific head on top."
    ),
...
class FlavaTextModel(FlavaPreTrainedModel):
    ...
    def __init__(
        self, config: FlavaTextConfig, add_pooling_layer: bool = True
    ):
        super().__init__(config)
        self.config = config
...
@add_start_docstrings(
    (
        "The bare FLAVA Multimodal Model transformer outputting raw"
...
@add_start_docstrings(
    (
        "The bare FLAVA Model transformer outputting raw hidden-states without"
...
class FlavaModel(FlavaPreTrainedModel):
    ...
    def __init__(self, config: FlavaConfig):
        ...
        if not isinstance(config.text_config, FlavaTextConfig):
            raise ValueError(
                "config.text_config is expected to be of type FlavaTextConfig"
                f" but is of type {type(config.text_config)}."
            )

        if not isinstance(config.image_config, FlavaImageConfig):
            raise ValueError(
                "config.image_config is expected to be of type"
                " FlavaImageConfig but is of type"
                f" {type(config.image_config)}."
            )

        if not isinstance(config.multimodal_config, FlavaMultimodalConfig):
            raise ValueError(
                "config.multimodal_config is expected to be of type"
                " FlavaMultimodalConfig but "
        ...
        self.image_projection = nn.Linear(
            self.image_hidden_size, self.projection_dim
        )
        self.text_projection = nn.Linear(
            self.text_hidden_size, self.projection_dim
        )
    ...
    @replace_return_docstrings(
        output_type=FlavaModelOutput, config_class=FlavaConfig
    )
    def forward(
        self,
        ...
        if not output_hidden_states:
            raise ValueError(
                "FLAVA model requires hidden states to work. Please set"
...
class FlavaImageCodebookResPath(nn.Module):
    def __init__(self, in_size: int, out_size: int, **kwargs):
        ...
        path["relu_2"] = nn.ReLU()
        path["conv_2"] = nn.Conv2d(
            hid_size, hid_size, kernel_size=3, padding=1
        )
        path["relu_3"] = nn.ReLU()
        path["conv_3"] = nn.Conv2d(
            hid_size, hid_size, kernel_size=3, padding=1
        )
        path["relu_4"] = nn.ReLU()
        path["conv_4"] = nn.Conv2d(
            hid_size, out_size, kernel_size=1, padding=0
        )
    ...
    def __init__(self, in_size: int, out_size: int, num_layers: int, **kwargs):
        ...
        if in_size != out_size:
            self.id_path = nn.Conv2d(
                in_size, out_size, kernel_size=1, padding=0
            )
        else:
...
class FlavaImageCodebook(FlavaPreTrainedModel):
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        if pixel_values.shape[1] != self.input_channels:
            raise ValueError(
                f"input has {pixel_values.shape[1]} channels but model built"
                f" for {self.input_channels}"
            )
...
class FlavaGlobalContrastiveHead(nn.Module):
    ...
    def forward(self, image_embeddings, text_embeddings, logit_scale):
        ...
        if (
            ...
            labels = (
                local_batch_size * torch.distributed.get_rank()
                + torch.arange(
...
class FlavaForPreTraining(FlavaPreTrainedModel):
    def __init__(
        self, config: FlavaConfig, image_codebook: Optional[nn.Module] = None
    ):
        super().__init__(config)
        ...
        if self.image_codebook is None and config.init_codebook:
            self.image_codebook = FlavaImageCodebook(
                config.image_codebook_config
            )
        ...
        self.skip_unmasked_multimodal_encoder = (
            config.skip_unmasked_multimodal_encoder
        )
    ...
    def forward(
        ...
        if input_ids_masked is None and input_ids is not None:
            logger.warning(
                "`input_ids_masked` isn't passed which means MLM loss won't be"
                " calculated correctlySetting it to `input_ids` so that model"
                " can work. Please pass it if this is unintentional. This is"
                " usually OKAY if you are doing inference on unmasked text..."
            )
        ...
        multimodal_masked_embeddings = (
            flava_masked_output.multimodal_embeddings
        )
        ...
        total_loss = (
            ...
            mlm_loss
        ) = mmm_text_loss = mmm_image_loss = gc_loss = itm_loss = None
        ...
        if (
            ...
            if mim_labels is None and return_loss:
                if self.image_codebook is None:
                    raise RuntimeError(
                        "`return_loss` is set to True but the image codebook"
                        " is not initialized and no `mim_labels`  have been"
                        " passed. Reinstantiate the model with `init_codebook`"
                        " set to True or pass in your custom `mim_labels`"
                    )
                if codebook_pixel_values is None:
                    raise ValueError(
                        "`codebook_pixel_value` are required to generate"
                        " `mim_labels` if loss is expected. Call"
                        " `FlavaProcessor` with `return_codebook_pixels` set"
                        " to True"
                    )
        ...
        if (
            ...
            if mim_labels is not None:
                ...
                sequence_for_image = sequence_for_image[
                    :, -mim_labels.size(1) :, :
                ]
        ...
        if self.itm_weight > 0 and multimodal_masked_embeddings is not None:
            ...
            if itm_labels is not None:
                ...
                if return_loss:
                    itm_loss = nn.functional.cross_entropy(
        ...
        # MMM Image Loss
        if (
            multimodal_masked_embeddings is not None
            and self.mmm_image_weight > 0
        ):
        ...
        if (
            ...
            text_embedding = self.flava.text_projection(
                text_embeddings[:, 0, :]
            )
            text_embedding = nn.functional.normalize(text_embedding, dim=-1)

            image_embedding = self.flava.image_projection(
                image_embeddings[:, 0, :]
            )
            ...
            if return_loss:
                gc_loss_image = nn.functional.cross_entropy(
                    logits_per_image, gc_labels
                )
                gc_loss_text = nn.functional.cross_entropy(
                    logits_per_text, gc_labels
                )
        ...
        if return_loss and not flava_losses.all_none():
            total_loss = sum(
                ...
                for loss in flava_losses.values()
            )

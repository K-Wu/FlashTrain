...
@add_start_docstrings(
    (
        "XLM-RoBERTa Model with a `language modeling` head on top for CLM"
        " fine-tuning."
    ),
    XLM_ROBERTA_START_DOCSTRING,
)
...
class XLMRobertaForCausalLM(XLMRobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning(
                "If you want to use `XLMRobertaLMHeadModel` as a standalone,"
                " add `is_decoder=True.`"
            )
...
class XLMRobertaForMaskedLM(XLMRobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `XLMRobertaForMaskedLM` make sure"

...
class TFResNetConvLayer(tf.keras.layers.Layer):
    ...
    def convolution(self, hidden_state: tf.Tensor) -> tf.Tensor:
        ...
        hidden_state = tf.pad(
            hidden_state, [(0, 0), height_pad, width_pad, (0, 0)]
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.conv1 = TFResNetConvLayer(
            out_channels, stride=stride, name="layer.0"
        )
        self.conv2 = TFResNetConvLayer(
            out_channels, activation=None, name="layer.1"
        )
        self.shortcut = (
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.conv0 = TFResNetConvLayer(
            reduces_channels, kernel_size=1, name="layer.0"
        )
        self.conv1 = TFResNetConvLayer(
            reduces_channels, stride=stride, name="layer.1"
        )
        self.conv2 = TFResNetConvLayer(
...
class TFResNetEncoder(tf.keras.layers.Layer):
    def __init__(self, config: ResNetConfig, **kwargs) -> None:
        ...
        for i, (in_channels, out_channels, depth) in enumerate(
            zip(
                config.hidden_sizes, config.hidden_sizes[1:], config.depths[1:]
            )
        ):
            self.stages.append(
                TFResNetStage(
    ...
    def call(
        ...
        if not return_dict:
            return tuple(
                v for v in [hidden_state, hidden_states] if v is not None
            )
...
@add_start_docstrings(
    (
        "The bare ResNet model outputting raw features without any specific"
        " head on top."
    ),

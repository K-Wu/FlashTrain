...
def load_tf_weights_in_imagegpt(model, config, imagegpt_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        ...
        if name[-1] == "q_proj":
            ...
            ] = torch.from_numpy(array.reshape(config.n_embd, config.n_embd)).T
        elif name[-1] == "v_proj":
...
class ImageGPTLayerNorm(nn.Module):
    ...
    def forward(self, tensor: torch.Tensor) -> tuple:
        ...
        return (
            tensor
            / torch.sqrt(
                torch.mean(torch.square(tensor), axis=-1, keepdim=True)
                + self.eps
            )
            * self.weight.data[..., :]
        )
...
class ImageGPTAttention(nn.Module):
    ...
    def _upcast_and_reordered_attn(
        ...
        with autocast(enabled=False):
            q, k = query.reshape(-1, q_seq_len, dk), key.transpose(
                -1, -2
        ...
        if attn_weights.dtype != torch.float32:
            raise RuntimeError(
                "Error with upcasting, attn_weights does not have dtype"
                " torch.float32"
            )
        attn_weights = attn_weights.type(value.dtype)
        attn_weights = self.attn_dropout(attn_weights)
...
@add_start_docstrings(
    (
        "The bare ImageGPT Model transformer outputting raw hidden-states"
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if "pixel_values" in kwargs:
            warnings.warn(
                (
                    "The `pixel_values` argument is deprecated and will be"
                    " removed in a future version, use `input_ids` instead."
                ),
                FutureWarning,
            )

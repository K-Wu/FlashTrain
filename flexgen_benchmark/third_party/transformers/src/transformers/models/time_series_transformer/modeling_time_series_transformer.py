...
class AffineTransformed(TransformedDistribution):
    def __init__(
        ...
        super().__init__(
            ...
            [
                AffineTransform(
                    loc=self.loc, scale=self.scale, event_dim=event_dim
                )
            ],
        )
        ...
        return torch.cat(
            [
                ...
                for embed, cat_feature_slice in zip(
                    self.embedders, cat_feature_slices
                )
            ],
            dim=-1,
        )
...
@add_start_docstrings(
    (
        "The bare Time Series Transformer Model outputting raw hidden-states"
    ...
    def get_lagged_subsequences(
        ...
        try:
            assert max(indices) + subsequences_length <= sequence_length, (
                ...
                f" {max(indices)} while history length is only"
                f" {sequence_length}"
            )
    ...
    def create_network_inputs(
        ...
        try:
            assert inputs.shape[1] == inputs_length, (
                (
                    f"input length {inputs.shape[1]} and dynamic feature"
                    f" lengths {inputs_length} does not match"
                ),
            )
        except AssertionError as e:
        ...
        static_feat = torch.cat(
            (embedded_cat, static_real_features, log_scale), dim=1
        )
...
@add_start_docstrings(
    (
        "The Time Series Transformer Model with a distribution head on top for"
...
class TimeSeriesTransformerForPrediction(TimeSeriesTransformerPreTrainedModel):
    def __init__(self, config: TimeSeriesTransformerConfig):
        ...
        self.parameter_projection = (
            self.distribution_output.get_parameter_projection(
    ...
    def generate(
        ...
        for k in range(self.config.prediction_length):
            ...
            decoder_input = torch.cat(
                (reshaped_lagged_sequence, repeated_features[:, : k + 1]),
                dim=-1,
            )

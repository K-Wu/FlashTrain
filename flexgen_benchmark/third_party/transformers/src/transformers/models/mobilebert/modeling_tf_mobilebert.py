...
_TOKEN_CLASS_EXPECTED_OUTPUT = (
    "['I-ORG', 'I-ORG', 'O', 'O', 'O', 'O', 'O', 'I-LOC', 'O', 'I-LOC',"
    " 'I-LOC']"
)
...
class TFMobileBertLMPredictionHead(tf.keras.layers.Layer):
    ...
    def call(self, hidden_states):
        ...
        hidden_states = tf.matmul(
            hidden_states,
            tf.concat([tf.transpose(self.decoder), self.dense], axis=0),
        )
...
class TFMobileBertMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, add_pooling_layer=True, **kwargs):
        ...
        self.pooler = (
            TFMobileBertPooler(config, name="pooler")
...
class TFMobileBertForMultipleChoice(
    TFMobileBertPreTrainedModel, TFMultipleChoiceLoss
):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    _keys_to_ignore_on_load_unexpected = [
        r"predictions___cls",

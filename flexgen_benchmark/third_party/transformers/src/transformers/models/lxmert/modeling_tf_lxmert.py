...
class TFLxmertSelfAttentionLayer(tf.keras.layers.Layer):
    ...
    def call(
        self, input_tensor, attention_mask, output_attentions, training=False
    ):
...
class TFLxmertLayer(tf.keras.layers.Layer):
    ...
    def call(
        self, hidden_states, attention_mask, output_attentions, training=False
    ):
...
class TFLxmertEncoder(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.layer = [
            TFLxmertLayer(config, name=f"layer_._{i}")
...
class TFLxmertMainLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        if visual_pos is None or visual_feats is None:
            raise ValueError(
                "visual_feats and visual_pos cannot be `None` in LXMERT's"
                " `call` method."
            )
...
@add_start_docstrings(
    (
        "The bare Lxmert Model transformer outputting raw hidden-states"
...
class TFLxmertForPreTraining(TFLxmertPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.loss_fcts = {
            ...
            "visn_ce": tf.keras.losses.SparseCategoricalCrossentropy(
                from_logits=True
            ),
    ...
    def get_prefix_bias_name(self):
        ...
        return (
            self.name + "/" + self.cls.name + "/" + self.cls.predictions.name
        )
    ...
    def call(
        ...
        if masked_lm_labels is not None and self.task_mask_lm:
            masked_lm_loss = self.loss_fcts["ce"](
                ...
                tf.reshape(
                    lang_prediction_scores, [-1, self.config.vocab_size]
                ),
            )

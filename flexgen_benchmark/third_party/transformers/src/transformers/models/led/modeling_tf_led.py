...
def _make_causal_mask(
    ...
    mask = tf.where(
        mask_cond < tf.reshape(mask_cond + 1, (shape_list(mask)[-1], 1)),
        0.0,
        mask,
    )
...
class TFLEDDecoderAttention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        query_states = tf.reshape(
            self._shape(query_states, tgt_len, bsz), proj_shape
        )
    ...
    def call(
        ...
        tf.debugging.assert_equal(
            ...
            message=(
                ...
                f" {shape_list(residual)} to {shape_list(hidden_states)}"
            ),
        )
...
class TFLEDEncoder(tf.keras.layers.Layer):
    ...
    def call(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            with ContextManagers(context):
                ...
                tf.debugging.assert_less(
                    input_ids,
                    tf.cast(
                        self.embed_tokens.input_dim, dtype=input_ids.dtype
                    ),
                    message=(
                        "input_ids must be smaller than the embedding layer's"
                        " input dimension (got"
                        f" {tf.math.reduce_max(input_ids)} >="
                        f" {self.embed_tokens.input_dim})"
                    ),
                )
        ...
        is_index_global_attn = tf.math.greater(
            tf.cast(attention_mask, tf.int8), 1
        )
...
class TFLEDDecoder(tf.keras.layers.Layer):
    ...
    def call(
        ...
        past_key_values_length = (
            shape_list(past_key_values[0][0])[2]
...
class TFLEDMainLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        encoder_outputs: Optional[
            Union[Tuple, TFLEDEncoderBaseModelOutput]
        ] = None,
        global_attention_mask=None,
        past_key_values=None,
        inputs_embeds=None,
...
@add_start_docstrings(
    (
        "The bare LED Model outputting raw hidden-states without any specific"
        " head on top."
    ),
    LED_START_DOCSTRING,
)
...
class TFLEDForConditionalGeneration(TFLEDPreTrainedModel):
    ...
    def call(
        ...
        past_key_values: Optional[
            Tuple[Tuple[Union[np.ndarray, tf.Tensor]]]
        ] = None,
        ...
        lm_logits = tf.matmul(
            outputs[0], self.led.shared.weights, transpose_b=True
        )

...
class TFRagTokenForGeneration(
    TFRagPreTrainedModel, TFCausalLanguageModelingLoss
):
    ...
    def shift_tokens_right(self, input_ids, start_token_id=None):
        ...
        if start_token_id is None:
            ...
            assert start_token_id is not None, (
                "self.generator.config.decoder_start_token_id has to be"
                " defined. In Rag we commonly use Bart as generator, see Bart"
        ...
        start_tokens = tf.fill(
            ...
            tf.cast(start_token_id, input_ids.dtype),
        )
        ...
        shifted_input_ids = tf.where(
            ...
            tf.fill(
                ...
                tf.cast(pad_token_id, input_ids.dtype),
            ),
            shifted_input_ids,
        )
        ...
        def torch_gather(param, id_tensor):
            ...
            def gather2d(target, id_tensor):
                idx = tf.stack(
                    [
                        tf.range(
                            tf.shape(id_tensor)[0], dtype=id_tensor.dtype
                        ),
                        id_tensor[:, 0],
                    ],
                    axis=-1,
        ...
        for index in range(batch_size):
            ...
            if do_deduplication:
                ...
                output_sequences = tf.stack(
                    list(
                        {
                            str(k.numpy().tolist()): k
                            for k in output_sequences
                        }.values()
                    )
                )

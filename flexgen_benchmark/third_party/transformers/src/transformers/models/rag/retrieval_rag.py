...
class LegacyIndex(Index):
    ...
    def _resolve_path(self, index_path, filename):
        ...
        try:
            ...
            msg = (
                ...
                " is a correct remote path to a directory containing a file"
                f" named {filename}\n\n- or '{index_path}' is the correct path"
                f" to a directory containing a file named {filename}.\n\n"
            )
        ...
        if is_local:
            ...
            logger.info(
                f"loading file {filename} from cache at"
    ...
    def get_top_docs(
        ...
        aux_dim = np.zeros(
            len(question_hidden_states), dtype="float32"
        ).reshape(-1, 1)
...
class HFIndexBase(Index):
    ...
    def _check_dataset_format(self, with_index: bool):
        ...
        if (
            ...
            raise ValueError(
                ...
                " title (str), text (str) and embeddings (arrays of dimension"
                f" vector_size), but got columns {self.dataset.column_names}"
            )
        if with_index and "embeddings" not in self.dataset.list_indexes():
            raise ValueError(
                "Missing faiss index in the dataset. Make sure you called"
                " `dataset.add_faiss_index` to compute it or"
                " `dataset.load_faiss_index` to load one from the disk."
            )
    ...
    def get_doc_dicts(self, doc_ids: np.ndarray) -> List[dict]:
        return [
            self.dataset[doc_ids[i].tolist()] for i in range(doc_ids.shape[0])
        ]
    ...
    def get_top_docs(
        ...
        docs = [
            self.dataset[[i for i in indices if i >= 0]] for indices in ids
        ]
        ...
        for i in range(len(vectors)):
            if len(vectors[i]) < n_docs:
                vectors[i] = np.vstack(
                    [
                        vectors[i],
                        np.zeros((n_docs - len(vectors[i]), self.vector_size)),
                    ]
                )
...
class CanonicalHFIndex(HFIndexBase):
    ...
    def init_index(self):
        if self.index_path is not None:
            ...
            logger.info(
                f"Loading index from {self.dataset_name} with index name"
                f" {self.index_name}"
            )
...
class CustomHFIndex(HFIndexBase):
    ...
    def __init__(self, vector_size: int, dataset, index_path=None):
        super().__init__(
            vector_size, dataset, index_initialized=index_path is None
        )
        self.index_path = index_path
    ...
    def load_from_disk(cls, vector_size, dataset_path, index_path):
        ...
        if dataset_path is None or index_path is None:
            raise ValueError(
                "Please provide `dataset_path` and `index_path` after calling"
                " `dataset.save_to_disk(dataset_path)` and"
                " `dataset.get_index('embeddings').save(index_path)`."
            )
        dataset = load_from_disk(dataset_path)
        return cls(
            vector_size=vector_size, dataset=dataset, index_path=index_path
        )
...
class RagRetriever:
    ...
    def _main_retrieve(
        ...
        for question_hidden_states in question_hidden_states_batched:
            ...
            logger.debug(
                f"index search time: {time.time() - start_time} sec, batch"
                f" size {question_hidden_states.shape}"
            )
            ...
            return BatchEncoding(
                {
                    ...
                    "tokenized_doc_attention_mask": tokenized_docs[
                        "attention_mask"
                    ],
                },

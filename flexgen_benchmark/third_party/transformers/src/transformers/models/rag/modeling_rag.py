        ...
        **kwargs,
        ...
        if question_encoder is None:
            assert (
                ...
                " `question_encoder_pretrained_model_name_or_path` has to be"
                " defined"
            )
        ...
        if generator is None:
            assert generator_pretrained_model_name_or_path is not None, (
                "If `generator_model` is not defined as an argument, a"
                " `generator_pretrained_model_name_or_path` has to be defined"
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert config is not None or (
            ...
            "Either a configuration or an question_encoder and a generator has"
            " to be provided."
        )
        ...
        if self.retriever is not None:
            assert isinstance(retriever, RagRetriever), (
                f"`self.retriever` is of type {type(self.retriever)}, but"
                " should be of type `RagRetriever`"
            )
            self.retriever = retriever
        ...
        if encoder_outputs is None:
            if has_to_retrieve:
                ...
                if self.context_encoder_training:
                    ...
                    retrived_doc_input_ids = retrived_doc_input_ids.to(
                        input_ids
                    )
                    retrived_doc_attention_mask = (
                        retrived_doc_attention_mask.to(input_ids)
                    )
                ...
                assert context_input_ids is not None, (
                    "Make sure that `context_input_ids` are passed, if no"
                    " `retriever` is set. Alternatively, you can set a"
                    " retriever using the `set_retriever(...)` function."
                )
                assert context_attention_mask is not None, (
                    "Make sure that `context_attention_mask` are passed, if no"
                    " `retriever` is set. Alternatively, you can set a"
                    " retriever using the `set_retriever(...)` function."
                )
                assert doc_scores is not None, (
                    "Make sure that `doc_scores` are passed, if no `retriever`"
                    " is set. Alternatively, you can set a retriever using the"
                    " `set_retriever(...)` function."
                )

        assert doc_scores is not None, (
            "Make sure that `doc_scores` are passed when passing"
            " `encoder_outputs` to the forward function."
        )

        assert (doc_scores.shape[1] % n_docs) == 0, (
            " The first dimension of `context_input_ids` should be a multiple"
            f" of `n_docs`={n_docs}, but is {context_input_ids.shape[0]}."
        )
...
class RagSequenceForGeneration(RagPreTrainedModel):
    def __init__(
        ...
        **kwargs,
        ...
        assert config is not None or (
            ...
            "Either a configuration or an encoder and a generator has to be"
            " provided."
        )
        ...
        **model_kwargs,
        ...
        for index in range(batch_size):
            ...
            if input_ids is not None:
                ...
                assert context_attention_mask is not None, (
                    "Make sure that `context_attention_mask` are passed, if no"
                    " `input_ids` is set. Alternatively, you can set a"
                    " retriever using the `set_retriever(...)` function."
                )
                assert doc_scores is not None, (
                    "Make sure that `doc_scores` are passed, if no `input_ids`"
                    " is set. Alternatively, you can set a retriever using the"
                    " `set_retriever(...)` function."
                )
            ...
            top_cand_inds = (-outputs["loss"]).topk(num_doc_return_sequences)[
                1
            ]
    ...
    def get_nll(
        ...
        bos_token_id = (
            self.config.bos_token_id or self.config.generator.bos_token_id
        )
        use_bos = (
            bos_token_id is not None and target[:, 0].eq(bos_token_id).all()
        )
        ...
        doc_logprobs = (
            nn.functional.log_softmax(doc_scores, dim=1)
            .unsqueeze(-1)
            .unsqueeze(-1)
        )
        ...
        rag_logprobs = torch.cat(
            [
                first_token_scores,
                second_token_scores + doc_logprobs,
                remainder,
            ],
            dim=2,
        )
        ...
        ll = (
            ll[:, :, 1:].sum(2) if exclude_bos_score and use_bos else ll.sum(2)
        )
    ...
    def _reorder_cache(past, beam_idx):
        ...
        def _reorder_stacked(hidden_states, new_order):
            ...
            hidden_states = hidden_states.view(
                -1, n_docs, *hidden_states.shape[1:]
            )
        ...
        for layer_past in past:
            ...
            reordered_past += (
                tuple(
                    _reorder_stacked(past_state, beam_idx)
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        if num_beams == 1:
            ...
            raise ValueError(
                "`num_beams` has to be an integer strictly superior to 0 (≥"
                f" 1), but is {num_beams}"
            )

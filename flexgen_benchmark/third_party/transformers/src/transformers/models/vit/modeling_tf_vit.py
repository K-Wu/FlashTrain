        ...
        if not interpolate_pos_encoding:
            if tf.executing_eagerly():
                if height != self.image_size[0] or width != self.image_size[1]:
                    raise ValueError(
                        f"Input image size ({height}*{width}) doesn't match"
                        f" model ({self.image_size[0]}*{self.image_size[1]})."
                    )
...
class TFViTMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        self, config: ViTConfig, add_pooling_layer: bool = True, **kwargs
    ):
        super().__init__(**kwargs)
        ...
        self.pooler = (
            TFViTPooler(config, name="pooler") if add_pooling_layer else None
        )
...
@add_start_docstrings(
    (
        "The bare ViT Model transformer outputting raw hidden-states without"
...
class TFViTModel(TFViTPreTrainedModel):
    def __init__(
        self, config: ViTConfig, *inputs, add_pooling_layer=True, **kwargs
    ):
...
class TFViTForImageClassification(
    TFViTPreTrainedModel, TFSequenceClassificationLoss
):

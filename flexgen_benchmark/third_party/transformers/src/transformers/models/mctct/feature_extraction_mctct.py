...
if not parsed_torchaudio_version_base >= version.parse("0.10"):
    logger.warning(
        f"You are using torchaudio=={torchaudio.__version__}, but"
        " torchaudio>=0.10.0 is required to use MCTCTFeatureExtractor. This"
        " requires torch>=1.10.0. Please upgrade torch and torchaudio."
    )
...
class MCTCTFeatureExtractor(SequenceFeatureExtractor):
    ...
    def _apply_preemphasis_inplace(frames, window_length, preemphasis_coeff):
        if frames.size % window_length != 0:
            raise ValueError(
                "`frames` is supposed to have length divisble by"
                f" `window_length`, but is {frames.size} with"
        ...
        for frame_idx in range(n_frames, 0, -1):
            ...
            frames[start + 1 : end + 1] -= (
                preemphasis_coeff * frames[start:end]
            )
    ...
    def _dft(frames, K, n_frames, n_samples, n_fft):
        ...
        for frame in range(n_frames):
            ...
            inwards_buffer = np.pad(
                inwards_buffer, (0, n_fft - n_samples), "constant"
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if sampling_rate is not None:
            if sampling_rate != self.sampling_rate:
                raise ValueError(
                    ...
                    f" {self} was trained using a sampling rate of"
                    f" {self.sampling_rate}. Please make sure that the"
                    " provided `raw_speech` input was sampled with"
                    f" {self.sampling_rate} and not {sampling_rate}."
                )
        else:
            logger.warning(
                "It is strongly recommended to pass the ``sampling_rate``"
                " argument to this function. Failing to do so can result in"
                " silent errors that might be hard to debug."
            )
        ...
        if is_batched:
            raw_speech = [
                np.asarray(speech, dtype=np.float32) for speech in raw_speech
            ]
        ...
        features = [
            self._extract_mfsc_features(one_waveform)
            for one_waveform in raw_speech
        ]

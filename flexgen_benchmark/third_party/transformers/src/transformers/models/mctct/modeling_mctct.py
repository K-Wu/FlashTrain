...
_CTC_EXPECTED_OUTPUT = (
    "\"Mr. Quilter is the apostle of the middle classes, and we're glad to"
    ' welcome his gospel."'
)
...
class MCTCTConv1dSubsampler(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.in_channels = (
            config.input_feat_per_channel * config.input_channels
        )

        if self.num_layers > 1:
            if config.conv_channels is None:
                raise ValueError(
                    "Need to specify `conv_channels` configuration in"
                    " `MCTCTConfig` to use multiple convolution layers."
                )
    ...
    def forward(self, input_features):
        ...
        padding = sum(
            [size // 2 for size in self.kernel_size]
        )  # (7, 7) -> (3, 3)
...
class MCTCTSelfAttention(nn.Module):
    ...
    def reshape_fortran(self, x, shape):
        ...
        return x.reshape(*reversed(shape)).permute(
    ...
    def relative_position_embedding_rotate(self, scores):
        ...
        scores = torch.cat(
            (
                scores,
                torch.zeros(
                    (batch, seq_len, seq_len, heads), device=scores.device
                ),
            ),
            dim=1,
        )
    ...
    def forward(
        ...
        mixed_query_layer = mixed_query_layer / math.sqrt(
            self.attention_head_size
        )
...
class MCTCTPreTrainedModel(PreTrainedModel):
    ...
    def _get_feat_extract_output_lengths(
        ...
        for _, kernel_sz, stride in zip(
            ...
            padding = kernel_sz // 2
            input_lengths = (
                input_lengths + 2 * padding - dilation * (kernel_sz - 1) - 1
            )
            input_lengths = (
                torch.div(input_lengths, stride, rounding_mode="trunc") + 1
            )
    ...
    def _get_feature_vector_attention_mask(
        ...
        attention_mask[
            (
                torch.arange(bsz, device=attention_mask.device),
                subsampled_lengths - 1,
            )
        ] = 1
...
@add_start_docstrings(
    (
        "The bare M-CTC-T Model transformer outputting raw hidden-states"
    ...
    def __init__(self, config):
        ...
        if config.vocab_size is None:
            raise ValueError(
                ...
                " follows: `MCTCTForCTC.from_pretrained(...,"

...
def rename_keys(s_dict):
    ...
    for key in keys:
        if "transformer_layers" in key:
            s_dict[key.replace("transformer_layers", "layers")] = s_dict.pop(
                key
            )
...
def convert_fairseq_s2t_checkpoint_to_tfms(
    ...
    if len(missing) > 0 and not set(missing) <= set(
        ...
        raise ValueError(
            "Only `encoder.embed_positions.weights` and"
            " `decoder.embed_positions.weights`  are allowed to be missing,"

...
class TFConv1dSubsampler(tf.keras.layers.Layer):
    ...
    def call(self, input_features: tf.Tensor) -> tf.Tensor:
        ...
        for i, conv in enumerate(self.conv_layers):
            ...
            )  # GLU over the Channel dimension
...
class TFSpeech2TextSinusoidalPositionalEmbedding(tf.keras.layers.Layer):
    ...
    def call(
        self, input_ids: tf.Tensor, past_key_values_length: int = 0
    ) -> tf.Tensor:
        ...
        return tf.reshape(
            tf.gather(
                self.embeddings, tf.reshape(position_ids, (-1,)), axis=0
            ),
            (bsz, seq_len, -1),
        )
    ...
    def create_position_ids_from_input_ids(
        ...
        """
        mask = tf.cast(
            tf.math.not_equal(input_ids, padding_idx), dtype=tf.int32
        )
        incremental_indices = (
        ...
        return {
            self.main_input_name: tf.random.uniform(
                [
                    1,
                    random.randint(
                        1, self.config.max_source_positions
                    ),  # time
...
class TFSpeech2TextEncoder(tf.keras.layers.Layer):
    ...
    def _get_feature_vector_attention_mask(
        ...
        attention_mask = tf.cast(
            tf.reverse(
                tf.math.cumsum(tf.reverse(attention_mask, [-1]), -1), [-1]
            ),
            tf.int64,
        )
    ...
    def call(
        ...
        if attention_mask is not None:
            ...
            padding_mask = tf.zeros(
                tf.shape(inputs_embeds)[:-1], dtype=tf.int64
            )
...
class TFSpeech2TextDecoder(tf.keras.layers.Layer):
    ...
    def __init__(self, config: Speech2TextConfig, **kwargs):
        ...
        self.embed_scale = (
            tf.math.sqrt(float(config.d_model))
        ...
        if inputs_embeds is None:
            ...
            tf.debugging.assert_less(
                ...
                message=(
                    ...
                    f" {self.embed_tokens.vocab_size})"
                ),
            )
        ...
        for attn_mask_name, attn_mask in [
            ...
            if attn_mask is not None:
                tf.debugging.assert_equal(
                    ...
                    len(self.layers),
                    message=(
                        f"The {attn_mask_name} should be specified for"
                        f" {len(self.layers)} layers, but it is for"

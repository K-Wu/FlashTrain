...
MAPPING = {
    ...
    "attention.self.query_proj": (
        "encoder.encoder.layer.*.attention.self.query_proj"
    ),
    "attention.self.key_proj": (
        "encoder.encoder.layer.*.attention.self.key_proj"
    ),
    "attention.self.value_proj": (
        "encoder.encoder.layer.*.attention.self.value_proj"
    ),
    "attention.output.dense": "encoder.encoder.layer.*.attention.output.dense",
    "attention.output.LayerNorm": (
        "encoder.encoder.layer.*.attention.output.LayerNorm"
    ),
...
def recursively_load_weights(fairseq_model, hf_model, is_finetuned):
    ...
    feature_extractor = (
        hf_model.sew_d.feature_extractor
        if is_finetuned

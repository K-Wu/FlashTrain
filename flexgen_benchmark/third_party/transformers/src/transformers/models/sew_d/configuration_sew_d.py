...
SEW_D_PRETRAINED_CONFIG_ARCHIVE_MAP = {
    "asapp/sew-d-tiny-100k": (
        "https://huggingface.co/asapp/sew-d-tiny-100k/resolve/main/config.json"
    ),
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            ...
            raise ValueError(
                "Configuration for convolutional layers is incorrect.It is"
                " required that `len(config.conv_dim)` =="
                " `len(config.conv_stride)` == `len(config.conv_kernel)`,but"
                f" is `len(config.conv_dim) = {len(self.conv_dim)}`,"
                f" `len(config.conv_stride)= {len(self.conv_stride)}`,"

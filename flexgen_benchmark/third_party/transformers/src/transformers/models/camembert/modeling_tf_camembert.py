...
class TFCamembertLayer(tf.keras.layers.Layer):
    def __init__(self, config: CamembertConfig, **kwargs):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = TFCamembertAttention(
                config, name="crossattention"
            )
        self.intermediate = TFCamembertIntermediate(
...
class TFCamembertMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, add_pooling_layer=True, **kwargs):
        ...
        self.pooler = (
            TFCamembertPooler(config, name="pooler")
...
class TFCamembertForMaskedLM(
    TFCamembertPreTrainedModel, TFMaskedLanguageModelingLoss
):
...
class TFCamembertForSequenceClassification(
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.classifier = TFCamembertClassificationHead(
...
class TFCamembertForMultipleChoice(
    TFCamembertPreTrainedModel, TFMultipleChoiceLoss
):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    ...
    @unpack_inputs
    @add_start_docstrings_to_model_forward(
        CAMEMBERT_INPUTS_DOCSTRING.format(
...
class TFCamembertForCausalLM(
    TFCamembertPreTrainedModel, TFCausalLanguageModelingLoss
):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    _keys_to_ignore_on_load_unexpected = [r"pooler", r"lm_head.decoder.weight"]
    ...
    def __init__(self, config: CamembertConfig, *inputs, **kwargs):
        ...
        if not config.is_decoder:
            logger.warning(
                "If you want to use `TFCamembertLMHeadModel` as a standalone,"
                " add `is_decoder=True.`"
            )

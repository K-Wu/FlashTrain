...
class Wav2Vec2PhonemeCTCTokenizer(PreTrainedTokenizer):
    ...
    def phonemize(
        self, text: str, phonemizer_lang: Optional[str] = None
    ) -> str:
        ...
        if (
            phonemizer_lang is not None
            and phonemizer_lang != self.phonemizer_lang
        ):
            self.init_backend(phonemizer_lang)
        else:
    ...
    def convert_tokens_to_string(
        ...
        processed_chars = list(
            filter(lambda char: char != self.pad_token, chars)
        )

        # also filter self.word_delimiter_token if not not
        if (
            filter_word_delimiter_token
            and self.word_delimiter_token is not None
        ):
            processed_chars = list(
                filter(
                    lambda token: token != self.word_delimiter_token,
                    processed_chars,
                )
            )
        ...
        if output_char_offsets:
            ...
            if len(char_offsets) != len(processed_chars):
                raise ValueError(
                    f"`char_offsets`: {char_offsets} and `processed_tokens`:"
                    f" {processed_chars} have to be of the same length, but"
                    f" are: `len(offsets)`: {len(char_offsets)} and"
                    f" `len(processed_tokens)`: {len(processed_chars)}"
                )
    ...
    def _compute_offsets(
        ...
        offsets = list(
            filter(lambda offsets: offsets["char"] != ctc_token, offsets)
        )
        ...
        if word_delimiter_token is not None:
            offsets = list(
                filter(
                    lambda offsets: offsets["char"] != word_delimiter_token,
                    offsets,
                )
            )
    ...
    def _add_tokens(
        ...
        for token in new_tokens:
            if not isinstance(token, str):
                raise ValueError(
                    f"Token {token} has to be of type string, but is of type"
                    f" {type(token)}."
                )
        ...
        for token in tokens_to_add:
            if len(token) > 1:
                ...
                _insert_one_token_to_ordered_list(
                    self.unique_no_split_tokens, token
                )

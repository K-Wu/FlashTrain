    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.forced_bos_token_id is None and kwargs.get(
            ...
            warnings.warn(
                "Please make sure the config includes"
                f" `forced_bos_token_id={self.bos_token_id}` in future"
                " versions. The config can simply be saved and uploaded again"
                " to be fixed."
            )
...
class BartOnnxConfig(OnnxSeq2SeqConfigWithPast):
        ...
        if self.task in ["default", "seq2seq-lm"]:
            ...
            if self.use_past:
                self.fill_with_past_key_values_(
                    common_inputs, direction="inputs"
                )
    ...
    def _generate_dummy_inputs_for_sequence_classification_and_question_answering(
        ...
        dummy_input = [
            " ".join([tokenizer.unk_token]) * seq_length
        ] * batch_size
    ...
    def generate_dummy_inputs(
        ...
        if self.task in ["default", "seq2seq-lm"]:
            common_inputs = (
                self._generate_dummy_inputs_for_default_and_seq2seq_lm(
                    tokenizer,

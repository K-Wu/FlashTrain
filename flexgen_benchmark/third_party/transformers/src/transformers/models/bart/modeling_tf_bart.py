...
class TFBartLearnedPositionalEmbedding(tf.keras.layers.Embedding):
    ...
    def call(
        ...
        offset_dtype = (
            position_ids.dtype
            if isinstance(position_ids, tf.Tensor)
            else tf.int32
        )
        return super().call(
            position_ids + tf.constant(self.offset, dtype=offset_dtype)
        )
...
class TFBartDecoder(tf.keras.layers.Layer):
    ...
    def call(
        ...
        if position_ids is None:
            ...
            positions = self.embed_positions(
                input_shape, position_ids=position_ids
            )
...
class TFBartModel(TFBartPretrainedModel):
    ...
    def __init__(
        self, config: BartConfig, load_weight_prefix=None, *inputs, **kwargs
    ):
...
class TFBartForConditionalGeneration(
    ...
    def call(
        ...
        lm_logits = tf.matmul(
            outputs[0], self.model.shared.weights, transpose_b=True
        )

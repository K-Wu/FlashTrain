...
class TransfoXLPreTrainedModel(PreTrainedModel):
    ...
    def _get_new_num_tokens_layer(self, new_num_tokens, layer):
        ...
        new_num_tokens_layer = (
            new_num_tokens
            - sum(
                [emb.weight.shape[0] for emb in embeddings.emb_layers[:layer]]
            )
            - sum(
                [
                    emb.weight.shape[0]
...
class TransfoXLModel(TransfoXLPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            return tuple(
                v
                for v in [core_out, new_mems, hids, attentions]
                if v is not None
            )
...
class TransfoXLLMHeadModel(TransfoXLPreTrainedModel):
    def __init__(self, config):
        ...
        if not self.trainer_compatible:
            warnings.warn(
                (
                    "The output of TransfoXL will be updated in v5 to support"
                    " a single loss as first argument. In orderto use that"
                    " updated output, please specify `trainer_compatible=True`"
                    " as your configuration attribute."
                ),
                DeprecationWarning,
            )

        assert self.sample_softmax <= 0, (
            "Sampling from the softmax is not implemented yet. Please look at"
            " issue: #3310:"
    ...
    def forward(
        ...
        if labels is not None:
            ...
            miss_valid_label = (
                labels[0, 1:].sum() == (labels.size(1) - 1) * -100
            )
            if miss_valid_label:
    ...
    def prepare_inputs_for_generation(
        self, input_ids, past=None, **model_kwargs
    ):
        inputs = {}

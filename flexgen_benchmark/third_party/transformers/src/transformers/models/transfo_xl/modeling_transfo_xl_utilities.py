    ...
    def forward(self, hidden, labels=None, keep_order=False):
        ...
        if labels is not None:
            ...
            if hidden.size(0) != labels.size(0):
                raise RuntimeError(
                    "Input and labels should have the same size in the batch"
                    " dimension."
                )
        else:
            hidden = hidden.view(-1, hidden.size(-1))
            ...
            for i in range(len(cutoff_values) - 1):
                ...
                if i == 0:
                    if labels is not None:
                        ...
                        out[:, : self.cutoffs[0]] = head_logprob[
                            :, : self.cutoffs[0]
                        ]
                else:
                    ...
                    tail_logprob_i = nn.functional.log_softmax(
                        tail_logit_i, dim=1
                    )
                    cluster_prob_idx = (
                        self.cutoffs[0] + i - 1
                    ...
                    if labels is not None:
                        ...
                        logprob_i = (
                            head_logprob[:, cluster_prob_idx, None]
                            + tail_logprob_i
                        )
                ...
                if labels is not None:
                    if (
                        ...
                        out[offset : offset + logprob_i.size(0)].copy_(
                            -logprob_i
                        )

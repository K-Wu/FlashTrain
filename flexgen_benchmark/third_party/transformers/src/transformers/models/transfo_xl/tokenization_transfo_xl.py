...
PRETRAINED_VOCAB_FILES_MAP = {
    "pretrained_vocab_file": {
        "transfo-xl-wt103": (
            "https://huggingface.co/transfo-xl-wt103/resolve/main/vocab.pkl"
        ),
    }
}
...
PRETRAINED_CORPUS_ARCHIVE_MAP = {
    "transfo-xl-wt103": (
        "https://huggingface.co/transfo-xl-wt103/resolve/main/corpus.bin"
    ),
}
...
class TransfoXLTokenizer(PreTrainedTokenizer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        try:
            ...
            if pretrained_vocab_file is not None:
                ...
                if type(vocab_dict) == int:
                    if not is_torch_available():
                        raise ImportError(
                            "Not trying to load dict with PyTorch as you need"
                            " to install pytorch to load from a PyTorch"
                            " pretrained vocabulary, or activate it with"
                            " environment variables USE_TORCH=1 and USE_TF=0."
                        )
            ...
            raise ValueError(
                f"Unable to parse file {pretrained_vocab_file}. Unknown"
                " format. If you tried to load a model saved through"
                " TransfoXLTokenizerFast, please note they are not"
                " compatible."
            ) from e
    ...
    def build_vocab(self):
        if self.vocab_file:
            ...
            logger.info(
                f"building vocab with min_freq={self.min_freq},"
            ...
            logger.info(
                f"final vocab size {len(self)} from {len(self.counter)} unique"
                " tokens"
            )
    ...
    def _convert_token_to_id(self, sym):
        ...
        if sym in self.sym2idx:
            ...
            # Backward compatibility with pre-trained models
                ...
                raise ValueError(
                    "Token not in vocabulary and no <unk> token in vocabulary"
                    " for replacement"
                )
...
class TransfoXLCorpus(object):
    ...
    def from_pretrained(
        ...
        try:
            ...
            logger.error(
                f"Corpus '{pretrained_model_name_or_path}' was not found in"
                " corpus list"
                f" ({', '.join(PRETRAINED_CORPUS_ARCHIVE_MAP.keys())}. We"
                f" assumed '{pretrained_model_name_or_path}' was a path or url"
                f" but couldn't find files {CORPUS_NAME} at this path or url."
            )
            return None
        if is_local:
            ...
            logger.info(
                f"loading corpus file {CORPUS_NAME} from cache at"
                f" {resolved_corpus_file}"
            )

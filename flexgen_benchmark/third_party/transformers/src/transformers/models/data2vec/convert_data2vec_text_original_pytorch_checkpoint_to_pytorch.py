...
def convert_data2vec_checkpoint_to_pytorch(
    ...
    model.data2vec_text.embeddings.token_type_embeddings.weight.data = (
        torch.zeros_like(
    ...
    for i in range(config.num_hidden_layers):
        ...
        data2vec_layer: TransformerSentenceEncoderLayer = (
            data2vec_sent_encoder.layers[i]
        )
        ...
        assert (
            ...
            "Shape for self_output.dense.weight should be"
            f" {data2vec_layer.self_attn.out_proj.weight.shape}"
        )
        ...
        self_output.LayerNorm.weight = (
            data2vec_layer.self_attn_layer_norm.weight
        )
        ...
        assert (
            ...
            "Shape for intermediate.dense.weight should be"
            f" {data2vec_layer.fc1.weight.shape}"
        )
        ...
        assert (
            ...
            "Shape for bert_output.dense.weight should be"
            f" {data2vec_layer.fc2.weight.shape}"
        )
    ...
    if classification_head:
        ...
        # LM Head
        model.lm_head.dense.weight = (
            data2vec_model.encoder.lm_head.dense.weight
        )
        ...
        model.lm_head.layer_norm.bias = (
            data2vec_model.encoder.lm_head.layer_norm.bias
        )

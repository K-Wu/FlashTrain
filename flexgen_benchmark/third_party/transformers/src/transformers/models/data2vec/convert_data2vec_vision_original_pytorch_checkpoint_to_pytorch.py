...
def read_in_q_k_v(
    ...
    for i in range(config.num_hidden_layers):
        ...
        if not has_lm_head:
            ...
            index = state_dict.pop(
                f"{prefix}blocks.{i}.attn.relative_position_index"
            )
...
def get_args():
    parser = argparse.ArgumentParser(
        (
            "Convert Data2VecVision to HF for image classification and"
            " pretraining"
        ),
        add_help=False,
    )
    ...
    parser.add_argument(
        "--beit_checkpoint", default="", help="beit checkpoint"
    )
...
def load_beit_model(args, is_finetuned, is_large):
    def load_state_dict(
        ...
        if len(ignore_missing_keys) > 0:
            print(
                "Ignored weights of {} not initialized from pretrained"
                " model: {}".format(
...
def main():
    ...
    read_in_q_k_v(
        state_dict, config, hf_prefix=hf_prefix, has_lm_head=has_lm_head
    )
    ...
    hf_output = (
        hf_model_output.logits
        if is_finetuned
        else hf_model_output.last_hidden_state
    )

    # 6. Compare
    max_absolute_diff = torch.max(
        torch.abs(hf_output - orig_model_output)
    ).item()

...
class Data2VecAudioEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [
                Data2VecAudioEncoderLayer(config)
...
class Data2VecAudioAdapter(nn.Module):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            Data2VecAudioAdapterLayer(config)
...
class Data2VecAudioPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(
            module, (Data2VecAudioEncoder, Data2VecAudioFeatureEncoder)
        ):
...
@add_start_docstrings(
    (
        "The bare Data2VecAudio Model transformer outputting raw hidden-states"
...
class Data2VecAudioModel(Data2VecAudioPreTrainedModel):
    def __init__(self, config: Data2VecAudioConfig):
        ...
        self.adapter = (
            Data2VecAudioAdapter(config) if config.add_adapter else None
        )
    ...
    def __init__(self, config):
        ...
        if config.vocab_size is None:
            raise ValueError(
                ...
                " follows: `Data2VecAudioForCTC.from_pretrained(...,"
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "add_adapter") and config.add_adapter:
            raise ValueError(
                ...
                " Data2VecAudio adapters (config.add_adapter=True)"
            )
        self.data2vec_audio = Data2VecAudioModel(config)
        num_layers = (

...
class TFData2VecVisionPatchEmbeddings(tf.keras.layers.Layer):
    ...
    def call(
        ...
        return tf.reshape(
            tensor=projection, shape=(batch_size, num_patches, -1)
        )
...
class TFData2VecVisionLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.intermediate = TFData2VecVisionIntermediate(
    ...
    def build(self, input_shape: tf.TensorShape):
        if self.init_values > 0:
            ...
            self.lambda_1.assign(
                self.init_values * tf.ones((self.config.hidden_size))
            )
            self.lambda_2.assign(
                self.init_values * tf.ones((self.config.hidden_size))
            )
        else:
...
class TFData2VecVisionRelativePositionBias(tf.keras.layers.Layer):
    ...
    def get_position_index(self):
        ...
        xx, yy = tf.meshgrid(
            range(self.window_size[0]), range(self.window_size[1])
        )
        ...
        top = tf.ones(
            (1, relative_position_index.shape[1]),
            dtype=relative_position_index.dtype,
        ) * (self.num_relative_distance - 3)
        left = tf.ones(
            (relative_position_index.shape[0], 1),
            dtype=relative_position_index.dtype,
        ...
        relative_position_index = tf.concat(
            [top, relative_position_index], axis=0
        )
        relative_position_index = tf.concat(
        ...
        **kwargs,
        ...
        dpr = [
            ...
            for x in tf.linspace(
                0.0, config.drop_path_rate, config.num_hidden_layers
            )
        ]
        self.layer = [
...
class TFData2VecVisionMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # NCHW output format
        self.pooler = (
            TFData2VecVisionPooler(config, name="pooler")
...
class TFData2VecVisionPooler(tf.keras.layers.Layer):
    ...
    def call(self, hidden_states: tf.Tensor) -> tf.Tensor:
        if self.layernorm is not None:
            ...
            pooled_output = self.layernorm(
                tf.reduce_mean(patch_tokens, axis=1)
            )
        else:
...
@add_start_docstrings(
    (
        "The bare Data2VecVision Model transformer outputting raw"
...
class TFAdaptiveAvgPool2D(tf.keras.layers.Layer):
    def __init__(self, output_shape, mode="dense", **kwargs):
        super().__init__(**kwargs)
        self.mode = mode
        self.h_pool = TFAdaptiveAvgPool1D(
            output_shape[0], mode=mode, name="h_pool"
        )
        self.w_pool = TFAdaptiveAvgPool1D(
            output_shape[1], mode=mode, name="w_pool"
        )
...
class TFData2VecVisionPyramidPoolingModule(tf.keras.layers.Layer):
    ...
    def __init__(
        self, pool_scales: Tuple[int, ...], channels: int, **kwargs
    ) -> None:
        super().__init__(**kwargs)
    ...
    def call(self, encoder_hidden_states: tf.Tensor) -> tf.Tensor:
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            fpn_outs[i] = tf.image.resize(
                fpn_outs[i],
                size=shape_list(fpn_outs[0])[1:-1],
                method="bilinear",
            )
...
class TFData2VecVisionForSemanticSegmentation(TFData2VecVisionPreTrainedModel):
    def __init__(
        self, config: Data2VecVisionConfig, *inputs, **kwargs
    ) -> None:
        super().__init__(config, *inputs, **kwargs)
        self.num_labels = config.num_labels
        self.data2vec_vision = TFData2VecVisionMainLayer(
    ...
    def call(
        ...
        def reshape_features(x):
            x = tf.reshape(
                x, (batch_size, patch_resolution, patch_resolution, -1)
            )
            return x

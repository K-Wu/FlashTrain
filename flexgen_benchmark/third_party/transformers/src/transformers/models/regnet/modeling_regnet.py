...
class RegNetYLayer(nn.Module):
    ...
    def __init__(
        ...
        self.layer = nn.Sequential(
            ...
            RegNetSELayer(
                out_channels, reduced_channels=int(round(in_channels / 4))
            ),
            RegNetConvLayer(
                out_channels, out_channels, kernel_size=1, activation=None
            ),
...
class RegNetStage(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.Sequential(
            ...
            *[
                layer(config, out_channels, out_channels)
...
@add_start_docstrings(
    (
        "The bare RegNet model outputting raw features without any specific"
        " head on top."
    ),

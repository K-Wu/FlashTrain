...
class FlaxVisionTextDualEncoderModule(nn.Module):
    ...
    def setup(self):
        ...
        text_module = FLAX_MODEL_MAPPING[
            self.config.text_config.__class__
        ].module_class
...
class FlaxVisionTextDualEncoderModel(FlaxPreTrainedModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not _do_init:
            raise ValueError(
                "`FlaxVisionTextDualEncoderModel` cannot be created without"
        ...
        **kwargs,
        ...
        if vision_model is None:
            if vision_model_name_or_path is None:
                raise ValueError(
                    "If `vision_model` is not defined as an argument, a"
                    " `vision_model_name_or_path` has to be defined"
                )
        ...
        if text_model is None:
            if text_model_name_or_path is None:
                raise ValueError(
                    "If `text_model` is not defined as an argument, a"
                    " `text_model_name_or_path` has to be defined"
                )
        ...
        logger.warning(
            "The projection layer and logit scale weights"
            " `[('visual_projection', 'kernel'), ('text_projection',"
            " 'kernel'), ('logit_scale',)]` are newly initialized. You should"
            " probably TRAIN this model on a down-stream task to be able to"

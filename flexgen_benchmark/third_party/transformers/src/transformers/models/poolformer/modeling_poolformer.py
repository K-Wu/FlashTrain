...
class PoolFormerEmbeddings(nn.Module):
    ...
    def __init__(
        ...
        stride = (
            stride
            if isinstance(stride, collections.abc.Iterable)
            else (stride, stride)
        )
        padding = (
            padding
...
class PoolFormerLayer(nn.Module):
    ...
    def forward(self, hidden_states):
        if self.use_layer_scale:
            ...
            scaled_op = (
                self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * pooling_output
            )
            ...
            scaled_op = (
                self.layer_scale_2.unsqueeze(-1).unsqueeze(-1) * layer_output
            )
...
@add_start_docstrings(
    (
        "The bare PoolFormer Model transformer outputting raw hidden-states"

...
class LevitResidualLayer(nn.Module):
    ...
    def forward(self, hidden_state):
        if self.training and self.drop_rate > 0:
            rnd = torch.rand(
                hidden_state.size(0), 1, 1, device=hidden_state.device
            )
        ...
        if down_ops[0] == "Subsample":
            ...
            if down_ops[4] > 0:
                ...
                self.layers.append(
                    LevitResidualLayer(
                        LevitMLPLayer(
                            self.config.hidden_sizes[idx + 1], hidden_dim
                        ),
                        self.config.drop_path_rate,
                    )
                )
...
class LevitEncoder(nn.Module):
    ...
    def forward(
        self, hidden_state, output_hidden_states=False, return_dict=True
    ):
        ...
        if not return_dict:
            return tuple(
                v for v in [hidden_state, all_hidden_states] if v is not None
            )
...
@add_start_docstrings(
    (
        "The bare Levit model outputting raw features without any specific"
        " head on top."
    ),
    ...
    def __init__(self, config):
        ...
        self.classifier = (
            LevitClassificationLayer(
                config.hidden_sizes[-1], config.num_labels
            )

...
PRETRAINED_POSITIONAL_EMBEDDINGS_SIZES = {
    "facebook/s2t-wav2vec2-large-en-de": 1024
}
...
class Speech2Text2Tokenizer(PreTrainedTokenizer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if merges_file is None:
            logger.info(
                f"No merges files provided. {self.__class__.__name__} can only"
    ...
    def _tokenize(self, text):
        ...
        if self.bpe_ranks is None:
            raise ValueError(
                "This tokenizer was instantiated without a `merges.txt` file,"
                " so that it can only be used for decoding, not for"
                " encoding.Make sure to provide `merges.txt` file at"
                " instantiation to enable encoding."
            )

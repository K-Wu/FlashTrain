...
if is_scatter_available():
    try:
        ...
        logger.error(
            "TAPAS models are not usable since `torch_scatter` can't be"
            " loaded. It seems you have `torch_scatter` installed with the"
            " wrong CUDA version. Please try to reinstall it following the"
            " instructions here: https://github.com/rusty1s/pytorch_scatter."
        )
...
class TapasEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i, type_vocab_sizes in enumerate(config.type_vocab_sizes):
            ...
            setattr(
                self, name, nn.Embedding(type_vocab_sizes, config.hidden_size)
            )
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        return module(
                            *inputs, past_key_values, output_attentions
                        )
            ...
            if self.config.num_aggregation_labels > 0:
                ...
                if self.config.use_answer_as_supervision:
                    if (
                        ...
                        assert (
                            numeric_values.shape == numeric_values_scale.shape
                        )
...
def flatten(index, name="segmented_flatten"):
    ...
    *num_segments* * (k - 1). The result is a tensor with *num_segments* multiplied by the number of elements in the
    ...
    for _ in range(
        index.batch_dims, len(index.indices.size())
    ):  # typically range(1,2)
...
def range_index_map(batch_shape, num_segments, name="range_index_map"):
    ...
    new_tensor = torch.cat(
        [
            torch.ones_like(
                batch_shape, dtype=torch.long, device=num_segments.device
            ),
...
def _segment_reduce(values, index, segment_reduce_fn, name):
    ...
    vector_shape = values.size()[
        len(index.indices.size()) :
    ]  # torch.Size object
...
def _single_column_cell_selection_loss(
    ...
    labels_per_column, _ = reduce_sum(
        torch.as_tensor(labels, dtype=torch.float32, device=labels.device),
        col_index,
    )
    ...
    labels_per_cell, labels_index = reduce_max(
        torch.as_tensor(labels, dtype=torch.long, device=labels.device),
        cell_index,
    )
    ...
    cell_loss /= (
        torch.sum(column_mask * cell_mask, dim=1) + EPSILON_ZERO_DIVISION
    )
    ...
    selected_column_mask = torch.as_tensor(
        torch.eq(
            column_id_for_cells, torch.unsqueeze(selected_column_id, dim=-1)
        ),
        dtype=torch.float32,
...
def compute_token_logits(
    ...
    """
    logits = (
        torch.einsum("bsj,j->bs", sequence_output, output_weights)
        + output_bias
    ) / temperature
...
def _calculate_aggregate_mask(
    ...
    aggregate_mask_init = (
        torch.logical_not(torch.isnan(answer))
    ...
    aggregate_mask = torch.where(
        torch.logical_and(
            is_pred_cell_selection, is_cell_supervision_available
        ).view(aggregate_mask_init.size()),
...
def _calculate_expected_result(
    ...
    sum_result = torch.sum(
        scaled_probability_per_cell * numeric_values_masked, dim=1
    )
    ...
    if avg_approximation == AverageApproximationFunction.RATIO:
        ...
        raise ValueError(
            "Invalid average_approximation_function:"
            f" {config.average_approximation_function}"
        )
...
def _calculate_regression_loss(
    ...
    # float32 (batch_size,)
    answer_masked = torch.where(
        torch.isnan(answer), torch.zeros_like(answer), answer
    )

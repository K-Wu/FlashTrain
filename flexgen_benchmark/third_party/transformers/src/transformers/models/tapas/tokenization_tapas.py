...
class TapasTokenizer(PreTrainedTokenizer):
    ...
    def __call__(
        ...
        **kwargs
        ...
        assert isinstance(
            table, pd.DataFrame
        ), "Table must be of type pd.DataFrame"
        ...
        if not valid_query:
            raise ValueError(
                "queries input must of type `str` (single example),"
    ...
    def batch_encode_plus(
        ...
        **kwargs
        ...
        if (answer_coordinates and not answer_text) or (
            ...
            raise ValueError(
                "In case you provide answers, both answer_coordinates and"
                " answer_text should be provided"
            )
    ...
    def _get_question_tokens(self, query):
        ...
        if (
            ...
            logger.warning(
                "Skipping query as its tokens are longer than the max question"
                " length"
            )
            return "", []
        if (
            ...
            logger.warning(
                "Skipping query as its tokens are shorter than the min"
                " question length"
            )
            return "", []
        ...
        **kwargs
        ...
        for index, example in enumerate(
            ...
            outputs = self.prepare_for_model(
                ...
                prev_answer_text=answer_text[index - 1]
                if index != 0
                else None,
            )
    ...
    def _encode_plus(
        ...
        **kwargs
        ...
        if query is None:
            query = ""
            logger.warning(
                "TAPAS is a question answering model but you have not passed a"
                " query. Please be aware that the model will probably not"
                " behave correctly."
            )
    ...
    def prepare_for_model(
        ...
        **kwargs
        ...
        if (
            "prev_answer_coordinates" in kwargs
            and "prev_answer_text" in kwargs
        ):
        ...
        table_ids = (
            list(zip(*table_data))[0]
            if len(table_data) > 0
            else list(zip(*table_data))
        )
        ...
        if (
            ...
            raise ValueError(
                "TAPAS does not return overflowing tokens as it works on"
                " tables."
            )
        ...
        if max_length is not None and len(input_ids) > max_length:
            raise ValueError(
                "Could not encode the query and table header given the"
                " maximum length. Encoding the query and table header results"
                f" in a length of {len(input_ids)} which is higher than the"
                f" max_length of {max_length}"
            )
        ...
        if return_special_tokens_mask:
            if add_special_tokens:
                encoded_inputs[
                    "special_tokens_mask"
                ] = self.get_special_tokens_mask(query_ids, table_ids)
            else:
        ...
        if (
            ...
            if not self.deprecation_warnings.get(
                ...
                logger.warning(
                    ...
                    f" ({len(encoded_inputs['input_ids'])} >"
                    f" {self.model_max_length}). Running this sequence through"
    ...
    def _get_table_cost(self, table, num_columns, num_rows, num_tokens):
        return sum(
            1
            for _ in self._get_table_values(
    ...
    def _get_max_num_tokens(
        ...
        if (
            self.cell_trim_length >= 0
            and max_num_tokens > self.cell_trim_length
        ):
    ...
    def _get_cell_token_indexes(self, column_ids, row_ids, column_id, row_id):
        for index in range(len(column_ids)):
            if (
                column_ids[index] - 1 == column_id
                and row_ids[index] - 1 == row_id
            ):
                yield index
    ...
    def _get_numeric_relations(self, question, column_ids, row_ids, table):
        ...
        for (
            column_index,
            row_index,
        ), relations in cell_indices_to_relations.items():
    ...
    def _get_all_answer_ids(self, column_ids, row_ids, answer_coordinates):
        ...
        return self._get_all_answer_ids_from_coordinates(
            column_ids,
            row_ids,
            answers_list=(_to_coordinates(answer_coordinates)),
        )
...
def filter_invalid_unicode_from_table(table):
    ...
    for row_index, row in table.iterrows():
        for col_index, cell in enumerate(row):
            ...
            if is_invalid:
                logging.warning(
                    (
                        "Scrub an invalid table body @ table_id:"
                        f" {table.table_id}, row_index: {row_index},"
    ...
    for col_index, column in enumerate(table.columns):
        ...
        if is_invalid:
            logging.warning(
                f"Scrub an invalid table header @ table_id: {table.table_id},"
                f" col_index: {col_index}"
            )

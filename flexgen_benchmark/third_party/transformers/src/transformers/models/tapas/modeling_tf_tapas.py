...
if is_tensorflow_probability_available():
    try:
        ...
        logger.error(
            "TAPAS models are not usable since `tensorflow_probability` can't"
            " be loaded.It seems you have `tensorflow_probability` installed"
            " with the wrong tensorflow version.Please try to reinstall it"
...
class TFTapasEmbeddings(tf.keras.layers.Layer):
    ...
    def __init__(self, config: TapasConfig, **kwargs):
        ...
        self.reset_position_index_per_cell = (
            config.reset_position_index_per_cell
        )
    ...
    def call(
        ...
        if token_type_ids is None:
            token_type_ids = tf.fill(
                dims=input_shape + [self.number_of_token_type_embeddings],
                value=0,
            )
        ...
        if position_ids is None:
            ...
            position_ids = tf.expand_dims(
                tf.range(start=0, limit=seq_length), axis=0
            )
            ...
            if self.reset_position_index_per_cell:
                ...
                col_index = IndexMap(
                    token_type_ids[:, :, 1],
                    self.type_vocab_sizes[1],
                    batch_dims=1,
                )
                # shape (batch_size, seq_len)
                row_index = IndexMap(
                    token_type_ids[:, :, 2],
                    self.type_vocab_sizes[2],
                    batch_dims=1,
                )
                # shape (batch_size, seq_len)
        ...
        position_embeddings = tf.gather(
            self.position_embeddings, indices=position_ids
        )
...
class TFTapasLayer(tf.keras.layers.Layer):
    def __init__(self, config: TapasConfig, **kwargs):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = TFTapasAttention(
        ...
        if self.is_decoder and encoder_hidden_states is not None:
            if not hasattr(self, "crossattention"):
                raise ValueError(
                    ...
                    " instantiated with cross-attention layers by setting"
...
class TFTapasLMPredictionHead(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs
        ...
        self.transform = TFTapasPredictionHeadTransform(
...
class TFTapasMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        self, config: TapasConfig, add_pooling_layer: bool = True, **kwargs
    ):
...
@add_start_docstrings(
    (
        "The bare Tapas Model transformer outputting raw hidden-states without"
...
class TFTapasForMaskedLM(TFTapasPreTrainedModel, TFMaskedLanguageModelingLoss):
    def __init__(self, config: TapasConfig, *inputs, **kwargs):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `TFTapasForMaskedLM` make sure"
                " `config.is_decoder=False` for bi-directional self-attention."
            )

        self.tapas = TFTapasMainLayer(
            config, add_pooling_layer=False, name="tapas"
        )
            ...
            if is_supervised:
                ...
                if float_answer is not None:
                    assert (
                        ...
                        "Make sure the answers are a FloatTensor of shape"
                        " (batch_size,)"
                    )
                    # <float32>[batch_size]
                    ...
                    aggregate_mask = None
                    raise ValueError(
                        "You have to specify float answers in order to"
                        " calculate the aggregate mask"
                    )
            ...
            if self.config.num_aggregation_labels > 0:
                if is_supervised:
                    ...
                    if aggregation_labels is not None:
                        assert (
                            shape_list(labels)[0]
                            == shape_list(aggregation_labels)[0]
                        ), (
                            "Make sure the aggregation labels are a LongTensor"
                            " of shape (batch_size,)"
                        )
                        ...
                        raise ValueError(
                            "You have to specify aggregation labels in order"
                            " to calculate the aggregation loss"
                        )
                else:
                    aggregation_labels = tf.zeros(
                        shape_list(labels)[0], dtype=tf.int32
                    )
                ...
                if self.config.use_answer_as_supervision:
                    if (
                        ...
                        and numeric_values_scale is not None
                    ):
                        ...
                        raise ValueError(
                            "You have to specify numeric values and numeric"
                            " values scale in order to calculate the"
                            " regression loss"
                        )
...
class ProductIndexMap(IndexMap):
    ...
    def __init__(self, outer_index, inner_index):
        ...
        if outer_index.batch_dims != inner_index.batch_dims:
            raise ValueError(
                "outer_index.batch_dims and inner_index.batch_dims must be the"
                " same."
            )
    ...
    def project_outer(self, index):
        ...
        return IndexMap(
            indices=tf.math.floordiv(
                index.indices, self.inner_index.num_segments
            ),
...
def _single_column_cell_selection_loss(
    ...
    column_label = tf.where(
        no_cell_selected, tf.zeros_like(column_label), column_label
    )
    ...
    labels_per_cell, labels_index = reduce_max(
        tf.cast(labels, tf.int32), cell_index
    )
    ...
    cell_loss /= (
        tf.reduce_sum(column_mask * cell_mask, axis=1) + EPSILON_ZERO_DIVISION
    )
    ...
    selected_column_mask = tf.cast(
        tf.equal(
            column_id_for_cells, tf.expand_dims(selected_column_id, axis=-1)
        ),
        tf.float32,
    )
...
def _calculate_aggregate_mask(
    ...
    aggregate_mask_init = tf.cast(
        tf.logical_not(tf.math.is_nan(answer)), tf.float32
    )
    ...
    is_pred_cell_selection = (
        aggregation_ops_total_mass <= cell_selection_preference
    )
...
def _calculate_expected_result(
    ...
    numeric_values_masked = tf.where(
        tf.math.is_nan(numeric_values),
        tf.zeros_like(numeric_values),
        numeric_values,
    ...
    if avg_approximation == AverageApproximationFunction.RATIO:
        ...
        var = (
            tf.reduce_sum(pointwise_var, axis=1, keepdims=True) - pointwise_var
        )
...
def _calculate_regression_loss(
    ...
    # <float32>[batch_size]
    answer_masked = tf.where(
        tf.math.is_nan(answer), tf.zeros_like(answer), answer
    )

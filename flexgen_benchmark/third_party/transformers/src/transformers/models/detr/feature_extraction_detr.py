...
class DetrFeatureExtractor(
    ...
    def prepare(
        self, image, target, return_segmentation_masks=False, masks_path=None
    ):
        if self.format == "coco_detection":
            ...
            image, target = self.prepare_coco_panoptic(
                image, target, masks_path
            )
            return image, target
        else:
    ...
    def prepare_coco_detection(
        self, image, target, return_segmentation_masks=False
    ):
        """
    ...
    def prepare_coco_panoptic(
        self, image, target, masks_path, return_masks=True
    ):
        w, h = image.size
        ...
        if "segments_info" in ann_info:
            target["iscrowd"] = np.asarray(
                [ann["iscrowd"] for ann in ann_info["segments_info"]],
                dtype=np.int64,
            )
            target["area"] = np.asarray(
                [ann["area"] for ann in ann_info["segments_info"]],
                dtype=np.float32,
            )
    ...
    def _resize(self, image, size, target=None, max_size=None):
        ...
        if "masks" in target:
            ...
            interpolated_masks = (
                nn.functional.interpolate(masks, size=(h, w), mode="nearest")[
                    :, 0
                ]
                > 0.5
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if isinstance(images, (Image.Image, np.ndarray)) or is_torch_tensor(
            images
        ):
            valid_images = True
        ...
        if not valid_images:
            raise ValueError(
                "Images must of type `PIL.Image.Image`, `np.ndarray` or"
                " `torch.Tensor` (single example), `List[PIL.Image.Image]`,"
                " `List[np.ndarray]` or `List[torch.Tensor]` (batch of"
                " examples)."
            )
            ...
            if not is_batched:
                if self.format == "coco_detection":
                    if (
                        ...
                        if isinstance(
                            annotations["annotations"], (list, tuple)
                        ):
                    ...
                    if (
                        ...
                        if isinstance(
                            annotations["segments_info"], (list, tuple)
                        ):
                ...
                if isinstance(annotations, (list, tuple)):
                    if len(images) != len(annotations):
                        raise ValueError(
                            "There must be as many annotations as there are"
                            " images"
                        )
                    if isinstance(annotations[0], Dict):
                        if self.format == "coco_detection":
                            if isinstance(
                                annotations[0]["annotations"], (list, tuple)
                            ):
                                valid_annotations = True
                        elif self.format == "coco_panoptic":
                            if isinstance(
        ...
        if masks_path is not None:
            if self.format == "coco_panoptic":
                ...
                if not valid_masks_path:
                    raise ValueError(
                        "The path to the directory containing the mask PNG"
                        " files should be provided as a `pathlib.Path` object."
                    )
        ...
        if pad_and_return_pixel_mask:
            ...
            max_size = self._max_by_axis(
                [list(image.shape) for image in images]
            )
            c, h, w = max_size
            padded_images = []
            pixel_mask = []
    ...
    def pad_and_create_pixel_mask(
        ...
        max_size = self._max_by_axis(
            [list(image.shape) for image in pixel_values_list]
        )
        c, h, w = max_size
        padded_images = []
        pixel_mask = []
    ...
    def post_process(self, outputs, target_sizes):
        ...
        warnings.warn(
            (
                "`post_process` is deprecated and will be removed in v5 of"
                " Transformers, please use `post_process_object_detection`"
            ),
            FutureWarning,
        )
        ...
        if target_sizes.shape[1] != 2:
            raise ValueError(
                "Each element of target_sizes must contain the size (h, w) of"
    ...
    def post_process_segmentation(
        ...
        warnings.warn(
            (
                "`post_process_segmentation` is deprecated and will be removed"
                " in v5 of Transformers, please use"
        ...
        for cur_logits, cur_masks, size in zip(
            out_logits, raw_masks, target_sizes
        ):
            # we filter empty queries and detection below threshold
            scores, labels = cur_logits.softmax(-1).max(-1)
            keep = labels.ne(outputs.logits.shape[-1] - 1) & (
                scores > threshold
            )
    ...
    def post_process_instance(
        ...
        warnings.warn(
            (
                "`post_process_instance` is deprecated and will be removed in"
                " v5 of Transformers, please use"
        ...
        if len(orig_target_sizes) != len(max_target_sizes):
            raise ValueError(
                "Make sure to pass in as many orig_target_sizes as"
                " max_target_sizes"
            )
    ...
    def post_process_panoptic(
        ...
        warnings.warn(
            (
                "`post_process_panoptic is deprecated and will be removed in"
                " v5 of Transformers, please use"
        ...
        if not len(out_logits) == len(raw_masks) == len(target_sizes):
            raise ValueError(
                "Make sure that you pass in as many target sizes as the batch"
                " dimension of the logits and masks"
            )
        preds = []
        ...
        for cur_logits, cur_masks, cur_boxes, size, target_size in zip(
            ...
            def get_ids_area(masks, scores, dedup=False):
                ...
                seg_img = Image.fromarray(
                    id_to_rgb(m_id.view(h, w).cpu().numpy())
                )
    ...
    def post_process_instance_segmentation(
        ...
        for i in range(batch_size):
            ...
            results.append(
                {"segmentation": segmentation, "segments_info": segments}
            )
        return results

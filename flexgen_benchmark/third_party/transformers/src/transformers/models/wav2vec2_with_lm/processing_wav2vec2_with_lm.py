...
class Wav2Vec2ProcessorWithLM(ProcessorMixin):
    ...
    def __init__(
        ...
        if not isinstance(decoder, BeamSearchDecoderCTC):
            raise ValueError(
                "`decoder` has to be of type"
                f" {BeamSearchDecoderCTC.__class__}, but is {type(decoder)}"
            )
        ...
        if len(missing_decoder_tokens) > 0:
            raise ValueError(
                f"The tokens {missing_decoder_tokens} are defined in the"
                " tokenizer's vocabulary, but not in the decoder's alphabet."
                f" Make sure to include {missing_decoder_tokens} in the"
                " decoder's alphabet."
            )
    ...
    def get_missing_alphabet_tokens(decoder, tokenizer):
        ...
        missing_tokens = set(tokenizer_vocab_list) - set(
    ...
    def __call__(self, *args, **kwargs):
        ...
        if "raw_speech" in kwargs:
            warnings.warn(
                "Using `raw_speech` as a keyword argument is deprecated. Use"
                " `audio` instead."
            )
        ...
        if audio is None and text is None:
            raise ValueError(
                "You need to specify either an `audio` or `text` input to"
                " process."
            )
    ...
    def batch_decode(
        ...
        beam_prune_logp = (
            beam_prune_logp
            if beam_prune_logp is not None
            else DEFAULT_PRUNE_LOGP
        )
        token_min_logp = (
            token_min_logp
            if token_min_logp is not None
            else DEFAULT_MIN_TOKEN_LOGP
        )
        hotword_weight = (
            hotword_weight
            if hotword_weight is not None
            else DEFAULT_HOTWORD_WEIGHT
        )
        ...
        logits_list = [
            array[(array != -100.0).all(axis=-1)] for array in logits
        ]
        ...
        if pool is None:
            ...
            if default_context == "fork":
                ...
                logger.warning(
                    "Parallel batch decoding is not currently supported in"
                    " this platform. Falling back to sequential decoding."
                )
                cm = nullcontext()
        else:
            ...
            if num_processes is not None:
                logger.warning(
                    "Parameter `num_process` was passed, but it will be"
                    " ignored since `pool` was also specified."
                )
    ...
    def as_target_processor(self):
        ...
        """
        warnings.warn(
            "`as_target_processor` is deprecated and will be removed in v5 of"
            " Transformers. You can process your labels by using the argument"
            " `text` of the regular `__call__` method (either in the same call"
            " as your audio inputs, or in a separate call."
        )

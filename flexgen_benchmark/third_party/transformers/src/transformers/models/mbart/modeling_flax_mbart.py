...
def shift_tokens_right(
    input_ids: jnp.ndarray, pad_token_id: int
) -> jnp.ndarray:
    """
    ...
    prev_output_tokens = np.where(
        prev_output_tokens == -100, pad_token_id, input_ids
    )
    index_of_eos = (
    ...
    decoder_start_tokens = np.array(
        [
            prev_output_tokens[i, eos_idx]
            for i, eos_idx in enumerate(index_of_eos)
        ],
        dtype=np.int32,
    ).squeeze()
...
@add_start_docstrings(
    (
        "The bare MBart Model transformer outputting raw hidden-states without"

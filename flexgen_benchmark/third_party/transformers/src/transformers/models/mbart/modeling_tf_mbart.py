...
def shift_tokens_right(input_ids: tf.Tensor, pad_token_id: int):
    ...
    language_id_index = (
        tf.reduce_sum(
            tf.cast(
                tf.math.not_equal(input_ids, pad_token_id),
    ...
    language_id_index = tf.stack(
        [
            tf.range(shape_list(input_ids)[0], dtype=input_ids.dtype),
            language_id_index,
        ],
        axis=-1,
    )
...
@add_start_docstrings(
    (
        "The bare MBART Model outputting raw hidden-states without any"

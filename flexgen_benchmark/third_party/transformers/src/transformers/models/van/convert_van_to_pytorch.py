...
class Tracker:
    ...
    def parametrized(self):
        ...
        return list(
            filter(lambda x: len(list(x.state_dict().keys())) > 0, self.traced)
        )
...
class ModuleTransfer:
    ...
    def __call__(self, x: Tensor):
        ...
        src_traced = list(
            filter(lambda x: type(x) not in self.src_skip, src_traced)
        )
        dest_traced = list(
            filter(lambda x: type(x) not in self.dest_skip, dest_traced)
        )

        if len(dest_traced) != len(src_traced):
            raise Exception(
                "Numbers of operations are different. Source module has"
                f" {len(src_traced)} operations while destination module has"
                f" {len(dest_traced)}."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "The name of the model you wish to convert, it must be one of the"
            " supported resnet* architecture, currently:"
            " van-tiny/small/base/large. If `None`, all of them will the"
            " converted."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "A path to VAN's original implementation directory. You can"
            " download from here:"

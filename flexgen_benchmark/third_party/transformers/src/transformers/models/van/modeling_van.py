...
class VanSpatialAttentionLayer(nn.Module):
    ...
    def __init__(self, hidden_size: int, hidden_act: str = "gelu"):
        ...
        self.post_projection = nn.Conv2d(
...
class VanLayer(nn.Module):
    ...
    def __init__(
        ...
        self.attention = VanSpatialAttentionLayer(
            hidden_size, config.hidden_act
        )
        ...
        self.mlp_scaling = VanLayerScaling(
            hidden_size, config.layer_scale_init_value
        )
...
@add_start_docstrings(
    (
        "The bare VAN model outputting raw features without any specific head"
        " on top. Note, VAN does not have an embedding layer."
    ),
    VAN_START_DOCSTRING,
)

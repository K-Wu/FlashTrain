...
def fftn(x):
    ...
    for axis in reversed(
        range(x.ndim)[1:]
    ):  # We don't need to apply FFT to last axis
    ...
    def _init_fourier_transform(self, config):
        if not config.use_tpu_fourier_optimizations:
            ...
            if is_scipy_available():
                ...
                logging.warning(
                    "SciPy is needed for DFT matrix calculation and is not"
                    " found. Using TPU optimized fast fourier transform"
                    " instead."
                )
...
@add_start_docstrings(
    (
        "The bare FNet Model transformer outputting raw hidden-states without"
...
class FNetModel(FNetPreTrainedModel):
    ...
    def forward(
        ...
        if (
            ...
            raise ValueError(
                "The `tpu_short_seq_length` in FNetConfig should be set equal"
                " to the sequence length being passed to the model when using"
                " TPU optimizations."
            )

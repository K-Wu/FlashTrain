...
class YosoCumulation(torch.autograd.Function):
    ...
    def forward(ctx, query_mask, key_mask, query, key, value, config):
        ...
        expectation = (
            1
            - torch.acos(torch.matmul(query, key.transpose(-1, -2))) / math.pi
        ) ** hash_code_len
        expectation = (
            expectation * query_mask[:, :, None] * key_mask[:, None, :]
        )
        cumulation_value = torch.matmul(expectation, value)

        ctx.save_for_backward(
            query_mask, key_mask, expectation, query, key, value
        )
        ctx.config = config
    ...
    def backward(ctx, grad):
        ...
        weighted_exp = (
            torch.matmul(grad, value.transpose(-1, -2)) * expectation
        )
...
class YosoLSHCumulation(torch.autograd.Function):
    ...
    def forward(ctx, query_mask, key_mask, query, key, value, config):
        if query_mask.size(0) != key_mask.size(0):
            raise ValueError(
                "Query mask and Key mask differ in sizes in dimension 0"
            )
        if query_mask.size(0) != query.size(0):
            raise ValueError(
                "Query mask and Query differ in sizes in dimension 0"
            )
        if query_mask.size(0) != key.size(0):
            raise ValueError(
                "Query mask and Key differ in sizes in dimension 0"
            )
        if query_mask.size(0) != value.size(0):
            raise ValueError(
                "Query mask and Value mask differ in sizes in dimension 0"
            )
    ...
    def backward(ctx, grad):
        ...
        if config["lsh_backward"]:
            ...
            expectation = (
                1
                - torch.acos(torch.matmul(query, key.transpose(-1, -2)))
                / math.pi
            ) ** hash_code_len
            expectation = (
                expectation * query_mask[:, :, None] * key_mask[:, None, :]
...
class YosoSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.use_conv:
            conv_value_layer = self.conv(
                value_layer * attention_mask[:, None, :, None]
            )
...
@add_start_docstrings(
    (
        "The bare YOSO Model transformer outputting raw hidden-states without"

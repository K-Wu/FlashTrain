...
template <typename T>
__device__ void init_buffer(T init_value, T *buffer, int buffer_size,
...
template <typename T>
__device__ void copy_data(T *src_pt, T *dist_pt, int data_length,
                          int num_threads, int thread_id) {
  __syncthreads();
...
template <typename T>
__device__ void init_buffer_nonblocking(T init_value, T *buffer,
                                        int buffer_size, int num_threads,
                                        int thread_id) {
  for (int i = 0; i < buffer_size; i = i + num_threads) {
    int offset_idx = i + thread_id;
    if (offset_idx < buffer_size) {
...
template <typename T>
__device__ void copy_data_nonblocking(T *src_pt, T *dist_pt, int data_length,
                                      int num_threads, int thread_id) {
  for (int i = 0; i < data_length; i = i + num_threads) {
    int offset_idx = i + thread_id;
    if (offset_idx < data_length) {

...
class VideoMAEFeatureExtractor(
    ...
    def __call__(
        ...
        **kwargs
        ...
        if not valid_videos:
            raise ValueError(
                "Videos must of type `List[PIL.Image.Image]`,"
                " `List[np.ndarray]`, `List[torch.Tensor]` (single example),"
                " `List[List[PIL.Image.Image]]`, `List[List[np.ndarray]]`,"
                " `List[List[torch.Tensor]]` (batch of examples)."
            )
        ...
        if self.do_resize and self.size is not None:
            videos = [
                self.resize_video(
                    video, size=self.size, resample=self.resample
                )
                for video in videos
            ]
        if self.do_center_crop and self.size is not None:
            videos = [
                self.crop_video(video, size=self.size) for video in videos
            ]
        if self.do_normalize:
            videos = [
                self.normalize_video(
                    video, mean=self.image_mean, std=self.image_std
                )
                for video in videos
            ]

...
def get_videomae_config(model_name):
    ...
    if "finetuned" in model_name:
        ...
        if "kinetics" in model_name:
            ...
            raise ValueError(
                "Model name should either contain 'kinetics' or 'ssv2' in case"
                " it's fine-tuned."
            )
        id2label = json.load(
...
def rename_key(name):
    ...
    if "patch_embed.proj" in name:
        name = name.replace(
            "patch_embed.proj",
            "videomae.embeddings.patch_embeddings.projection",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "URL of the original PyTorch checkpoint (on Google Drive) you'd"
            " like to convert. Should be a direct download link."
        ),
    )
    parser.add_argument(

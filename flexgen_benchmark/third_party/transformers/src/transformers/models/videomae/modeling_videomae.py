...
@add_start_docstrings(
    (
        "The bare VideoMAE Model transformer outputting raw hidden-states"
...
@add_start_docstrings(
    (
        "The VideoMAE Model transformer with the decoder on top for"
...
class VideoMAEForPreTraining(VideoMAEPreTrainedModel):
    ...
    def forward(
        ...
        expanded_position_embeddings = (
            expanded_position_embeddings.to(pixel_values.device)
            .clone()
            .detach()
        )
        pos_emb_visible = expanded_position_embeddings[
            ~bool_masked_pos
        ].reshape(batch_size, -1, num_channels)
        ...
        x_full = torch.cat(
            [
                sequence_output + pos_emb_visible,
                self.mask_token + pos_emb_mask,
            ],
            dim=1,
        )
        ...
        with torch.no_grad():
            ...
            if self.config.norm_pix_loss:
                ...
                frames_norm = (frames - frames.mean(dim=-2, keepdim=True)) / (
                    frames.var(dim=-2, unbiased=True, keepdim=True).sqrt()
                    + 1e-6
                )
            ...
            labels = videos_patch[bool_masked_pos].reshape(
                batch_size, -1, num_channels
            )

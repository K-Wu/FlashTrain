...
def load_tf_weights_in_canine(model, config, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        ...
        elif name[0] == "final_char_encoder" and name[1] in [
            "LayerNorm",
            "conv",
        ]:
        ...
        for m_name in name:
            if (
                re.fullmatch(r"[A-Za-z]+_\d+", m_name)
            ) and "Embedder" not in m_name:
...
class CanineEmbeddings(nn.Module):
    ...
    def _hash_bucket_tensors(
        self, input_ids, num_hashes: int, num_buckets: int
    ):
        """
    ...
    def _embed_hash_buckets(
        ...
        if embedding_size % num_hashes != 0:
            raise ValueError(
                f"Expected `embedding_size` ({embedding_size}) % `num_hashes`"
                f" ({num_hashes}) == 0"
            )
    ...
    def forward(
        ...
        if final_seq_char_positions is not None:
            ...
            raise NotImplementedError(
                "CanineForMaskedLM is currently not supported"
            )
        else:
...
class CanineAttention(nn.Module):
    ...
    def __init__(
        ...
        if attend_from_chunk_width < attend_from_chunk_stride:
            raise ValueError(
                "`attend_from_chunk_width` < `attend_from_chunk_stride` would"
                " cause sequence positions to get skipped."
            )
        if attend_to_chunk_width < attend_to_chunk_stride:
            raise ValueError(
                "`attend_to_chunk_width` < `attend_to_chunk_stride`would cause"
                " sequence positions to get skipped."
            )
    ...
    def forward(
        ...
        if not self.local:
            ...
            for chunk_start in range(
                0, to_seq_length, self.attend_to_chunk_stride
            ):
                chunk_end = min(
                    to_seq_length, chunk_start + self.attend_to_chunk_width
                )
                to_chunks.append((chunk_start, chunk_end))

            if len(from_chunks) != len(to_chunks):
                raise ValueError(
                    "Expected to have same number of `from_chunks`"
                    f" ({from_chunks}) and `to_chunks` ({from_chunks}). Check"
                    " strides."
                )
            ...
            for (from_start, from_end), (to_start, to_end) in zip(
                ...
                if self.always_attend_to_first_position:
                    cls_attention_mask = attention_mask[
                    ...
                    to_tensor_chunk = torch.cat(
                        [cls_position, to_tensor_chunk], dim=1
                    )
...
class CaninePooler(nn.Module):
    ...
    def forward(
        self, hidden_states: Tuple[torch.FloatTensor]
    ) -> torch.FloatTensor:
...
@add_start_docstrings(
    (
        "The bare CANINE Model transformer outputting raw hidden-states"
...
class CanineModel(CaninePreTrainedModel):
    ...
    def forward(
        ...
        pooled_output = (
            self.pooler(molecule_sequence_output)
        ...
        if not return_dict:
            ...
            output += tuple(
                v
                for v in [all_hidden_states, all_self_attentions]
                if v is not None
            )
            return output
...
class CanineForMultipleChoice(CaninePreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        CANINE_INPUTS_DOCSTRING.format(

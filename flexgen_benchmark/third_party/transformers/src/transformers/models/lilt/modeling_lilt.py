...
class LiltLayoutEmbeddings(nn.Module):
    ...
    def forward(self, bbox=None, position_ids=None):
        ...
        spatial_position_embeddings = self.LayerNorm(
            spatial_position_embeddings
        )
...
class LiltSelfAttention(nn.Module):
    ...
    def forward(
        ...
        tmp_attention_scores = attention_scores / math.sqrt(
            self.attention_head_size
        )
        ...
        layout_attention_scores = (
            tmp_layout_attention_scores + tmp_attention_scores
        )
        ...
        layout_context_layer = torch.matmul(
            layout_attention_probs, layout_value_layer
        )

        layout_context_layer = layout_context_layer.permute(
            0, 2, 1, 3
        ).contiguous()
        new_context_layer_shape = layout_context_layer.size()[:-2] + (
            self.all_head_size // self.channel_shrink_ratio,
        )
        layout_context_layer = layout_context_layer.view(
...
class LiltAttention(nn.Module):
    ...
    def forward(
        ...
        outputs = (
            (attention_output, layout_attention_output),
        ) + self_outputs[
            1:
...
class LiltLayer(nn.Module):
    ...
    def layout_feed_forward_chunk(self, attention_output):
        ...
        layer_output = self.layout_output(
...
@add_start_docstrings(
    (
        "The bare LiLT Model transformer outputting raw hidden-states without"
...
class LiltModel(LiltPreTrainedModel):
    ...
    def forward(
        ...
        if bbox is None:
            bbox = torch.zeros(
                input_shape + (4,), dtype=torch.long, device=device
            )

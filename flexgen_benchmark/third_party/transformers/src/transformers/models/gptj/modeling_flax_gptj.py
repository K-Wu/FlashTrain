...
def create_sinusoidal_positions(num_pos, dim):
    ...
    sinusoid_inp = np.einsum(
        "i , j -> i j", np.arange(num_pos), inv_freq
    ).astype("float32")
...
class FlaxGPTJMLP(nn.Module):
    ...
    def setup(self):
        ...
        self.fc_out = nn.Dense(
            embed_dim, dtype=self.dtype, kernel_init=kernel_init
        )
    ...
    def init_weights(
        ...
        if self.config.add_cross_attention:
            encoder_hidden_states = jnp.zeros(
                input_shape + (self.config.n_embd,)
            )
    ...
    def __call__(
        ...
        if position_ids is None:
            if past_key_values is not None:
                raise ValueError(
                    "Make sure to provide `position_ids` when passing"
                    " `past_key_values`."
                )
...
@add_start_docstrings(
    (
        "The bare GPTJ Model transformer outputting raw hidden-states without"

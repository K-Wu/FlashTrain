...
def create_sinusoidal_positions(num_pos: int, dim: int) -> tf.Tensor:
    inv_freq = tf.cast(
        1.0 / (10000 ** (tf.range(0, dim, 2) / dim)), tf.float32
    )
    sinusoid_inp = tf.cast(
        tf.einsum(
            "i , j -> i j", tf.range(num_pos, dtype=tf.float32), inv_freq
        ),
        tf.float32,
    )
...
class TFGPTJAttention(tf.keras.layers.Layer):
    def __init__(self, config: GPTJConfig, **kwargs):
        ...
        if self.head_dim * self.num_attention_heads != self.embed_dim:
            raise ValueError(
                "embed_dim must be divisible by num_attention_heads (got"
                f" `embed_dim`: {self.embed_dim} and `num_attention_heads`:"
    ...
    def _split_heads(
        self, hidden_states: tf.Tensor, rotary: bool
    ) -> tf.Tensor:
        """
        ...
        raise ValueError(
            ...
            f" {len(shape_list(hidden_states))}"
        )
    ...
    def call(
        ...
        sincos = tf.cast(
            tf.gather(self.embed_positions, position_ids, axis=0),
            hidden_states.dtype,
        )
...
class TFGPTJMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config: GPTJConfig, *inputs, **kwargs):
        ...
        self.h = [
            TFGPTJBlock(config, name=f"h_._{i}") for i in range(config.n_layer)
        ]
    ...
    def call(
        ...
        position_ids = tf.reshape(
            position_ids, [-1, shape_list(position_ids)[-1]]
        )
        ...
        token_type_embeds = tf.cast(
            token_type_embeds, dtype=inputs_embeds.dtype
        )
...
@add_start_docstrings(
    (
        "The bare GPT-J Model transformer outputting raw hidden-states without"
    ...
    def prepare_inputs_for_generation(
        ...
        if attention_mask is not None and position_ids is None:
            position_ids = tf.math.cumsum(
                attention_mask, axis=-1, exclusive=True
            )
            if past:
...
class TFGPTJForQuestionAnswering(
    TFGPTJPreTrainedModel, TFQuestionAnsweringLoss
):
    _keys_to_ignore_on_load_missing = [
        r"h.\d+.attn.masked_bias",
        r"h.\d+.attn.bias",

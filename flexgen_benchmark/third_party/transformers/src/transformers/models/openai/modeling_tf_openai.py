...
class TFAttention(tf.keras.layers.Layer):
    def __init__(self, nx, config, scale=False, **kwargs):
        ...
        assert n_state % config.n_head == 0, (
            f"Hidden dimension {n_state} not dividable by number of heads"
            f" {config.n_head}"
        )
    ...
    def call(
        self, x, attention_mask, head_mask, output_attentions, training=False
    ):
        x = self.c_attn(x)
    ...
    def call(
        ...
        if token_type_ids is not None:
            ...
            tf.debugging.assert_less(
                ...
                message=(
                    "token_type_ids must be smaller than the embedding"
                    " layer's input dimension (got"
                    f" {tf.math.reduce_max(token_type_ids)} >="
                    f" {self.vocab_size})"
                ),
            )
            token_type_embeds = self.tokens_embed(
                token_type_ids, mode="embedding"
            )
        else:
            token_type_embeds = 0
...
class TFOpenAIGPTLMHeadModel(
    TFOpenAIGPTPreTrainedModel, TFCausalLanguageModelingLoss
):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.transformer = TFOpenAIGPTMainLayer(config, name="transformer")

...
def load_tf_weights_in_openai_gpt(
    ...
    if model.tokens_embed.weight.shape != init_params[1].shape:
        raise ValueError(
            "tokens_embed.weight.shape:"
            f" {model.tokens_embed.weight.shape} does not match"
            f" init_param[1].shape: {init_params[1].shape}"
        )

    if model.positions_embed.weight.shape != init_params[0].shape:
        raise ValueError(
            "positions_embed.weight.shape:"
            f" {model.positions_embed.weight.shape} does not match"
            f" init_param[0].shape: {init_params[0].shape}"
        )
...
class Attention(nn.Module):
    def __init__(self, nx, n_positions, config, scale=False):
        ...
        if n_state % config.n_head != 0:
            raise ValueError(
                f"Attention n_state shape: {n_state} must be divisible by"
                f" config.n_head {config.n_head}"
            )
    ...
    def merge_heads(self, x):
        ...
        return x.view(
            *new_x_shape
        )  # in Tensorflow implementation: fct merge_states

    def split_heads(self, x, k=False):
        new_x_shape = x.size()[:-1] + (self.n_head, x.size(-1) // self.n_head)
        x = x.view(
            *new_x_shape
        )  # in Tensorflow implementation: fct split_states
        if k:
    ...
    def forward(
        self, x, attention_mask=None, head_mask=None, output_attentions=False
    ):
        x = self.c_attn(x)
...
class OpenAIGPTDoubleHeadsModel(OpenAIGPTPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(OPENAI_GPT_INPUTS_DOCSTRING)
    @replace_return_docstrings(
        output_type=OpenAIGPTDoubleHeadsModelOutput,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
        self,

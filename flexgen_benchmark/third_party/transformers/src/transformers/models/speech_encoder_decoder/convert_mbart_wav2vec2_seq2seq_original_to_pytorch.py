...
def set_recursively(hf_pointer, key, value, full_name, weight_type):
    ...
    assert hf_shape == value.shape, (
        "Shape of hf"
        f" {key + '.' + weight_type if weight_type is not None else ''} is"
        f" {hf_shape}, but should be {value.shape} for {full_name}"
    )
    ...
    logger.info(
        f"{key + '.' + weight_type if weight_type is not None else ''} was"
        f" initialized from {full_name}."
    )
...
def recursively_load_weights_wav2vec2(fairseq_model, hf_model):
    ...
    for name, value in fairseq_dict.items():
        ...
        if "conv_layers" in name:
            ...
            x in name for x in ["adaptor", "w2v_encoder.proj.", "w2v_proj_ln."]
        ):
            ...
            for key, mapped_key in MAPPING.items():
                if (
                    key in name
                    or key.split("w2v_model.")[-1] == name.split(".")[0]
                ):
                    is_used = True
                    if "*" in mapped_key:
                    ...
                    if "weight_g" in name:
                        ...
                        weight_type = None
                    set_recursively(
                        hf_model, mapped_key, value, name, weight_type
                    )
                continue
        if not is_used:
    ...
    if type_id == 0:
        if "bias" in name:
            assert (
                ...
                f" {feature_extractor.conv_layers[layer_id].conv.bias.data.shape} was"
                " found."
            )
            feature_extractor.conv_layers[layer_id].conv.bias.data = value
            logger.info(
                f"Feat extract conv layer {layer_id} was initialized from"
                f" {full_name}."
            )
        elif "weight" in name:
            assert (
                ...
                f"{full_name} has size {value.shape}, but"
                f" {feature_extractor.conv_layers[layer_id].conv.weight.data.shape} was"
                " found."
            )
        ...
        if "bias" in name:
            assert (
                ...
                f"{full_name} has size {value.shape}, but"
                f" {feature_extractor[layer_id].layer_norm.bias.data.shape} was"
                " found."
            )
            feature_extractor.conv_layers[
                layer_id
            ].layer_norm.bias.data = value
            logger.info(
                f"Feat extract layer norm weight of layer {layer_id} was"
                f" initialized from {full_name}."
            )
        elif "weight" in name:
            assert (
                ...
                f"{full_name} has size {value.shape}, but"
                f" {feature_extractor[layer_id].layer_norm.weight.data.shape} was"
                " found."
            )
            feature_extractor.conv_layers[
                layer_id
            ].layer_norm.weight.data = value
            logger.info(
                f"Feat extract layer norm weight of layer {layer_id} was"
                f" initialized from {full_name}."
            )
    ...
    if "adaptor" not in full_name:
        if "proj_ln" in full_name:
            ...
            if "bias" in name:
                assert (
                    ...
                    f"{full_name} has size {value.shape}, but"
                    f" {adapter.proj_layer_norm.bias.data.shape} was found."
                )
                adapter.proj_layer_norm.bias.data = value
                logger.info(
                    "Adapter proj layer norm bias was initialized from"
                    f" {full_name}."
                )
            if "weight" in name:
                assert (
                    ...
                    f"{full_name} has size {value.shape}, but"
                    f" {adapter.proj_layer_norm.weight.data.shape} was found."
                )
            ...
            if "bias" in name:
                assert value.shape == adapter.proj.bias.data.shape, (
                    f"{full_name} has size {value.shape}, but"
                    f" {adapter.proj.bias.data.shape} was found."
                )
                adapter.proj.bias.data = value
                logger.info(
                    "Adapter proj layer bias was initialized from"
                    f" {full_name}."
                )
            if "weight" in name:
                assert value.shape == adapter.proj.weight.data.shape, (
                    f"{full_name} has size {value.shape}, but"
                    f" {adapter.proj.weight.data.shape} was found."
                )
                adapter.proj.weight.data = value
                logger.info(
                    "Adapter proj layer weight was initialized from"
                    f" {full_name}."
                )
        ...
        if "bias" in name:
            assert (
                ...
                f"{full_name} has size {value.shape}, but"
                f" {adapter.layers[layer_id].conv.bias.data.shape} was found."
            )
            adapter.layers[layer_id].conv.bias.data = value
            logger.info(
                f"Adapter layer {layer_id} bias was initialized from"
                f" {full_name}."
            )
        elif "weight" in name:
            assert (
                ...
                f"{full_name} has size {value.shape}, but"
                f" {adapter.layers[layer_id].conv.weight.data.shape} was"
                " found."
            )
...
def convert_wav2vec2_checkpoint(
    ...
    logger.warning(
        "The following keys are missing when loading the decoder weights:"
        f" {missing_keys}"
    )
    logger.warning(
        "The following keys are unexpected when loading the decoder weights:"
        f" {unexpected_keys}"
    )

    hf_wav2vec = SpeechEncoderDecoderModel(
        encoder=hf_encoder, decoder=hf_decoder
    )

...
class FlaxSpeechEncoderDecoderModule(nn.Module):
    ...
    def setup(self):
        ...
        encoder_module = FLAX_MODEL_MAPPING[
            encoder_config.__class__
        ].module_class
    ...
    def _get_feat_extract_output_lengths(
        ...
        add_adapter = (
            self.config.encoder.add_adapter
...
class FlaxSpeechEncoderDecoderModel(FlaxPreTrainedModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not _do_init:
            raise ValueError(
                "`FlaxSpeechEncoderDecoderModel` cannot be created without"
                " initializing, `_do_init` must be `True`."
            )
        ...
        if input_shape is None:
            ...
            input_shape = (
                (1, encoder_input_length),
                (1, decoder_input_length),
            )
    ...
    def init_weights(
        ...
        if not decoder_batch_size == batch_size:
            raise ValueError(
                "The inputs of encoder and decoder should have the same batch"
                f" size, but got {batch_size} for encoder and"
                f" {decoder_batch_size} for decoder."
            )
    ...
    def decode(
        ...
        if decoder_position_ids is None:
            if past_key_values is not None:
                raise ValueError(
                    "Make sure to provide `decoder_position_ids` when passing"
                    " `past_key_values`."
                )
        ...
        outputs = self.module.apply(
            ...
            encoder_hidden_states=encoder_hidden_states,
            encoder_attention_mask=jnp.array(
                encoder_attention_mask, dtype="i4"
            ),
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
    ...
    def __call__(
        ...
        if decoder_input_ids is None:
            raise ValueError(
                "`decoder_input_ids` cannot be `None`. For sequence to"
                " sequence training, `decoder_position_ids` must be specified"
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        past_key_values = self.init_cache(
            batch_size, max_length, encoder_outputs
        )
    ...
    def from_encoder_decoder_pretrained(
        ...
        decoder_pretrained_model_name_or_path: Optional[
            Union[str, os.PathLike]
        ] = None,
        *model_args,
        **kwargs,

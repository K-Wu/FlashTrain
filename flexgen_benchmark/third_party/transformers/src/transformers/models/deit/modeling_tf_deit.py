...
class TFDeiTEmbeddings(tf.keras.layers.Layer):
    ...
    def call(
        ...
        embeddings = tf.concat(
            (cls_tokens, distillation_tokens, embeddings), axis=1
        )
...
class TFDeiTLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        layer_output = self.layernorm_after(
            inputs=hidden_states, training=training
        )
...
class TFDeiTMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.pooler = (
            TFDeiTPooler(config, name="pooler") if add_pooling_layer else None
        )
...
@add_start_docstrings(
    (
        "The bare DeiT Model transformer outputting raw hidden-states without"
...
class TFDeitDecoder(tf.keras.layers.Layer):
    def __init__(self, config: DeiTConfig, **kwargs) -> None:
        ...
        self.pixel_shuffle = TFDeitPixelShuffle(
...
@add_start_docstrings(
    (
        "DeiT Model with a decoder on top for masked image modeling, as"
...
class TFDeiTForMaskedImageModeling(TFDeiTPreTrainedModel):
    ...
    def call(
        ...
        reconstructed_pixel_values = self.decoder(
...
class TFDeiTForImageClassification(
    TFDeiTPreTrainedModel, TFSequenceClassificationLoss
):
    def __init__(self, config: DeiTConfig):
        ...
        self.deit = TFDeiTMainLayer(
            config, add_pooling_layer=False, name="deit"
        )
    ...
    def __init__(self, config: DeiTConfig) -> None:
        ...
        self.distillation_classifier = (
            tf.keras.layers.Dense(
                config.num_labels, name="distillation_classifier"
            )
            if config.num_labels > 0
            else tf.keras.layers.Activation(
                "linear", name="distillation_classifier"
            )
        )

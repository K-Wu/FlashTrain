...
def load_tf_weights_in_bert_generation(
    ...
    with tf.Session() as sess:
        ...
        for key in list(all_variables.keys()):
            ...
            for i, sub_layer in enumerate(sub_layers):
                ...
                if "layer" in sub_layer:
                    model_pointer = model_pointer.layer[
                        int(sub_layer.split("_")[-1])
                    ]
            ...
            if not is_embedding:
                logger.info(
                    f"Transposing numpy weight of shape {array.shape} for"
                    f" {key}"
                )
            ...
            if model_pointer.shape != array.shape:
                raise ValueError(
                    f"Pointer shape {model_pointer.shape} and array shape"
        ...
        logger.info(
            ...
            f" {', '.join(keep_track_variables.keys())}"
        )
        return model
...
@add_start_docstrings(
    (
        "The bare BertGeneration model transformer outputting raw"
...
class BertGenerationDecoder(BertGenerationPreTrainedModel):
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning(
                "If you want to use `BertGenerationDecoder` as a standalone,"
                " add `is_decoder=True.`"
            )

...
class TFBertTokenizer(tf.keras.layers.Layer):
    ...
    def from_tokenizer(
        ...
        vocab = sorted(
            [(wordpiece, idx) for wordpiece, idx in vocab.items()],
            key=lambda x: x[1],
        )
    ...
    def call(
        ...
        if max_length is not None and text_pair is not None:
            ...
            raise ValueError(
                "max_length cannot be overridden at call time when truncating"
                " paired texts!"
            )
        ...
        if text_pair is not None:
            if text.shape.rank > 1:
                raise ValueError(
                    "text argument should not be multidimensional when a text"
                    " pair is supplied!"
                )

...
class FlaxBertLayerCollection(nn.Module):
    ...
    def setup(self):
        if self.gradient_checkpointing:
            FlaxBertCheckpointLayer = remat(
                FlaxBertLayer, static_argnums=(5, 6, 7)
            )
            self.layers = [
                FlaxBertCheckpointLayer(
...
class FlaxBertLMPredictionHead(nn.Module):
    ...
    def setup(self):
        self.transform = FlaxBertPredictionHeadTransform(
...
class FlaxBertOnlyMLMHead(nn.Module):
    ...
    def setup(self):
        self.predictions = FlaxBertLMPredictionHead(
            self.config, dtype=self.dtype
        )

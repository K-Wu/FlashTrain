...
_TOKEN_CLASS_EXPECTED_OUTPUT = (
    "['O', 'I-ORG', 'I-ORG', 'I-ORG', 'O', 'O', 'O', 'O', 'O', 'I-LOC', 'O',"
    " 'I-LOC', 'I-LOC'] "
)
...
class TFBertPreTrainingLoss:
    ...
    def hf_compute_loss(
        ...
        lm_loss_mask = tf.cast(
            labels["labels"] != -100, dtype=unmasked_lm_losses.dtype
        )
        masked_lm_losses = unmasked_lm_losses * lm_loss_mask
        reduced_masked_lm_loss = tf.reduce_sum(
            masked_lm_losses
        ) / tf.reduce_sum(lm_loss_mask)
        ...
        return tf.reshape(
            reduced_masked_lm_loss + reduced_masked_ns_loss, (1,)
        )
...
class TFBertLayer(tf.keras.layers.Layer):
    def __init__(self, config: BertConfig, **kwargs):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = TFBertAttention(
...
class TFBertMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        self, config: BertConfig, add_pooling_layer: bool = True, **kwargs
    ):
        super().__init__(**kwargs)
        ...
        self.pooler = (
            TFBertPooler(config, name="pooler") if add_pooling_layer else None
        )
...
class TFBertForPreTraining(TFBertPreTrainedModel, TFBertPreTrainingLoss):
    ...
    def get_prefix_bias_name(self) -> str:
        ...
        return (
            self.name + "/" + self.mlm.name + "/" + self.mlm.predictions.name
        )
    ...
    def call(
        ...
        if labels is not None and next_sentence_label is not None:
            ...
            total_loss = self.hf_compute_loss(
                labels=d_labels,
                logits=(prediction_scores, seq_relationship_score),
            )
...
class TFBertForMaskedLM(TFBertPreTrainedModel, TFMaskedLanguageModelingLoss):
    ...
    def __init__(self, config: BertConfig, *inputs, **kwargs):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `TFBertForMaskedLM` make sure"
...
class TFBertLMHeadModel(TFBertPreTrainedModel, TFCausalLanguageModelingLoss):
    ...
    def __init__(self, config: BertConfig, *inputs, **kwargs):
        ...
        if not config.is_decoder:
            logger.warning(
                "If you want to use `TFBertLMHeadModel` as a standalone, add"
                " `is_decoder=True.`"
            )
...
class TFBertForTokenClassification(
    TFBertPreTrainedModel, TFTokenClassificationLoss
):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    _keys_to_ignore_on_load_unexpected = [
        r"pooler",
...
class TFBertForQuestionAnswering(
    TFBertPreTrainedModel, TFQuestionAnsweringLoss
):
    # names with a '.' represents the authorized unexpected/missing layers when a TF model is loaded from a PT model
    _keys_to_ignore_on_load_unexpected = [
        r"pooler",

...
def load_tf2_weights_in_bert(model, tf_checkpoint_path, config):
    ...
    if len(set(layer_depth)) != 1:
        raise ValueError(
            "Found layer names with different depths (layer depth"
            f" {list(set(layer_depth))})"
        )
    layer_depth = list(set(layer_depth))[0]
    if layer_depth != 1:
        raise ValueError(
            "The model contains more than just the embedding/encoder layers."
        ...
        for i, m_name in enumerate(name):
            ...
            if m_name.startswith("layer_with_weights"):
                ...
                if layer_num <= 2:
                    ...
                    layer_num > 3 and layer_num < config.num_hidden_layers + 4
                ):
                    # encoder layers
                ...
                if layer_num == 0:
                    ...
                    raise ValueError(
                        f"Unknown embedding layer with name {full_name}"
                    )
                trace.append("weight")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "The config json file corresponding to the BERT model. This"

...
PRETRAINED_VOCAB_FILES_MAP = {
    ...
    "tokenizer_file": {
        "Salesforce/codegen-350M-mono": "https://huggingface.co/Salesforce/codegen-350M-mono/resolve/main/tokenizer.json",
    },
}
...
class CodeGenTokenizerFast(PreTrainedTokenizerFast):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if kwargs.pop("add_bos_token", False):
            ...
            raise ValueError(
                "Currenty GPT2's fast tokenizer does NOT support adding a BOS"
                " token.Instead you should use GPT2's slow tokenizer class"
                " `CodeGenTokenizer` as follows:"
                f" \n`CodeGenTokenizer.from_pretrained('{model_id}')`\nor\n`AutoTokenizer.from_pretrained('{model_id}',"
                " use_fast=False)`\nThis issue will be fixed soon, see:"
                " https://github.com/huggingface/tokenizers/pull/1005. so"
                " that the fast tokenizer works correctly."
            )
    ...
    def _batch_encode_plus(self, *args, **kwargs) -> BatchEncoding:
        ...
        assert self.add_prefix_space or not is_split_into_words, (
            f"You need to instantiate {self.__class__.__name__} with"
    ...
    def decode(
        ...
        **kwargs,
        ...
        if (
            truncate_before_pattern is not None
            and len(truncate_before_pattern) > 0
        ):

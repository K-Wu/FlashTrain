...
_CTC_EXPECTED_OUTPUT = (
    "'mister quilter is the apposl of the midle classes and weare glad to"
    " welcom his gosepl'"
)
...
@add_start_docstrings(
    (
        "The bare UniSpeech Model transformer outputting raw hidden-states"
...
class UniSpeechForPreTraining(UniSpeechPreTrainedModel):
    def __init__(self, config: UniSpeechConfig):
        ...
        self.project_hid = nn.Linear(
            config.proj_codevector_dim, config.hidden_size
        )
    ...
    def forward(
        ...
        sampled_replace_matrix = (
            torch.bernoulli(prob_replace_matrix)
            .bool()
        ...
        logits = transformer_features.masked_fill(
    ...
    def __init__(self, config):
        ...
        if config.vocab_size is None:
            raise ValueError(
                ...
                " follows: `UniSpeechForCTC.from_pretrained(...,"
...
class UniSpeechForSequenceClassification(UniSpeechPreTrainedModel):
    def __init__(self, config):
        ...
        if hasattr(config, "add_adapter") and config.add_adapter:
            raise ValueError(
                "Sequence classification does not support the use of UniSpeech"

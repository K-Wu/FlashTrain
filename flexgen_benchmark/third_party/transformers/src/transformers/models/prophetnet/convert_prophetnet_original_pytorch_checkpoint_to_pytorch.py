...
def convert_prophetnet_checkpoint_to_pytorch(
    ...
    if "xprophetnet" in prophetnet_checkpoint_path:
        ...
        (
            prophet,
            loading_info,
        ) = XLMProphetNetForConditionalGeneration.from_pretrained(
        ...
        (
            prophet,
            loading_info,
        ) = ProphetNetForConditionalGeneration.from_pretrained(
            prophetnet_checkpoint_path, output_loading_info=True
        )
    ...
    for key in loading_info["missing_keys"]:
        ...
        for attribute in attributes:
            if attribute in mapping:
                ...
                if (
                    not hasattr(old_model, old_attribute)
                    and len(old_attribute) > 0
                ):
            ...
            if attribute == "weight":
                ...
                assert (
                    old_model.bias.shape == model.bias.shape
                ), "Shapes have to match!"
                model.bias = old_model.bias
                logger.info(f"{attribute} is initialized")
                is_key_init = True
                break
            elif attribute in special_keys and hasattr(
                old_model, "in_proj_weight"
            ):

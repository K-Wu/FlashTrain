    ...
    def build(self, input_shape):
        ...
        self.inv_freq.assign(
            ...
            / (
                ...
                ** (
                    tf.range(
                        start=0, limit=self.dim, delta=2, dtype=tf.float32
                    )
                    / self.dim
                )
            )
...
class TFEsmEmbeddings(Layer):
    ...
    def call(
        ...
        if self.token_dropout:
            ...
            src_lengths = tf.cast(
                tf.reduce_sum(attention_mask, axis=-1), tf.float32
            )
    ...
    def call(
        ...
        if (
            ...
            position_ids_l = tf.expand_dims(
                tf.range(seq_length, dtype=tf.int64), -1
            )
            position_ids_r = tf.expand_dims(
                tf.range(seq_length, dtype=tf.int64), 0
            )
...
@add_start_docstrings(
    (
        "The bare ESM Model transformer outputting raw hidden-states without"
...
class TFEsmMainLayer(Layer):
    ...
    def __init__(self, config, add_pooling_layer=True, name=None, **kwargs):
        ...
        self.pooler = (
            TFEsmPooler(config, name="pooler") if add_pooling_layer else None
        )
...
class TFEsmModel(TFEsmPreTrainedModel):
    def __init__(
        self, config: EsmConfig, add_pooling_layer=True, *inputs, **kwargs
    ):
...
class TFEsmForMaskedLM(TFEsmPreTrainedModel, TFMaskedLanguageModelingLoss):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `EsmForMaskedLM` make sure"
...
class TFEsmForTokenClassification(
    TFEsmPreTrainedModel, TFTokenClassificationLoss
):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
...
def create_position_ids_from_input_ids(
    ...
    incremental_indices = (
        tf.cumsum(mask, axis=1) + past_key_values_length
    ) * mask

...
class TFLayoutLMv3PatchEmbeddings(tf.keras.layers.Layer):
    ...
    def __init__(self, config: LayoutLMv3Config, **kwargs):
        ...
        self.num_patches = (config.input_size**2) // (
            patch_sizes[0] * patch_sizes[1]
        )
    ...
    def call(self, pixel_values: tf.Tensor) -> tf.Tensor:
        ...
        embeddings = tf.reshape(
            embeddings, (-1, self.num_patches, self.hidden_size)
        )
        return embeddings
...
class TFLayoutLMv3TextEmbeddings(tf.keras.layers.Layer):
    ...
    def calculate_spatial_position_embeddings(
        self, bbox: tf.Tensor
    ) -> tf.Tensor:
        try:
        ...
        try:
            ...
            raise IndexError(
                "The `bbox` coordinate values should be within"
                f" 0-{self.max_2d_positions} range."
            ) from exception
    ...
    def call(
        ...
        if inputs_embeds is None:
            ...
            tf.debugging.assert_less(
                ...
                message=(
                    ...
                    f" {self.word_embeddings.input_dim})"
                ),
            )
        ...
        spatial_position_embeddings = (
            self.calculate_spatial_position_embeddings(bbox)
        )
...
class TFLayoutLMv3SelfAttention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        attention_scores = tf.matmul(
            normalised_query_layer, transposed_key_layer
        )

        if (
            self.has_relative_attention_bias
            and self.has_spatial_attention_bias
        ):
            attention_scores += (
                rel_pos + rel_2d_pos
            ) / self.attention_score_normaliser
...
class TFLayoutLMv3Layer(tf.keras.layers.Layer):
    def __init__(self, config: LayoutLMv3Config, **kwargs):
        ...
        self.intermediate = TFLayoutLMv3Intermediate(
...
class TFLayoutLMv3Encoder(tf.keras.layers.Layer):
    ...
    def _cal_pos_emb(
        ...
        rel_pos_matrix = tf.expand_dims(
            position_ids, axis=-2
        ) - tf.expand_dims(position_ids, axis=-1)
...
class TFLayoutLMv3MainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config: LayoutLMv3Config, **kwargs):
        ...
        if config.text_embed:
            self.embeddings = TFLayoutLMv3TextEmbeddings(
                config, name="embeddings"
            )

        if config.visual_embed:
            self.patch_embed = TFLayoutLMv3PatchEmbeddings(
                config, name="patch_embed"
            )
            self.LayerNorm = tf.keras.layers.LayerNormalization(
                epsilon=config.layer_norm_eps, name="LayerNorm"
            )
            ...
            if (
                ...
                or config.has_spatial_attention_bias
            ):
    ...
    def init_visual_bbox(
        self, image_size: Tuple[int, int], max_len: int = 1000
    ):
        ...
        cls_token_box = tf.constant(
            [[1, 1, max_len - 1, max_len - 1]], dtype=tf.int32
        )
    ...
    def get_extended_attention_mask(
        ...
        if n_dims == 3:
            ...
            raise ValueError(
                "Wrong shape for attention_mask (shape"
        ...
        extended_attention_mask = tf.cast(
            extended_attention_mask, self.compute_dtype
        )
    ...
    def get_head_mask(
        ...
        if n_dims == 1:
            ...
            raise ValueError(
                f"Wrong shape for head_mask (shape {head_mask.shape})."
            )
        assert (
            tf.rank(head_mask) == 5
    ...
    def call(
        ...
        if input_ids is not None or inputs_embeds is not None:
            if attention_mask is None:
                attention_mask = tf.ones(
                    (batch_size, seq_length), dtype=int_dtype
                )
            if token_type_ids is None:
        ...
        if pixel_values is not None:
            ...
            if (
                ...
                visual_position_ids = tf.expand_dims(
                    visual_position_ids, axis=0
                )
                visual_position_ids = tf.tile(
                    visual_position_ids, [batch_size, 1]
                )
...
class TFLayoutLMv3PreTrainedModel(TFPreTrainedModel):
    ...
    def dummy_inputs(self) -> Dict[str, tf.Tensor]:
        ...
        pixel_values = tf.random.uniform(
            shape=image_shape, minval=-1, maxval=1
        )
        return {
...
@add_start_docstrings(
    (
        "The bare LayoutLMv3 Model transformer outputting raw hidden-states"
...
class TFLayoutLMv3ForSequenceClassification(
    ...
    def __init__(self, config: LayoutLMv3Config, **kwargs):
        ...
        self.config = config
        self.layoutlmv3 = TFLayoutLMv3MainLayer(config, name="layoutlmv3")
        self.classifier = TFLayoutLMv3ClassificationHead(
...
class TFLayoutLMv3ForQuestionAnswering(
    ...
    def __init__(self, config: LayoutLMv3Config, **kwargs):
        ...
        self.layoutlmv3 = TFLayoutLMv3MainLayer(config, name="layoutlmv3")
        self.qa_outputs = TFLayoutLMv3ClassificationHead(
            config, name="qa_outputs"
        )
    ...
    def call(
        ...
        if start_positions is not None and end_positions is not None:
            ...
            loss = self.hf_compute_loss(
                labels, logits=(start_logits, end_logits)
            )

...
def rename_keys(state_dict, architecture):
    for name in list(state_dict):
        ...
        name = name.replace(
            "embed/embeddings", "input_preprocessor.embeddings.weight"
        )
        ...
        name = name.replace(
            "image_preprocessor/~/conv2_d/w",
            "input_preprocessor.convnet_1x1.weight",
        )
        name = name.replace(
            "image_preprocessor/~/conv2_d/b",
            "input_preprocessor.convnet_1x1.bias",
        )
        name = name.replace(
        ...
        name = name.replace(
            "flow_decoder/~/basic_decoder/output/b",
            "decoder.decoder.final_layer.bias",
        )
        # image models
        name = name.replace(
        ...
        name = name.replace(
            "basic_decoder/cross_attention/",
            "decoder.decoding_cross_attention.",
        )
        ...
        name = name.replace(
            "encoder/~/trainable_position_encoding/pos_embs",
            "embeddings.latents",
        )
...
def convert_perceiver_checkpoint(
    ...
    if architecture == "MLM":
        ...
        audio_chunk_size = (
            audio.shape[1] // config.samples_per_patch // nchunks
        )
        # process the first chunk
        chunk_idx = 0
        subsampling = {
            "image": torch.arange(
                image_chunk_size * chunk_idx,
                image_chunk_size * (chunk_idx + 1),
            ),
            "audio": torch.arange(
                audio_chunk_size * chunk_idx,
    ...
    if architecture == "MLM":
        ...
        print(
            "Predicted class:", model.config.id2label[logits.argmax(-1).item()]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Path to local pickle file of a Perceiver checkpoint you'd like to"
            " convert."
        ),
    )
    parser.add_argument(
        ...
        required=True,
        help=(
            "Path to the output PyTorch model directory, provided as a string."
        ),
    )
    parser.add_argument(
        "--architecture",

...
class PerceiverEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.latents = nn.Parameter(
            torch.randn(config.num_latents, config.d_latents)
        )
...
class PerceiverSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        if qk_channels % num_heads != 0:
            raise ValueError(
                f"qk_channels ({qk_channels}) must be divisible by num_heads"
                f" ({num_heads})."
            )
        if v_channels % num_heads != 0:
            raise ValueError(
                f"v_channels ({v_channels}) must be divisible by num_heads"
                f" ({num_heads})."
            )
        ...
        self.layernorm2 = (
            nn.LayerNorm(kv_dim) if is_cross_attention else nn.Identity()
        )
    ...
    def __init__(
        ...
        if is_cross_attention and qk_channels is None:
            if config.cross_attention_shape_for_attention == "q":
                ...
                raise ValueError(
                    "Unknown value"
                    f" {config.cross_attention_shape_for_attention} for"
        ...
        self.output = PerceiverSelfOutput(
            config,
            input_channels=self.self.v_channels,
    ...
    def __init__(self, config, kv_dim=None):
        ...
        if config.d_latents % config.num_cross_attention_heads != 0:
            raise ValueError(
                ...
                f" ({config.num_cross_attention_heads})."
            )
    ...
    def forward(
        ...
        if self.input_preprocessor is not None:
            (
                ...
                modality_sizes,
                inputs_without_pos,
            ) = self.input_preprocessor(inputs)
        else:
            modality_sizes = None
            inputs_without_pos = None
            if inputs.size()[-1] != self.config.d_model:
                raise ValueError(
                    "Last dimension of the inputs:"
                    f" {inputs.size()[-1]} doesn't correspond to"
                    f" config.d_model: {self.config.d_model}. Make sure to set"
                    " config.d_model appropriately."
                )
        ...
        head_mask = self.get_head_mask(
            head_mask,
            self.config.num_blocks * self.config.num_self_attends_per_block,
        )
        ...
        if self.decoder:
            ...
            if (
                ...
                if return_dict:
                    ...
                    encoder_outputs = (
                        encoder_outputs + decoder_outputs.cross_attentions
                    )
        ...
        decoder = PerceiverMultimodalDecoder(
            ...
            modalities={
                "audio": PerceiverBasicDecoder(
                    ...
                    concat_preprocessed_input=False,
                    output_index_dims=(
                        n_audio_samples // config.samples_per_patch,
                    ),
                    output_num_channels=512,
                    use_query_residual=False,
                    position_encoding_only=True,
...
class PerceiverBasicDecoder(PerceiverAbstractDecoder):
    ...
    def num_query_channels(self) -> int:
        if (
            ...
            raise ValueError(
                "You cannot calculate number of decoder query channels when"
                " position_encoding_type is set to none"
            )
    ...
    def decoder_query(
        ...
        if (
            ...
            raise ValueError(
                "You cannot construct decoder queries when"
                " position_encoding_type is set to none"
            )
    ...
    def forward(
        ...
        return PerceiverDecoderOutput(
            logits=logits, cross_attentions=cross_attentions
        )
...
class PerceiverMultimodalDecoder(PerceiverAbstractDecoder):
    ...
    def __init__(
        ...
        subsampled_index_dims: Optional[
    ...
    def num_query_channels(self) -> int:
        max_channel_size = max(
            decoder.num_query_channels
            for _, decoder in self.modalities.items()
        )
    ...
    def decoder_query(
        ...
        def embed(modality, x):
            ...
            pos = torch.broadcast_to(
                pos,
                [x.shape[0], x.shape[1], self.num_query_channels - x.shape[2]],
            )
...
class PerceiverFourierPositionEncoding(PerceiverAbstractPositionEncoding):
    ...
    def __init__(
        self, num_bands, max_resolution, concat_pos=True, sine_only=False
    ):
        super().__init__()
...
class PerceiverEmbeddingDecoder(nn.Module):
    ...
    def forward(
        ...
        # Flatten batch dim
        output = torch.matmul(
            hidden_states.reshape([-1, d_model]),
            embedding_layer.weight.transpose(0, 1),
        )
    ...
    def forward(
        ...
        if not self.input_is_dict:
            ...
            if modality_sizes is None:
                raise ValueError(
                    "Modality sizes should be specified if input is not a"
                    " dictionary."
                )
...
class PerceiverImagePreprocessor(AbstractPreprocessor):
        ...
        **position_encoding_kwargs,
        ...
        if self.prep_type == "conv":
            ...
            if not convnet_num_layers_is_int or temporal_downsample != 1:
                raise ValueError(
                    "Only powers of 4 expected for spatial and 1 expected for"
                    " temporal downsampling with conv."
                )
    ...
    def forward(
        ...
        modality_sizes = (
            None  # Size for each modality, only needed for multimodal
        )
...
class PerceiverAudioPreprocessor(AbstractPreprocessor):
    ...
    def __init__(
        ...
        **position_encoding_kwargs,
        ...
        if concat_or_add_pos not in ["concat", "add"]:
            raise ValueError(
                f"Concat_or_pos {concat_or_add_pos} is invalid, can only be"
                " 'concat' or 'add'."
            )
        ...
        for modality, preprocessor in self.modalities.items():
            ...
            if modality in self.mask_probs:
                ...
                mask = torch.bernoulli(
                    torch.full([batch_size, num_samples], mask_prob)
                )

...
class TapexTokenizer(PreTrainedTokenizer):
    ...
    def __call__(
        ...
        **kwargs
        ...
        if table is not None:
            ...
            raise ValueError(
                "You need to provide either a `table` or an `answer`."
            )
    ...
    def source_call_func(
        ...
        **kwargs
        ...
        if isinstance(table, pd.DataFrame):
            valid_table = True
        elif isinstance(table, (list, tuple)) and isinstance(
        ...
        if not valid_table:
            raise ValueError(
                "table input must of type `pd.DataFrame` (single example),"
                " `List[pd.DataFrame]` (batch of examples). "
            )
        if not valid_query:
            raise ValueError(
                "query input must of type `str` (single example), `List[str]`"
                " (batch of examples). "
            )
    ...
    def _batch_encode_plus(
        ...
        **kwargs
        ...
        if isinstance(table, pd.DataFrame) and isinstance(
            query, (list, tuple)
        ):
    ...
    def prepare_table_query(
        ...
        truncation_strategy=Union[
            str, TruncationStrategy, TapexTruncationStrategy
        ],
        max_length=None,
    ):
        """
        ...
        if linear_table == "":
            logger.warning(
                "You provide an empty table, or all cells contain much tokens"
                " (e.g., >= 1024 tokens). "
                + "Please carefully check the corresponding table with the"
                f" query : {query}."
            )
        if query == "":
            logger.warning(
        ...
        joint_input = (
            (query + separator + linear_table) if query else linear_table
        )
    ...
    def truncate_cell(self, cell_value):
        ...
        if cell_value.strip() != "":
            ...
            if len(try_tokens) >= self.max_cell_length:
                ...
                retain_cell_value = self.convert_tokens_to_string(
                    retain_tokens
                )
    ...
    def truncate_table_rows(
        ...
        self.delete_unrelated_rows(
            table_content, question, answer, delete_ratio
        )
        # guarantee the result < max_length
        maximum_keep_rows = 0
        for ind, row_example in enumerate(table_content["rows"]):
            value_string = self.table_linearize.process_row(
                row_example, ind + 1
            )
    ...
    def estimate_delete_ratio(
        ...
        if "header" not in table_content or "rows" not in table_content:
            raise ValueError(
                "The table content should contain both 'header' and 'rows'"
                " keys."
            )
        ...
        header_string = self.table_linearize.process_header(
        ...
        for _, row_example in enumerate(table_content["rows"]):
            ...
            value_string += (
                self.table_linearize.process_row(row_example, 100) + " "
            )
        value_token_len = len(self.tokenize(value_string))
    ...
    def delete_unrelated_rows(
        ...
        for _row_idx, row in enumerate(table_content["rows"]):
            ...
            if (
                len(lower_row & answer_set) == 0
                and len(lower_row & question_set) == 0
            ):
        ...
        drop_row_indices = random.choices(
            truncated_unrelated_indices, k=drop_items
        )

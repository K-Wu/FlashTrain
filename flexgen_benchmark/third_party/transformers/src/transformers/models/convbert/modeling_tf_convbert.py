...
class TFConvBertSelfAttention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        unfold_conv_out_layer = tf.stack(
            [
                tf.slice(
                    ...
                    [
                        batch_size,
                        shape_list(mixed_query_layer)[1],
                        self.all_head_size,
                    ],
                )
        ...
        context_layer = tf.reshape(
            context_layer,
            (batch_size, -1, self.head_ratio * self.all_head_size),
...
class TFConvBertForMaskedLM(
    TFConvBertPreTrainedModel, TFMaskedLanguageModelingLoss
):
    def __init__(self, config, *inputs, **kwargs):
...
class TFConvBertForMultipleChoice(
    TFConvBertPreTrainedModel, TFMultipleChoiceLoss
):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)

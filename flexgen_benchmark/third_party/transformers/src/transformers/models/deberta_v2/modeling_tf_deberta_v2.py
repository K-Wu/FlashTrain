...
class TFDebertaV2Layer(tf.keras.layers.Layer):
    def __init__(self, config: DebertaV2Config, **kwargs):
        ...
        self.intermediate = TFDebertaV2Intermediate(
...
class TFDebertaV2ConvLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        if input_mask is None:
            ...
            if len(shape_list(input_mask)) != len(
                shape_list(layer_norm_input)
            ):
                if len(shape_list(input_mask)) == 4:
                    input_mask = tf.squeeze(
                        tf.squeeze(input_mask, axis=1), axis=1
                    )
                input_mask = tf.cast(
                    tf.expand_dims(input_mask, axis=2), tf.float32
                )
...
class TFDebertaV2DisentangledSelfAttention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        context_layer = tf.matmul(
            tf.reshape(
                ...
                [
                    -1,
                    shape_list(attention_probs)[-2],
                    shape_list(attention_probs)[-1],
                ],
            ),
            value_layer,
        )
    ...
    def disentangled_att_bias(
        ...
        if "c2p" in self.pos_att_type:
            scale = tf.math.sqrt(
                tf.cast(
                    shape_list(pos_key_layer)[-1] * scale_factor, tf.float32
                )
            )
            c2p_att = tf.matmul(
                query_layer, tf.transpose(pos_key_layer, [0, 2, 1])
            )
        ...
        if "p2c" in self.pos_att_type:
            scale = tf.math.sqrt(
                tf.cast(
                    shape_list(pos_query_layer)[-1] * scale_factor, tf.float32
                )
            )
            ...
            p2c_att = tf.matmul(
                key_layer, tf.transpose(pos_query_layer, [0, 2, 1])
            )
...
class TFDebertaV2LMPredictionHead(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.transform = TFDebertaV2PredictionHeadTransform(
...
class TFDebertaV2ForMaskedLM(
    TFDebertaV2PreTrainedModel, TFMaskedLanguageModelingLoss
):
    def __init__(self, config: DebertaV2Config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `TFDebertaV2ForMaskedLM` make sure"

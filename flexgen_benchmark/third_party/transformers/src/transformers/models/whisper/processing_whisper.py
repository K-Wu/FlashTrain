...
class WhisperProcessor(ProcessorMixin):
    ...
    def get_decoder_prompt_ids(
        self, task=None, language=None, no_timestamps=True
    ):
        ...
        if language is not None:
            if (
                f"<|{language}|>"
                not in self.tokenizer.additional_special_tokens
            ):
                raise ValueError(
                    f"{language} is not supported. The language should be one"
                    " of the following: '<|en|>', '<|zh|>', '<|de|>',"
                    " '<|es|>', '<|ru|>', '<|ko|>', '<|fr|>', '<|ja|>',"
                    " '<|pt|>', '<|tr|>', '<|pl|>', '<|ca|>', '<|nl|>',"
                    " '<|ar|>', '<|sv|>', '<|it|>', '<|id|>', '<|hi|>',"
                    " '<|fi|>', '<|vi|>', '<|iw|>', '<|uk|>', '<|el|>',"
        ...
        if task is not None:
            if f"<|{task}|>" not in self.tokenizer.additional_special_tokens:
                raise ValueError(
                    f"'{task}' is not supported. The language should be in :"
        ...
        forced_decoder_ids = [
            (rank + 1, token) for rank, token in enumerate(ids)
        ]

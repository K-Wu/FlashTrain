    ...
    def __init__(
        ...
        **kwargs
        ...
        self.k_proj = tf.keras.layers.Dense(
            embed_dim, use_bias=False, name="k_proj"
        )
        ...
        return {
            self.main_input_name: tf.random.uniform(
                [
                    ...
                    self.config.max_source_positions * 2 - 1,
                ],
                dtype=tf.float32,
            ),
...
class TFWhisperEncoder(tf.keras.layers.Layer):
    ...
    def __init__(self, config: WhisperConfig, **kwargs):
        ...
        self.embed_scale = (
            math.sqrt(self.embed_dim) if config.scale_embedding else 1.0
        )
        ...
        if head_mask is not None:
            tf.debugging.assert_equal(
                ...
                message=(
                    ...
                    f" {len(self.encoder_layers)} layers, but it is for"
...
class TFWhisperDecoder(tf.keras.layers.Layer):
    ...
    def _prepare_decoder_attention_mask(
        ...
        combined_attention_mask = tf.cond(
            ...
            lambda: _expand_mask(
                tf.ones((batch_size, seq_len + past_key_values_length)),
                tgt_len=seq_len,
            ),
        )
            ...
            if attn_mask is not None:
                tf.debugging.assert_equal(
                    ...
                    message=(
                        ...
                        f" {len(self.decoder_layers)} layers, but it is for"
...
@add_start_docstrings(
    (
        "The bare Whisper Model outputting raw hidden-states without any"

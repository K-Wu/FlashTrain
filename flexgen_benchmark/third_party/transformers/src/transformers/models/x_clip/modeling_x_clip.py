...
class XCLIPOutput(ModelOutput):
    ...
    def to_tuple(self) -> Tuple[Any]:
        return tuple(
            self[k]
            if k
            not in ["text_model_output", "vision_model_output", "mit_output"]
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            module.weight.data.normal_(
                mean=0.0, std=self.config.initializer_factor
            )
...
class XCLIPVisionEncoder(nn.Module):
    ...
    def __init__(self, config: XCLIPConfig):
        ...
        self.layers = nn.ModuleList(
            [
                XCLIPVisionEncoderLayer(config)
                for _ in range(config.num_hidden_layers)
            ]
        )
        self.gradient_checkpointing = False
...
class XCLIPMultiframeIntegrationTransformer(nn.Module):
    ...
    def forward(
        ...
        last_hidden_state = (
            last_hidden_state.type(hidden_states.dtype) + residual
        )
...
class XCLIPModel(XCLIPPreTrainedModel):
    ...
    def __init__(self, config: XCLIPConfig):
        ...
        if not isinstance(config.text_config, XCLIPTextConfig):
            raise ValueError(
                "config.text_config is expected to be of type XCLIPTextConfig"
        ...
        if not isinstance(config.vision_config, XCLIPVisionConfig):
            raise ValueError(
                ...
                " XCLIPVisionConfig but is of type"
    ...
    @replace_return_docstrings(
        output_type=XCLIPOutput, config_class=XCLIPConfig
    )
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        ...
        text_embeds = text_embeds + self.prompts_generator(
            text_embeds, img_features
        )

        # normalized features
        video_embeds = video_embeds / video_embeds.norm(

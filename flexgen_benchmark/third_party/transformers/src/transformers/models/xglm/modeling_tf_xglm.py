...
def create_sinusiodal_positions(
    ...
    emb = tf.reshape(
        tf.concat([tf.sin(emb), tf.cos(emb)], axis=1), (num_positions, -1)
    )
    ...
    if padding_idx is not None:
        _padding_mask = tf.concat(
            [
                ...
                tf.ones(
                    (shape_list(emb)[0] - padding_idx - 1, shape_list(emb)[1])
                ),
            ],
            axis=0,
        )
    ...
    return tf.Variable(
        emb, trainable=False, name="model.embed_positions.weights"
    )
...
def _create_position_ids_from_input_ids(
    ...
    incremental_indices = (
        tf.cast(tf.cumsum(mask, axis=1), dtype=mask.dtype)
...
class TFXGLMMainLayer(tf.keras.layers.Layer):
    ...
    def embed_positions(
        ...
        positions = tf.gather(
            self._embed_positions_weights, position_ids, axis=0
        )
        return positions
...
@add_start_docstrings(
    (
        "The bare XGLM Model transformer outputting raw hidden-states without"
...
class TFXGLMModel(TFXGLMPreTrainedModel):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.model = TFXGLMMainLayer(
            config, embed_tokens=embed_tokens, name="model"
        )

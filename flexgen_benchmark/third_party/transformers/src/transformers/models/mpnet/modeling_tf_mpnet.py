...
class TFMPNetEncoder(tf.keras.layers.Layer):
    ...
    def _relative_position_bucket(
        ...
        num_buckets //= 2
        ret += (
            tf.cast(tf.math.less(n, 0), dtype=relative_position.dtype)
            * num_buckets
        )
        n = tf.math.abs(n)
...
@add_start_docstrings(
    (
        "The bare MPNet Model transformer outputting raw hidden-states without"
...
class TFMPNetForTokenClassification(
    TFMPNetPreTrainedModel, TFTokenClassificationLoss
):
    _keys_to_ignore_on_load_missing = [r"pooler"]
...
class TFMPNetForQuestionAnswering(
    TFMPNetPreTrainedModel, TFQuestionAnsweringLoss
):
    _keys_to_ignore_on_load_missing = [r"pooler"]

...
_CTC_EXPECTED_OUTPUT = (
    "'MISTER QUILDER IS THE APOSTLE OF THE MIDDLE CLASSES AND WE ARE GLAD TO"
    " WELCOME HIS GOSPEL'"
)
...
class UniSpeechSatFeatureEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.feat_extract_norm == "group":
            conv_layers = [
                UniSpeechSatGroupNormConvLayer(config, layer_id=0)
            ] + [
...
class UniSpeechSatEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [
                UniSpeechSatEncoderLayer(config)
...
@add_start_docstrings(
    (
        "The bare UniSpeechSat Model transformer outputting raw hidden-states"
...
class UniSpeechSatForPreTraining(UniSpeechSatPreTrainedModel):
    def __init__(self, config: UniSpeechSatConfig):
        ...
        self.speaker_proj = nn.Linear(
            config.hidden_size, config.codevector_dim
        )
    ...
    @add_start_docstrings_to_model_forward(UNISPEECH_SAT_INPUTS_DOCSTRING)
    @replace_return_docstrings(
        output_type=UniSpeechSatForPreTrainingOutput,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
        self,
    ...
    def __init__(self, config):
        ...
        if config.vocab_size is None:
            raise ValueError(
                ...
                " follows: `UniSpeechSatForCTC.from_pretrained(...,"
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "add_adapter") and config.add_adapter:
            raise ValueError(
                ...
                " UniSpeechSat adapters (config.add_adapter=True)"
            )
        self.unispeech_sat = UniSpeechSatModel(config)
        num_layers = (

...
class XLNetConfig(PretrainedConfig):
        ...
        **kwargs
        ...
        if "d_head" in kwargs:
            if kwargs["d_head"] != d_model // n_head:
                raise ValueError(
                    f"`d_head` ({kwargs['d_head']}) should be equal to"
                    f" `d_model // n_head` ({d_model // n_head})"
                )
        ...
        if "use_cache" in kwargs:
            warnings.warn(
                (
                    ...
                    " removed in a future version, use `use_mems_eval`"
                    " instead."
                ),
                FutureWarning,
            )
    ...
    def max_position_embeddings(self):
        logger.info(
            f"The model {self.model_type} is one of the few models that has no"

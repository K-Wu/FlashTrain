    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        attn_scores = torch.baddbmm(
            ...
            alpha=(
                torch.tensor(
                    1.0,
                    dtype=self.norm_factor.dtype,
                    device=self.norm_factor.device,
                )
                / self.norm_factor
            ),
        )
...
class GPTNeoXJapaneseMLP(nn.Module):
    def __init__(self, config):
        ...
        intermediate_size = int(
            config.hidden_size * config.intermediate_multiple_size
        )
...
class GPTNeoXJapaneseLayer(nn.Module):
    ...
    def forward(
        ...
        attn_output = bias_dropout_add(
            attn_output,
            bias=attn_bias.expand_as(residual)
...
@add_start_docstrings(
    (
        "The bare GPTNeoXJapanese Model transformer outputting raw"
...
class GPTNeoXJapaneseModel(GPTNeoXJapanesePreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        GPT_NEOX_JAPANESE_INPUTS_DOCSTRING.format(

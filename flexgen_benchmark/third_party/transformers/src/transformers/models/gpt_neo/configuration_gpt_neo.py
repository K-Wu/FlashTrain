...
class GPTNeoConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attention_layers = self.expand_attention_types_params(
            attention_types
        )

        if len(self.attention_layers) != self.num_layers:
            raise ValueError(
                "Configuration for convolutional module is incorrect. It is"
                " required that `len(config.attention_layers)` =="
                " `config.num_layers` but is `len(config.attention_layers) ="
                f" {len(self.attention_layers)}`, `config.num_layers ="
                f" {self.num_layers}`. `config.attention_layers` is prepared"
                " using `config.attention_types`. Please verify the value of"
                " `config.attention_types` argument."
            )

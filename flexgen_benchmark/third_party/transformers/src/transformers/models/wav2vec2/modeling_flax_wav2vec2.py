...
def _sample_negative_indices(
    ...
    if sequence_length <= 1:
        raise ValueError(
            ...
            " (batch_size, sequence_length, hidden_size) ="
            f" ({batch_size, sequence_length, hidden_size})."
        )
    ...
    sampled_negative_indices = np.asarray(
        sampled_negative_indices, dtype=np.int32
    )
...
class FlaxConvWithWeightNorm(nn.Module):
    ...
    def setup(self):
        ...
        self.weight_g = self.param(
            "weight_g",
            lambda _: jnp.linalg.norm(self.weight_v, axis=(0, 1))[
                None, None, :
            ],
        )
        self.bias = self.param(
            "bias", jax.nn.initializers.zeros, (self.conv.features,)
        )
        self.prev_padding = self.conv.kernel_size[0] // 2

    def _get_normed_weights(self):
        weight_v_norm = jnp.linalg.norm(self.weight_v, axis=(0, 1))[
            None, None, :
        ]
    ...
    def __call__(self, hidden_states):
        ...
        hidden_states = jnp.pad(
            hidden_states,
            ((0, 0), (self.prev_padding, self.prev_padding), (0, 0)),
        )
...
class FlaxWav2Vec2PositionalConvEmbedding(nn.Module):
    ...
    def setup(self):
        ...
        self.activation = ACT2FN[self.config.feat_extract_activation]
        self.num_pad_remove = (
            1 if self.config.num_conv_pos_embeddings % 2 == 0 else 0
        )
    ...
    def setup(self):
        if self.config.feat_extract_norm == "layer":
            ...
            raise NotImplementedError(
                "At the moment only ``config.feat_extact_norm == 'layer'`` is"
                " supported"
            )
        else:
            raise ValueError(
                "`config.feat_extract_norm` is"
                f" {self.config.feat_extract_norm}, but has to be one of"
                " ['group', 'layer']"
            )
...
class FlaxWav2Vec2FeatureEncoder(nn.Module):
    ...
    def setup(self):
        self.conv_layers = FlaxConvLayersCollection(
    ...
    def __call__(
        ...
        if attention_mask is not None:
            ...
            attention_bias = lax.select(
                ...
                jnp.full(attention_mask.shape, float("-inf")).astype(
                    self.dtype
                ),
            )
        else:
...
class FlaxWav2Vec2FeedForward(nn.Module):
    ...
    def setup(self):
        self.intermediate_dropout = nn.Dropout(
...
class FlaxWav2Vec2EncoderLayerStableLayerNorm(nn.Module):
    ...
    def setup(self):
        ...
        self.feed_forward = FlaxWav2Vec2FeedForward(
            self.config, dtype=self.dtype
        )
    ...
    def __call__(
        ...
        if attention_mask is not None:
            ...
            hidden_states = jnp.where(
                jnp.broadcast_to(
                    attention_mask[:, :, None], hidden_states.shape
                ),
                hidden_states,
                0,
            )
...
class FlaxWav2Vec2GumbelVectorQuantizer(nn.Module):
    ...
    def setup(self):
        ...
        if self.config.codevector_dim % self.num_groups != 0:
            raise ValueError(
                f"`config.codevector_dim {self.config.codevector_dim} must be"
    ...
    def _compute_perplexity(probs, mask=None):
        if mask is not None:
            mask_extended = jnp.broadcast_to(
                mask.flatten()[:, None, None], probs.shape
            )
    ...
    def __call__(
        ...
        if not deterministic:
            ...
            codevector_probs = nn.softmax(
                (hidden_states + gumbels) / temperature
            )
...
class FlaxWav2Vec2Adapter(nn.Module):
    ...
    def setup(self):
        ...
        self.layers = FlaxWav2Vec2AdapterLayersCollection(
            self.config, dtype=self.dtype
        )
...
class FlaxWav2Vec2Module(nn.Module):
    ...
    def setup(self):
        ...
        if self.config.do_stable_layer_norm:
            ...
            raise NotImplementedError(
                "``config.do_stable_layer_norm is False`` is currently not"
                " supported."
            )
    ...
    def __call__(
        ...
        if (
            ...
            hidden_states = jnp.where(
                jnp.broadcast_to(
                    mask_time_indices[:, :, None], hidden_states.shape
                ),
                jnp.broadcast_to(
...
@add_start_docstrings(
    (
        "Wav2Vec2 Model with a `language modeling` head on top for"
...
append_replace_return_docstrings(
    FlaxWav2Vec2ForCTC,
    output_type=FlaxCausalLMOutput,
    config_class=Wav2Vec2Config,
)

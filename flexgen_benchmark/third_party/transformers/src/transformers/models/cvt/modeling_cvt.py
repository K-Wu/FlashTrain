...
class CvtConvEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values):
        ...
        pixel_values = pixel_values.view(
            batch_size, num_channels, hidden_size
        ).permute(0, 2, 1)
...
class CvtSelfAttention(nn.Module):
    ...
    def forward(self, hidden_state, height, width):
        if self.with_cls_token:
            cls_token, hidden_state = torch.split(
        ...
        attention_score = (
            torch.einsum("bhlk,bhtk->bhlt", [query, key]) * self.scale
        )
        ...
        self.layers = nn.Sequential(
            *[
                CvtLayer(
                    ...
                    qkv_projection_method=config.qkv_projection_method[
                        self.stage
                    ],

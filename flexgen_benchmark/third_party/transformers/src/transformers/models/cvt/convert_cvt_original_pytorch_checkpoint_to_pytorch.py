...
def convert_cvt_checkpoint(
    cvt_model, image_size, cvt_file_name, pytorch_dump_folder
):
    """
    ...
    id2label = json.load(
        open(
            cached_download(
                hf_hub_url(repo_id, img_labels_file, repo_type="dataset")
            ),
            "r",
        )
    )
    ...
    original_weights = torch.load(
        cvt_file_name, map_location=torch.device("cpu")
    )

...
class TFCvtSelfAttentionConvProjection(tf.keras.layers.Layer):
    ...
    def call(
        self, hidden_state: tf.Tensor, training: bool = False
    ) -> tf.Tensor:
...
class TFCvtSelfAttentionProjection(tf.keras.layers.Layer):
    ...
    def call(
        ...
        hidden_state = self.convolution_projection(
            hidden_state, training=training
        )
...
class TFCvtSelfAttention(tf.keras.layers.Layer):
    ...
    def rearrange_for_multi_head_attention(
        ...
        hidden_state = tf.reshape(
            hidden_state,
            shape=(batch_size, hidden_size, self.num_heads, head_dim),
        )
    ...
    def call(
        ...
        if self.with_cls_token:
            cls_token, hidden_state = tf.split(
                hidden_state, [1, height * width], 1
            )
        ...
        query = self.convolution_projection_query(
            hidden_state, training=training
        )
        value = self.convolution_projection_value(
            hidden_state, training=training
        )
        ...
        query = self.rearrange_for_multi_head_attention(
            self.projection_query(query)
        )
        key = self.rearrange_for_multi_head_attention(self.projection_key(key))
        value = self.rearrange_for_multi_head_attention(
...
class TFCvtAttention(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dense_output = TFCvtSelfOutput(
            config, embed_dim, drop_rate, name="output"
        )
    ...
    def call(
        ...
        self_output = self.attention(
            hidden_state, height, width, training=training
        )
...
class TFCvtOutput(tf.keras.layers.Layer):
    ...
    def __init__(
        self, config: CvtConfig, embed_dim: int, drop_rate: int, **kwargs
    ):
        super().__init__(**kwargs)
        self.dense = tf.keras.layers.Dense(
            units=embed_dim,
...
class TFCvtEncoder(tf.keras.layers.Layer):
    ...
    def call(
        ...
        for _, (stage_module) in enumerate(self.stages):
            hidden_state, cls_token = stage_module(
                hidden_state, training=training
            )
        ...
        if not return_dict:
            return tuple(
                v
                for v in [hidden_state, cls_token, all_hidden_states]
                if v is not None
            )
...
@add_start_docstrings(
    (
        "The bare Cvt Model transformer outputting raw hidden-states without"
...
class TFCvtForImageClassification(
    TFCvtPreTrainedModel, TFSequenceClassificationLoss
):
    def __init__(self, config: CvtConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
    ...
    def call(
        ...
        if self.config.cls_token[-1]:
            ...
            batch_size, num_channels, height, width = shape_list(
                sequence_output
            )

...
class BeitSelfAttention(nn.Module):
    def __init__(
        self, config: BeitConfig, window_size: Optional[tuple] = None
    ) -> None:
        super().__init__()
        if (
...
class BeitModel(BeitPreTrainedModel):
    def __init__(
        self, config: BeitConfig, add_pooling_layer: bool = True
    ) -> None:
    ...
    def forward(
        ...
        if labels is not None:
            ...
            masked_lm_loss = loss_fct(
                prediction_scores[bool_masked_pos], labels
            )
...
class BeitForSemanticSegmentation(BeitPreTrainedModel):
    def __init__(self, config: BeitConfig) -> None:
        ...
        self.auxiliary_head = (
            BeitFCNHead(config) if config.use_auxiliary_head else None
        )

...
class FlaxBeitEmbeddings(nn.Module):
    ...
    def setup(self):
        ...
        self.patch_embeddings = FlaxBeitPatchEmbeddings(
    ...
    def __call__(self, pixel_values, bool_masked_pos=None, deterministic=True):
        ...
        if self.config.use_absolute_position_embeddings:
            embeddings = embeddings + self.position_embeddings.astype(
                embeddings.dtype
            )
...
class FlaxBeitRelativePositionBias(nn.Module):
    ...
    def setup(self):
        ...
        self.relative_position_index = relative_position_index_init(
            self.window_size
        )
...
class FlaxBeitSelfAttention(nn.Module):
    ...
    def setup(self):
        if (
            ...
            raise ValueError(
                f"The hidden size {self.config.hidden_size,} is not a multiple"
...
class FlaxBeitLayer(nn.Module):
    ...
    def __call__(
        ...
        if self.lambda_2 is not None:
            layer_output = (
                self.lambda_2.astype(layer_output.dtype) * layer_output
            )

        # second residual connection
        layer_output = (
            self.drop_path(layer_output, deterministic=deterministic)
            + hidden_states
        )
...
class FlaxBeitPreTrainedModel(FlaxPreTrainedModel):
    ...
    def init_weights(
        ...
        random_params = self.module.init(
            rngs, pixel_values, return_dict=False
        )["params"]
...
@add_start_docstrings(
    (
        "The bare Beit Model transformer outputting raw hidden-states without"
...
append_replace_return_docstrings(
    FlaxBeitModel,
    output_type=FlaxBeitModelOutputWithPooling,
    config_class=BeitConfig,
)
...
@add_start_docstrings(
    (
        "Beit Model transformer with a 'language' modeling head on top (to"
...
overwrite_call_docstring(
    FlaxBeitForMaskedImageModeling, FLAX_BEIT_MLM_DOCSTRING
)
append_replace_return_docstrings(
...
overwrite_call_docstring(
    FlaxBeitForImageClassification, FLAX_BEIT_CLASSIF_DOCSTRING
)
append_replace_return_docstrings(

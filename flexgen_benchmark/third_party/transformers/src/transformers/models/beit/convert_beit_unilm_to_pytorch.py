...
def read_in_q_k_v(state_dict, config, has_lm_head=False, is_semantic=False):
    for i in range(config.num_hidden_layers):
        ...
        state_dict[
            f"beit.encoder.layer.{i}.attention.attention.query.bias"
        ] = q_bias
        state_dict[
        ...
        state_dict[
            f"beit.encoder.layer.{i}.attention.attention.value.bias"
        ] = v_bias
...
def convert_beit_checkpoint(checkpoint_url, pytorch_dump_folder_path):
    ...
    if checkpoint_url[-9:-4] == "pt22k":
        ...
        raise ValueError(
            "Checkpoint not supported, URL should either end with 'pt22k',"
            " 'ft22k', 'to1k' or 'ade20k'"
        )
    ...
    read_in_q_k_v(
        state_dict, config, has_lm_head=has_lm_head, is_semantic=is_semantic
    )
    if is_semantic:

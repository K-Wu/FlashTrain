...
@add_start_docstrings(
    (
        "The bare Splinter Model transformer outputting raw hidden-states"
...
class QuestionAwareSpanSelectionHead(nn.Module):
    ...
    def forward(self, inputs, positions):
        ...
        start_reps = self.start_transform(
            inputs
...
class SplinterForQuestionAnswering(SplinterPreTrainedModel):
    ...
    def forward(
        ...
        if question_positions is None:
            ...
            question_positions = question_position_for_each_example.unsqueeze(
                -1
            )
        ...
        if attention_mask is not None:
            ...
            end_logits = (
                end_logits
                + (1 - attention_mask) * torch.finfo(end_logits.dtype).min
            )
...
class SplinterForPreTraining(SplinterPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        SPLINTER_INPUTS_DOCSTRING.format(
    ...
    def forward(
        ...
        if (
            ...
            raise TypeError(
                "question_positions must be specified in order to calculate"
                " the loss"
            )

        elif question_positions is None and input_ids is None:
            raise TypeError(
                "question_positions must be specified when input_embeds is"
                " used"
            )
        ...
        if attention_mask is not None:
            attention_mask_for_each_question = attention_mask.unsqueeze(
                1
            ).expand(batch_size, num_questions, sequence_length)
            start_logits = (
                start_logits
    ...
    def _prepare_question_positions(
        ...
        rows, flat_positions = torch.where(
            input_ids == self.config.question_token_id
        )

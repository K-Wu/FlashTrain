...
class YolosEmbeddings(nn.Module):
    ...
    def __init__(self, config: YolosConfig) -> None:
        ...
        self.position_embeddings = nn.Parameter(
            torch.zeros(
                1,
                num_patches + config.num_detection_tokens + 1,
                config.hidden_size,
            )
        )
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        embeddings = torch.cat(
            (cls_tokens, embeddings, detection_tokens), dim=1
        )
...
class InterpolateInitialPositionEmbeddings(nn.Module):
    ...
    def forward(self, pos_embed, img_size=(800, 1344)) -> torch.Tensor:
        ...
        patch_pos_embed = pos_embed[
            :, 1 : -self.config.num_detection_tokens, :
        ]
...
@add_start_docstrings(
    (
        "The bare YOLOS Model transformer outputting raw hidden-states without"
...
class YolosForObjectDetection(YolosPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = sequence_output[
            :, -self.config.num_detection_tokens :, :
        ]

...
def read_in_q_k_v(
    state_dict: dict, config: YolosConfig, base_model: bool = False
):
...
def convert_state_dict(
    orig_state_dict: dict, model: YolosForObjectDetection
) -> dict:
...
def convert_yolos_checkpoint(
    ...
    feature_extractor = YolosFeatureExtractor(
        format="coco_detection", size=size
    )
    ...
    assert torch.allclose(
        pred_boxes[0, :3, :3], expected_slice_boxes, atol=1e-4
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Name of the YOLOS model you'd like to convert. Should be one of"
            " 'yolos_ti', 'yolos_s_200_pre', 'yolos_s_300_pre', 'yolos_s_dWr',"
            " 'yolos_base'."
        ),
    )
    parser.add_argument(

...
@torch.no_grad()
def convert_dit_checkpoint(
    checkpoint_url, pytorch_dump_folder_path, push_to_hub=False
):
    """
    ...
    assert logits.shape == torch.Size(
        expected_shape
    ), "Shape of logits not as expected"
    ...
    if push_to_hub:
        if has_lm_head:
            model_name = (
                "dit-base" if "base" in checkpoint_url else "dit-large"
            )
        else:
            model_name = (

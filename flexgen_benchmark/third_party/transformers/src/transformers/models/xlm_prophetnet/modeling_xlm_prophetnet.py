...
def softmax(hidden_state, dim, onnx_trace=False):
    if onnx_trace:
        ...
        return nn.functional.softmax(
            hidden_state, dim=dim, dtype=torch.float32
        )
    ...
    if is_bidirectional:
        ...
        rel_positions_bucket = (
            ...
            + torch.lt(
                ...
                torch.zeros_like(inv_relative_positions),
            ).int()
            * num_buckets
        )
...
class XLMProphetNetSeq2SeqLMOutput(ModelOutput):
    ...
    def decoder_cross_attentions(self):
        warnings.warn(
            (
                "`decoder_cross_attentions` is deprecated and will be removed"
                " soon. Please use `cross_attentions` instead."
            ),
            FutureWarning,
        )
...
class XLMProphetNetPreTrainedModel(PreTrainedModel):
    ...
    def _shift_right(self, input_ids):
        ...
        assert decoder_start_token_id is not None, (
            "self.model.config.decoder_start_token_id has to be defined. In"
            " XLMProphetNet it is usually set to the pad_token_id. See"
            " XLMProphetNet docs for more information"
        )
...
class XLMProphetNetPositionalEmbeddings(nn.Embedding):
    ...
    def forward(
        ...
        assert (position_ids is None) or (self.padding_idx is None), (
            "If position_ids is pre-computed then padding_idx should not be"
            " set."
        )
...
class XLMProphetNetAttention(nn.Module):
    ...
    def __init__(
        ...
        assert self.head_dim * num_attn_heads == hidden_size, (
            "`config.hidden_size` must be divisible by"
            " `config.num_encoder_attention_heads` and"
    ...
    def forward(
        ...
        assert list(hidden_states.size()) == [
            ...
            f" {batch_size, tgt_len, hidden_size}, but is"
            f" {hidden_states.size()}"
        )
        ...
        query_states = self._shape(query_states, tgt_len, batch_size).view(
            *proj_shape
        )
        ...
        assert attn_weights.size() == (
            ...
            "`attn_weights` should be of size"
            f" {batch_size * self.num_attn_heads, tgt_len, src_len}, but is of"
            f" size {attn_weights.shape}"
        )
        ...
        assert attention_mask is None or attention_mask.size() == (
            ...
            "`attention_mask` should be `None` or of shape"
            " attention_mask.size() =="
            f" {batch_size * self.num_attn_heads, 1, src_len}, but is"
        ...
        if layer_head_mask is not None:
            assert layer_head_mask.size() == (self.num_attn_heads,), (
                ...
                f" {(self.num_attn_heads,)}, but is {layer_head_mask.size()}"
            )
        ...
        assert attn_output.size() == (
            ...
            f" {batch_size * self.num_attn_heads, tgt_len, self.head_dim}, but"
            f" is of shape {attn_output.size()}"
        )
...
class XLMProphetNetNgramSelfAttention(nn.Module):
    ...
    def forward(
        ...
        assert list(hidden_states.size()) == [
            ...
            "`hidden_states` should be of shape"
            f" {batch_size, ngram_sequence_length, hidden_size}, but is of"
        ...
        if layer_head_mask is not None:
            ...
            main_attn_probs = layer_head_mask.view(
                1, -1, 1, 1
            ) * main_attn_probs.view(
        ...
        # [ngram, B*head, T, 2*T]
        predict_attn_weights = (
            predict_attn_weights + predict_relative_pos_embeddings
        )

        if extended_predict_attention_mask is not None:
            predict_attn_weights = (
                predict_attn_weights
                + extended_predict_attention_mask.to(
        ...
        attn_output = torch.cat(
            [main_attn_output, predict_attn_output], 1
    ...
    def get_main_relative_pos_embeddings(
        ...
        if main_relative_position_buckets is None:
            ...
            relative_positions = relative_positions - position_ids.unsqueeze(
                0
            ).repeat(
    ...
    def get_predict_relative_pos_embeddings(
        ...
        if predict_relative_position_buckets is None:
            ...
            assert position_ids[0][0] == key_sequence_length - 1, (
                "`position_ids` are incorrect. They should be of the format 1"
                " 2 3 4 5 ... (key_sequence_length - 1)"
            )
            relative_positions = (
        ...
        predict_relative_position_buckets = (
            predict_relative_position_buckets.unsqueeze(0).repeat(
        ...
        predict_relative_position_buckets = (
            predict_relative_position_buckets.view(
...
class XLMProphetNetEncoderLayer(nn.Module):
    ...
    def __init__(self, config: XLMProphetNetConfig):
        ...
        # 2nd residual block
        self.feed_forward = XLMProphetNetFeedForward(
...
class XLMProphetNetDecoder(XLMProphetNetPreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is None and inputs_embeds is None:
            raise ValueError(
                "Either `decoder_input_ids` or `decoder_inputs_embeds` has to"
                " be passed."
            )
        elif input_ids is not None and inputs_embeds is not None:
            raise ValueError(
                "Make sure to only pass `decoder_input_ids` or"
        ...
        if past_key_values is not None:
            assert hidden_states.size(1) == 1, (
                "At the moment `use_cache` is only supported for"
                " `decoder_input_ids` of length 1"
            )
            ...
            extended_predict_attention_mask = (
                self.prepare_predict_attention_mask(
                    hidden_states, attention_mask
                )
            )
        ...
        if encoder_attention_mask is not None:
            ...
            extended_encoder_attention_mask = (
                extended_encoder_attention_mask.to(inputs_embeds.dtype)
            )
        else:
    ...
    def prepare_predict_attention_mask(self, hidden_states, attention_mask):
        ...
        extended_predict_causal_mask = predict_causal_mask[
            :, None, :, :
        ].expand(
        ...
        if attention_mask is not None:
            ...
            extended_attention_mask = torch.cat(
                [
                    extended_attention_mask,
                    torch.zeros_like(extended_attention_mask),
                ],
                dim=-1,
            )
            extended_predict_attention_mask = (
...
@add_start_docstrings(
    (
        "The bare XLMProphetNet Model outputting raw hidden-states without any"
...
class XLMProphetNetModel(XLMProphetNetPreTrainedModel):
    def __init__(self, config: XLMProphetNetConfig):
        ...
        self.encoder = XLMProphetNetEncoder(
            encoder_config, self.word_embeddings
        )
        ...
        self.decoder = XLMProphetNetDecoder(
            decoder_config, self.word_embeddings
        )
...
@add_start_docstrings(
    (
        "The XLMProphetNet Model with a language modeling head. Can be used"
...
class XLMProphetNetForConditionalGeneration(XLMProphetNetPreTrainedModel):
    ...
    def _compute_loss(self, logits, labels, ignore_index=-100):
        ...
        loss = nn.functional.nll_loss(
            lprobs, expend_targets.view(-1), reduction="mean"
        )
...
@add_start_docstrings(
    (
        "The standalone decoder part of the XLMProphetNetModel with a lm head"
        " on top. The model can be used for causal language modeling."
    ),
    XLM_PROPHETNET_START_DOCSTRING,
)

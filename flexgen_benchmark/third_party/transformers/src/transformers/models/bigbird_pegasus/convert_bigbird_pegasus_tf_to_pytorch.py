...
def convert_bigbird_pegasus(
    ...
    remaining_weights = {
        k: tf_weights[k]
        for k in tf_weights
        if not k.startswith("pegasus/decoder")
    }
    ...
    for k, v in tqdm(decoder_weights.items(), "tf -> hf conversion"):
        ...
        if new_k not in state_dict:
            raise ValueError(
                f"could not find new key {new_k} in state dict. (converted"
                f" from {k})"
            )
        if any(
            [
                True if i in k else False
                for i in ["dense", "query", "key", "value"]
            ]
        ):
            v = v.T
    ...
    for k, v in tqdm(remaining_weights.items(), "tf -> hf conversion"):
        ...
        new_k = rename_state_dict_key(k, patterns)
        if (
            new_k not in state_dict
            and k != "pegasus/embeddings/position_embeddings"
        ):
            raise ValueError(
                f"could not find new key {new_k} in state dict. (converted"
                f" from {k})"
...
def get_tf_weights_as_numpy(path) -> Dict:
    ...
    for name, shape in tqdm(
        init_vars, desc="converting tf checkpoint to dict"
    ):

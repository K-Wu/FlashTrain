...
class BigBirdPegasusEncoderAttention(nn.Module):
    ...
    def set_attention_type(self, value: str):
        ...
        if value == "original_full":
            ...
            attn_weights = BigBirdPegasusBlockSparseAttention(
    ...
    def forward(
        ...
        head_mask = (
            head_mask.reshape(1, -1, 1, 1) if head_mask is not None else None
        )
...
class BigBirdPegasusEncoder(BigBirdPegasusPreTrainedModel):
    ...
    def forward(
        ...
        if self.attention_type == "block_sparse":
            (
                ...
                hidden_states,
                attention_mask,
            ) = self._pad_to_block_size(hidden_states, attention_mask)
        else:
            padding_len = 0
    ...
    def _pad_to_block_size(
        ...
        if padding_len > 0:
            ...
            input_ids_padding = (
                torch.ones(
                    (batch_size, padding_len), dtype=torch.long, device=device
                )
                * pad_id
            )
            inputs_embeds_padding = self.embed_tokens(input_ids_padding)
            hidden_states = torch.cat(
                [hidden_states, inputs_embeds_padding], dim=-2
            )
...
class BigBirdPegasusDecoder(BigBirdPegasusPreTrainedModel):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                BigBirdPegasusDecoderLayer(config)
...
@add_start_docstrings(
    (
        "The bare BigBirdPegasus Model outputting raw hidden-states without"
        ...
        if decoder_input_ids is None and decoder_inputs_embeds is None:
            if input_ids is None:
                raise ValueError(
                    ...
                    " `input_ids` or `decoder_input_ids` or"
                    " `decoder_inputs_embeds`."
                )
...
@add_start_docstrings(
    (
        "The BigBirdPegasus Model with a language modeling head. Can be used"
        " for summarization."
    ),
    BIGBIRD_PEGASUS_START_DOCSTRING,
)
...
class BigBirdPegasusForConditionalGeneration(BigBirdPegasusPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            if use_cache:
                logger.warning(
                    "The `use_cache` argument is changed to `False` since"
                    " `labels` is provided."
                )
            use_cache = False
            if decoder_input_ids is None and decoder_inputs_embeds is None:
    ...
    def forward(
        ...
        if input_ids is None and inputs_embeds is not None:
            raise NotImplementedError(
                "Passing input embeddings is currently not supported for"

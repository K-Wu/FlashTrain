...
class NezhaSelfAttention(nn.Module):
    ...
    def forward(
        ...
        value_position_scores = torch.matmul(
            attentions_probs_r, relations_values
        )
...
@add_start_docstrings(
    (
        "The bare Nezha Model transformer outputting raw hidden-states without"
...
class NezhaForMaskedLM(NezhaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `NezhaForMaskedLM` make sure"

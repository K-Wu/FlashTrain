...
_CTC_EXPECTED_OUTPUT = (
    "'mister quilter is the aposle of the middle classes and we are glad to"
    " welcome his gospel'"
)
...
_SEQ_CLASS_EXPECTED_OUTPUT = (  # TODO(anton) - could you quickly fine-tune a KS WavLM Model
    "'no'"
)
...
class WavLMAttention(nn.Module):
    ...
    def torch_multi_head_self_attention(
        ...
        key_padding_mask = (
            attention_mask.ne(1) if attention_mask is not None else None
        )
    ...
    def compute_bias(
        self, query_length: int, key_length: int
    ) -> torch.FloatTensor:
    ...
    def _relative_positions_bucket(
        ...
        relative_positions_if_large = torch.log(
            relative_positions.float() / max_exact
        )
        ...
        relative_position_if_large = (
            max_exact + relative_positions_if_large
        ).to(torch.long)
...
class WavLMEncoderLayer(nn.Module):
    def __init__(
        self, config: WavLMConfig, has_relative_position_bias: bool = True
    ):
        super().__init__()
...
class WavLMPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(
            module,
            (WavLMEncoder, WavLMEncoderStableLayerNorm, WavLMFeatureEncoder),
        ):
...
@add_start_docstrings(
    (
        "The bare WavLM Model transformer outputting raw hidden-states without"
...
class WavLMModel(WavLMPreTrainedModel):
    ...
    def freeze_feature_extractor(self):
        ...
        warnings.warn(
            (
                "The method `freeze_feature_extractor` is deprecated and will"
                " be removed in Transformers v5.Please use the equivalent"
                " `freeze_feature_encoder` method instead."
            ),
            FutureWarning,
        )
    ...
    def __init__(self, config):
        ...
        if config.vocab_size is None:
            raise ValueError(
                ...
                " follows: `WavLMForCTC.from_pretrained(...,"
...
class WavLMForSequenceClassification(WavLMPreTrainedModel):
    def __init__(self, config):
        ...
        if hasattr(config, "add_adapter") and config.add_adapter:
            raise ValueError(
                "Sequence classification does not support the use of WavLM"
                " adapters (config.add_adapter=True)"
            )
        self.wavlm = WavLMModel(config)
        num_layers = (
...
class WavLMForAudioFrameClassification(WavLMPreTrainedModel):
    def __init__(self, config):
        ...
        if hasattr(config, "add_adapter") and config.add_adapter:
            raise ValueError(
                "Audio frame classification does not support the use of WavLM"
                " adapters (config.add_adapter=True)"
            )
        self.wavlm = WavLMModel(config)
        num_layers = (

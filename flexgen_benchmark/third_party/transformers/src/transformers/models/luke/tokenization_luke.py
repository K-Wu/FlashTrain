...
class LukeTokenizer(RobertaTokenizer):
        ...
        **kwargs,
        ...
        for entity_special_token in [
            ...
            if entity_special_token not in self.entity_vocab:
                raise ValueError(
                    "Specified entity special token"
                    f" ``{entity_special_token}`` is not found in"
                    " entity_vocab. Probably an incorrect entity vocab file"
                    f" is loaded: {entity_vocab_file}."
                )
        ...
        if task is None or task == "entity_span_classification":
            ...
            raise ValueError(
                f"Task {task} not supported. Select task from"
                " ['entity_classification', 'entity_pair_classification',"
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not (is_valid_single_text or is_valid_batch_text):
            raise ValueError(
                "text input must be of type `str` (single example) or"
                " `List[str]` (batch)."
            )
        ...
        if not (
            text_pair is None
            or is_valid_single_text_pair
            or is_valid_batch_text_pair
        ):
            raise ValueError(
                "text_pair input must be of type `str` (single example) or"
                " `List[str]` (batch)."
            )
        ...
        **kwargs,
        ...
        if return_offsets_mapping:
            raise NotImplementedError(
                "return_offset_mapping is not available when using Python"
                " tokenizers. To use this feature, change your tokenizer to"
                " one deriving from transformers.PreTrainedTokenizerFast. More"
                " information on available tokenizers at"
        ...
        **kwargs,
        ...
        if return_offsets_mapping:
            raise NotImplementedError(
                "return_offset_mapping is not available when using Python"
                " tokenizers. To use this feature, change your tokenizer to"
                " one deriving from transformers.PreTrainedTokenizerFast."
            )
    ...
    def _check_entity_input_format(
        ...
        if not isinstance(entity_spans, list):
            ...
            raise ValueError(
                "entity_spans should be given as a list of tuples containing"
        ...
        if entities is not None:
            ...
            if len(entities) != len(entity_spans):
                raise ValueError(
                    "If you specify entities, entities and entity_spans must"
                    " be the same length"
                )
    ...
    def _create_input_sequence(
        ...
        **kwargs,
        ...
        def get_input_ids_and_entity_token_spans(text, entity_spans):
            ...
            split_char_positions = sorted(
                frozenset(itertools.chain(*entity_spans))
            )
            char_pos2token_pos = {}
            ...
            for split_char_position in split_char_positions:
                ...
                if (
                    split_char_position > 0
                    and text[split_char_position - 1] == " "
        ...
        if self.task is None:
            if entity_spans is None:
                ...
                if entities is None:
                    first_entity_ids = [self.entity_mask_token_id] * len(
                        entity_spans
                    )
                else:
                    first_entity_ids = [
            ...
            if text_pair is not None:
                if entity_spans_pair is None:
                    ...
                    self._check_entity_input_format(
                        entities_pair, entity_spans_pair
                    )
                    ...
                    if entities_pair is None:
                        ...
                        second_entity_ids = [
                            self.entity_vocab.get(
                                entity, self.entity_unk_token_id
                            )
            ...
            if not (
                ...
                raise ValueError(
                    ...
                    " containing the start and end character indices of an"
                    " entity"
                )
            ...
            first_entity_token_spans = [
                (entity_token_start, entity_token_end + 2)
            ]
            ...
            if not (
                ...
                raise ValueError(
                    ...
                    " each tuple containing the start and end character"
                    " indices of an entity"
                )

            head_span, tail_span = entity_spans
            first_entity_ids = [
                self.entity_mask_token_id,
                self.entity_mask2_token_id,
            ]
            (
                first_ids,
                first_entity_token_spans,
            ...
            if not (
                ...
                raise ValueError(
                    "Entity spans should be provided as a list of tuples, each"
                    " tuple containing the start and end character indices of"
                    " an entity"
                )
        ...
        if entity_ids is not None:
            ...
            if pair_entity_ids is not None:
                valid_pair_entity_ids = [
                    ent_id
                    for ent_id, span in zip(
                        pair_entity_ids, pair_entity_token_spans
                    )
            ...
            if num_invalid_entities != 0:
                logger.warning(
                    f"{num_invalid_entities} entities are ignored because"
                    " their entity spans are invalid due to the truncation of"
                    " input tokens"
                )
            ...
            for token_spans, offset in (
                ...
                if token_spans is not None:
                    for start, end in token_spans:
                        ...
                        position_ids += [-1] * (
                            self.max_mention_length - end + start
                        )
    ...
    def pad(
        ...
        if any(len(v) != batch_size for v in encoded_inputs.values()):
            raise ValueError(
                "Some items in the output dictionary have a different batch"
                " size than others."
            )

...
def convert_luke_checkpoint(
    ...
    config = LukeConfig(
        use_entity_aware_attention=True, **metadata["model_config"]
    )
    ...
    for layer_index in range(config.num_hidden_layers):
        for matrix_name in ["query.weight", "query.bias"]:
            ...
            state_dict[prefix + "w2e_" + matrix_name] = state_dict[
                prefix + matrix_name
            ]
            state_dict[prefix + "e2w_" + matrix_name] = state_dict[
                prefix + matrix_name
            ]
            state_dict[prefix + "e2e_" + matrix_name] = state_dict[
                prefix + matrix_name
            ]
    ...
    if not (
        len(missing_keys) == 1 and missing_keys[0] == "embeddings.position_ids"
    ):
        raise ValueError(
            f"Missing keys {', '.join(missing_keys)}. Expected only missing"
    ...
    text = (
        "Top seed Ana Ivanovic said on Thursday she could hardly believe her"
        " luck as a fortuitous netcord helped the new world number one avoid a"
        " humiliating second- round exit at Wimbledon ."
    )
    span = (39, 42)
    encoding = tokenizer(
    ...
    if not (outputs.last_hidden_state.shape == expected_shape):
        raise ValueError(
            "Outputs.last_hidden_state.shape is"
            f" {outputs.last_hidden_state.shape}, Expected shape is"
            f" {expected_shape}"
        )
    ...
    if not (outputs.entity_last_hidden_state.shape != expected_shape):
        raise ValueError(
            "Outputs.entity_last_hidden_state.shape is"
            f" {outputs.entity_last_hidden_state.shape}, Expected shape is"
            f" {expected_shape}"
        )
    if not torch.allclose(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Path to an entity_vocab.tsv file, containing the entity"
            " vocabulary."
        ),
    )
    parser.add_argument(

...
def input_values_processing(func, config, input_values, **kwargs):
    ...
    if isinstance(input_values, (tuple, list)):
        ...
        if "inputs" in input_values:
            warnings.warn(
                (
                    ...
                    " in a future version, use `input_values` instead."
                ),
                FutureWarning,
            )
        ...
        if isinstance(input_values, tf.Tensor) or input_values is None:
            ...
            raise ValueError(
                f"Data of type {type(input_values)} is not allowed only"
                f" {allowed_types} is accepted for {parameter_names[0]}."
            )
...
def _compute_mask_indices(
    ...
    spec_aug_mask = _scatter_values_on_batch_indices(
        tf.ones_like(spec_aug_mask_idxs),
        spec_aug_mask_idxs,
        tf.shape(spec_aug_mask),
    )
    ...
    def _check_if_input_shape_is_none(self, input_shape):
        ...
        if dim is None:
            raise ValueError(
                "Axis "
                + str(self.axis)
                + " of input tensor should have a defined dimension but the"
                " layer received an input with shape "
                + str(input_shape)
                + "."
            )
...
class TFHubertWeightNormConv1D(tf.keras.layers.Conv1D):
    ...
    def call(self, inputs):
        ...
        padded_inputs = tf.pad(
            inputs,
            ((0, 0), (self.explicit_padding, self.explicit_padding), (0, 0)),
        )
...
class TFHubertNoLayerNormConvLayer(tf.keras.layers.Layer):
    def __init__(
        self, config: HubertConfig, layer_id: int = 0, **kwargs: Any
    ) -> None:
        super().__init__(**kwargs)
...
class TFHubertFeedForward(tf.keras.layers.Layer):
    def __init__(self, config: HubertConfig, **kwargs):
        ...
        self.intermediate_dropout = tf.keras.layers.Dropout(
...
class TFHubertMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config: HubertConfig, **kwargs):
        ...
        if config.do_stable_layer_norm:
            self.encoder = TFHubertEncoderStableLayerNorm(
                config, name="encoder"
            )
        else:
    ...
    def call(
        ...
        **kwargs: Any,
        ...
        hidden_states = self.feature_extractor(
            tf.cast(inputs["input_values"], tf.float32),
...
class TFHubertPreTrainedModel(TFPreTrainedModel):
    ...
    def dummy_inputs(self) -> Dict[str, tf.Tensor]:
        pad_token = 0.0
        input_values = tf.convert_to_tensor(
            np.random.rand(1, 16000), tf.float32
        )
        dummy_inputs = {
            "input_values": input_values,
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        logger.warning(
            f"\n{self.__class__.__name__} has backpropagation operations that"
            " are NOT supported on CPU. If you wish to train/fine-tine this"
            " model, you need a GPU or a TPU"
        )
...
@add_start_docstrings(
    (
        "The bare TFHubert Model transformer outputing raw hidden-states"

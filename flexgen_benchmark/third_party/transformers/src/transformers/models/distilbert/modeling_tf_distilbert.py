...
class TFMultiHeadSelfAttention(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        assert self.dim % self.n_heads == 0, (
            f"Hidden size {self.dim} not dividable by number of heads"
            f" {self.n_heads}"
        )
...
class TFTransformerBlock(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        assert config.dim % config.n_heads == 0, (
            f"Hidden size {config.dim} not dividable by number of heads"
            f" {config.n_heads}"
        )
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if output_attentions:
                ...
                assert len(layer_outputs) == 1, (
                    f"Incorrect number of outputs {len(layer_outputs)} instead"
                    " of 1"
                )
...
class TFDistilBertForMaskedLM(
    ...
    def call(
        ...
        loss = (
            None
            if labels is None
            else self.hf_compute_loss(labels, prediction_logits)
        )
...
class TFDistilBertForSequenceClassification(
    ...
    def call(
        ...
        pooled_output = self.dropout(
            pooled_output, training=training
        )  # (bs, dim)
...
class TFDistilBertForMultipleChoice(
    TFDistilBertPreTrainedModel, TFMultipleChoiceLoss
):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)

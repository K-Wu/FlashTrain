...
class FlaxMultiHeadSelfAttention(nn.Module):
    ...
    def __call__(
        ...
        def shape(x):
            """separate heads"""
            return x.reshape(bs, -1, self.n_heads, dim_per_head).transpose(
                0, 2, 1, 3
            )
...
class FlaxTransformerBlock(nn.Module):
    ...
    def setup(self):
        assert self.config.dim % self.config.n_heads == 0, (
            f"Hidden size {self.config.dim} not dividable by number of heads"
            f" {self.config.n_heads}"
        )

        self.attention = FlaxMultiHeadSelfAttention(
...
class FlaxDistilBertModule(nn.Module):
    ...
    def setup(self):
        ...
        self.transformer = FlaxTransformerEncoder(
            self.config, dtype=self.dtype
        )
...
class FlaxDistilBertForMaskedLMModule(nn.Module):
    ...
    def __call__(
        ...
        if self.config.tie_word_embeddings:
            shared_embedding = self.distilbert.variables["params"][
                "embeddings"
...
class FlaxDistilBertForSequenceClassificationModule(nn.Module):
    ...
    def setup(self):
        self.distilbert = FlaxDistilBertModule(

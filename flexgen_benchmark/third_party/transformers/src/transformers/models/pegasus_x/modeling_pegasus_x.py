...
class PegasusXGlobalLocalAttention(nn.Module):
    ...
    def forward(
        ...
        local_attn_output = local_attn_output.permute(
            0, 2, 3, 1, 4
        ).contiguous()
        ...
        if output_attentions:
            attn_probs = {
                "global": global_attn_probs,
                "local": local_attn_probs,
            }
        else:
            attn_probs = None
    ...
    def compute_global_attention_representations(
        ...
        extended_mask = nn.functional.pad(
            mask, pad=(dim.global_len, 0), value=0
        )

        # [batch_size, num_heads, global_len, global_len+padded_seq_len]
        attn_weights = torch.einsum(
            "BHGF,BHXF->BHGX", global_q, global_and_local_k
        )
        ...
        attn_output = torch.einsum(
            "BHGX,BHXF->BHGF", attn_probs, global_and_local_v
        )
    ...
    def compute_local_attention_representations(
        ...
        attn_weights = torch.cat(
            [blocked_local2global, blocked_local2local], dim=-1
        )
        attn_weights = attn_weights + extended_mask[:, None, :, None, :]
        attn_probs = nn.functional.softmax(attn_weights, dim=-1)
        attn_probs = nn.functional.dropout(
...
class PegasusXEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        global_hidden_states = self.global_self_attn_layer_norm(
        ...
        global_hidden_states = self.activation_fn(
            self.fc1(global_hidden_states)
        )
        global_hidden_states = nn.functional.dropout(
            global_hidden_states,
...
class PegasusXEncoder(PegasusXPreTrainedModel):
    ...
    def resize_position_embeddings(self, new_num_position_embeddings: int):
        ...
        logger.info(
            "Setting"
            f" `config.max_position_embeddings={new_num_position_embeddings}`..."
        )
    ...
    def forward(
        ...
        global_hidden_states = self.embed_global(
            torch.arange(
                self.config.num_global_tokens, device=hidden_states.device
...
@add_start_docstrings(
    (
        "The bare PEGASUS-X Model outputting raw hidden-states without any"

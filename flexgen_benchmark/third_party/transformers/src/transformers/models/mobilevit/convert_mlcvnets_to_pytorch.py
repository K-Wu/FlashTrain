...
def rename_key(name, base_model=False):
        ...
        for j in range(0, 4):
            if f".{i}.{j}." in name:
                ...
                if "expand_1x1" in name:
                    name = name.replace(
                        "expand_1x1", "downsampling_layer.expand_1x1"
                    )
                if "conv_3x3" in name:
                    name = name.replace(
                        "conv_3x3", "downsampling_layer.conv_3x3"
                    )
                if "reduce_1x1" in name:
                    name = name.replace(
                        "reduce_1x1", "downsampling_layer.reduce_1x1"
                    )
    ...
    if ".pre_norm_mha.1.out_proj." in name:
        name = name.replace(
            ".pre_norm_mha.1.out_proj.", ".attention.output.dense."
        )
    ...
    if "segmentation_head.classifier.classifier." in name:
        name = name.replace(
            "segmentation_head.classifier.classifier.",
            "segmentation_head.classifier.",
        )
...
def convert_state_dict(orig_state_dict, model, base_model=False):
    ...
    for key in orig_state_dict.copy().keys():
        ...
        if "qkv" in key:
            ...
            layer = model.get_submodule(
                f"{model_prefix}encoder.layer.{layer_num}"
            )
...
def convert_movilevit_checkpoint(
    ...
    if mobilevit_name.startswith("deeplabv3_"):
        ...
        assert torch.allclose(
            logits[0, :3, :3, :3], expected_logits, atol=1e-4
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Name of the MobileViT model you'd like to convert. Should be one"
            " of 'mobilevit_s', 'mobilevit_xs', 'mobilevit_xxs',"
            " 'deeplabv3_mobilevit_s', 'deeplabv3_mobilevit_xs',"
            " 'deeplabv3_mobilevit_xxs'."
        ),
    )
    parser.add_argument(
        "--checkpoint_path",

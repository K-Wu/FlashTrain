    ...
    def __init__(
        ...
        **kwargs,
        ...
        if out_channels % groups != 0:
            raise ValueError(
                f"Output channels ({out_channels}) are not divisible by"
                f" {groups} groups."
            )
...
class TFMobileViTSelfAttention(tf.keras.layers.Layer):
    def __init__(
        self, config: MobileViTConfig, hidden_size: int, **kwargs
    ) -> None:
        super().__init__(**kwargs)
...
class TFMobileViTAttention(tf.keras.layers.Layer):
    def __init__(
        ...
        super().__init__(**kwargs)
        self.attention = TFMobileViTSelfAttention(
            config, hidden_size, name="attention"
        )
        self.dense_output = TFMobileViTSelfOutput(
            config, hidden_size, name="output"
        )
...
class TFMobileViTTransformerLayer(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(**kwargs)
        self.attention = TFMobileViTAttention(
            config, hidden_size, name="attention"
        )
...
class TFMobileViTLayer(tf.keras.layers.Layer):
    ...
    def unfolding(self, features: tf.Tensor) -> Tuple[tf.Tensor, Dict]:
        ...
        patches = tf.reshape(
            patches, (batch_size, channels, num_patches, patch_area)
        )
        patches = tf.transpose(patches, [0, 3, 2, 1])
        patches = tf.reshape(
            patches, (batch_size * patch_area, num_patches, channels)
        )
    ...
    def folding(self, patches: tf.Tensor, info_dict: Dict) -> tf.Tensor:
        ...
        features = tf.reshape(
            patches, (batch_size, patch_area, num_patches, -1)
        )
...
class TFMobileViTMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        self, config: MobileViTConfig, expand_output: bool = True, **kwargs
    ):
        super().__init__(**kwargs)
        self.config = config
    ...
    def call(
        ...
        if self.expand_output:
            ...
            last_hidden_state = tf.transpose(
                last_hidden_state, perm=[0, 3, 1, 2]
            )
...
@add_start_docstrings(
    (
        "The bare MobileViT model outputting raw hidden-states without any"
...
class TFMobileViTForImageClassification(
    ...
    def call(
        ...
        logits = self.classifier(
            self.dropout(pooled_output, training=training)
        )
        loss = (
            None
            if labels is None
...
class TFMobileViTASPPPooling(tf.keras.layers.Layer):
    def __init__(
        self, config: MobileViTConfig, out_channels: int, **kwargs
    ) -> None:
        super().__init__(**kwargs)
    ...
    def call(self, features: tf.Tensor, training: bool = False) -> tf.Tensor:
        ...
        features = self.conv_1x1(features, training=training)
        features = tf.image.resize(
            features, size=spatial_size, method="bilinear"
        )
        return features
...
class TFMobileViTForSemanticSegmentation(TFMobileViTPreTrainedModel):
    def __init__(self, config: MobileViTConfig, **kwargs) -> None:
        ...
        self.segmentation_head = TFMobileViTDeepLabV3(
            config, name="segmentation_head"
        )

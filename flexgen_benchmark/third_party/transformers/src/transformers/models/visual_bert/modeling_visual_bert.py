    ...
    def __init__(self, config):
        ...
        if config.special_visual_initialize:
            self.visual_token_type_embeddings.weight.data = nn.Parameter(
                self.token_type_embeddings.weight.data.clone(),
                requires_grad=True,
            )
        ...
        if visual_embeds is not None:
            ...
            if image_text_alignment is not None:
                ...
                image_text_alignment = (
                    image_text_alignment_mask * image_text_alignment
                )
                ...
                if (image_text_alignment_mask == 0).sum() != 0:
                    ...
                    logger.warning(
                        "Found 0 values in `image_text_alignment_mask`."
                        " Setting them to 1 to avoid divide-by-zero error."
                    )
                ...
                if visual_position_embeddings.size(1) != visual_embeds.size(1):
                    if visual_position_embeddings.size(1) < visual_embeds.size(
                        1
                    ):
                        raise ValueError(
                            "Visual position embeddings length:"
                            f" {visual_position_embeddings.size(1)} should be"
                            " the same as `visual_embeds` length:"
                            f" {visual_embeds.size(1)}"
                        )
    ...
    def forward(
        ...
        if visual_embeds is not None and visual_attention_mask is None:
            visual_attention_mask = torch.ones(
                visual_input_shape, device=device
            )
...
class VisualBertForPreTraining(VisualBertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None and sentence_image_labels is not None:
            total_size = attention_mask.size(-1) + visual_attention_mask.size(
                -1
            )
            if labels.size(-1) != total_size:
                raise ValueError(
                    "The labels provided should have same sequence length as"
                    " total attention mask. Found labels with sequence length"
                    f" {labels.size(-1)}, expected {total_size}."
                )
...
class VisualBertForMultipleChoice(VisualBertPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        VISUAL_BERT_INPUTS_DOCSTRING.format(
    ...
    def forward(
        ...
        visual_embeds = (
            visual_embeds.view(
                -1, visual_embeds.size(-2), visual_embeds.size(-1)
            )
            if visual_embeds is not None
            else None
        )
...
class VisualBertForRegionToPhraseAlignment(VisualBertPreTrainedModel):
    ...
    def forward(
        ...
        if region_to_phrase_position is None:
            raise ValueError(
                "`region_to_phrase_position` should not be None when using"
                " Flickr Model."
            )
        ...
        region_to_phrase_position_mask = (
            region_to_phrase_position != -1
        ).long()
        ...
        expanded_region_to_phrase_positions = (
            region_to_phrase_position.unsqueeze(2).expand(
        ...
        if visual_features.size(1) != visual_attention_mask.size(1):
            raise ValueError(
                f"Visual features length :{visual_features.size(1)} should be"
                " the same as visual attention mask length:"
                f" {visual_attention_mask.size(1)}."
            )

...
def convert_luke_checkpoint(
    ...
    for key, value in state_dict.items():
        if not (
            key.startswith("lm_head") or key.startswith("entity_predictions")
        ):
    ...
    model.tie_weights()
    assert (
        model.luke.embeddings.word_embeddings.weight
    ...
    text = (
        "ISO 639-3 uses the code fas for the dialects spoken across Iran and"
        " アフガニスタン (Afghanistan)."
    )
    span = (0, 9)

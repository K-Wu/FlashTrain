...
class TFAttention(tf.keras.layers.Layer):
    def __init__(
        self, nx, config, scale=False, is_cross_attention=False, **kwargs
    ):
    ...
    def call(
        ...
        if encoder_hidden_states is not None:
            if not hasattr(self, "q_attn"):
                raise ValueError(
                    "If class is used as cross attention, the weights `q_attn`"
                    " have to be defined. Please make sure to instantiate"
                    " class with `GPT2Attention(...,"
...
class TFBlock(tf.keras.layers.Layer):
    ...
    def call(
        ...
        if encoder_hidden_states is not None:
            ...
            ca = output_cross_attn[
                0
            ]  # output_attn: a, present, (cross_attentions)
            x = x + ca
            outputs = (
...
@add_start_docstrings(
    (
        "The bare GPT2 Model transformer outputting raw hidden-states without"

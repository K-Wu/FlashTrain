...
class ModelCard:
    ...
    def __init__(self, **kwargs):
        warnings.warn(
            (
                "The class `ModelCard` is deprecated and will be removed in"
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        if os.path.isfile(pretrained_model_name_or_path):
            ...
            try:
                ...
                if is_local:
                    logger.info(
                        f"loading model card file {resolved_model_card_file}"
                    )
                else:
                    logger.info(
                        f"loading model card file {MODEL_CARD_NAME} from cache"
                        f" at {resolved_model_card_file}"
                    )
                # Load model card
...
class TrainingSummary:
    ...
    def create_model_index(self, metric_mapping):
        ...
        dataset_mapping = {
            tag: name for tag, name in zip(dataset_tags, dataset_names)
        }
        dataset_arg_mapping = {
            tag: arg for tag, arg in zip(dataset_tags, dataset_args)
        }
        dataset_metadata_mapping = {
            tag: metadata
            for tag, metadata in zip(dataset_tags, dataset_metadata)
        }
        ...
        for task_tag, ds_tag in all_possibilities:
            ...
            if (
                "task" in result
                and "dataset" in result
                and "metrics" in result
            ):
                model_index["results"].append(result)
            else:
                logger.info(
                    "Dropping the following result as it does not have all"
                    f" the necessary fields:\n{result}"
                )
    ...
    def create_metadata(self):
        ...
        metadata = _insert_values_as_list(
            metadata, "datasets", self.dataset_tags
        )
        metadata = _insert_values_as_list(
    ...
    def to_model_card(self):
        ...
        if self.dataset is None:
            ...
            if isinstance(self.dataset, str):
                ...
                isinstance(self.dataset, (tuple, list))
    ...
    def from_trainer(
        ...
        _, eval_lines, eval_results = parse_log_history(
...
def make_markdown_table(lines):
    ...
    table = _regular_table_line(
        list(lines[0].keys()), list(col_widths.values())
    )
...
def extract_hyperparameters_from_trainer(trainer):
    hyperparameters = {
        k: getattr(trainer.args, k) for k in _TRAINING_ARGS_KEYS
    }
    ...
    total_eval_batch_size = (
        trainer.args.eval_batch_size * trainer.args.world_size
    )
    ...
    if trainer.args.adafactor:
        ...
        hyperparameters["optimizer"] = (
            "Adam with"
            f" betas=({trainer.args.adam_beta1},{trainer.args.adam_beta2}) and"
    ...
    if trainer.args.label_smoothing_factor != 0.0:
        ...
        ] = trainer.args.label_smoothing_factor

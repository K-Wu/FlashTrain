...
def keras_serializable(cls):
    ...
    def wrapped_init(self, *args, **kwargs):
        ...
        if isinstance(config, dict):
            ...
            raise ValueError(
                "Must pass either `config` (PretrainedConfig) or `config`"
                " (dict)"
            )
    ...
    def hf_compute_loss(self, labels, logits):
        ...
        if (
            ...
            if tf.math.reduce_any(labels == -1):
                tf.print(
                    "Using `-1` to mask the loss for the token is deprecated."
                    " Please use `-100` instead."
                )
...
def unpack_inputs(func):
    ...
    def run_call_with_unpacked_inputs(self, *args, **kwargs):
        ...
        fn_args_and_kwargs.update(
            dict(zip(func.__code__.co_varnames[1:], args))
        )
...
def input_processing(func, config, **kwargs):
    ...
    if "inputs" in kwargs["kwargs_call"]:
        warnings.warn(
            (
                "The `inputs` argument is deprecated and will be removed in a"
                " future version, use `input_ids` instead."
            ),
            FutureWarning,
        )
    ...
    if "decoder_cached_states" in kwargs["kwargs_call"]:
        warnings.warn(
            (
                "The `decoder_cached_states` argument is deprecated and will"
                " be removed in a future version, use `past_key_values`"
                " instead."
            ),
            FutureWarning,
        )
    ...
    if (
        "past" in kwargs["kwargs_call"]
        and "past_key_values" in parameter_names
    ):
        warnings.warn(
            (
                "The `past` argument is deprecated and will be removed in a"
                " future version, use `past_key_values` instead."
            ),
            FutureWarning,
        )
        kwargs["past_key_values"] = kwargs["kwargs_call"].pop("past")
    elif (
        "past_key_values" in kwargs["kwargs_call"]
        and "past" in parameter_names
    ):
    ...
    if has_kwargs:
        ...
        if len(kwargs["kwargs_call"]) > 0:
            raise ValueError(
                "The following keyword arguments are not supported by this"
                f" model: {list(kwargs['kwargs_call'].keys())}."
            )
    ...
    for k, v in kwargs.items():
        if isinstance(v, allowed_types) or v is None:
            ...
            raise ValueError(
                f"Data of type {type(v)} is not allowed only"
                f" {allowed_types} is accepted for {k}."
            )
    ...
    if isinstance(main_input, (tuple, list)):
        for i, input in enumerate(main_input):
            ...
            if type(input) == tf.Tensor:
                ...
                raise ValueError(
                    f"Data of type {type(input)} is not allowed only"
                    f" {allowed_types} is accepted for {parameter_names[i]}."
                )
    elif isinstance(main_input, Mapping):
        if "inputs" in main_input:
            warnings.warn(
                (
                    "The `inputs` argument is deprecated and will be removed"
                    " in a future version, use `input_ids` instead."
                ),
                FutureWarning,
            )
        ...
        if "decoder_cached_states" in main_input:
            warnings.warn(
                (
                    "The `decoder_cached_states` argument is deprecated and"
        ...
        for k, v in dict(main_input).items():
            if isinstance(v, allowed_types) or v is None:
                ...
                logger.warning(
                    f"The parameter {k} does not belongs to the parameter list"
                    f" {parameter_names} and will be ignored."
                )
                continue
            else:
                raise ValueError(
        ...
        if (
            isinstance(main_input, (tf.Tensor, KerasTensor))
            or main_input is None
        ):
            output[main_input_name] = main_input
        else:
            raise ValueError(
                f"Data of type {type(main_input)} is not allowed only"
                f" {allowed_types} is accepted for {main_input_name}."
            )
        ...
        with h5py.File(resolved_archive_file, "r") as sharded_checkpoint_file:
            ...
            for layer_name in saved_h5_model_layers_name:
                ...
                if layer_name not in model_layer_map:
                    ...
                    symbolic_weight = model.weights[
                        model_layer_map[layer_name]
                    ]
        ...
        try:
            with open(resolved_archive_file) as f:
                if f.read().startswith("version"):
                    raise OSError(
                        "You seem to have cloned a repository without having"
                        " git-lfs installed. Please install git-lfs and run"
                        " `git lfs install` followed by `git lfs pull` in the"
                        " folder you cloned."
                    )
                else:
                    raise ValueError(
                        "Unable to locate the file"
                        f" {resolved_archive_file} which is necessary to load"
                        " this pretrained model. Make sure you have saved the"
                        " model properly."
                    ) from e
        except (UnicodeDecodeError, ValueError):
            raise OSError(
                "Unable to load weights from TF checkpoint file for"
                f" '{resolved_archive_file}' at '{resolved_archive_file}'. If"
                " you tried to load a TF model from a sharded checkpoint, you"
                " should try converting the modelby loading it in pytorch and"
                " saving it localy. A convertion script should be realeased"
                " soon."
            )
...
def load_tf_weights_from_h5(
    ...
    with h5py.File(resolved_archive_file, "r") as sharded_checkpoint_file:
        ...
        missing_layers = list(
            set([layer.name for layer in model.layers])
        ...
        for layer in model.layers:
            ...
            if layer.name in saved_h5_model_layers_name:
                ...
                for symbolic_weight in symbolic_weights:
                    ...
                    if (
                        saved_weight_value is None
                        and symbolic_weight_name.endswith("embeddings:0")
                    ):
                        symbolic_weight_name = (
                            symbolic_weight_name[:-12] + "weight:0"
                        )
                        saved_weight_value = saved_weights.get(
                            symbolic_weight_name, None
                        )
...
def load_tf_weights_from_safetensors(
    ...
    weight_names = [
        format_weight_name(w.name, _prefix=_prefix) for w in model.weights
    ]
    ...
    for weight in model.weights:
        ...
        if weight_name in state_dict:
            ...
            if K.int_shape(weight) != weight_value.shape:
                ...
                # If the two shapes are not compatible we raise an issue
                try:
                    weight_value = tf.reshape(
                        weight_value, K.int_shape(weight)
                    )
                except ValueError as e:
                    if ignore_mismatched_sizes:
                        mismatched_layers.append(
...
class TFPreTrainedModel(
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        if not isinstance(config, PretrainedConfig):
            raise ValueError(
                ...
                "`PretrainedConfig`. To create a model from a pretrained model"
                " use "
                "`model ="
                f" {self.__class__.__name__}.from_pretrained(PRETRAINED_MODEL_NAME)`"
            )
        ...
        input_signature=[
            {
                "input_ids": tf.TensorSpec(
                    (None, None), tf.int64, name="input_ids"
                ),
    ...
    def load_repo_checkpoint(self, repo_path_or_name):
        ...
        if getattr(self, "optimizer", None) is None:
            raise RuntimeError(
                "Checkpoint loading failed as no optimizer is attached to the"
                " model. This is most likely caused by the model not being"
                " compiled."
            )
            ...
            for file in (
                ...
                if file not in repo_files:
                    raise FileNotFoundError(
                        f"Repo {repo_path_or_name} does not contain checkpoint"
                        f" file {file}!"
                    )
    ...
    def prepare_tf_dataset(
        ...
        model_inputs = list(
            dict(inspect.signature(self.call).parameters).keys()
        )
        ...
        if "cols_to_retain" in list(
            ...
            unwanted_columns = [
                ...
                if feature not in model_inputs
                and feature not in ("label_ids", "label")
            ]
    ...
    def compile(
        ...
        **kwargs,
        ...
        if loss == "passthrough":
            logger.warning(
                "No loss specified in compile() - the model's internal loss"
                " computation will be used as the loss. Don't panic - this is"
                " a common way to train TensorFlow models in Transformers! To"
                " disable this behaviour please pass a loss argument, or"
                " explicitly pass `loss=None` if you do not want your model to"
                " compute a loss."
            )
            loss = dummy_loss
        ...
        parent_args = list(
            inspect.signature(tf.keras.Model.compile).parameters.keys()
        )
    ...
    def compute_loss(self, *args, **kwargs):
        if hasattr(tf.keras.Model, "compute_loss"):
            ...
            warnings.warn(
                (
                    "The old compute_loss method is deprecated as it conflicts"
                    " with the Keras compute_loss method added in TF 2.8. If"
                    " you want the original HF compute_loss, please call"
                    " hf_compute_loss() instead. From TF versions >= 2.8, or"
                    " Transformers versions >= 5, calling compute_loss() will"
                    " get the Keras method instead."
                ),
                FutureWarning,
            )
    ...
    def get_label_to_output_name_mapping(self):
        ...
        if self._label_to_output_map is not None:
            ...
            return {
                ...
                "sentence_order_label": "sop_logits",
            }
    ...
    def train_step(self, data):
            ...
            # Otherwise, copy keys from y to x as long as they weren't already present in x
                ...
                for key, val in y.items():
                    if key in arg_names and key not in x:
                        ...
                        output_to_label.get(key, None) in arg_names
                        and key not in x
                    ):
        ...
        if y is None:
            ...
            if not y and not self._using_dummy_loss:
                raise ValueError(
                    "Could not find label column(s) in input dict and no"
                    " separate labels were provided!"
                )
    ...
    def set_input_embeddings(self, value):
        ...
        if main_layer is None:
            raise NotImplementedError(
                "The model does not implements the base_model_prefix"
                " attribute."
            )
    ...
    def get_output_layer_with_bias(self) -> Union[None, tf.keras.layers.Layer]:
        ...
        warnings.warn(
            (
                "The method get_output_layer_with_bias is deprecated. Please"
                " use `get_lm_head` instead."
            ),
            FutureWarning,
        )
    ...
    def get_prefix_bias_name(self) -> Union[None, str]:
        ...
        warnings.warn(
            (
                "The method get_prefix_bias_name is deprecated. Please use"
                " `get_bias` instead."
            ),
            FutureWarning,
        )
    ...
    def _v2_resize_token_embeddings(self, new_num_tokens):
        ...
        new_embeddings = self._v2_get_resized_embeddings(
            old_embeddings, new_num_tokens
        )
        self.set_input_embeddings(new_embeddings)
        ...
        # If word embeddings are not tied, make sure that lm head decoder is resized as well.
        tied_weights = (
            self.get_input_embeddings() == self.get_output_embeddings()
        )
    ...
    def _v2_get_resized_lm_head_bias(
        ...
        for attr, weight in old_lm_head_bias.items():
            ...
            if old_num_tokens > new_num_tokens:
                ...
                new_bias = tf.pad(
                    weight.value(), tf.convert_to_tensor(padding_shape)
                )

            new_lm_head_bias[attr] = new_bias
        return new_lm_head_bias

    def _get_resized_lm_head_decoder(
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        if saved_model:
            ...
            self.save(
                saved_model_dir, include_optimizer=False, signatures=signatures
            )
        ...
        weights_name = (
            SAFE_WEIGHTS_NAME if safe_serialization else TF2_WEIGHTS_NAME
        )
        ...
        if index is None:
            if safe_serialization:
                ...
                safe_save_file(
                    state_dict, output_model_file, metadata={"format": "tf"}
                )
            else:
            ...
            save_index_file = os.path.join(
                save_directory, TF2_WEIGHTS_INDEX_NAME
            )
            ...
            logger.info(
                ...
                f" ({max_shard_size}) and is going to be split in"
                f" {len(shards)} checkpoint shards. You can find where each"
                " parameters has been saved in the index located at"
                f" {save_index_file}."
            )
    ...
    def from_pretrained(
        ...
        if pretrained_model_name_or_path is not None:
            ...
            if os.path.isdir(pretrained_model_name_or_path):
                if from_pt and os.path.isfile(
                    ...
                    is_sharded = True
                    raise NotImplementedError(
                        "Support for sharded checkpoints using safetensors is"
                        " coming soon!"
                    )
                elif os.path.isfile(
                    os.path.join(
                ...
                # At this stage we don't have a weight file so we will raise an error.
                    ...
                    raise EnvironmentError(
                        f"Error no file named {TF2_WEIGHTS_NAME} found in"
                        f" directory {pretrained_model_name_or_path} but there"
                        " is a file for PyTorch weights. Use `from_pt=True`"
                        " to load this model from those weights."
                    )
                else:
                    raise EnvironmentError(
                        f"Error no file named {TF2_WEIGHTS_NAME} or"
                        f" {WEIGHTS_NAME} found in directory"
                ...
                try:
                    ...
                    if (
                        ...
                        if resolved_archive_file is not None:
                            is_sharded = True
                            raise NotImplementedError(
                                "Support for sharded checkpoints using"
                                " safetensors is coming soon!"
                            )
                        else:
                    ...
                    if resolved_archive_file is None:
                        ...
                        if has_file(
                            ...
                            **has_file_kwargs,
                            ...
                            raise EnvironmentError(
                                f"{pretrained_model_name_or_path} does not"
                                " appear to have a file named"
                                f" {TF2_WEIGHTS_NAME} but there is a file for"
                                " PyTorch weights. Use `from_pt=True` to load"
                    ...
                    raise EnvironmentError(
                        ...
                        f" '{pretrained_model_name_or_path}'. If you were"
                        " trying to load it from"
                        " 'https://huggingface.co/models', make sure you"
                        " don't have a local directory with the same name."
                        " Otherwise, make sure"
                        f" '{pretrained_model_name_or_path}' is the correct"
                        " path to a directory containing a file named"
                        f" {TF2_WEIGHTS_NAME} or {WEIGHTS_NAME}."
                    )
            if is_local:
                ...
                logger.info(
                    f"loading weights file {filename} from cache at"
        ...
        if filename == SAFE_WEIGHTS_NAME:
            ...
            if safetensors_metadata is None or safetensors_metadata.get(
                ...
                raise OSError(
                    ...
                    f" {resolved_archive_file} does not contain the valid"
                    " metadata. Make sure you save your model with the"
        ...
        # pre-trained weights.
        if (
            cls._requires_load_weight_prefix
        ...
        try:
            ...
            try:
                with open(resolved_archive_file) as f:
                    if f.read().startswith("version"):
                        raise OSError(
                            "You seem to have cloned a repository without"
                            " having git-lfs installed. Please install git-lfs"
                            " and run `git lfs install` followed by `git lfs"
                ...
                raise OSError(
                    "Unable to load weights from h5 file. If you tried to load"
                    " a TF 2.0 model from a PyTorch checkpoint, please set"
                    " from_pt=True. "
                )
        ...
        if len(unexpected_keys) > 0:
            logger.warning(
                "Some layers from the model checkpoint at"
                f" {pretrained_model_name_or_path} were not used when"
                f" initializing {model.__class__.__name__}:"
                f" {unexpected_keys}\n- This IS expected if you are"
                f" initializing {model.__class__.__name__} from the checkpoint"
                " of a model trained on another task or with another"
                ...
                " checkpoint of a model that you expect to be exactly"
                " identical (initializing a BertForSequenceClassification"
        ...
        if len(missing_keys) > 0:
            logger.warning(
                f"Some layers of {model.__class__.__name__} were not"
                " initialized from the model checkpoint at"
                f" {pretrained_model_name_or_path} and are newly initialized:"
                f" {missing_keys}\nYou should probably TRAIN this model on a"
                " down-stream task to be able to use it for predictions and"
                " inference."
            )
        elif len(mismatched_keys) == 0:
            logger.warning(
                f"All the layers of {model.__class__.__name__} were"
                " initialized from the model checkpoint at"
                f" {pretrained_model_name_or_path}.\nIf your task is similar"
                " to the task the model of the checkpoint was trained on, you"
                f" can already use {model.__class__.__name__} for predictions"
    ...
    def push_to_hub(
        ...
        **model_card_kwargs,
        ...
        if "repo_path_or_name" in model_card_kwargs:
            warnings.warn(
                "The `repo_path_or_name` argument is deprecated and will be"
                " removed in v5 of Transformers. Use `repo_id` instead."
            )

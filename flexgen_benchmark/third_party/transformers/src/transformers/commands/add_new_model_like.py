...
def replace_model_patterns(
    ...
    if (
        old_model_patterns.model_upper_cased
    ...
    attributes_to_check.extend(
        ["model_camel_cased", "model_lower_cased", "model_name"]
    )
    ...
    for attr, placeholder in ATTRIBUTE_TO_PLACEHOLDER.items():
        if placeholder in text:
            replacements.append(
                (
                    getattr(old_model_patterns, attr),
                    getattr(new_model_patterns, attr),
                )
            )
...
def duplicate_module(
    ...
    content = re.sub(
        "# Copyright (\d+)\s", f"# Copyright {CURRENT_YEAR} ", content
    )
...
"""
        ...
        if (
            ...
            obj = add_content_to_text(
                obj,
                (
                    f"# Copied from {module_name}.{old_object_name} with"
                    f" {replacement}"
                ),
...
def filter_framework_files(
    ...
    return [
        framework_to_file[f] for f in frameworks if f in framework_to_file
    ] + others
...
def get_model_files(
    ...
    test_files = [
        REPO_PATH / "tests" / "models" / module_name / f for f in test_files
    ]
...
def find_base_model_checkpoint(
    model_type: str,
    model_files: Optional[Dict[str, Union[Path, List[Path]]]] = None,
) -> str:
    """
...
def retrieve_model_classes(
    ...
    for framework in frameworks:
        ...
        if modules[framework] is None:
            raise ValueError(
                f"You selected {framework} in the frameworks, but it is not"
                " installed."
            )
        model_mappings = [
            attr
...
def retrieve_info_for_model(
    model_type, frameworks: Optional[List[str]] = None
):
    """
    ...
    config_class = auto_module.configuration_auto.CONFIG_MAPPING_NAMES[
        model_type
    ]
    archive_map = (
        auto_module.configuration_auto.CONFIG_ARCHIVE_MAP_MAPPING_NAMES.get(
            model_type, None
        )
    )
    if model_type in auto_module.tokenization_auto.TOKENIZER_MAPPING_NAMES:
        tokenizer_classes = (
            auto_module.tokenization_auto.TOKENIZER_MAPPING_NAMES[model_type]
        )
        tokenizer_class = (
            tokenizer_classes[0]
    ...
    model_patterns = ModelPatterns(
        model_name,
        checkpoint=find_base_model_checkpoint(
            model_type, model_files=model_files
        ),
        model_type=model_type,
...
def clean_frameworks_in_init(
    ...
    names = {"pt": "torch"}
    to_remove = [
        names.get(f, f) for f in ["pt", "tf", "flax"] if f not in frameworks
    ]
    ...
    while idx < len(lines):
        ...
        if (re_conditional_imports.search(lines[idx]) is not None) and (
            ...
            # Iterate until `else:`
            while (
                is_empty_line(lines[idx]) or re_else.search(lines[idx]) is None
            ):
                idx += 1
            idx += 1
            indent = find_indent(lines[idx])
            while find_indent(lines[idx]) >= indent or is_empty_line(
                lines[idx]
            ):
                idx += 1
        ...
        # Otherwise we keep the line, except if it's a tokenizer import and we don't want to keep it.
            ...
            re.search(
                '^\s*"(tokenization|processing|feature_extraction)', lines[idx]
            )
            is None
            and re.search(
...
def add_model_to_main_init(
    ...
    while idx < len(lines):
        ...
        if (
            ...
            re.search(
                rf'models.{old_model_patterns.model_lower_cased}( |")',
                lines[idx],
            )
            is not None
        ):
            ...
            if not with_processing:
                ...
                processing_classes = [
                    c for c in processing_classes if c is not None
                ]
    ...
    for filename in AUTO_CLASSES_PATTERNS:
        ...
        for pattern in AUTO_CLASSES_PATTERNS[filename]:
            if re.search("any_([a-z]*)_class", pattern) is not None:
                framework = re.search("any_([a-z]*)_class", pattern).groups()[
                    0
                ]
                ...
                new_patterns.append(
                    pattern.replace(
                        "{config_class}", old_model_patterns.config_class
                    )
                )
        ...
        for pattern in new_patterns:
            ...
            add_content_to_file(
                full_name, new_model_line, add_after=old_model_line
            )
...
def duplicate_doc_file(
    ...
    if dest_file is None:
        dest_file = (
            Path(doc_file).parent / f"{new_model_patterns.model_type}.mdx"
        )
    ...
    for block in blocks:
        ...
        # The config starts the part of the doc with the classes.
            ...
            not in_classes
            and old_model_patterns.config_class in block.split("\n")[0]
        ):
            in_classes = True
            new_blocks.append(
                DOC_OVERVIEW_TEMPLATE.format(
                    model_name=new_model_patterns.model_name
                )
            )
...
def create_new_model_like(
    ...
    if len(old_model_patterns.checkpoint) == 0:
        raise ValueError(
            "The old model checkpoint could not be recovered from the model"
            " type. Please pass it to the `old_checkpoint` argument."
        )
    ...
    module_folder = (
        TRANSFORMERS_PATH / "models" / new_model_patterns.model_lower_cased
    )
    ...
    tests_folder = (
        REPO_PATH / "tests" / "models" / new_model_patterns.model_lower_cased
    )
    ...
    if disabled_fx_test:
        print(
            "The tests for symbolic tracing with torch.fx were disabled, you"
            " can add those once symbolic tracing works for your new model."
        )

    # 4. Add model to auto classes
    add_model_to_auto_classes(
        old_model_patterns, new_model_patterns, model_classes
    )
    ...
    if old_model_patterns.model_type == old_model_patterns.checkpoint:
        print(
            "The model you picked has the same name for the model type and"
            f" the checkpoint name ({old_model_patterns.model_type}). As a"
            " result, it's possible some places where the new checkpoint"
            f" should be, you have {new_model_patterns.model_type} instead."
            " You should search for all instances of"
            f" {new_model_patterns.model_type} in the new files and check"
            " they're not badly used as checkpoints."
        )
    elif old_model_patterns.model_lower_cased == old_model_patterns.checkpoint:
        print(
            "The model you picked has the same name for the model type and"
            f" the checkpoint name ({old_model_patterns.model_lower_cased})."
            " As a result, it's possible some places where the new checkpoint"
            " should be, you have"
            f" {new_model_patterns.model_lower_cased} instead. You should"
            " search for all instances of"
            f" {new_model_patterns.model_lower_cased} in the new files and"
            " check they're not badly used as checkpoints."
        )
    if (
        old_model_patterns.model_type == old_model_patterns.model_lower_cased
        and new_model_patterns.model_type
        != new_model_patterns.model_lower_cased
    ):
        print(
            "The model you picked has the same name for the model type and"
            " the lowercased model name"
            f" ({old_model_patterns.model_lower_cased}). As a result, it's"
            " possible some places where the new model type should be, you"
            f" have {new_model_patterns.model_lower_cased} instead. You should"
            " search for all instances of"
            f" {new_model_patterns.model_lower_cased} in the new files and"
            " check they're not badly used as the model type."
        )

    if (
        not keep_old_processing
        and old_model_patterns.tokenizer_class is not None
    ):
        print(
            "The constants at the start of the new tokenizer file created"
            " needs to be manually fixed. If your new model has a tokenizer"
            " fast, you will also need to manually add the converter in the"
            " `SLOW_TO_FAST_CONVERTERS` constant of"
...
class AddNewModelLikeCommand(BaseTransformersCLICommand):
    ...
    def register_subcommand(parser: ArgumentParser):
        ...
        add_new_model_like_parser.add_argument(
            ...
            type=str,
            help=(
                "When not using an editable install, the path to the"
                " Transformers repo."
            ),
        )
        add_new_model_like_parser.set_defaults(
...
def get_user_input():
    ...
    """
    model_types = list(
        auto_module.configuration_auto.MODEL_NAMES_MAPPING.keys()
    )
    ...
    while not valid_model_type:
        old_model_type = input(
            "What is the model you would like to duplicate? Please provide the"
            " lowercase `model_type` (e.g. roberta): "
        )
        if old_model_type in model_types:
            valid_model_type = True
        else:
            print(f"{old_model_type} is not a valid model type.")
            near_choices = difflib.get_close_matches(
                old_model_type, model_types
            )
    ...
    if len(old_model_info["model_patterns"].checkpoint) == 0:
        old_checkpoint = get_user_field(
            "We couldn't find the name of the base checkpoint for that model,"
            " please enter it here."
        )

    model_name = get_user_field(
        "What is the name (with no special casing) for your new model in the"
        " paper (e.g. RoBERTa)? "
    )
    ...
    model_type = get_user_field(
        (
            "What identifier would you like to use for the `model_type` of"
            " this model? "
        ),
    ...
    model_lower_cased = get_user_field(
        (
            "What lowercase name would you like to use for the module (folder)"
            " of this model? "
        ),
    ...
    model_camel_cased = get_user_field(
        (
            "What prefix (camel-cased) would you like to use for the model"
            " classes of this model (e.g. Roberta)? "
        ),
        default_value=default_patterns.model_camel_cased,
    )
    model_upper_cased = get_user_field(
        (
            "What prefix (upper-cased) would you like to use for the constants"
    ...
    checkpoint = get_user_field(
        "Please give a checkpoint identifier (on the model Hub) for this new"
        " model (e.g. facebook/roberta-base): "
    )
    ...
    keep_processing = get_user_field(
        (
            "Will your new model use the same processing class as"
            f" {old_model_type} ({old_processing_classes}) (yes/no)? "
        ),
    ...
    if keep_processing:
        ...
        if old_tokenizer_class is not None:
            tokenizer_class = get_user_field(
                (
                    "What will be the name of the tokenizer class for this"
                    " model? "
                ),
        ...
        if old_feature_extractor_class is not None:
            feature_extractor_class = get_user_field(
                (
                    "What will be the name of the feature extractor class for"
                    " this model? "
                ),
        ...
        if old_processor_class is not None:
            processor_class = get_user_field(
                (
                    "What will be the name of the processor class for this"
                    " model? "
                ),
    ...
    add_copied_from = get_user_field(
        (
            "Should we add # Copied from statements when creating the new"
            " modeling file (yes/no)? "
        ),
        convert_to=convert_to_bool,
        default_value="yes",
        fallback_message="Please answer yes/no, y/n, true/false or 1/0.",
    ...
    all_frameworks = get_user_field(
        (
            "Should we add a version of your new model in all the frameworks"
            f" implemented by {old_model_type} ({old_frameworks}) (yes/no)? "
        ),
        convert_to=convert_to_bool,
        default_value="yes",
        fallback_message="Please answer yes/no, y/n, true/false or 1/0.",
    ...
    if all_frameworks:
        ...
        frameworks = get_user_field(
            (
                "Please enter the list of framworks you want (pt, tf, flax)"
                " separated by spaces"
            ),

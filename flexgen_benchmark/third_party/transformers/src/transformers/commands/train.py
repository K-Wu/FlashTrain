...
if not is_tf_available() and not is_torch_available():
    raise RuntimeError(
        "At least one of PyTorch or TensorFlow 2.0+ should be installed to use"
        " CLI training"
    )
...
class TrainCommand(BaseTransformersCLICommand):
    ...
    def register_subcommand(parser: ArgumentParser):
        ...
        train_parser.add_argument(
            ...
            required=True,
            help=(
                "path to train (and optionally evaluation) dataset as a csv"
                " with tab separated labels and sentences."
            ),
        )
        train_parser.add_argument(
            "--column_label",
        ...
        train_parser.add_argument(
            ...
            default=0.1,
            help=(
                "if validation dataset is not provided, fraction of train"
    ...
    def __init__(self, args: Namespace):
        ...
        if args.validation_data:
            self.logger.info(
                f"Loading validation dataset from {args.validation_data}"
            )

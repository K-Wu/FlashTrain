...
class AddNewModelCommand(BaseTransformersCLICommand):
    ...
    def register_subcommand(parser: ArgumentParser):
        ...
        add_new_model_parser.add_argument(
            ...
            type=str,
            help=(
                "Path to cookiecutter. Should only be used for testing"
                " purposes."
            ),
        )
    ...
    def run(self):
        warnings.warn(
            "The command `transformers-cli add-new-model` is deprecated and"
            " will be removed in v5 of Transformers. It is not actively"
            " maintained anymore, so might give a result that won't pass all"
            " tests and quality checks, you should use `transformers-cli"
            " add-new-model-like` instead."
        )
        if not _has_cookiecutter:
            raise ImportError(
                "Model creation dependencies are required to use the"
                " `add_new_model` command. Install them by running the"
                " following at the root of your `transformers` clone:\n\n\t$"
        ...
        if len(directories) > 0:
            raise ValueError(
                "Several directories starting with `cookiecutter-template-` in"
                " current working directory. Please clean your directory by"
                " removing all folders starting with `cookiecutter-template-`"
        ...
        if not self._testing:
            ...
            cookiecutter(
                str(
                    path_to_cookiecutter if self._path is None else self._path
                ),
                no_input=True,
        ...
        with open(
            directory + "/configuration.json", "r"

...
class PTtoTFCommand(BaseTransformersCLICommand):
    ...
    def register_subcommand(parser: ArgumentParser):
        ...
        train_parser = parser.add_parser(
            "pt-to-tf",
            help=(
                "CLI tool to run convert a transformers model from a PyTorch"
                " checkpoint to a TensorFlow checkpoint. Can also be used to"
                " validate existing weights without opening PRs, with --no-pr."
            ),
        )
        train_parser.add_argument(
            ...
            required=True,
            help=(
                "The model name, including owner/organization, as seen on the"
                " hub."
            ),
        )
        train_parser.add_argument(
            ...
            default="",
            help=(
                "Optional local directory of the model repository. Defaults to"
                " /tmp/{model_name}"
            ),
        )
        train_parser.add_argument(
            ...
            default=MAX_ERROR,
            help=(
                f"Maximum error tolerance. Defaults to {MAX_ERROR}. This flag"
        ...
        train_parser.add_argument(
            ...
            action="store_true",
            help=(
                "Optional flag to create new TensorFlow weights, even if they"
                " already exist."
            ),
        )
        train_parser.add_argument(
        ...
        train_parser.add_argument(
            ...
            action="store_true",
            help=(
                "Optional flag to push the weights directly to `main`"
        ...
        train_parser.add_argument(
            ...
            default="",
            help=(
                "Optional additional commit description to use when opening a"
                " PR (e.g. to tag the owner)."
            ),
        )
    ...
    def find_pt_tf_differences(pt_outputs, tf_outputs):
        ...
        if pt_out_attrs != tf_out_attrs:
            raise ValueError(
                "The model outputs have different attributes, aborting."
                f" (Pytorch: {pt_out_attrs}, TensorFlow: {tf_out_attrs})"
            )
        ...
        def _find_pt_tf_differences(pt_out, tf_out, differences, attr_name=""):
            ...
            if isinstance(pt_out, torch.Tensor):
                tensor_difference = np.max(
                    np.abs(pt_out.numpy() - tf_out.numpy())
                )
    ...
    def __init__(
        ...
        *args,
        ...
        self._local_dir = (
            local_dir if local_dir else os.path.join("/tmp", model_name)
        )
    ...
    def get_inputs(self, pt_model, config):
            ...
            processor_inputs.update(
                {
                    "text": [
                        "Hi there!",
                        (
                            "I am a batch with more than one row and different"
                            " input lengths."
                        ),
                    ],
                    "padding": True,
        ...
        if "pixel_values" in model_forward_signature:
            sample_images = load_dataset(
                "cifar10", "plain_text", split="test"
            )[:2]["img"]
        ...
        if "input_features" in model_forward_signature:
            ...
            if "padding" in feature_extractor_signature:
                default_strategy = feature_extractor_signature[
                    "padding"
                ].default
                if (
                    default_strategy is not False
                    and default_strategy is not None
                ):
        ...
        if "input_values" in model_forward_signature:  # Wav2Vec2 audio input
            processor_inputs.update(
                {"audio": _get_audio_input(), "padding": True}
            )
    ...
    def run(self):
        ...
        if version.parse(huggingface_hub.__version__) < version.parse("0.9.0"):
            raise ImportError(
                "The huggingface_hub version must be >= 0.9.0 to use this"
                " command. Please update your huggingface_hub installation."
            )
        else:
        ...
        # Fetch remote data
        repo = Repository(
            local_dir=self._local_dir, clone_from=self._model_name
        )
        ...
        if (
            ...
            if len(architectures) > 1:
                raise ValueError(
                    "More than one architecture was found, aborting."
            ...
            try:
                ...
                raise AttributeError(
                    f"The TensorFlow equivalent of {architectures[0]} doesn't"
        ...
        tf_from_pt_model = tf_class.from_pretrained(
        ...
        tf_from_pt_outputs = tf_from_pt_model(
            **tf_input, output_hidden_states=True
        )
        ...
        if len(output_differences) == 0 and architectures is not None:
            raise ValueError(
                ...
                f" ({architectures}), but no model head output was found. All"
                " outputs start with 'hidden'"
            )
        ...
        if (
            ...
            raise ValueError(
                "The cross-loaded TensorFlow model has different outputs,"
                ...
                + "\n\nList of maximum hidden layer differences above the"
                f" threshold ({self._max_error}):\n"
                + "\n".join(
                    [
                        f"{k}: {v:.3e}"
        ...
        tf_weights_index_path = os.path.join(
            self._local_dir, TF2_WEIGHTS_INDEX_NAME
        )
        if (
        ...
        output_differences = {
            k: v
            for k, v in conversion_differences.items()
            if "hidden" not in k
        }
        hidden_differences = {
        ...
        if (
            ...
            raise ValueError(
                "The converted TensorFlow model has different outputs,"
                " something went wrong!\n"
                + "\nList of maximum output differences above the threshold"
                f" ({self._max_error}):\n"
                + "\n".join(
        ...
        commit_message = (
            "Update TF weights" if self._new_weights else "Add TF weights"
        )
        if self._push:
            ...
            commit_descrition = (
                ...
                " All converted model outputs and hidden layers were"
                " validated against its Pytorch counterpart.\n\nMaximum"
                " crossload output"
                f" difference={max_crossload_output_diff:.3e}; Maximum"
                " crossload hidden layer"
                f" difference={max_crossload_hidden_diff:.3e};\nMaximum"
                " conversion output"
                f" difference={max_conversion_output_diff:.3e}; Maximum"
                " conversion hidden layer"
                f" difference={max_conversion_hidden_diff:.3e};\n"
            )
            if self._max_error > MAX_ERROR:
                commit_descrition += (
                    "\n\nCAUTION: The maximum admissible error was manually"
                    f" increased to {self._max_error}!"
                )
            ...
            if os.path.exists(tf_weights_index_path):
                ...
                for shard_path in tf.io.gfile.glob(
                    self._local_dir + "/tf_model-*.h5"
                ):
                    operations += [
                        CommitOperationAdd(

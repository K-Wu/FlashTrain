...
class ConvertCommand(BaseTransformersCLICommand):
    ...
    def register_subcommand(parser: ArgumentParser):
        ...
        train_parser = parser.add_parser(
            "convert",
            help=(
                "CLI tool to run convert model from original author"
        ...
        train_parser.add_argument(
            ...
            default=None,
            help=(
                "Optional fine-tuning task name if the TF model was a"
                " finetuned model."
            ),
        )
    ...
    def run(self):
        if self._model_type == "albert":
            ...
            raise ValueError(
                "--model_type should be selected in the list [bert, gpt, gpt2,"
                " t5, transfo_xl, xlnet, xlm, lxmert]"
            )

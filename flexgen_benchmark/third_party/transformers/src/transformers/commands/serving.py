...
class ServeCommand(BaseTransformersCLICommand):
    ...
    def register_subcommand(parser: ArgumentParser):
        ...
        serve_parser = parser.add_parser(
            "serve",
            help=(
                "CLI tool to run inference requests through REST and GraphQL"
                " endpoints."
            ),
        )
    ...
    def __init__(self, pipeline: Pipeline, host: str, port: int, workers: int):
        ...
        if not _serve_dependencies_installed:
            raise RuntimeError(
                "Using serve command requires FastAPI and uvicorn. Please"
                " install transformers with [serving]: pip install"
                ' "transformers[serving]".Or install FastAPI and uvicorn'
                " separately."
            )
        else:
    ...
    def tokenize(
        ...
        try:
            ...
            if return_ids:
                ...
                return ServeTokenizeResult(
                    tokens=tokens_txt, tokens_ids=tokens_ids
                )
            else:
            ...
            raise HTTPException(
                status_code=500, detail={"model": "", "error": str(e)}
            )

...
class TrainingArguments:
    ...
    prediction_loss_only: bool = field(
        ...
        metadata={
            "help": (
                "When performing evaluation and predictions, only returns the"
                " loss."
            )
        },
    )
    ...
    per_gpu_train_batch_size: Optional[int] = field(
        ...
        metadata={
            "help": (
                "Deprecated, the use of `--per_device_train_batch_size` is"
    ...
    eval_accumulation_steps: Optional[int] = field(
        ...
        metadata={
            "help": (
                "Number of predictions steps to accumulate before moving the"
                " tensors to the CPU."
            )
        },
    )
    ...
    log_level: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Logger log level to use on the main node. Possible choices"
                " are the log levels as strings: 'debug', 'info', 'warning',"
                " 'error' and 'critical', plus a 'passive' level which doesn't"
                " set anything and lets the application set the level."
                " Defaults to 'passive'."
            ),
    ...
    log_level_replica: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Logger log level to use on replica nodes. Same choices and"
    ...
    save_total_limit: Optional[int] = field(
        ...
        metadata={
            "help": (
                ...
                " checkpoints in the output_dir. Default is unlimited"
                " checkpoints"
            )
        },
    )
    save_on_each_node: bool = field(
        ...
        metadata={
            "help": (
                "When doing multi-node distributed training, whether to save"
    ...
    use_mps_device: bool = field(
        default=False,
        metadata={
            "help": "Whether to use Apple Silicon chip based `mps` device."
        },
    )
    seed: int = field(
        default=42,
    ...
    bf16: bool = field(
        ...
        metadata={
            "help": (
                ...
                " Requires Ampere or higher NVIDIA architecture or using CPU"
                " (no_cuda). This is an experimental API and it may change."
            )
        },
    )
    fp16: bool = field(
    ...
    fp16_opt_level: str = field(
        ...
        metadata={
            "help": (
                "For fp16: Apex AMP optimization level selected in ['O0',"
                " 'O1', 'O2', and 'O3']. See details at"
    ...
    tf32: Optional[bool] = field(
        ...
        metadata={
            "help": (
                "Whether to enable tf32 mode, available in Ampere and newer"
                " GPU architectures. This is an experimental API and it may"
                " change."
            )
        },
    )
    ...
    tpu_num_cores: Optional[int] = field(
        ...
        metadata={
            "help": (
                "TPU: Number of TPU cores (automatically passed by launcher"
                " script)"
            )
        },
    )
    ...
    debug: str = field(
        ...
        metadata={
            "help": (
                ...
                " activations and weights), `tpu_metrics_debug` (print debug"
                " metrics on TPU)."
            )
        },
    )
    ...
    dataloader_num_workers: int = field(
        ...
        metadata={
            "help": (
                "Number of subprocesses to use for data loading (PyTorch"
                " only). 0 means that the data will be loaded in the main"
                " process."
            )
        },
    )
    ...
    past_index: int = field(
        ...
        metadata={
            "help": (
                "If >=0, uses the corresponding part of the output as the past"
                " state for next step."
            )
        },
    )
    ...
    run_name: Optional[str] = field(
        ...
        metadata={
            "help": (
                "An optional descriptor for the run. Notably used for wandb"
                " logging."
            )
        },
    )
    ...
    label_names: Optional[List[str]] = field(
        ...
        metadata={
            "help": (
                "The list of keys in your dictionary of inputs that correspond"
                " to the labels."
            )
        },
    )
    ...
    greater_is_better: Optional[bool] = field(
        ...
        metadata={
            "help": (
                "Whether the `metric_for_best_model` should be maximized or"
                " not."
            )
        },
    )
    ignore_data_skip: bool = field(
        ...
        metadata={
            "help": (
                "When resuming training, whether or not to skip the first"
                " epochs and batches to get to the same training data."
            )
        },
    )
    sharded_ddp: str = field(
        ...
        metadata={
            "help": (
                "Whether or not to use sharded DDP training (in distributed"
                " training only). The base option should be `simple`,"
                " `zero_dp_2` or `zero_dp_3` and you can add CPU-offload to"
                " `zero_dp_2` or `zero_dp_3` like this: zero_dp_2 offload` or"
                " `zero_dp_3 offload`. You can add auto-wrap to `zero_dp_2` or"
                " `zero_dp_3` with the same syntax: zero_dp_2 auto_wrap` or"
                " `zero_dp_3 auto_wrap`."
            ),
        },
    )
    fsdp: str = field(
        ...
        metadata={
            "help": (
                ...
                " (FSDP) training (in distributed training only). The base"
                " option should be `full_shard`, `shard_grad_op` or `no_shard`"
                " and you can add CPU-offload to `full_shard` or"
                " `shard_grad_op` like this: full_shard offload` or"
                " `shard_grad_op offload`. You can add auto-wrap to"
                " `full_shard` or `shard_grad_op` with the same syntax:"
                " full_shard auto_wrap` or `shard_grad_op auto_wrap`."
            ),
        },
    )
    ...
    fsdp_transformer_layer_cls_to_wrap: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Transformer layer class name (case-sensitive) to wrap ,e.g,"
                " `BertLayer`, `GPTJBlock`, `T5Block` .... (useful only when"
                " `fsdp` flag is passed)."
            )
        },
    )
    deepspeed: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Enable deepspeed and pass the path to deepspeed json config"
                " file (e.g. ds_config.json) or an already loaded json file as"
                " a dict"
            )
        },
    )
    label_smoothing_factor: float = field(
        ...
        metadata={
            "help": (
                "The label smoothing epsilon to apply (zero means no label"
                " smoothing)."
            )
        },
    )
    ...
    group_by_length: bool = field(
        ...
        metadata={
            "help": (
                "Whether or not to group samples of roughly the same length"
    ...
    length_column_name: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Column name with precomputed lengths to use when grouping by"
                " length."
            )
        },
    )
    ...
    skip_memory_metrics: bool = field(
        ...
        metadata={
            "help": (
                "Whether or not to skip adding of memory profiler reports to"
                " metrics."
            )
        },
    )
    use_legacy_prediction_loop: bool = field(
        ...
        metadata={
            "help": (
                "Whether or not to use the legacy prediction_loop in the"
                " Trainer."
            )
        },
    )
    push_to_hub: bool = field(
        ...
        metadata={
            "help": (
                "Whether or not to upload the trained model to the model hub"
                " after training."
            )
        },
    )
    ...
    push_to_hub_model_id: Optional[str] = field(
        default=None,
        metadata={
            "help": "The name of the repository to which push the `Trainer`."
        },
    )
    ...
    mp_parameters: str = field(
        ...
        metadata={
            "help": (
                "Used by the SageMaker launcher to send mp-specific args."
                " Ignored in Trainer"
            )
        },
    )
    ...
    auto_find_batch_size: bool = field(
        ...
        metadata={
            "help": (
                "Whether to automatically decrease the batch size in half and"
                " rerun the training loop again each time a CUDA Out-of-Memory"
                " was reached"
            )
        },
    )
    full_determinism: bool = field(
        ...
        metadata={
            "help": (
                "Whether to call enable_full_determinism instead of set_seed"
    ...
    torchdynamo: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Sets up the backend compiler for TorchDynamo. TorchDynamo is"
                " a Python level JIT compiler designed to make unmodified"
                " PyTorch programs faster. TorchDynamo dynamically modifies"
                " the Python bytecode right before its executed. It rewrites"
                " Python bytecode to extract sequences of PyTorch operations"
                " and lifts them up into Fx graph. We can then pass these Fx"
                " graphs to other backend compilers. There are two options -"
                " eager and nvfuser. Eager defaults to pytorch eager and is"
                " useful for debugging. nvfuser path uses AOT Autograd and"
                " nvfuser compiler to optimize the models."
            ),
    ...
    ray_scope: Optional[str] = field(
        ...
        metadata={
            "help": (
                "The scope to use when doing hyperparameter search with Ray."
                ' By default, `"last"` will be used. Ray will then use the'
                " last checkpoint of all trials, compare those, and select the"
                " best one. However, other options are also available. See the"
                " Ray documentation"
    ...
    ddp_timeout: Optional[int] = field(
        ...
        metadata={
            "help": (
                "Overrides the default timeout for distributed training (value"
    ...
    def __post_init__(self):
        ...
        if isinstance(self.evaluation_strategy, EvaluationStrategy):
            warnings.warn(
                (
                    "using `EvaluationStrategy` for `evaluation_strategy` is"
                    " deprecated and will be removed in version 5 of 🤗"
                    " Transformers. Use `IntervalStrategy` instead"
                ),
                FutureWarning,
            )
        ...
        if self.evaluation_strategy == IntervalStrategy.STEPS and (
            ...
            if self.logging_steps > 0:
                logger.info(
                    "using `logging_steps` to initialize `eval_steps` to"
        ...
        if (
            ...
            raise ValueError(
                f"logging strategy {self.logging_strategy} requires non-zero"
                " --logging_steps"
            )
        ...
        if self.load_best_model_at_end:
            if self.evaluation_strategy != self.save_strategy:
                raise ValueError(
                    ...
                    " strategy to match, but found\n- Evaluation strategy:"
                    f" {self.evaluation_strategy}\n- Save strategy:"
            ...
            if (
                ...
                raise ValueError(
                    ...
                    " a round multiple of the evaluation steps, but found"
                    f" {self.save_steps}, which is not a round multiple of"
                    f" {self.eval_steps}."
                )
        ...
        if self.framework == "pt" and is_torch_available():
            if self.fp16_backend and self.fp16_backend != "auto":
                warnings.warn(
                    (
                        "`fp16_backend` is deprecated and will be removed in"
                        " version 5 of 🤗 Transformers. Use"
            ...
            if self.bf16 or self.bf16_full_eval:
                if self.no_cuda and not is_torch_bf16_cpu_available():
                    # cpu
                    raise ValueError(
                        "Your setup doesn't support bf16/cpu. You need"
                        " torch>=1.10"
                    )
                elif not self.no_cuda and not is_torch_bf16_gpu_available():
                    # gpu
                    raise ValueError(
                        "Your setup doesn't support bf16/gpu. You need"
                        " torch>=1.10, using Ampere GPU with cuda>=11.0"
                    )
        ...
        if self.fp16_full_eval and self.bf16_full_eval:
            raise ValueError(
                "At most one of fp16 and bf16 can be True for full eval, but"
                " not both"
            )
        ...
        if self.bf16:
            if self.half_precision_backend == "apex":
                raise ValueError(
                    " `--half_precision_backend apex`: GPU bf16 is not"
                    " supported by apex. Use `--half_precision_backend"
                    " cuda_amp` instead"
                )
        ...
        if self.adafactor:
            warnings.warn(
                (
                    "`--adafactor` is deprecated and will be removed in"
                    " version 5 of 🤗 Transformers. Use `--optim adafactor`"
                    " instead"
                ),
                FutureWarning,
            )
        ...
        if (
            ...
            raise ValueError(
                "FP16 Mixed precision training with AMP or APEX (`--fp16`) and"
                " FP16 half precision evaluation (`--fp16_full_eval`) can only"
        ...
        if (
            ...
            raise ValueError(
                "BF16 Mixed precision training with AMP (`--bf16`) and BF16"
                " half precision evaluation (`--bf16_full_eval`) can only be"
                " used on CUDA or CPU devices."
            )

        if (
            self.framework == "pt"
            and is_torch_available()
            and self.tf32 is not None
        ):
            if self.tf32:
                if is_torch_tf32_available():
                    ...
                    raise ValueError(
                        "--tf32 requires Ampere or a newer GPU arch, cuda>=11"
                        " and torch>=1.7"
                    )
            else:
                if is_torch_tf32_available():
        ...
        if self.report_to is None:
            logger.info(
                "The default value for the training argument `--report_to`"
                " will change in v5 (from all installed integrations to none)."
                " In v5, you will need to use `--report_to all` to get the"
                " same behavior as now. You should start updating your code"
        ...
        if self.warmup_ratio < 0 or self.warmup_ratio > 1:
            ...
            logger.info(
                "Both warmup_ratio and warmup_steps given, warmup_steps will"
                " override any effect of warmup_ratio during training"
            )
        ...
        if isinstance(self.sharded_ddp, str):
            self.sharded_ddp = [
                ShardedDDPOption(s) for s in self.sharded_ddp.split()
            ]
        if self.sharded_ddp == [ShardedDDPOption.OFFLOAD]:
            raise ValueError(
                "`--sharded_ddp offload` can't work on its own. It needs to"
                " be added to `--sharded_ddp zero_dp_2` or `--sharded_ddp"
                ' zero_dp_3`. For example, `--sharded_ddp "zero_dp_2'
                ' offload"`.'
            )
        elif (
            len(self.sharded_ddp) > 1
            and ShardedDDPOption.SIMPLE in self.sharded_ddp
        ):
            raise ValueError(
                "`--sharded_ddp simple` is not compatible with any other"
                " option."
            )
        elif (
            ...
            raise ValueError(
                "`--sharded_ddp zero_dp_2` is not compatible with"
        ...
        if self.fsdp == [FSDPOption.OFFLOAD]:
            raise ValueError(
                "`--fsdp offload` can't work on its own. It needs to be added"
                " to `--fsdp full_shard` or `--fsdp shard_grad_op`. For"
                ' example, `--fsdp "full_shard offload"`.'
            )
        elif (
            FSDPOption.FULL_SHARD in self.fsdp
            and FSDPOption.SHARD_GRAD_OP in self.fsdp
        ):
            raise ValueError(
                "`--fsdp full_shard` is not compatible with `--fsdp"
                " shard_grad_op`."
            )

        if len(self.fsdp) == 0 and self.fsdp_min_num_params > 0:
            warnings.warn(
                "`--fsdp_min_num_params` is useful only when `--fsdp` is"
                " specified."
            )
        ...
        if (
            ...
            warnings.warn(
                "`--fsdp_transformer_layer_cls_to_wrap` is useful only when"
                " `--fsdp` is specified."
            )
        ...
        if (
            ...
            raise ValueError(
                "`--fsdp_min_num_params` and"
                " `--fsdp_transformer_layer_cls_to_wrap` are mutually"
                " exclusive."
            )
        ...
        if self.tpu_metrics_debug:
            warnings.warn(
                (
                    "using `--tpu_metrics_debug` is deprecated and will be"
                    " removed in version 5 of 🤗 Transformers. Use `--debug"
                    " tpu_metrics_debug` instead"
                ),
                FutureWarning,
            )
        ...
        if self.deepspeed:
            ...
            if not is_accelerate_available():
                raise ValueError(
                    "--deepspeed requires Accelerate to be installed: `pip"
                    " install accelerate`."
                )
        ...
        if self.push_to_hub_token is not None:
            warnings.warn(
                (
                    "`--push_to_hub_token` is deprecated and will be removed"
                    " in version 5 of 🤗 Transformers. Use `--hub_token`"
                    " instead."
                ),
                FutureWarning,
            )
        ...
        if self.push_to_hub_model_id is not None:
            ...
            if self.push_to_hub_organization is not None:
                warnings.warn(
                    (
                        "`--push_to_hub_model_id` and"
                        " `--push_to_hub_organization` are deprecated and"
                        " will be removed in version 5 of 🤗 Transformers."
                        " Use `--hub_model_id` instead and pass the full repo"
                        " name to this argument (in this case"
                        f" {self.hub_model_id})."
                    ),
                    FutureWarning,
                )
                ...
                warnings.warn(
                    (
                        "`--push_to_hub_model_id` is deprecated and will be"
                        " removed in version 5 of 🤗 Transformers. Use"
                        " `--hub_model_id` instead and pass the full repo"
                        " name to this argument (in this case"
                        f" {self.hub_model_id})."
                    ),
                    FutureWarning,
            ...
            warnings.warn(
                (
                    "`--push_to_hub_organization` is deprecated and will be"
                    " removed in version 5 of 🤗 Transformers. Use"
                    " `--hub_model_id` instead and pass the full repo name to"
    ...
    def train_batch_size(self) -> int:
        ...
        if self.per_gpu_train_batch_size:
            logger.warning(
                "Using deprecated `--per_gpu_train_batch_size` argument which"
    ...
    def eval_batch_size(self) -> int:
        ...
        if self.per_gpu_eval_batch_size:
            logger.warning(
                "Using deprecated `--per_gpu_eval_batch_size` argument which"
                " will be removed in a future version. Using"
                " `--per_device_eval_batch_size` is preferred."
            )
        per_device_batch_size = (
    ...
    def _setup_devices(self) -> "torch.device":
        ...
        if (
            ...
            logger.warning(
                "torch.distributed process group is initialized, but"
                " local_rank == -1. In order to use Torch DDP, launch your"
                " script with `python -m torch.distributed.launch"
            )
        if self.no_cuda:
            ...
            if (
                ...
                if self.xpu_backend == "ccl":
                    ...
                    if int(os.environ.get("CCL_WORKER_COUNT", 0)) < 1:
                        raise ValueError(
                            ...
                            " CCL_WORKER_COUNT is not correctly set. Please"
                            " use like 'export CCL_WORKER_COUNT = 1' to set."
                        )
                ...
                if not os.environ.get("MASTER_ADDR", None):
                    if local_size != size or self.xpu_backend != "mpi":
                        raise ValueError(
                            "Looks like distributed multinode run but"
                            " MASTER_ADDR env not set, please try exporting"
                            " rank 0's hostname as MASTER_ADDR"
                        )
                if (
                    torch.get_num_threads() == 1
                    and get_int_from_env(
                        ["OMP_NUM_THREADS", "MKL_NUM_THREADS"], 0
                    )
                    == 0
                    ...
                    logger.info(
                        "num_cpu_threads_per_process unset, we set it at"
                        f" {num_cpu_threads_per_process} to improve oob"
                        " performance."
                    )
                ...
                if not torch.backends.mps.is_available():
                    if not torch.backends.mps.is_built():
                        raise AssertionError(
                            ...
                            " install was not built with MPS enabled. Please"
                            " install torch version >=1.12.0 on your Apple"
                            " silicon Mac running macOS 12.3 or later with a"
                            " native version (arm64) of Python"
                        )
                    else:
                        raise AssertionError(
                            "MPS not available because the current MacOS"
                            " version is not 12.3+ and/or you do not have an"
                    ...
                    if not version.parse(
                        ...
                        warnings.warn(
                            "We strongly recommend to install PyTorch >= 1.13"
                            " (nightly version at the time of writing) on your"
                            " MacOS machine. It has major fixes related to"
                            " model correctness and performance improvements"
                            " for transformer based models. Please refer to"
    ...
    def world_size(self):
        ...
        if is_torch_tpu_available():
            ...
            return (
                smp.dp_size()
                if not smp.state.cfg.prescaled_batch
                else smp.rdp_size()
            )
        elif is_sagemaker_dp_enabled():
    ...
    def get_process_log_level(self):
        ...
        return (
            log_level_main_node if self.should_log else log_level_replica_node
        )
        ...
        if is_torch_available() and self.world_size > 1:
            ...
            try:
                if not is_main_process:
                    ...
                    logger.debug(
                        f"{self.process_index}: waiting for the"
                ...
                if is_main_process:
                    # the wait is over
                    logger.debug(
                        f"{self.process_index}: {main_process_desc} completed"
                        f" {desc}, releasing all replicas"
                    )
                    if is_torch_tpu_available():
                        xm.rendezvous(desc)
                    elif is_sagemaker_dp_enabled():

        ...
        **generate_kwargs
        ...
        if handle_long_generation is not None:
            if handle_long_generation not in {"hole"}:
                raise ValueError(
                    f"{handle_long_generation} is not a valid value for"
                    " `handle_long_generation` parameter expected [None,"
                    " 'hole']"
                )
            preprocess_params[
        ...
        **generate_kwargs
        ...
        if handle_long_generation == "hole":
            ...
            if cur_len + new_tokens > self.tokenizer.model_max_length:
                ...
                if keep_length <= 0:
                    raise ValueError(
                        "We cannot use `hole` to handle this generation the"
                        " number of desired tokens exceeds the models max"
                        " length"
                    )

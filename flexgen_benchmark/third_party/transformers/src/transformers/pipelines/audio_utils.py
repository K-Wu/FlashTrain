...
def ffmpeg_read(bpayload: bytes, sampling_rate: int) -> np.array:
    ...
    try:
        ...
        raise ValueError(
            "ffmpeg was not found but is required to load audio files from"
            " filename"
        ) from error
...
def ffmpeg_microphone(
    ...
    if format_for_conversion == "s16le":
        ...
        raise ValueError(
            f"Unhandled format `{format_for_conversion}`. Please use `s16le`"
            " or `f32le`"
        )
...
def ffmpeg_microphone_live(
    ...
    stride_left = (
        int(round(sampling_rate * stride_length_s[0])) * size_of_sample
    )
    stride_right = (
        int(round(sampling_rate * stride_length_s[1])) * size_of_sample
    )
...
def chunk_bytes_iter(
    ...
    if stride_left + stride_right >= chunk_len:
        raise ValueError(
            "Stride needs to be strictly smaller than chunk_len:"
            f" ({stride_left}, {stride_right}) vs {chunk_len}"
        )
    _stride_left = 0
    for raw in iterator:
        acc += raw
...
def _ffmpeg_stream(ffmpeg_command, buflen: int):
    ...
    try:
        ...
        raise ValueError(
            "ffmpeg was not found but is required to stream audio files from"
            " filename"
        ) from error

...
SUPPORTED_TASKS = {
    "audio-classification": {
        ...
        "default": {
            "model": {"pt": ("superb/wav2vec2-base-superb-ks", "372e048")}
        },
        "type": "audio",
    },
    "automatic-speech-recognition": {
        ...
        "default": {
            "model": {"pt": ("facebook/wav2vec2-base-960h", "55bb623")}
        },
        "type": "multimodal",
    },
    "feature-extraction": {
    ...
    "table-question-answering": {
        ...
        "tf": (TFAutoModelForTableQuestionAnswering,)
        if is_tf_available()
        else (),
        "default": {
            "model": {
    ...
    "visual-question-answering": {
        ...
        "pt": (AutoModelForVisualQuestionAnswering,)
        if is_torch_available()
        else (),
        "tf": (),
        "default": {
    ...
    "image-classification": {
        ...
        "tf": (TFAutoModelForImageClassification,)
        if is_tf_available()
        else (),
    ...
    "image-segmentation": {
        ...
        "default": {
            "model": {"pt": ("facebook/detr-resnet-50-panoptic", "fc15262")}
        },
        "type": "image",
    },
    "image-to-text": {
    ...
    "zero-shot-object-detection": {
        ...
        "tf": (),
        "pt": (AutoModelForZeroShotObjectDetection,)
        if is_torch_available()
        else (),
        "default": {
            "model": {"pt": ("google/owlvit-base-patch32", "17740e1")}
        },
        "type": "multimodal",
    },
    "depth-estimation": {
...
def get_task(model: str, use_auth_token: Optional[str] = None) -> str:
    ...
    try:
        http_get(
            f"https://huggingface.co/api/models/{model}", tmp, headers=headers
        )
        tmp.seek(0)
        body = tmp.read()
    ...
    if "pipeline_tag" not in data:
        raise RuntimeError(
            f"The model {model} does not seem to have a correct `pipeline_tag`"
            " set to infer the task automatically"
        )
    if data.get("library_name", "transformers") != "transformers":
        raise RuntimeError(
            f"This model is meant to be used with {data['library_name']} not"
            " with transformers"
        )
...
def clean_custom_task(task_info):
    ...
    if "impl" not in task_info:
        raise RuntimeError(
            "This model introduces a custom pipeline without specifying its"
            " implementation."
        )
...
def pipeline(
    ...
    **kwargs,
    ...
    if task is None and model is None:
        raise RuntimeError(
            "Impossible to instantiate a pipeline without either a task or a"
            " model being specified. Please provide a task class or a model"
        )

    if model is None and tokenizer is not None:
        raise RuntimeError(
            "Impossible to instantiate a pipeline with tokenizer specified but"
            " not the model as the provided tokenizer may not be compatible"
            " with the default model. Please provide a PreTrainedModel class"
            " or a path/identifier to a pretrained model when providing"
            " tokenizer."
        )
    if model is None and feature_extractor is not None:
        raise RuntimeError(
            "Impossible to instantiate a pipeline with feature_extractor"
            " specified but not the model as the provided feature_extractor"
            " may not be compatible with the default model. Please provide a"
            " PreTrainedModel class or a path/identifier to a pretrained model"
            " when providing feature_extractor."
        )
    ...
    if config is not None and len(getattr(config, "custom_pipelines", {})) > 0:
        ...
        if task is None and trust_remote_code is not False:
            if len(custom_tasks) == 1:
                ...
                raise RuntimeError(
                    "We can't infer the task automatically for this model as"
                    " there are multiple tasks available. Pick one in"
                    f" {', '.join(custom_tasks.keys())}"
                )

    if task is None and model is not None:
        if not isinstance(model, str):
            raise RuntimeError(
                "Inferring the task automatically requires to check the hub"
                f" with a model_id defined as a `str`.{model} is not a valid"
                " model_id."
            )
    ...
    if task in custom_tasks:
        ...
        if pipeline_class is None:
            if not trust_remote_code:
                raise ValueError(
                    "Loading this pipeline requires you to execute the code in"
                    " the pipeline file in that repo on your local machine."
                    " Make sure you have read the code there to avoid"
                    " malicious use, then set the option"
    ...
    if model is None:
        ...
        logger.warning(
            ...
            " a pipeline without specifying a model name and revision in"
    ...
    if device_map is not None:
        if "device_map" in model_kwargs:
            raise ValueError(
                "You cannot use both `pipeline(... device_map=...,"
                ' model_kwargs={"device_map":...})` as those arguments might'
                " conflict, use only one.)"
            )
        model_kwargs["device_map"] = device_map
    if torch_dtype is not None:
        if "torch_dtype" in model_kwargs:
            raise ValueError(
                "You cannot use both `pipeline(... torch_dtype=...,"
                ' model_kwargs={"torch_dtype":...})` as those arguments might'
                " conflict, use only one.)"
            )
    ...
    if load_tokenizer:
        ...
        if tokenizer is None:
            if isinstance(model_name, str):
                ...
                raise Exception(
                    "Impossible to guess which tokenizer to use. Please"
                    " provide a PreTrainedTokenizer class or a path/identifier"
                    " to a pretrained tokenizer."
                )
        ...
        if feature_extractor is None:
            if isinstance(model_name, str):
                ...
                raise Exception(
                    ...
                    " Please provide a PreTrainedFeatureExtractor class or a"
                    " path/identifier to a pretrained feature extractor."
                )
        ...
        if isinstance(feature_extractor, (str, tuple)):
            ...
            if (
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not load the `decoder` for {model_name}."
                        f" Defaulting to raw CTC. Error: {e}"
                    )
                    ...
                    if not is_pyctcdecode_available():
                        logger.warning(
                            "Try to install `pyctcdecode`: `pip install"
                            " pyctcdecode"
                        )
    ...
    if task == "translation" and model.config.task_specific_params:
        for key in model.config.task_specific_params:
            if key.startswith("translation"):
                ...
                warnings.warn(
                    (
                        '"translation" task was used, instead of'
                        f' "translation_XX_to_YY", defaulting to "{task}"'
                    ),
                    UserWarning,
                )
                break
    ...
    return pipeline_class(
        model=model, framework=framework, task=task, **kwargs
    )

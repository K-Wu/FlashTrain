    ...
    def _sanitize_parameters(
        ...
        **generate_kwargs
        ...
        if return_tensors is not None and return_type is None:
            return_type = (
                ReturnType.TENSORS if return_tensors else ReturnType.TEXT
            )
        ...
        if stop_sequence is not None:
            ...
            if len(stop_sequence_ids) > 1:
                warnings.warn(
                    "Stopping on a multiple token sequence is not yet"
                    " supported on transformers. The first token of the stop"
                    " sequence will be used as the stop sequence string in the"
                    " interim."
                )
    ...
    def _parse_and_tokenize(self, *args, truncation):
        ...
        if isinstance(args[0], list):
            if self.tokenizer.pad_token_id is None:
                raise ValueError(
                    "Please make sure that the tokenizer has a pad_token_id"
                    " when using a batch input"
                )
            ...
            raise ValueError(
                f" `args[0]`: {args[0]} have the wrong format. The should be"
                " either of type `str` or type `list`"
            )
...
class SummarizationPipeline(Text2TextGenerationPipeline):
    ...
    def check_inputs(
        ...
        if max_length < min_length:
            logger.warning(
                f"Your min_length={min_length} must be inferior than your"
                f" max_length={max_length}."
            )

        if input_length < max_length:
            logger.warning(
                f"Your max_length is set to {max_length}, but you input_length"
                f" is only {input_length}. You might consider decreasing"
                " max_length manually, e.g. summarizer('...',"
                f" max_length={input_length//2})"
            )
...
class TranslationPipeline(Text2TextGenerationPipeline):
    ...
    def check_inputs(
        ...
        if input_length > 0.9 * max_length:
            logger.warning(
                f"Your input_length: {input_length} is bigger than 0.9 *"
                f" max_length: {max_length}. You might consider increasing"
                " your max_length manually, e.g. translator('...',"
                " max_length=400)"
            )
        return True

...
class FillMaskPipeline(Pipeline):
    ...
    def _ensure_exactly_one_mask_token(
        self, input_ids: GenericTensor
    ) -> np.ndarray:
        ...
        if numel < 1:
            raise PipelineException(
                ...
                (
                    f"No mask_token ({self.tokenizer.mask_token}) found on the"
                    " input"
                ),
            )
    ...
    def ensure_exactly_one_mask_token(self, model_inputs: GenericTensor):
        if isinstance(model_inputs, list):
            for model_input in model_inputs:
                self._ensure_exactly_one_mask_token(
    ...
    def postprocess(self, model_outputs, top_k=5, target_ids=None):
        ...
        if self.framework == "tf":
            ...
            if target_ids is not None:
                probs = tf.gather_nd(
                    tf.squeeze(probs, 0), target_ids.reshape(-1, 1)
                )
        ...
        for i, (_values, _predictions) in enumerate(
            ...
            for v, p in zip(_values, _predictions):
                ...
                # Filter padding out:
                tokens = tokens[
                    np.where(tokens != self.tokenizer.pad_token_id)
                ]
            ...
            if id_ is None:
                ...
                if len(input_ids) == 0:
                    logger.warning(
                        ...
                        " in the model vocabulary. We cannot replace it with"
                        " anything meaningful, ignoring it"
                    )
                    continue
                id_ = input_ids[0]
                ...
                logger.warning(
                    f"The specified target token `{target}` does not exist in"
                    " the model vocabulary. Replacing with"
                    f" `{self.tokenizer.convert_ids_to_tokens(id_)}`."
                )

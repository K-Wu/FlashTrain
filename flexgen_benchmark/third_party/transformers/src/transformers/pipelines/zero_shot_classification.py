...
class ZeroShotClassificationArgumentHandler(ArgumentHandler):
    ...
    def __call__(self, sequences, labels, hypothesis_template):
        if len(labels) == 0 or len(sequences) == 0:
            raise ValueError(
                "You must include at least one label and at least one"
                " sequence."
            )
        if hypothesis_template.format(labels[0]) == hypothesis_template:
            raise ValueError(
                (
                    ...
                    " formatted with the target labels. Make sure the passed"
                    " template includes formatting syntax such as {{}} where"
                    " the label should go."
...
class ZeroShotClassificationPipeline(ChunkPipeline):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.entailment_id == -1:
            logger.warning(
                "Failed to determine 'entailment' label id from the label2id"
                " mapping in the model config. Setting to -1. Define a"
                " descriptive label2id mapping in the model config to ensure"
                " correct outputs."
            )
    ...
    def _parse_and_tokenize(
        ...
        **kwargs
        ...
        if self.tokenizer.pad_token is None:
            ...
            logger.error(
                "Tokenizer was not supporting padding necessary for zero-shot,"
                " attempting to use  `pad_token=eos_token`"
            )
    ...
    def _sanitize_parameters(self, **kwargs):
        if kwargs.get("multi_class", None) is not None:
            ...
            logger.warning(
                "The `multi_class` argument has been deprecated and renamed to"
                " `multi_label`. `multi_class` will be removed in a future"
        ...
        if "hypothesis_template" in kwargs:
            preprocess_params["hypothesis_template"] = kwargs[
                "hypothesis_template"
            ]

...
def no_collate_fn(items):
    if len(items) != 1:
        raise ValueError(
            "This collate_fn is meant to be used with batch_size=1"
        )
    return items[0]
    ...
    if isinstance(items[0][key], torch.Tensor):
        ...
        if dim == 2:
            if max_length == min_length:
                ...
                return torch.cat([item[key] for item in items], dim=0)
            tensor = (
                torch.zeros((batch_size, max_length), dtype=dtype)
                + padding_value
            )
        elif dim == 3:
            tensor = (
...
def pad_collate_fn(tokenizer, feature_extractor):
    ...
    if tokenizer is None and feature_extractor is None:
        raise ValueError(
            "Pipeline without tokenizer or feature_extractor cannot do"
            " batching"
        )
    if tokenizer is not None:
        if tokenizer.pad_token_id is None:
            raise ValueError(
                ...
                " You can try to set it with `pipe.tokenizer.pad_token_id ="
    ...
    if (
        ...
        raise ValueError(
            "The feature extractor, and tokenizer don't agree on padding side"
            f" {t_padding_side} != {f_padding_side}"
        )
    padding_side = "right"
    ...
    def inner(items):
        ...
        for item in items:
            if set(item.keys()) != keys:
                raise ValueError(
                    "The elements of the batch contain different keys. Cannot"
                    f" batch them ({set(item.keys())} != {keys})"
                )
    ...
    **model_kwargs,
    ...
    if not is_tf_available() and not is_torch_available():
        raise RuntimeError(
            ...
            " https://www.tensorflow.org/install/ To install PyTorch, read the"
            " instructions at https://pytorch.org/."
        )
    if isinstance(model, str):
        ...
        if model_classes:
            if look_pt:
                class_tuple = class_tuple + model_classes.get(
                    "pt", (AutoModel,)
                )
            if look_tf:
                class_tuple = class_tuple + model_classes.get(
        ...
        if config.architectures:
            ...
            for architecture in config.architectures:
                ...
                if look_tf:
                    _class = getattr(
                        transformers_module, f"TF{architecture}", None
                    )
                    if _class is not None:
                        classes.append(_class)
        ...
        for model_class in class_tuple:
            ...
            if framework == "pt" and model.endswith(".h5"):
                kwargs["from_tf"] = True
                logger.warning(
                    "Model might be a TensorFlow model (ending with `.h5`) but"
                    " TensorFlow is not available. Trying to load the model"
                    " with PyTorch."
                )
            elif framework == "tf" and model.endswith(".bin"):
                kwargs["from_pt"] = True
                logger.warning(
                    "Model might be a PyTorch model (ending with `.bin`) but"
                    " PyTorch is not available. Trying to load the model with"
                    " Tensorflow."
                )
        ...
        if isinstance(model, str):
            raise ValueError(
                f"Could not load model {model} with any of the following"
                f" classes: {class_tuple}."
            )
...
def get_framework(model, revision: Optional[str] = None):
    ...
    warnings.warn(
        (
            "`get_framework` is deprecated and will be removed in v5, use"
            " `infer_framework_from_model` instead."
        ),
        FutureWarning,
    )
    if not is_tf_available() and not is_torch_available():
...
def get_default_model_and_revision(
    ...
    if task_options:
        ...
        # parametrized
        raise ValueError(
            "The task defaults can't be correctly selected. You probably"
            ' meant "translation_XX_to_YY"'
        )
...
class PipedPipelineDataFormat(PipelineDataFormat):
        ...
        for line in sys.stdin:
            ...
            if "\t" in line:
                ...
                if self.column:
                    ...
                    yield {
                        kwargs: l for (kwargs, _), l in zip(self.column, line)
                    }
                else:
                    yield tuple(line)
    ...
    def save_binary(self, data: Union[dict, List[dict]]) -> str:
        if self.output_path is None:
            raise KeyError(
                "When using piped input on pipeline outputting large object"
                " requires an output file path. Please provide such output"
                " path through --output argument."
            )
...
class Pipeline(_ScikitCompat):
    ...
    def check_model_type(self, supported_models: Union[List[str], dict]):
        ...
        ):  # Create from a model mapping
            supported_models_names = []
            for config, model in supported_models.items():
                # Mapping can now contain tuples of models for the same configuration.
                if isinstance(model, tuple):
                    supported_models_names.extend(
                        [_model.__name__ for _model in model]
                    )
                else:
        ...
        if self.model.__class__.__name__ not in supported_models:
            logger.error(
                f"The model '{self.model.__class__.__name__}' is not supported"
                f" for {self.task}. Supported models are {supported_models}."
            )
    ...
    def get_iterator(
        ...
        if isinstance(inputs, collections.abc.Sized):
            dataset = PipelineDataset(
                inputs, self.preprocess, preprocess_params
            )
        else:
            if num_workers > 1:
                logger.warning(
                    "For iterable dataset using num_workers>1 is likely to"
                    " result in errors since everything is iterable, setting"
                    " `num_workers=1` to guarantee correctness."
                )
                num_workers = 1
        ...
        if "TOKENIZERS_PARALLELISM" not in os.environ:
            logger.info(
                "Disabling tokenizer parallelism, we're using DataLoader"
    ...
    def __call__(
        self, inputs, *args, num_workers=None, batch_size=None, **kwargs
    ):
        if args:
    ...
    def run_multi(
        ...
        return [
            self.run_single(
                item, preprocess_params, forward_params, postprocess_params
            )
            for item in inputs
        ]
...
class ChunkPipeline(Pipeline):
    ...
    def get_iterator(
        ...
        if num_workers > 1:
            logger.warning(
                "For ChunkPipeline using num_workers>0 is likely to result in"
                " errors since everything is iterable, setting `num_workers=1`"
...
class PipelineRegistry:
    ...
    def check_task(self, task: str) -> Tuple[str, Dict, Any]:
        ...
        if task.startswith("translation"):
            ...
            raise KeyError(
                f"Invalid translation task {task}, use 'translation_XX_to_YY'"
                " format"
            )

        raise KeyError(
            f"Unknown task {task}, available tasks are"
            f" {self.get_supported_tasks() + ['translation_XX_to_YY']}"
        )
    ...
    def register_pipeline(
        ...
        if task in self.supported_tasks:
            logger.warning(
                f"{task} is already registered. Overwriting pipeline for task"
                f" {task}..."
            )

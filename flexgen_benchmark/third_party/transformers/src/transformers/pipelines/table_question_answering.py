        ...
        if table is None:
            ...
            if (
                ...
                if not all(isinstance(d, dict) for d in table):
                    raise ValueError(
                        "Keyword argument `table` should be a list of dict,"
                        f" but is {(type(d) for d in table)}"
                    )
                ...
                if (
                    ...
                    raise ValueError(
                        "If keyword argument `table` is a list of"
                        " dictionaries, each dictionary should have a `table`"
                        " and `query` key, but only dictionary has keys"
                        f" {table[0].keys()} `table` and `query` keys."
                    )
            elif (
                Dataset is not None
                ...
                raise ValueError(
                    "Invalid input. Keyword argument `table` should be either"
                    f" of type `dict` or `list`, but is {type(table)})"
                )
        else:
...
class TableQuestionAnsweringPipeline(Pipeline):
    ...
    def __init__(
        self,
        args_parser=TableQuestionAnsweringArgumentHandler(),
        *args,
        **kwargs
    ):
        ...
        self.type = (
            "tapas"
            if hasattr(self.model.config, "aggregation_labels")
            else None
        )
        ...
        if self.framework == "pt":
            ...
            for index in range(batch_size):
                ...
                if prev_answers is not None:
                    ...
                    token_type_ids_example = token_type_ids[
                        index
                    ]  # shape (seq_len, 7)
                    ...
                    token_type_ids_example[:, 3] = (
                        torch.from_numpy(model_labels)
                        .type(torch.long)
                        .to(self.device)
                    )

                input_ids_example = input_ids[index]
                attention_mask_example = attention_mask[
                    index
                ]  # shape (seq_len,)
                token_type_ids_example = token_type_ids[
                    index
            ...
            for index in range(batch_size):
                ...
                outputs = self.model(
                    ...
                    attention_mask=np.expand_dims(
                        attention_mask_example, axis=0
                    ),
                    token_type_ids=np.expand_dims(
                        token_type_ids_example, axis=0
                    ),
                )
                logits = outputs.logits
                ...
                for i, p in enumerate(
                    tf.squeeze(probabilities).numpy().tolist()
                ):
                    segment_id = token_type_ids_example[:, 0].tolist()[i]
                    col = token_type_ids_example[:, 1].tolist()[i] - 1
                    row = token_type_ids_example[:, 2].tolist()[i] - 1
    ...
    def postprocess(self, model_outputs):
        ...
        if self.type == "tapas":
            if self.aggregate:
                ...
                no_agg_label_index = (
                    self.model.config.no_aggregation_label_index
                )
                aggregators_prefix = {
                    i: aggregators[i] + " > "
                    for i, pred in enumerate(agg_predictions)
            ...
            for index, coordinates in enumerate(answer_coordinates_batch):
                ...
                answer = {
                    ...
                    "cells": [
                        table.iat[coordinate] for coordinate in coordinates
                    ],
                }
                if aggregator:

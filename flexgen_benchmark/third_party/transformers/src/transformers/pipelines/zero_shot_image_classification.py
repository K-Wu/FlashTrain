...
class ZeroShotImageClassificationPipeline(ChunkPipeline):
    ...
    def __call__(
        self, images: Union[str, List[str], "Image", List["Image"]], **kwargs
    ):
        """
    ...
    def _forward(self, model_inputs):
        ...
        # batch position.
        diag = (
            torch.diagonal if self.framework == "pt" else tf.linalg.diag_part
        )
    ...
    def postprocess(self, model_outputs):
        ...
        if self.framework == "pt":
            logits = torch.cat(
                [output["logits_per_image"] for output in model_outputs]
            )

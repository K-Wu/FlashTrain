...
def chunk_iter(
    inputs, feature_extractor, chunk_len, stride_left, stride_right
):
...
class AutomaticSpeechRecognitionPipeline(ChunkPipeline):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.framework == "tf":
            raise ValueError(
                "The AutomaticSpeechRecognitionPipeline is only available in"
                " PyTorch."
            )
    ...
    def _sanitize_parameters(self, **kwargs):
        ...
        if "return_timestamps" in kwargs:
            postprocess_params["return_timestamps"] = kwargs[
                "return_timestamps"
            ]
    ...
    def preprocess(self, inputs, chunk_length_s=0, stride_length_s=None):
        ...
        if isinstance(inputs, dict):
            ...
            if not (
                ...
                raise ValueError(
                    "When passing a dictionnary to"
                    " AutomaticSpeechRecognitionPipeline, the dict needs to"
                    ' contain a "raw" key containing the numpy array'
        ...
        if chunk_length_s:
            ...
            if stride is not None:
                if (
                    ...
                    in MODEL_FOR_SPEECH_SEQ_2_SEQ_MAPPING.values()
                ):
                    raise ValueError(
                        "Stride is only usable with CTC models, try"
                        " removing it"
                    )
    ...
    def postprocess(
        ...
        if self.type == "ctc_with_lm":
            ...
            text = self.tokenizer.decode(
                items, skip_special_tokens=skip_special_tokens
            )
            if return_timestamps:
                ...
                if return_timestamps == "word":
                    word_offsets = self.tokenizer._get_word_offsets(
                        char_offsets,
                        self.tokenizer.replace_word_delimiter_char,
                    )

...
class TextClassificationPipeline(Pipeline):
    ...
    def _sanitize_parameters(
        ...
        **tokenizer_kwargs
        ...
        if isinstance(top_k, int) or top_k is None:
            ...
            warnings.warn(
                (
                    "`return_all_scores` is now deprecated,  if want a similar"
                    " funcionality use `top_k=None` instead of"
                    " `return_all_scores=True` or `top_k=1` instead of"
                    " `return_all_scores=False`."
                ),
                UserWarning,
            )
    ...
    def preprocess(
        ...
        if isinstance(inputs, dict):
            ...
            raise ValueError(
                "The pipeline received invalid inputs, if you are trying to"
                ' send text pairs, you can try to send a dictionnary `{"text":'
                ' "My text", "text_pair": "My pair"}` in order to send a text'
                " pair."
            )
        return self.tokenizer(
            inputs, return_tensors=return_tensors, **tokenizer_kwargs

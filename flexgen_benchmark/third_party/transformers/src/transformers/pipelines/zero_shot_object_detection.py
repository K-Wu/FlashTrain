...
class ZeroShotObjectDetectionPipeline(Pipeline):
    ...
    def __init__(self, **kwargs):
        ...
        if self.framework == "tf":
            raise ValueError(
                f"The {self.__class__} is only available in PyTorch."
            )
    ...
    def __call__(
        ...
        **kwargs
        ...
        if isinstance(text_queries, str) or (
            isinstance(text_queries, List)
            and not isinstance(text_queries[0], List)
        ):
            if isinstance(images, (str, Image.Image)):
                ...
                raise TypeError(
                    f"Innapropriate type of images: {type(images)}"
                )

        elif isinstance(text_queries, str) or (
            isinstance(text_queries, List)
            and isinstance(text_queries[0], List)
        ):
    ...
    def preprocess(self, inputs):
        ...
        target_sizes = [
            torch.IntTensor([[img.height, img.width]]) for img in images
        ]
    ...
    def postprocess(self, model_outputs, threshold=0.1, top_k=None):
        ...
        for i in range(len(outputs)):
            ...
            boxes = [
                self._get_bounding_box(box)
                for box in outputs[i]["boxes"][keep]
            ]
    ...
    def _get_bounding_box(self, box: "torch.Tensor") -> Dict[str, int]:
        ...
        if self.framework != "pt":
            raise ValueError(
                "The ZeroShotObjectDetectionPipeline is only available in"
                " PyTorch."
            )
    ...
    def _processor(
        ...
        **kwargs
        ...
        if text is None and images is None:
            raise ValueError(
                "You have to specify at least one text or image. Both cannot"
                " be none."
            )
        ...
        if text is not None:
            if isinstance(text, str) or (
                ...
                raise TypeError(
                    "Input text should be a string, a list of strings or a"

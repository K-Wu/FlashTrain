...
def apply_tesseract(
    ...
    irrelevant_indices = [
        idx for idx, word in enumerate(words) if not word.strip()
    ]
    words = [
    ...
    left = [
        coord
        for idx, coord in enumerate(left)
        if idx not in irrelevant_indices
    ]
    top = [
        coord for idx, coord in enumerate(top) if idx not in irrelevant_indices
    ]
    width = [
        coord
        for idx, coord in enumerate(width)
        if idx not in irrelevant_indices
    ]
    height = [
        coord
        for idx, coord in enumerate(height)
        if idx not in irrelevant_indices
    ]
...
class DocumentQuestionAnsweringPipeline(ChunkPipeline):
    ...
    def _sanitize_parameters(
        ...
        **kwargs,
        ...
        if top_k is not None:
            if top_k < 1:
                raise ValueError(
                    f"top_k parameter should be >= 1 (got {top_k})"
                )
    ...
    def preprocess(
        ...
        if input.get("image", None) is not None:
            ...
            if self.feature_extractor is not None:
                ...
                raise ValueError(
                    "If you are using a VisionEncoderDecoderModel, you must"
                    " provide a feature extractor"
                )
        ...
        if not self.model_type == ModelType.VisionEncoderDecoder:
            if "word_boxes" in input:
                ...
                if not TESSERACT_LOADED:
                    raise ValueError(
                        "If you provide an image without word_boxes, then the"
                        " pipeline will run OCR using Tesseract, but"
                ...
                raise ValueError(
                    "You must provide an image or word_boxes. If you provide"
                    " an image, the pipeline will automatically run OCR to"
                    " derive words and boxes"
                )

        if self.tokenizer.padding_side != "right":
            raise ValueError(
                "Document question answering only supports tokenizers whose"
                f" padding side is 'right', not {self.tokenizer.padding_side}"
            )
        ...
        if self.model_type == ModelType.VisionEncoderDecoder:
            ...
            for span_idx in range(num_spans):
                if self.framework == "pt":
                    ...
                    raise ValueError(
                        "Unsupported: Tensorflow preprocessing for"
    ...
    def _forward(self, model_inputs):
        ...
        model_outputs["attention_mask"] = model_inputs.get(
            "attention_mask", None
        )
    ...
    def postprocess(self, model_outputs, top_k=1, **kwargs):
        if self.model_type == ModelType.VisionEncoderDecoder:
            answers = [
                self.postprocess_encoder_decoder_single(o)
        ...
        answers = sorted(
            answers, key=lambda x: x.get("score", 0), reverse=True
        )[:top_k]
        return answers

...
class Conversation:
    ...
    def add_user_input(self, text: str, overwrite: bool = False):
        ...
        if self.new_user_input:
            if overwrite:
                logger.warning(
                    "User input added while unprocessed input was existing:"
                    f' "{self.new_user_input}" was overwritten with: "{text}".'
                )
                self.new_user_input = text
            else:
                logger.warning(
                    "User input added while unprocessed input was existing:"
                    f' "{self.new_user_input}" new input ignored: "{text}".'
                    " Set `overwrite` to True to overwrite unprocessed user"
                    " input"
                )
        else:
            self.new_user_input = text
...
class ConversationalPipeline(Pipeline):
    ...
    def __call__(
        ...
        **kwargs
        ...
        outputs = super().__call__(
            conversations, num_workers=num_workers, **kwargs
        )
    ...
    def preprocess(
        ...
        if not isinstance(conversation, Conversation):
            raise ValueError(
                "ConversationalPipeline, expects Conversation as inputs"
            )
        if conversation.new_user_input is None:
            raise ValueError(
                f"Conversation with UUID {type(conversation.uuid)} does not"
                " contain new user input to process. Add user inputs with the"
                " conversation's `add_user_input` method"
            )
    ...
    def _forward(self, model_inputs, minimum_tokens=10, **generate_kwargs):
        ...
        if max_length - minimum_tokens < n:
            logger.warning(
                f"Conversation input is to long ({n}), trimming it to"
                f" ({max_length} - {minimum_tokens})"
            )

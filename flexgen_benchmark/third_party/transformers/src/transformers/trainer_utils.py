...
class TrainerMemoryTracker:
    ...
    def derive_stage(self):
        ...
        if caller in self.stages:
            ...
            raise ValueError(
                f"was called from {caller}, but only expect to be called from"
                f" one of {self.stages.keys()}"
            )
        ...
        if not self.message_logged and self.logger and self.model_name:
            ...
            if len(ignored_columns) > 0:
                ...
                self.logger.info(
                    ...
                    f" corresponding argument in `{self.model_name}.forward`"
                    f" and have been ignored: {', '.join(ignored_columns)}. If"
                    f" {', '.join(ignored_columns)} are not expected by"
                    f" `{self.model_name}.forward`,  you can safely ignore"
                    " this message."
                )
                self.message_logged = True
        return {
            k: v for k, v in feature.items() if k in self.signature_columns
        }

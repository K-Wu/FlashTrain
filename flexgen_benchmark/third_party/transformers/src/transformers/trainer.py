...
class Trainer:
    ...
    def __init__(
        ...
        if args is None:
            ...
            logger.info(
                "No `TrainingArguments` passed, using"
        ...
        if model is None:
            if model_init is not None:
                ...
                raise RuntimeError(
                    "`Trainer` requires either a `model` or `model_init`"
                    " argument"
                )
        else:
            if model_init is not None:
                warnings.warn(
                    (
                        "`Trainer` requires either a `model` or `model_init`"
                        " argument, but not both. `model_init` will overwrite"
                        " your model when calling the `train` method. This"
                        " will become a fatal error in the next release."
                    ),
                    FutureWarning,
                )
            self.model_init = model_init
        ...
        if model.__class__.__name__ in MODEL_MAPPING_NAMES:
            raise ValueError(
                f"The model you have picked ({model.__class__.__name__})"
                " cannot be used as is for training: it only computes hidden"
                " states and does not accept any labels. You should choose a"
                " model with a head suitable for your task like any of the"
                " `AutoModelForXxx` listed at"
        ...
        if len(args.sharded_ddp) > 0:
            if args.deepspeed:
                raise ValueError(
                    "Using --sharded_ddp xxx together with --deepspeed is not"
                    " possible, deactivate one of those flags."
                )
            if len(args.fsdp) > 0:
                raise ValueError(
                    "Using --sharded_ddp xxx together with --fsdp is not"
                    " possible, deactivate one of those flags."
                )
            ...
            if args.local_rank == -1:
                ...
                raise ImportError(
                    "Sharded DDP training requires fairscale: `pip install"
                    " fairscale`."
                )
            elif (
                ...
                raise ImportError(
                    "Sharded DDP in a mode other than simple training"
                    " requires fairscale version >= 0.3, found"
                    f" {fairscale.__version__}. Upgrade your fairscale"
                    " library: `pip install --upgrade fairscale`."
                )
        ...
        if len(args.fsdp) > 0:
            if args.deepspeed:
                raise ValueError(
                    "Using --fsdp xxx together with --deepspeed is not"
                    " possible, deactivate one of those flags."
                )
            if args.local_rank == -1:
                raise ValueError(
        ...
        if model_init is not None and (
            ...
            raise RuntimeError(
                "Passing a `model_init` is incompatible with providing the"
                " `optimizers` argument. You should subclass `Trainer` and"
                " override the `create_optimizer_and_scheduler` method."
            )
        if is_torch_tpu_available() and self.optimizer is not None:
            ...
            if model_device != optimizer_device:
                raise ValueError(
                    "The model and the optimizer parameters are not on the"
                    " same device, which probably means you created an"
                    " optimizer around your model **before** putting on the"
                    " device and passing it to the `Trainer`. Make sure the"
                    " lines `import torch_xla.core.xla_model as xm` and"
                    " `model.to(xm.xla_device())` is performed before the"
                    " optimizer creation in your script."
                )
        if (
            ...
            raise RuntimeError(
                "Passing `optimizers` is not allowed if Fairscale, Deepspeed"
                " or PyTorch FSDP is enabled.You should subclass `Trainer` and"
                " override the `create_optimizer_and_scheduler` method."
            )
        default_callbacks = (
            DEFAULT_CALLBACKS
        ...
        if not callable(self.data_collator) and callable(
            ...
            raise ValueError(
                "The `data_collator` should be a simple callable (function,"
                " class with `__call__`)."
            )
        ...
        if (
            ...
            raise ValueError(
                "train_dataset does not implement __len__, max_steps has to be"
                " specified"
            )
        ...
        if (
            ...
            raise ValueError(
                "the `--group_by_length` option is only available for"
                " `Dataset`, not `IterableDataset"
            )
        ...
        if is_sagemaker_mp_enabled():
            ...
            if args.bf16:
                raise ValueError(
                    "SageMaker Model Parallelism does not support BF16 yet."
                    " Please use FP16 instead "
                )
            ...
            if IS_SAGEMAKER_MP_POST_1_10:
                ...
                if args.fp16 != smp.state.cfg.fp16:
                    logger.warning(
                        "FP16 provided in SM_HP_MP_PARAMETERS is"
                        f" {smp.state.cfg.fp16},but FP16 provided in trainer"
                        f" argument is {args.fp16},setting to"
                        f" {smp.state.cfg.fp16}"
                    )
                ...
                if hasattr(smp.state.cfg, "fp16"):
                    logger.warning(
                        "FP16 provided in SM_HP_MP_PARAMETERS is"
                        f" {smp.state.cfg.fp16}, but SageMaker Model"
                        " Parallelism < 1.10 does not support FP16 in"
                        " trainer."
                    )
        ...
        if args.fp16 or args.bf16:
            if args.half_precision_backend == "auto":
                if args.device == torch.device("cpu"):
                    if args.fp16:
                        raise ValueError(
                            "Tried to use `fp16` but it is not supported"
                            " on cpu"
                        )
                        ...
                        raise ValueError(
                            "Tried to use cpu amp but native cpu amp is not"
                            " available"
                        )
                else:
        ...
        if (args.fp16 or args.bf16) and not (
            ...
            if args.half_precision_backend == "cuda_amp":
                ...
                if not is_apex_available():
                    raise ImportError(
                        "Using FP16 with APEX but APEX is not installed,"
        ...
        if (
            ...
            raise ValueError(
                "SageMaker Model Parallelism in mixed precision mode does not"
                " support gradient clipping yet. Pass along 'max_grad_norm': 0"
            ...
            def get_ctx():
                ...
                if args.torchdynamo in ["fx2trt-fp16", "fx2trt"]:
                    if not is_torch_tensorrt_fx_available():
                        raise RuntimeError(
                            "Torch-TensorRT FX path is not installed."
                        )
                    if args.torchdynamo == "fx2trt-fp16":
                        return torchdynamo.optimize(
                    ...
                    raise RuntimeError(
                        f"Torchdynamo backend {args.torchdynamo} is not"
                        " supported."
                    )
    ...
    def _remove_unused_columns(
        ...
        if len(ignored_columns) > 0:
            ...
            logger.info(
                f"The following columns {dset_description} don't have a"
                " corresponding argument in"
                f" `{self.model.__class__.__name__}.forward` and have been"
                f" ignored: {', '.join(ignored_columns)}. If"
                f" {', '.join(ignored_columns)} are not expected by"
                f" `{self.model.__class__.__name__}.forward`,  you can safely"
                " ignore this message."
            )
        ...
        if self.args.group_by_length:
            if is_datasets_available() and isinstance(
                ...
                lengths = (
                    ...
                    in self.train_dataset.column_names
                    else None
                )
            else:
                lengths = None
    ...
    def get_optimizer_cls_and_kwargs(
        ...
        if args.optim == OptimizerNames.ADAFACTOR:
            ...
            try:
                ...
                raise ValueError(
                    "Trainer tried to instantiate apex FusedAdam but apex is"
                    " not installed!"
                )
            ...
            try:
                ...
                raise ValueError(
                    "Trainer tried to instantiate bnb Adam8bit but bnb is not"
                    " installed!"
                )
            ...
            raise ValueError(
                "Trainer cannot instantiate unsupported optimizer:"
                f" {args.optim}"
            )
    ...
    def _hp_search_setup(self, trial: Union["optuna.Trial", Dict[str, Any]]):
        ...
        for key, value in params.items():
            if not hasattr(self.args, key):
                logger.warning(
                    f"Trying to set {key} in the hyperparameter search but"
                    " there is no corresponding field in `TrainingArguments`."
                )
                continue
    ...
    def torch_jit_model_eval(self, model, dataloader, training=False):
        if not training:
            if dataloader is None:
                logger.warning(
                    "failed to use PyTorch jit mode due to current dataloader"
                    " is none."
                )
                return model
            jit_inputs = []
            ...
            try:
                ...
                with ContextManagers(
                    ...
                    jit_model = torch.jit.trace(
                        jit_model, jit_inputs, strict=False
                    )
    ...
    def ipex_optimize_model(self, model, training=False, dtype=torch.float32):
        if not is_ipex_available():
            raise ImportError(
                "Using IPEX but IPEX is not installed or IPEX's version does"
                " not match current PyTorch, please refer to"
        ...
        if not training:
            ...
            model = ipex.optimize(
                model, dtype=dtype, level="O1", conv_bn_folding=False
            )
        else:
    ...
    def train(
        ...
        **kwargs,
        ...
        if "model_path" in kwargs:
            ...
            warnings.warn(
                (
                    "`model_path` is deprecated and will be removed in a"
                    " future version. Use `resume_from_checkpoint` instead."
                ),
                FutureWarning,
            )
        if len(kwargs) > 0:
    ...
    def _inner_training_loop(
        ...
        if has_length(train_dataloader):
            ...
            raise ValueError(
                "args.max_steps must be set to a positive value if dataloader"
                f" does not have a length, was {args.max_steps}"
            )
        ...
        if DebugOption.UNDERFLOW_OVERFLOW in self.args.debug:
            if self.args.n_gpu > 1:
                ...
                raise ValueError(
                    "Currently --debug underflow_overflow is not supported"
                    " under DP. Please use DDP (torch.distributed.launch)."
                )
            else:
        ...
        logger.info(
            ...
            f" accumulation) = {total_train_batch_size}"
        )
        logger.info(
        ...
        if resume_from_checkpoint is not None and os.path.isfile(
            ...
            if not args.ignore_data_skip:
                logger.info(
                    f"  Will skip the first {epochs_trained} epochs then the"
                    f" first {steps_trained_in_current_epoch} batches in the"
                    " first epoch. If this takes a lot of time, you can add"
                    " the `--ignore_data_skip` flag to your launch command,"
                    " but you will resume the training on data already seen"
                    " by your model."
                )
        ...
        if not args.ignore_data_skip:
            for epoch in range(epochs_trained):
                ...
                ) and isinstance(train_dataloader.sampler, RandomSampler)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            if step < 0:
                logger.warning(
                    "There seems to be not a single sample in your"
                    " epoch_iterator, stopping training at step"
                    f" {self.state.global_step}! This is expected if you're"
                    " using an IterableDataset and set num_steps"
                    f" ({max_steps}) higher than the number of available"
                    " samples."
                )
            ...
            if DebugOption.TPU_METRICS_DEBUG in self.args.debug:
                if is_torch_tpu_available():
                    ...
                    logger.warning(
                        ...
                        " have a TPU configured. Check your training"
        ...
        if os.path.isfile(os.path.join(resume_from_checkpoint, CONFIG_NAME)):
            ...
            if (
                ...
                logger.warning(
                    ...
                    f" {checkpoint_version} of Transformers but your current"
                    f" version is {__version__}. This is not recommended and"
                    " could yield to errors or unwanted behaviors."
                )
            ...
            if is_sagemaker_mp_enabled():
                if os.path.isfile(
                    ...
                    if hasattr(self.args, "fp16") and self.args.fp16 is True:
                        logger.warning(
                            "Enabling FP16 and loading from smp < 1.10"
                            " checkpoint together is not suppported."
                        )
    ...
    def _load_best_model(self):
        logger.info(
            ...
            f" {self.state.best_model_checkpoint} (score:"
        ...
        if os.path.exists(best_model_path):
            ...
            logger.warning(
                f"Could not locate the best model at {best_model_path}, if you"
                " are running a distributed training on multiple nodes, you"
                " should activate `--save_on_each_node`."
            )
    ...
    def _load_rng_state(self, checkpoint):
        ...
        if self.args.world_size > 1:
            ...
            rng_file = os.path.join(
                checkpoint, f"rng_state_{process_index}.pth"
            )
            if not os.path.isfile(rng_file):
                logger.info(
                    f"Didn't find an RNG file for process {process_index}, if"
                    " you are resuming a training that wasn't launched in a"
                    " distributed fashion, reproducibility is not guaranteed."
                )
                return
        else:
            ...
            if not os.path.isfile(rng_file):
                logger.info(
                    ...
                    " that was launched in a distributed fashion,"
        ...
        if torch.cuda.is_available():
            if self.args.local_rank != -1:
                ...
                try:
                    ...
                    logger.info(
                        "Didn't manage to set back the RNG states of the GPU"
                        f" because of the following error:\n {e}\nThis won't"
                        " yield the same results as if the training had not"
                        " been interrupted."
                    )
        if is_torch_tpu_available():
    ...
    def _load_optimizer_and_scheduler(self, checkpoint):
        ...
        if checkpoint_file_exists and os.path.isfile(
            ...
            if is_torch_tpu_available():
                ...
                map_location = (
                    "cpu" if is_sagemaker_mp_enabled() else self.args.device
                )
                if is_sagemaker_mp_enabled():
                    if os.path.isfile(
                        os.path.join(checkpoint, "user_content.pt")
                    ):
    ...
    def hyperparameter_search(
        ...
        **kwargs,
        ...
        if backend == HPSearchBackend.OPTUNA and not is_optuna_available():
            raise RuntimeError(
                "You picked the optuna backend, but it is not installed. Use"
                " `pip install optuna`."
            )
        ...
        if backend == HPSearchBackend.SIGOPT and not is_sigopt_available():
            raise RuntimeError(
                "You picked the sigopt backend, but it is not installed. Use"
                " `pip install sigopt`."
            )
        if backend == HPSearchBackend.WANDB and not is_wandb_available():
            raise RuntimeError(
                "You picked the wandb backend, but it is not installed. Use"
                " `pip install wandb`."
            )
        self.hp_search_backend = backend
        if self.model_init is None:
            raise RuntimeError(
                "To use hyperparameter search, you need to pass your model"
                " through a model_init function."
            )

        self.hp_space = (
            default_hp_space[backend] if hp_space is None else hp_space
        )
    ...
    def _prepare_inputs(
        ...
        if len(inputs) == 0:
            raise ValueError(
                "The batch received was empty, your model won't be able to"
                " train on it. Double-check that your training dataset"
                " contains keys expected by the model:"
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if labels is not None:
            ...
            if isinstance(outputs, dict) and "loss" not in outputs:
                raise ValueError(
                    "The model did not return a loss from the inputs, only"
                    f" the following keys: {','.join(outputs.keys())}. For"
    ...
    def save_model(
        ...
        if is_torch_tpu_available():
            ...
            if is_deepspeed_zero3_enabled():
                ...
                if not self.deepspeed.save_16bit_model(
                    output_dir, WEIGHTS_NAME
                ):
                    logger.warning(
                        "deepspeed.save_16bit_model didn't save the model,"
                        " since"
                        " stage3_gather_16bit_weights_on_model_save=false."
                        " Saving the full checkpoint instead, use"
        ...
        for path in glob_checkpoints:
            if use_mtime:
                ...
                if (
                    ...
                    and regex_match.groups() is not None
                ):
                    ordering_and_checkpoint_path.append(
    ...
    def _pad_across_processes(self, tensor, pad_index=-100):
        ...
        if isinstance(tensor, (list, tuple)):
            ...
            raise TypeError(
                f"Can't pad the values of type {type(tensor)}, only of nested"
                " list/tuple/dicts of tensors."
            )
    ...
    def init_git_repo(self, at_init: bool = False):
        ...
        if not self.is_world_process_zero():
            return
        use_auth_token = (
            True if self.args.hub_token is None else self.args.hub_token
        )
    ...
    def _push_from_checkpoint(self, checkpoint_folder):
        ...
        try:
            ...
            if self.args.save_strategy == IntervalStrategy.STEPS:
                commit_message = (
                    f"Training in progress, step {self.state.global_step}"
                )
            else:
                commit_message = (
                    f"Training in progress, epoch {int(self.state.epoch)}"
                )
        ...
        **kwargs
        ...
        if self.args.should_save:
            ...
            try:
                ...
                logger.error(
                    "Error pushing update to the model card. Please read logs"
                    f" and retry.\n${exc}"
                )
    ...
    def prediction_loop(
        ...
        label_ids = (
            labels_gatherer.finalize() if not prediction_loss_only else None
        )
        inputs_ids = (
            inputs_gatherer.finalize() if not prediction_loss_only else None
        )

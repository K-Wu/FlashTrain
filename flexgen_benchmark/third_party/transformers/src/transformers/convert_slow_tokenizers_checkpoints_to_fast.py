...
TOKENIZER_CLASSES = {
    name: getattr(transformers, name + "Fast")
...
def convert_slow_checkpoint_to_fast(
    ...
    if tokenizer_name is not None and tokenizer_name not in TOKENIZER_CLASSES:
        raise ValueError(
            "Unrecognized tokenizer name, should be one of"
    ...
    for tokenizer_name in tokenizer_names:
        ...
        logger.info(
            f"For tokenizer {tokenizer_class.__class__.__name__} loading"
        ...
        for checkpoint in checkpoint_names:
            ...
            # Save fast tokenizer
            logger.info(
                f"Save fast tokenizer to {dump_path} with prefix"
                f" {checkpoint} add_prefix {add_prefix}"
            )
            ...
            logger.info(
                f"=> {dump_path_full} with prefix {checkpoint_prefix_name},"
                f" add_prefix {add_prefix}"
            )

            if (
                checkpoint
                in list(tokenizer.pretrained_vocab_files_map.values())[0]
            ):
                file_path = list(
                    tokenizer.pretrained_vocab_files_map.values()
                )[0][checkpoint]
                ...
                logger.info(
                    f"=> {dump_path_full} with prefix"
                    f" {checkpoint_prefix_name}, add_prefix {add_prefix}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Optional tokenizer type selected in the list of"
            f" {list(TOKENIZER_CLASSES.keys())}. If not given, will download"
            " and convert all the checkpoints from AWS."
        ),
    )
    parser.add_argument(
        "--checkpoint_name",
        ...
        type=str,
        help=(
            "Optional checkpoint name. If not given, will download and convert"
            " the canonical checkpoints from AWS."
        ),
    )
    parser.add_argument(
        "--force_download",

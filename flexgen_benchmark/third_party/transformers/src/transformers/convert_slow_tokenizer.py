...
class SentencePieceExtractor:
    ...
    def extract(self) -> Tuple[Dict[str, int], List[Tuple]]:
        sp = self.sp
        vocab = {
            sp.id_to_piece(index): index for index in range(sp.GetPieceSize())
        }
...
class BertConverter(Converter):
    def converted(self) -> Tokenizer:
        ...
        if hasattr(self.original_tokenizer, "basic_tokenizer"):
            ...
            strip_accents = (
                self.original_tokenizer.basic_tokenizer.strip_accents
            )
            do_lower_case = (
                self.original_tokenizer.basic_tokenizer.do_lower_case
            )
...
class SpmConverter(Converter):
    def __init__(self, *args):
        ...
        if self.proto.trainer_spec.byte_fallback:
            warnings.warn(
                "The sentencepiece tokenizer that you are converting to a fast"
                " tokenizer uses the byte fallback option which is not"
                " implemented in the fast tokenizers. In practice this means"
                " that the fast version of the tokenizer can produce unknown"
                " tokens whereas the sentencepiece version would have"
                " converted these unknown tokens into a sequence of byte"
                " tokens matching the original piece of text."
            )
    ...
    def tokenizer(self, proto):
        ...
        if model_type == 1:
            ...
            raise Exception(
                ...
                " trained with a different algorithm"
            )
    ...
    def post_processor(self):
        return processors.TemplateProcessing(
            ...
            special_tokens=[
                (
                    "en_XX",
                    self.original_tokenizer.convert_tokens_to_ids("en_XX"),
                ),
                (
                    "</s>",
                    self.original_tokenizer.convert_tokens_to_ids("</s>"),
    ...
    def post_processor(self):
        return processors.TemplateProcessing(
            ...
            special_tokens=[
                (
                    "eng_Latn",
                    self.original_tokenizer.convert_tokens_to_ids("eng_Latn"),
                ),
                (
                    "</s>",
                    self.original_tokenizer.convert_tokens_to_ids("</s>"),
...
class PegasusConverter(SpmConverter):
    def vocab(self, proto):
        ...
        if (
            ...
            and self.original_tokenizer.mask_token_id
...
class T5Converter(SpmConverter):
    def vocab(self, proto):
        ...
        vocab += [
            (f"<extra_id_{i}>", 0.0) for i in range(num_extra_ids - 1, -1, -1)
        ]
        return vocab
...
def convert_slow_tokenizer(transformer_tokenizer) -> Tokenizer:
    ...
    if tokenizer_class_name not in SLOW_TO_FAST_CONVERTERS:
        raise ValueError(
            f"An instance of tokenizer class {tokenizer_class_name} cannot be"
            " converted in a Fast tokenizer instance. No converter was found."
            " Currently available slow->fast convertors:"

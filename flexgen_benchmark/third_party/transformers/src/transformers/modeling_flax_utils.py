...
class FlaxPreTrainedModel(PushToHubMixin, FlaxGenerationMixin):
    ...
    def __init__(
        ...
        if _do_init:
            ...
            params_shape_tree = jax.eval_shape(
                lambda params: params, random_params
            )
        else:
            ...
            logger.info(
                "Model weights are not initialized as `_do_init` is set to"
                " `False`. Make sure to call"
                f" `{self.__class__.__name__}.init_weights` manually to"
    ...
    def params(self) -> Union[Dict, FrozenDict]:
        if not self._is_initialized:
            raise ValueError(
                "`params` cannot be accessed from model when the model is"
                " created with `_do_init=False`. You must call `init_weights`"
                " manually and store the params outside of the model and pass"
    ...
    def params(self, params: Union[Dict, FrozenDict]):
        ...
        if not self._is_initialized:
            raise ValueError(
                "`params` cannot be set from model when the model is created"
                " with `_do_init=False`. You store the params outside of the"
                " model."
            )
        ...
        if len(self.required_params - param_keys) > 0:
            raise ValueError(
                "Some parameters are missing. Make sure that `params` include"
                " the following parameters"
                f" {self.required_params - param_keys}"
            )
        self._params = params
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if pretrained_model_name_or_path is not None:
            ...
            if os.path.isdir(pretrained_model_name_or_path):
                ...
                # At this stage we don't have a weight file so we will raise an error.
                    ...
                    raise EnvironmentError(
                        f"Error no file named {FLAX_WEIGHTS_NAME} or"
                ...
                try:
                    ...
                    if (
                        ...
                        and filename == FLAX_WEIGHTS_NAME
                    ):
                        ...
                        if has_file(
                            ...
                            **has_file_kwargs,
                            ...
                            raise EnvironmentError(
                                ...
                                f" {FLAX_WEIGHTS_NAME} but there is a file for"
                            ...
                            **has_file_kwargs,
                            ...
                            raise EnvironmentError(
                                f"{pretrained_model_name_or_path} does not"
                                " appear to have a file named"
                                f" {FLAX_WEIGHTS_INDEX_NAME} but there is a"
                                " sharded file for PyTorch weights. Use"
                                " `from_pt=True` to load this model from"
                                " those weights."
                            )
                        else:
                            raise EnvironmentError(
                ...
                try:
                    ...
                    try:
                        with open(resolved_archive_file) as f:
                            if f.read().startswith("version"):
                                raise OSError(
                                    "You seem to have cloned a repository"
                                    " without having git-lfs installed. Please"
                                    " install git-lfs and run `git lfs"
                                    " install` followed by `git lfs pull` in"
                        ...
                        raise EnvironmentError(
                            f"Unable to convert {archive_file} to Flax"
        ...
        if missing_keys and not _do_init:
            logger.warning(
                f"The checkpoint {pretrained_model_name_or_path} is missing"
                f" required keys: {missing_keys}. Make sure to call"
                " model.init_weights to initialize the missing weights."
            )
        ...
        for key in state.keys():
            if (
                key in random_state
                and state[key].shape != random_state[key].shape
            ):
                if ignore_mismatched_sizes:
                    ...
                    raise ValueError(
                        "Trying to load the pretrained weight for"
                        f" {key} failed: checkpoint has shape"
                        f" {state[key].shape} which is incompatible with the"
                        f" model shape {random_state[key].shape}. Using"
                        " `ignore_mismatched_sizes=True` if you really want"
                        " to load this checkpoint inside this model."
                    )
        ...
        if len(fp16_params) > 0:
            logger.warning(
                f"Some of the weights of {model.__class__.__name__} were"
                " initialized in float16 precision from the model checkpoint"
                f" at {pretrained_model_name_or_path}:\n{fp16_params}\nYou"
                " should probably UPCAST the model weights to float32 if this"
                " was not intended. See [`~FlaxPreTrainedModel.to_fp32`] for"
        ...
        if len(bf16_params) > 0:
            logger.warning(
                f"Some of the weights of {model.__class__.__name__} were"
                " initialized in bfloat16 precision from the model checkpoint"
                f" at {pretrained_model_name_or_path}:\n{bf16_params}\nYou"
                " should probably UPCAST the model weights to float32 if this"
                " was not intended. See [`~FlaxPreTrainedModel.to_fp32`] for"
                " further information on how to do this."
        ...
        **kwargs,
        ...
        if index is None:
            ...
            save_index_file = os.path.join(
                save_directory, FLAX_WEIGHTS_INDEX_NAME
            )
            ...
            for shard_file, shard in shards.items():
                ...
                with open(
                    os.path.join(save_directory, shard_file), mode="wb"
                ) as f:

...
class ProcessorMixin(PushToHubMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(kwargs) != len(self.attributes):
            raise ValueError(
                f"This processor requires {len(self.attributes)} arguments:"
                f" {', '.join(self.attributes)}. Got {len(args)} arguments"
                " instead."
            )
        ...
        for attribute_name, arg in kwargs.items():
            ...
            if not isinstance(arg, proper_class):
                raise ValueError(
                    f"Received a {type(arg).__name__} for argument"
                    f" {attribute_name}, but a {class_name} was expected."
                )

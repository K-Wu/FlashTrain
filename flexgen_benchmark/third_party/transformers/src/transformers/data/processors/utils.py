...
class SingleSentenceClassificationProcessor(DataProcessor):
    ...
    def create_from_examples(
        cls, texts_or_text_and_labels, labels=None, **kwargs
    ):
        processor = cls(**kwargs)
    ...
    def add_examples(
        ...
        for text_or_text_and_label, label, guid in zip(
            ...
            if (
                isinstance(text_or_text_and_label, (tuple, list))
                and label is None
            ):
    ...
    def get_features(
        ...
        for ex_index, (input_ids, example) in enumerate(
            ...
            attention_mask = [1 if mask_padding_with_zero else 0] * len(
                input_ids
            )
            ...
            if len(input_ids) != batch_length:
                raise ValueError(
                    f"Error with input length {len(input_ids)} vs"
                    f" {batch_length}"
                )
            if len(attention_mask) != batch_length:
                raise ValueError(
                    f"Error with input length {len(attention_mask)} vs"
                    f" {batch_length}"
                )
            ...
            if ex_index < 5 and self.verbose:
                ...
                logger.info(
                    "attention_mask:"
                    f" {' '.join([str(x) for x in attention_mask])}"
                )
        ...
        if return_tensors is None:
            ...
            if not is_tf_available():
                raise RuntimeError(
                    "return_tensors set to 'tf' but TensorFlow 2.0 can't be"
                    " imported"
                )

...
DEPRECATION_WARNING = (
    "This dataset will be removed from the library soon, preprocessing should"
    " be handled with the 🤗 Datasets library. You can have a look at this"
    " example script for pointers: {0}"
)
            ...
            if os.path.exists(cached_features_file) and not overwrite_cache:
                ...
                logger.info(
                    (
                        ...
                        f" {cached_features_file} [took %.3f s]"
                    ),
                    time.time() - start,
                )
...
class LineByLineWithRefDataset(Dataset):
    ...
    def __init__(
        ...
        if len(data) != len(ref):
            raise ValueError(
                "Length of Input file should be equal to Ref file. But the"
                f" length of {file_path} is {len(data)} while length of"
                f" {ref_path} is {len(ref)}"
            )
            ...
            if i == len(document) - 1 or current_length >= target_seq_length:
                if current_chunk:
                    ...
                    def truncate_seq_pair(tokens_a, tokens_b, max_num_tokens):
                        ...
                        while True:
                            ...
                            if not (len(trunc_tokens) >= 1):
                                raise ValueError(
                                    "Sequence length to be truncated must be"
                                    " no less than one"
                                )
                    ...
                    if not (len(tokens_a) >= 1):
                        raise ValueError(
                            f"Length of sequence a is {len(tokens_a)} which"
                            " must be no less than 1"
                        )
                    if not (len(tokens_b) >= 1):
                        raise ValueError(
                            f"Length of sequence b is {len(tokens_b)} which"
                            " must be no less than 1"
                        )

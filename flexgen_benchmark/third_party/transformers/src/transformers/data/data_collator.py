...
def tf_default_data_collator(features: List[InputDataClass]) -> Dict[str, Any]:
    ...
    if label_col_name is not None:
        if isinstance(first[label_col_name], tf.Tensor):
            dtype = (
                tf.int64
                if first[label_col_name].dtype.is_integer()
                else tf.float32
            )
            ...
            dtype = (
                tf.int64
                if isinstance(first[label_col_name][0], int)
                else tf.float32
            )
        else:
            dtype = (
                tf.int64
                if isinstance(first[label_col_name], int)
                else tf.float32
            )
...
def numpy_default_data_collator(
    ...
    if "label" in first and first["label"] is not None:
        ...
        if isinstance(first["label_ids"], np.ndarray):
            ...
            dtype = (
                np.int64 if type(first["label_ids"][0]) is int else np.float32
            )
...
class DataCollatorForTokenClassification(DataCollatorMixin):
    ...
    def tf_call(self, features):
        ...
        batch = {
            k: tf.convert_to_tensor(v, dtype=tf.int64)
...
def _tf_collate_batch(
    ...
    for example in examples:
        ...
        result.append(
            tf.pad(example, paddings, constant_values=tokenizer.pad_token_id)
        )
...
class DataCollatorForLanguageModeling(DataCollatorMixin):
    ...
    def __post_init__(self):
        ...
        if self.tf_experimental_compile:
            ...
            self.tf_mask_tokens = tf.function(
                self.tf_mask_tokens, jit_compile=True
            )
    ...
    def tf_bernoulli(shape, probability):
        ...
        return tf.cast(
            prob_matrix - tf.random.uniform(shape, 0, 1) >= 0, tf.bool
        )
    ...
    def tf_mask_tokens(
        ...
        indices_random = (
            self.tf_bernoulli(input_shape, 0.1)
            & masked_indices
            & ~indices_replaced
        )
        random_words = tf.random.uniform(
            input_shape, maxval=vocab_size, dtype=tf.int64
        )
...
class DataCollatorForWholeWordMask(DataCollatorForLanguageModeling):
    ...
    def _whole_word_mask(self, input_tokens: List[str], max_predictions=512):
        ...
        if not isinstance(self.tokenizer, (BertTokenizer, BertTokenizerFast)):
            warnings.warn(
                "DataCollatorForWholeWordMask is only suitable for"
                " BertTokenizer-like tokenizers. Please refer to the"
        ...
        if len(covered_indexes) != len(masked_lms):
            raise ValueError(
                "Length of covered_indexes is not equal to length of"
                " masked_lms."
            )
        mask_labels = [
    ...
    def torch_mask_tokens(
        self, inputs: Any, mask_labels: Any
    ) -> Tuple[Any, Any]:
        """
        ...
        if self.tokenizer.mask_token is None:
            raise ValueError(
                "This tokenizer does not have a mask token which is necessary"
                " for masked language modeling. Remove the --mlm flag if you"
                " want to use this tokenizer."
            )
        labels = inputs.clone()
    ...
    def tf_mask_tokens(self, inputs: Any, mask_labels: Any) -> Tuple[Any, Any]:
        ...
        masked_indices = masked_indices & ~tf.cast(
            special_tokens_mask, dtype=tf.bool
        )
        if self.tokenizer._pad_token is not None:
        ...
        inputs = tf.where(
            indices_replaced, self.tokenizer.mask_token_id, inputs
        )
...
class DataCollatorForSOP(DataCollatorForLanguageModeling):
    ...
    def __init__(self, *args, **kwargs):
        warnings.warn(
            (
                "DataCollatorForSOP is deprecated and will be removed in a"
                " future version, you can now use"
    ...
    def __call__(self, examples: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        sop_label_list = [
            example["sentence_order_label"] for example in examples
        ]
...
class DataCollatorForPermutationLanguageModeling(DataCollatorMixin):
    ...
    def torch_call(
        ...
        inputs, perm_mask, target_mapping, labels = self.torch_mask_tokens(
            batch
        )
        return {
            "input_ids": inputs,
    ...
    def numpy_call(
        ...
        inputs, perm_mask, target_mapping, labels = self.numpy_mask_tokens(
            batch
        )
        return {
            "input_ids": inputs,
    ...
    def torch_mask_tokens(self, inputs: Any) -> Tuple[Any, Any, Any, Any]:
        ...
        if self.tokenizer.mask_token is None:
            raise ValueError(
                "This tokenizer does not have a mask token which is necessary"
                " for permutation language modeling. Please add a mask token"
                " if you want to use this tokenizer."
            )

        if inputs.size(1) % 2 != 0:
            raise ValueError(
                "This collator requires that sequence lengths be even to"
                " create a leakage-free perm_mask. Please see relevant"
                " comments in source code for details."
            )
    ...
    def tf_mask_tokens(self, inputs: Any) -> Tuple[Any, Any, Any, Any]:
        ...
        target_mapping = np.zeros(
            (labels_shape[0], labels_shape[1], labels_shape[1]),
            dtype=np.float32,
        )
        ...
        for i in range(len(labels)):
            ...
            while cur_len < max_len:
                ...
                context_length = int(span_length / self.plm_probability)
                # Sample a starting point `start_index` from the interval `[cur_len, cur_len + context_length - span_length]` and mask tokens `start_index:start_index + span_length`
                start_index = cur_len + randint(
                    0, context_length - span_length + 1
                )
                masked_indices[i, start_index : start_index + span_length] = 1
                # Set `cur_len = cur_len + context_length`
                cur_len += context_length
        ...
        masked_indices = tf.cast(
            tf.convert_to_tensor(masked_indices), dtype=tf.bool
        )
        ...
        for i in range(len(labels)):
            ...
            # (2) They cannot see masked positions, so there won't be information leak
            perm_index = tf.where(
                ~masked_indices[i] & non_func_mask[i], -1, perm_index
            )
            # The logic for whether the i-th token can attend on the j-th token based on the factorisation order:
            # 0 (can attend): If perm_index[i] > perm_index[j] or j is neither masked nor a functional token
            # 1 (cannot attend): If perm_index[i] <= perm_index[j] and j is either masked or a functional token

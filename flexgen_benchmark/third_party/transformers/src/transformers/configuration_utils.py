...
class PretrainedConfig(PushToHubMixin):
    ...
    def __init__(self, **kwargs):
        ...
        if self.id2label is not None:
            ...
            if num_labels is not None and len(self.id2label) != num_labels:
                logger.warning(
                    f"You passed along `num_labels={num_labels}` with an"
                    f" incompatible id to label map: {self.id2label}. The"
                    " number of labels wil be overwritten to"
                    f" {self.num_labels}."
                )
            self.id2label = dict(
        ...
        if (
            ...
            raise ValueError(
                ...
                f" received {self.problem_type} but only 'regression',"
                " 'single_label_classification' and"
        ...
        if kwargs.pop("xla_device", None) is not None:
            logger.warning(
                "The `xla_device` argument has been deprecated in v4.4.0 of"
                " Transformers. It is ignored and you can safely remove it"
                " from your `config.json` file."
            )
        ...
        if kwargs.get("gradient_checkpointing", False):
            warnings.warn(
                "Passing `gradient_checkpointing` to a config initialization"
                " is deprecated and will be removed in v5 Transformers. Using"
                " `model.gradient_checkpointing_enable()` instead, or if you"
                " are using the `Trainer` API, pass"
                " `gradient_checkpointing=True` in your `TrainingArguments`."
            )
    ...
    def from_pretrained(
        ...
        if (
            ...
            logger.warning(
                ...
                f" instantiate a model of type {cls.model_type}. This is not"
    ...
    def _get_config_dict(
        ...
        if os.path.isfile(
            ...
            try:
                ...
                raise EnvironmentError(
                    ...
                    f" '{pretrained_model_name_or_path}'. If you were trying"
                    " to load it from 'https://huggingface.co/models', make"
                    " sure you don't have a local directory with the same"
                    " name. Otherwise, make sure"
                    f" '{pretrained_model_name_or_path}' is the correct path"
                    f" to a directory containing a {configuration_file} file"
                )
        ...
        try:
            ...
            raise EnvironmentError(
                f"It looks like the config file at '{resolved_config_file}' is"
        ...
        if is_local:
            ...
            logger.info(
                f"loading configuration file {configuration_file} from cache"
                f" at {resolved_config_file}"
            )
    ...
    def from_dict(
        ...
        if "num_labels" in kwargs and "id2label" in kwargs:
            ...
            if len(id2label) != num_labels:
                raise ValueError(
                    f"You passed along `num_labels={num_labels }` with an"
                    f" incompatible id to label map: {kwargs['id2label']}."
                    " Since those arguments are inconsistent with each other,"
    ...
    def update_from_string(self, update_str: str):
        ...
        for k, v in d.items():
            ...
            if isinstance(old_v, bool):
                ...
                raise ValueError(
                    "You can only update int, float, bool or string values in"
                    f" the config, got {v} for key {k}"
                )
...
PretrainedConfig.push_to_hub.__doc__ = (
    PretrainedConfig.push_to_hub.__doc__.format(
        object="config",

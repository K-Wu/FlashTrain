    ...
    def _measure_speed(self, func) -> float:
        try:
            if self.args.is_tpu or self.args.torchscript:
                ...
                logger.info(
                    "Do inference on TPU or torchscript. Running model 5 times"
                    " to stabilize compilation"
                )
                timeit.repeat(
                    func,
                    repeat=1,
    ...
    def _measure_memory(
        ...
        try:
            ...
            if self.args.is_tpu:
                # tpu
                raise NotImplementedError(
                    "Memory Benchmarking is currently not implemented for TPU."
                    " Please disable memory benchmarking with `--no-memory` or"
                    " `args.memory=False`"
                )
                ...
                if not is_py3nvml_available():
                    ...
                    logger.info(
                        "Measuring total GPU usage on GPU device. Make sure to"
                        " not have additional processes running on the same"
                        " GPU."
                    )
                    # init nvml
                    nvml.nvmlInit()

...
class PreTrainedTokenizerFast(PreTrainedTokenizerBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            ...
            raise ValueError(
                "Cannot instantiate this tokenizer from a slow version. If"
                " it's based on sentencepiece, make sure you have"
        ...
        if tokenizer_object is not None:
            ...
            raise ValueError(
                ...
                " a `tokenizers` library serialization file, \n(2) a slow"
                " tokenizer instance to convert or \n(3) an equivalent slow"
                " tokenizer class to instantiate and convert. \nYou need to"
                " have sentencepiece installed to convert a slow tokenizer to"
                " a fast one."
            )
    ...
    def _batch_encode_plus(
        ...
        if not isinstance(batch_text_or_text_pairs, (tuple, list)):
            raise TypeError(
                "batch_text_or_text_pairs has to be a list or a tuple (got"
    ...
    def _save_pretrained(
        ...
        if self.slow_tokenizer_class is None and legacy_format is True:
            raise ValueError(
                "Your tokenizer does not have a legacy version defined and"
                " therefore cannot register this version. You might consider"
                " leaving the legacy_format at `None` or setting it to"
                " `False`."
            )
        ...
        **kwargs,
        ...
        if tokenizer_json["model"]["type"] == "BPE":
            ...
            raise ValueError(
                ...
                f" {tokenizer_json['model']['type']}) only BPE, Unigram,"
                " WordLevel and WordPiece."
            )

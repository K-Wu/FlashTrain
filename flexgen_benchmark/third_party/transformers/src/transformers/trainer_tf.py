...
class TFTrainer:
    ...
    def __init__(
        ...
        warnings.warn(
            (
                "The class `TFTrainer` is deprecated and will be removed in"
                " version 5 of Transformers. "
                "We recommend using native Keras instead, by calling methods"
                " like `fit()` and `predict()` "
                "directly on the model object. Detailed examples of the Keras"
                " style can be found in our "
                "examples at"
        ...
        if is_wandb_available():
            ...
            os.getenv("WANDB_DISABLED", "").upper() not in ENV_VARS_TRUE_VALUES
        ):
            logger.info(
                "You are instantiating a Trainer but W&B is not installed. To"
                " use wandb logging, run `pip install wandb && wandb login`"
    ...
    def prediction_loop(
        ...
        if description == "Evaluation":
            logger.info(
                "  Num examples in used in evaluation ="
                f" {self.args.eval_batch_size * steps}"
            )
    ...
    def train(self) -> None:
        ...
        with self.args.strategy.scope():
            ...
            if self.model.ckpt_manager.latest_checkpoint:
                logger.info(
                    "Checkpoint file"
                    f" {self.model.ckpt_manager.latest_checkpoint} found and"
                ...
                logger.info(
                    f"  Continuing training from epoch {epochs_trained}"
                )
                logger.info(
                ...
                logger.info(
                    "  Will skip the first"
                    f" {steps_trained_in_current_epoch} steps in the first"
                    " epoch"
                )
            ...
            logger.info(
                ...
                f" accumulation) = {self.total_train_batch_size}"
            )
            logger.info(
                "  Gradient Accumulation steps ="
                f" {self.args.gradient_accumulation_steps}"
            )
            ...
            for epoch_iter in range(epochs_trained, int(epochs)):
                ...
                for step, batch in enumerate(train_ds):
                    ...
                    self.global_step = iterations.numpy()
                    self.epoch_logging = (
                        epoch_iter + (step + 1) / self.steps_per_epoch
                    )
                    ...
                    if (
                        ...
                        and self.args.evaluation_strategy
                    ...
                    if (
                        ...
                        logger.info(
                            f"Saving checkpoint for step {self.global_step} at"
                            f" {ckpt_save_path}"
                        )
    ...
    def run_model(self, features, labels, training):
        ...
        if (
            ...
            and getattr(self, "_past", None) is not None
        ):
        ...
        if isinstance(labels, (dict)):
            ...
            outputs = self.model(features, labels=labels, training=training)[
                :2
            ]

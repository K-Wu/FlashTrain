...
def apply_chunking_to_forward(
    ...
    *input_tensors
    ...
    if num_args_in_forward_chunk_fn != len(input_tensors):
        raise ValueError(
            "forward_chunk_fn expects"
            f" {num_args_in_forward_chunk_fn} arguments, but only"
        ...
        for input_tensor in input_tensors:
            if input_tensor.shape[chunk_dim] != tensor_shape:
                raise ValueError(
                    ...
                    f" {tensor_shape}, found shape"
                    f" {input_tensor.shape[chunk_dim]}"
                )

        if input_tensors[0].shape[chunk_dim] % chunk_size != 0:
            raise ValueError(
                "The dimension to be chunked"
                f" {input_tensors[0].shape[chunk_dim]} has to be a multiple of"

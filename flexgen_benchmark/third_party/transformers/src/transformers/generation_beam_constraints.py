    ...
    def test(self):
        ...
        while not completed:
            ...
            if not self.does_advance(advance):
                raise Exception(
                    "Custom Constraint is not defined correctly."
                    " self.does_advance(self.advance()) must be true."
                )
...
class PhrasalConstraint(Constraint):
    ...
    def __init__(self, token_ids: List[int]):
        ...
        if any(
            ...
            raise ValueError(
                "Each list in `token_ids` has to be a list of positive"
                f" integers, but is {token_ids}."
            )
    ...
    def does_advance(self, token_id: int):
        if not isinstance(token_id, int):
            raise ValueError(
                f"`token_id` has to be an `int`, but is {token_id} of type"
                f" {type(token_id)}"
            )
...
class DisjunctiveTrie:
    def __init__(self, nested_token_ids: List[List[int]], no_subsets=True):
        ...
        if no_subsets and self.has_subsets(root, nested_token_ids):
            raise ValueError(
                "Each list in `nested_token_ids` can't be a complete subset"
                f" of another list, but is {nested_token_ids}."
            )
...
class DisjunctiveConstraint(Constraint):
    ...
    def __init__(self, nested_token_ids: List[List[int]]):
        ...
        if (
            not isinstance(nested_token_ids, list)
            or len(nested_token_ids) == 0
        ):
            raise ValueError(
                "`nested_token_ids` has to be a non-empty list, but is"
                f" {nested_token_ids}."
            )
        if any(
            ...
            raise ValueError(
                "`nested_token_ids` has to be a list of lists, but is"
                f" {nested_token_ids}."
            )
        if any(
            any(
            ...
            raise ValueError(
                "Each list in `nested_token_ids` has to be a list of positive"
                f" integers, but is {nested_token_ids}."
            )
    ...
    def does_advance(self, token_id: int):
        if not isinstance(token_id, int):
            raise ValueError(
                "`token_id` is supposed to be type `int`, but is"
                f" {token_id} of type {type(token_id)}"
            )
...
class ConstraintListState:
    ...
    def add(self, token_id: int):
        ...
        if self.inprogress_constraint is not None:
            ...
            for cidx, pending_constraint in enumerate(
                self.pending_constraints
            ):
                if pending_constraint.does_advance(token_id):
                    ...
                    if not stepped:
                        raise Exception(
                            "`constraint.update(token_id)` is not yielding"
                            " incremental progress, even though"
                            " `constraint.does_advance(token_id)` is true."
                        )

...
def apply_transpose(
    transpose: TransposeType, weight, match_shape=None, pt_to_tf=True
):
    """
...
def load_pytorch_checkpoint_in_tf2_model(
    ...
    try:
        ...
        logger.error(
            "Loading a PyTorch model in TensorFlow, requires both PyTorch and"
            " TensorFlow to be installed. Please see https://pytorch.org/ and"
...
def load_pytorch_state_dict_in_tf2_model(
    ...
    if not any(
        s.startswith(tf_model.base_model_prefix) for s in pt_state_dict.keys()
    ):
    ...
    for symbolic_weight in symbolic_weights:
        ...
        array = apply_transpose(
            transpose, pt_state_dict[name], symbolic_weight.shape
        )
    ...
    if len(unexpected_keys) > 0:
        logger.warning(
            "Some weights of the PyTorch model were not used when"
            f" initializing the TF 2.0 model {tf_model.__class__.__name__}:"
            f" {unexpected_keys}\n- This IS expected if you are initializing"
            f" {tf_model.__class__.__name__} from a PyTorch model trained on"
            " another task or with another architecture (e.g. initializing a"
            " TFBertForSequenceClassification model from a BertForPreTraining"
            " model).\n- This IS NOT expected if you are initializing"
            f" {tf_model.__class__.__name__} from a PyTorch model that you"
            " expect to be exactly identical (e.g. initializing a"
            " TFBertForSequenceClassification model from a"
    ...
    if len(missing_keys) > 0:
        logger.warning(
            ...
            f" {tf_model.__class__.__name__} were not initialized from the"
            f" PyTorch model and are newly initialized: {missing_keys}\nYou"
            " should probably TRAIN this model on a down-stream task to be"
            " able to use it for predictions and inference."
        )
    else:
        logger.warning(
            f"All the weights of {tf_model.__class__.__name__} were"
            " initialized from the PyTorch model.\nIf your task is similar to"
            " the task the model of the checkpoint was trained on, you can"
            f" already use {tf_model.__class__.__name__} for predictions"
...
def load_tf2_weights_in_pytorch_model(
    ...
    tf_state_dict = {
        tf_weight.name: tf_weight.numpy() for tf_weight in tf_weights
    }
...
def load_tf2_state_dict_in_pytorch_model(
    ...
    if not any(
        ...
        for s in current_pt_params_dict.keys()
    ):
    ...
    for pt_weight_name, pt_weight in current_pt_params_dict.items():
        ...
        array = apply_transpose(
            transpose, array, pt_weight.shape, pt_to_tf=False
        )
    ...
    if len(unexpected_keys) > 0:
        logger.warning(
            "Some weights of the TF 2.0 model were not used when initializing"
            f" the PyTorch model {pt_model.__class__.__name__}:"
            f" {unexpected_keys}\n- This IS expected if you are initializing"
            f" {pt_model.__class__.__name__} from a TF 2.0 model trained on"
            " another task or with another architecture (e.g. initializing a"
            " BertForSequenceClassification model from a TFBertForPreTraining"
            " model).\n- This IS NOT expected if you are initializing"
            f" {pt_model.__class__.__name__} from a TF 2.0 model that you"
            " expect to be exactly identical (e.g. initializing a"
    ...
    if len(missing_keys) > 0:
        ...
        logger.warning(
            ...
            " initialized from the TF 2.0 model.\nIf your task is similar to"
            " the task the model of the checkpoint was trained on, you can"

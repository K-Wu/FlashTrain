...
def check_imports(filename):
    ...
    if len(missing_packages) > 0:
        raise ImportError(
            ...
            f" found in your environment: {', '.join(missing_packages)}. Run"
...
def get_cached_module_file(
    ...
    try:
        ...
        logger.error(
            f"Could not locate the {module_file} inside"
...
def custom_object_save(obj, folder, config=None):
    ...
    if obj.__module__ == "__main__":
        logger.warning(
            f"We can't save the code defining {obj} in {folder} as it's been"
            " defined in __main__. You should put this code in a separate"
            " module so we can include it in the saved folder and make it"
            " easier to share via the Hub."
        )
        ...
        if "Tokenizer" in full_name:
            ...
            if obj.__class__.__name__.endswith("Fast"):
                ...
                fast_tokenizer_class = (
                    f"{last_module}.{obj.__class__.__name__}"
                )

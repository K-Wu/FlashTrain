...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The framework to use for the ONNX export. If not provided, will"
            " attempt to use the local checkpoint's original framework or what"
    ...
    parser.add_argument(
        ...
        default="auto",
        help=(
            "Which type of preprocessor to use. 'auto' tries to automatically"
            " detect it."
        ),
    )
    ...
    if model_kind in ENCODER_DECODER_MODELS:
        ...
        if args.opset < min(
            ...
            raise ValueError(
                ...
                " At least "
                f" {min(encoder_onnx_config.default_onnx_opset, decoder_onnx_config.default_onnx_opset)} is"
                " required."
            )
        ...
        validate_model_outputs(
            ...
            args.atol
            if args.atol
            else encoder_onnx_config.atol_for_validation,
        )
        ...
        validate_model_outputs(
            ...
            args.atol
            if args.atol
            else decoder_onnx_config.atol_for_validation,
        )
        logger.info(
            "All good, model saved at:"
            f" {args.output.parent.joinpath('encoder_model.onnx').as_posix()},"
        ...
        if args.preprocessor == "auto":
            ...
            raise ValueError(
                f"Unknown preprocessor type '{args.preprocessor}'"
            )

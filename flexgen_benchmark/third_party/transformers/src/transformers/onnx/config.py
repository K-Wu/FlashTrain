...
class OnnxConfig(ABC):
    ...
    _tasks_to_common_outputs = {
        ...
        "image-classification": OrderedDict(
        ...
        "token-classification": OrderedDict(
            {"logits": {0: "batch", 1: "sequence"}}
        ),
    ...
    def __init__(
        ...
        if task not in self._tasks_to_common_outputs:
            raise ValueError(
                f"{task} is not a supported task, supported tasks:"
                f" {self._tasks_to_common_outputs.keys()}"
            )
        self.task = task
    ...
    def use_external_data_format(num_parameters: int) -> bool:
        ...
        return (
            compute_serialized_parameters_size(
                num_parameters, ParameterFormat.Float
            )
    ...
    def _generate_dummy_images(
        ...
        for _ in range(batch_size):
            data = (
                np.random.rand(image_height, image_width, num_channels) * 255
            )
    ...
    def _generate_dummy_audio(
        ...
        for _ in range(batch_size):
            ...
            t = np.linspace(
                0,
                time_duration,
                int(time_duration * sampling_rate),
                endpoint=False,
            )
    ...
    def generate_dummy_inputs(
        ...
        if tokenizer is not None:
            warnings.warn(
                (
                    "The `tokenizer` argument is deprecated and will be"
                    " removed in version 5 of Transformers. Use `preprocessor`"
                    " instead."
                ),
                FutureWarning,
            )
        ...
        if isinstance(preprocessor, PreTrainedTokenizerBase):
            ...
            if self.task == "multiple-choice":
                ...
                tokenized_input = preprocessor(
                    dummy_input, text_pair=dummy_input
                )
                ...
                return dict(
                    tokenized_input.convert_to_tensors(tensor_type=framework)
                )
            ...
            return dict(
                preprocessor(images=dummy_input, return_tensors=framework)
            )
        elif (
            isinstance(preprocessor, FeatureExtractionMixin)
            ...
            raise ValueError(
                "Unable to generate dummy inputs for the model. Please provide"
                " a tokenizer or a preprocessor."
            )
...
class OnnxConfigWithPast(OnnxConfig, ABC):
    ...
    def outputs(self) -> Mapping[str, Mapping[int, str]]:
        ...
        if self.use_past:
            self.fill_with_past_key_values_(
                common_outputs, direction="outputs"
            )
    ...
    def num_layers(self) -> int:
        ...
        if not hasattr(self._config, "num_layers"):
            raise AttributeError(
                "could not find the number of layers attribute in the model"
                " configuration, override the num_layers property of the model"
                " OnnxConfig to solve this"
            )
    ...
    def num_attention_heads(self) -> int:
        ...
        if not hasattr(self._config, "num_attention_heads"):
            raise AttributeError(
                "could not find the number of attention heads attribute in the"
                " model configuration, override the num_attention_heads"
    ...
    def generate_dummy_inputs(
        ...
        if self.use_past:
            if not is_torch_available():
                raise ValueError(
                    "Cannot generate dummy past_keys inputs without PyTorch"
                    " installed."
                )
            else:
                import torch
            ...
            if "attention_mask" in common_inputs:
                ...
                common_inputs["attention_mask"] = torch.cat(
                    [
                        ...
                        torch.ones(
                            batch, past_key_values_length, dtype=mask_dtype
                        ),
                    ],
                    dim=1,
                )
    ...
    def fill_with_past_key_values_(
        ...
        if direction not in ["inputs", "outputs"]:
            raise ValueError(
                'direction must either be "inputs" or "outputs", but'
                f" {direction} was given"
            )
    ...
    def flatten_output_collection_property(
        ...
        if name in ["present", "past_key_values"]:
            ...
            flattened_output = super().flatten_output_collection_property(
                name, field
            )
...
class OnnxSeq2SeqConfigWithPast(OnnxConfigWithPast):
    ...
    def num_layers(self) -> Tuple[int]:
        try:
            ...
            if hasattr(self._config, "encoder_layers") and hasattr(
                ...
                num_layers = (
                    self._config.encoder_layers,
                    self._config.decoder_layers,
                )
            else:
                raise AttributeError(
                    "could not find the number of encoder and decoder layers"
                    " attributes in the model configuration, override the"
                    " num_layers property of the model OnnxConfig to solve"
                    " this"
                )
    ...
    def num_attention_heads(self) -> Tuple[int]:
        try:
            ...
            if hasattr(self._config, "encoder_attention_heads") and hasattr(
                ...
                raise AttributeError(
                    "could not find the number of attention heads for the"
                    " encoder and the decoder attributes in the model"
                    " configuration, override the num_attention_heads property"
                    " of the model OnnxConfig to solve this"
                )
    ...
    def fill_with_past_key_values_(
        ...
        for i in range(min_num_layers, max_num_layers):
            ...
            inputs_or_outputs[
                f"{name}.{i}.{remaining_side_name}.key"
            ] = axes_info

...
if not is_torch_available() and not is_tf_available():
    logger.warning(
        "The ONNX export features are only supported for PyTorch or"
        " TensorFlow. You will not be able to export models without one of"
    ...
    *supported_features: str, onnx_config_cls: str = None
    ...
    for feature in supported_features:
        if "-with-past" in feature:
            ...
            mapping[feature] = partial(
                config_cls.from_model_config, task=feature
            )
...
class FeaturesManager:
    ...
    _SUPPORTED_MODEL_TYPE = {
        ...
        "blenderbot-small": supported_features_mapping(
            ...
            "seq2seq-lm-with-past",
            onnx_config_cls=(
                "models.blenderbot_small.BlenderbotSmallOnnxConfig"
            ),
        ),
    ...
    AVAILABLE_FEATURES = sorted(
        reduce(
            lambda s1, s2: s1 | s2,
            (v.keys() for v in _SUPPORTED_MODEL_TYPE.values()),
        )
    )
    ...
    def get_supported_features_for_model_type(
        ...
        if model_type not in FeaturesManager._SUPPORTED_MODEL_TYPE:
            ...
            raise KeyError(
                f"{model_type_and_model_name} is not supported yet. Only"
                f" {list(FeaturesManager._SUPPORTED_MODEL_TYPE.keys())} are"
                f" supported. If you want to support {model_type} please"
                " propose a PR or open up an issue."
            )
    ...
    def _validate_framework_choice(framework: str):
        ...
        if framework not in ["pt", "tf"]:
            raise ValueError(
                "Only two frameworks are supported for ONNX export: pt or tf,"
                f" but {framework} was provided."
            )
        elif framework == "pt" and not is_torch_available():
            raise RuntimeError(
                "Cannot export model to ONNX using PyTorch because no PyTorch"
                " package was found."
            )
        elif framework == "tf" and not is_tf_available():
            raise RuntimeError(
                "Cannot export model to ONNX using TensorFlow because no"
                " TensorFlow package was found."
            )

    @staticmethod
    def get_model_class_for_feature(
        ...
        if task not in task_to_automodel:
            raise KeyError(
                f"Unknown task: {feature}. Possible values are"
                f" {list(FeaturesManager._TASKS_TO_AUTOMODELS.values())}"
            )
        ...
        if os.path.isdir(model):
            if os.path.isfile(os.path.join(model, WEIGHTS_NAME)):
                ...
                raise FileNotFoundError(
                    "Cannot determine framework from given checkpoint"
                    f" location. There should be a {WEIGHTS_NAME} for PyTorch"
            ...
            if is_torch_available():
                ...
                raise EnvironmentError(
                    "Neither PyTorch nor TensorFlow found in environment."
                    " Cannot export to ONNX."
                )

        logger.info(
            f"Framework not requested. Using {exporter_map[framework]} to"
            " export to ONNX."
        )
    ...
    def get_model_from_feature(
        ...
        try:
            ...
            if framework == "pt":
                logger.info(
                    "Loading TensorFlow model in PyTorch before exporting to"
                    " ONNX."
                )
                ...
                logger.info(
                    "Loading PyTorch model in TensorFlow before exporting to"
                    " ONNX."
                )
                model = model_class.from_pretrained(

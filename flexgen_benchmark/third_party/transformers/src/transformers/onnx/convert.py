    ...
    try:
        ...
        if ort_version < ORT_QUANTIZE_MINIMUM_VERSION:
            raise ImportError(
                ...
                f" ({onnxruntime.__version__}) but we require onnxruntime to"
                f" be >= {minimum_version} to enable all the conversions"
                " options.\nPlease update onnxruntime by running `pip install"
                " --upgrade onnxruntime`"
            )

    except ImportError:
        raise ImportError(
            "onnxruntime doesn't seem to be currently installed. Please"
            " install the onnxruntime by running `pip install onnxruntime` and"
...
def export_pytorch(
    ...
    if (
        isinstance(preprocessor, PreTrainedTokenizerBase)
        and tokenizer is not None
    ):
        raise ValueError(
            "You cannot provide both a tokenizer and a preprocessor to export"
            " the model."
        )
    if tokenizer is not None:
        warnings.warn(
            (
                "The `tokenizer` argument is deprecated and will be removed in"
                " version 5 of Transformers. Use `preprocessor` instead."
            ),
            FutureWarning,
        )
        logger.info(
            "Overwriting the `preprocessor` argument with `tokenizer` to"
            " generate dummmy inputs."
        )
    ...
    if issubclass(type(model), PreTrainedModel):
        ...
        with torch.no_grad():
            ...
            if config.values_override is not None:
                logger.info(
                    f"Overriding {len(config.values_override)} configuration"
                    " item(s)"
                )
                for (
                    override_config_key,
            ...
            if is_torch_less_than_1_11:
                ...
                try:
                    ...
                    if (
                        message
                        == "Exporting model exceed maximum protobuf size of"
                        " 2GB. Please call torch.onnx.export without"
                        " setting use_external_data_format parameter."
                    ):
                        message = (
                            "Exporting model exceed maximum protobuf size of"
                            " 2GB. Please call torch.onnx.export without"
                            " setting use_external_data_format parameter or"
                            " try with torch 1.10+."
                        )
...
def export_tensorflow(
    ...
    if (
        ...
        raise ValueError(
            "You cannot provide both a tokenizer and preprocessor to export"
            " the model."
        )
    if tokenizer is not None:
        warnings.warn(
...
def export(
    ...
    if not (is_torch_available() or is_tf_available()):
        raise ImportError(
            "Cannot convert because neither PyTorch nor TensorFlow are not"
            " installed. Please install torch or tensorflow first."
        )
    ...
    if is_torch_available():
        ...
        if not config.is_torch_support_available:
            logger.warning(
                "Unsupported PyTorch version for this model. Minimum required"
                f" is {config.torch_onnx_minimum_version}, got:"
                f" {torch_version}"
            )
...
def validate_model_outputs(
    ...
    if (
        ...
        raise ValueError(
            "You cannot provide both a tokenizer and a preprocessor to"
    ...
    if is_torch_available() and issubclass(
        type(reference_model), PreTrainedModel
    ):
    ...
    reference_model_inputs_onnxruntime = (
        config.generate_dummy_inputs_onnxruntime(reference_model_inputs)
    )
    ...
    if not onnx_outputs_set.issubset(ref_outputs_set):
        logger.info(
            f"\t-[x] ONNX model output names {onnx_outputs_set} do not match"
            f" reference model {ref_outputs_set}"
        )

        raise ValueError(
            "Outputs doesn't match between reference model and ONNX exported"
            f" model: {onnx_outputs_set.difference(ref_outputs_set)}"
        )
    else:
        logger.info(
            "\t-[✓] ONNX model output names match reference model"
            f" ({onnx_outputs_set})"
        )
    ...
    for name, ort_value in zip(onnx_named_outputs, onnx_outputs):
        ...
        if not ort_value.shape == ref_value.shape:
            logger.info(
                f"\t\t-[x] shape {ort_value.shape} doesn't match"
                f" {ref_value.shape}"
            )
            raise ValueError(
                "Outputs shape doesn't match between reference model and ONNX"
                f" exported model: Got {ref_value.shape} (reference) and"
                f" {ort_value.shape} (ONNX)"
            )
        else:
            logger.info(
        ...
        if not np.allclose(ref_value, ort_value, atol=atol):
            bad_indices = np.logical_not(
                np.isclose(ref_value, ort_value, atol=atol)
            )
            logger.info(f"\t\t-[x] values not close enough (atol: {atol})")
            raise ValueError(
                "Outputs values doesn't match between reference model and"
                " ONNX exported model: Got max absolute difference of:"
                f" {np.amax(np.abs(ref_value - ort_value))} for"

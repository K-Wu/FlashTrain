...
def convert_pt_checkpoint_to_tf(
    ...
    if compare_with_pt_model:
        tfo = tf_model(
            tf_model.dummy_inputs, training=False
        )  # build the network
        ...
        assert (
            diff <= 2e-2
        ), f"Error, model absolute difference is >2e-2: {diff}"
...
def convert_all_pt_checkpoints_to_tf(
    ...
    for j, model_type in enumerate(model_types, start=1):
        ...
        if model_type not in MODEL_CLASSES:
            raise ValueError(
                f"Unrecognized model type {model_type}, should be one of"
                f" {list(MODEL_CLASSES.keys())}."
            )
        ...
        for i, (model_shortcut_name, config_shortcut_name) in enumerate(
            zip(model_shortcut_names_or_path, config_shortcut_names_or_path),
            start=1,
        ):
            print("-" * 100)
            if (
                ...
                continue
            print(
                f"    Converting checkpoint {i}/{len(aws_config_map)}:"
                f" {model_shortcut_name} - model_type {model_type}"
            )
            print("-" * 100)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            f"Model type selected in the list of {list(MODEL_CLASSES.keys())}."
            " If not given, will download and convert all the models from"
            " AWS."
        ),
    )
    parser.add_argument(
        ...
        type=str,
        help=(
            "Path to the PyTorch checkpoint path or shortcut name to download"
            " from AWS. If not given, will download and convert all the"
            " checkpoints from AWS."
        ),
    )
    parser.add_argument(
        "--config_file",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use cached models if possible instead of updating to latest"
            " checkpoint versions."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Remove pytorch models after conversion (save memory when"

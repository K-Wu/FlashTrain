...
def add_start_docstrings_to_model_forward(*docstr):
    def docstring_decorator(fn):
        ...
        intro = (
            f"   The {class_name} forward method, overrides the `__call__`"
            " special method."
        )
        note = r"""
...
def _prepare_output_docstrings(output_type, config_class, min_indent=None):
    ...
    if min_indent is not None:
        ...
        if indent < min_indent:
            ...
            lines = [
                (f"{to_add}{line}" if len(line) > 0 else line)
                for line in lines
            ]
            result = "\n".join(lines)
...
def replace_return_docstrings(output_type=None, config_class=None):
    def docstring_decorator(fn):
        ...
        while (
            i < len(lines)
            and re.search(r"^\s*Returns?:\s*$", lines[i]) is None
        ):
            i += 1
        if i < len(lines):
            indent = len(_get_indent(lines[i]))
            ...
            raise ValueError(
                f"The function {fn} should have an empty 'Return:' or"
                " 'Returns:' in its docstring as placeholder, current"
                f" docstring is:\n{func_doc}"
            )
        fn.__doc__ = func_doc
        return fn

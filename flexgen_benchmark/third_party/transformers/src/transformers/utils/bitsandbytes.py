...
def set_module_8bit_tensor_to_device(module, tensor_name, device, value=None):
    ...
    if (
        ...
        raise ValueError(
            f"{module} does not have a parameter or a buffer named"
            f" {tensor_name}."
        )
    ...
    if (
        ...
        raise ValueError(
            f"{tensor_name} is on the meta device, we need a `value` to put in"
            f" on {device}."
        )
        ...
        if param.device.type != "cuda":
            if value is None:
                ...
                if value.dtype == torch.int8:
                    raise ValueError(
                        (
                            "You cannot load weights that are saved in int8"
                            " using `load_in_8bit=True`, make sure you are"
                        ),
                        (
                            " using `load_in_8bit=True` on"
                            " float32/float16/bfloat16 weights."
                        ),
                    )
            else:

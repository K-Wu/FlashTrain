...
_IS_IN_DEBUG_MODE = (
    os.environ.get("FX_DEBUG_MODE", "").upper() in ENV_VARS_TRUE_VALUES
)
...
for item in _REGULAR_SUPPORTED_MODEL_NAMES_AND_TASKS:
    if isinstance(item, dict):
        _REGULAR_SUPPORTED_MODELS.extend(
            _generate_supported_model_class_names(**item)
        )
    else:
        _REGULAR_SUPPORTED_MODELS.extend(
            _generate_supported_model_class_names(item)
...
def torch_where(condition, x, y):
    ...
    return (
        condition.to(device="meta") + x.to(device="meta") + y.to(device="meta")
    )
...
def torch_unique_consecutive(input, **kwargs):
    output = torch.unique_consecutive(
        torch.zeros_like(input, device="cpu"), **kwargs
    )
...
def torch_nn_functional_one_hot(tensor, num_classes=-1):
    if num_classes < 0:
        raise ValueError(
            "Don't support automatic num_classes inference for MetaTensor"
            " analysis"
        )
...
def operator_getitem(a, b):
    ...
    if isinstance(a, torch.Tensor):
        ...
        if isinstance(b, tuple):
            ...
            b = to_concrete(b)
        return operator.getitem(torch.empty_like(a, device="cpu"), b).to(
            "meta"
        )
...
class HFTracer(Tracer):
    ...
    def __init__(self, autowrap_modules=(math,), autowrap_functions=()):
        ...
        if not is_torch_fx_available():
            ...
            raise ImportError(
                "Found an incompatible version of torch. Found version"
                f" {torch_version}, but only version"
            ...
            if model_class_name in [
                ...
                *get_values(
                ...
                if (
                    ...
                    raise ValueError(
                        "Could not retrieve the problem type for the sequence"
                        " classification task, please set"
                        " model.config.problem_type to one of the following"
                        ' values: "regression", "single_label_classification",'
                ...
                if model.config.problem_type == "regression":
                    ...
                    raise ValueError(
                        "Expected model.config.problem_type to be either:"
                        ' "regression", "single_label_classification", or'
                        ' "multi_label_classification", but'
                ...
                *get_values(MODEL_FOR_SEMANTIC_SEGMENTATION_MAPPING_NAMES),
                ...
                raise NotImplementedError(
                    f"Generating the dummy input named {input_name} for"
                    f" {model_class_name} is not supported yet."
                )
    ...
    def create_proxy(
        ...
        try:
            ...
            if kind == "call_function":
                ...
                if not hasattr(self, "orig_forward"):
                    raise AttributeError(
                        f"{self} does not have an attribute called"
                        " orig_forward"
                    )
                ...
                try:
                    ...
                    if mod_type in _MANUAL_META_OVERRIDES:
                        ...
                        meta_out = self.orig_forward(
                            *args_metas, **kwargs_metas
                        )
                finally:
            ...
            if _IS_IN_DEBUG_MODE:
                warnings.warn(
                    f"Could not compute metadata for {kind} target"
                    f" {target}: {e}"
                )
        ...
        if getattr(self, "_disable_module_getattr", False):
            ...
            if isinstance(attr_val, torch.nn.Parameter):
                maybe_parameter_proxy = maybe_get_proxy_for_attr(
                    attr_val,
                    self.root.named_parameters(),
                    parameter_proxy_cache,
                )
                if maybe_parameter_proxy is not None:
                    return maybe_parameter_proxy

            if self.proxy_buffer_attributes and isinstance(
    ...
    def trace(
        ...
        if (
            ...
            for param in sig.parameters.values():
                ...
                if param.default is inspect.Parameter.empty:
                    raise ValueError(
                        "You need to specify a default value for the"
        ...
        for input_name in input_names:
            ...
            if isinstance(root, PreTrainedModel) or type(
                root
            ).__qualname__.startswith("_deserialize_graph_module"):
                inputs.update(
                    self._generate_dummy_input(root, input_name, shape)
                )
            else:
                raise RuntimeError(
                    f"Could not generate input named {input_name} for because"
                    " root is not a transformers.PreTrainedModel."
                )
    ...
    def _stateless_mod_instanciation_depends_on_proxies(
        self, mod: nn.Module
    ) -> bool:
        """
...
def get_concrete_args(model: nn.Module, input_names: List[str]):
    ...
    if not (set(input_names) <= set(sig.parameters.keys())):
        ...
        raise ValueError(
            f"The model does not have input(s) named: {formatted_input_names},"
            " expected a subset of the following:"
...
def check_if_model_is_supported(model: PreTrainedModel):
    if model.__class__.__name__ not in _SUPPORTED_MODELS:
        ...
        raise NotImplementedError(
            f"Model {model.__class__.__name__} is not supported yet, supported"
            f" models: {supported_model_names}"
        )

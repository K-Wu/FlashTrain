    ...
    if (
        ...
        if _tf_available:
            if version.parse(_tf_version) < version.parse("2"):
                logger.info(
                    f"TensorFlow found but with version {_tf_version}."
                    " Transformers requires version 2 minimum."
                )
                _tf_available = False
            else:
...
if USE_JAX in ENV_VARS_TRUE_AND_AUTO_VALUES:
    ...
    if _flax_available:
        try:
            ...
            logger.info(
                f"JAX version {_jax_version}, Flax version"
                f" {_flax_version} available."
            )
        except importlib_metadata.PackageNotFoundError:
...
try:
    ...
    logger.debug(
        "Successfully imported pytorch-quantization version"
        f" {_pytorch_quantization_version}"
    )
except importlib_metadata.PackageNotFoundError:
...
try:
    ...
    logger.debug(
        "Successfully imported tensorflow-probability version"
        f" {_tensorflow_probability_version}"
    )
except importlib_metadata.PackageNotFoundError:
...
def is_torch_bf16_available():
    ...
    warnings.warn(
        (
            "The util is_torch_bf16_available is deprecated, please use"
            " is_torch_bf16_gpu_available or is_torch_bf16_cpu_available"
            " instead according to whether it's used with cpu or gpu"
        ),
        FutureWarning,
    )
...
def is_ipex_available():
    ...
    if torch_major_and_minor != ipex_major_and_minor:
        logger.warning(
            f"Intel Extension for PyTorch {ipex_major_and_minor} needs to work"
            f" with PyTorch {ipex_major_and_minor}.*, but PyTorch"
            f" {_torch_version} is found. Please switch to the matching"
...
def torch_only_method(fn):
    def wrapper(*args, **kwargs):
        if not _torch_available:
            raise ImportError(
                "You need to install pytorch to use this method or class, or"
                " activate it with environment variables USE_TORCH=1 and"
                " USE_TF=0."
            )
        else:
...
class _LazyModule(ModuleType):
    ...
    def _get_module(self, module_name: str):
        try:
            ...
            raise RuntimeError(
                f"Failed to import {self.__name__}.{module_name} because of"
                f" the following error (look up to see its traceback):\n{e}"
            ) from e

...
if (
    ...
    logger.warning(
        "In Transformers v4.0.0, the default path to cache downloaded models"
        " changed from '~/.cache/torch/transformers' to"
        " '~/.cache/huggingface/transformers'. Since you don't seem to have"
        " overridden and '~/.cache/torch/transformers' is a directory that"
        " exists, we're moving it to '~/.cache/huggingface/transformers' to"
        " avoid redownloading models you have already in the cache. You should"
...
if os.environ.get("HUGGINGFACE_CO_RESOLVE_ENDPOINT", None) is not None:
    warnings.warn(
        (
            ...
            " is deprecated and will be removed in Transformers v5. Use"
            " `HF_ENDPOINT` instead."
        ),
        FutureWarning,
    )
...
def http_user_agent(user_agent: Union[Dict, str, None] = None) -> str:
    ...
    """
    ua = (
        f"transformers/{__version__}; python/{sys.version.split()[0]};"
    ...
    if (
        os.environ.get("TRANSFORMERS_IS_CI", "").upper()
        in ENV_VARS_TRUE_VALUES
    ):
        ua += "; is_ci/true"
...
def cached_file(
        ...
        if not os.path.isfile(resolved_file):
            if _raise_exceptions_for_missing_entries:
                raise EnvironmentError(
                    f"{path_or_repo_id} does not appear to have a file named"
                    f" {full_filename}. Checkout"
    ...
    if _commit_hash is not None:
        ...
        if resolved_file is not None:
            if resolved_file is not _CACHED_NO_EXIST:
                ...
                raise EnvironmentError(
                    f"Could not locate {full_filename} inside"
                    f" {path_or_repo_id}."
                )
    ...
    try:
        ...
        raise EnvironmentError(
            f"{path_or_repo_id} is not a local folder and is not a valid model"
            " identifier listed on 'https://huggingface.co/models'\nIf this"
            " is a private repository, make sure to pass a token having"
            " permission to this repo with `use_auth_token` or log in with"
            " `huggingface-cli login` and pass `use_auth_token=True`."
        )
    except RevisionNotFoundError:
        raise EnvironmentError(
            f"{revision} is not a valid git identifier (branch name, tag name"
            " or commit id) that exists for this model name. Check the model"
            f" page at 'https://huggingface.co/{path_or_repo_id}' for"
            " available revisions."
        )
        ...
        if (
            ...
            return None
        raise EnvironmentError(
            f"We couldn't connect to '{HUGGINGFACE_CO_RESOLVE_ENDPOINT}' to"
            " load this file, couldn't find it in the"
            f" cached files and it looks like {path_or_repo_id} is not the"
            " path to a directory containing a file named"
            f" {full_filename}.\nCheckout your internet connection or see how"
            " to run the library in offline mode at"
        ...
        raise EnvironmentError(
            f"{path_or_repo_id} does not appear to have a file named"
            f" {full_filename}. Checkout"
            f" 'https://huggingface.co/{path_or_repo_id}/{revision}' for"
            " available files."
        )
...
def download_url(url, proxies=None):
    ...
    """
    warnings.warn(
        f"Using `from_pretrained` with the url of a file (here {url}) is"
        " deprecated and won't be possible anymore in v5 of Transformers. You"
        " should host your file on the Hub (hf.co) instead and use the"
        " repository ID. Note that this is not compatible with the caching"
        " system (your file will be downloaded at each execution) or multiple"
        " processes (each process will download the file in a different"
        " temporary file)."
    )
...
def has_file(
    ...
    if isinstance(use_auth_token, str):
        ...
        if token is None:
            raise EnvironmentError(
                "You specified use_auth_token=True, but a huggingface token"
                " was not found."
            )
    ...
    try:
        ...
        logger.error(e)
        raise EnvironmentError(
            f"{path_or_repo} is not a local folder or a valid repository name"
            " on 'https://hf.co'."
        )
        ...
        raise EnvironmentError(
            f"{revision} is not a valid git identifier (branch name, tag name"
            " or commit id) that exists for this model name. Check the model"
            f" page at 'https://huggingface.co/{path_or_repo}' for available"
            " revisions."
        )
...
class PushToHubMixin:
    ...
    def _create_repo(
        ...
        if repo_url is not None:
            warnings.warn(
                "The `repo_url` argument is deprecated and will be removed in"
                " v5 of Transformers. Use `repo_id` instead."
            )
        ...
        if organization is not None:
            warnings.warn(
                "The `organization` argument is deprecated and will be removed"
                " in v5 of Transformers. Set your organization directly in the"
                " `repo_id` passed instead"
...
def convert_file_size_to_int(size: Union[int, str]):
    ...
    if size.upper().endswith("KB"):
        ...
        return int_size // 8 if size.endswith("b") else int_size
    raise ValueError(
        "`size` is not in a valid format. Use an integer followed by the unit,"
        " e.g., '5GB'."
    )
...
def get_checkpoint_shard_files(
    ...
    if not os.path.isfile(index_filename):
        raise ValueError(
            f"Can't find a checkpoint index ({index_filename}) in"
    ...
    for shard_filename in shard_filenames:
        ...
        # we don't have to catch them here.
            ...
            raise EnvironmentError(
                ...
                f" file named {shard_filename} which is required according to"
                " the checkpoint index."
            )
        except HTTPError:
            raise EnvironmentError(
                f"We couldn't connect to '{HUGGINGFACE_CO_RESOLVE_ENDPOINT}'"
                f" to load {shard_filename}. You should try again after"
...
if cache_version < 1 and cache_is_not_empty:
    if is_offline_mode():
        logger.warning(
            "You are offline and the cache for model files in Transformers"
            " v4.22.0 has been updated while your local cache seems to be the"
            " one of a previous version. It is very likely that all your calls"
            " to any `from_pretrained()` method will fail. Remove the offline"
            " mode and enable internet connection to have your cache be"
            " updated automatically, then you can go back to offline mode."
        )
    else:
        logger.warning(
            "The cache for model files in Transformers v4.22.0 has been"
            " updated. Migrating your old cache. This is a one-time only"
            " operation. You can interrupt this and resume the migration later"
            " on by calling `transformers.utils.move_cache()`."
        )
    try:
        ...
        logger.error(
            "There was a problem when trying to move your"
            f" cache:\n\n{trace}\n{e.__class__.__name__}: {e}\n\nPlease file"
            " an issue at"
            " https://github.com/huggingface/transformers/issues/new/choose"
            " and copy paste this whole message and we will do our best to"
            " help."
        )
...
if cache_version < 1:
    try:
        ...
        logger.warning(
            ...
            f" ({TRANSFORMERS_CACHE}). You should set the environment variable"

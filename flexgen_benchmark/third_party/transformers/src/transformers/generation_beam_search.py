    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not isinstance(num_beams, int) or num_beams <= 1:
            raise ValueError(
                "`num_beams` has to be an integer strictly greater than 1,"
                f" but is {num_beams}. For `num_beams` == 1, one should make"
                " use of `greedy_search` instead."
            )
        ...
        if (
            ...
            raise ValueError(
                ...
                f" `num_beam_groups`, but is {num_beam_groups} with"
                f" `num_beams` being {num_beams}."
            )

        if "max_length" in kwargs:
            warnings.warn(
                "Passing `max_length` to BeamSearchScorer is deprecated and"
                " has no effect. `max_length` should be passed directly to"
                " `beam_search(...)`, `beam_sample(...)`, or"
                " `group_beam_search(...)`."
            )
    ...
    def process(
        ...
        if not (batch_size == (input_ids.shape[0] // self.group_size)):
            if self.num_beam_groups > 1:
                raise ValueError(
                    f"A group beam size of {input_ids.shape[0]} is used as the"
                    f" input, but a group beam size of {self.group_size} is"
                    " expected by the beam scorer."
                )
            else:
                raise ValueError(
                    f"A beam size of {input_ids.shape[0]} is used as the"
                    f" input, but a beam size of {self.group_size} is expected"
                    " by the beam scorer."
                )
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            if self._done[batch_idx]:
                if self.num_beams < len(beam_hyp):
                    raise ValueError(
                        "Batch can only be done if at least"
                        f" {self.num_beams} beams have been generated"
                    )
            ...
            if beam_idx < self.group_size:
                raise ValueError(
                    f"At most {self.group_size} tokens in"
                    f" {next_tokens[batch_idx]} can be equal to `eos_token_id:"
                    f" {eos_token_id}`. Make sure {next_tokens[batch_idx]} are"
                    " corrected."
                )
...
class ConstrainedBeamSearchScorer(BeamScorer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "max_length" in kwargs:
            warnings.warn(
                "Passing `max_length` to ConstrainedBeamSearchScorer is"
                " deprecated and has no effect. `max_length` should be passed"
                " directly to `beam_search(...)`, `beam_sample(...)`, or"
                " `group_beam_search(...)`."
            )
    ...
    def step_sentence_constraint(
                ...
                for advance_token in advance_tokens:
                    ...
                    if advance_seq not in track_new["new_seqs"]:
                        ...
                        track_new["new_scores"].append(
                            this_batch_token_scores[seq_idx].take(
                                advance_token
                            )
                        )
        ...
        if len(track_new["new_indices"]) > 0:
            ...
            all_banks = torch.tensor(
                [one.get_bank() for one in all_states]
            ).to(device)
            ...
            rearrangers = torch.tensor(
                np.argsort(increments, kind="mergesort")
            )

...
class CallbackHandler(TrainerCallback):
    ...
    def __init__(self, callbacks, model, tokenizer, optimizer, lr_scheduler):
        ...
        if not any(
            ...
            logger.warning(
                ...
                " `DefaultFlowCallback` in its callbacks. You\n"
                + "should add one before training with"
                " `trainer.add_callback(DefaultFlowCallback). The current"
                " list of"
                + "callbacks is\n:"
                + self.callback_list
            )
    ...
    def add_callback(self, callback):
        ...
        if cb_class in [c.__class__ for c in self.callbacks]:
            logger.warning(
                f"You are adding a {cb_class} to the callbacks of this"
                " Trainer, but there is already one. The current"
    ...
    def on_evaluate(self, args, state, control, metrics, **kwargs):
        ...
        if metric_value is None:
            logger.warning(
                ...
                f" find {metric_to_check} so early stopping is disabled"
            )
            return

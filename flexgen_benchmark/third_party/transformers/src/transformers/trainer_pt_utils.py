...
def nested_concat(tensors, new_tensors, padding_index=-100):
    ...
    assert type(tensors) == type(new_tensors), (
        ...
        f" {type(tensors)} and {type(new_tensors)}."
    )
    if isinstance(tensors, (list, tuple)):
        ...
        return type(tensors)(
            {
                k: nested_concat(
                    t, new_tensors[k], padding_index=padding_index
                )
...
def nested_numpify(tensors):
    ...
    if isinstance(tensors, Mapping):
        return type(tensors)(
            {k: nested_numpify(t) for k, t in tensors.items()}
        )
...
class DistributedSamplerWithLoop(DistributedSampler):
    ...
    def __iter__(self):
        ...
        start_remainder = (
            1 if self.rank < len(self.dataset) % self.num_replicas else 0
        )
...
class SequentialDistributedSampler(Sampler):
    ...
    def __init__(self, dataset, num_replicas=None, rank=None, batch_size=None):
        warnings.warn(
            (
                "SequentialDistributedSampler is deprecated and will be"
        ...
        if batch_size is not None:
            self.num_samples = (
                int(math.ceil(num_samples / (batch_size * num_replicas)))
                * batch_size
            )
        else:
    ...
    def __iter__(self):
        ...
        assert len(indices) == self.total_size, (
            f"Indices length {len(indices)} and total size"
            f" {self.total_size} mismatched"
        )
        ...
        assert len(indices) == self.num_samples, (
            f"Indices length {len(indices)} and sample number"
            f" {self.num_samples} mismatched"
        )
...
def nested_new_like(arrays, num_samples, padding_index=-100):
    ...
    return np.full_like(
        arrays, padding_index, shape=(num_samples, *arrays.shape[1:])
    )
...
def nested_truncate(tensors, limit):
    ...
    if isinstance(tensors, Mapping):
        return type(tensors)(
            {k: nested_truncate(t, limit) for k, t in tensors.items()}
        )
...
class DistributedTensorGatherer:
    ...
    def __init__(
        ...
        warnings.warn(
            (
                "DistributedTensorGatherer is deprecated and will be removed"
        ...
        total_size = (
            world_size
            if make_multiple_of is None
            else world_size * make_multiple_of
        )
        self.total_samples = (
            int(np.ceil(num_samples / total_size)) * total_size
        )
    ...
    def add_arrays(self, arrays):
        ...
        slice_len, self._storage = self._nested_set_tensors(
            self._storage, arrays
        )
    ...
    def _nested_set_tensors(self, storage, arrays):
        if isinstance(arrays, (list, tuple)):
            result = [
                self._nested_set_tensors(x, y) for x, y in zip(storage, arrays)
            ]
            return result[0][0], type(arrays)(r[1] for r in result)
        assert arrays.shape[0] % self.world_size == 0, (
            "Arrays passed should all have a first dimension multiple of"
            f" {self.world_size}, found {arrays.shape[0]}."
        )
        ...
        for i in range(self.world_size):
            if len(arrays.shape) == 1:
                storage[
                    self._offsets[i] : self._offsets[i] + slice_len
                ] = arrays[i * slice_len : (i + 1) * slice_len]
            else:
                # Expand the array on the fly if needed.
                if (
                    len(storage.shape) > 1
                    and storage.shape[1] < arrays.shape[1]
                ):
                    storage = expand_like(
                        storage,
                        arrays.shape[1],
    ...
    def finalize(self):
        ...
        if self._offsets[0] != self.process_length:
            logger.warning(
                "Not all data has been set. Are you sure you passed all"
                " values?"
            )
        ...
        if lengths is None:
            ...
            if (
                ...
                raise ValueError(
                    ...
                    f" items are dictionaries with an '{model_input_name}'"
                    " key."
                )
            lengths = [len(feature[model_input_name]) for feature in dataset]
        elif isinstance(lengths, torch.Tensor):
            logger.info(
                "If lengths is a torch.Tensor, LengthGroupedSampler will be"
                " slow. Converting lengths to List[int]..."
            )
...
class DistributedLengthGroupedSampler(DistributedSampler):
    ...
    def __init__(
        ...
        if lengths is None:
            ...
            logger.info(
                "If lengths is a torch.Tensor, DistributedLengthGroupedSampler"
                " will be slow. Converting lengths to List[int]..."
            )
            lengths = lengths.tolist()
...
if is_sagemaker_mp_enabled():
    ...
    def smp_gather(tensor):
        if isinstance(tensor, (list, tuple)):
            ...
            raise TypeError(
                f"Can't gather the values of type {type(tensor)}, only of"
                " nested list/tuple/dicts of tensors."
            )
    ...
    def smp_nested_concat(tensor):
        if isinstance(tensor, (list, tuple)):
            ...
            return type(tensor)(
                {k: smp_nested_concat(v) for k, v in tensor.items()}
            )

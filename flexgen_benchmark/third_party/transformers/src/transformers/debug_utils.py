...
class DebugUnderflowOverflow:
    ...
    **Performance**:
    ...
    def batch_start_frame(self):
        self.expand_frame(
            f"\n\n{self.prefix} *** Starting batch"
            f" number={self.batch_number} ***"
        )
        self.expand_frame(f"{'abs min':8} {'abs max':8} metadata")

    def batch_end_frame(self):
        self.expand_frame(
            f"{self.prefix} *** Finished batch"
            f" number={self.batch_number-1} ***\n\n"
        )
    ...
    def forward_hook(self, module, input, output):
        ...
        trace_mode = (
            True if self.batch_number in self.trace_batch_nums else False
        )
        if trace_mode:
        ...
        if self.detected_overflow and not trace_mode:
            ...
            raise ValueError(
                ...
                " is no point running further. Please scroll up above this"
                " traceback to see the activation values prior to this event."
            )
        ...
        if (
            ...
            raise ValueError(
                "DebugUnderflowOverflow: aborting after"
                f" {self.batch_number} batches due to"

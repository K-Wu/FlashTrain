...
def build_processor(config_class, processor_class):
    ...
    if checkpoint is None:
        ...
        config_class_from_processor_class = (
            get_config_class_from_processor_class(processor_class)
        )
        checkpoint = get_checkpoint_from_config_class(
            config_class_from_processor_class
        )
    ...
    if (
        ...
        and issubclass(
            processor_class, (PreTrainedTokenizerBase, AutoTokenizer)
        )
    ):
        try:
        ...
        if config is not None:
            ...
            if tokenizer_class is not None:
                new_processor_class = getattr(
                    transformers_module, tokenizer_class
                )
                if new_processor_class != processor_class:
                    processor = build_processor(
                        config_class, new_processor_class
                    )
            ...
            if processor is None:
                ...
                new_processor_classes = [
                    ...
                    if x is not None
                    and x.__name__.replace("Fast", "") not in names
                ]
    ...
    if processor is None:
        ...
        if issubclass(processor_class, ProcessorMixin):
            ...
            for attr_name in processor_class.attributes:
                ...
                attr_class_names = getattr(
                    processor_class, f"{attr_name}_class"
                )
...
def get_tiny_config(config_class):
    ...
    try:
        ...
        module = importlib.import_module(
            f".models.{module_name}.test_modeling_{modeling_name}",
            package="tests",
        )
        ...
        error = (
            f"Tiny config not created for {model_type} - cannot find the"
            " testing module from the model name."
        )
        raise ValueError(f"{error}: {str(e)}")

    if model_tester_class is None:
        error = (
            f"Tiny config not created for {model_type} - no model tester is"
            " found in the testing module."
        )
    ...
    if hasattr(model_tester, "get_pipeline_config"):
        ...
        error = (
            f"Tiny config not created for {model_type} - the model tester"
            f" {model_tester_class.__name__} lacks necessary method to create"
            " config."
        )
        raise ValueError(error)
...
def convert_processors(processors, tiny_config, output_folder, result):
    ...
    assert (
        len(
            set([x.__class__.__name__.replace("Fast", "") for x in tokenizers])
        )
        < 2
    )
            ...
            if fast_tokenizer is None:
                ...
                try:
                    ...
                    result["warnings"].append(
                        "Failed to convert the fast tokenizer for"
                        f" {fast_tokenizer.__class__.__name__}: {e}"
                    )
                    continue
    ...
    if fast_tokenizer:
        try:
            ...
            result["warnings"].append(
                "Failed to save the fast tokenizer for"
                f" {fast_tokenizer.__class__.__name__}: {e}"
            )
            fast_tokenizer = None
    ...
    if fast_tokenizer:
        try:
            ...
            result["warnings"].append(
                "Failed to load the slow tokenizer saved from"
                f" {fast_tokenizer.__class__.__name__}: {e}"
            )
    ...
    if not fast_tokenizer and slow_tokenizer:
        try:
            ...
            result["warnings"].append(
                "Failed to save the slow tokenizer for"
                f" {slow_tokenizer.__class__.__name__}: {e}"
            )
            slow_tokenizer = None
    ...
    try:
        feature_extractors = [
            convert_feature_extractor(p, tiny_config)
            for p in feature_extractors
        ]
    except Exception as e:
...
def build_model(model_arch, tiny_config, output_dir):
    ...
    if os.path.isdir(processor_output_dir):
        shutil.copytree(
            processor_output_dir, checkpoint_dir, dirs_exist_ok=True
        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            ...
            encoder_path = os.path.join(
                encoder_output_dir, encoder_class.__name__
            )
            decoder_path = os.path.join(
                decoder_output_dir, decoder_class.__name__
            )
            ...
            if tf_model_class is not None:
                model = tf_model_class.from_pretrained(
                    model_path, from_pt=True
                )
                model.save_pretrained(model_path)

            # copy the processors
            encoder_processor_path = os.path.join(
                encoder_output_dir, "processors"
            )
            decoder_processor_path = os.path.join(
                decoder_output_dir, "processors"
            )
            if os.path.isdir(encoder_processor_path):
                shutil.copytree(
                    encoder_processor_path, model_path, dirs_exist_ok=True
                )
            if os.path.isdir(decoder_processor_path):
                shutil.copytree(
                    decoder_processor_path, model_path, dirs_exist_ok=True
                )

            # fill `result`
            result["processor"] = tuple(
                set(
                    [x.__name__ for x in encoder_processor + decoder_processor]
                )
            )
            ...
            result[
                "error"
            ] = f"Failed to build models for {config_class.__name__}: {e}"
...
def build(config_class, models_to_create, output_dir):
    ...
    if len(processor_classes) == 0:
        error = (
            f"No processor class could be found in {config_class.__name__}."
        )
    ...
    result["processor"] = {
        type(p).__name__: p.__class__.__name__ for p in processors
    }
    ...
    config_overrides = {
        ...
        if k in ["vocab_size"] and v is not None
    }
    # Update `vocab_size`
    ...
    for pytorch_arch in models_to_create["pytorch"]:
        ...
        try:
            model = build_model(
                pytorch_arch, tiny_config, output_dir=output_dir
            )
        except Exception as e:
            model = None
            error = (
                f"Failed to create the pytorch model for {pytorch_arch}: {e}"
            )
        ...
        result["pytorch"][pytorch_arch.__name__]["checkpoint"] = (
            get_checkpoint_dir(output_dir, pytorch_arch)
    ...
    for tensorflow_arch in models_to_create["tensorflow"]:
        ...
        if (
            ...
            try:
                ...
                model = None
                error = (
                    "Failed to convert the pytorch model to the tensorflow"
                    f" model for {pt_arch}: {e}"
                )
        else:
            try:
                model = build_model(
                    tensorflow_arch, tiny_config, output_dir=output_dir
                )
            except Exception as e:
                model = None
                error = (
                    "Failed to create the tensorflow model for"
                    f" {tensorflow_arch}: {e}"
                )
...
def build_failed_report(results, include_warning=True):
    ...
    for config_name in results:
        if "error" in results[config_name]:
            ...
            failed_results[config_name] = {
                "error": results[config_name]["error"]
            }

        if include_warning and "warnings" in results[config_name]:
            if config_name not in failed_results:
                failed_results[config_name] = {}
            failed_results[config_name]["warnings"] = results[config_name][
                "warnings"
            ]
...
def build_simple_report(results):
    ...
    for config_name in results:
        for framework in FRAMEWORKS:
            ...
            for arch_name in results[config_name][framework]:
                if "error" in results[config_name][framework][arch_name]:
                    result = results[config_name][framework][arch_name][
                        "error"
                    ]
...
if __name__ == "__main__":
    ...
    if os.getcwd() != clone_path:
        raise ValueError(
            "This script should be run from the root of the clone of"
            f" `transformers` {clone_path}"
        )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If set, will not check the validity of architectures. Use with"
            " caution."
        ),
    )
    parser.add_argument(
        ...
        type=list_str,
        help=(
            "Comma-separated list of model type(s) from which the tiny models"
            " will be created."
        ),
    )
    parser.add_argument(
        "output_path",
        type=Path,
        help="Path indicating where to store generated model.",
    )
    ...
    if not args.all and not args.model_types:
        raise ValueError(
            "Please provide at least one model type or pass `--all` to export"
            " all architectures."
        )
    ...
    if not args.all:
        config_classes = [
            CONFIG_MAPPING[model_type] for model_type in args.model_types
        ]
    ...
    to_create = {
        c: {
            ...
            "pytorch": get_architectures_from_config_class(
                c, pytorch_arch_mappings
            ),

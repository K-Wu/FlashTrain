...
class Message:
    def __init__(
        ...
        self.n_model_success = sum(
            r["success"] for r in model_results.values()
        )
        ...
        self.n_model_unknown_failures = sum(
            dicts_to_sum(r["failed"])["unclassified"]
        ...
        self.n_additional_single_gpu_failures = all_additional_failures[
            "single"
        ]
        self.n_additional_multi_gpu_failures = all_additional_failures["multi"]
        self.n_additional_unknown_gpu_failures = all_additional_failures[
            "unclassified"
        ]
    ...
    def time(self) -> str:
        all_results = [
            *self.model_results.values(),
            *self.additional_results.values(),
        ]
        time_spent = [
            r["time_spent"].split(", ")[0]
    ...
    def ci_title_section(self) -> Dict:
        return {
            "type": "section",
            "text": {"type": "mrkdwn", "text": self.ci_title},
        }
    ...
    def no_failures(self) -> Dict:
        return {
            ...
            "text": {
                "type": "plain_text",
                "text": (
                    f"🌞 There were no failures: all {self.n_tests} tests"
                    f" passed. The suite ran in {self.time}."
                ),
                "emoji": True,
            },
            "accessory": {
    ...
    def failures(self) -> Dict:
        return {
            ...
            "text": {
                "type": "plain_text",
                "text": (
                    f"There were {self.n_failures} failures, out of"
    ...
    def get_device_report(report, rjust=6):
        if "single" in report and "multi" in report:
            return (
                f"{str(report['single']).rjust(rjust)} |"
                f" {str(report['multi']).rjust(rjust)} | "
            )
        elif "single" in report:
            return (
                f"{str(report['single']).rjust(rjust)} | {'0'.rjust(rjust)} | "
            )
        elif "multi" in report:
            return (
                f"{'0'.rjust(rjust)} | {str(report['multi']).rjust(rjust)} | "
            )
        ...
        for model_failure in model_failures:
            for key, value in model_failure.items():
                if key not in category_failures:
                    ...
                    category_failures[key]["unclassified"] += value[
                        "unclassified"
                    ]
    ...
    def model_failures(self) -> Dict:
        ...
        model_header = (
            "Single PT |  Multi PT | Single TF |  Multi TF |     Other |"
            " Category\n"
        )
        sorted_model_reports = sorted(
            model_reports, key=lambda s: s.split("] ")[-1]
        )
    ...
    def additional_failures(self) -> Dict:
        ...
        return {
            "type": "section",
            "text": {"type": "mrkdwn", "text": failures_report},
        }
            ...
            if len(job_result["failures"]):
                for device, failures in job_result["failures"].items():
                    blocks = self.get_reply_blocks(
                        ...
                        text=(
                            "Number of failures:"
                            f" {sum(job_result['failed'].values())}"
                        ),
                    )
...
def get_job_links():
    ...
    try:
        ...
        for i in range(pages_to_iterate_over):
            ...
            jobs.update(
                {job["name"]: job["html_url"] for job in result["jobs"]}
            )
    ...
    if os.path.exists(name):
        ...
        for file in files:
            try:
                ...
                raise ValueError(
                    f"Could not open {os.path.join(name, file)}."
                ) from e
...
def retrieve_available_artifacts():
    ...
    for directory in directories:
        if directory.startswith("single-gpu"):
            ...
            _available_artifacts[artifact_name].add_path(
                directory, gpu="single"
            )
...
if __name__ == "__main__":
    ...
    runner_not_available = (
        True
        if runner_status is not None and runner_status != "success"
        else False
    )
    runner_failed = (
        True
    ...
    setup_failed = (
        True
        if setup_status is not None and setup_status != "success"
        else False
    )
    ...
    if ci_title is not None:
        if ci_url is None:
            raise ValueError(
                "When a title is found (`ci_title`), it means a `push` event"
                " or a `workflow_run` even (triggered by another `push`"
                " event), and the commit SHA has to be provided in order to"
                " create the URL to the commit page."
            )
    ...
    for model in model_results.keys():
        for artifact_path in available_artifacts[
            ...
            artifact = retrieve_artifact(
                artifact_path["name"], artifact_path["gpu"]
            )
            if "stats" in artifact:
                # Link to the GitHub Action job
                # The job names use `matrix.folder` which contain things like `models/bert` instead of `models_bert`
                job_name = (
                    f"Model tests ({model.replace('models_', 'models/')},"
                    f" {artifact_path['gpu']}-gpu)"
                )
                if job_name_prefix:
                ...
                failed, success, time_spent = handle_test_results(
                    artifact["stats"]
                )
                ...
                for line in artifact["summary_short"].split("\n"):
                    if re.search("FAILED", line):
                        ...
                        if (
                            artifact_path["gpu"]
                            not in model_results[model]["failures"]
                        ):

...
LOCALIZED_READMES = {
    ...
    "README.md": {
        ...
        "format_model_list": (
            "**[{title}]({model_link})** (from {paper_affiliations}) released"
            " with the paper {paper_title_link} by"
    ...
    "README_zh-hans.md": {
        ...
        "format_model_list": (
            "**[{title}]({model_link})** (来自 {paper_affiliations}) 伴随论文"
            " {paper_title_link} 由 {paper_authors} 发布。{supplements}"
        ),
    },
    "README_zh-hant.md": {
    ...
    "README_es.md": {
        "start_prompt": (
            "🤗 Transformers actualmente proporciona las siguientes"
            " arquitecturas"
        ),
...
def find_code_in_transformers(object_name):
    ...
    if i >= len(parts):
        raise ValueError(
            "`object_name` should begin with the name of a module of"
            f" transformers but got {object_name}."
        )
    ...
    for name in parts[i + 1 :]:
        while (
            ...
            and re.search(
                rf"^{indent}(class|def)\s+{name}(\(|\:)", lines[line_index]
            )
            is None
        ):
            line_index += 1
    ...
    while line_index < len(lines) and _should_continue(
        lines[line_index], indent
    ):
        line_index += 1
...
def is_copy_consistent(filename, overwrite=False):
    ...
    while line_index < len(lines):
        ...
        start_index = (
            line_index + 1 if indent == theoretical_indent else line_index + 2
        )
        ...
        if len(replace_pattern) > 0:
            ...
            theoretical_code = blackify(
                lines[start_index - 1] + theoretical_code
            )
...
def check_copies(overwrite: bool = False):
    ...
    if not overwrite and len(diffs) > 0:
        ...
        raise Exception(
            ...
            + diff
            + "\nRun `make fix-copies` or `python utils/check_copies.py"
            " --fix_and_overwrite` to fix them."
        )
        ...
        if source_code != target_code:
            if overwrite:
                with open(target, "w", encoding="utf-8") as f:
                    print(
                        f"Replacing the content of {target} by the one of"
                        f" {source}."
                    )
                    f.write(source_code)
            else:
...
def convert_to_localized_md(model_list, localized_model_list, format_str):
    ...
    readmes_match = not any(
        [k not in model_keys for k in localized_model_index]
    )
    localized_model_index = {
    ...
    for model in model_list.strip().split("\n"):
        ...
        if title not in localized_model_index:
            ...
            localized_model_index[title] = _re_capture_meta.sub(
                _rep, model + " "
            )
            ...
            localized_model_index[title] = _re_capture_title_link.sub(
                f"**[{title}]({model_link})**",
                localized_model_index[title],
                count=1,
            )

    sorted_index = sorted(
        localized_model_index.items(), key=lambda x: x[0].lower()
    )
...
def check_model_list_copy(overwrite=False, max_per_line=119):
    ...
    if new_readme != readme:
        if overwrite:
            ...
            raise ValueError(
                "The main README contains wrong links to the documentation of"
                " Transformers. Run `make fix-copies` to automatically fix"
                " them."
            )
    ...
    for filename, value in LOCALIZED_READMES.items():
        ...
        localized_md_list = get_model_list(
            filename, _start_prompt, _end_prompt
        )
    ...
    if converted_md_list != index_list:
        if overwrite:
            ...
            raise ValueError(
                "The model list in the README changed and the list in"
                " `index.mdx` has not been updated. Run `make fix-copies` to"
                " fix this."
            )
    ...
    for converted_md_list in converted_md_lists:
        ...
        if overwrite:
            ...
            with open(
                ...
                f.writelines(
                    lines[:start_index] + [converted_md] + lines[end_index:]
                )
        elif not readmes_match:
            raise ValueError(
                "The model list in the README changed and the list in"
                f" `{filename}` has not been updated. Run `make fix-copies` to"
                " fix this."
            )
...
README_TEMPLATE = (
    "1. **[{model_name}](https://huggingface.co/docs/main/transformers/model_doc/{model_type})**"
    " (from <FILL INSTITUTION>) released with the paper [<FILL PAPER"
    " TITLE>](<FILL ARKIV LINK>) by <FILL AUTHORS>."
)
...
def check_readme(overwrite=False):
    ...
    absents = [
        (key, name)
        for key, name in absents
        if name not in MODELS_NOT_IN_README
    ]
    if len(absents) > 0 and not overwrite:
        print(absents)
        raise ValueError(
            "The main README doesn't contain all models, run `make fix-copies`"
            " to fill it with the missing model(s) then complete the generated"
            " entries.\nIf the model is not supposed to be in the main README,"
            " add it to the list `MODELS_NOT_IN_README` in"
            " utils/check_copies.py.\nIf it has a different name in the repo"
            " than in the README, map the correspondence in"
            " `SPECIAL_MODEL_NAMES` in utils/check_copies.py."
        )

    new_models = [
        README_TEMPLATE.format(model_name=name, model_type=key)
        for key, name in absents
    ]
    ...
    all_models = sorted(
        all_models,
        key=lambda x: re.search(r"\*\*\[([^\]]*)", x).groups()[0].lower(),
    )
    all_models = "\n".join(all_models) + "\n"
    ...
    if all_models != models:
        if overwrite:
            ...
            with open(
                ...
                f.writelines(
                    lines[:start_index] + [all_models] + lines[end_index:]
                )
        else:
            raise ValueError(
                "The main README model list is not properly sorted. Run `make"
                " fix-copies` to fix this."
            )

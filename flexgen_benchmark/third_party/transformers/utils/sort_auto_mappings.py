...
_re_intro_mapping = re.compile(
    "[A-Z_]+_MAPPING(\s+|_[A-Z_]+\s+)=\s+OrderedDict"
)
...
def sort_auto_mapping(fname, overwrite: bool = False):
    ...
    while line_idx < len(lines):
        if _re_intro_mapping.search(lines[line_idx]) is not None:
            indent = (
                len(re.search(r"^(\s*)\S", lines[line_idx]).groups()[0]) + 8
            )
            ...
            blocks = sorted(
                blocks, key=lambda x: _re_identifier.search(x).groups()[0]
            )
            new_lines += blocks
        else:
            new_lines.append(lines[line_idx])
...
def sort_all_auto_mappings(overwrite: bool = False):
    ...
    if not overwrite and any(diffs):
        ...
        raise ValueError(
            "The following files have auto mappings that need sorting:"
            f" {', '.join(failures)}. Run `make style` to fix this."
        )

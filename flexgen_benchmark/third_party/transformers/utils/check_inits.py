...
_re_one_line_import_struct = re.compile(
    r"^_import_structure\s+=\s+\{([^\}]+)\}"
)
        ...
        if backend is not None:
            ...
            while len(lines[line_index]) <= 1 or lines[line_index].startswith(
                ...
                if _re_import_struct_add_one.search(line) is not None:
                    objects.append(
                        _re_import_struct_add_one.search(line).groups()[0]
                    )
                elif _re_import_struct_add_many.search(line) is not None:
                    imports = (
                        _re_import_struct_add_many.search(line)
                        .groups()[0]
                        .split(", ")
                    )
                    ...
                    imports = (
                        _re_between_brackets.search(line)
                        .groups()[0]
                        .split(", ")
                    )
                    imports = [obj[1:-1] for obj in imports if len(obj) > 0]
...
def analyze_results(import_dict_objects, type_hint_objects):
    ...
    for key in import_dict_objects.keys():
        ...
        if duplicate_imports:
            errors.append(
                "Duplicate _import_structure definitions for:"
                f" {duplicate_imports}"
            )
        ...
        if sorted(set(import_dict_objects[key])) != sorted(
            set(type_hint_objects[key])
        ):
    ...
    for root, _, files in os.walk(PATH_TO_TRANSFORMERS):
        if "__init__.py" in files:
            ...
            if objects is not None:
                ...
                if len(errors) > 0:
                    errors[0] = (
                        f"Problem in {fname}, both halves do not define the"
                        f" same objects.\n{errors[0]}"
                    )
                    failures.append("\n".join(errors))
    if len(failures) > 0:
    ...
    for path, directories, files in os.walk(PATH_TO_TRANSFORMERS):
        for folder in directories:
            ...
            if len(list((Path(path) / folder).glob("*.py"))) == 0:
                continue
            short_path = str(
                (Path(path) / folder).relative_to(PATH_TO_TRANSFORMERS)
            )
        ...
        for fname in files:
            if fname == "__init__.py":
                continue
            short_path = str(
                (Path(path) / fname).relative_to(PATH_TO_TRANSFORMERS)
            )
...
def check_submodules():
    ...
    if len(module_not_registered) > 0:
        list_of_modules = "\n".join(
            f"- {module}" for module in module_not_registered
        )
        raise ValueError(
            "The following submodules are not properly registed in the main"
            f" init of Transformers:\n{list_of_modules}\nMake sure they appear"
            " somewhere in the keys of `_import_structure` with an empty list"
            " as value."
        )

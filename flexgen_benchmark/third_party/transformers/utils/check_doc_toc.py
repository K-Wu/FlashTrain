    ...
    for duplicate_key in duplicates:
        titles = list(
            set(
                doc["title"]
                for doc in model_doc
                if doc["local"] == duplicate_key
            )
        )
        if len(titles) > 1:
            raise ValueError(
                f"{duplicate_key} is present several times in the"
                " documentation table of content at"
                " `docs/source/en/_toctree.yml` with different *Title*"
                " values. Choose one of those and remove the others."
            )
        # Only add this once
...
def check_model_doc(overwrite=False):
    ...
    modalities_docs = [
        (idx, section)
        for idx, section in enumerate(model_doc)
    ...
    if diff:
        if overwrite:
            ...
            raise ValueError(
                "The model doc part of the table of content is not properly"
                " sorted, run `make style` to fix this."
            )

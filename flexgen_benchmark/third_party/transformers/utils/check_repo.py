...
def check_model_list():
    ...
    if missing_models:
        raise Exception(
            ...
            f" {models_dir}/__init__.py: {','.join(missing_models)}."
        )
...
def check_models_are_in_init():
    ...
    if len(models_not_in_init) > 0:
        raise Exception(
            "The following models should be in the main init:"
            f" {','.join(models_not_in_init)}."
        )
...
def find_tested_models(test_file):
    ...
    with open(
        os.path.join(PATH_TO_TESTS, test_file),
        "r",
        encoding="utf-8",
        newline="\n",
    ) as f:
...
def check_models_are_tested(module, test_file):
    ...
    if tested_models is None:
        if (
            test_file.replace(os.path.sep, "/")
            in TEST_FILES_WITH_NO_COMMON_TESTS
        ):
            return
        return [
            f"{test_file} should define `all_model_classes` to apply common"
            " tests to the models it tests. "
            + "If this intentional, add the test filename to"
            " `TEST_FILES_WITH_NO_COMMON_TESTS` in the file "
            + "`utils/check_repo.py`."
        ]
    failures = []
    for model_name, _ in defined_models:
        if (
            model_name not in tested_models
            and model_name not in IGNORE_NON_TESTED
        ):
            failures.append(
                f"{model_name} is defined in {module.__name__} but is not"
                " tested in "
                + f"{os.path.join(PATH_TO_TESTS, test_file)}. Add it to the"
                " all_model_classes in that file."
                + "If common tests should not applied to that model, add its"
                " name to `IGNORE_NON_TESTED`"
...
def check_all_models_are_tested():
    ...
    for module in modules:
        ...
        if len(test_file) == 0:
            failures.append(
                f"{module.__name__} does not have its corresponding test file"
                f" {test_file}."
            )
        elif len(test_file) > 1:
            failures.append(
                f"{module.__name__} has several test files: {test_file}."
            )
        else:
        ...
        for attr_name in dir(transformers.models.auto.modeling_auto):
            if attr_name.startswith("MODEL_") and attr_name.endswith(
                ...
                result = result | set(
                    get_values(
                        getattr(
                            transformers.models.auto.modeling_auto, attr_name
                        )
                    )
                )
    if is_tf_available():
...
def check_models_are_auto_configured(module, all_auto_models):
    ...
    for model_name, _ in defined_models:
        if model_name not in all_auto_models and not ignore_unautoclassed(
            ...
            failures.append(
                f"{model_name} is defined in {module.__name__} but is not"
                " present in any of the auto mapping. If that is intended"
                " behavior, add its name to `IGNORE_NON_AUTO_CONFIGURED` in"
                " the file `utils/check_repo.py`."
            )
    return failures
...
def check_all_models_are_auto_configured():
    ...
    if len(missing_backends) > 0:
        ...
        if os.getenv("TRANSFORMERS_IS_CI", "").upper() in ENV_VARS_TRUE_VALUES:
            raise Exception(
                "Full quality checks require all backends to be installed"
                " (with `pip install -e .[dev]` in the Transformers repo, the"
            ...
            warnings.warn(
                "Full quality checks require all backends to be installed"
                " (with `pip install -e .[dev]` in the Transformers repo, the"
                f" following are missing: {missing}. While it's probably fine"
                " as long as you didn't make any change in one of those"
                " backends modeling files, you should probably execute the"
...
def check_all_decorator_order():
    ...
    if len(errors) > 0:
        msg = "\n".join(errors)
        raise ValueError(
            "The parameterized decorator (and its variants) should always be"
            f" first, but this is not the case in the following files:\n{msg}"
        )
...
def ignore_undocumented(name):
    ...
    if os.path.isdir(
        os.path.join(PATH_TO_TRANSFORMERS, name)
    ) or os.path.isfile(os.path.join(PATH_TO_TRANSFORMERS, f"{name}.py")):
        return True
...
def check_all_objects_are_documented():
    ...
    if len(undocumented_objs) > 0:
        raise Exception(
            "The following objects are in the public init so should be"
            " documented:\n - "
...
def check_model_type_doc_match():
    ...
    if len(errors) > 0:
        raise ValueError(
            ...
            + "\n".join(errors)
            + "\nYou can add any missing model type to the"
            " `MODEL_NAMES_MAPPING` constant in"
...
def check_docstrings_are_in_md():
    ...
    if len(files_with_rst) > 0:
        raise ValueError(
            ...
            + "\nTo fix this run `doc-builder convert path_to_py_file` after"
            " installing `doc-builder`\n(`pip install"
            " git+https://github.com/huggingface/doc-builder`)"
        )

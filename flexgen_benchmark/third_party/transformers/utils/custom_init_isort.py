...
def sort_objects(objects, key=None):
    ...
    classes = [
        obj
        for obj in objects
        if key(obj)[0].isupper() and not key(obj).isupper()
    ]
...
def sort_objects_in_import(import_statement):
    ...
    if len(lines) > 3:
        ...
        if _re_bracket_content.search(lines[1]) is not None:
            ...
            keys = [
                part.strip().replace('"', "") for part in lines[1].split(",")
            ]
            # We will have a final empty element if the line finished with a comma.
            if len(keys[-1]) == 0:
                keys = keys[:-1]
...
def sort_imports(file, check_only=True):
    ...
    main_blocks = split_code_in_indented_blocks(
        code,
        start_prompt="_import_structure = {",
        end_prompt="if TYPE_CHECKING:",
    )
    ...
    for block_idx in range(1, len(main_blocks) - 1):
        ...
        keys_to_sort = [
            (i, key) for i, key in enumerate(keys) if key is not None
        ]
        sorted_indices = [
            x[0] for x in sorted(keys_to_sort, key=lambda x: x[1])
        ]
        ...
        for i in range(len(internal_blocks)):
            if keys[i] is None:
                ...
                block = sort_objects_in_import(
                    internal_blocks[sorted_indices[count]]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--check_only",
        action="store_true",
        help="Whether to only check or fix style.",
    )

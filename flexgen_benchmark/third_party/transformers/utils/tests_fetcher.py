...
def get_all_tests():
    ...
    model_test_folders = sorted(
        list(
            filter(
                os.path.isdir,
                [f"tests/models/{x}" for x in model_tests_folders],
            )
        )
    )
...
def get_diff(repo, base_commit, commits):
    ...
    for commit in commits:
        for diff_obj in commit.diff(base_commit):
            ...
            elif diff_obj.change_type == "D" and diff_obj.a_path.endswith(
                ".py"
            ):
                code_diff.append(diff_obj.a_path)
            # Now for modified files
            elif diff_obj.change_type in [
                "M",
                "R",
                ...
                if diff_obj.a_path != diff_obj.b_path:
                    ...
                    if diff_is_docstring_only(repo, commit, diff_obj.b_path):
                        print(
                            f"Ignoring diff in {diff_obj.b_path} as it only"
                            " concerns docstrings or comments."
                        )
                    else:
                        code_diff.append(diff_obj.a_path)
...
def get_module_dependencies(module_fname):
    ...
    direct_imports = [
        mod for mod, imp in direct_imports if "# tests_ignore" not in imp
    ]
    ...
    for imported_module in imported_modules:
        if os.path.isfile(
            os.path.join(PATH_TO_TRANFORMERS, f"{imported_module}.py")
        ):
            ...
            os.path.sep.join(
                [PATH_TO_TRANFORMERS, imported_module, "__init__.py"]
            )
        ):
            dependencies.append(
                os.path.sep.join([imported_module, "__init__.py"])
            )
    return dependencies
...
def get_test_dependencies(test_fname):
    ...
    return [
        ...
        if os.path.isfile(os.path.join(PATH_TO_TRANFORMERS, f))
    ]
...
def create_reverse_dependency_tree():
    ...
    modules = [
        ...
        for f in (Path(PATH_TO_TRANFORMERS) / "src/transformers").glob(
            "**/*.py"
        )
    ]
    module_edges = [
        (d, m) for m in modules for d in get_module_dependencies(m)
    ]
...
def get_tree_starting_at(module, edges):
    ...
    new_edges = [
        edge for edge in edges if edge[0] == module and edge[1] != module
    ]
    tree = [module]
...
SPECIAL_MODULE_TO_TEST_MAP = {
    ...
    "models/auto/modeling_flax_auto.py": (
        "models/auto/test_modeling_flax_auto.py"
    ),
    ...
    "pipelines/zero_shot_classification.py": (
        "pipelines/test_pipelines_zero_shot.py"
    ),
...
def sanity_check():
    ...
    all_files = [
        ...
        for p in (Path(PATH_TO_TRANFORMERS) / "src/transformers").glob(
            "**/*.py"
        )
    ]
    all_files += [
    ...
    for test_f in test_files_found:
        if os.path.isdir(os.path.join(PATH_TO_TRANFORMERS, test_f)):
            test_files.extend(
                [
                    str(p.relative_to(PATH_TO_TRANFORMERS))
                    for p in (Path(PATH_TO_TRANFORMERS) / test_f).glob(
                        "**/test*.py"
                    )
                ]
            )
    ...
    not_touched_test_files = [
        f for f in existing_test_files if f not in test_files
    ]
    ...
    if len(should_be_tested) > 0:
        raise ValueError(
            "The following test files are not currently associated with any"
            " module or utils files, which means they will never get run by"
            f" the CI:\n{_print_list(should_be_tested)}\n. Make sure the names"
            " of these test files match the name of the module or utils they"
            " are testing, or adapt the constant `SPECIAL_MODULE_TO_TEST_MAP`"
            " in `utils/tests_fetcher.py` to add them. If your test file is"
            " triggered separately and is not supposed to be run by the"
            " regular CI, add it to the `EXPECTED_TEST_FILES_NEVER_TOUCHED`"
            " constant instead."
        )
...
def filter_tests(output_file, filters):
    ...
    if test_files == ["tests"]:
        ...
        test_files = [
            f for f in test_files if f.split(os.path.sep)[1] not in filters
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            "Where to store the tests to run in a dictionary format mapping"
            " test categories to test files"
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "To fetch the tests between the current commit and the last commit"
        ),
    )
    parser.add_argument(
        "--filters",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Will filter the pipeline/repo utils tests outside of the"
            " generated list of tests."
        ),
    )
    parser.add_argument(
        ...
        type=str,
        help=(
            "Will only print the tree of modules depending on the file passed."
        ),
        default=None,
    )

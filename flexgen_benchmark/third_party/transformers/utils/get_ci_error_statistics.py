...
def download_artifact(artifact_name, artifact_url, output_dir, token):
    ...
    cmd = (
        'curl -v -H "Accept: application/vnd.github+json" -H "Authorization:'
        f' token {token}" {artifact_url}'
    )
...
def get_errors_from_single_artifact(artifact_zip_path, job_links=None):
    ...
    if len(errors) != len(failed_tests):
        raise ValueError(
            "`errors` and `failed_tests` should have the same number of"
            f" elements. Got {len(errors)} for `errors` and"
            f" {len(failed_tests)} for `failed_tests` instead. The test"
            f" reports in {artifact_zip_path} have some problem."
        )
...
def reduce_by_error(logs, error_filter=None):
    ...
    r = dict(
        sorted(r.items(), key=lambda item: item[1]["count"], reverse=True)
    )
    return r
...
if __name__ == "__main__":
    ...
    with open(
        os.path.join(args.output_dir, "reduced_by_error.txt"),
        "w",
        encoding="UTF-8",
    ) as fp:
        fp.write(s1)
    with open(
        os.path.join(args.output_dir, "reduced_by_model.txt"),
        "w",
        encoding="UTF-8",
    ) as fp:
        fp.write(s2)

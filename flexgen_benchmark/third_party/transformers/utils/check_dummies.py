...
def create_dummy_files(backend_specific_objects=None):
    ...
    for backend, objects in backend_specific_objects.items():
        backend_name = (
            "[" + ", ".join(f'"{b}"' for b in backend.split("_and_")) + "]"
        )
        dummy_file = (
            "# This file is autogenerated by the command `make fix-copies`, do"
            " not edit.\n"
        )
        dummy_file += "# flake8: noqa\n"
        dummy_file += "from ..utils import DummyObject, requires_backends\n\n"
        dummy_file += "\n".join(
            [create_dummy_object(o, backend_name) for o in objects]
        )
        ...
        if dummy_files[backend] != actual_dummies[backend]:
            if overwrite:
                print(
                    ...
                    " as the main __init__ has new objects."
                )
                with open(
                    dummy_file_paths[backend],
                    "w",
                    encoding="utf-8",
                    newline="\n",
                ) as f:

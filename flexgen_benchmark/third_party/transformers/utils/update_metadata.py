...
def get_frameworks_table():
    ...
    for t in all_models:
        if (
            t
            in transformers_module.models.auto.processing_auto.PROCESSOR_MAPPING_NAMES
        ):
...
def update_pipeline_and_auto_class_table(table):
    ...
    for (
        ...
        model_mapping,
        auto_class,
    ) in PIPELINE_TAGS_AND_AUTO_MODELS:
        model_mappings = [
            model_mapping,
            f"TF_{model_mapping}",
            f"FLAX_{model_mapping}",
        ]
        auto_classes = [auto_class, f"TF_{auto_class}", f"Flax_{auto_class}"]
        # Loop through all three frameworks
        for module, cls, mapping in zip(
            auto_modules, auto_classes, model_mappings
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--commit_sha",
        type=str,
        help="The sha of the commit going with this update.",
    )
    parser.add_argument(
        "--check-only",
        action="store_true",

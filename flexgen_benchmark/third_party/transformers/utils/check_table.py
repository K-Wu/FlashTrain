    ...
    if current_table != new_table:
        if overwrite:
            with open(
                ...
                f.writelines(
                    lines[:start_index] + [new_table] + lines[end_index:]
                )
        else:
            raise ValueError(
                "The model table in the `index.mdx` has not been updated. Run"
...
def has_onnx(model_type):
    ...
    """
    config_mapping = (
        transformers_module.models.auto.configuration_auto.CONFIG_MAPPING
    )
...
def get_onnx_model_list():
    ...
    onnx_model_types = [
        model_type
        for model_type in config_mapping.keys()
        if has_onnx(model_type)
    ]
    onnx_model_names = [
        model_names[model_type] for model_type in onnx_model_types
    ]
...
def check_onnx_model_list(overwrite=False):
    ...
    current_list, start_index, end_index, lines = _find_text_in_file(
        ...
        start_prompt=(
            "<!--This table is automatically generated by `make fix-copies`,"
    ...
    if current_list != new_list:
        if overwrite:
            with open(
                ...
                f.writelines(
                    lines[:start_index] + [new_list] + lines[end_index:]
                )
        else:
            raise ValueError(
                "The list of ONNX-supported models needs an update. Run `make"
